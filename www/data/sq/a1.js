const A1_WORDS = [
  {
    "de": "Apfel",
    "de_article": "der",
    "de_plural": "die Äpfel",
    "lv": "MollaMollë",
    "level": "A1"
  },
  {
    "de": "Brot",
    "de_article": "das",
    "de_plural": "die Brote",
    "lv": "Buka",
    "level": "A1"
  },
  {
    "de": "Wasser",
    "de_article": "das",
    "lv": "Uji",
    "level": "A1"
  },
  {
    "de": "Haus",
    "de_article": "das",
    "de_plural": "die Häuser",
    "lv": "Për në shtëpi.",
    "level": "A1"
  },
  {
    "de": "lernen",
    "lv": "Të mësoj?",
    "level": "A1"
  },
  {
    "de": "sprechen",
    "lv": "Thuaj diçka.",
    "level": "A1",
    "study": {
      "id": "a1-sprechen-study",
      "layout": "standardStudy",
      "translation": "Thuaj diçka.",
      "explanation": [
        "Ideja kryesore: Flisni, flisni ose përdorni gjuhën.",
        "Sprechen në thelb do të thotë: të flasësh ose të flasësh.",
        "Kryesisht karakterizohet nga karakteristikat e mëposhtme: gjuha/të folurit.",
        "Sprechen përshkruan të folurit ose përdorimin e gjuhës."
      ],
      "examples": [
        {
          "de": "Ich spreche Deutsch.",
          "lv": "Unë flas gjermanisht."
        },
        {
          "de": "Wir sprechen über die Arbeit.",
          "lv": "Po flasim për punë."
        },
        {
          "de": "Sie spricht mit ihrer Lehrerin.",
          "lv": "Unë flas gjermanisht"
        }
      ],
      "comparison": [
        {
          "word": "sprechen",
          "meaning": "Flisni (procesi, gjuha)",
          "example": "Wir sprechen über die Arbeit. – Po flasim për punë."
        },
        {
          "word": "sagen",
          "meaning": "Trego (tekst specifik)",
          "example": "Sag mir die Wahrheit. – Duhet të më tregosh të vërtetën."
        }
      ],
      "tip": [
        "Sprechen = bisedë",
        "Përdor sprechen kur konteksti i përshtatet kuptimit."
      ],
      "important": [
        "Sprechen = duke folur.",
        "Flisni, flisni ose përdorni gjuhën."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "sprechen"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "spreche"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "sprechen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "spreche"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "sprechen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "klein",
    "lv": "I vogël",
    "level": "A1",
    "study": {
      "id": "a1-klein-study",
      "layout": "standardStudy",
      "translation": "I vogël",
      "explanation": [
        "Ideja kryesore: Të vogla në madhësi ose fushëveprim.",
        "Klein kryesisht do të thotë: madhësi e vogël.",
        "Shpesh përcakton: madhësinë e diçkaje/personi.",
        "Klein në thelb do të thotë: i vogël/i ri.",
        "Ai shpesh e përshkruan: një fëmijë.",
        "Klein përshkruan madhësinë e vogël."
      ],
      "examples": [
        {
          "de": "Das Zimmer ist klein.",
          "lv": "Dhoma është e vogël."
        },
        {
          "de": "Das Kind ist noch klein.",
          "lv": "Dhoma është e vogël."
        },
        {
          "de": "Ich habe eine kleine Tasche.",
          "lv": "Foshnja është ende e re."
        },
        {
          "de": "Ich habe eine kleine Tasche.",
          "lv": "Kam një çantë të vogël."
        },
        {
          "de": "Das Kind ist klein.",
          "lv": "Foshnja është e vogël."
        }
      ],
      "tip": [
        "Klein = i vogël",
        "Përdor ngjitës kur konteksti mbivendoset me kuptimin."
      ],
      "important": [
        "Klein = madhësi e vogël.",
        "Klein = i vogël.",
        "Madhësia e vogël ose hapësira."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "klein"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "klein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "klein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "klein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kleine"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "klein"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "klein"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "alle",
    "lv": "Të gjithë ata",
    "level": "A1"
  },
  {
    "de": "allein",
    "lv": "-Vetëm.",
    "level": "A1"
  },
  {
    "de": "alles",
    "lv": "Të gjitha",
    "level": "A1"
  },
  {
    "de": "alt",
    "lv": "I vjetëruar",
    "level": "A1"
  },
  {
    "de": "Alter",
    "de_article": "das",
    "lv": "Mosha?",
    "level": "A1"
  },
  {
    "de": "an",
    "lv": "Nga • Deri • Aktualisht",
    "level": "A1",
    "study": {
      "id": "a1-an",
      "layout": "standardStudy",
      "translation": "Hap • Në sipërfaqe • Në buzë",
      "explanation": "Përdoret kur diçka është pranë murit, dritares, derës, lumit, bregut të detit ose skajit/sipërfaqes tjetër.",
      "examples": [
        {
          "de": "an der Wand",
          "lv": "Në mur/në mur"
        },
        {
          "de": "am Fenster",
          "lv": "Pranë dritares"
        },
        {
          "de": "am Meer",
          "lv": "Pranë detit"
        }
      ],
      "comparison": [
        {
          "word": "an",
          "meaning": "Në sipërfaqe ose në buzë",
          "example": "an der Wand – Pranë murit"
        },
        {
          "word": "auf",
          "meaning": "Në një sipërfaqe horizontale",
          "example": "auf dem Tisch – Shishja është në tryezë."
        },
        {
          "word": "bei",
          "meaning": "Për një person ose vend",
          "example": "beim Arzt – Të lutem."
        }
      ],
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "an"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "am"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "am"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "an"
              ]
            },
            "example": {
              "green": [
                "an"
              ]
            }
          },
          {
            "word": {
              "green": [
                "auf"
              ]
            },
            "example": {
              "yellow": [
                "auf"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bei"
              ]
            },
            "example": {
              "red": [
                "beim"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "Momenti"
            ]
          },
          {
            "yellow": [
              "auf"
            ]
          }
        ]
      },
      "tip": {
        "text": "Shërbimet: Pie sienas/loga/malas → an."
      },
      "important": [
        "Momenti nuk është vetëm \"kali\". Kjo zakonisht do të thotë pranë një sipërfaqeje, muri, dritareje ose buze.",
        "Auf zakonisht përdoret në një sipërfaqe horizontale."
      ]
    }
  },
  {
    "de": "Anfang",
    "de_article": "der",
    "de_plural": "die Anfänge",
    "lv": "Fillimi",
    "level": "A1"
  },
  {
    "de": "anfangen",
    "lv": "Fillo!",
    "level": "A1"
  },
  {
    "de": "anders",
    "lv": "Përndryshe",
    "level": "A1"
  },
  {
    "de": "anrufen",
    "lv": "Kërko",
    "level": "A1"
  },
  {
    "de": "ab",
    "lv": "DHE",
    "level": "A1",
    "study": {
      "id": "a1-ab",
      "layout": "standardStudy",
      "translation": "DHE",
      "explanation": "Përdoret kur diçka fillon në një kohë, vend ose pikë specifike. Zakonisht do të thotë \"Duka më filloi\".",
      "examples": [
        {
          "de": "ab heute",
          "lv": "Që nga sot.",
          "level": "A1"
        },
        {
          "de": "ab Montag",
          "lv": "Nga e hëna"
        },
        {
          "de": "ab 8 Uhr",
          "lv": "Nga 8"
        },
        {
          "de": "ab Bahnhof",
          "lv": "Nga stacioni"
        }
      ],
      "comparison": [
        {
          "word": "ab",
          "meaning": "Duke filluar nga pika/ora",
          "example": "ab Montag – Nga e hëna"
        },
        {
          "word": "von",
          "meaning": "Nga dikush/diçka • Origjina",
          "example": "von mir – Ju qeras une."
        },
        {
          "word": "aus",
          "meaning": "Pjesa e jashtme e brendshme",
          "example": "aus dem Haus – Nga shtëpia/nga distanca"
        }
      ],
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "ab"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ab"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ab"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ab"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "ab"
              ]
            },
            "example": {
              "green": [
                "ab"
              ]
            }
          },
          {
            "word": {
              "green": [
                "von"
              ]
            },
            "example": {
              "yellow": [
                "von"
              ]
            }
          },
          {
            "word": {
              "green": [
                "aus"
              ]
            },
            "example": {
              "blue": [
                "aus"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "ab"
            ]
          },
          {
            "yellow": [
              "von"
            ],
            "red": [
              "aus"
            ]
          }
        ]
      },
      "tip": {
        "text": "Mos harroni: pika fillestare për sa i përket kohës/hapësirës është → përafërsisht."
      },
      "important": [
        "Ab tregon pikën e fillimit në kohë ose hapësirë.",
        "Von ose AUS përdoren më shpesh nëse mendimi vjen ose rrjedh nga brenda."
      ]
    }
  },
  {
    "de": "Abend",
    "de_article": "der",
    "de_plural": "die Abende",
    "lv": "Akşam",
    "level": "A1"
  },
  {
    "de": "Abendessen",
    "de_article": "das",
    "de_plural": "die Abendessen",
    "lv": "Po përgatis darkën.",
    "level": "A1"
  },
  {
    "de": "abends",
    "lv": "Natë",
    "level": "A1"
  },
  {
    "de": "aber",
    "lv": "Megjithatë...",
    "level": "A1",
    "study": {
      "id": "a1-aber",
      "layout": "standardStudy",
      "translation": "Megjithatë...",
      "explanation": "Përdoret për të krijuar kontrast ose mosmarrëveshje. Zakonisht do të thotë \"por \", \"por\" ose \"por\".",
      "examples": [
        {
          "de": "Ich möchte mitkommen, aber ich habe keine Zeit.",
          "lv": "Dua të vij, por nuk kam kohë."
        },
        {
          "de": "Das Essen war lecker, aber zu teuer.",
          "lv": "Ushqimi ishte i shijshëm, por shumë i shtrenjtë."
        },
        {
          "de": "Er hat recht, aber ich sehe das anders.",
          "lv": "Ka të drejtë, por nuk mendoj kështu."
        }
      ],
      "comparison": [
        {
          "word": "aber",
          "meaning": "Kundërshtuar • Apeluar • Megjithatë",
          "example": "Ich komme, aber später. – Por do të vij më vonë."
        },
        {
          "word": "sondern",
          "meaning": "Huh? Jo, por ...",
          "example": "Ich wollte keinen Tee, sondern Kaffee. – Doja çaj, jo kafe."
        },
        {
          "word": "jedoch",
          "meaning": "Por...",
          "example": "Es ist kalt, jedoch sonnig. – Është ftohtë, por me diell."
        }
      ],
      "tip": {
        "text": "Karakteristikat: statistikat paraprake/iebilde → aber."
      },
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "aber"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aber"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aber"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "aber"
              ]
            },
            "example": {
              "green": [
                "aber"
              ]
            }
          },
          {
            "word": {
              "green": [
                "sondern"
              ]
            },
            "example": {
              "yellow": [
                "sondern"
              ]
            }
          },
          {
            "word": {
              "green": [
                "jedoch"
              ]
            },
            "example": {
              "red": [
                "jedoch"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "green": [
              "aber"
            ]
          },
          {
            "yellow": [
              "sondern"
            ]
          }
        ]
      },
      "important": [
        "Aberi tregon të kundërtën ose anasjelltas.",
        "Kur e kundërta është \"jo... por...\", fjala gjermane zakonisht është sondern."
      ]
    }
  },
  {
    "de": "achten",
    "lv": "Ndiqi.",
    "level": "A1"
  },
  {
    "de": "Adresse",
    "de_article": "die",
    "de_plural": "die Adressen",
    "lv": "Vendbanimi",
    "level": "A1"
  },
  {
    "de": "Affe",
    "de_article": "der",
    "de_plural": "die Affen",
    "lv": "Majmun.",
    "level": "A1"
  },
  {
    "de": "Album",
    "de_article": "das",
    "de_plural": "die Alben",
    "lv": "Albumi",
    "level": "A1"
  },
  {
    "de": "also",
    "lv": "Në lidhje me",
    "level": "A1",
    "study": {
      "id": "a1-also",
      "layout": "standardStudy",
      "translation": "Në lidhje me",
      "explanation": "Përdoret për të vizatuar ose shfaqur rezultatin. \"Kjo është arsyeja\" do të thotë \"kjo është arsyeja \".",
      "examples": [
        {
          "de": "Es regnet, also bleibe ich zu Hause.",
          "lv": "Po bie shi, kështu që po qëndroj në shtëpi."
        },
        {
          "de": "Du bist krank, also gehst du nicht zur Arbeit.",
          "lv": "Je i sëmurë, prandaj mos shko në punë."
        },
        {
          "de": "Ich habe viel gelernt, also verstehe ich es jetzt.",
          "lv": "Kam punuar shumë, tani e kuptoj."
        }
      ],
      "comparison": [
        {
          "word": "also",
          "meaning": "Dhe si rrjedhojë...",
          "example": "Es regnet, also bleibe ich zu Hause. – Po bie shi, kështu që po qëndroj në shtëpi."
        },
        {
          "word": "auch",
          "meaning": "Por",
          "example": "Ich komme auch. – Edhe unë do të vij."
        },
        {
          "word": "deshalb",
          "meaning": "Në lidhje me",
          "example": "Es regnet, deshalb bleibe ich zu Hause. – Po bie shi, kështu që po qëndroj në shtëpi."
        }
      ],
      "tip": {
        "text": "Mos harroni: kështu → është edhe kërkesa."
      },
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "also"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "also"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "also"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "also"
              ]
            },
            "example": {
              "green": [
                "also"
              ]
            }
          },
          {
            "word": {
              "green": [
                "auch"
              ]
            },
            "example": {
              "yellow": [
                "auch"
              ]
            }
          },
          {
            "word": {
              "green": [
                "deshalb"
              ]
            },
            "example": {
              "green": [
                "deshalb"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {
            "green": [
              "deshalbe"
            ]
          }
        ]
      },
      "important": [
        "Ajo gjithashtu tregon konkluzionin e mëposhtëm: Mendimi tjetër rrjedh nga ajo që është thënë më lart.",
        "\"Po\" -ja letoneze shpesh mund të jetë gjithashtu një deshalbe."
      ]
    }
  },
  {
    "de": "Ameise",
    "de_article": "die",
    "de_plural": "die Ameisen",
    "lv": "Po, një milingonë.",
    "level": "A1"
  },
  {
    "de": "ankommen",
    "lv": "Mbërritja",
    "level": "A1"
  },
  {
    "de": "anschauen",
    "lv": "Vëzhgo.",
    "level": "A1"
  },
  {
    "de": "anziehen",
    "lv": "Açma",
    "level": "A1"
  },
  {
    "de": "anhalten",
    "lv": "Ndalo!",
    "level": "A1"
  },
  {
    "de": "Angst",
    "de_article": "die",
    "de_plural": "die Ängste",
    "lv": "Frikë.",
    "level": "A1"
  },
  {
    "de": "angenehm",
    "lv": "-Fare.",
    "level": "A1"
  },
  {
    "de": "Antenne",
    "de_article": "die",
    "de_plural": "die Antennen",
    "lv": "Antena",
    "level": "A1"
  },
  {
    "de": "Antwort",
    "de_article": "die",
    "de_plural": "die Antworten",
    "lv": "Reagimi?",
    "level": "A1"
  },
  {
    "de": "antworten",
    "lv": "Përgjigjja",
    "level": "A1"
  },
  {
    "de": "Anzug",
    "de_article": "der",
    "de_plural": "die Anzüge",
    "lv": "Po.",
    "level": "A1"
  },
  {
    "de": "Apfelbaum",
    "de_article": "der",
    "de_plural": "die Apfelbäume",
    "lv": "Pemë molle",
    "level": "A1"
  },
  {
    "de": "Apfelsine",
    "de_article": "die",
    "de_plural": "die Apfelsinen",
    "lv": "-Um... ngjyr portokalli. Portkalli?",
    "level": "A1"
  },
  {
    "de": "Aprikose",
    "de_article": "die",
    "de_plural": "die Aprikosen",
    "lv": "Kajsi",
    "level": "A1"
  },
  {
    "de": "April",
    "de_article": "der",
    "lv": "Martha Stewart perballoi shpenzimet...",
    "level": "A1"
  },
  {
    "de": "Arbeit",
    "de_article": "die",
    "de_plural": "die Arbeiten",
    "lv": "Biznes",
    "level": "A1"
  },
  {
    "de": "arbeiten",
    "lv": "Biznes",
    "level": "A1"
  },
  {
    "de": "Arm",
    "de_article": "der",
    "de_plural": "die Arme",
    "lv": "Dora",
    "level": "A1"
  },
  {
    "de": "Armbanduhr",
    "de_article": "die",
    "de_plural": "die Armbanduhren",
    "lv": "Orë dore",
    "level": "A1"
  },
  {
    "de": "Ärztin",
    "de_article": "die",
    "de_plural": "die Ärztinnen",
    "lv": "-A do e thuash prapë që lindin në mes të fushës me oriz?",
    "level": "A1"
  },
  {
    "de": "atmen",
    "lv": "Merr frymë.",
    "level": "A1"
  },
  {
    "de": "auch",
    "lv": "Por",
    "level": "A1",
    "study": {
      "id": "a1-auch-study",
      "layout": "standardStudy",
      "translation": "Por",
      "explanation": [
        "Ideja kryesore: “edhe” më e popullarizuar dhe e paanshme.",
        "Auch në thelb do të thotë: e thjeshtë \"shumë\".",
        "Zakonisht karakterizohet si: bashkim.",
        "Auch është fjala më e zakonshme për \"në të njëjtën kohë\"."
      ],
      "examples": [
        {
          "de": "Ich komme auch.",
          "lv": "Edhe unë do të vij."
        },
        {
          "de": "Sie arbeitet auch hier.",
          "lv": "Edhe unë do të vij."
        },
        {
          "de": "Ich wünsche Ihnen auch einen schönen Tag.",
          "lv": "Ai punon këtu?"
        }
      ],
      "tip": [
        "Auch = gjithashtu",
        "OUCH përdoret kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Ich auch wünsche Ihnen nuk është renditja e saktë e fjalëve.",
        "Ahh = në të njëjtën kohë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "auch"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "auch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "auch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "auch"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "auch"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "auf",
    "lv": "-Poshtë.",
    "level": "A1",
    "study": {
      "id": "a1-auf",
      "layout": "standardStudy",
      "translation": "-Poshtë.",
      "explanation": "Përdoret për të treguar një drejtim në një vend ose në majë të një sipërfaqeje.",
      "examples": [
        {
          "de": "Ich stelle das Buch auf den Tisch.",
          "lv": "E vura librin në tryezë."
        },
        {
          "de": "Wir fahren auf den Berg.",
          "lv": "Do të shkojmë në male."
        },
        {
          "de": "Die Katze springt auf das Sofa.",
          "lv": "Macja kërcen në divan."
        }
      ],
      "comparison": [
        {
          "word": "auf",
          "meaning": "Objektivi (sipërfaqja ose lart)",
          "example": "Ich stelle das Glas auf den Tisch. – E vendosa gotën në tavolinë."
        },
        {
          "word": "an",
          "meaning": "Na (sipërfaqe vertikale)",
          "example": "Ich hänge das Bild an die Wand. – Po e var një fotografi në mur."
        },
        {
          "word": "in",
          "meaning": "Është për ty.",
          "example": "Ich lege das Buch in die Tasche. – Librin e futa në çantë."
        }
      ],
      "tip": {
        "text": "Mos harroni: → auf në sipërfaqe/sipër."
      },
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "auf"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "auf"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "auf"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "auf"
              ]
            },
            "example": {
              "blue": [
                "auf"
              ]
            }
          },
          {
            "word": {
              "green": [
                "an"
              ]
            },
            "example": {
              "green": [
                "an"
              ]
            }
          },
          {
            "word": {
              "green": [
                "in"
              ]
            },
            "example": {
              "yellow": [
                "in"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "auf"
            ]
          },
          {}
        ]
      },
      "important": [
        "Auf nuk do të thotë vetëm \"on\". Kjo zakonisht do të thotë të lëvizësh ose të jesh në sipërfaqe.",
        "Nëse diçka është afër një sipërfaqeje vertikale, zakonisht të duhet • Nëse je, duhet të hysh brenda."
      ]
    }
  },
  {
    "de": "aufmachen",
    "lv": "Hapur.",
    "level": "A1"
  },
  {
    "de": "aufpassen",
    "lv": "Thjesht ki kujdes.",
    "level": "A1"
  },
  {
    "de": "aufstehen",
    "lv": "Zgjimi",
    "level": "A1"
  },
  {
    "de": "aufwärts",
    "lv": "Lart.",
    "level": "A1"
  },
  {
    "de": "Auge",
    "de_article": "das",
    "de_plural": "die Augen",
    "lv": "-Syri.",
    "level": "A1"
  },
  {
    "de": "Augenblick",
    "de_article": "der",
    "de_plural": "die Augenblicke",
    "lv": "Një sekondë",
    "level": "A1"
  },
  {
    "de": "August",
    "de_article": "der",
    "lv": "Gusht",
    "level": "A1"
  },
  {
    "de": "aus",
    "lv": "Z • Në ambiente të jashtme",
    "level": "A1",
    "study": {
      "id": "a1-aus",
      "layout": "standardStudy",
      "translation": "Z • Në ambiente të jashtme",
      "explanation": "Përdoret kur diçka vjen nga brenda, del ose tregon një origjinë.",
      "examples": [
        {
          "de": "Ich komme aus Deutschland.",
          "lv": "Jam nga Gjermania."
        },
        {
          "de": "Er geht aus dem Haus.",
          "lv": "Ajo po largohet nga shtëpia."
        },
        {
          "de": "Ich nehme das Buch aus der Tasche.",
          "lv": "E nxjerr librin nga çanta."
        }
      ],
      "comparison": [
        {
          "word": "aus",
          "meaning": "Nga brenda,",
          "example": "aus dem Haus – Nga shtëpia"
        },
        {
          "word": "von",
          "meaning": "Nga personi, nga toka, nga sipërfaqja",
          "example": "von meinem Freund – Nga miku im"
        },
        {
          "word": "ab",
          "meaning": "Duke filluar nga një pikë ose kohë",
          "example": "ab Montag – Nga e hëna"
        }
      ],
      "tip": {
        "text": "Mos harroni: → ndryshe nga e saja."
      },
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "green": [
                "aus"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "aus"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "aus"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "aus"
              ]
            },
            "example": {
              "green": [
                "aus"
              ]
            }
          },
          {
            "word": {
              "green": [
                "von"
              ]
            },
            "example": {
              "green": [
                "von"
              ]
            }
          },
          {
            "word": {
              "green": [
                "ab"
              ]
            },
            "example": {
              "yellow": [
                "ab"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "green": [
              "aus"
            ]
          },
          {
            "blue": [
              "ab"
            ]
          }
        ]
      },
      "important": [
        "AUS zakonisht tregon lëvizje nga brenda ose nga fillimi.",
        "Fjala ab përdoret shpesh vetëm kur i referohet një pike fillestare në kohë ose hapësirë."
      ]
    }
  },
  {
    "de": "auf dem Boden",
    "lv": "Në pistë.",
    "level": "A1"
  },
  {
    "de": "auf dem Bahnhof",
    "lv": "Në stacion",
    "level": "A1"
  },
  {
    "de": "aufs",
    "lv": "Ku • Ku • Ku?",
    "level": "A1",
    "study": {
      "id": "a1-aufs",
      "layout": "standardStudy",
      "translation": "Ku • Ku • Ku?",
      "explanation": [
        "Aufs është shkurtimi i parafjalës auf dhe substancës das.",
        "Formati i plotë: auf das (ku?).",
        "Përdoret kur një veprim tregon një drejtim drejt një gjëje ose sipërfaqeje specifike • I përgjigjet pyetjes \"Ku?\"",
        "Përdoret shpesh në lëvizje: ngjitja, ulja, rënia në tokë, afrimi i diçkaje.",
        "Në të folur dhe të folur të përditshëm, auf përdoret pothuajse gjithmonë në vend të auf das të plotë."
      ],
      "examples": [
        {
          "de": "Ich gehe aufs Dach.",
          "lv": "Po shkojë në kulmë."
        },
        {
          "de": "Sie setzt sich aufs Sofa.",
          "lv": "Ai ulet në divan."
        },
        {
          "de": "Wir fahren aufs Land.",
          "lv": "Do të shkojmë në fshat."
        },
        {
          "de": "Stell die Tasche aufs Bett.",
          "lv": "Vendose çantën në krevat."
        },
        {
          "de": "Er springt aufs Pferd.",
          "lv": "Ai kalëron me kalë."
        },
        {
          "de": "Leg das Buch aufs Regal.",
          "lv": "Vendoseni librin në raft."
        },
        {
          "de": "Komm schnell aufs Boot!",
          "lv": "Ejani në barkë shpejt!"
        },
        {
          "de": "Wir gehen aufs Fest.",
          "lv": "Do të shkojmë në një festë."
        }
      ],
      "comparison": [
        {
          "word": "aufs",
          "meaning": "Rast i veçantë (ak.)",
          "example": "aufs Dach – Atje lart!"
        },
        {
          "word": "auf",
          "meaning": "Sipërfaqja ose lart",
          "example": "auf den Tisch – Shishja është në tryezë."
        },
        {
          "word": "an",
          "meaning": "Në një sipërfaqe vertikale",
          "example": "an die Wand – Pranë murit"
        },
        {
          "word": "ins",
          "meaning": "Brenda",
          "example": "ins Zimmer – Ajo është në dhomë ."
        },
        {
          "word": "zum",
          "meaning": "Për / nga (kush?)",
          "example": "zum Arzt – Të lutem."
        }
      ],
      "tip": [
        "Mos harroni: auf + das aufs (→ku?, ku?).",
        "Tam auf das rrallë përdoret në gjuhën e folur - përdoret auf."
      ],
      "important": [
        "Aufs = auf das, only me një emër të çdo gjinie, ku ̈ në xhirim.",
        "Përgjigje ku? — për të lëvizur në një vend ose sipërfaqe specifike.",
        "Në një sipërfaqe horizontale, zakonisht përdoret nga auf në vend të auf.",
        "Nuk duhet të ngatërrohet me (në mur) ose (brenda dhomës)."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "aufs",
            "auf das"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "aufs"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "aufs"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "aufs Dach"
              ]
            }
          },
          {
            "word": {
              "green": [
                "auf"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "auf den Tisch"
              ]
            }
          },
          {
            "word": {
              "green": [
                "an"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "an die Wand"
              ]
            }
          },
          {
            "word": {
              "green": [
                "ins"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "ins Zimmer"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zum"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "zum Arzt"
              ]
            }
          }
        ],
        "tip": [
          {
            "blue": [
              "aufs"
            ]
          },
          {
            "purple": [
              "auf das"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "aufs"
            ],
            "purple": [
              "auf das"
            ]
          },
          {},
          {
            "yellow": [
              "një"
            ],
            "red": [
              "auf"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "Augenbraue",
    "de_article": "die",
    "de_plural": "die Augenbrauen",
    "lv": "Zierja",
    "level": "A1"
  },
  {
    "de": "Augenfarbe",
    "de_article": "die",
    "de_plural": "die Augenfarben",
    "lv": "Ngjyra e syve.",
    "level": "A1"
  },
  {
    "de": "Auto",
    "de_article": "das",
    "de_plural": "die Autos",
    "lv": "Henry, karroca!",
    "level": "A1"
  },
  {
    "de": "Bad",
    "de_article": "das",
    "de_plural": "die Bäder",
    "lv": "Banjo.",
    "level": "A1"
  },
  {
    "de": "Badezimmer",
    "de_article": "das",
    "de_plural": "die Badezimmer",
    "lv": "Banjo.",
    "level": "A1"
  },
  {
    "de": "Schwimmbad",
    "de_article": "das",
    "de_plural": "die Schwimmbäder",
    "lv": "Pishina!",
    "level": "A1"
  },
  {
    "de": "Sauna",
    "de_article": "die",
    "de_plural": "die Saunen",
    "lv": "Pesë dhoma gjumi.",
    "level": "A1"
  },
  {
    "de": "baden",
    "lv": "Not",
    "level": "A1",
    "study": {
      "id": "a1-baden",
      "layout": "standardStudy",
      "translation": "Not",
      "explanation": [
        "Ideja kryesore: Baden do të thotë të lahesh, të qëndrosh ose të shijosh ujin.",
        "Baden përdoret kur bëhet fjalë për t 'u çlodhur në ujë, liqen, det ose pishinë.",
        "Baden mund të nënkuptojë edhe banjën.",
        "Kur theksi është te sporti i lëvizjes ose vetë noti, fjala \"schwimmen\" përdoret më shpesh në gjermanisht."
      ],
      "examples": [
        {
          "de": "Ich gehe baden.",
          "lv": "Do shkoj të notoj."
        },
        {
          "de": "Wir gehen im See baden.",
          "lv": "Do të notojmë në liqen."
        },
        {
          "de": "Er schwimmt sehr gut.",
          "lv": "Ajo noton shumë mirë."
        },
        {
          "de": "Ich schwimme jeden Montag.",
          "lv": "Shkoj të notoj çdo të hënë."
        }
      ],
      "comparison": [
        {
          "word": "baden",
          "meaning": "Noti/qëndrimi në ujë/larja",
          "example": "Ich gehe baden."
        },
        {
          "word": "schwimmen",
          "meaning": "Noti si lëvizje ose sport",
          "example": "Er schwimmt sehr gut."
        },
        {
          "word": "duschen",
          "meaning": "Bëj një dush.",
          "example": "Ich dusche am Morgen."
        },
        {
          "word": "schwimmen gehen",
          "meaning": "Shko të notosh",
          "example": "Ich gehe heute schwimmen."
        }
      ],
      "tip": {
        "text": "Mos harroni: të pushoni në ujë të → ndotur • → Notarë të lëvizjes së notit."
      },
      "important": [
        "Baden dhe schwimmen nuk janë sinonime.",
        "Letonishtja zakonisht thotë \"noto\", por në gjermanisht duhet të zgjedhësh sipas situatës."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "baden"
          ],
          "red": [
            "schwimmen",
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "baden"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "baden"
              ],
              "green": [
                "See"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "schwimmt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "schwimme"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "baden"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "baden"
              ]
            }
          },
          {
            "word": {
              "green": [
                "schwimmen"
              ]
            },
            "meaning": {
              "purple": [
                "spor"
              ]
            },
            "example": {
              "red": [
                "schwimmt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "duschen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "dusche"
              ]
            }
          },
          {
            "word": {
              "green": [
                "schwimmen gehen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "schwimmen"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "baden"
            ],
            "red": [
              "schwimmen"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "bald",
    "lv": "Kënaqëm.",
    "level": "A1"
  },
  {
    "de": "Balkon",
    "de_article": "der",
    "de_plural": "die Balkons",
    "lv": "Në ballkon...",
    "level": "A1"
  },
  {
    "de": "Ball",
    "de_article": "der",
    "de_plural": "die Bälle",
    "lv": "Do ta hedh dhe ti do ta kapësh.",
    "level": "A1"
  },
  {
    "de": "Banane",
    "de_article": "die",
    "de_plural": "die Bananen",
    "lv": "Plantains.",
    "level": "A1"
  },
  {
    "de": "Bauch",
    "de_article": "der",
    "de_plural": "die Bäuche",
    "lv": "Stomaku",
    "level": "A1"
  },
  {
    "de": "Baum",
    "de_article": "der",
    "de_plural": "die Bäume",
    "lv": "Druri.",
    "level": "A1"
  },
  {
    "de": "bedeuten",
    "lv": "Mos e harroni.",
    "level": "A1"
  },
  {
    "de": "Erdbeere",
    "de_article": "die",
    "de_plural": "die Erdbeeren",
    "lv": "Luleshtrydhe.",
    "level": "A1"
  },
  {
    "de": "beginnen",
    "lv": "Fillo!",
    "level": "A1"
  },
  {
    "de": "bei",
    "lv": "Hapur.",
    "level": "A1",
    "study": {
      "id": "a1-bei",
      "layout": "standardStudy",
      "translation": "Hapur.",
      "explanation": "Përdoret kur diçka është e afërt me një person, organizatë, vend ose ndodh në kushte të caktuara.",
      "examples": [
        {
          "de": "Ich bin bei meinem Freund.",
          "lv": "Jam në shtëpinë e mikut tim."
        },
        {
          "de": "Sie arbeitet bei Siemens.",
          "lv": "Punon në Siemens."
        },
        {
          "de": "Bei Regen bleiben wir zu Hause.",
          "lv": "Qëndrojmë në shtëpi kur bie shi."
        }
      ],
      "comparison": [
        {
          "word": "bei",
          "meaning": "Një person, kompani ose në rrethana të caktuara",
          "example": "Ich bin bei meiner Schwester. – Jam me motrën time."
        },
        {
          "word": "an",
          "meaning": "Në mur, buzë, buzë, buzë sipërfaqësore",
          "example": "Das Bild hängt an der Wand. – Piktura është e varur në mur."
        },
        {
          "word": "zu",
          "meaning": "Kush do të shkojë (drejtimi)",
          "example": "Ich gehe zu meinem Freund. – Po e takoj një mik."
        }
      ],
      "tip": {
        "text": "Mos harroni: → bei në një person/vend/kompani."
      },
      "sectionAccents": {
        "examples": [
          {
            "blue": [
              "bei"
            ]
          },
          {
            "blue": [
              "bei"
            ]
          },
          {
            "blue": [
              "Bei"
            ]
          }
        ],
        "comparison": [
          {
            "word": {
              "purple": [
                "bei"
              ]
            },
            "example": {
              "purple": [
                "bei"
              ]
            }
          },
          {
            "word": {
              "green": [
                "an"
              ]
            },
            "example": {
              "green": [
                "an"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zu"
              ]
            },
            "example": {
              "red": [
                "zu"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "purple": [
              "bei"
            ]
          },
          {
            "blue": [
              "auf"
            ],
            "red": [
              "bei"
            ]
          }
        ]
      },
      "important": [
        "BEI zakonisht i referohet një personi, vendi ose kompanie.",
        "Kur bëhet fjalë për sipërfaqen, zakonisht keni nevojë për auf, jo bei."
      ]
    }
  },
  {
    "de": "beide",
    "lv": "Jo.",
    "level": "A1"
  },
  {
    "de": "Bein",
    "de_article": "das",
    "de_plural": "die Beine",
    "lv": "Ishte më i madhi.",
    "level": "A1"
  },
  {
    "de": "Beispiel",
    "de_article": "das",
    "de_plural": "die Beispiele",
    "lv": "Shembull:",
    "level": "A1"
  },
  {
    "de": "bekommen",
    "lv": "Për të marrë",
    "level": "A1"
  },
  {
    "de": "benutzen",
    "lv": "Përdorimi",
    "level": "A1"
  },
  {
    "de": "Berg",
    "de_article": "der",
    "de_plural": "die Berge",
    "lv": "Lart.",
    "level": "A1"
  },
  {
    "de": "besser",
    "lv": "Filloi ..",
    "level": "A1"
  },
  {
    "de": "beste",
    "lv": "Më të mirat",
    "level": "A1"
  },
  {
    "de": "Besuch",
    "de_article": "der",
    "de_plural": "die Besuche",
    "lv": "Vizitë",
    "level": "A1"
  },
  {
    "de": "Besucher",
    "de_article": "der",
    "de_plural": "die Besucher",
    "lv": "Vizitor",
    "level": "A1"
  },
  {
    "de": "besuchen",
    "lv": "Bashkohu • Vizito",
    "level": "A1"
  },
  {
    "de": "Bett",
    "de_article": "das",
    "de_plural": "die Betten",
    "lv": "Krevati.",
    "level": "A1"
  },
  {
    "de": "bis",
    "lv": "Değin",
    "level": "A1",
    "study": {
      "id": "a1-bis",
      "layout": "standardStudy",
      "translation": "Değin",
      "explanation": "I referohet një kufiri, pike kohore ose kushti.",
      "examples": [
        {
          "de": "Ich warte bis zu deiner Ankunft.",
          "lv": "Po pres ardhjen tënde."
        },
        {
          "de": "Bleib hier, bis ich zurückkomme.",
          "lv": "Qëndro këtu derisa të kthehem."
        },
        {
          "de": "Ich lerne Deutsch bis zum Abend.",
          "lv": "Po mësoj gjermanisht deri në mbrëmje."
        },
        {
          "de": "Bis jetzt habe ich nichts verstanden.",
          "lv": "Nuk kuptoj ende asgjë."
        }
      ],
      "comparison": [
        {
          "word": "bis",
          "meaning": "Deri në (u arrit pika kohore)",
          "example": "Ich bleibe bis morgen. – Do të qëndroj deri në orën18:00."
        },
        {
          "word": "bis zu",
          "meaning": "Deri në (disa kohë)",
          "example": "bis zum Bahnhof – Punoj nga ora 9 deri në 5."
        },
        {
          "word": "bis jetzt",
          "meaning": "Değin",
          "example": "Bis jetzt habe ich nichts verstanden. – Po pres që të vish."
        },
        {
          "word": "bis jetzt",
          "meaning": "Deri më tani",
          "example": "Bis jetzt ist alles gut. – Deri tani, mirë."
        }
      ],
      "tip": {
        "text": "Mos harroni: kufiri kohor/i statusit → përsëri."
      },
      "sectionAccents": {
        "examples": [
          {
            "purple": [
              "bis"
            ]
          },
          {
            "purple": [
              "bis"
            ]
          },
          {
            "purple": [
              "bis"
            ]
          },
          {
            "blue": [
              "Bis jetzt"
            ]
          }
        ],
        "comparison": [
          {
            "word": {
              "purple": [
                "bis"
              ]
            },
            "example": {
              "purple": [
                "bis"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bis ... zu"
              ]
            },
            "example": {
              "green": [
                "bis"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bis dass"
              ]
            },
            "example": {
              "yellow": [
                "bis dass"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bis jetzt"
              ]
            },
            "example": {
              "blue": [
                "Bis jetzt"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "purple": [
              "bis"
            ]
          },
          {
            "blue": [
              "bis jetzt",
              "bis dass"
            ]
          }
        ]
      },
      "important": [
        "Bis = në kufirin ose pikën në kohë.",
        "Bis jetzt = deri tani • Bis dass = deri."
      ]
    }
  },
  {
    "de": "bisschen",
    "lv": "Ke fjetur ndopak?",
    "level": "A1"
  },
  {
    "de": "bitte",
    "lv": ".:. Ju lutem ...",
    "level": "A1",
    "study": {
      "id": "a1-bitte",
      "layout": "standardStudy",
      "translation": ".:. Ju lutem ...",
      "explanation": [
        "Ideja kryesore: Një fjalë e sjellshme e shkruar me shkronja të vogla. Isha mirë, të lutem.",
        "Bititi kryesisht do të thotë: mirësjellje.",
        "Zakonisht përshkruan: një fjalë të sjellshme.",
        "Bitit në thelb do të thotë: kërkesë/kërkesë.",
        "Zakonisht karakterizohet nga: një emër (vdes).",
        "Bitte në shkronja të vogla është një fjalë e sjellshme - do të thotë ju lutem (Bitte schön!, Eine Tasse Kaffee, bitte).",
        "Capitalized Die Bitte and die është një emër artikulli - kjo do të thotë një kërkesë ose dëshirë (Ich habe eine Bitte = Kam një kërkesë).",
        "Shumësi: të vdesësh duke u kafshuar."
      ],
      "examples": [
        {
          "de": "Eine Tasse Kaffee, bitte.",
          "lv": ".:. Ju lutem ..."
        },
        {
          "de": "Komm bitte herein.",
          "lv": ".:. Ju lutem ..."
        },
        {
          "de": "Bitte schön!",
          "lv": "Do të doja një filxhan kafe, të lutem."
        },
        {
          "de": "Kann ich bitte fragen?",
          "lv": "Ben te ju pyes ..."
        },
        {
          "de": "Ich habe eine Bitte.",
          "lv": "Unë jam babai yt!"
        },
        {
          "de": "Die Bitte ist wichtig.",
          "lv": "Kërkesa është e rëndësishme."
        }
      ],
      "tip": [
        "Little bitte = please (Bitte schön!, Kaffee, bitte). capitalize die Bitte = request (eine Bitte, meine Bitte).",
        "Me mirësjellje, me shkronja të vogla. Dikur isha i sjellshëm, të lutem."
      ],
      "important": [
        "Bitte është e vogël • Nuk është emër, është fjalë e sjellshme.",
        "Die Bitte shkruhet me shkronja të mëdha dhe artikulli die është një emër.",
        "Plural: vdes nga morrat (dëshirat).",
        "Gabim: Die Bitte schön! → E saktë: Bitte schön!"
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "bitte"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bitte"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "bitte"
            ]
          }
        ]
      },
      "comparison": [
        {
          "word": "bitte",
          "meaning": "lūdzu",
          "example": "Komm bitte herein. – Lūdzu, nāc iekšā."
        },
        {
          "word": "die Bitte",
          "meaning": "lūgums",
          "example": "Ich habe eine Bitte. – Man ir lūgums."
        }
      ]
    }
  },
  {
    "de": "Bitte",
    "de_article": "die",
    "de_plural": "die Bitten",
    "lv": "Kërkesë",
    "level": "A1",
    "study": {
      "id": "a1-bitte-study",
      "layout": "standardStudy",
      "translation": "Kërkesë",
      "explanation": [
        "Ideja kryesore: Një emër me nenin vdesin dhe shkronja të mëdha. Një kërkesë ose kërkesë specifike.",
        "Die Bitte në thelb do të thotë mirësi.",
        "Zakonisht përshkruan: një fjalë të sjellshme.",
        "Die Bitte në thelb do të thotë: dua/dua.",
        "Zakonisht karakterizohet nga: një emër (vdes).",
        "Bitte në shkronja të vogla është një fjalë e sjellshme - do të thotë ju lutem (Bitte schön!, Eine Tasse Kaffee, bitte).",
        "Capitalized Die Bitte and die është një emër artikulli - kjo do të thotë një kërkesë ose dëshirë (Ich habe eine Bitte = Kam një kërkesë).",
        "Shumësi: të vdesësh duke u kafshuar."
      ],
      "examples": [
        {
          "de": "Ich habe eine Bitte.",
          "lv": "Unë jam babai yt!"
        },
        {
          "de": "Er erfüllt meine Bitte.",
          "lv": ".:. Ju lutem ..."
        },
        {
          "de": "Sie hat zwei Bitten.",
          "lv": "Do të doja një filxhan kafe, të lutem."
        },
        {
          "de": "Kann ich bitte fragen?",
          "lv": "Ben te ju pyes ..."
        },
        {
          "de": "Ich habe eine Bitte.",
          "lv": "Unë jam babai yt!"
        },
        {
          "de": "Die Bitte ist wichtig.",
          "lv": "Kërkesa është e rëndësishme."
        }
      ],
      "tip": [
        "Little bitte = please (Bitte schön!, Kaffee, bitte). capitalize die Bitte = request (eine Bitte, meine Bitte).",
        "Një emër që përmban një artikull dhe një shkronjë të madhe. Një kërkesë ose kërkesë specifike."
      ],
      "important": [
        "Bitte është e vogël • Nuk është emër, është fjalë e sjellshme.",
        "Die Bitte shkruhet me shkronja të mëdha dhe artikulli die është një emër.",
        "Plural: vdes nga morrat (dëshirat).",
        "Gabim: Die Bitte schön! → E saktë: Bitte schön!"
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "die Bitte",
            "bitte"
          ],
          "yellow": [
            "Bitte"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "bitte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "die Bitte",
                "bitte"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      },
      "comparison": [
        {
          "word": "die Bitte",
          "meaning": "lūgums",
          "example": "Ich habe eine Bitte. – Man ir lūgums."
        },
        {
          "word": "bitte",
          "meaning": "lūdzu",
          "example": "Komm bitte herein. – Lūdzu, nāc iekšā."
        }
      ]
    }
  },
  {
    "de": "billig",
    "lv": "I pavlerë.",
    "level": "A1"
  },
  {
    "de": "Bier",
    "de_article": "das",
    "de_plural": "die Biere",
    "lv": "Birrë.",
    "level": "A1"
  },
  {
    "de": "Bild",
    "de_article": "das",
    "de_plural": "die Bilder",
    "lv": "-Autogram.",
    "level": "A1"
  },
  {
    "de": "bitten",
    "lv": "Dashuria",
    "level": "A1"
  },
  {
    "de": "Blatt",
    "de_article": "das",
    "de_plural": "die Blätter",
    "lv": "Festë",
    "level": "A1"
  },
  {
    "de": "blau",
    "lv": "Blu",
    "level": "A1"
  },
  {
    "de": "bleiben",
    "lv": "Qëndro",
    "level": "A1",
    "study": {
      "id": "a1-bleiben",
      "layout": "standardStudy",
      "translation": "Qëndro",
      "explanation": [
        "Ideja kryesore: bleiben do të thotë të qëndrosh.",
        "Bleibeni përdoret kur një person ose send nuk humbet dhe mbetet në të njëjtin vend ose situatë.",
        "Kur bëhet fjalë për ndarjen, është e kundërta e ferrit dhe fahrenheit.",
        "Një term shumë popullor është Ich bleibe zu Hause."
      ],
      "examples": [
        {
          "de": "Ich bleibe zu Hause.",
          "lv": "Do të qëndroj në shtëpi"
        },
        {
          "de": "Bleib hier!",
          "lv": "Qendro ketu!"
        },
        {
          "de": "Wir bleiben noch eine Stunde.",
          "lv": "Do të qëndrojmë për një orë tjetër."
        },
        {
          "de": "Ich gehe nach Hause.",
          "lv": "Do shkoj në shtëpi."
        }
      ],
      "comparison": [
        {
          "word": "bleiben",
          "meaning": "Qëndro",
          "example": "Ich bleibe hier."
        },
        {
          "word": "gehen",
          "meaning": "Shko/shko në këmbë",
          "example": "Ich gehe nach Hause."
        },
        {
          "word": "fahren",
          "meaning": "Shkuarja/vozitja me transport",
          "example": "Ich fahre nach Hause."
        },
        {
          "word": "warten",
          "meaning": "Prit",
          "example": "Ich warte hier."
        }
      ],
      "tip": {
        "text": "Mos harroni→: bleiben • → Mos i lini gjethet në këmbë."
      },
      "important": [
        "Bleiben do të thotë të qëndrosh, jo të presësh.",
        "Hier e tyre e zymtë = Unë do të qëndroj këtu • Vlera e tyre hier = Unë jam duke pritur këtu."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "bleiben"
          ],
          "red": [
            "fahrenheit"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "bleibe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Bleib"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bleiben"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "gehe"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "bleiben"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "bleibe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "gehen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "gehe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "fahren"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "fahre"
              ]
            }
          },
          {
            "word": {
              "green": [
                "warten"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "warto"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "bleiben"
            ]
          },
          {
            "blue": [
              "Hier"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Bleistift",
    "de_article": "der",
    "de_plural": "die Bleistifte",
    "lv": "Një stilolaps.",
    "level": "A1"
  },
  {
    "de": "blond",
    "lv": "Ajo është rreth 1m 80 cm e gjatë.",
    "level": "A1"
  },
  {
    "de": "Blume",
    "de_article": "die",
    "de_plural": "die Blumen",
    "lv": "You are sunshine",
    "level": "A1"
  },
  {
    "de": "Boot",
    "de_article": "das",
    "de_plural": "die Boote",
    "lv": "Barkë.",
    "level": "A1"
  },
  {
    "de": "brauchen",
    "lv": "Kërko",
    "level": "A1"
  },
  {
    "de": "braun",
    "lv": "Ngjyre kafe",
    "level": "A1"
  },
  {
    "de": "breit",
    "lv": "Thellë.",
    "level": "A1"
  },
  {
    "de": "Brief",
    "de_article": "der",
    "de_plural": "die Briefe",
    "lv": "Letrën.",
    "level": "A1"
  },
  {
    "de": "Brille",
    "de_article": "die",
    "de_plural": "die Brillen",
    "lv": "Syze.",
    "level": "A1"
  },
  {
    "de": "bringen",
    "lv": "Sill • Merr",
    "level": "A1",
    "study": {
      "id": "a1-bringen",
      "layout": "standardStudy",
      "translation": "Sill • Merr",
      "explanation": [
        "Ideja kryesore: Sjellja e mjeteve dikujt për të sjellë, mbajtur ose ofruar diçka.",
        "Ne përdorim Bringen kur diçka zhvendoset në një vend tjetër ose në një person tjetër.",
        "Kjo nuk është e njëjtë me Nehmenin, sepse Nehmen do të thotë ta marrësh vetë.",
        "Holen do të thotë të ndjekësh, të sjellësh ose të marrësh dikë."
      ],
      "examples": [
        {
          "de": "Ich bringe dir ein Buch.",
          "lv": "Ju lutem më sillni ujë"
        },
        {
          "de": "Ich bringe das Paket zur Post.",
          "lv": "Do të dërgoj në shtëpi."
        },
        {
          "de": "Ich bringe die Kinder zur Schule.",
          "lv": "Ajo e çon librin në shkollë."
        },
        {
          "de": "Ich nehme das Buch.",
          "lv": "Po e marr librin"
        }
      ],
      "comparison": [
        {
          "word": "bringen",
          "meaning": "Sill /merr/dorëzo",
          "example": "Ich bringe dir ein Buch. – Mir Wasser'ı getir."
        },
        {
          "word": "bringen",
          "meaning": "Merre atë.",
          "example": "Ich bringe das Paket zur Post. – Ich nehme das Buch."
        },
        {
          "word": "bringen",
          "meaning": "Merre.",
          "example": "Ich bringe die Kinder zur Schule. – Ich dziura Wasser."
        },
        {
          "word": "bringen",
          "meaning": "Merre dhe sille",
          "example": "Ich bringe dir ein Buch. – Bringst du Brot mit?"
        },
        {
          "word": "nehmen",
          "meaning": "paņemt",
          "example": "Ich nehme das Buch. – Es paņemu grāmatu."
        }
      ],
      "tip": {
        "text": "Mos harro: shko → merre një të tillë • Merre për vete → nehmen."
      },
      "important": [
        "Trego rrugën për te dikush ose diku.",
        "Nehmen do të thotë të marrësh, por jo t 'i dorëzosh tjetrit."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "bringen"
          ],
          "red": [
            "nehmen",
            "holen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Bring"
              ],
              "yellow": [
                "Wasser"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bringe"
              ],
              "green": [
                "dich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bringt"
              ],
              "yellow": [
                "Buch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "nehme"
              ],
              "yellow": [
                "Buch"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "bringen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "nehmen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "nehme"
              ]
            }
          },
          {
            "word": {
              "green": [
                "holen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "mitbringen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "mit"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {
            "red": [
              "nehmen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Butterbrot",
    "de_article": "das",
    "de_plural": "die Butterbrote",
    "lv": "Sanduiç me mish.",
    "level": "A1"
  },
  {
    "de": "Brötchen",
    "de_article": "das",
    "de_plural": "die Brötchen",
    "lv": "Rrotullim",
    "level": "A1"
  },
  {
    "de": "Brücke",
    "de_article": "die",
    "de_plural": "die Brücken",
    "lv": "Ura.",
    "level": "A1"
  },
  {
    "de": "Bruder",
    "de_article": "der",
    "de_plural": "die Brüder",
    "lv": "Motër ...",
    "level": "A1"
  },
  {
    "de": "Buch",
    "de_article": "das",
    "de_plural": "die Bücher",
    "lv": "Librin!",
    "level": "A1"
  },
  {
    "de": "Buchstabe",
    "de_article": "der",
    "de_plural": "die Buchstaben",
    "lv": "Letrën.",
    "level": "A1"
  },
  {
    "de": "Büro",
    "de_article": "das",
    "de_plural": "die Büros",
    "lv": "Në zyrë.",
    "level": "A1"
  },
  {
    "de": "Bus",
    "de_article": "der",
    "de_plural": "die Busse",
    "lv": "Një autobus.",
    "level": "A1"
  },
  {
    "de": "Butter",
    "de_article": "die",
    "lv": "Gjalpë",
    "level": "A1"
  },
  {
    "de": "Café",
    "de_article": "das",
    "de_plural": "die Cafés",
    "lv": "Kafene",
    "level": "A1"
  },
  {
    "de": "CD",
    "de_article": "die",
    "de_plural": "die CDs",
    "lv": "CD",
    "level": "A1"
  },
  {
    "de": "Computer",
    "de_article": "der",
    "de_plural": "die Computer",
    "lv": "Kompjuteri",
    "level": "A1"
  },
  {
    "de": "Cousin",
    "de_article": "der",
    "de_plural": "die Cousins",
    "lv": "Cuz.",
    "level": "A1"
  },
  {
    "de": "Cousine",
    "de_article": "die",
    "de_plural": "die Cousinen",
    "lv": "Cuz.",
    "level": "A1"
  },
  {
    "de": "da",
    "lv": "Atje ju ...",
    "level": "A1",
    "study": {
      "id": "a1-da",
      "layout": "standardStudy",
      "translation": "Atje ju ...",
      "explanation": [
        "Ideja kryesore: Niveli A1 nënkupton edhe atë më shpesh.",
        "Tregon gjithashtu një vend ose i referohet diçkaje të përmendur më parë.",
        "Në varësi të rastit, kjo mund të përkthehet edhe këtu ose këtu.",
        "Në nivelin A1, ne kryesisht mësojmë da si një fjalë e përgjithshme e vendit."
      ],
      "examples": [
        {
          "de": "Da ist mein Auto.",
          "lv": "Makina ime është atje."
        },
        {
          "de": "Ich war da.",
          "lv": "Isha atje."
        },
        {
          "de": "Da kommt er.",
          "lv": "Ha ku po vjen."
        },
        {
          "de": "Komm mal da her!",
          "lv": "Bruce!"
        }
      ],
      "comparison": [
        {
          "word": "da",
          "meaning": "Atje • Këtu • Këtu (të përgjithshme)",
          "example": "Da ist mein Auto."
        },
        {
          "word": "hier",
          "meaning": "Këtu (në një vend specifik)",
          "example": "Hier ist mein Auto."
        },
        {
          "word": "dort",
          "meaning": "Atje (përpara)",
          "example": "Dort ist mein Auto."
        },
        {
          "word": "dann",
          "meaning": "Atëherë...",
          "example": "Dann gehen wir nach Hause."
        }
      ],
      "tip": {
        "text": "Mos harroni: Gjenerali do → ta vendosë atje."
      },
      "sectionAccents": {
        "explanation": {
          "blue": [
            "da"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Da"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "da"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Da"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "da"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "da"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "hier"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "dort"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "dann"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "Dann"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {}
        ]
      },
      "accents": {
        "blue": [
          "da",
          "Da"
        ],
        "purple": [
          "tur",
          "Tur"
        ],
        "green": [
          "te",
          "Te",
          "Tutaj"
        ],
        "yellow": [
          "hier"
        ],
        "red": [
          "dort",
          "dann"
        ]
      },
      "important": [
        "Kjo do t 'i japë vendit emrin e tij të përgjithshëm.",
        "Loja është e veçanta “këtu”, torta është edhe më shumë “atje”."
      ]
    }
  },
  {
    "de": "danken",
    "lv": "Të të falenderoj.",
    "level": "A1"
  },
  {
    "de": "dann",
    "lv": "Atëherë...",
    "level": "A1"
  },
  {
    "de": "das",
    "lv": "Artikull i përcaktuar neutral",
    "level": "A1",
    "study": {
      "id": "a1-das",
      "layout": "standardStudy",
      "translation": "Artikull i përcaktuar neutral",
      "explanation": "Përdoret për emra të butë. Në disa fjali, \"ajo\" mund të përdoret gjithashtu si përemër ose përemër relativ.",
      "examples": [
        {
          "de": "Das ist mein Auto.",
          "lv": "Ajo është vetura ime."
        },
        {
          "de": "Das ist gut.",
          "lv": "Mirë me kaq."
        },
        {
          "de": "Das Buch, das ich lese, ist interessant.",
          "lv": "Libri që kam lexuar është interesant."
        }
      ],
      "comparison": [
        {
          "word": "das",
          "meaning": "Bu (artikull/përemër)",
          "example": "Das ist mein Auto. – Ajo është vetura ime."
        },
        {
          "word": "dies",
          "meaning": "Kjo ishte?",
          "example": "Dies ist mein Auto. – Ajo është vetura ime."
        },
        {
          "word": "welches",
          "meaning": "Kush? Kush? Kush ata?",
          "example": "Das ist das Buch, welches ich lese. – Është një libër që kam lexuar."
        }
      ],
      "tip": {
        "text": "Çështjet: vidhosni dzimte → das • Ka → das."
      },
      "sectionAccents": {
        "examples": [
          {
            "blue": [
              "Das"
            ]
          },
          {
            "blue": [
              "Das"
            ]
          },
          {
            "de": {
              "blue": [
                "Das"
              ],
              "yellow": [
                "das"
              ]
            }
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "das"
              ]
            },
            "example": {
              "blue": [
                "Das"
              ]
            }
          },
          {
            "word": {
              "green": [
                "dies"
              ]
            },
            "example": {
              "green": [
                "Dies"
              ]
            }
          },
          {
            "word": {
              "green": [
                "welches"
              ]
            },
            "example": {
              "blue": [
                "Das"
              ],
              "yellow": [
                "welches"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "das"
            ]
          },
          {
            "blue": [
              "das"
            ],
            "red": [
              "dass"
            ]
          }
        ]
      },
      "important": [
        "Në nivelin A1, DAS fillimisht shqyrtohet si një artikull për seksin e mesëm.",
        "Das nuk është i njëjtë me dass - das mund të jetë një artikull ose një përemër, dass do të thotë \"o\"."
      ]
    }
  },
  {
    "de": "dass",
    "lv": "Asaj.",
    "level": "A1",
    "study": {
      "id": "a1-dass",
      "layout": "standardStudy",
      "translation": "Asaj.",
      "explanation": "Sjell një artikull ndihmës që shpreh një fakt, mendim ose shprehje.",
      "examples": [
        {
          "de": "Ich weiß, dass du müde bist.",
          "lv": "E di që je lodhur."
        },
        {
          "de": "Er sagt, dass er kommt.",
          "lv": "Ai thotë se do të vijë."
        },
        {
          "de": "Ich glaube, dass das stimmt.",
          "lv": "Unë mendoj që është ashtu."
        }
      ],
      "comparison": [
        {
          "word": "dass",
          "meaning": "Asaj.",
          "example": "Ich weiß, dass er kommt. – E di që do të vish."
        },
        {
          "word": "weil",
          "meaning": "Sepse jam unë... mirë është... sepse është më e bukur...",
          "example": "Ich bleibe zu Hause, weil es regnet. – Do të qëndroj në shtëpi sepse po bie shi."
        },
        {
          "word": "damit",
          "meaning": "-Poshtë.",
          "example": "Ich lerne Deutsch, damit ich in Deutschland arbeiten kann. – Unë po mësoj gjermanisht për të qenë në gjendje të punoj në Gjermani."
        },
        {
          "word": "ob",
          "meaning": "Apo...",
          "example": "Ich weiß nicht, ob er kommt. – Nuk e di nëse do të vish."
        }
      ],
      "tip": {
        "text": "Pyetjet: ka → dass."
      },
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "dass"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "dass"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "dass"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "dass"
              ]
            },
            "example": {
              "blue": [
                "dass"
              ]
            }
          },
          {
            "word": {
              "green": [
                "weil"
              ]
            },
            "example": {
              "green": [
                "weil"
              ]
            }
          },
          {
            "word": {
              "green": [
                "damit"
              ]
            },
            "example": {
              "yellow": [
                "damit"
              ]
            }
          },
          {
            "word": {
              "green": [
                "ob"
              ]
            },
            "example": {
              "red": [
                "ob"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "dass"
            ]
          },
          {
            "red": [
              "das"
            ]
          }
        ]
      },
      "important": [
        "Dass do të thotë \"o\" dhe shton një eksipient.",
        "Nuk duhet të ngatërrohet me një artikull ose das, i cili mund të jetë \"ky\"."
      ]
    }
  },
  {
    "de": "Datum",
    "de_article": "das",
    "de_plural": "die Daten",
    "lv": "Data",
    "level": "A1"
  },
  {
    "de": "dein",
    "lv": "-Fëmijët e tu.",
    "level": "A1"
  },
  {
    "de": "denken",
    "lv": "Të menduarit",
    "level": "A1"
  },
  {
    "de": "der",
    "lv": "Speciet e një seksi të veçantë mashkullor",
    "level": "A1",
    "study": {
      "id": "a1-der",
      "layout": "standardStudy",
      "translation": "Speciet e një seksi të veçantë mashkullor",
      "explanation": "Përdoret në lidhje me emrat mashkullorë. Në disa fjali, \"der\" mund të funksionojë edhe si përemër ose përemër interesi.",
      "examples": [
        {
          "de": "Der Mann ist hier.",
          "lv": "Ky njeri është këtu."
        },
        {
          "de": "Der Bus kommt.",
          "lv": "Autobusi po vjen."
        },
        {
          "de": "Der Lehrer spricht.",
          "lv": "Mësuesi flet."
        }
      ],
      "tip": {
        "text": "Mos harroni: → thotë tipi mashkullor."
      },
      "sectionAccents": {
        "examples": [
          {
            "blue": [
              "Der"
            ]
          },
          {
            "blue": [
              "Der"
            ]
          },
          {
            "blue": [
              "Der"
            ]
          }
        ],
        "comparison": [
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {}
        ]
      },
      "important": [
        "Në nivelin A1, duhet të punoni kryesisht si një artikull mashkullor.",
        "Përemrat dhe praktikat relative dalin më vonë."
      ]
    }
  },
  {
    "de": "deutsch",
    "lv": "Gjermanisht",
    "level": "A1"
  },
  {
    "de": "Dezember",
    "de_article": "der",
    "lv": "Akcioni i parë është e thjeshtë. \"Gap.\"",
    "level": "A1"
  },
  {
    "de": "die",
    "lv": "Gjërat personale të grave",
    "level": "A1",
    "study": {
      "id": "a1-die",
      "layout": "standardStudy",
      "translation": "Gjërat personale të grave",
      "explanation": "Përdoret me emra femrash. Në disa fjali, \"vdekja\" mund të funksionojë edhe si përemër ose përemër interesi.",
      "examples": [
        {
          "de": "Die Frau ist hier.",
          "lv": "Ajo është këtu."
        },
        {
          "de": "Die Katze schläft.",
          "lv": "Kotelja po fle."
        },
        {
          "de": "Die Lehrerin erklärt.",
          "lv": "-Mësuesja shpjegon."
        }
      ],
      "tip": {
        "text": "Mos harroni: feminiteti → vdes."
      },
      "sectionAccents": {
        "examples": [
          {
            "blue": [
              "Die"
            ]
          },
          {
            "blue": [
              "Die"
            ]
          },
          {
            "blue": [
              "Die"
            ]
          }
        ],
        "comparison": [
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {}
        ]
      },
      "important": [
        "Në nivelin A1, kubi ekzaminohet së pari si send i një gruaje.",
        "Shumësi përdoret gjithashtu për të gjitha gjinitë."
      ]
    }
  },
  {
    "de": "Dienstag",
    "de_article": "der",
    "de_plural": "die Dienstage",
    "lv": "E martë",
    "level": "A1"
  },
  {
    "de": "dieser",
    "lv": "Kjo ishte?",
    "level": "A1",
    "study": {
      "id": "a1-dieser",
      "layout": "standardStudy",
      "translation": "Kjo ishte?",
      "explanation": "Tregon një person, objekt ose kafshë në afërsi. Përdoret me emër mashkullor.",
      "examples": [
        {
          "de": "Dieser Mann ist nett.",
          "lv": "Ky djalë është i këndshëm."
        },
        {
          "de": "Ich sehe diesen Hund.",
          "lv": "Më pëlqen ky qen"
        },
        {
          "de": "Dieser Stift ist neu.",
          "lv": "Ky stilolaps është i ri."
        }
      ],
      "tip": {
        "text": "Mos harroni: o + → vrasës mashkullor."
      },
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "Dieser"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "diesen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Dieser"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "dieser"
            ],
            "green": [
              "diese"
            ],
            "yellow": [
              "Dieser"
            ]
          },
          {
            "red": [
              "diese"
            ]
          }
        ]
      },
      "important": [
        "Dieser, diese dhe diese ndryshojnë sipas gjinisë.",
        "Shumësi është përsëri diese."
      ]
    }
  },
  {
    "de": "Ding",
    "de_article": "das",
    "de_plural": "die Dinge",
    "lv": "E pra,",
    "level": "A1"
  },
  {
    "de": "Donnerstag",
    "de_article": "der",
    "de_plural": "die Donnerstage",
    "lv": "E enjte",
    "level": "A1"
  },
  {
    "de": "dort",
    "lv": "Atje ju ...",
    "level": "A1"
  },
  {
    "de": "drei",
    "lv": "Dy...",
    "level": "A1"
  },
  {
    "de": "dreihundert",
    "lv": "Treqind",
    "level": "A1"
  },
  {
    "de": "dreimal",
    "lv": "-E kam pallosur. 3 herë.",
    "level": "A1"
  },
  {
    "de": "dreißig",
    "lv": "30.",
    "level": "A1"
  },
  {
    "de": "dreizehn",
    "lv": "Trembëdhjetë!",
    "level": "A1"
  },
  {
    "de": "dritte",
    "lv": "Të tretën.",
    "level": "A1"
  },
  {
    "de": "du",
    "lv": "Dukesh ...",
    "level": "A1"
  },
  {
    "de": "dürfen",
    "lv": "E lejueshme",
    "level": "A1"
  },
  {
    "de": "Dusche",
    "de_article": "die",
    "de_plural": "die Duschen",
    "lv": "Dush",
    "level": "A1"
  },
  {
    "de": "Ecke",
    "de_article": "die",
    "de_plural": "die Ecken",
    "lv": "Këndi",
    "level": "A1"
  },
  {
    "de": "Ei",
    "de_article": "das",
    "de_plural": "die Eier",
    "lv": "Vezë.",
    "level": "A1"
  },
  {
    "de": "ein",
    "lv": "Artikull i paqartë • Një • Dikush",
    "level": "A1",
    "study": {
      "id": "a1-ein",
      "layout": "standardStudy",
      "translation": "Artikull i paqartë • Një • Dikush",
      "explanation": "Përdoret me emër mashkullor. Duke treguar diçka ose dikë nga një shumëllojshmëri mundësish.",
      "examples": [
        {
          "de": "Ein Mann wartet draußen.",
          "lv": "Një burrë po pret jashtë."
        },
        {
          "de": "Ich habe ein Buch.",
          "lv": "Kam një libër."
        },
        {
          "de": "Er sucht einen Stift.",
          "lv": "Ai po kërkon një stilolaps."
        },
        {
          "de": "Ein Kind spielt.",
          "lv": "Bērns spēlējas."
        }
      ],
      "tip": {
        "text": "Mos harroni: dikush i → pasigurt."
      },
      "sectionAccents": {
        "examples": [
          {
            "de": {
              "blue": [
                "Ein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "einen"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "ein"
            ]
          },
          {
            "blue": [
              "Nëse",
              "die",
              "das"
            ],
            "green": [
              "Nëse"
            ]
          }
        ]
      },
      "important": [
        "Ein nuk është një artikull i caktuar.",
        "Nëse subjekti tashmë është i njohur konkretisht, shpesh nevojitet Die ose das."
      ],
      "comparison": [
        {
          "word": "ein Mann",
          "meaning": "vīriešu dzimte",
          "example": "Ein Mann wartet draußen."
        },
        {
          "word": "eine Frau",
          "meaning": "sieviešu dzimte",
          "example": "eine Frau"
        },
        {
          "word": "ein Buch",
          "meaning": "vidus dzimte",
          "example": "Ich habe ein Buch."
        },
        {
          "word": "einen Mann",
          "meaning": "akuzatīvs",
          "example": "einen Mann"
        }
      ]
    }
  },
  {
    "de": "einfach",
    "lv": "Për shkaqe praktike.",
    "level": "A1"
  },
  {
    "de": "eins",
    "lv": "Një.",
    "level": "A1"
  },
  {
    "de": "Eis",
    "de_article": "das",
    "lv": "Akullore • Akullore",
    "level": "A1",
    "study": {
      "id": "a1-eis",
      "layout": "standardStudy",
      "translation": "Akullore • Akullore",
      "explanation": [
        "Ideja kryesore: das Eis mund të nënkuptojë akullore dhe xhelatinë.",
        "Letonishtja shpesh thotë \"akull\" kur flet për ujë të ftohtë dhe të ngrirë.",
        "Kur bëhet fjalë për ushqim ose ëmbëlsirë, das Eis kryesisht do të thotë akullore në jetën e përditshme.",
        "Konteksti shpesh ju tregon menjëherë se cili është kuptimi i synuar.",
        "Shprehjet më të rëndësishme në nivelin A1 janë ein Eis essen dhe Eis im Glas."
      ],
      "examples": [
        {
          "de": "Ich esse ein Eis.",
          "lv": "Ambienti im i akullores"
        },
        {
          "de": "Möchtest du ein Eis?",
          "lv": "Dëshiron akullore"
        },
        {
          "de": "Im Winter liegt Eis auf dem See.",
          "lv": "Në dimër ka akull në liqen."
        },
        {
          "de": "Das Eis ist kalt.",
          "lv": "Ftohje akulli."
        },
        {
          "de": "Ich nehme ein Eis mit Schokolade.",
          "lv": "Dua akullore me çokollatë."
        }
      ],
      "comparison": [
        {
          "word": "das Eis",
          "meaning": "Akullore / akullore",
          "example": "Ich esse ein Eis. = Es ēdu saldējumu."
        },
        {
          "word": "der Schnee",
          "meaning": "Do të bjerë borë",
          "example": "Der Schnee ist weiß. = Sniegs ir balts."
        },
        {
          "word": "kalt",
          "meaning": "Ftohët.",
          "example": "Das Wasser ist kalt. = Ūdens ir auksts."
        },
        {
          "word": "das Dessert",
          "meaning": "E bukur.",
          "example": "Eis ist ein Dessert. = Saldējums ir deserts."
        }
      ],
      "tip": {
        "text": "Mos harroni: ushqimi është → akull • Dimri/uji është → akull."
      },
      "important": [
        "Akullorja dhe xhelatina janë dy fjalë të ndryshme në letonisht, por në gjermanisht das Eis përdoren shpesh të dyja fjalët.",
        "Konteksti ka rëndësi: Ushqimi do të thotë akullore, një sipërfaqe e ftohtë ose ujë do të thotë akull."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "das Eis",
            "Eis"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Eis"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Eis"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Eis"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Eis"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Eis"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "das Eis"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "Eis"
              ]
            }
          },
          {
            "word": {
              "green": [
                "der Schnee"
              ]
            },
            "meaning": {
              "purple": [
                "bjerë"
              ]
            },
            "example": {
              "green": [
                "Schnee"
              ]
            }
          },
          {
            "word": {
              "green": [
                "kalt"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "kalt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "das Dessert"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "deser"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "das Eis"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "elf",
    "lv": "Njëmbëdhjetë!",
    "level": "A1"
  },
  {
    "de": "E-Mail",
    "de_article": "die",
    "de_plural": "die E-Mails",
    "lv": "E-Posta",
    "level": "A1"
  },
  {
    "de": "Eltern",
    "de_article": "die",
    "lv": "Jo, jo, Babi.",
    "level": "A1"
  },
  {
    "de": "Ende",
    "de_article": "das",
    "lv": "Mbarimi",
    "level": "A1"
  },
  {
    "de": "er",
    "lv": "/body> >",
    "level": "A1"
  },
  {
    "de": "Erbse",
    "de_article": "die",
    "de_plural": "die Erbsen",
    "lv": "Fasule.",
    "level": "A1"
  },
  {
    "de": "Erde",
    "de_article": "die",
    "de_plural": "die Erden",
    "lv": "Çfarë kati?",
    "level": "A1"
  },
  {
    "de": "erst",
    "lv": "Së pari • Vetëm",
    "level": "A1",
    "study": {
      "id": "a1-erst",
      "layout": "standardStudy",
      "translation": "Së pari • Vetëm",
      "explanation": "\"Së pari\" përdoret për të treguar sekuencën (së pari) ose për të theksuar se diçka ka ndodhur më vonë ose në një masë më të vogël (vetëm) sesa pritej.",
      "examples": [
        {
          "de": "Erst lernen, dann spielen.",
          "lv": "Pini së pari, pastaj vozitni."
        },
        {
          "de": "Ich komme erst morgen.",
          "lv": "Nuk do të jem atje deri nesër."
        },
        {
          "de": "Er ist erst 18 Jahre alt.",
          "lv": "Ai është vetëm 18 vjeç."
        },
        {
          "de": "Wir essen erst um acht Uhr.",
          "lv": "Ne nuk hamë deri në orën tetë."
        }
      ],
      "comparison": [
        {
          "word": "erst",
          "meaning": "Së pari • Vetëm",
          "example": "Erst lernen, dann spielen. – Ndalo së pari. = Së pari jep, makarona ndërpritet."
        },
        {
          "word": "zuerst",
          "meaning": "Fillestar • Fillimisht",
          "example": "Zuerst frühstücken wir. = Vispirms mēs brokastojam."
        },
        {
          "word": "nur",
          "meaning": "Më vjen keq.",
          "example": "Ich habe nur 5 Euro. = Man ir tikai 5 eiro."
        },
        {
          "word": "dann",
          "meaning": "Atëherë...",
          "example": "Dann gehen wir nach Hause. = Tad mēs ejam mājās."
        }
      ],
      "tip": {
        "text": "Pyetjet: laiks/skaits → erst • Daudzumlar → nur."
      },
      "accents": {
        "blue": [
          "erst",
          "Erst"
        ],
        "green": [
          "zuerst",
          "Zuerst"
        ],
        "yellow": [
          "nur"
        ],
        "red": [
          "dann",
          "Dann"
        ]
      },
      "sectionAccents": {
        "examples": [
          {
            "blue": [
              "Erst"
            ],
            "red": [
              "dann"
            ]
          },
          {
            "blue": [
              "erst"
            ]
          },
          {
            "blue": [
              "erst"
            ]
          },
          {
            "blue": [
              "erst"
            ]
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "erst"
              ]
            },
            "example": {}
          },
          {
            "word": {
              "green": [
                "zuerst"
              ]
            },
            "example": {
              "green": [
                "Zuerst"
              ]
            }
          },
          {
            "word": {
              "green": [
                "nur"
              ]
            },
            "example": {}
          },
          {
            "word": {
              "green": [
                "dann"
              ]
            },
            "example": {
              "red": [
                "Dann"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "erst"
            ],
            "yellow": [
              "nur"
            ]
          },
          {
            "yellow": [
              "nur"
            ]
          }
        ]
      },
      "important": [
        "Erst dhe Nur mund të tingëllojnë \"të drejtë\", por nuk janë e njëjta gjë.",
        "I pari shpesh flet për kohën, renditjen ose arritjen në një pikë. • Nur kufizon shumën."
      ]
    }
  },
  {
    "de": "erste",
    "lv": "I pari",
    "level": "A1"
  },
  {
    "de": "es",
    "lv": "Për • Për • Formularin jo-personal",
    "level": "A1",
    "study": {
      "id": "a1-es",
      "layout": "standardStudy",
      "translation": "Për • Për • Formularin jo-personal",
      "explanation": "\"Unë\" -ja gjermane nuk përdoret për të folur për veten. Përdoret për të treguar: këtë, atë, ose një formë jopersonale (moti, koha, fjali të ndryshme jopersonale).",
      "examples": [
        {
          "de": "Es regnet.",
          "lv": "Po mësoj gjermanisht."
        },
        {
          "de": "Es ist kalt.",
          "lv": "E lodhur."
        },
        {
          "de": "Das Kind schläft.",
          "lv": "Punon këtu."
        },
        {
          "de": "Es ist müde.",
          "lv": "Ky është libri im."
        },
        {
          "de": "Es regnet.",
          "lv": "Po bie shi!"
        },
        {
          "de": "Es schneit.",
          "lv": "Po bie borë. - Po."
        }
      ],
      "info": [
        "Łotewskie „es” = niemieckie „ich”",
        "Almanca \"I\" = bu • Yani • kişisel olmayan biçim"
      ],
      "tip": {
        "text": "Mos harroni: \"es\" në → letonisht, jo në gjermanisht."
      },
      "sectionAccents": {
        "examples": [
          {
            "blue": [
              "Ich"
            ]
          },
          {
            "blue": [
              "Er"
            ]
          },
          {
            "red": [
              "Sie"
            ]
          },
          {
            "yellow": [
              "Das"
            ]
          },
          {
            "blue": [
              "Es"
            ]
          },
          {
            "blue": [
              "Es"
            ]
          }
        ],
        "comparison": [
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          },
          {
            "word": {},
            "example": {}
          }
        ],
        "info": [
          {},
          {}
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {
            "red": [
              "Letonishtja"
            ],
            "blue": [
              "es"
            ]
          }
        ]
      },
      "important": [
        "Gjermanisht \"Unë\" jo letonisht \"Unë\".",
        "Letonishtja \"I\" është në gjermanisht. • Es në gjermanisht zakonisht do të thotë kjo/ajo ose jo e përkthyer."
      ],
      "comparison": [
        {
          "word": "es",
          "meaning": "tas • bezpersoniska forma",
          "example": "Es regnet. – Līst."
        },
        {
          "word": "ich",
          "meaning": "es (persona)",
          "example": "Ich lerne Deutsch. – Es mācos vācu valodu."
        }
      ]
    }
  },
  {
    "de": "Esslöffel",
    "de_article": "der",
    "de_plural": "die Esslöffel",
    "lv": "Lugën!",
    "level": "A1"
  },
  {
    "de": "etwas",
    "lv": "Diçka • Disa",
    "level": "A1",
    "study": {
      "id": "a1-etwas",
      "layout": "standardStudy",
      "translation": "Diçka • Disa",
      "explanation": [
        "Ideja kryesore: etwas do të thotë diçka ose pak, në varësi të kontekstit.",
        "Nëse zëvendëson diçka të panjohur, letonishtja zakonisht thotë diçka.",
        "Kur Etwas i paraprin një mbiemri ose sasie, zakonisht do të thotë pak.",
        "Etwas është një fjalë shumë e zakonshme dhe neutrale që fillon me shkronjën A1 në biseda.",
        "Dallimi më i rëndësishëm është: etwas kaufen = blej diçka, etwas mude = pak i lodhur."
      ],
      "examples": [
        {
          "de": "Ich möchte etwas trinken.",
          "lv": "Dua diçka për të pirë."
        },
        {
          "de": "Hast du etwas Zeit?",
          "lv": "Ke një minutë për mua?"
        },
        {
          "de": "Ich bin etwas müde.",
          "lv": "Jam disi e lodhur."
        },
        {
          "de": "Ich habe etwas für dich.",
          "lv": "Kam diqka per ty."
        },
        {
          "de": "Das ist etwas teuer.",
          "lv": "Është, um-- Është mjaft shtrenjtë."
        }
      ],
      "comparison": [
        {
          "word": "etwas",
          "meaning": "Diçka/pak",
          "example": "Ich brauche etwas. = Man kaut kas vajadzīgs."
        },
        {
          "word": "was",
          "meaning": "Diçka (bisedore)",
          "example": "Willst du was trinken? = Vai gribi kaut ko dzert?"
        },
        {
          "word": "ein bisschen",
          "meaning": "Ke fjetur ndopak?",
          "example": "Ich bin ein bisschen müde. = Es esmu mazliet noguris."
        },
        {
          "word": "nichts",
          "meaning": "Fije",
          "example": "Ich brauche nichts. = Man neko nevajag."
        }
      ],
      "tip": {
        "text": "Mos harroni: → diçka • → njëfarë shkalle."
      },
      "important": [
        "Etwas nuk është i njëjtë me nichts: etwas do të thotë diçka, por nichts nuk do të thotë asgjë.",
        "Në letonisht, diçka tingëllon më mirë se diçka, për shembull: etwas trinken = duke pirë një pije."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "etwas"
          ],
          "purple": [
            "kaufen"
          ],
          "green": [
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "etwas"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "etwas"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "etwas"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "etwas"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "etwas"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "etwas"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "was"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "ein bisschen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "ein bisschen"
              ]
            }
          },
          {
            "word": {
              "green": [
                "nichts"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "etwas"
            ],
            "red": [
              "nichts"
            ]
          },
          {
            "blue": [
              "etwas"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "euch",
    "lv": "Ti • Ti",
    "level": "A1",
    "study": {
      "id": "a1-euch",
      "layout": "standardStudy",
      "translation": "Ti • Ti",
      "explanation": "\"euch\" është përemri shumës i personit të dytë. Si plotësim i drejtpërdrejtë (ku?) - \"ti\" dhe plotësim i tërthortë (kujt?) - përdoren si \"madhësi\".",
      "examples": [
        {
          "de": "Ich sehe euch.",
          "lv": "Dhe une të shikoj."
        },
        {
          "de": "Ich helfe euch.",
          "lv": "Do ju jap mundësinë për t'iu afruar"
        },
        {
          "de": "Ich gebe euch das Buch.",
          "lv": "Po të jap një libër"
        },
        {
          "de": "Ich danke euch.",
          "lv": "Ju faleminderit."
        },
        {
          "de": "Ihr erinnert euch.",
          "lv": "Të kujtohet."
        }
      ],
      "comparison": [
        {
          "word": "ihr",
          "meaning": "Dukesh ...",
          "example": "Ihr seid freundlich. = Jūs esat draudzīgi."
        },
        {
          "word": "euch",
          "meaning": "Ti / ti",
          "example": "Ich helfe euch. = Es jums palīdzu."
        },
        {
          "word": "euer",
          "meaning": "-Fëmijët e tu.",
          "example": "Das ist euer Haus. = Tā ir jūsu māja."
        }
      ],
      "info": [
        "ihr = ty (podmiotowa forma zdania)",
        "euch = ty (gdzie? tworzysz) / ty (kogo? tworzysz)",
        "euer = seninki (iyelik formu)"
      ],
      "tip": {
        "text": "\"Euch\" është përgjigjja e pyetjes \"kujt?\" ose është plotësimi i drejtpërdrejtë i fjalive që përmbajnë \"ju\".",
        "example": "Sana yardım edeceğim. = İyiyim. seni görüyorum = Ich sehe euch. Sana söylüyorum. = Ich erzähle euch."
      },
      "accents": {
        "blue": [
          "ihr"
        ],
        "yellow": [
          "euch"
        ],
        "green": [
          "euer"
        ]
      }
    }
  },
  {
    "de": "euer",
    "lv": "-Fëmijët e tu.",
    "level": "A1"
  },
  {
    "de": "fahren",
    "lv": "Drive • Drive • Get",
    "level": "A1",
    "study": {
      "id": "a1-fahren",
      "layout": "standardStudy",
      "translation": "Drive • Drive • Get",
      "explanation": [
        "Ideja kryesore: fahrenheit do të thotë të ngasësh makinën, dhe në disa fjali gjithashtu do të thotë të largosh dikë.",
        "Fahrene përdoret kur udhëtoni me makinë, autobus, tren, biçikletë ose ndonjë automjet tjetër.",
        "Nëse subjekti i dënimit është një person, fahren mund të nënkuptojë udhëzimin dhe udhëheqjen.",
        "Nëse lëvizja zhvillohet në këmbë, zakonisht përdoret gehen ose laufen."
      ],
      "examples": [
        {
          "de": "Ich fahre nach Berlin.",
          "lv": "Po shkoj në Berlin."
        },
        {
          "de": "Ich fahre mit dem Auto.",
          "lv": "Jam me makinë."
        },
        {
          "de": "Ich fahre meine Tochter zur Schule.",
          "lv": "Do ta çoj kohën e vajzës në shkollë."
        },
        {
          "de": "Ich fahre dich nach Hause.",
          "lv": "Do të dërgoj në shtëpi."
        },
        {
          "de": "Wir fahren morgen nach München.",
          "lv": "Nesër do të shkojmë në Mynih."
        }
      ],
      "comparison": [
        {
          "word": "fahren",
          "meaning": "Transporti me makinë",
          "example": "Ich fahre mit dem Bus."
        },
        {
          "word": "gehen",
          "meaning": "Bredhës.",
          "example": "Ich gehe nach Hause."
        },
        {
          "word": "laufen",
          "meaning": "Vrapim/ecje",
          "example": "Er läuft schnell."
        },
        {
          "word": "bringen",
          "meaning": "Sill/dorëzo",
          "example": "Ich bringe das Buch."
        },
        {
          "word": "mitnehmen",
          "meaning": "Merre me vete",
          "example": "Ich nehme dich mit."
        }
      ],
      "tip": {
        "text": "Mos harroni: automjeti → fahren • → gehen në këmbë."
      },
      "important": {
        "text": "Fahren ≠ tikai “braukt”",
        "example": "Almanca'da aynı fiil genellikle bağlama bağlı olarak şu anlama gelir: git • git • al."
      },
      "accents": {
        "blue": [
          "fahren",
          "fahre"
        ],
        "purple": [
          "braukt",
          "braucu",
          "vest",
          "vedu",
          "aizvest"
        ],
        "green": [
          "transportu",
          "pojazd",
          "auto",
          "autobusu",
          "vilcienu",
          "rower"
        ],
        "yellow": [
          "gehen",
          "laufen"
        ],
        "red": [
          "bringen",
          "mitnehmen"
        ]
      },
      "sectionAccents": {
        "explanation": {
          "blue": [
            "fahren"
          ],
          "yellow": [
            "gehen",
            "laufen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "fahre"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "fahre"
              ],
              "green": [
                "Auto"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "fahre"
              ],
              "green": [
                "Tochter",
                "Schule"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "fahre"
              ],
              "green": [
                "dich",
                "Hause"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "fahren"
              ],
              "green": [
                "München"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "fahren"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "fahre"
              ]
            }
          },
          {
            "word": {
              "green": [
                "gehen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "gehe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "laufen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "läuft"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bringen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "bringe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "mitnehmen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "nehme",
                "mit"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "text": {},
            "example": {}
          }
        ]
      }
    }
  },
  {
    "de": "falsch",
    "lv": "\"agoraphobia.\" Saktë?",
    "level": "A1"
  },
  {
    "de": "Familie",
    "de_article": "die",
    "de_plural": "die Familien",
    "lv": "Familja.",
    "level": "A1"
  },
  {
    "de": "Nachname",
    "de_article": "der",
    "de_plural": "die Nachnamen",
    "lv": "Është e familjes...",
    "level": "A1"
  },
  {
    "de": "Farbe",
    "de_article": "die",
    "de_plural": "die Farben",
    "lv": "Ngjyrë.",
    "level": "A1"
  },
  {
    "de": "fast",
    "lv": "Pak a shumë.",
    "level": "A1"
  },
  {
    "de": "Februar",
    "de_article": "der",
    "lv": "SHKURT",
    "level": "A1"
  },
  {
    "de": "Fehler",
    "de_article": "der",
    "de_plural": "die Fehler",
    "lv": "\"Gabime.",
    "level": "A1"
  },
  {
    "de": "Fenster",
    "de_article": "das",
    "de_plural": "die Fenster",
    "lv": "Dritarja!",
    "level": "A1"
  },
  {
    "de": "Wochenende",
    "de_article": "das",
    "lv": "Fundjavë",
    "level": "A1",
    "de_plural": "die Wochenenden"
  },
  {
    "de": "Fernseher",
    "de_article": "der",
    "de_plural": "die Fernseher",
    "lv": "Televizor!",
    "level": "A1"
  },
  {
    "de": "fertig",
    "lv": "Gati!",
    "level": "A1"
  },
  {
    "de": "fett",
    "lv": "Vaj.",
    "level": "A1"
  },
  {
    "de": "Film",
    "de_article": "der",
    "de_plural": "die Filme",
    "lv": "Film!",
    "level": "A1"
  },
  {
    "de": "Filzstift",
    "de_article": "der",
    "de_plural": "die Filzstifte",
    "lv": "Shënues",
    "level": "A1"
  },
  {
    "de": "finden",
    "lv": "Gjej • Mendo",
    "level": "A1",
    "study": {
      "id": "a1-finden",
      "layout": "standardStudy",
      "translation": "Gjej • Mendo",
      "explanation": [
        "Ideja kryesore: të gjesh shpesh do të thotë të gjesh.",
        "Gjetja e tij gjatë një bisede shumë shpesh do të thotë gjithashtu të mendosh ose të mendosh për diçka.",
        "Kur bëhet fjalë për diçka që mungon, përkthehet si gjetje.",
        "Sa i përket bindjes, ajo përkthehet si mendim ose paraqitje."
      ],
      "examples": [
        {
          "de": "Ich finde meinen Schlüssel.",
          "lv": "Nuk mund ta gjej çelësin tim"
        },
        {
          "de": "Ich finde das gut.",
          "lv": "E gjete telefonin tënd?"
        },
        {
          "de": "Wie findest du den Film?",
          "lv": "... i përbashkët, nuk është vetëm vendimi i saj. - Mua më duket mirë..."
        },
        {
          "de": "Wie findest du den Film?",
          "lv": "Çfarë mendon për filmin?"
        }
      ],
      "comparison": [
        {
          "word": "finden",
          "meaning": "Gjej/mendo",
          "example": "Ich finde das gut. = Man tas šķiet labi."
        },
        {
          "word": "suchen",
          "meaning": "Kërko",
          "example": "Ich suche den Schlüssel. = Po gjej çëselsin."
        },
        {
          "word": "denken",
          "meaning": "Të menduarit",
          "example": "Seni düşünüyorum. = Po mendoj për ty."
        },
        {
          "word": "glauben",
          "meaning": "Beso/mendo",
          "example": "Ich glaube, er kommt. = Mendoj për të ardhmen tënde."
        }
      ],
      "tip": {
        "text": "Mos harroni: artikulli i humbur → u gjet • Vizioni → mendoj…"
      },
      "important": [
        "Gjetja nuk do të thotë vetëm \"gjej\".",
        "I finde das gut do të thotë \"Unë mendoj se kjo është e mirë\", jo \"Unë mendoj se kjo është e mirë.\""
      ],
      "sectionAccents": {
        "explanation": {},
        "examples": [
          {
            "de": {
              "blue": [
                "finde"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gefunden"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "finde"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "findest"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "finden"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "finde"
              ]
            }
          },
          {
            "word": {
              "green": [
                "suchen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "suche"
              ]
            }
          },
          {
            "word": {
              "green": [
                "denken"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "glauben"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "glaube"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "Gjetja"
            ]
          },
          {
            "blue": [
              "finde"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Finger",
    "de_article": "der",
    "de_plural": "die Finger",
    "lv": "Unaza.",
    "level": "A1"
  },
  {
    "de": "Fisch",
    "de_article": "der",
    "de_plural": "die Fische",
    "lv": "Peshq.",
    "level": "A1"
  },
  {
    "de": "Flasche",
    "de_article": "die",
    "de_plural": "die Flaschen",
    "lv": "Shishe",
    "level": "A1"
  },
  {
    "de": "Fleisch",
    "de_article": "das",
    "lv": "Mish.",
    "level": "A1"
  },
  {
    "de": "fliegen",
    "lv": "Mizë?",
    "level": "A1"
  },
  {
    "de": "Flugzeug",
    "de_article": "das",
    "de_plural": "die Flugzeuge",
    "lv": "Aeroplan.",
    "level": "A1"
  },
  {
    "de": "Fluss",
    "de_article": "der",
    "de_plural": "die Flüsse",
    "lv": "Lumi!",
    "level": "A1"
  },
  {
    "de": "Foto",
    "de_article": "das",
    "de_plural": "die Fotos",
    "lv": "Fotografia",
    "level": "A1"
  },
  {
    "de": "Frage",
    "de_article": "die",
    "de_plural": "die Fragen",
    "lv": "Pyetje.",
    "level": "A1"
  },
  {
    "de": "fragen",
    "lv": "Dashuria",
    "level": "A1"
  },
  {
    "de": "Frau",
    "de_article": "die",
    "de_plural": "die Frauen",
    "lv": "Gruaja • Gruaja",
    "level": "A1",
    "study": {
      "id": "a1-frau",
      "layout": "standardStudy",
      "translation": "Gruaja • Gruaja",
      "explanation": [
        "Ideja kryesore: Die Frau mund të nënkuptojë një grua (gjinore) ose një bashkëshort (partner).",
        "Nëse është vetëm gjinie ose personi, vdisni zonjë = grua.",
        "Në rastin e një bashkëshorti, vdes Gruaja = gruaja ime (meine Frau = gruaja ime).",
        "Përemri pronor (meine/deine/seine Frau) pothuajse gjithmonë do të thotë bashkëshortor.",
        "Plural: Die Frauen.",
        "Forma mashkullore, thotë Mann, ka të njëjtin kuptim të dyfishtë: mashkull dhe burrë."
      ],
      "examples": [
        {
          "de": "Sie ist eine nette Frau.",
          "lv": "Ajo është një grua e mirë."
        },
        {
          "de": "Das ist meine Frau.",
          "lv": "Ajo është gruaja ime."
        },
        {
          "de": "Wie viele Frauen sind hier?",
          "lv": "Sa gra janë atje?"
        },
        {
          "de": "Meine Frau arbeitet in Berlin.",
          "lv": "Gruaja ime punon në Berlin."
        },
        {
          "de": "Die Frau trägt ein Kleid.",
          "lv": "Gruaja ka veshur një fustan."
        },
        {
          "de": "Seine Frau ist Ärztin.",
          "lv": "Gruaja e tij është mjeke."
        }
      ],
      "tip": [
        "Përemri pronor (meine/deine/seine Frau) pothuajse gjithmonë do të thotë bashkëshort.",
        "Pa posedues (die Frau, eine Frau), zakonisht do të thotë një grua."
      ],
      "important": [
        "Die Frau = femër OSE BASHKË-KONTEKSTUALISHT e varur.",
        "Meine Frau = gruaja ime (jo \"gruaja ime\").",
        "Plural: Die Frauen."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "die Frau",
            "Frau"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Frau"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "meine Frau"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Frauen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Meine Frau"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Frau"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Seine Frau"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "green": [
              "meine",
              "deine",
              "seine Frau"
            ]
          },
          {
            "blue": [
              "die Frau",
              "eine Frau"
            ]
          }
        ],
        "important": [
          {},
          {
            "green": [
              "meine Frau"
            ]
          },
          {
            "blue": [
              "die Frauen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "frei",
    "lv": "Lirë!",
    "level": "A1"
  },
  {
    "de": "Freitag",
    "de_article": "der",
    "de_plural": "die Freitage",
    "lv": "E premte.",
    "level": "A1"
  },
  {
    "de": "Freund",
    "de_article": "der",
    "de_plural": "die Freunde",
    "lv": "Shok",
    "level": "A1"
  },
  {
    "de": "Freundin",
    "de_article": "die",
    "de_plural": "die Freundinnen",
    "lv": "Miq?",
    "level": "A1"
  },
  {
    "de": "freundlich",
    "lv": "E dashur.",
    "level": "A1"
  },
  {
    "de": "frisch",
    "lv": "E freskët.",
    "level": "A1"
  },
  {
    "de": "früh",
    "lv": "Është herët.",
    "level": "A1"
  },
  {
    "de": "Frühling",
    "de_article": "der",
    "de_plural": "die Frühlinge",
    "lv": "Pranverë.",
    "level": "A1"
  },
  {
    "de": "Frühstück",
    "de_article": "das",
    "lv": "Mëngjes.",
    "level": "A1",
    "de_plural": "die Frühstücke"
  },
  {
    "de": "frühstücken",
    "lv": "Ngrënia e mëngjesit",
    "level": "A1"
  },
  {
    "de": "fünf",
    "lv": "5, janë 5.",
    "level": "A1"
  },
  {
    "de": "fünfhundert",
    "lv": "Mblidhi keto. Pes qinde.",
    "level": "A1"
  },
  {
    "de": "fünfte",
    "lv": "-Tek e 5-ta.",
    "level": "A1"
  },
  {
    "de": "fünfzehn",
    "lv": "Pesëmbëdhjetë?",
    "level": "A1"
  },
  {
    "de": "fünfzehnte",
    "lv": "Pesëmbëdhjetë",
    "level": "A1"
  },
  {
    "de": "fünfzig",
    "lv": "50.",
    "level": "A1"
  },
  {
    "de": "fünfzigste",
    "lv": "E pesëdhjeta",
    "level": "A1"
  },
  {
    "de": "für",
    "lv": "Profesionistë.",
    "level": "A1",
    "study": {
      "id": "a1-fuer",
      "layout": "standardStudy",
      "translation": "Profesionistë.",
      "explanation": [
        "Ideja kryesore: für është një parafjalë që qeveris gjithmonë akuzuesin – zakonisht pi ose pi në letonisht.",
        "Kur flasim për blerësin ose qëllimin für = për (für dich = për ju).",
        "Duke folur për shkëmbim, tarifë ose përmbytje, für = për (danke für das Geschenk = faleminderit për dhuratën).",
        "Fur gjithmonë ka për një situatë akuzuese, pavarësisht nga kuptimi i saj."
      ],
      "examples": [
        {
          "de": "Das ist für dich.",
          "lv": "Për mua?"
        },
        {
          "de": "Danke für die Hilfe.",
          "lv": "Faleminderit për ndihmën."
        },
        {
          "de": "Ich kaufe ein Geschenk für meine Mutter.",
          "lv": "Po blej një dhuratë për nënën time."
        },
        {
          "de": "Was bezahlst du für das Auto?",
          "lv": "Sa paguani për një makinë?"
        },
        {
          "de": "Das Buch ist für Kinder.",
          "lv": "Libri është për fëmijë."
        },
        {
          "de": "Für heute ist das genug.",
          "lv": "Aq për sot."
        }
      ],
      "tip": [
        "Fur gjithmonë + inkriminuese-pavarësisht nga kuptimi.",
        "Blerësi/Qëllimi → • Modifikimi/Arsyeja/→Tarifa."
      ],
      "important": [
        "Für + Akkusativ gjithmonë, p.sh. für mich, für dich, für das Kind.",
        "Danke für /bezahlen für = \"për\", jo \"para\"."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "für"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "für"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "für"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "für"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "für"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "für"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Für"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "für"
            ]
          },
          {}
        ],
        "important": [
          {
            "blue": [
              "für",
              "Akkusativ"
            ]
          },
          {
            "green": [
              "danke für",
              "bezahlen für"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Fuß",
    "de_article": "der",
    "de_plural": "die Füße",
    "lv": "Këmbë.",
    "level": "A1"
  },
  {
    "de": "Fußball",
    "de_article": "der",
    "de_plural": "die Fußbälle",
    "lv": "Bukuroshe! Oh!",
    "level": "A1"
  },
  {
    "de": "ganz",
    "lv": "Të gjitha",
    "level": "A1"
  },
  {
    "de": "Garage",
    "de_article": "die",
    "de_plural": "die Garagen",
    "lv": "Garazh",
    "level": "A1"
  },
  {
    "de": "Garten",
    "de_article": "der",
    "de_plural": "die Gärten",
    "lv": "...kopshti...",
    "level": "A1"
  },
  {
    "de": "Gast",
    "de_article": "der",
    "de_plural": "die Gäste",
    "lv": "Vizitor",
    "level": "A1"
  },
  {
    "de": "geben",
    "lv": "Jep",
    "level": "A1",
    "study": {
      "id": "a1-geben",
      "layout": "standardStudy",
      "translation": "Jep",
      "explanation": [
        "Ideja kryesore: do të thotë të lindësh.",
        "Kur njëri i jep diçka tjetrit, ne përdorim Gebenin.",
        "Ky është drejtimi i kundërt i Nehmenit.",
        "Të bëhesh bektashian do të thotë të jesh marrës i diçkaje."
      ],
      "examples": [
        {
          "de": "Gib mir bitte das Buch.",
          "lv": "Më jep librin, të lutem"
        },
        {
          "de": "Ich gebe dir meine Nummer.",
          "lv": "Të jap numrin tim"
        },
        {
          "de": "Ich nehme das Buch.",
          "lv": "Po e marr librin"
        },
        {
          "de": "Ich bekomme ein Geschenk.",
          "lv": "Po marr një dhuratë"
        }
      ],
      "comparison": [
        {
          "word": "geben",
          "meaning": "Jep",
          "example": "Gib mir das Buch."
        },
        {
          "word": "nehmen",
          "meaning": "Merre atë.",
          "example": "Ich nehme das Buch."
        },
        {
          "word": "bekommen",
          "meaning": "Merr/merr",
          "example": "Ich bekomme ein Geschenk."
        },
        {
          "word": "bringen",
          "meaning": "Sill/dorëzo",
          "example": "Ich bringe dir das Buch."
        }
      ],
      "tip": {
        "text": "Mos harroni: lëreni → shtatzënë • Merreni për vete → nehmen."
      },
      "important": [
        "Gebeni dhe Nehmeni janë drejtime të kundërta.",
        "Bekommen do të thotë të marrësh, jo të japësh."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "geben"
          ],
          "red": [
            "nehmen",
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Gib"
              ],
              "yellow": [
                "Buch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gebe"
              ],
              "yellow": [
                "Nummer"
              ]
            },
            "lv": {
              "yellow": [
                "jap"
              ]
            }
          },
          {
            "de": {
              "red": [
                "nehme"
              ],
              "yellow": [
                "Buch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "bekomme"
              ],
              "yellow": [
                "Geschenk"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "geben"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "Gib"
              ]
            }
          },
          {
            "word": {
              "green": [
                "nehmen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "nehme"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bekommen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "bringen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "bringe"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "geben"
            ],
            "red": [
              "nehmen"
            ]
          },
          {
            "yellow": [
              "bekommen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "geboren",
    "lv": ".",
    "level": "A1"
  },
  {
    "de": "gefallen",
    "lv": "Më pëlqen",
    "level": "A1"
  },
  {
    "de": "gegen",
    "lv": "Vs",
    "level": "A1"
  },
  {
    "de": "gehen",
    "lv": "Të largohesh?",
    "level": "A1"
  },
  {
    "de": "gelb",
    "lv": "Sarı",
    "level": "A1"
  },
  {
    "de": "Geld",
    "de_article": "das",
    "lv": "Paraja",
    "level": "A1"
  },
  {
    "de": "genug",
    "lv": "Mjaftueshëm",
    "level": "A1"
  },
  {
    "de": "geradeaus",
    "lv": "Drejtë, po.",
    "level": "A1"
  },
  {
    "de": "gern",
    "lv": "Asgjë.",
    "level": "A1"
  },
  {
    "de": "Geschichte",
    "de_article": "die",
    "de_plural": "die Geschichten",
    "lv": "Data • Data",
    "level": "A1"
  },
  {
    "de": "Geschwister",
    "de_article": "die",
    "lv": "Vëllezër e motra.",
    "level": "A1"
  },
  {
    "de": "Gesicht",
    "de_article": "das",
    "de_plural": "die Gesichter",
    "lv": "100.",
    "level": "A1"
  },
  {
    "de": "gestern",
    "lv": "Për dje. - Mm-hmm.",
    "level": "A1"
  },
  {
    "de": "gesund",
    "lv": "Ai është i shëndetshëm.",
    "level": "A1"
  },
  {
    "de": "Gesundheit",
    "de_article": "die",
    "lv": "Shëndetësia",
    "level": "A1"
  },
  {
    "de": "Getränk",
    "de_article": "das",
    "de_plural": "die Getränke",
    "lv": "Një gotë ujë",
    "level": "A1"
  },
  {
    "de": "Giraffe",
    "de_article": "die",
    "de_plural": "die Giraffen",
    "lv": "Gjirafë",
    "level": "A1"
  },
  {
    "de": "Glas",
    "de_article": "das",
    "de_plural": "die Gläser",
    "lv": "Gota.",
    "level": "A1"
  },
  {
    "de": "glauben",
    "lv": "Të besosh",
    "level": "A1"
  },
  {
    "de": "gleich",
    "lv": "Menjëherë • E barabartë",
    "level": "A1",
    "study": {
      "id": "a1-gleich",
      "layout": "standardStudy",
      "translation": "Menjëherë • E barabartë",
      "explanation": [
        "Ideja kryesore: gleich do të thotë përkohësisht menjëherë, relativisht në mënyrë të barabartë.",
        "Sa për kohën, gleich = menjëherë/menjëherë (ich komme gleich. = Do të vij menjëherë.).",
        "Për krahasim, gleich = e barabartë/e njëjtë (die gleiche Farbe = e njëjta ngjyrë).",
        "Gleich mund të përdoret gjithashtu si një parafjalë dative (gleich mir = si unë).",
        "Konteksti (koha ose krahasimi) tregon kuptimin e saktë."
      ],
      "examples": [
        {
          "de": "Ich komme gleich.",
          "lv": "15 sekonda. - Menjeher do vije."
        },
        {
          "de": "Wir haben die gleiche Farbe.",
          "lv": "Kemi të njëjtën ngjyrë."
        },
        {
          "de": "Das Essen ist gleich fertig.",
          "lv": "Darka do të jetë gati së shpejti."
        },
        {
          "de": "Beide Wege sind gleich lang.",
          "lv": "Të dyja rrugët janë me gjatësi të njëjtë."
        },
        {
          "de": "Bis gleich!",
          "lv": "Do shihemi se shpejti!"
        },
        {
          "de": "Sie sind gleich groß.",
          "lv": "Ato janë në të njëjtën lartësi."
        }
      ],
      "tip": [
        "Papritmas (papritmas) → tani.",
        "E njëjta (e → njëjta) për krahasim."
      ],
      "important": [
        "Gleich = e menjëhershme (koha) OSE e barabartë (krahasimi).",
        "Përsëri! = Shihemi së shpejti! – një frazë popullore ndarjeje."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "gleich"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "gleich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "gleiche"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gleich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "gleich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gleich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "gleich"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {},
          {}
        ]
      }
    }
  },
  {
    "de": "Glück",
    "de_article": "das",
    "lv": "\"Lumturi.\"",
    "level": "A1"
  },
  {
    "de": "glücklich",
    "lv": "Dukeshe e lumtur.",
    "level": "A1"
  },
  {
    "de": "Gramm",
    "de_article": "das",
    "de_plural": "die Gramm",
    "lv": "Gram",
    "level": "A1"
  },
  {
    "de": "Grammatik",
    "de_article": "die",
    "de_plural": "die Grammatiken",
    "lv": "Gramatikë",
    "level": "A1"
  },
  {
    "de": "Gras",
    "de_article": "das",
    "de_plural": "die Gräser",
    "lv": "Barishte.",
    "level": "A1"
  },
  {
    "de": "grau",
    "lv": "Gri",
    "level": "A1"
  },
  {
    "de": "groß",
    "lv": "Major.",
    "level": "A1",
    "study": {
      "id": "a1-gross-study",
      "layout": "standardStudy",
      "translation": "Major.",
      "explanation": [
        "Ideja kryesore: Madhësia e madhe ose për person - madhësia e gjatë.",
        "Groß do të thotë para së gjithash madhësi e madhe.",
        "Zakonisht karakterizohet nga: madhësia e përgjithshme.",
        "Groß nënkupton mbi të gjitha rritjen njerëzore.",
        "Zakonisht përcakton: një person.",
        "Groß në përgjithësi përcakton madhësinë ose gjatësinë e një personi."
      ],
      "examples": [
        {
          "de": "Das Haus ist groß.",
          "lv": "Shtëpia është e madhe."
        },
        {
          "de": "Berlin ist eine große Stadt.",
          "lv": "Shtëpia është e madhe."
        },
        {
          "de": "Er ist groß.",
          "lv": "Rezervova dhe pak cinik,"
        },
        {
          "de": "Das Zimmer ist groß.",
          "lv": "Dhoma është e madhe."
        }
      ],
      "tip": [
        "Bruto = e madhe",
        "Përdorni groß kur konteksti përshtatet me të kuptuarit."
      ],
      "important": [
        "E shpifur për një person do të thotë e gjatë.",
        "Bruto = e madhe.",
        "Madhësia e madhe ose për një person - madhësia e lartë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "groß"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "groß"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "groß"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "groß"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "groß"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "Großeltern",
    "de_article": "die",
    "lv": "Gjyshërit",
    "level": "A1"
  },
  {
    "de": "Großmutter",
    "de_article": "die",
    "de_plural": "die Großmütter",
    "lv": "Gjyshe !",
    "level": "A1"
  },
  {
    "de": "Großvater",
    "de_article": "der",
    "de_plural": "die Großväter",
    "lv": "Gjysh!",
    "level": "A1"
  },
  {
    "de": "grün",
    "lv": "E gjelber.",
    "level": "A1"
  },
  {
    "de": "Gruppe",
    "de_article": "die",
    "de_plural": "die Gruppen",
    "lv": "Grupi",
    "level": "A1"
  },
  {
    "de": "Gruß",
    "de_article": "der",
    "de_plural": "die Grüße",
    "lv": "Jam mirë.",
    "level": "A1"
  },
  {
    "de": "grüßen",
    "lv": "Mirë se ju gjeta.",
    "level": "A1"
  },
  {
    "de": "Gurke",
    "de_article": "die",
    "de_plural": "die Gurken",
    "lv": "Kastravec",
    "level": "A1"
  },
  {
    "de": "gut",
    "lv": "Mirë",
    "level": "A1",
    "study": {
      "id": "a1-gut-study",
      "layout": "standardStudy",
      "translation": "Mirë",
      "explanation": [
        "Ideja kryesore: zorra është mbiemër/ndajfolje – e mirë, e suksesshme, në rregull.",
        "Zorra përshkruan cilësinë, shëndetin ose si është diçka (Es geht mir gut. = Jam mirë.).",
        "Në shprehjen e sjellshme, zorra guten Tag/Abend/Morgen ndryshon fundin e saj pas shkrepjes.",
        "Nëse zorrët përcaktojnë një folje, është një ndajfolje (gut schwimmen = not i mirë).",
        "Të mos ngatërrohet me Das Gut • Është një emër i shkruar me shkronja të mëdha dhe artikuj (pronë, rezidencë)."
      ],
      "examples": [
        {
          "de": "Das Essen ist gut.",
          "lv": "Ushqimi është i mirë."
        },
        {
          "de": "Wie geht es dir? – Gut, danke!",
          "lv": "Si je - mirë, faleminderit!"
        },
        {
          "de": "Er spricht gut Deutsch.",
          "lv": "Ai flet mirë gjermanisht."
        },
        {
          "de": "Guten Morgen!",
          "lv": "Mirëmëngjes."
        },
        {
          "de": "Das ist eine gute Idee.",
          "lv": "-Kjo është ide e mirë."
        },
        {
          "de": "Alles ist gut.",
          "lv": "Gjithcka eshte ne rregull."
        }
      ],
      "tip": [
        "Zorra pa artikullin është mbiemër/ndajfolje - e mirë/e mirë.",
        "Das Gut me shkronja të mëdha dhe artikuj është një fjalë krejtësisht e ndryshme - një emër (pronë, rezidencë)."
      ],
      "important": [
        "Cerma = e mirë/e mirë (mbiemër/mbiemër).",
        "Das Gut = pronë/rezidencë (emri) – të mos ngatërrohet me zorrën.",
        "Guten Tag/Morgen/Abend – lezione të zorrëve që përfundojnë pas nxjerrjes."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "gut"
          ],
          "green": [
            "das Gut"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "gut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Gut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Guten"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gute"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gut"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "gut"
            ]
          },
          {
            "green": [
              "das Gut"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "Cerma"
            ]
          },
          {
            "green": [
              "das Gut"
            ]
          },
          {
            "blue": [
              "Guten Tag",
              "Morgen",
              "Abend"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Haar",
    "de_article": "das",
    "de_plural": "die Haare",
    "lv": "Flokë.",
    "level": "A1"
  },
  {
    "de": "haben",
    "lv": "Po, e kam.",
    "level": "A1",
    "study": {
      "id": "a1-haben",
      "layout": "standardStudy",
      "translation": "Po, e kam.",
      "explanation": [
        "Ideja kryesore: haben do të thotë që dikush ka ose mund të ketë qasje në diçka.",
        "Struktura dative letoneze \"I have/you have\" është e thjeshtë + haben në gjermanisht: Ich habe…, Du have…, Er hat… - * not mir habe.",
        "Situata akuzuese vijon Haben: Ich habe ein Auto. = Kam makinë.",
        "Haben përdoret gjithashtu si folje ndihmëse në kohën e përsosur: Ich habe gelernt."
      ],
      "examples": [
        {
          "de": "Ich habe ein Auto.",
          "lv": "Kam edhe një makinë tjetër."
        },
        {
          "de": "Hast du Zeit?",
          "lv": "Ke një sekondë?"
        },
        {
          "de": "Wir haben Hunger.",
          "lv": "Jemi të uritur."
        },
        {
          "de": "Ich habe das gemacht.",
          "lv": "Bëra..."
        }
      ],
      "comparison": [
        {
          "word": "haben",
          "meaning": "Po, e kam.",
          "example": "Ich habe Zeit. = Man ir laiks."
        },
        {
          "word": "sein",
          "meaning": "Të jesh",
          "example": "Ich bin hier. = Es esmu šeit."
        },
        {
          "word": "bekommen",
          "meaning": "Për të marrë",
          "example": "Ich bekomme ein Geschenk. = Es saņemu dāvanu."
        },
        {
          "word": "machen",
          "meaning": "Do / do",
          "example": "Ich mache das. = Es to daru."
        }
      ],
      "tip": {
        "text": "Atceries: Ich habe → man ir."
      },
      "important": [
        "Letonisht \"my\" = Gjermanisht Ich habe + akuzues. Mos përdorni gjendjen dative: false *mir habe.",
        "Me sein dhe datif: Mir ist kalt. = Kam ftohtë. (jo ti!)",
        "Shkëlqyeshëm: Ich habe gelernt = Mësova."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "haben",
            "Ich habe"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "habe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Hast"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "haben"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "habe"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "haben"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "sein"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "bekommen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "machen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "mache"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "habe"
            ]
          },
          {
            "blue": [
              "sein"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "halb",
    "lv": "Festë",
    "level": "A1"
  },
  {
    "de": "Hälfte",
    "de_article": "die",
    "de_plural": "die Hälften",
    "lv": "Festë",
    "level": "A1"
  },
  {
    "de": "Hals",
    "de_article": "der",
    "de_plural": "die Hälse",
    "lv": "Qafa",
    "level": "A1"
  },
  {
    "de": "halten",
    "lv": "Mbaje • Ndalo",
    "level": "A1",
    "study": {
      "id": "a1-halten",
      "layout": "standardStudy",
      "translation": "Mbaje • Ndalo",
      "explanation": [
        "Ideja kryesore: Do të thotë mbajtja e helmetës, por gjithashtu mund të nënkuptojë ndalimin ose pauzën në rastin e transportit ose lëvizjes.",
        "Kur keni një objekt në dorë, zakonisht duhet të mbahet nga aureola.",
        "Ndalimi në autobus, tren ose makinë zakonisht do të thotë ndalim.",
        "Në fjalinë ich halte das für... do të thotë të konsiderosh si."
      ],
      "examples": [
        {
          "de": "Ich halte die Tasche.",
          "lv": "E mbaj çantën"
        },
        {
          "de": "Der Bus hält hier.",
          "lv": "Autobusi ndalon këtu."
        },
        {
          "de": "Bitte halten Sie an.",
          "lv": "Ju lutem ndaluni."
        },
        {
          "de": "Ich halte das für richtig.",
          "lv": "Mendoj se është e drejtë."
        }
      ],
      "comparison": [
        {
          "word": "halten",
          "meaning": "Mbaje/ndalo",
          "example": "Der Bus hält. = Autobuss pietur."
        },
        {
          "word": "nehmen",
          "meaning": "-Blyej.",
          "example": "Ich nehme die Tasche. = Es ņemu somu."
        },
        {
          "word": "anhalten",
          "meaning": "Ndalo!",
          "example": "Bitte halten Sie an. = Lūdzu, apstājieties."
        },
        {
          "word": "denken",
          "meaning": "Të menduarit",
          "example": "Ich denke, das ist richtig. = Es domāju, ka tas ir pareizi."
        }
      ],
      "tip": {
        "text": "Mos harroni: Jashtë → kontrollit • → Ndalesa (t) e transportit."
      },
      "important": [
        "Halten nuk do të thotë vetëm \"prit\". Në transport, kjo zakonisht do të thotë ndalim.",
        "Ich halte das für... është një deklaratë e opinionit: \"Unë e shoh këtë si...\"."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "halte"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "halte"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "hält"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "halten"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "halte"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "halten"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "nehmen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "nehme"
              ]
            }
          },
          {
            "word": {
              "green": [
                "stoppen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "denken"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "halten"
            ]
          },
          {
            "blue": [
              "halte"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Haltestelle",
    "de_article": "die",
    "de_plural": "die Haltestellen",
    "lv": "Ndalo!",
    "level": "A1"
  },
  {
    "de": "Hand",
    "de_article": "die",
    "de_plural": "die Hände",
    "lv": "Dora",
    "level": "A1"
  },
  {
    "de": "Handschuh",
    "de_article": "der",
    "de_plural": "die Handschuhe",
    "lv": "Dorëza",
    "level": "A1"
  },
  {
    "de": "Handtasche",
    "de_article": "die",
    "de_plural": "die Handtaschen",
    "lv": "Ndeshje.",
    "level": "A1"
  },
  {
    "de": "Handtuch",
    "de_article": "das",
    "de_plural": "die Handtücher",
    "lv": "Peshqir!",
    "level": "A1"
  },
  {
    "de": "Handy",
    "de_article": "das",
    "de_plural": "die Handys",
    "lv": "Xhepi",
    "level": "A1"
  },
  {
    "de": "Hauptstadt",
    "de_article": "die",
    "de_plural": "die Hauptstädte",
    "lv": "Kapital.",
    "level": "A1"
  },
  {
    "de": "Heft",
    "de_article": "das",
    "de_plural": "die Hefte",
    "lv": "Gol!",
    "level": "A1"
  },
  {
    "de": "heiraten",
    "lv": "Harroje atë... vetëm martohu me mua.",
    "level": "A1"
  },
  {
    "de": "heiß",
    "lv": "E nxehtë.",
    "level": "A1"
  },
  {
    "de": "heißen",
    "lv": "Quaje veten • Dua të them",
    "level": "A1",
    "study": {
      "id": "a1-heißen",
      "layout": "standardStudy",
      "translation": "Quaje veten • Dua të them",
      "explanation": [
        "Ideja kryesore: heißen përdoret më së shumti për të thënë emrin e dikujt.",
        "Ich heiße... do të thotë \"emri im është...\".",
        "Me fjalë ose fraza, heißen mund të nënkuptojë edhe mesataren.",
        "Shprehja më e rëndësishme në nivelin A1 ishte Wie heißt?"
      ],
      "examples": [
        {
          "de": "Ich heiße Anna.",
          "lv": "Më quajnë Ana."
        },
        {
          "de": "Wie heißt du?",
          "lv": "-Emri juaj?"
        },
        {
          "de": "Wie heißt das auf Deutsch?",
          "lv": "Si quhet në gjermanisht?"
        },
        {
          "de": "Was heißt das?",
          "lv": "Çfarë do të thotë kjo?"
        }
      ],
      "comparison": [
        {
          "word": "heißen",
          "meaning": "Duke u thirrur/etiketuar",
          "example": "Ich heiße Anna. = Mani sauc Anna."
        },
        {
          "word": "nennen",
          "meaning": "Kërko/emri",
          "example": "Er nennt mich Tom. = Viņš mani sauc par Tomu."
        },
        {
          "word": "bedeuten",
          "meaning": "Mos e harroni.",
          "example": "Was bedeutet das? = Ko tas nozīmē?"
        },
        {
          "word": "rufen",
          "meaning": "Kërko",
          "example": "Ich rufe dich. = Es tevi pasaucu."
        },
        {
          "word": "anrufen",
          "meaning": "zvanīt",
          "example": "Ich rufe dich an. = Es tev piezvanu."
        }
      ],
      "tip": {
        "text": "Atceries: Ich heiße... → mani sauc..."
      },
      "important": [
        "Pra, çfarë ndodhi? Nuk do të thotë fjalë për fjalë \"Si e ke emrin?\" por \"Si e ke emrin?\"",
        "A ishte edhe kjo? Zakonisht do të thotë, \"Çfarë do të thotë kjo?\""
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "heißen",
            "Ich heiße",
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "heiße"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "heißt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "heißt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "heißt"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "heißen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "heiße"
              ]
            }
          },
          {
            "word": {
              "green": [
                "nennen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "nennt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bedeuten"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "Bedutet"
              ]
            }
          },
          {
            "word": {
              "green": [
                "rufen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "rufe"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {}
        ]
      }
    }
  },
  {
    "de": "helfen",
    "lv": "Më ndihmo.",
    "level": "A1"
  },
  {
    "de": "Hemd",
    "de_article": "das",
    "de_plural": "die Hemden",
    "lv": "Këmishën.",
    "level": "A1"
  },
  {
    "de": "Herbst",
    "de_article": "der",
    "de_plural": "die Herbste",
    "lv": "Vjeshta!",
    "level": "A1"
  },
  {
    "de": "Herr",
    "de_article": "der",
    "de_plural": "die Herren",
    "lv": "Dukesh ...",
    "level": "A1"
  },
  {
    "de": "heute",
    "lv": "Ju e kishit mundësinë tuaj.",
    "level": "A1"
  },
  {
    "de": "hier",
    "lv": "Këtu është!",
    "level": "A1"
  },
  {
    "de": "Hilfe",
    "de_article": "die",
    "de_plural": "die Hilfen",
    "lv": "Më ndihmo.",
    "level": "A1"
  },
  {
    "de": "hinter",
    "lv": "Për",
    "level": "A1"
  },
  {
    "de": "hoch",
    "lv": "Mirë.",
    "level": "A1",
    "study": {
      "id": "a1-hoch-study",
      "layout": "standardStudy",
      "translation": "Mirë.",
      "explanation": [
        "Ideja kryesore: vertikale, horizontale ose e lartë në lartësi.",
        "Hoch do të thotë, mbi të gjitha, rritje e madhe.",
        "Zakonisht karakterizohet nga: lartësia vertikale.",
        "Hoch në thelb do të thotë: nivel i lartë.",
        "Zakonisht karakterizohet nga: çmimet/numrat.",
        "Hoch përcakton lartësinë, drejtimin vertikal ose horizontal."
      ],
      "examples": [
        {
          "de": "Der Berg ist hoch.",
          "lv": "Pjesa e sipërme është e lartë."
        },
        {
          "de": "Das Regal ist zwei Meter hoch.",
          "lv": "Pjesa e sipërme është e lartë."
        },
        {
          "de": "Die Miete ist hoch.",
          "lv": "Qiratë janë të larta."
        },
        {
          "de": "Die Mauer ist hoch.",
          "lv": "Muri është i lartë."
        },
        {
          "de": "Die Preise sind hoch.",
          "lv": "Çmimet janë të larta."
        }
      ],
      "tip": [
        "Hoh = i lartë",
        "Përdorni hoch kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Hoch përdoret shpesh për të treguar çmimet dhe nivelet.",
        "Hoh = i lartë.",
        "Vertikal, horizontal ose i gjatë në lartësi."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "hoch"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "hoch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "hoch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "hoch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "hoch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "hoch"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "hoch"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "höflich",
    "lv": "Mirë",
    "level": "A1"
  },
  {
    "de": "hören",
    "lv": "Dëgjoni, dëgjoni.",
    "level": "A1",
    "study": {
      "id": "a1-hoeren-study",
      "layout": "standardStudy",
      "translation": "Dëgjoni, dëgjoni.",
      "explanation": [
        "Ideja kryesore: dëgjimi i tingujve ose dëgjimi i muzikës.",
        "Hören do të thotë mbi të gjitha: të perceptosh tingullin.",
        "Zakonisht karakterizohet nga: tingujt.",
        "Hören • Përdoret për të përshkruar tingujt, muzikën dhe atë dëgjohet."
      ],
      "examples": [
        {
          "de": "Ich höre Musik.",
          "lv": "Dëgjoj muzikë."
        },
        {
          "de": "Die Kinder hören eine Geschichte.",
          "lv": "Fëmijët dëgjojnë histori."
        },
        {
          "de": "Ich höre dich.",
          "lv": "Po të dëgjoj."
        }
      ],
      "tip": [
        "Dëgjoni tingullin ose dëgjoni muzikën.",
        "Përdorni orë kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Hören = Dëgjimi/dëgjimi i zërit.",
        "Dëgjoni tingullin ose dëgjoni muzikën."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "hören"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "höre"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "hören"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "höre"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {
            "blue": [
              "hören"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "hübsch",
    "lv": "E rregullt • E bukur",
    "level": "A1"
  },
  {
    "de": "hundert",
    "lv": "100.",
    "level": "A1"
  },
  {
    "de": "hungrig",
    "lv": "Issalcis",
    "level": "A1"
  },
  {
    "de": "ich",
    "lv": "Unë e bëra.",
    "level": "A1"
  },
  {
    "de": "ihr",
    "lv": "Ti • O",
    "level": "A1",
    "study": {
      "id": "a1-ihr",
      "layout": "standardStudy",
      "translation": "Ti • O",
      "explanation": [
        "Ideja kryesore: ihr janë dy përemra të ndryshëm me të njëjtën drejtshkrim - u drejtohet disa njerëzve (ju) dhe është forma dative e përemrit sie (o).",
        "Kur shkronja e vogël ihr përdoret si adresë, ajo mund të përkthehet si ju në më shumë se një person (Kommt ihr mit? = A do të vish me ne?).",
        "Si përemër pronor, Ihr do të thotë atë (Ihr Buch = libri i tij).",
        "Forma dative e ihr (nga sie) është për të (ich shtatzënë ihr das Buch. = I jap një libër.)",
        "Forma foljore (kommt, habt) tregon se kjo ka të bëjë me ju - u bën thirrje disa njerëzve.",
        "Një adresë e sjellshme është gjithmonë e kapitalizuar, SIE, jo IHR."
      ],
      "examples": [
        {
          "de": "Kommt ihr heute Abend?",
          "lv": "Domethënë, do të vjen sonte?"
        },
        {
          "de": "Ich gebe ihr das Buch.",
          "lv": "I jap librin."
        },
        {
          "de": "Wo wohnt ihr?",
          "lv": "Vendi ku jeton"
        },
        {
          "de": "Er schreibt ihr einen Brief.",
          "lv": "Ajo i shkruan një letër."
        },
        {
          "de": "Habt ihr Zeit?",
          "lv": "Ke një sekondë?"
        },
        {
          "de": "Das ist ihr Auto.",
          "lv": "Kjo është makina e tij."
        }
      ],
      "tip": [
        "IHR përdoret në lidhje me foljen dsk. formë (kommt, habt) = ju • Ihr = o pranë një fjale si një gjendje ose mbiemër posedues.",
        "Kontrolli Habt ihr...? / Kommt ihr...? = ti • Unë jam shtatzënë ihr... / ihr Buch = për të."
      ],
      "important": [
        "Ihr = ju (duke iu drejtuar disa njerëzve) OSE o (dative) OSE o (qiramarrës) – në varësi të kontekstit.",
        "Në një adresë të sjellshme, ne gjithmonë shkruajmë në kapital SIE në vend të ihr.",
        "E gabuar: Ihr (me mirësjellje) → E saktë: Gusht."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "ihr"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "ihr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ihr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ihr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ihr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ihr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ihr"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "ihr"
            ]
          },
          {
            "blue": [
              "ihr"
            ]
          }
        ],
        "important": [
          {},
          {
            "blue": [
              "Sie"
            ]
          },
          {
            "red": [
              "Ihr"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "im",
    "lv": "Ku është ajo?",
    "level": "A1",
    "study": {
      "id": "a1-im",
      "layout": "standardStudy",
      "translation": "Ku është ajo?",
      "explanation": [
        "Im është shkurtimi i parafjalës në dhe artikullit dem.",
        "Formati i plotë: në dem (në?).",
        "Ku përdoret në lidhje me emrat mashkullorë dhe emrat e çdo gjinie kur i përgjigjeni pyetjes - vendndodhja.",
        "Me kohë dhe stinë: në janar, verë, dimër.",
        "Në praktikë, im përdoret pothuajse gjithmonë në vend të plotë në demo."
      ],
      "examples": [
        {
          "de": "Ich bin im Park.",
          "lv": "Jam në park"
        },
        {
          "de": "Wir wohnen im Zentrum.",
          "lv": "Jetojmë në qendër të qytetit."
        },
        {
          "de": "Im Sommer ist es warm.",
          "lv": "Në verë bën vapë."
        },
        {
          "de": "Er arbeitet im Büro.",
          "lv": "Ai punon në një zyrë."
        },
        {
          "de": "Das Kind spielt im Garten.",
          "lv": "Një fëmijë po luan në kopsht."
        },
        {
          "de": "Im Januar fahre ich nach Wien.",
          "lv": "Shkova në Vjenë në janar."
        },
        {
          "de": "Sie ist im Kino.",
          "lv": "Ai është në kinema."
        },
        {
          "de": "Wir treffen uns im Restaurant.",
          "lv": "Takohemi në restorant."
        }
      ],
      "comparison": [
        {
          "word": "im",
          "meaning": "Brenda ku? (kujt?)",
          "example": "im Park – Në park"
        },
        {
          "word": "ins",
          "meaning": "Brenda, për ku? (Ak.)",
          "example": "ins Kino – Në kinema"
        },
        {
          "word": "in",
          "meaning": "Hyrje / dalje (nuk ka artikuj)",
          "example": "in Berlin – Në Berlin"
        },
        {
          "word": "am",
          "meaning": "Në, ku?",
          "example": "am Fenster – Pranë dritares"
        },
        {
          "word": "auf",
          "meaning": "Nga jashtë",
          "example": "auf dem Tisch – Shishja është në tryezë."
        }
      ],
      "tip": [
        "Mos harroni: in + dem → im (kujt?, ku?).",
        "Ku → në? • Ku në? - → Mos i ngatërro këto të dyja!"
      ],
      "important": [
        "Im = in dem, vetëm mashkullor ose me emër neutral, për kë? në xhirime.",
        "Përgjigjet e pyetjeve: ku? dhe ku jo? — vendndodhja, jo lëvizja.",
        "Sipas muajve dhe stinëve: im März, im Herbst.",
        "Për gratë: in der Schule, not im Schule."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "im"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "im"
              ]
            },
            "lv": {
              "purple": [
                "Jam"
              ]
            }
          },
          {
            "de": {
              "blue": [
                "im"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Im"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "im"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "im"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Im"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "im"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "im"
              ]
            },
            "lv": {
              "purple": [
                "restorant"
              ]
            }
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "im"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "im Park"
              ]
            }
          },
          {
            "word": {
              "green": [
                "ins"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "ins Kino"
              ]
            }
          },
          {
            "word": {
              "green": [
                "in"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "in Berlin"
              ]
            }
          },
          {
            "word": {
              "green": [
                "am"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "am Fenster"
              ]
            }
          },
          {
            "word": {
              "green": [
                "auf"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "auf dem Tisch"
              ]
            }
          }
        ],
        "tip": [
          {
            "blue": [
              "im"
            ]
          },
          {}
        ],
        "important": [
          {
            "blue": [
              "im"
            ],
            "purple": [
              "in dem"
            ]
          },
          {},
          {
            "green": [
              "im März",
              "im Herbst"
            ]
          },
          {
            "yellow": [
              "in der Schule"
            ],
            "red": [
              "im Schule"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "immer",
    "lv": "Alarme",
    "level": "A1"
  },
  {
    "de": "in",
    "lv": "-Poshtë.",
    "level": "A1",
    "study": {
      "id": "a1-in",
      "layout": "standardStudy",
      "translation": "-Poshtë.",
      "explanation": [
        "Ideja kryesore: në përgjithësi i referohet një vendi ose vendi kur i referohet një vendi, vendi, qyteti ose ndërtese.",
        "Në rastin e vendndodhjes, në zakonisht përkthehet si në ose në: në Berlin = në Berlin.",
        "Duke lëvizur drejt qendrës: ins Kino = në kinema.",
        "Përkthimi në letonisht ndryshon në varësi të kontekstit."
      ],
      "examples": [
        {
          "de": "Ich bin in Berlin.",
          "lv": "Jam në Berlin"
        },
        {
          "de": "Ich gehe in die Schule.",
          "lv": "Do të shkoj në shkollë"
        },
        {
          "de": "Das Buch ist in der Tasche.",
          "lv": "Libri është në çantë."
        },
        {
          "de": "Wir gehen ins Kino.",
          "lv": "Do të shkojmë në kinema."
        }
      ],
      "tip": {
        "text": "Mos harroni: w/w → w."
      },
      "important": [
        "Në nuk do të thotë gjithmonë fjalë për fjalë \"në\" • Letonishtja përdoret shpesh në Berlin, në shkollë, në kinema.",
        "Kur bëhet fjalë për sipërfaqen, zakonisht dëshironi auf në vend të in."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "in"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "in"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "in"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {},
            "meaning": {},
            "example": {}
          },
          {
            "word": {},
            "meaning": {},
            "example": {}
          },
          {
            "word": {},
            "meaning": {},
            "example": {}
          },
          {
            "word": {},
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "nuk"
            ]
          },
          {
            "yellow": [
              "auf"
            ],
            "red": [
              "in"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "ins",
    "lv": "W • W • Ku?",
    "level": "A1",
    "study": {
      "id": "a1-ins",
      "layout": "standardStudy",
      "translation": "W • W • Ku?",
      "explanation": [
        "INS është shkurtimi i parafjalës dhe artikullit das.",
        "Forma e plotë: në das (ku?).",
        "Ku përdoret në lidhje me emrat e çdo gjinie kur i përgjigjemi pyetjes? - lëvizja e brendshme.",
        "Zakonisht me folje: gehen, fahren, kommen, legen, stecken.",
        "Në praktikë, INS përdoret pothuajse gjithmonë në vend të indas-it të plotë."
      ],
      "examples": [
        {
          "de": "Ich gehe ins Kino.",
          "lv": "Po shkoj në kinema"
        },
        {
          "de": "Sie geht ins Bett.",
          "lv": "Ajo shkon të flejë."
        },
        {
          "de": "Wir fahren ins Ausland.",
          "lv": "Do të shkojmë jashtë vendit."
        },
        {
          "de": "Komm ins Haus!",
          "lv": "Ktheu në shtëpi."
        },
        {
          "de": "Er steckt das Geld in den Geldbeutel.",
          "lv": "Ai i fut paratë në portofolin e tij."
        },
        {
          "de": "Wir gehen ins Museum.",
          "lv": "Do të shkojmë në muze."
        },
        {
          "de": "Sie legt die Blumen ins Wasser.",
          "lv": "Ajo i hedh lulet në ujë."
        },
        {
          "de": "Fahr bitte ins Zentrum.",
          "lv": "Shko në qendër."
        }
      ],
      "comparison": [
        {
          "word": "ins",
          "meaning": "Brenda, për ku? (Ak.)",
          "example": "ins Kino – Në kinema"
        },
        {
          "word": "im",
          "meaning": "Brenda ku? (kujt?)",
          "example": "im Kino – Kinema"
        },
        {
          "word": "in",
          "meaning": "Brenda/për (me artikull të pavarur)",
          "example": "in die Stadt – Në qytet."
        },
        {
          "word": "aufs",
          "meaning": "Sipërfaqja (sëpata)",
          "example": "aufs Dach – Atje lart!"
        },
        {
          "word": "zum",
          "meaning": "Për / nga (kush?)",
          "example": "zum Arzt – Të lutem."
        }
      ],
      "tip": [
        "Mos harroni: in + das → ins (where?, where?).",
        "Ku? → në • Ku? → Ata - ky është ndryshimi kryesor!"
      ],
      "important": [
        "Ins = in das, vetëm me një emër të çdo gjinie, where? në foto.",
        "Përgjigjet e pyetjeve: ku? dhe ku jo? — lëvizje, jo vendndodhje.",
        "Për gjininë mashkullore: in den Wald • Women: in die Schule.",
        "Mos ngatërroni: ins Kino gehen (në kinema) dhe im Kino sein (për të qenë në kinema)."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "ins"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "in den"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ins"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "ins"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "ins Kino"
              ]
            }
          },
          {
            "word": {
              "green": [
                "im"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "im Kino"
              ]
            }
          },
          {
            "word": {
              "green": [
                "in"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "in die Stadt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "aufs"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "aufs Dach"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zum"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "zum Arzt"
              ]
            }
          }
        ],
        "tip": [
          {
            "blue": [
              "ins"
            ]
          },
          {}
        ],
        "important": [
          {
            "blue": [
              "ins"
            ],
            "purple": [
              "in das"
            ]
          },
          {},
          {
            "yellow": [
              "in den Wald"
            ],
            "green": [
              "in die Schule"
            ]
          },
          {
            "blue": [
              "ins Kino gehen"
            ],
            "red": [
              "im Kino sein"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "ja",
    "lv": "Po,po.",
    "level": "A1"
  },
  {
    "de": "Januar",
    "de_article": "der",
    "lv": "Janar",
    "level": "A1"
  },
  {
    "de": "jawohl",
    "lv": "Natyrisht.",
    "level": "A1"
  },
  {
    "de": "jeder",
    "lv": "Të gjithë...",
    "level": "A1"
  },
  {
    "de": "jemand",
    "lv": "Dikush...",
    "level": "A1"
  },
  {
    "de": "jetzt",
    "lv": "Tani • Tani për tani",
    "level": "A1"
  },
  {
    "de": "Juli",
    "de_article": "der",
    "lv": "KORRIK",
    "level": "A1"
  },
  {
    "de": "jung",
    "lv": "Të rinj (rreth njerëzve)",
    "level": "A1",
    "study": {
      "id": "a1-jung",
      "layout": "standardStudy",
      "translation": "Të rinj (rreth njerëzve)",
      "explanation": [
        "Ideja kryesore: jung do të thotë moshë e re – i referohet njerëzve dhe kafshëve, jo objekteve.",
        "Jung përcakton moshën • E kundërta është më e ulët (më e vjetër).",
        "Në letonisht, fjala i ri ka dy kuptime: mosha e re (jung) dhe e reja/e krijuar kohët e fundit (neu).",
        "Përdorni neu, as jung për sendet e krijuara ose të blera kohët e fundit.",
        "Jung përdoret gjithashtu në një kuptim metaforik: brez i ri, i dyfishtë i ri, të rinj.",
        "Ka edhe emrin die Jugend (rini)."
      ],
      "examples": [
        {
          "de": "Sie ist noch jung.",
          "lv": "Ai është ende i ri."
        },
        {
          "de": "Der Hund ist jung.",
          "lv": "Qeni është i ri."
        },
        {
          "de": "Wir sind noch jung.",
          "lv": "Jemi më të rinj."
        },
        {
          "de": "Er sieht sehr jung aus.",
          "lv": "Ai duket kaq i ri."
        },
        {
          "de": "Das ist ein junges Paar.",
          "lv": "Ky është një dopio i ri."
        },
        {
          "de": "Die junge Frau lächelt.",
          "lv": "E reja po buzëqesh."
        },
        {
          "de": "Mein Bruder ist jünger als ich.",
          "lv": "Vëllai im është më i ri se unë."
        }
      ],
      "tip": [
        "Jungu ka të bëjë me moshën (njerëzit, kafshët) - përdorni neu kur flisni për gjërat që kanë dalë kohët e fundit.",
        "Përkundrazi: jung ↔ alt (↔plak i ri)."
      ],
      "important": [
        "Jung përshkruan epokën e tij, jo risinë e gjërave.",
        "Përdorni neu në vend të jung për gjëra të reja (telefon, makinë, shtëpi).",
        "E gabuar: Mein Handy ist jung. → E saktë: Mein Handy ist neu."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "jung"
          ],
          "green": [
            "neu"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "jung"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "jung"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "jung"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "jung"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "junges"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "junge"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "jünger"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "jung"
            ],
            "green": [
              "neu"
            ]
          },
          {
            "blue": [
              "jung"
            ],
            "purple": [
              "alt"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "jung"
            ]
          },
          {
            "green": [
              "neu"
            ],
            "blue": [
              "jung"
            ]
          },
          {
            "blue": [
              "jung"
            ],
            "green": [
              "neu"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Juni",
    "de_article": "der",
    "lv": "Qershor",
    "level": "A1"
  },
  {
    "de": "Jeans",
    "de_article": "die",
    "lv": "Xhinse",
    "level": "A1"
  },
  {
    "de": "kaufen",
    "lv": "Ta blesh?",
    "level": "A1"
  },
  {
    "de": "kein",
    "lv": "Askush • Asgjë",
    "level": "A1",
    "study": {
      "id": "a1-kein",
      "layout": "standardStudy",
      "translation": "Askush • Asgjë",
      "explanation": [
        "Ideja kryesore: KEIN është një klauzolë mohimi që mohon emrin letonez, në varësi të përmbajtjes, askush ose askush.",
        "Kein është i zhdrejtë si ein (kein/keine/keinen...) dhe vjen para një emri.",
        "Në rastin e emrave të numërueshëm (njerëzve), kein shpesh përkthehet si askush (kein Mensch = askush).",
        "Për emrat jo detyrues ose abstrakt, kein shpesh përkthehet si asgjë/asgjë (kein Geld = pa para/pa para).",
        "Kein mohon të gjithë emrin, jo vetëm foljen (krahaso nicht)."
      ],
      "examples": [
        {
          "de": "Ich habe kein Geld.",
          "lv": "Nuk kam para."
        },
        {
          "de": "Es gibt keine Milch mehr.",
          "lv": "Nuk ka më qumësht."
        },
        {
          "de": "Kein Mensch war da.",
          "lv": "Por aty s'ishte askush."
        },
        {
          "de": "Ich habe keine Zeit.",
          "lv": "Nuk ka kohë të mjaftueshme."
        },
        {
          "de": "Das ist kein Problem.",
          "lv": "E di. S'ka gje."
        },
        {
          "de": "Wir haben keine Kinder.",
          "lv": "S'kemi fëmijë."
        }
      ],
      "tip": [
        "Kein mohon një emër (kein + emër), ndërsa nicht mohon një folje ose fjali.",
        "Kein, ein: ndryshon si kein/keine/keinen/keiner."
      ],
      "important": [
        "Kein + name = \"X po/jo\", jo \"nicht ein X\".",
        "E gabuar: Ich habe nicht ein Geld. → E saktë: Ich habe kein Geld."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "kein"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "kein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "keine"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Kein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "keine"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kein"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "keine"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "kein"
            ],
            "green": [
              "nicht"
            ]
          },
          {
            "blue": [
              "kein/keine/keinen/keiner"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "kein"
            ]
          },
          {
            "red": [
              "nicht ein Geld"
            ],
            "blue": [
              "kein Geld"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Keks",
    "de_article": "der",
    "de_plural": "die Kekse",
    "lv": "Kuki",
    "level": "A1"
  },
  {
    "de": "kennen",
    "lv": "Dije",
    "level": "A1",
    "id": "a1-kennen",
    "study": {
      "id": "a1-kennen-study",
      "layout": "standardStudy",
      "translation": "Dije",
      "explanation": [
        "Ideja kryesore: të njohësh një person, vend ose gjë nga përvoja.",
        "Kennen do të thotë informacion personal para së gjithash.",
        "Ai shpesh përshkruan: njerëz, të ngrënit.",
        "Kennen përdoret kur njeh një person, vend ose gjë nga përvoja jote."
      ],
      "examples": [
        {
          "de": "Ich kenne ihn.",
          "lv": "Ai ju ka ngrënë ushqimin."
        },
        {
          "de": "Kennen Sie diese Frau?",
          "lv": "E njeh ate grua?"
        },
        {
          "de": "Wo habt ihr euch kennengelernt?",
          "lv": "Ku e ke takuar?"
        },
        {
          "de": "Ich kenne ihn.",
          "lv": "Shiko !"
        },
        {
          "de": "kennen",
          "lv": "Njihu me Wissen"
        }
      ],
      "comparison": [
        {
          "word": "kennen",
          "meaning": "Njihu (person, vend, gjë)",
          "example": "Ich kenne ihn. – Ai ju ka ngrënë ushqimin."
        },
        {
          "word": "wissen",
          "meaning": "Njohja (e vërteta, njohuria)",
          "example": "Ich weiß seinen Namen. – E di emrin e tij!"
        }
      ],
      "tip": [
        "Kennen = të dish",
        "Përdorni Kenen kur konteksti përshtatet me të kuptuarit."
      ],
      "important": [
        "Kennen = njohja e një personi/vendi.",
        "Kennen = për të ditur.",
        "Njohja me një person, vend ose gjë përmes përvojave të tyre."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "kennen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "kenne"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kennen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kennengelernt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kenne"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kennen"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "kennen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "wissen",
    "lv": "Dije",
    "level": "A1",
    "id": "a1-wissen",
    "study": {
      "id": "a1-wissen-study",
      "layout": "standardStudy",
      "translation": "Dije",
      "explanation": [
        "Ideja kryesore: të dish një fakt, përgjigje ose informacion.",
        "Wissen në thelb do të thotë: njohuri/e vërtetë.",
        "Shpesh karakterizohet nga: përgjigje, të dhëna.",
        "Ne përdorim Wissen kur e di një fakt, një përgjigje ose informacion."
      ],
      "examples": [
        {
          "de": "Ich weiß, wo er wohnt.",
          "lv": "E di ku jeton."
        },
        {
          "de": "Woher wissen Sie das?",
          "lv": "Si e kuptove këtë?"
        },
        {
          "de": "Ich weiß die Antwort.",
          "lv": "E di pergjigjen."
        }
      ],
      "comparison": [
        {
          "word": "wissen",
          "meaning": "Njohja (e vërteta, njohuria)",
          "example": "Ich weiß, wo er wohnt. – E di ku jeton."
        },
        {
          "word": "kennen",
          "meaning": "Njihu (person, vend, gjë)",
          "example": "Ich kenne die Stadt. – E njoh këtë qytet."
        }
      ],
      "tip": [
        "Wissen = të dish",
        "Përdorni WISEN kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Wissen = njohja e një të vërtete.",
        "Wissen = për të ditur.",
        "Zbulo një fakt, përgjigje ose informacion."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "wissen"
          ]
        },
        "examples": [
          {
            "de": {},
            "lv": {}
          },
          {
            "de": {
              "green": [
                "wissen"
              ]
            },
            "lv": {}
          },
          {
            "de": {},
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "wissen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Kilogramm",
    "de_article": "das",
    "de_plural": "die Kilogramm",
    "lv": "Kilogramë",
    "level": "A1"
  },
  {
    "de": "Kilometer",
    "de_article": "der",
    "de_plural": "die Kilometer",
    "lv": "Kilometra",
    "level": "A1"
  },
  {
    "de": "Kind",
    "de_article": "das",
    "de_plural": "die Kinder",
    "lv": "Djali.",
    "level": "A1"
  },
  {
    "de": "Kindergarten",
    "de_article": "der",
    "de_plural": "die Kindergärten",
    "lv": "Kopsht fëmijësh",
    "level": "A1"
  },
  {
    "de": "Kirche",
    "de_article": "die",
    "de_plural": "die Kirchen",
    "lv": "Kishe.",
    "level": "A1"
  },
  {
    "de": "kochen",
    "lv": "Guzhinieri.",
    "level": "A1"
  },
  {
    "de": "kommen",
    "lv": "Po vjen",
    "level": "A1"
  },
  {
    "de": "können",
    "lv": "Të jesh në gjendje • Të dish",
    "level": "A1",
    "study": {
      "id": "a1-können",
      "layout": "standardStudy",
      "translation": "Të jesh në gjendje • Të dish",
      "explanation": [
        "Ideja kryesore: të pjatë të bësh diçka ose të jesh në gjendje ta bësh atë.",
        "Kur bëhet fjalë për talentet ose aftësitë, letonishtja zakonisht thotë \"E di\".",
        "Shpesh thuhet se kjo është e mundur kur bëhet fjalë për probabilitetin.",
        "Können është një folje modale, kështu që folja e dytë zakonisht është në fund."
      ],
      "examples": [
        {
          "de": "Ich kann Deutsch sprechen.",
          "lv": "Mund të flas gjermanisht"
        },
        {
          "de": "Kannst du mir helfen?",
          "lv": "A mund të më ndihmoni?"
        },
        {
          "de": "Wir können heute kommen.",
          "lv": "Mund të vijmë sot."
        },
        {
          "de": "Er kann gut schwimmen.",
          "lv": "Ai di të notojë mirë."
        }
      ],
      "comparison": [
        {
          "word": "können",
          "meaning": "Dije",
          "example": "Ich kann schwimmen. = Es protu peldēt."
        },
        {
          "word": "dürfen",
          "meaning": "E lejueshme",
          "example": "Darf ich gehen? = Vai drīkstu iet?"
        },
        {
          "word": "müssen",
          "meaning": "Më duhet /duhettëjem-",
          "example": "Ich muss lernen. = Man jāmācās."
        },
        {
          "word": "wissen",
          "meaning": "Dije",
          "example": "Ich weiß das. = Es to zinu."
        }
      ],
      "tip": {
        "text": "Mos harroni: talentin → tuaj."
      },
      "important": [
        "Können nuk është e njëjtë me dürfen. können = të jesh në gjendje/të dish, dürfen = të jesh në gjendje.",
        "Në fjalinë nga Können, folja e dytë zakonisht shfaqet në fund: Ich kann schwimmen."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "können"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "kann"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Kannst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "können"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kann"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "können"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "dürfen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "Darf"
              ]
            }
          },
          {
            "word": {
              "green": [
                "müssen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "wissen"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "können"
            ],
            "red": [
              "dürfen"
            ]
          },
          {
            "blue": [
              "kann"
            ],
            "green": [
              "schwimmen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "kosten",
    "lv": "Paguaj",
    "level": "A1",
    "study": {
      "id": "a1-kosten",
      "layout": "standardStudy",
      "translation": "Paguaj",
      "explanation": [
        "Ideja kryesore: costen do të thotë të paguash në një mënyrë ose në një tjetër - flet për çmimin e gjërave.",
        "Ne e përdorim këtë fjalë kur kërkojmë ose themi se sa kushton diçka, jo kur një person paguan.",
        "Kërkimi i çmimeve në gjermanisht zakonisht më fillon fjalët Was kostet...?.",
        "Në këtë kontekst, fjala “pagesë” në letonisht është e saktë: Das kostet 5 Euro. = Çmimi është 5 euro.",
        "Kur dikush jep para për një mall ose shërbim, ajo përdoret në gjermanisht si bezahlen ose zahlen."
      ],
      "examples": [
        {
          "de": "Das kostet 5 Euro.",
          "lv": "Kushton 5 Euro."
        },
        {
          "de": "Was kostet das?",
          "lv": "Sa kohë..."
        },
        {
          "de": "Wie viel kostet der Pullover?",
          "lv": "Sa kushton një pulovër?"
        },
        {
          "de": "Das Essen kostet nicht viel.",
          "lv": "Ushqimi nuk është shumë i shtrenjtë."
        },
        {
          "de": "Ich bezahle die Rechnung.",
          "lv": "Paguaj faturën"
        },
        {
          "de": "Kann ich bar bezahlen?",
          "lv": "A mund të paguaj me para në dorë?"
        },
        {
          "de": "Er zahlt mit Karte.",
          "lv": "Ai paguan me kartë."
        },
        {
          "de": "Ich zahle gleich.",
          "lv": "Do të paguaj tani"
        }
      ],
      "comparison": [
        {
          "word": "kosten",
          "meaning": "Pagesa (çmimi) • Sa",
          "example": "Das kostet 5 Euro. = Tas maksā 5 eiro."
        },
        {
          "word": "bezahlen",
          "meaning": "Pagesa • Pagesa (para)",
          "example": "Ich bezahle die Rechnung. = Es maksāju rēķinu."
        },
        {
          "word": "zahlen",
          "meaning": "Pagesa • Pagesa",
          "example": "Kann ich bar zahlen? = Vai varu maksāt skaidrā naudā?"
        },
        {
          "word": "Was kostet...?",
          "meaning": "Sa kohë?",
          "example": "Was kostet das Buch? = Cik maksā grāmata?"
        }
      ],
      "tip": [
        "Mos harro: mos kërko çmimin kosten → (Do kostet das?).",
        "Mos harroni: paguani→ për bezahle/zahlen (Ich bezahle die Rechnung.)"
      ],
      "important": [
        "Kosten dhe bezahlen nuk janë sinonime: kosten = sa kushton • Bezahlen = paguaj para.",
        "Në letonisht, fjala \"pagë\" përdoret shpesh në të dy rastet, por në gjermanisht, zgjedhja duhet të bëhet rast pas rasti."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "costen"
          ],
          "green": [
            "Was kostet"
          ],
          "yellow": [
            "bezahlen",
            "zahlen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "kostet"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kostet"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kostet"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "kostet"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "bezahle"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "bezahlen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "zahlt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "zahle"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "kosten"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "Kosztuje"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bezahlen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "bezahle"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zahlen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "zahlen"
              ]
            }
          },
          {
            "word": {
              "green": [
                "Was kostet"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "kostet"
              ]
            }
          }
        ],
        "tip": [
          {
            "blue": [
              "kosten"
            ]
          },
          {
            "yellow": [
              "bezahle",
              "zahlen"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "kosten"
            ],
            "yellow": [
              "bezahlen"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "küssen",
    "lv": "Puthja.",
    "level": "A1"
  },
  {
    "de": "lächeln",
    "lv": "Buzëqesh",
    "level": "A1"
  },
  {
    "de": "lachen",
    "lv": "Gül",
    "level": "A1"
  },
  {
    "de": "Honig",
    "de_article": "der",
    "de_plural": "die Honige",
    "lv": "Bukuroshja ime!",
    "level": "A1"
  },
  {
    "de": "Hose",
    "de_article": "die",
    "de_plural": "die Hosen",
    "lv": "Pantallonat.",
    "level": "A1"
  },
  {
    "de": "Huhn",
    "de_article": "das",
    "de_plural": "die Hühner",
    "lv": "Frikacak!",
    "level": "A1"
  },
  {
    "de": "Hund",
    "de_article": "der",
    "de_plural": "die Hunde",
    "lv": "Lshte një qen.",
    "level": "A1"
  },
  {
    "de": "Hut",
    "de_article": "der",
    "de_plural": "die Hüte",
    "lv": "Kapelën! - Kapelën! Mirë.",
    "level": "A1"
  },
  {
    "de": "Jahr",
    "de_article": "das",
    "de_plural": "die Jahre",
    "lv": "Viti",
    "level": "A1"
  },
  {
    "de": "Kaffee",
    "de_article": "der",
    "lv": "Ja, akoma kafe.",
    "level": "A1"
  },
  {
    "de": "Kalender",
    "de_article": "der",
    "de_plural": "die Kalender",
    "lv": "Kalendar?",
    "level": "A1"
  },
  {
    "de": "Kamera",
    "de_article": "die",
    "de_plural": "die Kameras",
    "lv": "Aparat fotografik.",
    "level": "A1"
  },
  {
    "de": "Karotte",
    "de_article": "die",
    "de_plural": "die Karotten",
    "lv": "Karotat.",
    "level": "A1"
  },
  {
    "de": "Kartoffel",
    "de_article": "die",
    "de_plural": "die Kartoffeln",
    "lv": "#Patatet ...",
    "level": "A1"
  },
  {
    "de": "Käse",
    "de_article": "der",
    "lv": "\"Rebels\"",
    "level": "A1"
  },
  {
    "de": "Katze",
    "de_article": "die",
    "de_plural": "die Katzen",
    "lv": "Mace.",
    "level": "A1"
  },
  {
    "de": "Kleid",
    "de_article": "das",
    "de_plural": "die Kleider",
    "lv": "Fustani!",
    "level": "A1"
  },
  {
    "de": "Kleidung",
    "de_article": "die",
    "lv": "Rroba",
    "level": "A1"
  },
  {
    "de": "Knoblauch",
    "de_article": "der",
    "de_plural": null,
    "lv": "Hudhër",
    "level": "A1"
  },
  {
    "de": "Koch",
    "de_article": "der",
    "de_plural": "die Köche",
    "lv": "Guzhinieri.",
    "level": "A1"
  },
  {
    "de": "Köchin",
    "de_article": "die",
    "de_plural": "die Köchinnen",
    "lv": "Guzhinieri.",
    "level": "A1"
  },
  {
    "de": "Kopf",
    "de_article": "der",
    "de_plural": "die Köpfe",
    "lv": "Goditje me kokë",
    "level": "A1"
  },
  {
    "de": "Kraftwagen",
    "de_article": "der",
    "de_plural": "die Kraftwagen",
    "lv": "Henry, karroca!",
    "level": "A1"
  },
  {
    "de": "Küche",
    "de_article": "die",
    "de_plural": "die Küchen",
    "lv": "Kuzhina.",
    "level": "A1"
  },
  {
    "de": "Kuchen",
    "de_article": "der",
    "de_plural": "die Kuchen",
    "lv": "Kek!",
    "level": "A1"
  },
  {
    "de": "Kuh",
    "de_article": "die",
    "de_plural": "die Kühe",
    "lv": "Lopë",
    "level": "A1"
  },
  {
    "de": "Kühlschrank",
    "de_article": "der",
    "de_plural": "die Kühlschränke",
    "lv": "Frigoriferin.",
    "level": "A1"
  },
  {
    "de": "Kuss",
    "de_article": "der",
    "de_plural": "die Küsse",
    "lv": "Puthja.",
    "level": "A1"
  },
  {
    "de": "Laden",
    "de_article": "der",
    "de_plural": "die Läden",
    "lv": "Dyqani?",
    "level": "A1",
    "study": {
      "id": "a1-laden-study",
      "layout": "standardStudy",
      "translation": "Dyqani?",
      "explanation": [
        "Ideja kryesore: der Spruce kapitalizohet, dhe artikulli der është një emër - një dyqan i vogël.",
        "Ngarkuar është një folje e vogël që përdoret për të ngarkuar ose ngarkuar.",
        "Der Laden shpesh i referohet një dyqani të vogël në jetën e përditshme (im Laden einkaufen = pazar në dyqan).",
        "Plural: die Läden."
      ],
      "examples": [
        {
          "de": "Ich gehe in den Laden.",
          "lv": "Po shkoj në dyqan"
        },
        {
          "de": "Der Laden ist geschlossen.",
          "lv": "Dyqani është i mbyllur."
        },
        {
          "de": "Es gibt viele Läden hier.",
          "lv": "Ka shumë dyqane këtu."
        },
        {
          "de": "Ich muss mein Handy laden.",
          "lv": "Më duhet të karikoj telefonin tim."
        }
      ],
      "tip": [
        "Der Spruce kapitalizohet - emri (dyqani).",
        "Ngarkuar me shkronja të vogla - folja (për të ngarkuar/ngarkuar)."
      ],
      "important": [
        "Der Spruce = dyqan (emri).",
        "Ngarkuar = ngarkim/ngarkim (folje).",
        "Plural: die Läden."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "der Laden"
          ],
          "green": [
            "laden"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Laden"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Laden"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Läden"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "laden"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "Der"
            ]
          },
          {
            "green": [
              "Der"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "Der"
            ]
          },
          {},
          {
            "blue": [
              "die Läden"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Lampe",
    "de_article": "die",
    "de_plural": "die Lampen",
    "lv": "Flesh!",
    "level": "A1"
  },
  {
    "de": "Land",
    "de_article": "das",
    "de_plural": "die Länder",
    "lv": "Shteti • Bota",
    "level": "A1",
    "study": {
      "id": "a1-land",
      "layout": "standardStudy",
      "translation": "Shteti • Bota",
      "explanation": [
        "Ideja kryesore: das Land kryesisht i referohet një vendi ose toke jashtë qytetit.",
        "Në rastin e Gjermanisë, Letonisë ose ddo rajoni tjetër me kufij, ai përkthehet si shtet.",
        "Duhet të përkthehet si fshat ose tokë kur flitet për fshat ose tokë në vend të qytetit.",
        "Konteksti përcakton nëse e kemi fjalën për vendin, fshatin apo tokën."
      ],
      "examples": [
        {
          "de": "Deutschland ist ein schönes Land.",
          "lv": "Gjermania është një vend i bukur."
        },
        {
          "de": "Ich komme aus einem kleinen Land.",
          "lv": "Vij nga një vend i vogël."
        },
        {
          "de": "Wir fahren aufs Land.",
          "lv": "Do të shkojmë në fshat."
        },
        {
          "de": "Auf dem Land ist es ruhig.",
          "lv": "Fshati është i qetë."
        }
      ],
      "comparison": [
        {
          "word": "das Land",
          "meaning": "Shteti / toka / fshati",
          "example": "Deutschland ist ein Land."
        },
        {
          "word": "die Stadt",
          "meaning": "Qytet",
          "example": "Ich wohne in der Stadt."
        },
        {
          "word": "das Dorf",
          "meaning": "Ky ishte fshati.",
          "example": "Er lebt in einem Dorf."
        },
        {
          "word": "die Erde",
          "meaning": "Toka/Planeti",
          "example": "Die Erde ist rund."
        }
      ],
      "tip": {
        "text": "Mos harroni: country → das Land • City → die Stadt."
      },
      "important": [
        "Aufs Land do të thotë \"fshat\", jo \"fshat\".",
        "Das Land nuk është i njëjtë me Die Stadt."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "das Land",
            "Land"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Land"
              ],
              "green": [
                "Deutschland"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Land"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Land"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Land"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "das Land"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "land"
              ]
            }
          },
          {
            "word": {
              "green": [
                "die Stadt"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "Stadt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "das Dorf"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "Dorf"
              ]
            }
          },
          {
            "word": {
              "green": [
                "die Erde"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "aufs Land"
            ]
          },
          {
            "blue": [
              "das Land"
            ],
            "yellow": [
              "die Stadt"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "lang",
    "lv": "Shumë gjatë. Rreth jetës sime.",
    "level": "A1",
    "study": {
      "id": "a1-lang",
      "layout": "standardStudy",
      "translation": "Shumë gjatë. Rreth jetës sime.",
      "explanation": [
        "Ideja kryesore: gjuha do të thotë haptazi e gjatë, përkohësisht e gjatë.",
        "Për sa i përket madhësisë ose distancës, gjuha = e gjatë (ein langer Tisch = tavolina e gjatë).",
        "Për sa i përket kohëzgjatjes, lang = long (ein langer Tag = long day).",
        "Den ganzen do të thotë një ditë e plotë (gjatësi) në shprehjen Tag lang.",
        "Letonishtja \"e gjatë\" dhe \"e gjatë\" janë dy fjalë të ndryshme, por në gjermanisht ka të dyja kuptimet."
      ],
      "examples": [
        {
          "de": "Der Tisch ist sehr lang.",
          "lv": "Tavolina është shumë e gjatë."
        },
        {
          "de": "Der Film war sehr lang.",
          "lv": "Filmi ishte shumë i gjatë."
        },
        {
          "de": "Wie lange dauert es?",
          "lv": "Edhe sa kohë?"
        },
        {
          "de": "Sie hat lange Haare.",
          "lv": "Ajo ka flokë të gjatë."
        },
        {
          "de": "Ich warte schon lange.",
          "lv": "Kemi shumë kohë që presim."
        },
        {
          "de": "Den ganzen Tag lang.",
          "lv": "Gjithë ditën (gjatësia)."
        }
      ],
      "tip": [
        "→ I gjatë për shkak të madhësisë ose distancës (flokët, trajektorja, tabela).",
        "Koha (dita, pritja, filmi) është e → gjatë."
      ],
      "important": [
        "Gjuhë = e gjatë (madhësia) OSE e gjatë (koha) - në varësi të kontekstit.",
        "Lange = e di se sa do të zgjasë (është çështje kohe, jo madhësie)."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "lang"
          ],
          "purple": [
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "lang"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "lang"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "lange"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "lange"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "lange"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "lang"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {},
          {}
        ]
      }
    }
  },
  {
    "de": "lange",
    "lv": "Ka pak kohe ...",
    "level": "A1"
  },
  {
    "de": "langsam",
    "lv": "Mund ta vërtetoj. - Lehtë.",
    "level": "A1"
  },
  {
    "de": "langweilig",
    "lv": "Ajo është çmenduri.",
    "level": "A1"
  },
  {
    "de": "lassen",
    "lv": "Leja • Lejo",
    "level": "A1",
    "study": {
      "id": "a1-lassen",
      "layout": "standardStudy",
      "translation": "Leja • Lejo",
      "explanation": [
        "Ideja kryesore: Lassen do të thotë të lëshoj diçka pas ose të lëshoj diçka të ndodhë.",
        "Nëse diçka mbetet në vend, do të thotë pushim i ngadaltë.",
        "Nëse i jep leje dikujt, do të thotë ta lësh të shkojë.",
        "Një formë shumë e zakonshme e të folurit është Lass mich! = Më lër të qetë! Ose më lër!"
      ],
      "examples": [
        {
          "de": "Ich lasse die Tasche hier.",
          "lv": "E lashë çantën këtu"
        },
        {
          "de": "Lass das bitte auf dem Tisch.",
          "lv": "Të lutem, lëre mbi tavolinë."
        },
        {
          "de": "Meine Eltern lassen mich gehen.",
          "lv": "Prindërit e mi më lanë të shkoj."
        },
        {
          "de": "Lass mich in Ruhe!",
          "lv": "Me ler vetem!"
        }
      ],
      "comparison": [
        {
          "word": "lassen",
          "meaning": "Lësho/lejo",
          "example": "Ich lasse das hier."
        },
        {
          "word": "bleiben",
          "meaning": "Qëndro",
          "example": "Ich bleibe hier."
        },
        {
          "word": "erlauben",
          "meaning": "Aktivizo",
          "example": "Sie erlaubt mir das."
        },
        {
          "word": "geben",
          "meaning": "Jep",
          "example": "Gib mir das Buch."
        }
      ],
      "tip": {
        "text": "Mos harro: diçka mbetet e ngadaltë → • Dikush mund të → ecë ngadalë."
      },
      "important": [
        "Lassen nuk do të thotë vetëm \"ndarje\". Gjithashtu shpesh do të thotë \"lejim\".",
        "Lass mich! në Ruhe ka një thënie shumë popullore: \"Më lër të qetë!\""
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "lassen",
            "Lass mich"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "lasse"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Lass"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "lassen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Lass mich"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "lassen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "lass"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bleiben"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "bleibe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "erlauben"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "erlaubt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "geben"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "Gib"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "lassen"
            ]
          },
          {
            "blue": [
              "Lass"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "laufen",
    "lv": "Ekzekuto • Vepro",
    "level": "A1",
    "study": {
      "id": "a1-laufen",
      "layout": "standardStudy",
      "translation": "Ekzekuto • Vepro",
      "explanation": [
        "Ideja kryesore: laufen do të thotë vrapim, por mund të nënkuptojë edhe vrapim kur bëhet fjalë për pajisjet.",
        "Për një person ose kafshë, laufen zakonisht do të thotë vrapim ose ecje me ritëm të shpejtë.",
        "Në rastin e një filmi, makinerie ose programi, laufen do të thotë se diçka po funksionon ose po ndodh.",
        "Në rastin e lëvizjeve të këmbëve në nivelin A1, gehen dhe laufen krahasohen më së shumti."
      ],
      "examples": [
        {
          "de": "Er läuft sehr schnell.",
          "lv": "Ai vrapon shumë shpejt."
        },
        {
          "de": "Die Kinder laufen im Park.",
          "lv": "Fëmijët vrapojnë rreth parkut."
        },
        {
          "de": "Der Film läuft schon.",
          "lv": "Filmi tashmë po publikohet."
        },
        {
          "de": "Die Maschine läuft gut.",
          "lv": "Makina funksionon mirë."
        }
      ],
      "comparison": [
        {
          "word": "laufen",
          "meaning": "Vrapo/vepro",
          "example": "Er läuft schnell."
        },
        {
          "word": "gehen",
          "meaning": "Bredhës.",
          "example": "Ich gehe nach Hause."
        },
        {
          "word": "fahren",
          "meaning": "Transporti me makinë",
          "example": "Ich fahre mit dem Bus."
        },
        {
          "word": "funktionieren",
          "meaning": "Biznes",
          "example": "Das funktioniert gut."
        }
      ],
      "tip": {
        "text": "Mos harroni: këmbët e → shpejta laufen • Mbani → fahrenheit."
      },
      "important": [
        "Laufen nuk është vetëm \"vrapim\". Për një video ose pajisje, kjo mund të nënkuptojë \"shko\" ose \"bëj\".",
        "Reputacioni i tyre do të thotë të udhëtojnë në këmbë, jo me makinë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "laufen"
          ],
          "green": [
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "läuft"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "laufen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "läuft"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "läuft"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "laufen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "läuft"
              ]
            }
          },
          {
            "word": {
              "green": [
                "gehen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "gehe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "fahren"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "fahre"
              ]
            }
          },
          {
            "word": {
              "green": [
                "funktionieren"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "laufen"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "laut",
    "lv": "Kështu?",
    "level": "A1",
    "study": {
      "id": "a1-laut",
      "layout": "standardStudy",
      "translation": "Kështu?",
      "explanation": [
        "Ideja kryesore: mbiemër i vogël. Përshkruan volumin — sa i zhurmshëm është tingulli ose të folurit.",
        "Laut në thelb do të thotë: zhurmë e madhe.",
        "Shpesh përkufizon: mbiemër.",
        "Laut në thelb do të thotë: sinjal zanor.",
        "Identifikon shpesh: emrin (der).",
        "Lauti i shkallës së ulët është një mbiemër - ai përcakton lartësinë e zërit (Die Musik ist laut = muzikë me zë të lartë).",
        "Shkronja e madhe Der Laut dhe artikulli der është një emër-që do të thotë tingull si diçka ose sinjal (Der Laut ist schön = tingull është i bukur).",
        "Plural: Lavdëro të vdesësh."
      ],
      "examples": [
        {
          "de": "Die Musik ist laut.",
          "lv": "Muzika është shumë e zhurmshme."
        },
        {
          "de": "Die Musik ist laut.",
          "lv": "Muzika është shumë e zhurmshme."
        },
        {
          "de": "Sprich nicht so laut!",
          "lv": "Mos fol kaq me zë të lartë!"
        },
        {
          "de": "Das ist sehr laut.",
          "lv": "Ajo ishte e zhurmshme."
        },
        {
          "de": "Der Laut ist schön.",
          "lv": "Zëri i saj është kaq i bukur."
        },
        {
          "de": "Ich höre einen Laut.",
          "lv": "Dëgjoj një zë..."
        }
      ],
      "tip": [
        "Laut i vogël = me zë të lartë (mbiemri: ist laut). der Laut i kapitalizuar = tingull (emri: ein Laut, der Laut).",
        "Laut = tingull"
      ],
      "important": [
        "Laut është shkruar me shkronja të vogla dhe nuk përmban artikuj • Ky është një mbiemër.",
        "Der Laut kapitalizohet dhe artikulli der është emër.",
        "Plural: die Laute (tinguj të gjuhës, sinjale zanore).",
        "E gabuar: Der Laut ist sehr. → E saktë: Das ist sehr laut."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "laut"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "laut"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "laut"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Laut",
    "de_article": "der",
    "de_plural": "die Laute",
    "lv": "Nga _hurma.",
    "level": "A1",
    "study": {
      "id": "a1-laut-study",
      "layout": "standardStudy",
      "translation": "Nga _hurma.",
      "explanation": [
        "Ideja kryesore: Një emër me një artikull përshtatet dhe kapitalizohet. Diçka do të thotë tingull si sinjal ose tingull i një gjuhe.",
        "Der Laut do të thotë me zë të lartë mbi të gjitha.",
        "Shpesh përkufizon: mbiemër.",
        "Der Laut kryesisht do të thotë: sinjal akustik.",
        "Identifikon shpesh: emrin (der).",
        "Lauti i shkallës së ulët është një mbiemër - ai përcakton lartësinë e zërit (Die Musik ist laut = muzikë me zë të lartë).",
        "Shkronja e madhe Der Laut dhe artikulli der është një emër-që do të thotë tingull si diçka ose sinjal (Der Laut ist schön = tingull është i bukur).",
        "Plural: Lavdëro të vdesësh."
      ],
      "examples": [
        {
          "de": "Der Laut ist schön.",
          "lv": "Zëri i saj është kaq i bukur."
        },
        {
          "de": "Die Musik ist laut.",
          "lv": "Muzika është shumë e zhurmshme."
        },
        {
          "de": "Sprich nicht so laut!",
          "lv": "Mos fol kaq me zë të lartë!"
        },
        {
          "de": "Das ist sehr laut.",
          "lv": "Ajo ishte e zhurmshme."
        },
        {
          "de": "Der Laut ist schön.",
          "lv": "Zëri i saj është kaq i bukur."
        },
        {
          "de": "Ich höre einen Laut.",
          "lv": "Dëgjoj një zë..."
        }
      ],
      "tip": [
        "Laut i vogël = me zë të lartë (mbiemri: ist laut). der Laut i kapitalizuar = tingull (emri: ein Laut, der Laut).",
        "Der Laut = tingull"
      ],
      "important": [
        "Laut është shkruar me shkronja të vogla dhe nuk përmban artikuj • Ky është një mbiemër.",
        "Der Laut kapitalizohet dhe artikulli der është emër.",
        "Plural: die Laute (tinguj të gjuhës, sinjale zanore).",
        "E gabuar: Der Laut ist sehr. → E saktë: Das ist sehr laut."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "der Laut",
            "laut"
          ],
          "yellow": [
            "Laut",
            "Laut"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "der Laut",
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "der Laut",
                "laut"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "laut"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "leben",
    "lv": "-Ai është mirë.",
    "level": "A1"
  },
  {
    "de": "lecker",
    "lv": "Qenka shumë e shijshme.",
    "level": "A1"
  },
  {
    "de": "leer",
    "lv": "Zbrazët",
    "level": "A1"
  },
  {
    "de": "legen",
    "lv": "Vendos",
    "level": "A1",
    "study": {
      "id": "a1-legen",
      "layout": "standardStudy",
      "translation": "Vendos",
      "explanation": [
        "Ideja kryesore: legjendë do të thotë të vendosësh diçka në tokë ose horizontalisht.",
        "Legen përdoret kur e lëviz artikullin vetë dhe e vendos në një tavolinë, krevat ose sipërfaqe tjetër.",
        "Është ndryshe nga Liegen, që do të thotë se diçka tashmë po gënjen ose gënjen.",
        "Dallimi më i rëndësishëm në nivelin A1 është: legen = shtrirë, Liegen = shtrirë."
      ],
      "examples": [
        {
          "de": "Ich lege das Buch auf den Tisch.",
          "lv": "E vura librin në tryezë."
        },
        {
          "de": "Leg den Schlüssel hierhin.",
          "lv": "Vendos çelësin këtu."
        },
        {
          "de": "Sie legt das Kind ins Bett.",
          "lv": "Ajo e vë fëmijën në shtrat."
        },
        {
          "de": "Das Buch liegt auf dem Tisch.",
          "lv": "Libri është në tryezë."
        }
      ],
      "comparison": [
        {
          "word": "legen",
          "meaning": "Vendos",
          "example": "Ich lege das Buch auf den Tisch."
        },
        {
          "word": "liegen",
          "meaning": "Të jesh /të flesh",
          "example": "Das Buch liegt auf dem Tisch."
        },
        {
          "word": "stellen",
          "meaning": "Vendos vertikalisht",
          "example": "Ich stelle die Flasche auf den Tisch."
        },
        {
          "word": "setzen",
          "meaning": "Uluni, uluni.",
          "example": "Ich setze mich."
        }
      ],
      "tip": {
        "text": "Features: tu noliec → legend • Lie or atrodas → lie."
      },
      "important": [
        "Legen dhe Liegen nuk janë e njëjta gjë.",
        "Ich lege das Buch = E lashë librin mënjanë. Das Buch Liegt = libri gënjen."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "legen",
            "liegen"
          ],
          "yellow": [
            "Liegen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "lege"
              ],
              "yellow": [
                "Buch",
                "Tisch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Leg"
              ],
              "yellow": [
                "Schlüssel"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "legt"
              ],
              "green": [
                "Kind"
              ],
              "yellow": [
                "Bett"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "liegt"
              ],
              "yellow": [
                "Buch",
                "Tisch"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "legen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "lege"
              ]
            }
          },
          {
            "word": {
              "green": [
                "liegen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "liegt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "stellen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "stelle"
              ]
            }
          },
          {
            "word": {
              "green": [
                "setzen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "setze"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "legen"
            ],
            "red": [
              "liegen"
            ]
          },
          {
            "blue": [
              "lege"
            ],
            "red": [
              "liegt"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Lehrer",
    "de_article": "der",
    "de_plural": "die Lehrer",
    "lv": "Mësues.",
    "level": "A1"
  },
  {
    "de": "Lehrerin",
    "de_article": "die",
    "de_plural": "die Lehrerinnen",
    "lv": "Mësues.",
    "level": "A1"
  },
  {
    "de": "leicht",
    "lv": "Asgjë.",
    "level": "A1"
  },
  {
    "de": "leider",
    "lv": "Po e kapim atë? Jo.",
    "level": "A1"
  },
  {
    "de": "leise",
    "lv": "Ndegjoje.",
    "level": "A1",
    "study": {
      "id": "a1-leise-study",
      "layout": "standardStudy",
      "translation": "Ndegjoje.",
      "explanation": [
        "Ideja kryesore: I qetë ose me volum të ulët.",
        "Leise në thelb do të thotë: vëllim i ulët.",
        "Shpesh përshkruhet si: tingull/tingull/muzikë.",
        "Leise identifikon vëllim të ulët ose vëllim/tingull të ulët."
      ],
      "examples": [
        {
          "de": "Bitte sei leise.",
          "lv": "-Ju lutem qetesi."
        },
        {
          "de": "Bitte sei leise.",
          "lv": "Këkobosh."
        },
        {
          "de": "Die Musik ist leise.",
          "lv": "Muzika është e qetë."
        },
        {
          "de": "Sprich bitte leise.",
          "lv": "Ju lutemi të ulni zërin tuaj."
        }
      ],
      "tip": [
        "Leise = heshtje",
        "Përdorni leise kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Leise = një zë i qetë.",
        "Leise = volumi.",
        "I qetë ose me volum të ulët."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "leise"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "leise"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "leise"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "leise"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "leise"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "leise"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "lesen",
    "lv": "Të lexosh?",
    "level": "A1"
  },
  {
    "de": "letzte",
    "lv": "Mbarimi",
    "level": "A1"
  },
  {
    "de": "Leute",
    "de_article": "die",
    "lv": "Të gjithë...",
    "level": "A1"
  },
  {
    "de": "Licht",
    "de_article": "das",
    "de_plural": "die Lichter",
    "lv": "E lehtë",
    "level": "A1"
  },
  {
    "de": "lieb",
    "lv": "Oh, e dashur. Jam këtu.",
    "level": "A1"
  },
  {
    "de": "Liebe",
    "de_article": "die",
    "lv": "Dashuria",
    "level": "A1"
  },
  {
    "de": "lieben",
    "lv": "Dashuria",
    "level": "A1"
  },
  {
    "de": "Lied",
    "de_article": "das",
    "de_plural": "die Lieder",
    "lv": "Kënga!",
    "level": "A1"
  },
  {
    "de": "liegen",
    "lv": "Ji • Gjumë",
    "level": "A1",
    "study": {
      "id": "a1-liegen",
      "layout": "standardStudy",
      "translation": "Ji • Gjumë",
      "explanation": [
        "Ideja kryesore: Liegen do të thotë të gënjesh ose të gënjesh horizontalisht.",
        "Për një person, gënjeshtra zakonisht do të thotë gjumë.",
        "Në një farë mënyre, Liegen do të thotë se ai është atje diku.",
        "Është ndryshe nga legeni, që do të thotë të lësh diçka mënjanë."
      ],
      "examples": [
        {
          "de": "Das Buch liegt auf dem Tisch.",
          "lv": "Libri është në tryezë."
        },
        {
          "de": "Mein Handy liegt im Auto.",
          "lv": "Telefoni im është në makinë."
        },
        {
          "de": "Er liegt im Bett.",
          "lv": "Duke fjetur në shtrat."
        },
        {
          "de": "Ich lege das Buch auf den Tisch.",
          "lv": "E vura librin në tryezë."
        }
      ],
      "comparison": [
        {
          "word": "liegen",
          "meaning": "Të jesh /të flesh",
          "example": "Das Buch liegt hier."
        },
        {
          "word": "legen",
          "meaning": "Vendos",
          "example": "Ich lege das Buch hierhin."
        },
        {
          "word": "stehen",
          "meaning": "Qëndrim/Qëndrim",
          "example": "Die Flasche steht auf dem Tisch."
        },
        {
          "word": "sein",
          "meaning": "Të jesh",
          "example": "Ich bin hier."
        }
      ],
      "tip": {
        "text": "Mos harroni: gjëja është tashmë në → vend Liegen • Ju e latë mënjanë → legjendën."
      },
      "important": [
        "Liegen tregon statusin ose vendndodhjen.",
        "Legen tregon veprimin: dikush vendos diçka në tokë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "liegen",
            "legen"
          ],
          "yellow": [
            "Liegen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "liegt"
              ],
              "yellow": [
                "Buch",
                "Tisch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "liegt"
              ],
              "yellow": [
                "Handy",
                "Auto"
              ]
            },
            "lv": {
              "yellow": [
                "Telefoni"
              ]
            }
          },
          {
            "de": {
              "blue": [
                "liegt"
              ],
              "green": [
                "Bett"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "lege"
              ],
              "yellow": [
                "Buch",
                "Tisch"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "liegen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "legen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "lege"
              ]
            }
          },
          {
            "word": {
              "green": [
                "stehen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "steht"
              ]
            }
          },
          {
            "word": {
              "green": [
                "sein"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "liegen"
            ]
          },
          {
            "red": [
              "legen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Limonade",
    "de_article": "die",
    "de_plural": "die Limonaden",
    "lv": "Limonadë!",
    "level": "A1"
  },
  {
    "de": "Lineal",
    "de_article": "das",
    "de_plural": "die Lineale",
    "lv": "Abetare",
    "level": "A1"
  },
  {
    "de": "links",
    "lv": "Majtas, majtas.",
    "level": "A1"
  },
  {
    "de": "Liste",
    "de_article": "die",
    "de_plural": "die Listen",
    "lv": "Një list.",
    "level": "A1"
  },
  {
    "de": "Liter",
    "de_article": "der",
    "de_plural": "die Liter",
    "lv": "Litra",
    "level": "A1",
    "study": {
      "id": "a1-liter",
      "layout": "standardStudy",
      "translation": "Litra",
      "explanation": "Në Gjermani shpesh quhet \"der Liter\", por në Austri dhe Zvicër mund të dëgjosh edhe fjalën \"das Liter\". Shumësi mbetet i pandryshuar: “die Litre”.",
      "sectionAccents": {
        "explanation": {
          "green": [
            "der Liter"
          ],
          "blue": [
            "das Liter"
          ],
          "purple": [
            "die"
          ]
        }
      }
    }
  },
  {
    "de": "Löffel",
    "de_article": "der",
    "de_plural": "die Löffel",
    "lv": "Lugën!",
    "level": "A1"
  },
  {
    "de": "Luft",
    "de_article": "die",
    "de_plural": "die Lüfte",
    "lv": "Ajri.",
    "level": "A1"
  },
  {
    "de": "lustig",
    "lv": "Futem.",
    "level": "A1"
  },
  {
    "de": "machen",
    "lv": "Bëj • Bëj",
    "level": "A1",
    "study": {
      "id": "a1-machen",
      "layout": "standardStudy",
      "translation": "Bëj • Bëj",
      "explanation": [
        "Ideja kryesore: është një fjalë shumë e njohur që do të thotë të bësh ose të bësh machen.",
        "Kur bëhet fjalë për veprimin në përgjithësi, domethënë si ta bëjmë atë.",
        "Nëse diçka është bërë ose përgatitur, ajo përkthehet si bërje ose gatim.",
        "Në shumë shprehje, machen përkthehet natyrshëm sipas letonishtes, jo fjalë për fjalë."
      ],
      "examples": [
        {
          "de": "Was machst du?",
          "lv": "Çfarë do të thuash?"
        },
        {
          "de": "Ich mache Hausaufgaben.",
          "lv": "Po bëj detyrat e shtëpisë."
        },
        {
          "de": "Wir machen Pizza.",
          "lv": "Po bëjmë pica."
        },
        {
          "de": "Das macht Spaß.",
          "lv": "Ajo është shumë argëtuese."
        }
      ],
      "tip": {
        "text": "Atceries: A ishte Machst? = Kush mund të vijë këtu?"
      },
      "important": [
        "Machen është një fjalë shumë e gjerë, por në varësi të situatës, letonishtja shpesh duhet të përkthehet në mënyrë të natyrshme.",
        "Das macht Spaß fjalë për fjalë do të thotë \"argëtim\", jo \"argëtim\"."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "machen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "machst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "mache"
              ],
              "yellow": [
                "Hausaufgaben"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "machen"
              ],
              "yellow": [
                "Pizza"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "macht Spaß"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {},
            "meaning": {},
            "example": {}
          },
          {
            "word": {},
            "meaning": {},
            "example": {}
          },
          {
            "word": {},
            "meaning": {},
            "example": {}
          },
          {
            "word": {},
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "machen"
            ]
          },
          {
            "blue": [
              "Das macht Spaß"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Mädchen",
    "de_article": "das",
    "de_plural": "die Mädchen",
    "lv": "Një mbretëreshë.",
    "level": "A1"
  },
  {
    "de": "Mahlzeit",
    "de_article": "die",
    "de_plural": "die Mahlzeiten",
    "lv": "-Po, ushqimi.",
    "level": "A1"
  },
  {
    "de": "Mai",
    "de_article": "der",
    "lv": "Për të qenë në gjendje të",
    "level": "A1"
  },
  {
    "de": "Mal",
    "de_article": "das",
    "de_plural": "die Male",
    "lv": "Koha.",
    "level": "A1",
    "study": {
      "id": "a1-mal",
      "layout": "standardStudy",
      "translation": "Koha.",
      "explanation": [
        "Ideja kryesore: das Mal do të thotë një herë si ngjarje ose mundësi.",
        "Përdoret kryesisht në kombinim me numrat: ein Mal, zwei Mal, drei Mal.",
        "Me numrin e sekuencës: das erste mal, das zweite mal.",
        "Mos flisni me mallin (Komm commodity her!) në gjuhë bisedore • Kjo ka një kuptim tjetër."
      ],
      "examples": [
        {
          "de": "Das erste Mal war schwer.",
          "lv": "Hera e parë ishte e vështirë."
        },
        {
          "de": "Ich war schon zwei Mal in Berlin.",
          "lv": "Unë kam qenë tashmë në Berlin dy herë."
        },
        {
          "de": "Ein Mal reicht.",
          "lv": "Një herë është e mjaftueshme."
        },
        {
          "de": "Noch ein Mal, bitte!",
          "lv": "Edhe njëherë, të lutem."
        }
      ],
      "tip": {
        "text": "Mos harroni: das Mal = koha (emri) • Mal pa artikuj = grimca në kolokualizëm."
      },
      "important": [
        "Emri që përmban das Mal / die Male-artikel.",
        "Ein Mal, zwei Mal – numëro kohët.",
        "Mallrat pa artikuj (mallrat e saj Komm!) nuk janë të njëjta me mallrat das."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "das Mal",
            "ein Mal",
            "zwei Mal",
            "das erste Mal"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "erste Mal"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zwei Mal"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Ein Mal"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ein Mal"
              ]
            },
            "lv": {}
          }
        ],
        "tip": {},
        "important": [
          {
            "blue": [
              "das Mal",
              "die Male"
            ]
          },
          {
            "blue": [
              "ein Mal",
              "zwei Mal"
            ]
          },
          {
            "blue": [
              "mal"
            ],
            "purple": [
              "Mallrat"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "malen",
    "lv": "Bojë • Bojë",
    "level": "A1"
  },
  {
    "de": "manchmal",
    "lv": "Ndonjëherë.",
    "level": "A1"
  },
  {
    "de": "Mandarine",
    "de_article": "die",
    "de_plural": "die Mandarinen",
    "lv": "Mandarinë.",
    "level": "A1"
  },
  {
    "de": "Mann",
    "de_article": "der",
    "de_plural": "die Männer",
    "lv": "Burri • Burri",
    "level": "A1",
    "study": {
      "id": "a1-mann",
      "layout": "standardStudy",
      "translation": "Burri • Burri",
      "explanation": [
        "Ideja kryesore: der Mann mund të nënkuptojë një burrë (gjinia) ose një burrë (gruaja).",
        "Nëse është vetëm çështje gjinie ose personi, Mann = mashkull.",
        "Për një grua, thotë Mann = burri (mein Mann = burri).",
        "Përemri pronor (mein/dein/ihr Mann) pothuajse gjithmonë do të thotë burrë-grua.",
        "Plural: die Männer.",
        "Forma femërore die frau ka të njëjtin kuptim të dyfishtë: grua dhe grua."
      ],
      "examples": [
        {
          "de": "Er ist ein netter Mann.",
          "lv": "Ai është njeri i mirë."
        },
        {
          "de": "Das ist mein Mann.",
          "lv": "Burri im, Josh."
        },
        {
          "de": "Wie viele Männer sind hier?",
          "lv": "Sa burra janë këtu?"
        },
        {
          "de": "Mein Mann arbeitet in Berlin.",
          "lv": "Burri im punon në Berlin."
        },
        {
          "de": "Der Mann trägt einen Anzug.",
          "lv": "Burri ka veshur një kostum."
        },
        {
          "de": "Ihr Mann ist Arzt.",
          "lv": "Burri i saj është mjek."
        }
      ],
      "tip": [
        "Përemri pronësor (mein/dein/ihr Mann) pothuajse gjithmonë do të thotë burri (gruaja).",
        "Pa prapashtesën e pronësisë (der Mann, ein Mann) zakonisht do të thotë një burrë."
      ],
      "important": [
        "Der Mann = mashkull OSE bashkëshort – në varësi të kontekstit.",
        "Mein Mann = burri im (jo \"burri im\").",
        "Plural: die Männer."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "der Mann",
            "Mann"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Mann"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "mein Mann"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Männer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Mein Mann"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Mann"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Ihr Mann"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "green": [
              "mein",
              "dein",
              "ihr Mann"
            ]
          },
          {
            "blue": [
              "der Mann",
              "ein Mann"
            ]
          }
        ],
        "important": [
          {},
          {
            "green": [
              "mein Mann"
            ]
          },
          {
            "blue": [
              "die Männer"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Marmelade",
    "de_article": "die",
    "de_plural": "die Marmeladen",
    "lv": "Reçel",
    "level": "A1"
  },
  {
    "de": "März",
    "de_article": "der",
    "lv": "MARS",
    "level": "A1"
  },
  {
    "de": "Maus",
    "de_article": "die",
    "de_plural": "die Mäuse",
    "lv": "Një mi!",
    "level": "A1"
  },
  {
    "de": "Meer",
    "de_article": "das",
    "de_plural": "die Meere",
    "lv": "Deti.",
    "level": "A1"
  },
  {
    "de": "Mehl",
    "de_article": "das",
    "de_plural": "die Mehle",
    "lv": "Mielli",
    "level": "A1"
  },
  {
    "de": "mehr",
    "lv": "Ai thotë, \"Mirëmëngjes\" dhe pyet, \"Si e keni dorën?\"",
    "level": "A1"
  },
  {
    "de": "mein",
    "lv": "E imja.",
    "level": "A1"
  },
  {
    "de": "Mensch",
    "de_article": "der",
    "de_plural": "die Menschen",
    "lv": "Person.",
    "level": "A1"
  },
  {
    "de": "Messer",
    "de_article": "das",
    "de_plural": "die Messer",
    "lv": "Bisturi.",
    "level": "A1"
  },
  {
    "de": "Meter",
    "de_article": "der",
    "de_plural": "die Meter",
    "lv": "Këtë shirit matës?",
    "level": "A1"
  },
  {
    "de": "Milch",
    "de_article": "die",
    "lv": "Qumështi",
    "level": "A1"
  },
  {
    "de": "Million",
    "de_article": "die",
    "de_plural": "die Millionen",
    "lv": "Një milion.",
    "level": "A1"
  },
  {
    "de": "Minute",
    "de_article": "die",
    "de_plural": "die Minuten",
    "lv": "Një moment.",
    "level": "A1"
  },
  {
    "de": "mit",
    "lv": "DHE",
    "level": "A1",
    "study": {
      "id": "a1-mit",
      "layout": "standardStudy",
      "translation": "DHE",
      "explanation": [
        "Ideja kryesore: Miti shpesh do të thotë \"me\".",
        "Ne përdorim mitin kur dikush është me dikë ose bën diçka me diçka.",
        "Miti i transportit shpesh nënkupton udhëtimin me autobus, tren ose makinë.",
        "Në nivelin A1, mit dem bus, mit dem Auto dhe mit janë shprehje shumë të zakonshme."
      ],
      "examples": [
        {
          "de": "Ich komme mit dir.",
          "lv": "Do futem brënda me ty."
        },
        {
          "de": "Ich fahre mit dem Bus.",
          "lv": "Po marr autobusin"
        },
        {
          "de": "Sie schreibt mit dem Stift.",
          "lv": "Ai shkruan me laps."
        },
        {
          "de": "Kommst du mit?",
          "lv": "A po vjen me mua?"
        }
      ],
      "comparison": [
        {
          "word": "mit",
          "meaning": "Së bashku / së bashku",
          "example": "Ich komme mit dir."
        },
        {
          "word": "ohne",
          "meaning": "Pa",
          "example": "Ich komme ohne dich."
        },
        {
          "word": "bei",
          "meaning": "Në ambientin e dikujt",
          "example": "Ich bin bei Anna."
        },
        {
          "word": "zu",
          "meaning": "Për / në",
          "example": "Ich gehe zum Arzt."
        }
      ],
      "tip": {
        "text": "Mos harroni: → legjendë me dikë ose me transport."
      },
      "important": [
        "Miti kërkon një dativ: mit dem Bus, mit der Mutter, është mit.",
        "Ku jeton do të thotë: \"A do të vish me mua?\""
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "mit",
            "mit dem Bus",
            "mit dem Auto",
            "Ideja"
          ],
          "green": [
            "Auto"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "mit dir"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "mit dem Bus"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "mit dem Stift"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "mit"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "mit"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "mit"
              ]
            }
          },
          {
            "word": {
              "green": [
                "ohne"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "ohne dich"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bei"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "zu"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "zum Arzt"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "mit"
            ],
            "yellow": [
              "dem Bus",
              "der Mutter",
              "der"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "mitnehmen",
    "lv": "Merre me vete",
    "level": "A1"
  },
  {
    "de": "Mittag",
    "de_article": "der",
    "de_plural": "die Mittage",
    "lv": "Koha e drekes.",
    "level": "A1"
  },
  {
    "de": "Mittagessen",
    "de_article": "das",
    "de_plural": "die Mittagessen",
    "lv": "Koha e drekes.",
    "level": "A1"
  },
  {
    "de": "Mittwoch",
    "de_article": "der",
    "de_plural": "die Mittwoche",
    "lv": "E mërkurë",
    "level": "A1"
  },
  {
    "de": "mögen",
    "lv": "Më pëlqen",
    "level": "A1",
    "study": {
      "id": "a1-mögen",
      "layout": "standardStudy",
      "translation": "Më pëlqen",
      "explanation": [
        "Ideja kryesore: mögen kryesisht do të thotë të pëlqesh diçka.",
        "Magjistarët... zakonisht do të thotë \"Më pëlqen...\" në letonisht.",
        "Möchte është një formë tjetër që përdoret për të shprehur një dëshirë të sjellshme: E dua.",
        "Shprehja më e rëndësishme në nivelin A1 është ich mag das."
      ],
      "examples": [
        {
          "de": "Ich mag Musik.",
          "lv": "Më pëlqen muzika"
        },
        {
          "de": "Magst du Kaffee?",
          "lv": "Të pëlqen kafeja?"
        },
        {
          "de": "Sie mag Kinder.",
          "lv": "Ai i do fëmijët."
        },
        {
          "de": "Ich möchte einen Kaffee.",
          "lv": "Dua kafe"
        }
      ],
      "comparison": [
        {
          "word": "mögen",
          "meaning": "Më pëlqen",
          "example": "Ich mag Musik."
        },
        {
          "word": "möchte",
          "meaning": "Dua nje.",
          "example": "Ich möchte Kaffee."
        },
        {
          "word": "wollen",
          "meaning": "Dua",
          "example": "Ich will nach Hause."
        },
        {
          "word": "lieben",
          "meaning": "Dashuria",
          "example": "Ich liebe dich."
        }
      ],
      "tip": {
        "text": "Mos harroni: magjia e tyre... = Më pëlqen..."
      },
      "important": [
        "Mögen nuk është një emër që me edukatë do të thotë \"Dua\". fjala möchte përdoret shpesh për këtë.",
        "Kafeneja e tyre magjike do të thotë \"Më pëlqen kafeja\"."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "mögen",
            "Ich mag",
            "möchte"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "mag"
              ]
            },
            "lv": {
              "yellow": [
                "pëlqen"
              ]
            }
          },
          {
            "de": {
              "blue": [
                "Magst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "mag"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "möchte"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "mögen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "möchte"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "möchte"
              ]
            }
          },
          {
            "word": {
              "green": [
                "wollen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "lieben"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "mögen"
            ],
            "red": [
              "möchte"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "Möhre",
    "de_article": "die",
    "de_plural": "die Möhren",
    "lv": "Karotat.",
    "level": "A1"
  },
  {
    "de": "Monat",
    "de_article": "der",
    "de_plural": "die Monate",
    "lv": "Hëna",
    "level": "A1"
  },
  {
    "de": "Montag",
    "de_article": "der",
    "de_plural": "die Montage",
    "lv": "E hënë",
    "level": "A1"
  },
  {
    "de": "morgen",
    "lv": "Neser.",
    "level": "A1",
    "study": {
      "id": "a1-morgen",
      "layout": "standardStudy",
      "translation": "Neser.",
      "explanation": [
        "Ideja kryesore: Afati kohor shkruhet me shkronja të vogla. Kjo do të thotë të nesërmen - nesër.",
        "Morgen kryesisht do të thotë: të nesërmen.",
        "Zakonisht karakterizohet nga: moti.",
        "Morgen do të thotë pjesë e ditës para së gjithash.",
        "Identifikon shpesh: emrin (der).",
        "Morgen në thelb do të thotë: disa mëngjeseve.",
        "Zakonisht karakterizohet si: emër (p.sh.).",
        "Morgen me një inicial të vogël do të thotë nesër - të nesërmen (ich komme morgen = Do të vij nesër, Bis morgen!"
      ],
      "examples": [
        {
          "de": "Ich komme morgen.",
          "lv": "Do të vij nesër përsëri."
        },
        {
          "de": "Bis morgen!",
          "lv": "Do jutra!"
        },
        {
          "de": "Ich komme morgen.",
          "lv": "Do të vij nesër përsëri."
        },
        {
          "de": "Morgen ist Montag.",
          "lv": "Nesër e hënë"
        },
        {
          "de": "Guten Morgen!",
          "lv": "Mirëmëngjes."
        },
        {
          "de": "Der Morgen ist schön.",
          "lv": "Mëngjesi është i bukur."
        }
      ],
      "tip": [
        "Morgen i vogël = nesër (ich komme morgen). der Morgen capital = mëngjes (Guten Morgen!, am Morgen).",
        "Morgen = nesër"
      ],
      "important": [
        "Morgu shkruhet me shkronja të vogla - një zarf që përshkruan kohën (nesër).",
        "Uppercase Der Morgen dhe artikulli der është një emër (mëngjes).",
        "Mirëmëngjes! — gjithmonë me M-në e madhe (mëngjes, jo nesër).",
        "E rreme: Guten Morgen! → E saktë: Guten Morgen!"
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "morgen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "morgen"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "Morgu"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Morgen",
    "de_article": "der",
    "lv": "Fije mëndafshi?",
    "level": "A1",
    "study": {
      "id": "a1-morgen-study",
      "layout": "standardStudy",
      "translation": "Fije mëndafshi?",
      "explanation": [
        "Ideja kryesore: emri i artikullit përshtatet dhe kapitalizohet. Pjesë e ditës - mëngjes.",
        "Der Morgen kryesisht do të thotë të nesërmen.",
        "Zakonisht karakterizohet nga: moti.",
        "Morgen do të thotë para së gjithash pjesë e ditës.",
        "Identifikon shpesh: emrin (der).",
        "Morgen në thelb do të thotë: disa mëngjeseve.",
        "Zakonisht karakterizohet si: emër (p.sh.).",
        "Morgen me një inicial të vogël do të thotë nesër - të nesërmen (ich komme morgen = Do të vij nesër, Bis morgen!"
      ],
      "examples": [
        {
          "de": "Guten Morgen!",
          "lv": "Mirëmëngjes."
        },
        {
          "de": "Bis morgen!",
          "lv": "Do jutra!"
        },
        {
          "de": "Ich komme morgen.",
          "lv": "Do të vij nesër përsëri."
        },
        {
          "de": "Morgen ist Montag.",
          "lv": "Nesër e hënë"
        },
        {
          "de": "Guten Morgen!",
          "lv": "Mirëmëngjes."
        },
        {
          "de": "Der Morgen ist schön.",
          "lv": "Mëngjesi është i bukur."
        }
      ],
      "tip": [
        "Morgen i vogël = nesër (ich komme morgen). der Morgen capital = mëngjes (Guten Morgen!, am Morgen).",
        "Der Morgen = nesër"
      ],
      "important": [
        "Morgu shkruhet me shkronja të vogla - një zarf që përshkruan kohën (nesër).",
        "Uppercase Der Morgen dhe artikulli der është një emër (mëngjes).",
        "Mirëmëngjes! — gjithmonë me M-në e madhe (mëngjes, jo nesër).",
        "E rreme: Guten Morgen! → E saktë: Guten Morgen!"
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "der Morgen",
            "morgen"
          ],
          "yellow": [
            "Morgen"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "morgen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "der Morgen",
                "morgen"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "morgens",
    "lv": "Fije mëndafshi?",
    "level": "A1"
  },
  {
    "de": "müde",
    "lv": "E lodhur.",
    "level": "A1"
  },
  {
    "de": "Mund",
    "de_article": "der",
    "de_plural": "die Münder",
    "lv": "Goja",
    "level": "A1"
  },
  {
    "de": "Musik",
    "de_article": "die",
    "lv": "Muzika, ndalu.",
    "level": "A1"
  },
  {
    "de": "müssen",
    "lv": "Kërko",
    "level": "A1",
    "study": {
      "id": "a1-müssen",
      "layout": "standardStudy",
      "translation": "Kërko",
      "explanation": [
        "Ideja kryesore: nëse je i shkujdesur, do të thotë të bësh diçka.",
        "Në letonisht, shpesh përkthehet si \"Unë bëj...\", \"ti bën...\", \"ne bëjmë...\".",
        "Në një fjali gjermane, folja e dytë zakonisht vjen në fund.",
        "Forma më e rëndësishme në nivelin A1 është ich muss..."
      ],
      "examples": [
        {
          "de": "Ich muss gehen.",
          "lv": "Dua të shkoj në banjo..."
        },
        {
          "de": "Du musst warten.",
          "lv": "Sot nuk dua. Ky është"
        },
        {
          "de": "Wir müssen lernen.",
          "lv": "Duhet ta zbulojmë."
        },
        {
          "de": "Ich muss heute arbeiten.",
          "lv": "Më duhet të punoj sot"
        }
      ],
      "comparison": [
        {
          "word": "müssen",
          "meaning": "I/I need to do",
          "example": "Ich muss gehen."
        },
        {
          "word": "können",
          "meaning": "Dije",
          "example": "Ich kann kommen."
        },
        {
          "word": "wollen",
          "meaning": "Dua",
          "example": "Ich will nach Hause."
        },
        {
          "word": "dürfen",
          "meaning": "E lejueshme",
          "example": "Darf ich gehen?"
        }
      ],
      "tip": {
        "text": "Mos harroni: Ich muss... = Duhet të..."
      },
      "important": [
        "Müssen është folje modale.",
        "Folja e dytë zakonisht është në fund të fjalisë: Ich muss heute arbeiten."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "Ideja",
            "Ich muss"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "muss",
                "gehen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "musst",
                "warten"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "müssen",
                "lernen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "muss",
                "arbeiten"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "müssen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "können"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "wollen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "dürfen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "Darf"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "müssen"
            ],
            "purple": [
              "modal"
            ]
          },
          {
            "blue": [
              "muss",
              "arbeiten"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Mutter",
    "de_article": "die",
    "de_plural": "die Mütter",
    "lv": "Mom!",
    "level": "A1"
  },
  {
    "de": "Mütze",
    "de_article": "die",
    "de_plural": "die Mützen",
    "lv": "Kapelën! - Kapelën! Mirë.",
    "level": "A1"
  },
  {
    "de": "nach",
    "lv": "Deri: • Pas",
    "level": "A1",
    "study": {
      "id": "a1-nach",
      "layout": "standardStudy",
      "translation": "Deri: • Pas",
      "explanation": [
        "Ideja kryesore: nach do të thotë me vende dhe pas kohës ose sekuencës.",
        "Për qytetet dhe shtetet ku artikulli nuk është i disponueshëm, kjo zakonisht do të thotë:",
        "Me kalimin e kohës do të thotë nach po.",
        "Nach Hause do të thotë shtëpi në idiomën e saj."
      ],
      "examples": [
        {
          "de": "Ich fahre nach Berlin.",
          "lv": "Po shkoj në Berlin."
        },
        {
          "de": "Wir gehen nach Hause.",
          "lv": "Do të vish në shtëpi me mua?"
        },
        {
          "de": "Nach dem Essen gehen wir spazieren.",
          "lv": "Pasi hamë, dalim për një shëtitje."
        },
        {
          "de": "Es ist zehn nach acht.",
          "lv": "Është ora dhjetë e tetë."
        }
      ],
      "comparison": [
        {
          "word": "nach",
          "meaning": "Deri/Pas",
          "example": "Ich fahre nach Berlin."
        },
        {
          "word": "zu",
          "meaning": "Për / në",
          "example": "Ich gehe zum Arzt."
        },
        {
          "word": "in",
          "meaning": "Vendndodhja/ vendndodhja e artikullit",
          "example": "Ich gehe in die Schule."
        },
        {
          "word": "vor",
          "meaning": "Para/Para",
          "example": "Vor dem Essen wasche ich die Hände."
        }
      ],
      "tip": {
        "text": "Mos harroni: nach Hause • Në Berlin • Pas darkës."
      },
      "important": [
        "Nach nuk përdoret kudo.",
        "Njerëzit zakonisht shkojnë në shkollë në Die Schule, jo në Nach Schule."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "nach",
            "nach Hause"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "nach"
              ],
              "green": [
                "Berlin"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "nach Hause"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Nach"
              ],
              "yellow": [
                "Essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "nach"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "nach"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "nach"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zu"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "zum Arzt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "in"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "in die Schule"
              ]
            }
          },
          {
            "word": {
              "green": [
                "vor"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "Vor"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "nach"
            ]
          },
          {
            "red": [
              "nach Schule"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Nachmittag",
    "de_article": "der",
    "de_plural": "die Nachmittage",
    "lv": "Pasdite vonë.",
    "level": "A1"
  },
  {
    "de": "nachmittags",
    "lv": "PM",
    "level": "A1"
  },
  {
    "de": "Nacht",
    "de_article": "die",
    "de_plural": "die Nächte",
    "lv": "Natë",
    "level": "A1"
  },
  {
    "de": "Nase",
    "de_article": "die",
    "de_plural": "die Nasen",
    "lv": "Hunda",
    "level": "A1"
  },
  {
    "de": "nass",
    "lv": "Lagështi",
    "level": "A1"
  },
  {
    "de": "Natur",
    "de_article": "die",
    "lv": "Natyra.",
    "level": "A1"
  },
  {
    "de": "natürlich",
    "lv": "Sigurisht • Natyrale",
    "level": "A1",
    "study": {
      "id": "a1-natuerlich",
      "layout": "standardStudy",
      "translation": "Sigurisht • Natyrale",
      "explanation": [
        "Ideja kryesore: naturlich si ndajfolje, natyrisht, do të thotë e natyrshme si mbiemër.",
        "Në bisedë, kur pohon diçka, naturlich = sigurisht (Kommst du mit? – Natürlich! = A do të vish? – Sigurisht!).",
        "Naturlich = natyror (naturliche Schönheit = bukuri natyrore) kur flitet për natyrën, origjinën ose veçoritë.",
        "Konteksti (përgjigjja/konfirmimi ose shpjegimi) tregon kuptimin e saktë."
      ],
      "examples": [
        {
          "de": "Kommst du mit? – Natürlich!",
          "lv": "Do të vish me mua?"
        },
        {
          "de": "Das ist eine natürliche Reaktion.",
          "lv": "Është një reagim i natyrshëm."
        },
        {
          "de": "Natürlich helfe ich dir.",
          "lv": "Sigurisht që do të të ndihmoj."
        },
        {
          "de": "Sie hat natürliche rote Haare.",
          "lv": "Ajo ka flokë të kuq natyralë."
        },
        {
          "de": "Natürlich kann ich das machen.",
          "lv": "Sigurisht që mundem."
        },
        {
          "de": "Das ist ganz natürlich.",
          "lv": "Kjo është plotësisht e natyrshme/normale."
        }
      ],
      "tip": [
        "Si një fjalë e veçantë, që miraton ose korrespondon, → natyrisht.",
        "→ Natyrale pranë një emri që identifikon një origjinë ose atribut."
      ],
      "important": [
        "Natürlich = sigurisht (zarf, konfirmim) OSE natyral (mbiemër).",
        "Natyrisht! gjithmonë si një pikëçuditje e veçantë = sigurisht!"
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "natürlich"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Natürlich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "natürliche"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Natürlich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "natürliche"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Natürlich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "natürlich"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {},
          {}
        ]
      }
    }
  },
  {
    "de": "neben",
    "lv": "Digjeni.",
    "level": "A1"
  },
  {
    "de": "nehmen",
    "lv": "Merre atë.",
    "level": "A1",
    "study": {
      "id": "a1-nehmen",
      "layout": "standardStudy",
      "translation": "Merre atë.",
      "explanation": [
        "Ideja kryesore: do të thotë të marrësh ose të marrësh nehmen.",
        "Nehmen përdoret kur blini ose zgjidhni diçka për veten tuaj.",
        "Kjo nuk është e njëjtë me sjelljen, sepse sjellja do të thotë të sjellësh ose të marrësh nga dikush.",
        "Holen do të thotë të shkosh dhe të sjellësh/marrësh."
      ],
      "examples": [
        {
          "de": "Ich nehme den Bus.",
          "lv": "Po marr autobusin"
        },
        {
          "de": "Nimm das Buch!",
          "lv": "Merr një libër."
        },
        {
          "de": "Ich bringe dir das Buch.",
          "lv": "Të solla një libër"
        },
        {
          "de": "Ich hole dich ab.",
          "lv": "Do ta marr."
        }
      ],
      "comparison": [
        {
          "word": "nehmen",
          "meaning": "Merre atë.",
          "example": "Nimm das Buch!"
        },
        {
          "word": "bringen",
          "meaning": "Sill /merr/dorëzo",
          "example": "Ich bringe dir das Buch."
        },
        {
          "word": "holen",
          "meaning": "Merre.",
          "example": "Ich hole Wasser."
        },
        {
          "word": "mitnehmen",
          "meaning": "Merre me vete",
          "example": "Ich nehme dich mit."
        }
      ],
      "tip": {
        "text": "Mos harro: merre → vetë Nehmen • → Sill dikë."
      },
      "important": [
        "Ich nehme den Bus do të thotë \"Unë ngas një autobus\" në letonisht.",
        "Nehmeni nuk është i njëjtë me atë që u soll."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "nehmen"
          ],
          "red": [
            "holen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "nehme"
              ],
              "yellow": [
                "Bus"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Nimm"
              ],
              "yellow": [
                "Buch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "bringe"
              ],
              "yellow": [
                "Buch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "hole"
              ],
              "green": [
                "dich"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "nehmen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "Nimm"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bringen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "bringe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "holen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "mitnehmen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "nehme",
                "mit"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "nehme den Bus"
            ]
          },
          {
            "blue": [
              "nehmen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "nein",
    "lv": "Jo.",
    "level": "A1"
  },
  {
    "de": "nennen",
    "lv": "Është...",
    "level": "A1"
  },
  {
    "de": "nett",
    "lv": "E bukur.",
    "level": "A1"
  },
  {
    "de": "neu",
    "lv": "E re (rreth gjërave)",
    "level": "A1",
    "study": {
      "id": "a1-neu",
      "layout": "standardStudy",
      "translation": "E re (rreth gjërave)",
      "explanation": [
        "Ideja kryesore: neu nënkupton diçka të re • Krijuar, blerë ose përdorur kohët e fundit për të parë.",
        "Neu përcakton gjërat, pajisjet, rrobat, shtëpitë, idetë etj., jo moshën e një personi apo të një kafshe.",
        "Në letonisht, fjala i ri ka dy kuptime: mosha e re (jung) dhe e reja/e krijuar kohët e fundit (neu).",
        "Për moshën e një personi ose kafshe, përdoret xhungë, jo neu.",
        "Neu përdoret gjithashtu në mënyrë metaforike: punë e re, njohuri e re, fillim i ri.",
        "E kundërta do të thotë alt (i vjetër) • Das Neue new."
      ],
      "examples": [
        {
          "de": "Mein Handy ist neu.",
          "lv": "Telefoni im është i ri."
        },
        {
          "de": "Wir haben ein neues Auto.",
          "lv": "Kemi një makinë të re."
        },
        {
          "de": "Das ist meine neue Wohnung.",
          "lv": "Ky është apartamenti im."
        },
        {
          "de": "Ich habe neue Schuhe gekauft.",
          "lv": "Bleva këpucë të reja."
        },
        {
          "de": "Das ist eine neue Idee.",
          "lv": "Idenë origjinale."
        },
        {
          "de": "Er hat einen neuen Job.",
          "lv": "Ai ka një punë të re."
        },
        {
          "de": "Was gibt es Neues?",
          "lv": "Miki!"
        }
      ],
      "tip": [
        "Neu ka të bëjë me gjëra, pajisje dhe inovacione - përdorni jung kur flisni për moshën e një personi.",
        "Kontrasti: neu ↔ alt (i ri ↔ i vjetër)."
      ],
      "important": [
        "Neu përshkruan ngjarje dhe lajme, ose moshën e një personi ose kafshe.",
        "Përdor xhungën në vend të neut për të përcaktuar moshën e një personi ose kafshe.",
        "E gabuar: Meine Schwester ist neu. → E saktë: Meine Schwester ist jung."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "neu"
          ],
          "green": [
            "jung"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "neu"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "neues"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "neue"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "neue"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "neue"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "neuen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Neues"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "neu"
            ],
            "green": [
              "jung"
            ]
          },
          {
            "blue": [
              "neu"
            ],
            "purple": [
              "alt"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "neu"
            ]
          },
          {
            "green": [
              "Përdor"
            ],
            "blue": [
              "neu"
            ]
          },
          {
            "blue": [
              "neu"
            ],
            "green": [
              "jung"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "neun",
    "lv": "Nëntë.",
    "level": "A1"
  },
  {
    "de": "neunhundert",
    "lv": "Nëntëqind",
    "level": "A1"
  },
  {
    "de": "neunte",
    "lv": "E nënta",
    "level": "A1"
  },
  {
    "de": "neunzehn",
    "lv": "Nëntëmbëdhjetë!",
    "level": "A1"
  },
  {
    "de": "neunzehnte",
    "lv": "I nëntëmbëdhjeti",
    "level": "A1"
  },
  {
    "de": "neunzig",
    "lv": "Nëntëdhjetë",
    "level": "A1"
  },
  {
    "de": "neunzigste",
    "lv": "E nëntëdhjeta",
    "level": "A1"
  },
  {
    "de": "nicht",
    "lv": "Jo.",
    "level": "A1"
  },
  {
    "de": "nichts",
    "lv": "Fije",
    "level": "A1"
  },
  {
    "de": "nie",
    "lv": "Kurrë",
    "level": "A1"
  },
  {
    "de": "niemand",
    "lv": "-Asnjë.",
    "level": "A1"
  },
  {
    "de": "noch",
    "lv": "Jemi të vdekur.",
    "level": "A1",
    "study": {
      "id": "a1-noch-study",
      "layout": "standardStudy",
      "translation": "Jemi të vdekur.",
      "explanation": [
        "Ideja kryesore: Diçka po ndodh ende ose nuk ka mbaruar ende.",
        "Noch në thelb do të thotë se diçka ende po ndodh.",
        "Zakonisht përcakton: vazhdimin ose gjendjen e papërfunduar.",
        "Noch ende do të thotë: diçka po ndodh ende ose nuk ka mbaruar ende."
      ],
      "examples": [
        {
          "de": "Ich bin noch zu Hause.",
          "lv": "Jam ende në shtëpi."
        },
        {
          "de": "Ich bin noch zu Hause.",
          "lv": "Jam ende në shtëpi"
        },
        {
          "de": "Bist du noch da?",
          "lv": "Ti je ende aty."
        }
      ],
      "tip": [
        "Diçka po ndodh ende ose nuk ka mbaruar ende.",
        "Përdorni NOH kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Noch = ende.",
        "Diçka po ndodh ende ose nuk ka mbaruar ende."
      ],
      "sectionAccents": {
        "explanation": {},
        "examples": [
          {
            "de": {
              "green": [
                "noch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "noch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "noch"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "noch"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "normal",
    "lv": "Isha e saktë.",
    "level": "A1"
  },
  {
    "de": "November",
    "de_article": "der",
    "lv": "Nëntor",
    "level": "A1"
  },
  {
    "de": "Null",
    "de_article": "die",
    "de_plural": "die Nullen",
    "lv": "Asgjë.",
    "level": "A1"
  },
  {
    "de": "Nummer",
    "de_article": "die",
    "de_plural": "die Nummern",
    "lv": "Gol!",
    "level": "A1"
  },
  {
    "de": "nur",
    "lv": "Vetëm • Vetëm",
    "level": "A1",
    "study": {
      "id": "a1-nur-study",
      "layout": "standardStudy",
      "translation": "Vetëm • Vetëm",
      "explanation": [
        "Ideja kryesore: kufizon numrin, numrin e njerëzve, përzgjedhjen ose opsionet.",
        "Nur në thelb do të thotë: sasi ose zgjedhje e kufizuar.",
        "Shpjegon shpesh: sa, saktësisht, ose kush është i vetmi.",
        "Nur thjesht, thjesht, nuk do të thotë asgjë tjetër: kufizon sasinë ose zgjedhjen."
      ],
      "examples": [
        {
          "de": "Ich habe nur zehn Euro.",
          "lv": "Kam vetëm dhjetë euro."
        },
        {
          "de": "Ich habe nur zehn Euro.",
          "lv": "Kam vetëm dhjetë euro."
        },
        {
          "de": "Nur du kannst mir helfen.",
          "lv": "Vetëm ti mund të më ndihmosh."
        },
        {
          "de": "Ich möchte nur Kaffee.",
          "lv": "Do pi pak kafe."
        },
        {
          "de": "Ich habe nur acht Euro.",
          "lv": "Kam vetëm tetë euro."
        }
      ],
      "tip": [
        "Kufizon sasinë, numrin e njerëzve, përzgjedhjen ose opsionet.",
        "Përdoret vetëm kur konteksti përputhet me këtë kuptim."
      ],
      "important": [
        "Në gjermanisht, \"vetëm\" në letonisht nuk do të thotë gjithmonë nur.",
        "Nur = vetëm/ekskluzivisht."
      ],
      "sectionAccents": {
        "explanation": {
          "orange": [
            "nur"
          ]
        },
        "examples": [
          {
            "de": {
              "orange": [
                "nur"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "orange": [
                "nur"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "orange": [
                "nur"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "orange": [
                "nur"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "orange": [
                "nur"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {
            "orange": [
              "nur"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "ob",
    "lv": "Apo...",
    "level": "A1",
    "study": {
      "id": "a1-ob",
      "layout": "standardStudy",
      "translation": "Apo...",
      "explanation": [
        "Ideja kryesore: Ob ngre një pyetje indirekte dhe do të thotë kryesisht ose në gjuhën letoneze.",
        "Ob përdoret pas fjalëve të tilla si fragen, wissen, sehen, sagen kur nuk ka pyetje të drejtpërdrejtë.",
        "Ob zakonisht nuk përdoret në një pyetje të drejtpërdrejtë në gjermanisht.",
        "Në nivelin A1, është e rëndësishme të bëhet dallimi midis ob dhe ob."
      ],
      "examples": [
        {
          "de": "Ich weiß nicht, ob er kommt.",
          "lv": "Nuk e di nëse do të vish."
        },
        {
          "de": "Sie fragt, ob du Zeit hast.",
          "lv": "Ai pyet nëse ke kohë."
        },
        {
          "de": "Sag mir, ob das stimmt.",
          "lv": "Më thuaj nëse është e vërtetë."
        },
        {
          "de": "Kommst du heute oder morgen?",
          "lv": "Do të vish sot apo nesër?"
        }
      ],
      "comparison": [
        {
          "word": "ob",
          "meaning": "Ose në një pyetje indirekte",
          "example": "Ich weiß nicht, ob er kommt."
        },
        {
          "word": "oder",
          "meaning": "Ose zgjidh një nga opsionet",
          "example": "Kaffee oder Tee?"
        },
        {
          "word": "wenn",
          "meaning": "Nëse/kur",
          "example": "Wenn du Zeit hast..."
        },
        {
          "word": "dass",
          "meaning": "Asaj.",
          "example": "Ich weiß, dass er kommt."
        }
      ],
      "tip": {
        "text": "Mos harroni: Nuk e di... → ob • Nëse është kafe → apo çaj."
      },
      "important": [
        "Ob nuk është forma tipike e zgjedhjes \"ose\" midis dy gjërave.",
        "Kafeja ose bluza? përdoret në vend të ob."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "ob",
            "Ideja"
          ],
          "green": [
            "fragen",
            "wissen",
            "sehen",
            "sagen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "ob"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ob"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ob"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "oder"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "ob"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "ob"
              ]
            }
          },
          {
            "word": {
              "green": [
                "oder"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "wenn"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "dass"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "dass"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "ob"
            ]
          },
          {
            "red": [
              "Kafeja"
            ],
            "blue": [
              "ob"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "oben",
    "lv": "-Ky është një kostum kllouni.",
    "level": "A1"
  },
  {
    "de": "oder",
    "lv": "OR • OR",
    "level": "A1",
    "study": {
      "id": "a1-oder",
      "layout": "standardStudy",
      "translation": "OR • OR",
      "explanation": [
        "Ideja kryesore: Oder përdoret kur zgjedh midis dy ose më shumë opsioneve.",
        "Në letonisht oder do të thotë kryesisht ose.",
        "Kjo nuk është e njëjtë me Ob që shtron një pyetje indirekte.",
        "Në fjalime, urdhri mund të vendoset edhe në fund të fjalisë: Du kommst, oder?"
      ],
      "examples": [
        {
          "de": "Kaffee oder Tee?",
          "lv": "Kafe apo çaj?"
        },
        {
          "de": "Heute oder morgen?",
          "lv": "Sot apo nesër?"
        },
        {
          "de": "Willst du Pizza oder Salat?",
          "lv": "Dëshiron pica apo sallatë?"
        },
        {
          "de": "Du kommst, oder?",
          "lv": "Ti je duke ardhur, saktë?"
        }
      ],
      "comparison": [
        {
          "word": "oder",
          "meaning": "Ose zgjidh",
          "example": "Kaffee oder Tee?"
        },
        {
          "word": "ob",
          "meaning": "Ose në një pyetje indirekte",
          "example": "Ich weiß nicht, ob er kommt."
        },
        {
          "word": "und",
          "meaning": "Dhe, hmm...",
          "example": "Kaffee und Kuchen."
        },
        {
          "word": "aber",
          "meaning": "Megjithatë...",
          "example": "Ich komme, aber später."
        }
      ],
      "tip": {
        "text": "Mos harroni: zgjidhni midis opsioneve → ose."
      },
      "important": [
        "Ju mund të zgjidhni nga: Kaffee oder tee.",
        "Në një pyetje indirekte, \"nëse\" zakonisht është ob."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "oder",
            "ob"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "oder"
              ],
              "yellow": [
                "Kaffee",
                "Tee"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "oder"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "oder"
              ],
              "yellow": [
                "Pizza",
                "Salat"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "oder"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "oder"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "ob"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "ob"
              ]
            }
          },
          {
            "word": {
              "green": [
                "und"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "aber"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "aber"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "oder"
            ]
          },
          {
            "red": [
              "ob"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "öffnen",
    "lv": "Hapur.",
    "level": "A1"
  },
  {
    "de": "oft",
    "lv": "Shumë.",
    "level": "A1"
  },
  {
    "de": "ohne",
    "lv": "Pa",
    "level": "A1"
  },
  {
    "de": "Ohr",
    "de_article": "das",
    "de_plural": "die Ohren",
    "lv": "Një vesh.",
    "level": "A1"
  },
  {
    "de": "Oktober",
    "de_article": "der",
    "lv": "Tetor.",
    "level": "A1"
  },
  {
    "de": "Onkel",
    "de_article": "der",
    "de_plural": "die Onkel",
    "lv": "Xhaxha......",
    "level": "A1"
  },
  {
    "de": "Orange",
    "de_article": "die",
    "de_plural": "die Orangen",
    "lv": "-Um... ngjyr portokalli. Portkalli?",
    "level": "A1"
  },
  {
    "de": "Ostern",
    "lv": "Pashkët.",
    "level": "A1"
  },
  {
    "de": "Paar",
    "de_article": "das",
    "de_plural": "die Paare",
    "lv": "Çift",
    "level": "A1"
  },
  {
    "de": "Papier",
    "de_article": "das",
    "de_plural": "die Papiere",
    "lv": "Faqen.",
    "level": "A1"
  },
  {
    "de": "Park",
    "de_article": "der",
    "de_plural": "die Parks",
    "lv": "Në park.",
    "level": "A1"
  },
  {
    "de": "passen",
    "lv": "Po, në rregull.",
    "level": "A1",
    "study": {
      "id": "a1-passen",
      "layout": "standardStudy",
      "translation": "Po, në rregull.",
      "explanation": [
        "Ideja kryesore: passen do të thotë përshtatje, përshtatje ose përshtatje.",
        "Kur bëhet fjalë për veshjet, passen zakonisht nënkupton rregullimin e madhësisë.",
        "Kur bëhet fjalë për ngjyrat dhe stilin, passen përfaqëson përputhjen.",
        "Një shprehje shumë e njohur është das passt. = Përputhet."
      ],
      "examples": [
        {
          "de": "Die Jacke passt mir.",
          "lv": "Xhaketa më përshtatet."
        },
        {
          "de": "Das Kleid passt gut.",
          "lv": "Fustani të duket shumë mirë."
        },
        {
          "de": "Die Farbe passt zu dir.",
          "lv": "Kjo ngjyrë të përshtatet ty."
        },
        {
          "de": "Das passt.",
          "lv": "Përshtatet."
        }
      ],
      "comparison": [
        {
          "word": "passen",
          "meaning": "Ai po fIe!",
          "example": "Die Jacke passt mir."
        },
        {
          "word": "stehen",
          "meaning": "Në këmbë / në këmbë",
          "example": "Rot steht dir gut."
        },
        {
          "word": "geeignet sein",
          "meaning": "Kualifikohu",
          "example": "Das ist geeignet."
        },
        {
          "word": "funktionieren",
          "meaning": "Biznes",
          "example": "Das funktioniert."
        }
      ],
      "tip": {
        "text": "Atceries: Das passt. = Tas der."
      },
      "important": [
        "Passen nuk ka të bëjë vetëm me veshjet.",
        "Kjo mund të nënkuptojë gjithashtu se koha, plani ose zgjidhja është e duhura."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "passen",
            "Das passt"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "passt"
              ],
              "yellow": [
                "Jacke"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "passt"
              ],
              "yellow": [
                "Kleid"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "passt"
              ],
              "yellow": [
                "Farbe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Das passt"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "passen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "stehen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "steht"
              ]
            }
          },
          {
            "word": {
              "green": [
                "geeignet sein"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "funktionieren"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "passen"
            ]
          },
          {
            "purple": [
              "plani"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Pause",
    "de_article": "die",
    "de_plural": "die Pausen",
    "lv": "Pauzë",
    "level": "A1"
  },
  {
    "de": "Person",
    "de_article": "die",
    "de_plural": "die Personen",
    "lv": "Person.",
    "level": "A1"
  },
  {
    "de": "Pferd",
    "de_article": "das",
    "de_plural": "die Pferde",
    "lv": "Gjuajtje madhështore!",
    "level": "A1"
  },
  {
    "de": "Pflanze",
    "de_article": "die",
    "de_plural": "die Pflanzen",
    "lv": "Objekti",
    "level": "A1"
  },
  {
    "de": "Pizza",
    "de_article": "die",
    "de_plural": "die Pizzen",
    "lv": "E vogël.",
    "level": "A1"
  },
  {
    "de": "Plan",
    "de_article": "der",
    "de_plural": "die Pläne",
    "lv": "Plani",
    "level": "A1"
  },
  {
    "de": "Platz",
    "de_article": "der",
    "de_plural": "die Plätze",
    "lv": "Dhoma të lira.",
    "level": "A1"
  },
  {
    "de": "Polizei",
    "de_article": "die",
    "lv": "Policia",
    "level": "A1"
  },
  {
    "de": "Post",
    "de_article": "die",
    "lv": "Posta",
    "level": "A1"
  },
  {
    "de": "Preis",
    "de_article": "der",
    "de_plural": "die Preise",
    "lv": "Çmimi",
    "level": "A1"
  },
  {
    "de": "probieren",
    "lv": "Provoje • Shijo",
    "level": "A1",
    "study": {
      "id": "a1-probieren",
      "layout": "standardStudy",
      "translation": "Provoje • Shijo",
      "explanation": [
        "Ideja kryesore: probieren do të thotë të shijosh ose të shijosh.",
        "Kur bëhet fjalë për ushqim dhe pije, probieren zakonisht do të thotë shijim.",
        "Do të thotë të provosh prova kur bëhet fjalë për një veprim, metodë ose gjë.",
        "Kjo nuk është e njëjtë me prüfen, që do të thotë kontroll më i plotë."
      ],
      "examples": [
        {
          "de": "Probier mal die Suppe!",
          "lv": "Shijo supën!"
        },
        {
          "de": "Ich möchte den Kuchen probieren.",
          "lv": "Dua të provoj tortën."
        },
        {
          "de": "Wir probieren eine neue Methode.",
          "lv": "Po provojmë një metodë të re."
        },
        {
          "de": "Kann ich die Jacke anprobieren?",
          "lv": "A mund ta provoj xhaketën?"
        }
      ],
      "comparison": [
        {
          "word": "probieren",
          "meaning": "Provoje/shijoje",
          "example": "Probier mal die Suppe!"
        },
        {
          "word": "versuchen",
          "meaning": "Bëj ca gjuajte.",
          "example": "Ich versuche es."
        },
        {
          "word": "prüfen",
          "meaning": "Dhe ti.",
          "example": "Ich prüfe die Rechnung."
        },
        {
          "word": "anprobieren",
          "meaning": "Bëj ca gjuajte.",
          "example": "Ich probiere die Jacke an."
        }
      ],
      "tip": {
        "text": "Mos harroni: probieren → e ushqimit = degustim."
      },
      "important": [
        "Probieren nuk është fjala kryesore e një provimi zyrtar.",
        "Zakonisht preferohet të kontrollosh një dokument ose faturë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "probieren",
            "prüfen"
          ],
          "yellow": [
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Probier"
              ],
              "yellow": [
                "Suppe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "probieren"
              ],
              "yellow": [
                "Kuchen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "probieren"
              ],
              "yellow": [
                "Methode"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "anprobieren"
              ],
              "yellow": [
                "Jacke"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "probieren"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "versuchen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "versuche"
              ]
            }
          },
          {
            "word": {
              "green": [
                "prüfen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "prüfe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "anprobieren"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "probiere",
                "an"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "probieren"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "Problem",
    "de_article": "das",
    "de_plural": "die Probleme",
    "lv": "Probleme.",
    "level": "A1"
  },
  {
    "de": "Programm",
    "de_article": "das",
    "de_plural": "die Programme",
    "lv": "Programi",
    "level": "A1"
  },
  {
    "de": "Pullover",
    "de_article": "der",
    "de_plural": "die Pullover",
    "lv": "Trikon.",
    "level": "A1"
  },
  {
    "de": "Punkt",
    "de_article": "der",
    "de_plural": "die Punkte",
    "lv": "Pikë.",
    "level": "A1"
  },
  {
    "de": "putzen",
    "lv": "Për të pastruar",
    "level": "A1"
  },
  {
    "de": "Rad fahren",
    "lv": "Ciclizëm",
    "level": "A1"
  },
  {
    "de": "Radiergummi",
    "de_article": "der",
    "de_plural": "die Radiergummis",
    "lv": "Fshirës",
    "level": "A1"
  },
  {
    "de": "rauchen",
    "lv": "Për të djegur.",
    "level": "A1"
  },
  {
    "de": "rechts",
    "lv": "Djathtas, djathtas.",
    "level": "A1"
  },
  {
    "de": "reden",
    "lv": "Thuaj diçka.",
    "level": "A1"
  },
  {
    "de": "Regen",
    "de_article": "der",
    "lv": "Shiu",
    "level": "A1"
  },
  {
    "de": "Regenschirm",
    "de_article": "der",
    "de_plural": "die Regenschirme",
    "lv": "Një çadër.",
    "level": "A1"
  },
  {
    "de": "regnen",
    "lv": "Po bie shi!",
    "level": "A1"
  },
  {
    "de": "Reis",
    "de_article": "der",
    "lv": "Oriz.",
    "level": "A1",
    "study": {
      "id": "a1-reis",
      "layout": "standardStudy",
      "translation": "Oriz.",
      "explanation": "Almanca'da \"der Reis\" kelimesi yalnızca tekil olarak kullanılır, bu nedenle cümledeki fiilin tekil olması gerekir (örneğin \"sind\" değil \"ist\"). Ancak Letonyalılar sıklıkla \"pirinç\" derler.",
      "examples": [
        {
          "de": "Der Reis ist fertig.",
          "lv": "Orizi është gati."
        },
        {
          "de": "Ich esse Reis.",
          "lv": "Unë ha oriz"
        },
        {
          "de": "Kochst du Reis?",
          "lv": "Pirinç pişirir misin?"
        },
        {
          "de": "Der Reis schmeckt gut.",
          "lv": "Orizi ka shije të mirë."
        }
      ],
      "tip": {
        "text": "Mos harroni: der Reis është njëjës në gjermanisht, por në letonisht zakonisht do të thotë oriz."
      },
      "important": [
        "Der Reis - Almanca'da yalnızca tekil (Der Reis ist..., nicht *sind).",
        "Kur përkthehet në letonisht, shpesh përdoret në shumës: orizi është gati."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "der Reis",
            "ist"
          ],
          "purple": [
            "Pirinç"
          ],
          "red": [
            "sind"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Reis",
                "ist"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Reis"
              ]
            },
            "lv": {}
          }
        ],
        "tip": {}
      }
    }
  },
  {
    "de": "richtig",
    "lv": "Isha e saktë.",
    "level": "A1"
  },
  {
    "de": "rosa",
    "lv": "Pembe",
    "level": "A1"
  },
  {
    "de": "rot",
    "lv": "E kuqe",
    "level": "A1"
  },
  {
    "de": "rufen",
    "lv": "Kërko",
    "level": "A1"
  },
  {
    "de": "rund",
    "lv": "Yuvarlak",
    "level": "A1"
  },
  {
    "de": "Rose",
    "de_article": "die",
    "de_plural": "die Rosen",
    "lv": "Gül",
    "level": "A1"
  },
  {
    "de": "Sache",
    "de_article": "die",
    "de_plural": "die Sachen",
    "lv": "E pra,",
    "level": "A1"
  },
  {
    "de": "Saft",
    "de_article": "der",
    "de_plural": "die Säfte",
    "lv": "Lëng frutash.",
    "level": "A1"
  },
  {
    "de": "sagen",
    "lv": "Trego",
    "level": "A1",
    "study": {
      "id": "a1-sagen-study",
      "layout": "standardStudy",
      "translation": "Trego",
      "explanation": [
        "Ideja kryesore: Përshkruani një ide, fjalë ose frazë specifike.",
        "Sagen në thelb do të thotë: të paraqesë një pikë të veçantë.",
        "Shpesh përcakton: fjalët/fjalitë.",
        "Sagen përdoret për të treguar një tekst specifik."
      ],
      "examples": [
        {
          "de": "Was hast du gesagt?",
          "lv": "Çfarë the? - Nuk jemi në një vend..."
        }
      ],
      "comparison": [
        {
          "word": "sagen",
          "meaning": "Trego (tekst specifik)",
          "example": "Was hast du gesagt? – Nuk jemi në një vend..."
        },
        {
          "word": "sprechen",
          "meaning": "Konuş (dil, konuş)",
          "example": "Ich spreche Deutsch. – Unë flas gjermanisht."
        }
      ],
      "tip": [
        "Sagen = söylemek",
        "Përdorni sagen kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Sagen = söylemek.",
        "Komunikimi i një mendimi, fjale ose deklarate të veçantë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "sagen"
          ]
        },
        "examples": [
          {
            "de": {},
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "sagen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Salat",
    "de_article": "der",
    "de_plural": "die Salate",
    "lv": "Salata",
    "level": "A1"
  },
  {
    "de": "Salz",
    "de_article": "das",
    "de_plural": "die Salze",
    "lv": "Kripë.",
    "level": "A1"
  },
  {
    "de": "sauber",
    "lv": "Temiz",
    "level": "A1"
  },
  {
    "de": "Schaf",
    "de_article": "das",
    "de_plural": "die Schafe",
    "lv": "Koyun",
    "level": "A1"
  },
  {
    "de": "schauen",
    "lv": "Izlemek için",
    "level": "A1",
    "study": {
      "id": "a1-schauen-study",
      "layout": "standardStudy",
      "translation": "Izlemek için",
      "explanation": [
        "Ideja kryesore: Shikim aktiv ose shikim.",
        "Schauen do të thotë mbi të gjitha: vëzhgim aktiv.",
        "Çoğunlukla şunu tanımlar: eylem.",
        "Schauen do të thotë pamje aktive."
      ],
      "examples": [
        {
          "de": "Ich schaue fern.",
          "lv": "Televizyon izliyorum."
        },
        {
          "de": "Wir schauen aus dem Fenster.",
          "lv": "Shikojmë nga dritarja."
        },
        {
          "de": "Ich schaue fern.",
          "lv": "Po shikoj televizor."
        }
      ],
      "comparison": [
        {
          "word": "schauen",
          "meaning": "Monitor (aktiv)",
          "example": "Ich schaue aus dem Fenster. – Es patenos pa logu."
        },
        {
          "word": "sehen",
          "meaning": "Shiko (nuk ka qëllim)",
          "example": "Ich sehe dich. – Dhe une të shikoj."
        }
      ],
      "tip": [
        "Schauen = shiko",
        "Përdorni schauen kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Schauen = bakmak.",
        "Vëzhgoni ose shikoni në mënyrë aktive."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "schauen"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "schaue"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "schauen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "schaue"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "schauen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "schicken",
    "lv": "Göndermek",
    "level": "A1"
  },
  {
    "de": "schlafen",
    "lv": "-Une?",
    "level": "A1"
  },
  {
    "de": "schlecht",
    "lv": "Keq.",
    "level": "A1"
  },
  {
    "de": "Schlüssel",
    "de_article": "der",
    "de_plural": "die Schlüssel",
    "lv": "Anahtar",
    "level": "A1"
  },
  {
    "de": "schmecken",
    "lv": "Tatmak",
    "level": "A1"
  },
  {
    "de": "schmutzig",
    "lv": "Kirli",
    "level": "A1"
  },
  {
    "de": "Schnee",
    "de_article": "der",
    "lv": "Do të bjerë borë",
    "level": "A1"
  },
  {
    "de": "schneien",
    "lv": "Po bie borë. - Po.",
    "level": "A1"
  },
  {
    "de": "schnell",
    "lv": "Poshtë .",
    "level": "A1"
  },
  {
    "de": "Schokolade",
    "de_article": "die",
    "lv": "Çikolata",
    "level": "A1"
  },
  {
    "de": "schon",
    "lv": "Jemi të vdekur.",
    "level": "A1",
    "study": {
      "id": "a1-schon-study",
      "layout": "standardStudy",
      "translation": "Jemi të vdekur.",
      "explanation": [
        "Ideja kryesore: diçka tashmë ka ndodhur ose është tashmë në fuqi.",
        "Schon në thelb do të thotë se diçka tashmë ka ndodhur ose është në fuqi.",
        "Më së shumti përshkruan: një fenomen që ndodh ose një situatë aktuale.",
        "Schon tashmë do të thotë: diçka tashmë ka ndodhur ose tashmë është në fuqi."
      ],
      "examples": [
        {
          "de": "Ich bin schon zu Hause.",
          "lv": "Zaten evdeyim"
        }
      ],
      "tip": [
        "Bir şey zaten olmuştur veya halihazırda yürürlüktedir.",
        "Përdorni schon kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Schon = zaten.",
        "Bir şey zaten olmuştur veya halihazırda yürürlüktedir."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "schon"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "schon"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "schon"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "schön",
    "lv": "Jo.",
    "level": "A1"
  },
  {
    "de": "Schrank",
    "de_article": "der",
    "de_plural": "die Schränke",
    "lv": "Në zyrë.",
    "level": "A1"
  },
  {
    "de": "schreiben",
    "lv": "Yazmak",
    "level": "A1"
  },
  {
    "de": "Schuh",
    "de_article": "der",
    "de_plural": "die Schuhe",
    "lv": "Këpucët.",
    "level": "A1"
  },
  {
    "de": "Schule",
    "de_article": "die",
    "de_plural": "die Schulen",
    "lv": "Okul",
    "level": "A1"
  },
  {
    "de": "Schüler",
    "de_article": "der",
    "de_plural": "die Schüler",
    "lv": "Student",
    "level": "A1"
  },
  {
    "de": "schwarz",
    "lv": "Siyah",
    "level": "A1"
  },
  {
    "de": "Schwein",
    "de_article": "das",
    "de_plural": "die Schweine",
    "lv": "Derr.",
    "level": "A1"
  },
  {
    "de": "Schwester",
    "de_article": "die",
    "de_plural": "die Schwestern",
    "lv": "'Motër'?",
    "level": "A1"
  },
  {
    "de": "schwimmen",
    "lv": "Not",
    "level": "A1",
    "study": {
      "id": "a1-schwimmen",
      "layout": "standardStudy",
      "translation": "Not",
      "explanation": [
        "Ideja kryesore: \"schwimmen\" nënkupton notin si lëvizje ose sport.",
        "Schwimmen përdoret kur një person noton në ujë me një lëvizje noti.",
        "Suda rahatlamak veya yüzmek söz konusu olduğunda baden sıklıkla kullanılır.",
        "Në nivelin A1 është e rëndësishme të dallojmë: schwimmen = not, baden = banjë."
      ],
      "examples": [
        {
          "de": "Ich schwimme gern.",
          "lv": "Më pëlqen noti"
        },
        {
          "de": "Er schwimmt sehr gut.",
          "lv": "Ajo noton shumë mirë."
        },
        {
          "de": "Wir schwimmen im Schwimmbad.",
          "lv": "Havuzda yüzüyoruz."
        },
        {
          "de": "Ich gehe baden.",
          "lv": "Do shkoj të notoj."
        }
      ],
      "comparison": [
        {
          "word": "schwimmen",
          "meaning": "Noti si lëvizje ose sport",
          "example": "Er schwimmt sehr gut."
        },
        {
          "word": "baden",
          "meaning": "Noto/qëndro në ujë",
          "example": "Ich gehe baden."
        },
        {
          "word": "schwimmen gehen",
          "meaning": "Shko të notosh",
          "example": "Wir gehen schwimmen."
        },
        {
          "word": "duschen",
          "meaning": "Bëj një dush.",
          "example": "Ich dusche morgens."
        }
      ],
      "tip": {
        "text": "Mos harroni: → notarët e lëvizjes së notit • Të pushosh në vaskë me → ujë."
      },
      "important": [
        "Schwimmen ve Baden aynı değil.",
        "Shpesh thuhet \"not\" në letonisht, por në gjermanisht duhet të kontrollosh nëse kjo do të thotë të lëvizësh ose të bësh banjë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "schwimmen"
          ],
          "red": [
            "baden"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "schwimme"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "schwimmt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "schwimmen"
              ],
              "green": [
                "Schwimmbad"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "baden"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "schwimmen"
              ]
            },
            "meaning": {
              "purple": [
                "spor"
              ]
            },
            "example": {
              "blue": [
                "schwimmt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "baden"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "baden"
              ]
            }
          },
          {
            "word": {
              "green": [
                "schwimmen gehen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "schwimmen"
              ]
            }
          },
          {
            "word": {
              "green": [
                "duschen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "dusche"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "schwimmen"
            ],
            "red": [
              "baden"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "sechs",
    "lv": "-Gjashtë?",
    "level": "A1"
  },
  {
    "de": "sechshundert",
    "lv": "Gjashtëqind",
    "level": "A1"
  },
  {
    "de": "sechste",
    "lv": "Altıncı",
    "level": "A1"
  },
  {
    "de": "sechzehn",
    "lv": "16.",
    "level": "A1"
  },
  {
    "de": "sechzehnte",
    "lv": "On altıncı",
    "level": "A1"
  },
  {
    "de": "sechzig",
    "lv": "Gjashtedhjete!",
    "level": "A1"
  },
  {
    "de": "sechzigste",
    "lv": "Altmışıncı",
    "level": "A1"
  },
  {
    "de": "sehen",
    "lv": "Görmek",
    "level": "A1",
    "study": {
      "id": "a1-sehen",
      "layout": "standardStudy",
      "translation": "Görmek",
      "explanation": [
        "Ideja kryesore: të shohësh do të thotë të shohësh me sy.",
        "Fjala sehen përdoret kur bëhet fjalë për atë që percepton syri.",
        "Kasıtlı izleme genellikle schauen veya ansehen'dir.",
        "Një frazë shumë e njohur është ich sehe dich. = Të shoh."
      ],
      "examples": [
        {
          "de": "Ich sehe dich.",
          "lv": "Dhe une të shikoj."
        },
        {
          "de": "Siehst du das Auto?",
          "lv": "E shihni këtë makinë?"
        },
        {
          "de": "Ich sehe nichts.",
          "lv": "Nuk po shohe asgje. Shiko me shume."
        },
        {
          "de": "Wir schauen einen Film.",
          "lv": "Po shohim një film."
        }
      ],
      "comparison": [
        {
          "word": "sehen",
          "meaning": "Görmek",
          "example": "Ich sehe dich."
        },
        {
          "word": "schauen",
          "meaning": "Izlemek için",
          "example": "Ich schaue auf das Bild."
        },
        {
          "word": "ansehen",
          "meaning": "Shiko dhe dicka tjeter.",
          "example": "Ich sehe mir den Film an."
        },
        {
          "word": "hören",
          "meaning": "Seanca dëgjimore",
          "example": "Ich höre Musik."
        }
      ],
      "tip": {
        "text": "Unutmayın: gözler algılar → bakın • Bilinçli olarak görün → schauen/ansehen."
      },
      "important": [
        "Seehen nuk është i njëjtë me anschauen.",
        "Ich sehe dich = I see you • Ich schaue den Film = Po shikoj një film."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "sehen",
            "Ich sehe"
          ],
          "red": [
            "schauen",
            "ansehen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "sehe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Siehst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sehe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "schauen"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "sehen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "schauen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "schaue"
              ]
            }
          },
          {
            "word": {
              "green": [
                "ansehen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "sehe",
                "an"
              ]
            }
          },
          {
            "word": {
              "green": [
                "hören"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "Seehen"
            ],
            "red": [
              "anschauen"
            ]
          },
          {
            "blue": [
              "sehe"
            ],
            "red": [
              "schaue"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "sehr",
    "lv": "Çok",
    "level": "A1"
  },
  {
    "de": "Seife",
    "de_article": "die",
    "de_plural": "die Seifen",
    "lv": "Sabun",
    "level": "A1"
  },
  {
    "de": "sein",
    "lv": "Të jesh",
    "level": "A1",
    "study": {
      "id": "a1-sein",
      "layout": "standardStudy",
      "translation": "Të jesh",
      "explanation": [
        "Ana fikir: sein olmak demektir.",
        "Sein është një nga foljet më të rëndësishme në gjermanisht.",
        "Format veçanërisht të rëndësishme në nivelin A1 janë ich bin, du bist, er ist dhe wir sind.",
        "Sein ayrıca birçok yerel veya karakteristik cümlede de kullanılır."
      ],
      "examples": [
        {
          "de": "Ich bin hier.",
          "lv": "-Këtej."
        },
        {
          "de": "Du bist müde.",
          "lv": "Je e lodhur."
        },
        {
          "de": "Er ist Lehrer.",
          "lv": "Ai është mësues."
        },
        {
          "de": "Wir sind zu Hause.",
          "lv": "Jemi në shtëpi."
        }
      ],
      "comparison": [
        {
          "word": "sein",
          "meaning": "Të jesh",
          "example": "Ich bin hier."
        },
        {
          "word": "haben",
          "meaning": "Po, e kam.",
          "example": "Ich habe Zeit."
        },
        {
          "word": "werden",
          "meaning": "Duke u bërë",
          "example": "Ich werde müde."
        },
        {
          "word": "bleiben",
          "meaning": "Qëndro",
          "example": "Ich bleibe hier."
        }
      ],
      "tip": {
        "text": "Atceries: ich bin = esmu • Du bist = tu esi."
      },
      "important": [
        "Duhet të shqyrtohet edhe forma SEIN: BIN, BIST, IST, Sind.",
        "Onların kutusu \"ben varım\" değil, \"ben varım\"dır."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "sein",
            "ich bin",
            "du bist",
            "er ist",
            "wir sind"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "bin"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "bist"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ist"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sind"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "sein"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "haben"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "werden"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "werde"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bleiben"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "bleibe"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "bin",
              "bist",
              "ist",
              "sind"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "seit",
    "lv": "Nga",
    "level": "A1"
  },
  {
    "de": "Seite",
    "de_article": "die",
    "de_plural": "die Seiten",
    "lv": "Sayfa • Sayfa",
    "level": "A1",
    "study": {
      "id": "a1-seite",
      "layout": "standardStudy",
      "translation": "Sayfa • Sayfa",
      "explanation": [
        "Ana fikir: die Seite bir kitabın/belgenin sayfası veya bir şeyin yan tarafı/kenarı anlamına gelebilir.",
        "Die Seite = faqe (Faqe 5 = faqe 5) në një libër, revistë ose faqe interneti.",
        "Mekansal anlamda, die Seite = yan (auf der linken Seite = solda).",
        "Metaforikisht, die Seite mund të nënkuptojë gjithashtu një palë në konflikt ose mendim (auf meiner Seite = në anën time).",
        "Konteksti (libri/leximi ose artikulli/marrëdhënia) tregon kuptimin e saktë.",
        "Shumës për të dy kuptimet: die Seiten."
      ],
      "examples": [
        {
          "de": "Schlagt die Seite zwanzig auf.",
          "lv": "Shko te faqja 20."
        },
        {
          "de": "Auf der linken Seite ist ein Park.",
          "lv": "Ka një park në anën e majtë."
        },
        {
          "de": "Die Webseite lädt langsam.",
          "lv": "Web sitesi yavaş yükleniyor."
        },
        {
          "de": "Er steht auf meiner Seite.",
          "lv": "Ai është në anën time."
        },
        {
          "de": "Das Buch hat 200 Seiten.",
          "lv": "Libri ka 200 faqe."
        },
        {
          "de": "Auf der anderen Seite der Straße.",
          "lv": "Përtej rrugës."
        }
      ],
      "tip": [
        "Të flasësh për një libër ose të lexosh një → faqe. Ai jep informacion në → lidhje me pozicionin, drejtimin ose marrëdhëniet e palës.",
        "Faqja X është gjithmonë një faqe e një libri, jo gjysma e tij."
      ],
      "important": [
        "Die Seite = sayfa VEYA sayfa – bağlam belirler.",
        "Shumës për të dy kuptimet: die Seiten."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "die Seite",
            "Seite"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Seite"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Seite"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Webseite"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Seite"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Seiten"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Seite"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {
            "blue": [
              "flasësh"
            ]
          }
        ],
        "important": [
          {},
          {
            "blue": [
              "die Seiten"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Sekunde",
    "de_article": "die",
    "de_plural": "die Sekunden",
    "lv": "\"Më vjen keq, por ti po humbet kohë. \"",
    "level": "A1"
  },
  {
    "de": "September",
    "de_article": "der",
    "lv": "Shtator",
    "level": "A1"
  },
  {
    "de": "sich",
    "lv": "Kendiniz • Kendiniz için",
    "level": "A1",
    "study": {
      "id": "a1-sich",
      "layout": "standardStudy",
      "translation": "Kendiniz • Kendiniz için",
      "explanation": [
        "Ideja kryesore: Tregon se veprimi është i lidhur me vetë autorin.",
        "Letonca'da genellikle benlik veya benlik olarak tercüme edilir.",
        "Në disa folje gjermane, sich është pjesë e detyrueshme, p.sh. sich waschen.",
        "Një pikë e rëndësishme për t 'u theksuar në nivelin A1: ich wasche mich, er wäscht sich."
      ],
      "examples": [
        {
          "de": "Er wäscht sich.",
          "lv": "Ai po bën banjë."
        },
        {
          "de": "Ich setze mich.",
          "lv": "Oturuyorum."
        },
        {
          "de": "Sie freut sich.",
          "lv": "Ajo është e lumtur."
        },
        {
          "de": "Ich wasche das Auto.",
          "lv": "Po laj makinën"
        }
      ],
      "comparison": [
        {
          "word": "sich",
          "meaning": "Mendja ime.",
          "example": "Er wäscht sich."
        },
        {
          "word": "mich",
          "meaning": "Unë/Unë jam i tyre",
          "example": "Ich wasche mich."
        },
        {
          "word": "dich",
          "meaning": "Sen/ben du'da",
          "example": "Du wäschst dich."
        },
        {
          "word": "ihn",
          "meaning": "Ai është...",
          "example": "Ich sehe ihn."
        }
      ],
      "tip": {
        "text": "Mos harroni: veproni → sipas dëshirës suaj."
      },
      "important": [
        "Sich bağımsız bir isim değildir.",
        "Varet nga personi: ich → mich, du → dich, er/sie/es → sich."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "sich",
            "ich wasche mich",
            "er wäscht sich"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "sich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "mich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sich"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "wasche"
              ],
              "yellow": [
                "Auto"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "sich"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "sich"
              ]
            }
          },
          {
            "word": {
              "green": [
                "mich"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "mich"
              ]
            }
          },
          {
            "word": {
              "green": [
                "dich"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "ihn"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "sich"
            ]
          },
          {
            "green": [
              "mich",
              "dich"
            ],
            "blue": [
              "sich"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "sicher",
    "lv": "Güvenli • Kesinlikle",
    "level": "A1",
    "study": {
      "id": "a1-sicher",
      "layout": "standardStudy",
      "translation": "Güvenli • Kesinlikle",
      "explanation": [
        "Ideja kryesore: sicher do të thotë e përcaktuar si mbiemër, dhe patjetër/ndoshta si zarf.",
        "Bir yer, durum veya kişi hakkında konuştuğumuzda, sicher = güvenli (ein sicherer Ort = güvenli yer).",
        "Si konfirmim ose siguri në një fjali sicher = absolutisht/natyrisht (Das ist sicher wahr. = Absolutisht e vërtetë.).",
        "Elbette! ayrı bir cevap olarak elbette!/muhtemelen! anlamına gelir."
      ],
      "examples": [
        {
          "de": "Ist das Wasser sicher?",
          "lv": "A është uji i sigurt?"
        },
        {
          "de": "Kommst du morgen? – Sicher!",
          "lv": "Do të vish nesër - me siguri!"
        },
        {
          "de": "Er ist sicher zu Hause.",
          "lv": "Me siguri është në shtëpi."
        },
        {
          "de": "Das ist eine sichere Lösung.",
          "lv": "Kjo është një zgjidhje e sigurt."
        },
        {
          "de": "Ich bin mir sicher.",
          "lv": "Eminim"
        },
        {
          "de": "Fahr sicher!",
          "lv": "Güvenli bir şekilde sürün!"
        }
      ],
      "tip": [
        "→ I sigurt për një vend ose situatë.",
        "→ Definitivisht/ndoshta si besim ose konfirmim në një fjali."
      ],
      "important": [
        "Sicher = i sigurt (mbiemër) OSE patjetër/ndoshta (ndajfolje).",
        "Sich sicher sein = kesinlikle."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "sicher"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "sicher"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "Sicher"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "sicher"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sichere"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "sicher"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sicher"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {},
          {
            "green": [
              "sich sicher sein"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "sie",
    "lv": "Onlar / o",
    "level": "A1",
    "study": {
      "id": "a1-sie-study",
      "layout": "standardStudy",
      "translation": "Onlar / o",
      "explanation": [
        "Ideja kryesore: Plural – përmend më shumë se një person. Folja mbaron me -en: kochen, essen, gehen.",
        "Sie'nin esas anlamı: bir kadın.",
        "Zakonisht karakterizohet nga folja njëjës (-t).",
        "SIE në thelb do të thotë: disa njerëz.",
        "Genellikle şu şekilde karakterize edilir: çoğul bir fiil (-en).",
        "SIE në thelb do të thotë: adresë e sjellshme.",
        "Shpesh përshkruhet: domosdoshmërisht me kapital S.",
        "Rrethimi i vogël e shpreh atë kur folja është njëjës (Sie kocht = ai gatim)."
      ],
      "examples": [
        {
          "de": "Sie kochen.",
          "lv": "Yemek pişiriyorlar."
        },
        {
          "de": "Sie kocht.",
          "lv": "Yemek yapıyor."
        },
        {
          "de": "Sie isst.",
          "lv": "O yiyor"
        },
        {
          "de": "Sie kochen.",
          "lv": "Yemek pişiriyorlar."
        },
        {
          "de": "Sie spielen Fußball.",
          "lv": "Ata po luajnë futboll."
        },
        {
          "de": "Sie kochen, bitte.",
          "lv": "Sen yemek yap lütfen"
        }
      ],
      "tip": [
        "Shumësi: I referohet më shumë se një personi. Folja mbaron me -en: kochen, essen, gehen.",
        "Përdoreni kur konteksti i përshtatet kuptimit."
      ],
      "important": [
        "Her zaman büyük S harfiyle nezaketle: Ağu, Ağustos değil.",
        "Ai: Më vjen keq. Ata: Të dua. Ti: Sie kochen.",
        "E gabuar: Sie kocht → E saktë: Sie kocht",
        "E gabuar: Sie kocht (them) → E saktë: Sie kochen"
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "sie",
            "kochen"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "sie"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "Sie",
    "lv": "Dukesh ...",
    "level": "A1",
    "study": {
      "id": "a1-sie-study-2",
      "layout": "standardStudy",
      "translation": "Dukesh ...",
      "explanation": [
        "Ana fikir: Nezaket adresi - her zaman dodze S. Letonyalı: siz. Çoğu zaman sıradan çoğul bir fiille kullanılır.",
        "SIE në thelb do të thotë: një grua.",
        "Zakonisht karakterizohet nga folja njëjës (-t).",
        "SIE në thelb do të thotë: disa njerëz.",
        "Genellikle şu şekilde karakterize edilir: çoğul bir fiil (-en).",
        "SIE në thelb do të thotë: adresë e sjellshme.",
        "Shpesh përshkruhet: domosdoshmërisht me kapital S.",
        "Rrethimi i vogël e shpreh atë kur folja është njëjës (Sie kocht = ai gatim)."
      ],
      "examples": [
        {
          "de": "Sie kochen, bitte.",
          "lv": "Pişir lütfen."
        },
        {
          "de": "Sie kocht.",
          "lv": "Yemek yapıyor."
        },
        {
          "de": "Sie isst.",
          "lv": "O yiyor"
        },
        {
          "de": "Sie kochen.",
          "lv": "Yemek pişiriyorlar."
        },
        {
          "de": "Sie spielen Fußball.",
          "lv": "Ata po luajnë futboll."
        },
        {
          "de": "Sie kochen, bitte.",
          "lv": "Sen yemek yap lütfen"
        }
      ],
      "tip": [
        "Adresa e mirësjelljes - gjithmonë me dorëzimin S. Letonisht: ju. zakonisht me një folje në shumësin e zakonshëm.",
        "Përdorni SIE kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Her zaman büyük S harfiyle nezaketle: Ağu, Ağustos değil.",
        "Ai: Më vjen keq. Ata: Të dua. Ti: Sie kochen.",
        "E gabuar: Sie kocht → E saktë: Sie kocht",
        "E gabuar: Sie kocht (them) → E saktë: Sie kochen"
      ],
      "sectionAccents": {
        "explanation": {
          "yellow": [
            "Sie",
            "kocht"
          ]
        },
        "examples": [
          {
            "de": {
              "yellow": [
                "Sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "Sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "Sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "Sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "Sie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "Sie"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "siebenhundert",
    "lv": "Shtatëqind",
    "level": "A1"
  },
  {
    "de": "siebente",
    "lv": "I shtati.",
    "level": "A1"
  },
  {
    "de": "siebzehn",
    "lv": "On yedi",
    "level": "A1"
  },
  {
    "de": "siebzehnte",
    "lv": "On yedinci",
    "level": "A1"
  },
  {
    "de": "siebzig",
    "lv": "Shtatëdhjetë.",
    "level": "A1"
  },
  {
    "de": "siebzigste",
    "lv": "E shtatëdhjeta",
    "level": "A1"
  },
  {
    "de": "singen",
    "lv": "Këndim",
    "level": "A1"
  },
  {
    "de": "sitzen",
    "lv": "Ulja",
    "level": "A1",
    "study": {
      "id": "a1-sitzen",
      "layout": "standardStudy",
      "translation": "Ulja",
      "explanation": [
        "Ideja kryesore: sitzen do të thotë të ulesh.",
        "Sitzen përdoret për të përshkruar pozicionin e ulur të një personi ose kafshe.",
        "Ndonjëherë sitzen gjithashtu do të thotë të jesh në një vend të caktuar, por në A1, kuptimi i vërtetë është të ulesh.",
        "Dallimi është i rëndësishëm: sitzen = ulur, stehen = në këmbë, Liegen = shtrirë."
      ],
      "examples": [
        {
          "de": "Ich sitze am Tisch.",
          "lv": "Jam ulur në tavolinë."
        },
        {
          "de": "Die Kinder sitzen im Bus.",
          "lv": "Çocuklar otobüste oturuyor."
        },
        {
          "de": "Er steht an der Tür.",
          "lv": "Ai po qëndron në pragun e derës."
        },
        {
          "de": "Die Katze liegt auf dem Sofa.",
          "lv": "Macja po fle në divan."
        }
      ],
      "comparison": [
        {
          "word": "sitzen",
          "meaning": "Ulja",
          "example": "Ich sitze am Tisch."
        },
        {
          "word": "stehen",
          "meaning": "Të jesh",
          "example": "Er steht an der Tür."
        },
        {
          "word": "liegen",
          "meaning": "Uyumak/uzanmak",
          "example": "Die Katze liegt dort."
        },
        {
          "word": "setzen",
          "meaning": "Otur / otur",
          "example": "Ich setze mich."
        }
      ],
      "tip": {
        "text": "Mos harroni: → ulur • → Stehen në këmbë • → Shtrirë shtrirë."
      },
      "important": [
        "Sitzen tregon statusin \"ulur\".",
        "Oturmak oturmak değil, sich setzendir."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "sitzen"
          ],
          "red": [
            "stehen",
            "liegen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "sitze"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sitzen"
              ],
              "green": [
                "Kinder"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "steht"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "liegt"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "sitzen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "sitze"
              ]
            }
          },
          {
            "word": {
              "green": [
                "stehen"
              ]
            },
            "meaning": {
              "purple": [
                "jesh"
              ]
            },
            "example": {
              "red": [
                "steht"
              ]
            }
          },
          {
            "word": {
              "green": [
                "liegen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "liegt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "setzen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "setze"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "sitzen"
            ]
          },
          {
            "green": [
              "sich setzendir"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "so",
    "lv": "Bu yüzden",
    "level": "A1"
  },
  {
    "de": "Socke",
    "de_article": "die",
    "de_plural": "die Socken",
    "lv": "Çorape",
    "level": "A1"
  },
  {
    "de": "Sofa",
    "de_article": "das",
    "de_plural": "die Sofas",
    "lv": "Divan",
    "level": "A1"
  },
  {
    "de": "sofort",
    "lv": "Nuk e di, zotëri... - Tani.",
    "level": "A1"
  },
  {
    "de": "Sohn",
    "de_article": "der",
    "de_plural": "die Söhne",
    "lv": "Oğul",
    "level": "A1"
  },
  {
    "de": "sollen",
    "lv": "Olmalı",
    "level": "A1",
    "study": {
      "id": "a1-sollen",
      "layout": "standardStudy",
      "translation": "Olmalı",
      "explanation": [
        "Ideja kryesore: parakalimi do të thotë se dikush duhet ose duhet të bëjë diçka siç udhëzohet.",
        "E enjtura shpesh përdor kur dikush tjetër ju tregon se çfarë duhet të bëni.",
        "Nuk është aq i fortë sa Museni.",
        "Çok popüler bir ifade You soll ich machen? = Ne yapmalıyım?"
      ],
      "examples": [
        {
          "de": "Was soll ich machen?",
          "lv": "Cduhet te bej?"
        },
        {
          "de": "Du sollst kommen.",
          "lv": ". S'e bën?"
        },
        {
          "de": "Ich soll zu Hause bleiben.",
          "lv": "Më duhet të qëndroj në shtëpi"
        },
        {
          "de": "Ich muss jetzt gehen.",
          "lv": "Şimdi gitmek zorundayım"
        }
      ],
      "comparison": [
        {
          "word": "sollen",
          "meaning": "Ndiq/ndiq rekomandimet",
          "example": "Was soll ich machen?"
        },
        {
          "word": "müssen",
          "meaning": "Më duhet patjetër kjo",
          "example": "Ich muss gehen."
        },
        {
          "word": "können",
          "meaning": "Për të qenë në gjendje të",
          "example": "Ich kann kommen."
        },
        {
          "word": "wollen",
          "meaning": "Dua",
          "example": "Ich will bleiben."
        }
      ],
      "tip": {
        "text": "Unutmayın: birisi size ne yapmanız gerektiğini söyler → sollen • Yapılması gerekir → müssen."
      },
      "important": [
        "A është shumë e mirë? Është një shprehje shumë e njohur.",
        "E enjtur dhe laksativ nuk janë e njëjta gjë."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "sollen"
          ],
          "red": [
            "Ideja"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "soll",
                "machen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sollst",
                "kommen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "soll",
                "bleiben"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "muss",
                "gehen"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "sollen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "soll"
              ]
            }
          },
          {
            "word": {
              "green": [
                "müssen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "können"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "wollen"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {
            "blue": [
              "enjtur"
            ],
            "red": [
              "enjtur"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Sommer",
    "de_article": "der",
    "de_plural": "die Sommer",
    "lv": "Vera",
    "level": "A1"
  },
  {
    "de": "Samstag",
    "de_article": "der",
    "de_plural": "die Samstage",
    "lv": "Cumartesi",
    "level": "A1"
  },
  {
    "de": "Sonne",
    "de_article": "die",
    "de_plural": "die Sonnen",
    "lv": "Güneş",
    "level": "A1"
  },
  {
    "de": "Sonntag",
    "de_article": "der",
    "de_plural": "die Sonntage",
    "lv": "E dielë",
    "level": "A1"
  },
  {
    "de": "spät",
    "lv": "Hyr.",
    "level": "A1"
  },
  {
    "de": "spazieren gehen",
    "lv": "Yürüyüşe çık",
    "level": "A1"
  },
  {
    "de": "Spiel",
    "de_article": "das",
    "de_plural": "die Spiele",
    "lv": "Futem.",
    "level": "A1"
  },
  {
    "de": "spielen",
    "lv": "Oynamak",
    "level": "A1"
  },
  {
    "de": "Spielzeug",
    "de_article": "das",
    "de_plural": "die Spielzeuge",
    "lv": "Lodër",
    "level": "A1"
  },
  {
    "de": "Sprache",
    "de_article": "die",
    "de_plural": "die Sprachen",
    "lv": "Gjuha",
    "level": "A1"
  },
  {
    "de": "stark",
    "lv": "Güçlü",
    "level": "A1"
  },
  {
    "de": "stehen",
    "lv": "Të jesh",
    "level": "A1",
    "study": {
      "id": "a1-stehen",
      "layout": "standardStudy",
      "translation": "Të jesh",
      "explanation": [
        "Ideja kryesore: kofshët do të thotë të qëndrosh në këmbë ose në këmbë.",
        "Për njerëzit, stehen do të thotë të qëndrosh në këmbë.",
        "Në rastin e një objekti, stehen do të thotë se ndalon ose është në një vend të caktuar.",
        "Dallimi është i rëndësishëm: stehen = në këmbë, sitzen = ulur, Liegen = shtrirë."
      ],
      "examples": [
        {
          "de": "Ich stehe an der Tür.",
          "lv": "Po qëndroj te dera."
        },
        {
          "de": "Der Stuhl steht in der Küche.",
          "lv": "Karrigia është në kuzhinë."
        },
        {
          "de": "Er sitzt am Tisch.",
          "lv": "Ajo ulet në tavolinë."
        },
        {
          "de": "Das Buch liegt auf dem Tisch.",
          "lv": "Libri është në tryezë."
        }
      ],
      "comparison": [
        {
          "word": "stehen",
          "meaning": "Qëndrim/Qëndrim",
          "example": "Ich stehe hier."
        },
        {
          "word": "sitzen",
          "meaning": "Ulja",
          "example": "Er sitzt am Tisch."
        },
        {
          "word": "liegen",
          "meaning": "Uyumak/uzanmak",
          "example": "Das Buch liegt dort."
        },
        {
          "word": "stellen",
          "meaning": "Vendos vertikalisht",
          "example": "Ich stelle die Flasche hin."
        }
      ],
      "tip": {
        "text": "Mos harroni: → në këmbë me avull • → ulur • → shtrirë shtrirë."
      },
      "important": [
        "Stehen tregon situatën, jo aktin e \"lënies mënjanë\".",
        "Bir nesneyi dik bir şekilde koymak stehen değil, stelen'dir."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "stehen"
          ],
          "red": [
            "sitzen",
            "liegen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "stehe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "steht"
              ],
              "yellow": [
                "Stuhl"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "sitzt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "liegt"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "stehen"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "stehe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "sitzen"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "sitzt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "liegen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "liegt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "stellen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "stelle"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "stehen"
            ],
            "green": [
              "stehen"
            ]
          },
          {
            "green": [
              "stehen"
            ],
            "blue": [
              "stehen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Stern",
    "de_article": "der",
    "de_plural": "die Sterne",
    "lv": "I zgjuar.",
    "level": "A1"
  },
  {
    "de": "Straße",
    "de_article": "die",
    "de_plural": "die Straßen",
    "lv": "Rruga",
    "level": "A1"
  },
  {
    "de": "Straßenbahn",
    "de_article": "die",
    "de_plural": "die Straßenbahnen",
    "lv": "Pastaj të shëndoshët mund të vraponin, të humbin peshë dhe të ishin të shëndetshëm.",
    "level": "A1"
  },
  {
    "de": "Stück",
    "de_article": "das",
    "de_plural": "die Stücke",
    "lv": "Copa e thyer.",
    "level": "A1"
  },
  {
    "de": "Student",
    "de_article": "der",
    "de_plural": "die Studenten",
    "lv": "Student",
    "level": "A1"
  },
  {
    "de": "Stuhl",
    "de_article": "der",
    "de_plural": "die Stühle",
    "lv": "Për karrigen.",
    "level": "A1"
  },
  {
    "de": "Stunde",
    "de_article": "die",
    "de_plural": "die Stunden",
    "lv": "Koha.",
    "level": "A1"
  },
  {
    "de": "suchen",
    "lv": "Kërko",
    "level": "A1"
  },
  {
    "de": "süß",
    "lv": "E bukur.",
    "level": "A1"
  },
  {
    "de": "Tag",
    "de_article": "der",
    "de_plural": "die Tage",
    "lv": "Gün",
    "level": "A1"
  },
  {
    "de": "Tante",
    "de_article": "die",
    "de_plural": "die Tanten",
    "lv": "Hala",
    "level": "A1"
  },
  {
    "de": "tanzen",
    "lv": "Po.",
    "level": "A1"
  },
  {
    "de": "Tasche",
    "de_article": "die",
    "de_plural": "die Taschen",
    "lv": "Ndeshje.",
    "level": "A1"
  },
  {
    "de": "Tasse",
    "de_article": "die",
    "de_plural": "die Tassen",
    "lv": "Saldırıp soymak",
    "level": "A1"
  },
  {
    "de": "Taxi",
    "de_article": "das",
    "de_plural": "die Taxis",
    "lv": "Taksi!",
    "level": "A1"
  },
  {
    "de": "Tee",
    "de_article": "der",
    "de_plural": "die Tees",
    "lv": "Çay",
    "level": "A1"
  },
  {
    "de": "Telefon",
    "de_article": "das",
    "de_plural": "die Telefone",
    "lv": "Telefon",
    "level": "A1"
  },
  {
    "de": "telefonieren",
    "lv": "Telefono",
    "level": "A1"
  },
  {
    "de": "Teller",
    "de_article": "der",
    "de_plural": "die Teller",
    "lv": "Pllaka",
    "level": "A1"
  },
  {
    "de": "teuer",
    "lv": "Oh, e dashur. Jam këtu.",
    "level": "A1"
  },
  {
    "de": "Text",
    "de_article": "der",
    "de_plural": "die Texte",
    "lv": "Metin",
    "level": "A1"
  },
  {
    "de": "Tier",
    "de_article": "das",
    "de_plural": "die Tiere",
    "lv": "Kafshë.",
    "level": "A1"
  },
  {
    "de": "Tisch",
    "de_article": "der",
    "de_plural": "die Tische",
    "lv": "Masa",
    "level": "A1"
  },
  {
    "de": "Tochter",
    "de_article": "die",
    "de_plural": "die Töchter",
    "lv": "Kız çocuğu",
    "level": "A1"
  },
  {
    "de": "Toilette",
    "de_article": "die",
    "de_plural": "die Toiletten",
    "lv": "Banjë.",
    "level": "A1"
  },
  {
    "de": "Tomate",
    "de_article": "die",
    "de_plural": "die Tomaten",
    "lv": "# Domatet ... #",
    "level": "A1"
  },
  {
    "de": "Treppe",
    "de_article": "die",
    "de_plural": "die Treppen",
    "lv": "Merdiven",
    "level": "A1"
  },
  {
    "de": "trinken",
    "lv": "Pije",
    "level": "A1"
  },
  {
    "de": "tun",
    "lv": "Yapmak",
    "level": "A1"
  },
  {
    "de": "Tür",
    "de_article": "die",
    "de_plural": "die Türen",
    "lv": "Derë, hapu.",
    "level": "A1"
  },
  {
    "de": "U-Bahn",
    "de_article": "die",
    "de_plural": "die U-Bahnen",
    "lv": "Metro",
    "level": "A1"
  },
  {
    "de": "über",
    "lv": "Mbi • Për",
    "level": "A1",
    "study": {
      "id": "a1-über",
      "layout": "standardStudy",
      "translation": "Mbi • Për",
      "explanation": [
        "Ideja kryesore: uber do të thotë sipër ose përreth, në varësi të kontekstit.",
        "Kur bëhet fjalë për vendndodhjen, uber do të thotë shpesh më lart.",
        "Bir konuşma, metin veya konu söz konusu olduğunda über, hakkında anlamına gelir.",
        "Në trafikun e Uber, kjo mund të nënkuptojë fundin."
      ],
      "examples": [
        {
          "de": "Die Lampe hängt über dem Tisch.",
          "lv": "Lamba masanın üzerinde asılı duruyor."
        },
        {
          "de": "Wir sprechen über das Wetter.",
          "lv": "Zamandan bahsediyoruz."
        },
        {
          "de": "Das Kind läuft über die Straße.",
          "lv": "Një djalë po vrapon nëpër rrugë."
        },
        {
          "de": "Ich freue mich über das Geschenk.",
          "lv": "Jam i kënaqur me dhuratën."
        }
      ],
      "comparison": [
        {
          "word": "über",
          "meaning": "Mbi/mbi/kryq",
          "example": "Wir sprechen über das Wetter."
        },
        {
          "word": "auf",
          "meaning": "Nga jashtë",
          "example": "Das Buch liegt auf dem Tisch."
        },
        {
          "word": "unter",
          "meaning": "Altında",
          "example": "Die Tasche ist unter dem Tisch."
        },
        {
          "word": "von",
          "meaning": "Bir kaynaktan/etrafından",
          "example": "Ich höre von dir."
        }
      ],
      "tip": {
        "text": "Unutmayın: konuşmanın konusu → über • Masanın üstü → über."
      },
      "important": [
        "Uber nuk është vetëm një emër vendi.",
        "Sprechen über do të thotë \"të flasësh për\"."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "über"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "über"
              ],
              "yellow": [
                "Tisch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "über"
              ],
              "green": [
                "Wetter"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "über"
              ],
              "yellow": [
                "Straße"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "über"
              ],
              "yellow": [
                "Geschenk"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "über"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "über"
              ]
            }
          },
          {
            "word": {
              "green": [
                "auf"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "auf"
              ]
            }
          },
          {
            "word": {
              "green": [
                "unter"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "unter"
              ]
            }
          },
          {
            "word": {
              "green": [
                "von"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "von"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "über"
            ]
          },
          {
            "blue": [
              "sprechen über"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "überall",
    "lv": "Her yer",
    "level": "A1"
  },
  {
    "de": "übermorgen",
    "lv": "Pasnesër?",
    "level": "A1"
  },
  {
    "de": "um",
    "lv": "Yaklaşık • Saat",
    "level": "A1",
    "study": {
      "id": "a1-um",
      "layout": "standardStudy",
      "translation": "Yaklaşık • Saat",
      "explanation": [
        "Ideja kryesore: hmm do të thotë një orë që shpesh shfaq kohën ose është rreth/rreth një vendi.",
        "Koha e saktë, hm, do të thotë orë.",
        "\"Vendi\" do të thotë \"rreth\" ose \"rreth\".",
        "Um… zu ndihmon në artikulimin e qëllimit në fjalinë: të."
      ],
      "examples": [
        {
          "de": "Ich komme um acht Uhr.",
          "lv": "Do të vij në orën tetë."
        },
        {
          "de": "Wir sitzen um den Tisch.",
          "lv": "Ulemi rreth tryezës."
        },
        {
          "de": "Er geht um die Ecke.",
          "lv": "Köşeyi dönüyor."
        },
        {
          "de": "Ich lerne, um Deutsch zu sprechen.",
          "lv": "Almanca konuşmayı öğreniyorum."
        }
      ],
      "comparison": [
        {
          "word": "um",
          "meaning": "Brenda/përreth/brenda/brenda/brenda/brenda/brenda",
          "example": "Ich komme um acht."
        },
        {
          "word": "am",
          "meaning": "Çdo ditë / orë",
          "example": "Am Montag komme ich."
        },
        {
          "word": "gegen",
          "meaning": "Zaman / vs hakkında",
          "example": "Ich komme gegen acht."
        },
        {
          "word": "für",
          "meaning": "Lehine / lehine",
          "example": "Das ist für dich."
        }
      ],
      "tip": {
        "text": "Unutmayın: um acht = saat sekiz."
      },
      "important": [
        "Hmm, me kalimin e kohës kjo zakonisht bëhet një \"orë\".",
        "Hmm… zu zakonisht do të thotë “për…”."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "um"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "um"
              ],
              "yellow": [
                "acht Uhr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "um"
              ],
              "yellow": [
                "Tisch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "um"
              ],
              "yellow": [
                "Ecke"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "um",
                "zu"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "um"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "um"
              ]
            }
          },
          {
            "word": {
              "green": [
                "am"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "Am"
              ]
            }
          },
          {
            "word": {
              "green": [
                "gegen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "gegen"
              ]
            }
          },
          {
            "word": {
              "green": [
                "für"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {},
          {}
        ]
      }
    }
  },
  {
    "de": "und",
    "lv": "Dhe, hmm...",
    "level": "A1"
  },
  {
    "de": "unser",
    "lv": "Bizim",
    "level": "A1"
  },
  {
    "de": "unten",
    "lv": "Poshtë.",
    "level": "A1"
  },
  {
    "de": "unter",
    "lv": "Altında",
    "level": "A1",
    "study": {
      "id": "a1-unter",
      "layout": "standardStudy",
      "translation": "Altında",
      "explanation": [
        "Ideja kryesore: do të thotë më poshtë ose midis në varësi të kontekstit.",
        "Përdor arin nëse ka diçka nën tavolinë, karrige ose objekt tjetër.",
        "Bir grup insandan bahsederken unter, arasında anlamına gelebilir.",
        "Është e kundërta e uber për sa i përket drejtimit lart/poshtë."
      ],
      "examples": [
        {
          "de": "Die Tasche ist unter dem Tisch.",
          "lv": "Çanta masanın altındadır."
        },
        {
          "de": "Die Katze liegt unter dem Stuhl.",
          "lv": "Kedi sandalyenin altında uyuyor."
        },
        {
          "de": "Unter Freunden sagt man das so.",
          "lv": "Thonë se është mes miqsh."
        },
        {
          "de": "Die Lampe hängt über dem Tisch.",
          "lv": "Lamba masanın üzerinde asılı duruyor."
        }
      ],
      "comparison": [
        {
          "word": "unter",
          "meaning": "Nën / Midis",
          "example": "Die Tasche ist unter dem Tisch."
        },
        {
          "word": "über",
          "meaning": "Teprica/për",
          "example": "Die Lampe hängt über dem Tisch."
        },
        {
          "word": "zwischen",
          "meaning": "Midis dy gjërave",
          "example": "Zwischen den Häusern."
        },
        {
          "word": "auf",
          "meaning": "Nga jashtë",
          "example": "Auf dem Tisch."
        }
      ],
      "tip": {
        "text": "Hizmetler: zem galda → Tisch'in altında."
      },
      "important": [
        "Unter gjithashtu mund të nënkuptojë \"ndërmjet\", veçanërisht midis individëve ose grupeve.",
        "Unter dhe über janë shpesh të kundërta në kuptimin e vendit."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "unter"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "unter"
              ],
              "yellow": [
                "Tisch"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "unter"
              ],
              "yellow": [
                "Stuhl"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Unter"
              ],
              "green": [
                "Freunden"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "über"
              ],
              "yellow": [
                "Tisch"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "unter"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "unter"
              ]
            }
          },
          {
            "word": {
              "green": [
                "über"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "über"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zwischen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "Zwischen"
              ]
            }
          },
          {
            "word": {
              "green": [
                "auf"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "Auf"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "unter"
            ]
          },
          {
            "blue": [
              "unter"
            ],
            "red": [
              "über"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Vase",
    "de_article": "die",
    "de_plural": "die Vasen",
    "lv": "Vazo",
    "level": "A1"
  },
  {
    "de": "Vater",
    "de_article": "der",
    "de_plural": "die Väter",
    "lv": "Baba",
    "level": "A1"
  },
  {
    "de": "verheiratet",
    "lv": "İ Martuar",
    "level": "A1"
  },
  {
    "de": "verkaufen",
    "lv": "Satmak",
    "level": "A1"
  },
  {
    "de": "verlieren",
    "lv": "Humbje",
    "level": "A1"
  },
  {
    "de": "verstehen",
    "lv": "Anlamak",
    "level": "A1",
    "study": {
      "id": "a1-verstehen",
      "layout": "standardStudy",
      "translation": "Anlamak",
      "explanation": [
        "Ideja kryesore: do të thotë të kuptosh verstehen.",
        "Përdoret kur kupton një gjuhë, një person, një tekst ose një situatë.",
        "Burada Letonca'yı \"bilmeye\" veya \"öğrenmeye\" genellikle gerek yoktur. • Çoğunlukla könnendirler.",
        "Një shprehje shumë e njohur është ich verstehe. = E kuptoj."
      ],
      "examples": [
        {
          "de": "Ich verstehe dich.",
          "lv": "Seni anlıyorum"
        },
        {
          "de": "Verstehst du Deutsch?",
          "lv": "A kupton gjermanisht?"
        },
        {
          "de": "Ich verstehe das nicht.",
          "lv": "Nuk e kuptoj"
        },
        {
          "de": "Ich kann Deutsch sprechen.",
          "lv": "Mund të flas gjermanisht"
        }
      ],
      "comparison": [
        {
          "word": "verstehen",
          "meaning": "Anlamak",
          "example": "Ich verstehe dich."
        },
        {
          "word": "können",
          "meaning": "Dije",
          "example": "Ich kann schwimmen."
        },
        {
          "word": "wissen",
          "meaning": "E di të vërtetën.",
          "example": "Ich weiß das."
        },
        {
          "word": "kennen",
          "meaning": "Dije",
          "example": "Ich kenne ihn."
        }
      ],
      "tip": {
        "text": "Unutmayın: metni/kişiyi anlayın → verstehen • Bir şeyin nasıl yapılacağını bilin → können."
      },
      "important": [
        "Verstehen \"anlamak\" kelimesinin kökü değildir.",
        "Ich verstehe Deutsch \"Almancayı anlıyorum\" anlamına gelir."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "verstehen",
            "Ich verstehe"
          ],
          "red": [
            "könnendirler"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "verstehe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Verstehst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "verstehe"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "kann"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "verstehen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "können"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "wissen"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "kennen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "kenne"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "verstehen"
            ]
          },
          {
            "blue": [
              "verstehe"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "versuchen",
    "lv": "Bëj ca gjuajte.",
    "level": "A1"
  },
  {
    "de": "viel",
    "lv": "Çok",
    "level": "A1"
  },
  {
    "de": "vielleicht",
    "lv": "Mbase.",
    "level": "A1"
  },
  {
    "de": "vier",
    "lv": "4.",
    "level": "A1"
  },
  {
    "de": "vierhundert",
    "lv": "Katërqind",
    "level": "A1"
  },
  {
    "de": "vierte",
    "lv": "E katërta",
    "level": "A1"
  },
  {
    "de": "vierzehn",
    "lv": "14.",
    "level": "A1"
  },
  {
    "de": "vierzehnte",
    "lv": "I katërmbëdhjeti",
    "level": "A1"
  },
  {
    "de": "vierzig",
    "lv": "40.",
    "level": "A1"
  },
  {
    "de": "vierzigste",
    "lv": "E dyzeta",
    "level": "A1"
  },
  {
    "de": "Vogel",
    "de_article": "der",
    "de_plural": "die Vögel",
    "lv": "Kuş",
    "level": "A1"
  },
  {
    "de": "voll",
    "lv": "Tam dolu",
    "level": "A1"
  },
  {
    "de": "vom",
    "lv": "DHE",
    "level": "A1",
    "study": {
      "id": "a1-vom",
      "layout": "standardStudy",
      "translation": "DHE",
      "explanation": [
        "Vom është shkurtimi i parafjalës së trenit dhe artikullit dem.",
        "Formati i plotë: von dem (kujt?).",
        "Përdoret së bashku me emrat mashkullorë dhe neutralë për të treguar origjinën ose drejtimin e saj.",
        "Duke iu përgjigjur pyetjeve nga kush - ose ku?",
        "Në praktikë, vom përdoret pothuajse gjithmonë në vend të vomit të plotë."
      ],
      "examples": [
        {
          "de": "Ich komme vom Bahnhof.",
          "lv": "İstasyondan geliyorum"
        },
        {
          "de": "Das Geschenk ist vom Vater.",
          "lv": "Dhurata është nga babai im."
        },
        {
          "de": "Er kommt vom Arzt.",
          "lv": "Doktordan geliyor."
        },
        {
          "de": "Sie fährt vom Flughafen.",
          "lv": "Po vjen nga aeroporti."
        },
        {
          "de": "Das ist vom Markt.",
          "lv": "Pazardan."
        },
        {
          "de": "Wir kommen vom Fest.",
          "lv": "Kthehemi nga festa."
        },
        {
          "de": "Er holt Milch vom Bauern.",
          "lv": "Sütünü çiftçiden alıyor."
        },
        {
          "de": "Die Nachricht ist vom Chef.",
          "lv": "Mesazhi është nga shefi."
        }
      ],
      "comparison": [
        {
          "word": "vom",
          "meaning": "Kimden (belirli bir şeyden, kimin için?)",
          "example": "vom Bahnhof – Nga stacioni"
        },
        {
          "word": "von",
          "meaning": "Gönderen (genel)",
          "example": "von mir – Ju qeras une."
        },
        {
          "word": "aus",
          "meaning": "Nga brenda/origjina",
          "example": "aus Deutschland – Almanya'dan"
        },
        {
          "word": "ab",
          "meaning": "Data ​​e fillimit: (ora/vendi)",
          "example": "ab Montag – Nga e hëna"
        },
        {
          "word": "zu",
          "meaning": "In/Out (drejtimi i kundërt)",
          "example": "zum Arzt – Të lutem."
        }
      ],
      "tip": [
        "Unutmayın: von + dem → vom (kimin için?).",
        "Pothuajse kurrë nuk thua të përdorësh von dem-vom në bisedat e përditshme."
      ],
      "important": [
        "Vom = von dem, vetëm mashkullor ose me emër neutral, për kë? në xhirim.",
        "Tregon origjinën, burimin ose drejtimin e një sendi të veçantë.",
        "Kadın cinsiyeti için: von der Mutter, vom Mutter değil.",
        "Të mos ngatërrohet me Aus (vendi i origjinës) ose ab (pika fillestare)."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "vom",
            "von dem"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vom"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "vom"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "vom Bahnhof"
              ]
            }
          },
          {
            "word": {
              "green": [
                "von"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "von mir"
              ]
            }
          },
          {
            "word": {
              "green": [
                "aus"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "aus Deutschland"
              ]
            }
          },
          {
            "word": {
              "green": [
                "ab"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "ab Montag"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zu"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "zum Arzt"
              ]
            }
          }
        ],
        "tip": [
          {
            "blue": [
              "vom"
            ]
          },
          {
            "purple": [
              "von dem"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "vom"
            ],
            "purple": [
              "von dem"
            ]
          },
          {},
          {
            "yellow": [
              "von der Mutter"
            ],
            "red": [
              "vom Mutter"
            ]
          },
          {
            "green": [
              "aus"
            ],
            "red": [
              "ab"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "von",
    "lv": "DHE",
    "level": "A1"
  },
  {
    "de": "vor",
    "lv": "Para • Para",
    "level": "A1",
    "study": {
      "id": "a1-vor",
      "layout": "standardStudy",
      "translation": "Para • Para",
      "explanation": [
        "Ideja kryesore: vor do të thotë më herët në kohë ose më parë diku.",
        "Për sa i përket kohës, vor do të thotë më herët.",
        "Yer açısından vor, önce veya içeri anlamına gelir.",
        "Në zonën kohore vor do të thotë \"deri\", p.sh. fünf vor acht."
      ],
      "examples": [
        {
          "de": "Vor dem Essen wasche ich die Hände.",
          "lv": "Yemekten önce ellerimi yıkarım."
        },
        {
          "de": "Das Auto steht vor dem Haus.",
          "lv": "Makina është e parkuar para shtëpisë."
        },
        {
          "de": "Es ist fünf vor acht.",
          "lv": "Është ora tetë deri në pesë."
        },
        {
          "de": "Nach dem Essen gehen wir spazieren.",
          "lv": "Pasi hamë, dalim për një shëtitje."
        }
      ],
      "comparison": [
        {
          "word": "vor",
          "meaning": "Para/Para",
          "example": "Vor dem Essen..."
        },
        {
          "word": "nach",
          "meaning": "Sonra / bitene kadar",
          "example": "Nach dem Essen..."
        },
        {
          "word": "neben",
          "meaning": "Digjeni.",
          "example": "Neben dem Haus."
        },
        {
          "word": "hinter",
          "meaning": "Për",
          "example": "Hinter dem Haus."
        }
      ],
      "tip": {
        "text": "Mos harroni: → vor para kohe, para vendit."
      },
      "important": [
        "Vor mund të nënkuptojë kohën dhe vendin.",
        "Vor dem Essen = yemekten önce • Vor dem Haus = evden önce."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "vor",
            "fünf vor acht"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Vor"
              ],
              "yellow": [
                "Essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vor"
              ],
              "yellow": [
                "Haus"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "vor"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "Nach"
              ],
              "yellow": [
                "Essen"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "vor"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "nach"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "Nach"
              ]
            }
          },
          {
            "word": {
              "green": [
                "neben"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "Neben"
              ]
            }
          },
          {
            "word": {
              "green": [
                "hinter"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "vor"
            ]
          },
          {
            "blue": [
              "vor dem Essen",
              "vor dem Haus"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Vorname",
    "de_article": "der",
    "de_plural": "die Vornamen",
    "lv": "Kelime",
    "level": "A1"
  },
  {
    "de": "Wald",
    "de_article": "der",
    "de_plural": "die Wälder",
    "lv": "Orman",
    "level": "A1"
  },
  {
    "de": "Wand",
    "de_article": "die",
    "de_plural": "die Wände",
    "lv": "Mur!",
    "level": "A1"
  },
  {
    "de": "wann",
    "lv": "Shpërthimi është jashtë kontrollit. Nuk keni shannsë.",
    "level": "A1"
  },
  {
    "de": "warm",
    "lv": "I ngrohtë",
    "level": "A1"
  },
  {
    "de": "warten",
    "lv": "Prit",
    "level": "A1"
  },
  {
    "de": "warum",
    "lv": "Pse",
    "level": "A1"
  },
  {
    "de": "was",
    "lv": "Kim • Ne",
    "level": "A1",
    "study": {
      "id": "a1-was",
      "layout": "standardStudy",
      "translation": "Kim • Ne",
      "explanation": [
        "Ana fikir: Şeyler ve olaylarla ilgili soru kelimesi nedir - Letonca'da cümlenin kısmına bağlı olarak ne veya ne'dir.",
        "Njerëzit u pyetën për objekte, ngjarje dhe fakte, jo për persona.",
        "Në gjermanisht, nuk ndryshon pas xhirimit - gjithmonë kështu duket.",
        "Subjekti i fjalisë është \"by\" në muskujt letonezë (Was ist das? = Çfarë është?).",
        "Nëse ishte objekti i foljes, në Letonisht ko (Was machst du? = Çfarë po bën?).",
        "İnsanlara wer (kim/kim) soruluyor ve bu sorulmuyor."
      ],
      "examples": [
        {
          "de": "Was ist das?",
          "lv": "Cfar është ky?"
        },
        {
          "de": "Was ist passiert?",
          "lv": "!"
        },
        {
          "de": "Was machst du gerade?",
          "lv": "Çfarë do të thuash?"
        },
        {
          "de": "Was möchtest du trinken?",
          "lv": "Ne içmek istersin?"
        },
        {
          "de": "Was bedeutet dieses Wort?",
          "lv": "Çfarë do të thotë kjo fjalë?"
        },
        {
          "de": "Was ist dein Lieblingsessen?",
          "lv": "En sevdiğiniz yemek nedir?"
        },
        {
          "de": "Was hast du gesagt?",
          "lv": "Çfarë the? - Nuk jemi në një vend..."
        }
      ],
      "tip": [
        "Vetë i pandryshueshëm - ishte gjithmonë kështu në gjermanisht • Në letonisht, zgjidh kush ose çfarë është sipas pjesës së fjalisë.",
        "Hızlı ipucu: Eğer soru \"Şu...\" ile cevaplanabiliyorsa, \"kim\" kullanın. • Cevap, tümleç olarak fiilden sonra geliyorsa, ko kullanın."
      ],
      "important": [
        "Njerëzit u pyetën për objekte, ngjarje dhe fakte • Asnjëherë për njerëzit.",
        "İnsanlara wer (kim/kim) soruluyor ve bu sorulmuyor.",
        "Was für (ein/eine) birisi/ne anlamına gelir ve kalite veya tür hakkında soru sorar (Was für ein Film ist das? = Ne tür bir film?).",
        "Yanlış: Pasif miydik? → Doğru: Daha pasif mi?"
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "was"
          ],
          "purple": [
            "pas",
            "ko"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Was"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Was"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Was"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Was"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Was"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Was"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Was"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {
            "purple": [
              "ko"
            ]
          }
        ],
        "important": [
          {},
          {
            "blue": [
              "wer"
            ]
          },
          {
            "blue": [
              "was für"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "waschen",
    "lv": "Larje",
    "level": "A1"
  },
  {
    "de": "sich waschen",
    "lv": "Larje",
    "level": "A1"
  },
  {
    "de": "Weg",
    "de_article": "der",
    "de_plural": "die Wege",
    "lv": "Në rrugë.",
    "level": "A1"
  },
  {
    "de": "Weihnachten",
    "lv": "Noel",
    "level": "A1"
  },
  {
    "de": "Wein",
    "de_article": "der",
    "de_plural": "die Weine",
    "lv": "Qenka venë.",
    "level": "A1"
  },
  {
    "de": "weinen",
    "lv": "Ağlamak",
    "level": "A1"
  },
  {
    "de": "weiß",
    "lv": "Beyaz",
    "level": "A1"
  },
  {
    "de": "welcher",
    "lv": "OBSH",
    "level": "A1"
  },
  {
    "de": "Welt",
    "de_article": "die",
    "de_plural": "die Welten",
    "lv": "Toka",
    "level": "A1"
  },
  {
    "de": "wenig",
    "lv": "Disa",
    "level": "A1"
  },
  {
    "de": "wenn",
    "lv": "Nëse • Kur",
    "level": "A1",
    "study": {
      "id": "a1-wenn",
      "layout": "standardStudy",
      "translation": "Nëse • Kur",
      "explanation": [
        "Ideja kryesore: wen do të thotë nëse ose kur do të ndodhë në varësi të situatës.",
        "Nëse kjo është një gjendje, përktheje sikur të ishte një gjendje.",
        "Tekrarlayan veya genel zaman için, ne zaman şeklinde tercüme edin.",
        "Pas Wen, folja zakonisht përfundon me një fjali në gjermanisht."
      ],
      "examples": [
        {
          "de": "Wenn du Zeit hast, komm vorbei.",
          "lv": "Ndalo nëse ke kohë."
        },
        {
          "de": "Wenn es regnet, bleibe ich zu Hause.",
          "lv": "Yağmur yağarsa evde kalırım."
        },
        {
          "de": "Wenn ich müde bin, trinke ich Kaffee.",
          "lv": "Unë pi kafe kur jam i lodhur."
        },
        {
          "de": "Ich weiß nicht, ob er kommt.",
          "lv": "Nuk e di nëse do të vish."
        }
      ],
      "comparison": [
        {
          "word": "wenn",
          "meaning": "Nëse/kur",
          "example": "Wenn du Zeit hast..."
        },
        {
          "word": "ob",
          "meaning": "Ose në një pyetje indirekte",
          "example": "Ich weiß nicht, ob..."
        },
        {
          "word": "wann",
          "meaning": "O noktaya geldiğinde",
          "example": "Wann kommst du?"
        },
        {
          "word": "weil",
          "meaning": "Sepse...",
          "example": "Ich bleibe, weil ich krank bin."
        }
      ],
      "tip": {
        "text": "Mos harroni: kushti → wen • → Dua pyetjen \"kur?\""
      },
      "important": [
        "Wenn dhe Wann nuk janë të njëjta.",
        "Kur do të vish? Ka një pyetje. Wenn du kommst... a condition/time."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "wen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Wenn",
                "hast"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wenn",
                "regnet"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wenn",
                "bin"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "ob"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "wenn"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "ob"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "wann"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "weil"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "weil"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "wenn"
            ],
            "yellow": [
              "wann"
            ]
          },
          {
            "blue": [
              "Wenn du kommst"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "wer",
    "lv": "Kim • Kim",
    "level": "A1",
    "study": {
      "id": "a1-wer",
      "layout": "standardStudy",
      "translation": "Kim • Kim",
      "explanation": [
        "Ideja kryesore: WER është një fjalë pyetëse në lidhje me identitetin e një personi - do të thotë \"kush\" ose \"kush\" në letonisht.",
        "Şeyler veya olaylar hakkında değil, insanlar hakkında sorular sorduk.",
        "Şeyler ve olaylar sizin aracılığınızla sorulur, ver.",
        "Në gjermanisht, WER është zakonisht subjekt i një fjalie (e thjeshtë) - Wer është das? = Çfarë është?",
        "Shpesh përdorim fjalën wer z von (wer von euch = cili nga ju) kur pyesim se cili nga të paktët është cili.",
        "Wer, çeşitliliğe bağlı olarak biçimini değiştirir: wen (suçlama), wem (datif), wessen (genitive) - en sık A1 düzeyinde ortaya çıkan wer biçimidir."
      ],
      "examples": [
        {
          "de": "Wer ist das?",
          "lv": "Cfar është ky?"
        },
        {
          "de": "Wer bist du?",
          "lv": "Sen kimsin"
        },
        {
          "de": "Wer kommt heute?",
          "lv": "Çfarë do të ndodhë sot?"
        },
        {
          "de": "Wer ist deine Lehrerin?",
          "lv": "Kush të trajnon ty?"
        },
        {
          "de": "Wer von euch spricht Deutsch?",
          "lv": "Cili nga ju flet gjermanisht?"
        },
        {
          "de": "Wer hat das gesagt?",
          "lv": "Kush e tha këtë?"
        },
        {
          "de": "Wer möchte Kaffee?",
          "lv": "Kim biraz kahve ister?"
        }
      ],
      "tip": [
        "Ne u bëjmë njerëzve (kush/kush) - pyetje rreth objekteve dhe ngjarjeve, ne ju përdorim.",
        "Përdorni wer von... (që është...) për të zgjedhur midis disa njerëzve."
      ],
      "important": [
        "Ne vetëm bëmë pyetje për njerëzit, kurrë për objektet.",
        "Şeyler ve olaylar sizin aracılığınızla sorulur, ver.",
        "Wer, biçimini çekimle değiştirir: wen, wem, wessen - ancak temel biçim wer'dir.",
        "Yanlış: Pasif miydik? → Doğru: Daha pasif mi?"
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "wer"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Wer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wer"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {
            "blue": [
              "wer"
            ]
          },
          {
            "blue": [
              "wer von"
            ]
          }
        ],
        "important": [
          {},
          {},
          {
            "blue": [
              "wer"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "werden",
    "lv": "Duke u bërë",
    "level": "A1",
    "study": {
      "id": "a1-werden",
      "layout": "standardStudy",
      "translation": "Duke u bërë",
      "explanation": [
        "Ideja kryesore: Werden në A1 do të thotë kryesisht qëndrim.",
        "Përdoret kur diçka ndryshon ose dallon.",
        "Daha sonraki Almanca'da werden geleceği ve pasif sesi belirtmek için de kullanıldı.",
        "A1 düzeyinde en önemli ifade Ich werde müde'dir. = Yorulmaya başladım."
      ],
      "examples": [
        {
          "de": "Ich werde müde.",
          "lv": "Po filloj të ndjehem i lodhur."
        },
        {
          "de": "Es wird kalt.",
          "lv": "Bën të ftohët."
        },
        {
          "de": "Sie wird Ärztin.",
          "lv": "Ajo bëhet mjeke."
        },
        {
          "de": "Ich bin müde.",
          "lv": "Yorgunum"
        }
      ],
      "comparison": [
        {
          "word": "werden",
          "meaning": "Duke u bërë",
          "example": "Ich werde müde."
        },
        {
          "word": "sein",
          "meaning": "Të jesh",
          "example": "Ich bin müde."
        },
        {
          "word": "bleiben",
          "meaning": "Qëndro",
          "example": "Ich bleibe hier."
        },
        {
          "word": "machen",
          "meaning": "Do / do",
          "example": "Ich mache das."
        }
      ],
      "tip": {
        "text": "Mos harroni: ndryshimi/situata do të jetë → ndryshe."
      },
      "important": [
        "Nuk është njësoj si seini i Verdenit.",
        "Ich werde dede = Kam filluar të lodhem • Ich bin dede = Jam i lodhur."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "werden",
            "Ich werde"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "werde"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "wird"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "wird"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "bin"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "werden"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "werde"
              ]
            }
          },
          {
            "word": {
              "green": [
                "sein"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "bin"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bleiben"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "bleibe"
              ]
            }
          },
          {
            "word": {
              "green": [
                "machen"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "mache"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "Nuk"
            ],
            "red": [
              "sein"
            ]
          },
          {
            "blue": [
              "werde"
            ],
            "red": [
              "bin"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Wetter",
    "de_article": "das",
    "lv": "Koha (moti)",
    "level": "A1",
    "study": {
      "id": "a1-wetter",
      "layout": "standardStudy",
      "translation": "Koha (moti)",
      "explanation": [
        "Ideja kryesore: das Wetter do të thotë mot • Me diell, me shi, të ftohtë ose të nxehtë.",
        "Fjala \"laike\" në letonisht mund të nënkuptojë motin dhe kohën e ditës • Në gjermanisht, situata është e ndryshme.",
        "Das Wetter: Bisedo me Wie ist das Wetter heute? për motin në natyrë.",
        "Das Wetter shpesh përdoret në lidhje me fjalë të tilla si i nxehtë, viç, dhe kështu me radhë në fjali.",
        "Die Zeit ile karıştırılmamalıdır • Bir an veya fırsat olarak zamandır (Ich habe keine Zeit)."
      ],
      "examples": [
        {
          "de": "Wie ist das Wetter heute?",
          "lv": "Sa është ora sot?"
        },
        {
          "de": "Das Wetter ist schön.",
          "lv": "Ditë e bukur, sot."
        },
        {
          "de": "Das Wetter ist schlecht.",
          "lv": "Hava kötü."
        },
        {
          "de": "Im Winter ist das Wetter oft kalt.",
          "lv": "Zakonisht është ftohtë në dimër."
        },
        {
          "de": "Wir sprechen über das Wetter.",
          "lv": "Zamandan bahsediyoruz."
        },
        {
          "de": "Morgen wird das Wetter besser.",
          "lv": "E di, por do t'a shohësh që nesër do shkojë mirë."
        }
      ],
      "comparison": [
        {
          "word": "Wetter",
          "meaning": "Hava koşulları",
          "example": "Das Wetter ist schön."
        },
        {
          "word": "Zeit",
          "meaning": "Zaman (an)",
          "example": "Ich habe keine Zeit."
        },
        {
          "word": "Regen",
          "meaning": "Shiu",
          "example": "Es gibt viel Regen."
        },
        {
          "word": "Sonne",
          "meaning": "Güneş",
          "example": "Die Sonne scheint."
        }
      ],
      "tip": [
        "Güneşten, yağmurdan veya dışarıdaki sıcaklıktan bahsederken das Wetter'ı kullanın.",
        "Të kujtohet: Wie ist das Wetter? = Sa është ora? (jo koha)."
      ],
      "important": [
        "Das Wetter = moti, jo koha e orës.",
        "Die Zeit = bir an veya şans olarak zaman - başka bir A1 kartı."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "Wetter",
            "Zeit"
          ],
          "purple": [
            "laike"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Wetter"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wetter"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wetter"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wetter"
              ],
              "yellow": [
                "Winter"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wetter"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wetter"
              ],
              "green": [
                "besser"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "Wetter"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "Wetter"
              ]
            }
          },
          {
            "word": {
              "green": [
                "Zeit"
              ]
            },
            "meaning": {},
            "example": {}
          },
          {
            "word": {
              "green": [
                "Regen"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "Regen"
              ]
            }
          },
          {
            "word": {
              "green": [
                "Sonne"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "Sonne"
              ]
            }
          }
        ],
        "tip": [
          {
            "blue": [
              "Wetter"
            ]
          },
          {
            "blue": [
              "Wetter"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "Wetter"
            ]
          },
          {
            "green": [
              "Zeit"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "wichtig",
    "lv": "E di se është me rëndësi.",
    "level": "A1"
  },
  {
    "de": "wie",
    "lv": "Nasıl • Ne kadar",
    "level": "A1",
    "study": {
      "id": "a1-wie",
      "layout": "standardStudy",
      "translation": "Nasıl • Ne kadar",
      "explanation": [
        "Ideja kryesore: bën pyetje në lidhje me llojin ose cilësinë (si) dhe sasinë ose numrin (sa) në varësi të kontekstit.",
        "Wie sam (Wie geht?) pyet për udhëzime - si në letonisht.",
        "Wie + sıfat (wie viel, wie alt, wie lange) Letonca'da miktar, yaş veya süreyi (ne kadar) sorar.",
        "Wie viel(e) do të thotë sa vjet • Wie sub do të thotë sa vjet • Wie lange do të thotë sa kohë.",
        "Karşılaştırmalarda wie benzer anlamına gelir (yani brüt wie = kadar büyük)."
      ],
      "examples": [
        {
          "de": "Wie geht es dir?",
          "lv": "Si jeni"
        },
        {
          "de": "Wie heißt du?",
          "lv": "-Emri juaj?"
        },
        {
          "de": "Wie viel kostet das?",
          "lv": "Sa kohë..."
        },
        {
          "de": "Wie alt bist du?",
          "lv": "- Sa vjeç je?"
        },
        {
          "de": "Wie lange dauert der Film?",
          "lv": "Bu film ne kadar sürüyor?"
        },
        {
          "de": "Er ist so groß wie sein Vater.",
          "lv": "Ai është po aq i gjatë sa i ati."
        }
      ],
      "tip": [
        "E di vetë = si (stil) • E di + mbiemër (viel/nën/lange) = sa (sasi).",
        "Karşılaştırma için, yani... bilir = nasıl... nasıl."
      ],
      "important": [
        "Wie viel(e) = sa • Wie alt = sa vite • Wie lange = sa kohë.",
        "Ai e njeh veten (Ai e di...?) zakonisht = si, jo sa.",
        "E rreme: Sa vjeç jeni? → E vërtetë: Si jeni? (Wie geht's?)"
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "wie"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Wie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Wie"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "wie"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {},
          {},
          {}
        ]
      }
    }
  },
  {
    "de": "wieder",
    "lv": "\"Shtyrje me dy duar.\" Përsëri.",
    "level": "A1"
  },
  {
    "de": "wie viel",
    "lv": "Kaç tane",
    "level": "A1"
  },
  {
    "de": "Wind",
    "de_article": "der",
    "de_plural": "die Winde",
    "lv": "Rüzgâr",
    "level": "A1"
  },
  {
    "de": "Zigarette",
    "de_article": "die",
    "de_plural": "die Zigaretten",
    "lv": "Sigara",
    "level": "A1"
  },
  {
    "de": "Zimmer",
    "de_article": "das",
    "de_plural": "die Zimmer",
    "lv": "-Kabinë?",
    "level": "A1"
  },
  {
    "de": "Zitrone",
    "de_article": "die",
    "de_plural": "die Zitronen",
    "lv": "Sitrik",
    "level": "A1"
  },
  {
    "de": "Zoo",
    "de_article": "der",
    "de_plural": "die Zoos",
    "lv": "Hayvanat Bahçesi",
    "level": "A1"
  },
  {
    "de": "zu",
    "lv": " • Deri në orë",
    "level": "A1",
    "study": {
      "id": "a1-zu",
      "layout": "standardStudy",
      "translation": " • Deri në orë",
      "explanation": [
        "Ideja kryesore: zu shpesh do të thotë në ose dhjetë, por ajo gjithashtu luan një rol me matësin.",
        "Kişiler ve kurumlar söz konusu olduğunda zu çoğunlukla içeri veya içeri anlamına gelir.",
        "Kur bëhet fjalë për mbiemrat, zu mund të nënkuptojë gjithashtu.",
        "Zu + ndihmon në krijimin e pasigurisë në formimin e pasigurisë: zu lernen, zu gehen."
      ],
      "examples": [
        {
          "de": "Ich gehe zum Arzt.",
          "lv": "Po shkoj te mjeku."
        },
        {
          "de": "Wir gehen zur Schule.",
          "lv": "Jemi duke shkuar në shkollë."
        },
        {
          "de": "Das ist zu teuer.",
          "lv": "Çok pahalı."
        },
        {
          "de": "Ich habe keine Zeit zu lernen.",
          "lv": "Nuk kam kohë të studioj."
        }
      ],
      "comparison": [
        {
          "word": "zu",
          "meaning": "To / in / too / mastar",
          "example": "Ich gehe zum Arzt."
        },
        {
          "word": "nach",
          "meaning": "Me qytete/shtete",
          "example": "Ich fahre nach Berlin."
        },
        {
          "word": "in",
          "meaning": "Ne toke!",
          "example": "Ich gehe in die Schule."
        },
        {
          "word": "bei",
          "meaning": "Birinin/iş yerinde",
          "example": "Ich bin bei Anna."
        }
      ],
      "tip": {
        "text": "Mos harroni: vizitoni një mjek → zum Arzt • Shumë e shtrenjtë → zu teuer."
      },
      "important": [
        "Zu'nun birçok kullanımı vardır, bu nedenle her zaman tasarıma dikkat edin.",
        "Zu teuer nuk do të thotë \"shumë e shtrenjtë\", por \"shumë e shtrenjtë\"."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "zu",
            "zu lernen",
            "zu gehen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "zum Arzt"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zur Schule"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zu teuer"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zu lernen"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "zu"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "zum Arzt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "nach"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "nach Berlin"
              ]
            }
          },
          {
            "word": {
              "green": [
                "in"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "in die Schule"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bei"
              ]
            },
            "meaning": {},
            "example": {}
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "zu"
            ]
          },
          {
            "blue": [
              "zu teuer"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Zucker",
    "de_article": "der",
    "lv": "E ëmbël.",
    "level": "A1"
  },
  {
    "de": "zuerst",
    "lv": "Her şeyden önce",
    "level": "A1"
  },
  {
    "de": "Zug",
    "de_article": "der",
    "de_plural": "die Züge",
    "lv": "Tren",
    "level": "A1",
    "study": {
      "id": "a1-zug",
      "layout": "standardStudy",
      "translation": "Tren",
      "explanation": [
        "Ideja kryesore: der Zug në nivelin A1 kryesisht do të thotë tren.",
        "Përdoret në situatat e përditshme të drejtimit, mbërritjes dhe nisjes.",
        "Në kuptime të tjera, Zug mund të jetë një shëtitje, një skicë ose një funksion, por këto nuk janë kuptimet kryesore të A1.",
        "Fraza shumë të njohura janë mit dem Zug fahren dhe Der Zug kommt."
      ],
      "examples": [
        {
          "de": "Der Zug kommt um acht Uhr.",
          "lv": "Tren sekizde varır."
        },
        {
          "de": "Ich fahre mit dem Zug.",
          "lv": "Trenle seyahat ediyorum"
        },
        {
          "de": "Der Zug ist voll.",
          "lv": "Treni është plot."
        },
        {
          "de": "Der Bus kommt später.",
          "lv": "Otobüs daha sonra gelecek."
        }
      ],
      "comparison": [
        {
          "word": "der Zug",
          "meaning": "Tren",
          "example": "Der Zug kommt."
        },
        {
          "word": "die Bahn",
          "meaning": "Udhëtim me hekurudhë/tren",
          "example": "Ich fahre mit der Bahn."
        },
        {
          "word": "der Bus",
          "meaning": "Një autobus.",
          "example": "Der Bus kommt."
        },
        {
          "word": "die Straßenbahn",
          "meaning": "Pastaj të shëndoshët mund të vraponin, të humbin peshë dhe të ishin të shëndetshëm.",
          "example": "Die Straßenbahn ist hier."
        }
      ],
      "tip": {
        "text": "Mos harroni: një tren i caktuar → thërret Zug."
      },
      "important": [
        "Der Zug në titull duhet të lexohet si \"tren\".",
        "Nuk ka nevojë për kuptime më pak të zakonshme në titullin kryesor A1."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "der Zug",
            "Zug",
            "mit dem Zug fahren"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "Zug"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "mit dem Zug"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Zug"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "red": [
                "Bus"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "der Zug"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "Zug"
              ]
            }
          },
          {
            "word": {
              "green": [
                "die Bahn"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "Bahn"
              ]
            }
          },
          {
            "word": {
              "green": [
                "der Bus"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "Bus"
              ]
            }
          },
          {
            "word": {
              "green": [
                "die Straßenbahn"
              ]
            },
            "meaning": {
              "purple": [
                "Pastaj"
              ]
            },
            "example": {
              "red": [
                "Straßenbahn"
              ]
            }
          }
        ],
        "tip": {
          "left": {}
        },
        "important": [
          {
            "blue": [
              "der Zug"
            ]
          },
          {}
        ]
      }
    }
  },
  {
    "de": "zum",
    "lv": " • Deri në orë",
    "level": "A1",
    "study": {
      "id": "a1-zum",
      "layout": "standardStudy",
      "translation": " • Deri në orë",
      "explanation": [
        "Zoom është shkurtimi i parafjalës zu dhe artikullit dem.",
        "Tam form: zu dem (kime?).",
        "Përdoret në lidhje me emrat mashkullorë dhe neutralë kur tregohet drejtimi ose qëllimi.",
        "Shpesh do të thotë diçka ose dikush • Një mjek, një stacion, një mik.",
        "Pratikte tam zu dem yerine neredeyse her zaman zum kullanılır."
      ],
      "examples": [
        {
          "de": "Ich gehe zum Arzt.",
          "lv": "Po shkoj te mjeku."
        },
        {
          "de": "Wir fahren zum Bahnhof.",
          "lv": "İstasyona gidiyoruz."
        },
        {
          "de": "Sie geht zum Supermarkt.",
          "lv": "Ai do të shkojë në dyqan."
        },
        {
          "de": "Komm zum Essen!",
          "lv": "Eja të hamë!"
        },
        {
          "de": "Er fährt zum Flughafen.",
          "lv": "Havaalanına gidiyor."
        },
        {
          "de": "Wir gehen zum Konzert.",
          "lv": "Ne kemi një koncert."
        },
        {
          "de": "Das Geschenk ist zum Geburtstag.",
          "lv": "Hediye doğum günü içindir."
        },
        {
          "de": "Ich gehe zum Friseur.",
          "lv": "Po shkoj te parukierja."
        }
      ],
      "comparison": [
        {
          "word": "zum",
          "meaning": "Për / nga (kush?)",
          "example": "zum Arzt – Të lutem."
        },
        {
          "word": "zur",
          "meaning": "Kime/içine (karısının ailesi)",
          "example": "zur Schule – Okula"
        },
        {
          "word": "zu",
          "meaning": "/w/'ye de",
          "example": "zu Hause – Ne shtepi."
        },
        {
          "word": "nach",
          "meaning": "Hedef (şehirler/ülkeler)",
          "example": "nach Berlin – Në Berlin"
        },
        {
          "word": "bei",
          "meaning": "W (lokalizacja)",
          "example": "beim Arzt – Të lutem."
        }
      ],
      "tip": [
        "Mos harroni: zu + dem → zum (për kë?).",
        "Për fjalët femërore: zu + der → zur."
      ],
      "important": [
        "Zoom = zu dem, vetëm mashkullor ose me emër pa gjini, për kë? në xhirim.",
        "Tregon një drejtim ose qëllim: te mjeku, te stacioni, te një mik.",
        "Dişil cinsiyet durumunda zur kullanılır: zur Bank, zur Post.",
        "Nuk duhet të ngatërrohet me bei (e vendosur në) ose nach (qytete pa artikuj)."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "zum",
            "zu dem"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "zum"
              ]
            },
            "lv": {}
          }
        ],
        "comparison": [
          {
            "word": {
              "green": [
                "zum"
              ]
            },
            "meaning": {},
            "example": {
              "blue": [
                "zum Arzt"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zur"
              ]
            },
            "meaning": {},
            "example": {
              "yellow": [
                "zur Schule"
              ]
            }
          },
          {
            "word": {
              "green": [
                "zu"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "zu Hause"
              ]
            }
          },
          {
            "word": {
              "green": [
                "nach"
              ]
            },
            "meaning": {},
            "example": {
              "green": [
                "nach Berlin"
              ]
            }
          },
          {
            "word": {
              "green": [
                "bei"
              ]
            },
            "meaning": {},
            "example": {
              "red": [
                "beim Arzt"
              ]
            }
          }
        ],
        "tip": [
          {
            "blue": [
              "zum"
            ]
          },
          {
            "yellow": [
              "zur"
            ]
          }
        ],
        "important": [
          {
            "blue": [
              "zu"
            ],
            "purple": [
              "zu dem"
            ]
          },
          {},
          {
            "yellow": [
              "zur Bank",
              "zur Post"
            ]
          },
          {
            "green": [
              "bei"
            ],
            "red": [
              "nach"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "zumachen",
    "lv": "Kapatmak",
    "level": "A1"
  },
  {
    "de": "zurück",
    "lv": "Geri",
    "level": "A1"
  },
  {
    "de": "zusammen",
    "lv": "Së bashku.",
    "level": "A1"
  },
  {
    "de": "zu viel",
    "lv": "Nuk mundem më.",
    "level": "A1"
  },
  {
    "de": "zwanzig",
    "lv": "Yirmi",
    "level": "A1"
  },
  {
    "de": "zwanzigste",
    "lv": "Njëzetë",
    "level": "A1"
  },
  {
    "de": "zwei",
    "lv": "Dy",
    "level": "A1"
  },
  {
    "de": "zweihundert",
    "lv": "Dyqind",
    "level": "A1"
  },
  {
    "de": "zweimal",
    "lv": "İki kere",
    "level": "A1"
  },
  {
    "de": "zweite",
    "lv": "5 sekonda.",
    "level": "A1"
  },
  {
    "de": "Zwiebel",
    "de_article": "die",
    "de_plural": "die Zwiebeln",
    "lv": "Soğan",
    "level": "A1"
  },
  {
    "de": "zwischen",
    "lv": "Midis",
    "level": "A1"
  },
  {
    "de": "zwölf",
    "lv": "On iki",
    "level": "A1"
  },
  {
    "de": "zwölfte",
    "lv": "Onikinci",
    "level": "A1"
  },
  {
    "de": "fernsehen",
    "lv": "TV izle",
    "level": "A1",
    "study": {
      "id": "a1-fernsehen",
      "layout": "standardStudy",
      "translation": "TV izle",
      "explanation": "Ideja kryesore: fernsehen është një folje e ndarë – ich sehe fern, du siehst fern. Kjo do të thotë të shikosh televizor. Të mos ngatërrohet me Das Fernsehen (televizioni si medium).",
      "examples": [
        {
          "de": "Ich sehe heute Abend fern.",
          "lv": "Bu gece televizyon izliyorum."
        },
        {
          "de": "Siehst du oft fern?",
          "lv": "Shikon shpesh televizor?"
        },
        {
          "de": "Die Kinder sehen am Nachmittag fern.",
          "lv": "Fëmijët shikojnë televizor pasdite."
        }
      ],
      "comparison": [
        {
          "word": "fernsehen",
          "meaning": "TV izle",
          "example": "Ich sehe fern. = Es skatos televīziju."
        },
        {
          "word": "das Fernsehen",
          "meaning": "Televizioni (media)",
          "example": "Im Fernsehen läuft ein Film. = Televīzijā rāda filmu."
        },
        {
          "word": "sehen",
          "meaning": "Görmek",
          "example": "Ich sehe einen Film. = Es redzu filmu."
        }
      ],
      "tip": {
        "leftBlocks": [
          {
            "text": "Fernsehen (ich sehe fern) përdoret për veprimin. Das Fernsehen përdoret në një program ose mjedis televiziv."
          }
        ]
      },
      "important": {
        "text": "Fernsehen = folje (ich sehe fern). das Fernsehen = emër, vetëm njëjës."
      },
      "sectionAccents": {
        "explanation": {
          "blue": [
            "fernsehen",
            "sehe",
            "fern"
          ],
          "purple": [
            "Ideja"
          ],
          "green": [
            "das Fernsehen"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "sehe",
                "fern"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "Siehst",
                "fern"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "sehen",
                "fern"
              ]
            },
            "lv": {}
          }
        ]
      }
    }
  },
  {
    "de": "Fernsehen",
    "de_article": "das",
    "lv": "Televizor!",
    "level": "A1",
    "study": {
      "id": "a1-fernsehen-study",
      "layout": "standardStudy",
      "translation": "Televizor!",
      "explanation": [
        "Ideja kryesore: Emri, vetëm njëjës. Në përgjithësi e përkufizon televizionin si një medium ose një transmetim.",
        "Das Fernsehen temel olarak şu anlama gelir: yayını izleyin.",
        "Çoğunlukla şunu tanımlar: eylem.",
        "Das Fernsehen her şeyden önce bir araç olarak televizyondur.",
        "Shpesh përcakton: emrin (vetëm njëjës).",
        "Fernsehen është një folje që mund të ndahet në: ich sehe fern, du siehst fern.",
        "Das Fernsehen bir isimdir ve yalnızca tekil hali vardır • Çoğul hali yoktur."
      ],
      "examples": [
        {
          "de": "Was gibt es heute im Fernsehen?",
          "lv": "Bugün televizyonda ne var?"
        },
        {
          "de": "Was gibt es heute im Fernsehen?",
          "lv": "Bugün televizyonda ne var?"
        },
        {
          "de": "Im Fernsehen läuft ein Film.",
          "lv": "Ka një film në TV."
        },
        {
          "de": "Das Fernsehen ist heute langweilig.",
          "lv": "Shfaqja e sotme televizive është e mërzitshme."
        },
        {
          "de": "Ich sehe heute Abend fern.",
          "lv": "Bu gece televizyon izliyorum."
        },
        {
          "de": "Was gibt es im Fernsehen?",
          "lv": "Çfarë shfaqet në televizor?"
        }
      ],
      "tip": [
        "Përdorni fernsehen (ich sehe fern) kur flisni për një aktivitet. Përdorni das Fernsehen kur flisni për një shfaqje televizive ose media.",
        "Emri është vetëm njëjës. Ai përcakton televizionin në përgjithësi si një medium ose transmetim."
      ],
      "important": [
        "Fernsehen bölünebilir: sehen + eğrelti otu.",
        "Das Fernsehen nuk është shumës - *die Fernsehen nuk është.",
        "E gabuar: die Fernsehen → E saktë: das Fernsehen",
        "Veprimi: fernsehen → ich sehe fern."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "Fernsehen"
          ],
          "yellow": [
            "Fernsehen"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "Fernsehen"
              ]
            },
            "lv": {
              "purple": [
                "televizyonda"
              ]
            }
          },
          {
            "de": {
              "green": [
                "Fernsehen"
              ]
            },
            "lv": {
              "purple": [
                "televizyonda"
              ]
            }
          },
          {
            "de": {
              "green": [
                "Fernsehen"
              ]
            },
            "lv": {
              "purple": [
                "një"
              ]
            }
          },
          {
            "de": {
              "green": [
                "das Fernsehen",
                "fernsehen"
              ]
            },
            "lv": {
              "purple": [
                "televizive"
              ]
            }
          },
          {
            "de": {},
            "lv": {
              "purple": [
                "televizyon"
              ]
            }
          },
          {
            "de": {
              "green": [
                "Fernsehen"
              ]
            },
            "lv": {
              "purple": [
                "Çfarë"
              ]
            }
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "fernsehen"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Appetit",
    "de_article": "der",
    "lv": "İştah",
    "level": "A1",
    "study": {
      "id": "a1-appetit",
      "layout": "standardStudy",
      "translation": "İştah",
      "explanation": [
        "Ideja kryesore: ndjenja e dëshirës për të ngrënë. vetëm njëjës – pa shumës.",
        "Der Appetit kısaca yemek yeme arzusu anlamına gelir.",
        "Shpesh përkufizon: emocionin (vetëm njëjës).",
        "Der Appetit është vetëm një gjë e veçantë • Oreksi.",
        "A1 seviyesinde, genellikle birbirleriyle tanışırlar, örneğin: Guten Appetit!"
      ],
      "examples": [
        {
          "de": "Guten Appetit!",
          "lv": "Oreks i mirë!"
        },
        {
          "de": "Guten Appetit!",
          "lv": "Hani!"
        },
        {
          "de": "Ich habe keinen Appetit.",
          "lv": "İştahım yok."
        }
      ],
      "tip": [
        "Der Appetit = oreks",
        "Der përdor Appetit kur konteksti është i përshtatshëm për këtë kuptim."
      ],
      "important": [
        "Der Appetit yalnızca tekildir.",
        "False: die Appetite → True: der Appetit",
        "E gabuar: Ich bin Appetit. → E saktë: Ich habe Appetit.",
        "Poczucie: thotë Apetyt."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "der Appetit",
            "appetit"
          ],
          "purple": [
            "Appetit"
          ],
          "yellow": [
            "Appetit"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "appetit"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "appetit"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "appetit"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "der Appetit"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "essen",
    "lv": "Ngrënë? Ngrë... ngrënë?",
    "level": "A1",
    "study": {
      "id": "a1-essen",
      "layout": "standardStudy",
      "translation": "Ngrënë? Ngrë... ngrënë?",
      "explanation": [
        "Ana fikir: Fiil – yemek.",
        "Eseni do të thotë të hash para së gjithash.",
        "Çoğunlukla şunu tanımlar: eylem.",
        "Essen temel olarak şu anlama gelir: yiyecek veya yemek.",
        "Shpesh përshkruhet si shi.",
        "Essen yemek demektir.",
        "Das Essen mund të nënkuptojë ushqim ose ushqim në përgjithësi."
      ],
      "examples": [
        {
          "de": "Ich esse gern Pizza.",
          "lv": "Pizza yemeyi severim."
        },
        {
          "de": "Was wollt ihr essen?",
          "lv": "Ne yemek istersin?"
        },
        {
          "de": "Wir essen um 12 Uhr.",
          "lv": "Saat 12.00'de yemek yiyoruz."
        },
        {
          "de": "Das Essen ist fertig.",
          "lv": "(kamarier) Darka është gati."
        },
        {
          "de": "Das Essen schmeckt sehr gut.",
          "lv": "Ushqimi ka shije shumë të mirë."
        },
        {
          "de": "Das Essen schmeckt gut.",
          "lv": "Ushqimi ka shije të mirë."
        }
      ],
      "tip": [
        "Essen = yemek",
        "Bağlam bu anlama uygun olduğunda essen kullanın."
      ],
      "important": [
        "Eseni është folje pa artikuj.",
        "Das Eseni nuk është i njëjtë me Eseninin.",
        "Eylem: essen.",
        "Rasti/vakti: das Essen."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "essen"
          ],
          "blue": [
            "essen"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "esse"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "essen"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "Eseni"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Essen",
    "de_article": "das",
    "lv": "Yiyecek • Yemek",
    "level": "A1",
    "study": {
      "id": "a1-essen-study",
      "layout": "standardStudy",
      "translation": "Yiyecek • Yemek",
      "explanation": [
        "Ana fikir: İsim – yiyecek veya bütün bir öğün.",
        "Das Essen do të thotë të hash para së gjithash.",
        "Çoğunlukla şunu tanımlar: eylem.",
        "Das Essen në thelb do të thotë: ushqim ose ushqim.",
        "Shpesh përshkruhet si shi.",
        "Essen yemek demektir.",
        "Das Essen mund të nënkuptojë ushqim ose ushqim në përgjithësi."
      ],
      "examples": [
        {
          "de": "Das Essen schmeckt gut.",
          "lv": "Ushqimi ka shije të mirë."
        },
        {
          "de": "Was wollt ihr essen?",
          "lv": "Ne yemek istersin?"
        },
        {
          "de": "Wir essen um 12 Uhr.",
          "lv": "Saat 12.00'de yemek yiyoruz."
        },
        {
          "de": "Das Essen ist fertig.",
          "lv": "(kamarier) Darka është gati."
        },
        {
          "de": "Das Essen schmeckt sehr gut.",
          "lv": "Ushqimi ka shije shumë të mirë."
        },
        {
          "de": "Das Essen schmeckt gut.",
          "lv": "Ushqimi ka shije të mirë."
        }
      ],
      "tip": [
        "Das Essen = yemek",
        "Përdor Essen kur konteksti përshtatet."
      ],
      "important": [
        "Eseni është folje pa artikuj.",
        "Das Eseni nuk është i njëjtë me Eseninin.",
        "Eylem: essen.",
        "Rasti/vakti: das Essen."
      ],
      "sectionAccents": {
        "explanation": {
          "yellow": [
            "das Essen",
            "essen"
          ],
          "green": [
            "Essen"
          ]
        },
        "examples": [
          {
            "de": {
              "yellow": [
                "das Essen",
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "das Essen",
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "das Essen",
                "essen"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "das Essen",
                "essen"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "Gemüse",
    "de_article": "das",
    "lv": "Perime",
    "level": "A1",
    "study": {
      "id": "a1-gemuese",
      "layout": "standardStudy",
      "translation": "Perime",
      "explanation": [
        "Ideja kryesore: Rreth perimeve në përgjithësi. *die Gemüse nuk ka një shumës në gjermanisht.",
        "Das Gemüse do të thotë perime në përgjithësi.",
        "Çoğunlukla tanımlanır: herhangi bir cinsiyette (yalnızca tekil)."
      ],
      "examples": [
        {
          "de": "Ich esse gern Gemüse.",
          "lv": "Më pëlqen të ha perime."
        },
        {
          "de": "Ich esse gern Gemüse.",
          "lv": "Më pëlqen të ha perime."
        },
        {
          "de": "Das Gemüse ist frisch.",
          "lv": "Perime të freskëta."
        },
        {
          "de": "Wir kaufen Gemüse auf dem Markt.",
          "lv": "Marketten sebze alıyoruz."
        },
        {
          "de": "Ich mag Obst und Gemüse.",
          "lv": "Meyve ve sebzeleri severim."
        },
        {
          "de": "Ich esse Gemüse.",
          "lv": "Sebze yerim."
        }
      ],
      "tip": [
        "Das Gemüse = perime",
        "Përdorni das Gemüse kur kontexti përputhet me këtë kuptim."
      ],
      "important": [
        "E gabuar: vdes Gemüse, vdes Obsts.",
        "Yanlış: die Gemüse → Doğru: das Gemüse",
        "Das Gemüse = sebzeler (genel olarak)."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "das Gemüse",
            "gemüse"
          ],
          "green": [
            "Gemüse"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "gemüse"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gemüse"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "das Gemüse",
                "gemüse"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gemüse"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gemüse"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "gemüse"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "Obst",
    "de_article": "das",
    "lv": "-Frutë.",
    "level": "A1",
    "study": {
      "id": "a1-obst",
      "layout": "standardStudy",
      "translation": "-Frutë.",
      "explanation": [
        "Ideja kryesore: Zakonisht për frutat. Nuk ka pengesa shumës *në gjermanisht.",
        "Das Obst esas olarak şu anlama gelir: genel olarak meyve.",
        "Çoğunlukla tanımlanır: herhangi bir cinsiyette (yalnızca tekil)."
      ],
      "examples": [
        {
          "de": "Wir essen viel Obst.",
          "lv": "Ne hamë shumë fruta."
        },
        {
          "de": "Wir essen viel Obst.",
          "lv": "Ne hamë shumë fruta."
        },
        {
          "de": "Obst ist gesund.",
          "lv": "Frutat janë të shëndetshme."
        },
        {
          "de": "Ich mag Obst und Gemüse.",
          "lv": "Meyve ve sebzeleri severim."
        },
        {
          "de": "Wir essen Obst.",
          "lv": "Meyve yeriz."
        }
      ],
      "tip": [
        "Das Obst = fruta",
        "Përdorni Obst kur konteksti përshtatet me të kuptuarit."
      ],
      "important": [
        "Yanlış: die Obsts → Doğru: das Obst",
        "Das Obst = fruta (në përgjithësi)."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "das Obst",
            "obst"
          ],
          "yellow": [
            "Obst"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "obst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "obst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "obst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "obst"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "obst"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "das Obst"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Ferien",
    "de_article": "die",
    "lv": "Tatiller (okul)",
    "level": "A1",
    "study": {
      "id": "a1-ferien",
      "layout": "standardStudy",
      "translation": "Tatiller (okul)",
      "explanation": [
        "Ana fikir: yalnızca çoğul. Okul veya eğitim tatilleri - her zaman çoğul olarak kullanılır.",
        "Die Ferien në thelb do të thotë: pushimet shkollore.",
        "Genellikle şu şekilde karakterize edilir: yalnızca çoğul.",
        "Die Ferien shfaqet vetëm në shumës - gjithmonë në shumës (den Ferien)."
      ],
      "examples": [
        {
          "de": "In den Ferien fahren wir ans Meer.",
          "lv": "Shkojmë në bregdet në fundjavë."
        },
        {
          "de": "In den Ferien habe ich viel Zeit.",
          "lv": "Tatillerde çok zamanım oluyor."
        },
        {
          "de": "Was macht ihr in den Ferien?",
          "lv": "Çfarë bën gjatë pushimeve?"
        },
        {
          "de": "Die Schule ist in den Ferien zu.",
          "lv": "Shkolla është e mbyllur gjatë festave."
        },
        {
          "de": "In den Ferien fahren wir ans Meer.",
          "lv": "Do të shkojmë në bregdet për pushime."
        },
        {
          "de": "In den Ferien",
          "lv": "Me pushime (në shkollë)."
        }
      ],
      "comparison": [
        {
          "word": "die Ferien",
          "meaning": "Pushim nga shkolla/puna (vetëm në tavolinë)",
          "example": "In den Ferien fahren wir weg. – Do të shkojmë diku në fundjavë."
        },
        {
          "word": "der Urlaub",
          "meaning": "İşten ayrılma (yalnızca herkes)",
          "example": "Ich habe zwei Wochen Urlaub. – Kam dy javë pushim."
        }
      ],
      "tip": [
        "Yalnızca çoğul. Okul veya eğitim tatilleri - her zaman çoğul olarak kullanılır.",
        "Bağlam bu anlama uygun olduğunda die Ferien'i kullanın."
      ],
      "important": [
        "Ferien është gjithmonë i pandehuri: në den Ferien.",
        "False: in der Ferien → True: in den Ferien",
        "Shkolla: die Ferien (vetëm në shumës).",
        "Yalnızca çoğul. Okul veya eğitim tatilleri - her zaman çoğul olarak kullanılır."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "die Ferien",
            "ferien"
          ],
          "green": [
            "Ferien"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "ferien"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ferien"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ferien"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ferien"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ferien"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "ferien"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  },
  {
    "de": "Urlaub",
    "de_article": "der",
    "lv": "Pushime",
    "level": "A1",
    "study": {
      "id": "a1-urlaub",
      "layout": "standardStudy",
      "translation": "Pushime",
      "explanation": [
        "Ana fikir: sadece tekil. İşten ayrılın - her zaman tekildir.",
        "Der Urlaub w zasadzie oznacza: czas wolny od pracy.",
        "Zakonisht karakterizohet si: vetëm njëjës.",
        "Der Urlaub, işten ayrılmanın tekil bir şeklidir (im Urlaub)."
      ],
      "examples": [
        {
          "de": "Mein Vater ist im Urlaub.",
          "lv": "Babai im është me pushime."
        },
        {
          "de": "Mein Vater ist im Urlaub.",
          "lv": "Babai im është me pushime."
        },
        {
          "de": "Nächste Woche habe ich Urlaub.",
          "lv": "Javën tjetër kam pushime."
        },
        {
          "de": "Wir machen Urlaub in Spanien.",
          "lv": "Jemi me pushime në Spanjë."
        },
        {
          "de": "im Urlaub",
          "lv": "Na wakacjach (w pracy)."
        }
      ],
      "comparison": [
        {
          "word": "der Urlaub",
          "meaning": "İşten ayrılma (yalnızca herkes)",
          "example": "Mein Vater ist im Urlaub. – Babai im është me pushime."
        },
        {
          "word": "die Ferien",
          "meaning": "Pushim nga shkolla/puna (vetëm në tavolinë)",
          "example": "Die Kinder haben Ferien. – Fëmijët kanë pushime."
        }
      ],
      "tip": [
        "Vetëm një. Lëre punën - është gjithmonë unike.",
        "Përdorni der Urlaub kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Niepoprawnie: die Ferie, der Urlabe (na poziomie A1).",
        "Urlaub: im Urlaub sein / Urlaub machen.",
        "Yanlış: die Urlaube → Doğru: der Urlaub",
        "Puna: der Urlaub (vetëm njëjës)."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "der Urlaub",
            "urlaub"
          ],
          "yellow": [
            "Urlaub"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "urlaub"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "urlaub"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "urlaub"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "urlaub"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "urlaub"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "der"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Stadt",
    "de_article": "die",
    "de_plural": "die Städte",
    "lv": "Qytet",
    "level": "A1"
  },
  {
    "de": "Staat",
    "de_article": "der",
    "de_plural": "die Staaten",
    "lv": "Shteti",
    "level": "A1"
  },
  {
    "de": "Uhr",
    "de_article": "die",
    "de_plural": "die Uhren",
    "lv": "Ora!",
    "level": "A1",
    "study": {
      "id": "a1-uhr",
      "layout": "standardStudy",
      "translation": "Ora!",
      "explanation": [
        "Ana fikir: Saat veya kol saati. Ayrıca saatteki saat: Es ist acht Uhr.",
        "Die Uhr në thelb do të thotë: koha në pajisje ose orë.",
        "Zakonisht karakterizohet si: një kohë specifike.",
        "Die Uhr saat anlamına gelir • Saatin içindeki bir cihaz veya zaman (Es ist acht Uhr, meine Uhr)."
      ],
      "examples": [
        {
          "de": "Es ist acht Uhr.",
          "lv": "Saat sekiz (sekiz)."
        },
        {
          "de": "Es ist acht Uhr.",
          "lv": "Saat sekiz (sekiz)."
        },
        {
          "de": "Meine Uhr ist kaputt.",
          "lv": "Më është prishur ora."
        },
        {
          "de": "Es ist acht Uhr.",
          "lv": "Saat sekiz."
        },
        {
          "de": "Es ist acht Uhr.",
          "lv": "Ora tetë (orë)."
        },
        {
          "de": "die Uhr",
          "lv": "Pajisje/orë në orë • Die Zeit"
        }
      ],
      "tip": [
        "Saat veya kol saati. Ayrıca saatteki saat: Es ist acht Uhr.",
        "Bağlam bu anlama uygun olduğunda die Uhr'u kullanın."
      ],
      "important": [
        "Die Uhr: pajisje (meine Uhr) ose kohë (acht Uhr).",
        "Die Uhr: Kullanmadan önce bağlamı kontrol edin."
      ],
      "sectionAccents": {
        "explanation": {
          "blue": [
            "Uhr"
          ]
        },
        "examples": [
          {
            "de": {
              "blue": [
                "uhr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "uhr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "uhr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "uhr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "uhr"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "blue": [
                "die Uhr",
                "uhr"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "blue": [
              "die Uhr"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "Zeit",
    "de_article": "die",
    "de_plural": "die Zeiten",
    "lv": "Zaman (an/zaman dilimi)",
    "level": "A1",
    "study": {
      "id": "a1-zeit",
      "layout": "standardStudy",
      "translation": "Zaman (an/zaman dilimi)",
      "explanation": [
        "Ideja kryesore: Koha si koncept • Një moment, një shans, një kornizë kohore.",
        "Mbi të gjitha, Die Zeit është një moment, një shans.",
        "Genellikle şu şekilde karakterize edilir: soyut bir kavram.",
        "Die Zeit është një koncept abstrakt • Koha, momenti ose shansi (ich habe keine Zeit)."
      ],
      "examples": [
        {
          "de": "Ich habe keine Zeit.",
          "lv": "Nuk ka kohë të mjaftueshme."
        },
        {
          "de": "Ich habe keine Zeit.",
          "lv": "Nuk ka kohë të mjaftueshme."
        },
        {
          "de": "Hast du Zeit?",
          "lv": "Ke një sekondë?"
        },
        {
          "de": "Die Zeit vergeht schnell.",
          "lv": "Koha po mbaronte."
        }
      ],
      "tip": [
        "Koha si koncept • Një moment është një shans, një pjesë e kohës.",
        "Bağlam bu anlama uygun olduğunda die Zeit'ı kullanın."
      ],
      "important": [
        "Die Zeit: Kontrolloni përmbajtjen para përdorimit.",
        "Die Zeit: Kontrolloni përmbajtjen para përdorimit."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "die Zeit",
            "zeit"
          ],
          "yellow": [
            "Zeit"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "zeit"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "zeit"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "zeit"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "die Zeit",
                "zeit"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {
            "green": [
              "die Zeit"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "einmal",
    "lv": "Një herë • Një herë",
    "level": "A1",
    "study": {
      "id": "a1-einmal",
      "layout": "standardStudy",
      "translation": "Një herë • Një herë",
      "explanation": [
        "Ideja kryesore: I referohet kohës ose së kaluarës (unë një herë...).",
        "Einmal temel olarak şu anlama gelir: bir kez/geçmişte.",
        "Genellikle şu şekilde karakterize edilir: hava koşulları.",
        "Einmal i referohet një kohe ose të kaluare specifike (një herë...)."
      ],
      "examples": [
        {
          "de": "Ich war einmal in Berlin.",
          "lv": "Isha një herë në Berlin."
        },
        {
          "de": "Ich war einmal in Berlin.",
          "lv": "Isha një herë në Berlin."
        }
      ],
      "tip": [
        "Einmal = një herë",
        "Bağlam bu anlama uygun olduğunda einmal kullanın."
      ],
      "important": [
        "Einmal = një herë ose një herë në të kaluarën.",
        "I referohet kohës ose të kaluarës (dikur isha...)."
      ],
      "sectionAccents": {
        "explanation": {
          "green": [
            "einmal"
          ]
        },
        "examples": [
          {
            "de": {
              "green": [
                "einmal"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "green": [
                "einmal"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {},
          {}
        ],
        "important": [
          {
            "green": [
              "einmal"
            ]
          }
        ]
      }
    }
  },
  {
    "de": "noch mal",
    "lv": "\"Shtyrje me dy duar.\" Përsëri.",
    "level": "A1",
    "study": {
      "id": "a1-noch-mal",
      "layout": "standardStudy",
      "translation": "\"Shtyrje me dy duar.\" Përsëri.",
      "explanation": [
        "Ana fikir: Tekrar anlamına gelir - bir eylemi tekrarlamak veya tekrarlanmasını istemek."
      ],
      "examples": [
        {
          "de": "Noch mal, bitte.",
          "lv": "Bir kez daha lütfen."
        },
        {
          "de": "Noch mal, bitte.",
          "lv": "Bir kez daha lütfen"
        },
        {
          "de": "Sag das noch mal.",
          "lv": "Lusian, përsërite."
        }
      ],
      "tip": [
        "Përdorni noch mal kur konteksti i përshtatet këtij kuptimi.",
        "Përdorni noch mal kur konteksti i përshtatet këtij kuptimi."
      ],
      "important": [
        "Yine, bir eylemi tekrarlamak veya tekrarlanmasını istemek anlamına gelir.",
        "Mallrat noch: Kontrolloni kontekstin para përdorimit."
      ],
      "sectionAccents": {
        "explanation": {},
        "examples": [
          {
            "de": {
              "yellow": [
                "noch mal"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "noch mal"
              ]
            },
            "lv": {}
          },
          {
            "de": {
              "yellow": [
                "noch mal"
              ]
            },
            "lv": {}
          }
        ],
        "tip": [
          {}
        ],
        "important": [
          {}
        ]
      }
    }
  }
];

window.A1_WORDS = A1_WORDS;
