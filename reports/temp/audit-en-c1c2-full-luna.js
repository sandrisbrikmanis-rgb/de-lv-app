#!/usr/bin/env node
/**
 * EN-DE C1/C2 full Luna linguistic audit (read-only). All 791 cards.
 */
const fs = require("fs");
...
const {
  ...
  auditCardsBatch,
  recordRetryReason,
} = require("./openai-luna-en-c1c2-full-audit");

const ROOT = path.join(__dirname, "..", "..");
const OUT_JSON = path.join(ROOT, "reports", "temp", "en-c1c2-luna-linguistic-findings.json");
const PROGRESS_PATH = path.join(ROOT, "reports", "temp", ".en-c1c2-luna-audit-progress.json");
const STATS_PATH = path.join(ROOT, "reports", "temp", ".en-c1c2-luna-audit-stats.json");
...
function buildSimpleCard(lvE, enE, index, level) {
  return {
    cardId: entryId(enE, index, level),
    level,
    field: "lv",
    de: enE.de,
...
function buildStudyCard(lvE, enE, index, level) {
  return {
    cardId: entryId(enE, index, level),
    level,
    de: enE.de,
...
async function main() {
  ...
  for (const cfg of LEVELS) {
    hashBefore[cfg.level] = {
      data: md5(path.join(ROOT, cfg.enFile)),
      www: md5(path.join(ROOT, cfg.wwwFile)),
    };
  }
  ...
  let auditData = loadAuditJson() || {
    meta: { model: DEFAULT_MODEL, startedAt: new Date().toISOString(), scope: "EN-DE C1/C2" },
    batches: [],
    allResults: [],
    findings: [],
    auditedCardIds: [],
  ...
  for (const cfg of LEVELS) {
    const lv = load(cfg.deFile, cfg.key);
    const en = load(cfg.enFile, cfg.key);
    for (let i = 0; i < lv.length; i++) {
      if (en[i].study) studyCards.push(buildStudyCard(lv[i], en[i], i, cfg.level));
      else simpleCards.push(buildSimpleCard(lv[i], en[i], i, cfg.level));
    }
  }

  console.log(`EN C1/C2 Luna audit: ${simpleCards.length + studyCards.length} entries (${simpleCards.length} flash, ${studyCards.length} study)`);
  ...
  await processBatches(simpleCards, SIMPLE_BATCH, "simple", "en_c1c2_full_simple");
  await processBatches(studyCards, STUDY_BATCH, "study", "en_c1c2_full_study");
  ...
  for (const cfg of LEVELS) {
    ...
    if (hashBefore[cfg.level].data !== hashAfter[cfg.level].data || hashBefore[cfg.level].www !== hashAfter[cfg.level].www) {
      throw new Error(`DATA FILES CHANGED DURING AUDIT (${cfg.level})`);
    }
  }

  const totalExpected = simpleCards.length + studyCards.length;
  ...
  auditData.meta.cardsExpected = totalExpected;
  ...
  auditData.meta.c1Cards = LEVELS[0] ? load(LEVELS[0].enFile, LEVELS[0].key).length : 0;
  auditData.meta.c2Cards = LEVELS[1] ? load(LEVELS[1].enFile, LEVELS[1].key).length : 0;
...
main().catch((error) => {
  console.error("EN C1/C2 Luna audit failed:", error.message);
  process.exit(1);
});
