#!/usr/bin/env node
/**
 * Generate EN-DE B1 HIGH owner review block #6 (25 unique cards).
...
const RESOLVED_WORKFLOW_EXCLUSIONS = ["b1-Gen-1055"];
...
/** HIGH #4 biegen audit artifact (soft hyphen U+00AD) — must not reappear in HIGH #6. */
...
function verifyExclusionIntegrity(blockIds, criticalSet) {
  ...
  if (dup.length) {
    throw new Error(`Duplicate logical cards across HIGH #1–#5: ${[...new Set(dup)].join(", ")}`);
  }
...
function verifyResolvedExclusions(excludedSet, selectedCardIds) {
  ...
  if (genInSelection) {
    throw new Error("b1-Gen-1055 resolved exclusion: FAIL — present in HIGH #6 selection");
  }

  const biegenInSelection = selectedCardIds.some((id) => normalizeCardId(id) === HIGH4_BIEGEN_NORMALIZED);
  if (biegenInSelection) {
    throw new Error("HIGH #4 biegen normalized exclusion: FAIL — biegen in HIGH #6 selection");
  }
...
function main() {
  ...
  const allPriorIds = [...high1Ids, ...high2Ids, ...high3Ids, ...high4Ids, ...high5Ids];
  console.log(`Total previous HIGH selections: ${allPriorIds.length}`);

  const integrity = verifyExclusionIntegrity(
    [high1Ids, high2Ids, high3Ids, high4Ids, high5Ids],
    criticalSet
  );
  console.log(`Duplicate logical cards across HIGH #1–#5: ${integrity.duplicateCount}`);
  ...
  const high5Json = JSON.parse(
    fs.readFileSync(path.join(ROOT, "reports/temp/en-b1-high-owner-review-05.json"), "utf8")
  );
  const workflowRemainingBeforeHigh6 =
    high5Json.workflowUnresolvedHighCardsAfterHigh5 ?? high5Json.remainingAfterHigh5 ?? 323;
  const auditPoolAfterHigh5 =
    high5Json.auditSelectionPoolAfterHigh5 ?? high5Json.remainingAfterSelectionPool ?? workflowRemainingBeforeHigh6;
  ...
  if (internalDup.length) {
    console.error("Duplicate logical cards inside HIGH #6:", internalDup);
    process.exit(1);
  }
  ...
  const auditPoolAfterHigh6 = auditSelectionPool.size - selectedCardIds.length;
  const workflowRemainingAfterHigh6 = workflowRemainingBeforeHigh6 - BLOCK_SIZE;
  const expectedArithmeticRemaining = workflowRemainingBeforeHigh6 - BLOCK_SIZE;
  const expectedReferenceArithmetic = 323 - BLOCK_SIZE;

  const remainingDiscrepancy =
    workflowRemainingAfterHigh6 !== expectedArithmeticRemaining
      ? {
          workflowRemainingAfter: workflowRemainingAfterHigh6,
          expectedArithmetic: expectedArithmeticRemaining,
          expectedReference323Minus25: expectedReferenceArithmetic,
          workflowRemainingBefore: workflowRemainingBeforeHigh6,
          auditPoolBefore: auditSelectionPool.size,
          auditPoolAfter: auditPoolAfterHigh6,
        }
      : workflowRemainingAfterHigh6 !== expectedReferenceArithmetic
        ? {
            workflowRemainingAfter: workflowRemainingAfterHigh6,
            expectedReference323Minus25: expectedReferenceArithmetic,
            note: "Calculated matches prior block arithmetic but differs from 323-25 reference",
          }
        : null;

  if (remainingDiscrepancy && workflowRemainingAfterHigh6 !== expectedReferenceArithmetic) {
  ...
  const discrepancyText =
    workflowRemainingAfterHigh6 === expectedReferenceArithmetic ? "none" : "see remainingDiscrepancy in JSON";

  mdParts.push("---", "", "## Coverage summary", "");
  mdParts.push("```text");
  mdParts.push("EN–DE B1 HIGH OWNER REVIEW #6");
  mdParts.push("");
  ...
  mdParts.push(`Workflow unresolved HIGH cards before HIGH #6: ${workflowRemainingBeforeHigh6}`);
  mdParts.push(`HIGH #6 selected: ${BLOCK_SIZE}`);
  mdParts.push(`Workflow unresolved HIGH cards after HIGH #6: ${workflowRemainingAfterHigh6}`);
  mdParts.push(`Audit-selection pool before HIGH #6: ${auditSelectionPool.size}`);
  mdParts.push(`Audit-selection pool after HIGH #6: ${auditPoolAfterHigh6}`);
  mdParts.push(`Expected arithmetic: 323 - 25 = ${expectedReferenceArithmetic}`);
  mdParts.push(`Calculated workflow remaining: ${workflowRemainingAfterHigh6}`);
  ...
  const output = {
    ...
    exclusionVerification: {
      ...
      totalPreviousHighSelections: 125,
      ...
      high4BiegenNormalizedExclusion: "PASS",
      duplicateLogicalCardsAcrossHigh1To5: 0,
      duplicateLogicalCardsInsideHigh6: 0,
    ...
    workflowUnresolvedHighCardsBeforeHigh6: workflowRemainingBeforeHigh6,
    high6Selected: BLOCK_SIZE,
    workflowUnresolvedHighCardsAfterHigh6: workflowRemainingAfterHigh6,
    auditSelectionPoolBeforeHigh6: auditSelectionPool.size,
    auditSelectionPoolAfterHigh6: auditPoolAfterHigh6,
    auditPoolAfterHigh5Reference: auditPoolAfterHigh5,
    expectedArithmeticRemaining: expectedReferenceArithmetic,
    calculatedWorkflowRemaining: workflowRemainingAfterHigh6,
    remainingDiscrepancy: workflowRemainingAfterHigh6 === expectedReferenceArithmetic ? null : remainingDiscrepancy,
  ...
  fs.writeFileSync(path.join(ROOT, "reports/en-b1-high-owner-review-06.md"), mdParts.join("\n"));
  fs.writeFileSync(
    path.join(ROOT, "reports/temp/en-b1-high-owner-review-06.json"),
    ...
    JSON.stringify(
      {
        ...
        duplicateLinks,
        metadataAnomalies,
        workflowBefore: workflowRemainingBeforeHigh6,
        workflowAfter: workflowRemainingAfterHigh6,
        auditPoolBefore: auditSelectionPool.size,
        auditPoolAfter: auditPoolAfterHigh6,
