#!/usr/bin/env node
/**
 * Generate EN-DE B2 OWNER repair Group 2 report + regression summary.
 */
const fs = require("fs");
...
const REPAIRS = path.join(__dirname, "en-b2-owner-repair-group-02-repairs.json");
const APPLY_LOG = path.join(__dirname, "en-b2-owner-repair-group-02-apply-log.json");
const OUT_JSON = path.join(__dirname, "en-b2-owner-repair-group-02.json");
const OUT_MD = path.join(ROOT, "reports", "en-b2-owner-repair-group-02.md");
...
function main() {
  ...
  const regression = {
    ...
    lunaApiAvailable: false,
    severity: { CRITICAL: 0, HIGH: 0, MEDIUM: 0, LOW: 0 },
    note:
      "Full Luna re-audit not run (no API key). Post-apply exact match + structural/heuristic checks only.",
  };

  const fullJson = {
    meta: {
      group: "2/10",
      manifestPositions: "051-100",
  ...
  const mdLines = [
    "# EN–DE B2 — OWNER-approved repair Group 2 (items 051–100)",
    "",
    "**Date:** " + new Date().toISOString().slice(0, 10),
    "**Mode:** OWNER-approved exact apply (Group 2/10)",
    "",
    "## Scope",
    "",
    "| Metric | Value |",
    "| --- | ---: |",
    "| Group | 2 |",
    "| Manifest positions | 051–100 |",
    "| Approved | 50 |",
    "| Attempted | " + apply.attempted + " |",
    "| Applied | " + apply.applied + " |",
    "| Exact verified | " + apply.verified + "/" + apply.applied + " |",
    ...
    "## Artefacts",
    "",
    "- reports/en-b2-owner-repair-group-02.md",
    "- reports/temp/en-b2-owner-repair-group-02.json",
    "- reports/temp/en-b2-owner-repair-group-02-repairs.json",
    "- reports/temp/en-b2-owner-repair-group-02-apply-log.json",
    "",
