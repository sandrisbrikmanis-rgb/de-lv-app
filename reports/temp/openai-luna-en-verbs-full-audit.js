...
const SYSTEM_PROMPT = [
  ...
  "Category: TRANSLATION | GRAMMAR | SEMANTICS | ORTHOGRAPHY | NATURALNESS | FOREIGN_REMNANT | PARADIGM.",
...
async function auditVerbsBatch(options) {
  const { verbs, model = DEFAULT_MODEL, stats = null, batchLabel = "", auditType = "verb_form_audit" } = options;
  ...
  const input = [
    "Full EN-DE VERBS linguistic audit. Return JSON items array. PASS per form slot; findings only for real issues.",
