#!/usr/bin/env node
/**
 * Generate EN-DE B2 OWNER repair Group 4 report + regression summary.
 */
const fs = require("fs");
...
const REPAIRS = path.join(__dirname, "en-b2-owner-repair-group-04-repairs.json");
const APPLY_LOG = path.join(__dirname, "en-b2-owner-repair-group-04-apply-log.json");
const OUT_JSON = path.join(__dirname, "en-b2-owner-repair-group-04.json");
const OUT_MD = path.join(ROOT, "reports", "en-b2-owner-repair-group-04.md");
...
function main() {
  ...
  const lunaRegression = lunaApiAvailable
    ? "NOT RUN"
    : "NOT RUN — API unavailable";
  ...
  const mdLines = [
    "# EN–DE B2 — OWNER-approved repair Group 4 (items 151–200)",
    "",
    "**Date:** " + new Date().toISOString().slice(0, 10),
    "**Mode:** OWNER-approved exact apply (Group 4/10)",
    "",
    "## Group 4",
    "",
    "| Metric | Value |",
    ...
    "## Artefacts",
    "",
    "- reports/en-b2-owner-repair-group-04.md",
    "- reports/temp/en-b2-owner-repair-group-04.json",
    "- reports/temp/en-b2-owner-repair-group-04-repairs.json",
    "- reports/temp/en-b2-owner-repair-group-04-apply-log.json",
    "",
