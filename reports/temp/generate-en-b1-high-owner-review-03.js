#!/usr/bin/env node
/**
 * Generate EN-DE B1 HIGH owner review block #3 (25 unique cards).
...
function isExcluded(cardId, excludedSets) {
  ...
  for (const set of excludedSets) {
    if (set.has(k) || set.has(cardKey(cardId))) return true;
  }
  return false;
}
...
function main() {
  ...
  const high1Ids = loadPriorBlockCards(
    "reports/temp/en-b1-high-owner-review-01.json",
    "HIGH #1"
  );
  const high2Ids = loadPriorBlockCards(
    "reports/temp/en-b1-high-owner-review-02.json",
    "HIGH #2"
  );
  const high1Set = new Set(high1Ids.map(cardKey));
  const high2Set = new Set(high2Ids.map(cardKey));
  const excludedSets = [criticalSet, high1Set, high2Set];
  ...
  if (overlapHigh1.length || overlapHigh2.length || overlapCritical.length) {
    console.error("Overlap detected:", { overlapHigh1, overlapHigh2, overlapCritical });
    process.exit(1);
  }
  ...
  const expectedArithmeticRemaining = 423 - BLOCK_SIZE;
  const remainingDiscrepancy =
    remainingHighCards !== expectedArithmeticRemaining
      ? {
          calculated: remainingHighCards,
          expectedArithmetic: expectedArithmeticRemaining,
          priorBlockRemaining: 423,
          note: "Mismatch vs 423−25 arithmetic; investigate workflow state.",
        }
      : null;
  ...
  const mdParts = [
    ...
    ...high1Ids.map((c) => `- ${c}`),
    "",
    "### HIGH #2 cards excluded (from `reports/temp/en-b1-high-owner-review-02.json`)",
    "",
    ...high2Ids.map((c) => `- ${c}`),
    "",
    "---",
  ...
  mdParts.push("```text");
  mdParts.push("EN–DE B1 HIGH OWNER REVIEW #3");
  mdParts.push("");
  ...
  if (remainingDiscrepancy) {
    mdParts.push(`Remaining count discrepancy: calculated ${remainingDiscrepancy.calculated}, expected ${remainingDiscrepancy.expectedArithmetic}`);
  }
  mdParts.push("```");
  ...
  const output = {
    ...
    priorBlockRemainingReference: 423,
  ...
  fs.writeFileSync(path.join(ROOT, "reports/en-b1-high-owner-review-03.md"), mdParts.join("\n"));
  fs.writeFileSync(
    path.join(ROOT, "reports/temp/en-b1-high-owner-review-03.json"),
