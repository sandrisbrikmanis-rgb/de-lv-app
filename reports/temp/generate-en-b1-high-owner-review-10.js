#!/usr/bin/env node
/**
 * Generate EN-DE B1 HIGH owner review block #10 (50 unique cards).
...
const HIGH9_GHOST_DE_LEMMAS = ["Tagesordnung"];
...
function verifyResolvedExclusions(excludedSet, selectedCardIds) {
  ...
  if (selectedCardIds.some((id) => normalizeCardId(id) === genNorm)) {
    throw new Error("b1-Gen-1055 in HIGH #10 selection");
  }
  ...
  if (selectedCardIds.some((id) => normalizeCardId(id) === kruNorm)) {
    throw new Error("b1-Krüppel-1651 in HIGH #10 selection");
  }
...
function main() {
  ...
  console.log(`Duplicate logical cards across HIGH #1–#9: ${integrity.duplicateCount}`);
  ...
  const high9Json = JSON.parse(
    fs.readFileSync(path.join(ROOT, "reports/temp/en-b1-high-owner-review-09.json"), "utf8")
  );
  const workflowRemainingBeforeHigh10 =
    high9Json.workflowUnresolvedHighCardsAfterHigh9 ?? high9Json.remainingAfterHigh9 ?? 173;
  const auditPoolAfterHigh9 =
    high9Json.auditSelectionPoolAfterHigh9 ??
    high9Json.remainingAfterSelectionPool ??
    workflowRemainingBeforeHigh10;
  ...
  const auditPoolAfterHigh10 = auditSelectionPool.size - selectedCardIds.length;
  const workflowRemainingAfterHigh10 = workflowRemainingBeforeHigh10 - BLOCK_SIZE;
  const expectedArithmeticRemaining = workflowRemainingBeforeHigh10 - BLOCK_SIZE;
  const expectedReferenceArithmetic = 173 - BLOCK_SIZE;

  if (workflowRemainingAfterHigh10 !== expectedReferenceArithmetic) {
    console.error("WORKFLOW REMAINING DISCREPANCY — STOP", {
      workflowRemainingAfter: workflowRemainingAfterHigh10,
      expectedReference: expectedReferenceArithmetic,
      workflowRemainingBefore: workflowRemainingBeforeHigh10,
    });
    process.exit(1);
  }
  ...
  mdParts.push(`Workflow unresolved HIGH cards before HIGH #${BLOCK}: ${workflowRemainingBeforeHigh10}`);
  mdParts.push(`HIGH #${BLOCK} selected: ${BLOCK_SIZE}`);
  mdParts.push(`Workflow unresolved HIGH cards after HIGH #${BLOCK}: ${workflowRemainingAfterHigh10}`);
  mdParts.push(`Audit-selection pool before HIGH #${BLOCK}: ${auditSelectionPool.size}`);
  mdParts.push(`Audit-selection pool after HIGH #${BLOCK}: ${auditPoolAfterHigh10}`);
  mdParts.push(`Expected arithmetic: 173 - 50 = ${expectedReferenceArithmetic}`);
  mdParts.push(`Calculated workflow remaining: ${workflowRemainingAfterHigh10}`);
  ...
  const output = {
    ...
    exclusionVerification: {
      ...
      totalPreviousHighSelections: 275,
      high9TagungResolutionExclusion: "PASS",
      tageordnungGhostAuditExclusion: "PASS",
      b1Gen1055ResolvedExclusion: "PASS",
      b1Kruppel1651ResolvedExclusion: "PASS",
      high4BiegenNormalizedExclusion: "PASS",
      duplicateLogicalCardsAcrossHigh1To9: 0,
      duplicateLogicalCardsInsideHigh10: 0,
    },
    qualityGate: {
    ...
    workflowUnresolvedHighCardsBeforeHigh10: workflowRemainingBeforeHigh10,
    high10Selected: BLOCK_SIZE,
    workflowUnresolvedHighCardsAfterHigh10: workflowRemainingAfterHigh10,
    auditSelectionPoolBeforeHigh10: auditSelectionPool.size,
    auditSelectionPoolAfterHigh10: auditPoolAfterHigh10,
    auditPoolAfterHigh9Reference: auditPoolAfterHigh9,
    expectedArithmeticRemaining: expectedReferenceArithmetic,
    calculatedWorkflowRemaining: workflowRemainingAfterHigh10,
    ...
    JSON.stringify(
      {
        ...
        duplicateLinks,
        metadataAnomalies,
        workflowBefore: workflowRemainingBeforeHigh10,
        workflowAfter: workflowRemainingAfterHigh10,
        auditPoolBefore: auditSelectionPool.size,
        auditPoolAfter: auditPoolAfterHigh10,
