#!/usr/bin/env node
/**
 * Generate EN-DE B1 HIGH owner review block #7 (25 unique cards).
...
/** HIGH #4 biegen audit artifact (soft hyphen U+00AD) — must not reappear in HIGH #7. */
...
function verifyExclusionIntegrity(blockIds, criticalSet) {
  ...
  if (dup.length) {
    throw new Error(`Duplicate logical cards across HIGH #1–#6: ${[...new Set(dup)].join(", ")}`);
  }
...
function verifyResolvedExclusions(excludedSet, selectedCardIds) {
  ...
  if (genInSelection) {
    throw new Error("b1-Gen-1055 resolved exclusion: FAIL — present in HIGH #7 selection");
  }

  const biegenInSelection = selectedCardIds.some((id) => normalizeCardId(id) === HIGH4_BIEGEN_NORMALIZED);
  if (biegenInSelection) {
    throw new Error("HIGH #4 biegen normalized exclusion: FAIL — biegen in HIGH #7 selection");
  }
...
function main() {
  ...
  const allPriorIds = [...high1Ids, ...high2Ids, ...high3Ids, ...high4Ids, ...high5Ids, ...high6Ids];
  console.log(`Total previous HIGH selections: ${allPriorIds.length}`);

  const integrity = verifyExclusionIntegrity(
    [high1Ids, high2Ids, high3Ids, high4Ids, high5Ids, high6Ids],
    criticalSet
  );
  console.log(`Duplicate logical cards across HIGH #1–#6: ${integrity.duplicateCount}`);
  ...
  const high6Json = JSON.parse(
    fs.readFileSync(path.join(ROOT, "reports/temp/en-b1-high-owner-review-06.json"), "utf8")
  );
  const workflowRemainingBeforeHigh7 =
    high6Json.workflowUnresolvedHighCardsAfterHigh6 ?? high6Json.remainingAfterHigh6 ?? 298;
  const auditPoolAfterHigh6 =
    high6Json.auditSelectionPoolAfterHigh6 ?? high6Json.remainingAfterSelectionPool ?? workflowRemainingBeforeHigh7;
  ...
  if (internalDup.length) {
    console.error("Duplicate logical cards inside HIGH #7:", internalDup);
    process.exit(1);
  }
  ...
  const auditPoolAfterHigh7 = auditSelectionPool.size - selectedCardIds.length;
  const workflowRemainingAfterHigh7 = workflowRemainingBeforeHigh7 - BLOCK_SIZE;
  const expectedArithmeticRemaining = workflowRemainingBeforeHigh7 - BLOCK_SIZE;
  const expectedReferenceArithmetic = 298 - BLOCK_SIZE;

  const remainingDiscrepancy =
    workflowRemainingAfterHigh7 !== expectedArithmeticRemaining
      ? {
          workflowRemainingAfter: workflowRemainingAfterHigh7,
          expectedArithmetic: expectedArithmeticRemaining,
          expectedReference298Minus25: expectedReferenceArithmetic,
          workflowRemainingBefore: workflowRemainingBeforeHigh7,
          auditPoolBefore: auditSelectionPool.size,
          auditPoolAfter: auditPoolAfterHigh7,
        }
      : workflowRemainingAfterHigh7 !== expectedReferenceArithmetic
        ? {
            workflowRemainingAfter: workflowRemainingAfterHigh7,
            expectedReference298Minus25: expectedReferenceArithmetic,
            note: "Calculated matches prior block arithmetic but differs from 298-25 reference",
          }
        : null;

  if (remainingDiscrepancy && workflowRemainingAfterHigh7 !== expectedReferenceArithmetic) {
  ...
  const discrepancyText =
    workflowRemainingAfterHigh7 === expectedReferenceArithmetic ? "none" : "see remainingDiscrepancy in JSON";

  mdParts.push("---", "", "## Coverage summary", "");
  mdParts.push("```text");
  mdParts.push("EN–DE B1 HIGH OWNER REVIEW #7");
  mdParts.push("");
  ...
  mdParts.push(`Workflow unresolved HIGH cards before HIGH #7: ${workflowRemainingBeforeHigh7}`);
  mdParts.push(`HIGH #7 selected: ${BLOCK_SIZE}`);
  mdParts.push(`Workflow unresolved HIGH cards after HIGH #7: ${workflowRemainingAfterHigh7}`);
  mdParts.push(`Audit-selection pool before HIGH #7: ${auditSelectionPool.size}`);
  mdParts.push(`Audit-selection pool after HIGH #7: ${auditPoolAfterHigh7}`);
  mdParts.push(`Expected arithmetic: 298 - 25 = ${expectedReferenceArithmetic}`);
  mdParts.push(`Calculated workflow remaining: ${workflowRemainingAfterHigh7}`);
  ...
  const output = {
    ...
    exclusionVerification: {
      ...
      totalPreviousHighSelections: 150,
      ...
      high4BiegenNormalizedExclusion: "PASS",
      duplicateLogicalCardsAcrossHigh1To6: 0,
      duplicateLogicalCardsInsideHigh7: 0,
    ...
    workflowUnresolvedHighCardsBeforeHigh7: workflowRemainingBeforeHigh7,
    high7Selected: BLOCK_SIZE,
    workflowUnresolvedHighCardsAfterHigh7: workflowRemainingAfterHigh7,
    auditSelectionPoolBeforeHigh7: auditSelectionPool.size,
    auditSelectionPoolAfterHigh7: auditPoolAfterHigh7,
    auditPoolAfterHigh6Reference: auditPoolAfterHigh6,
    expectedArithmeticRemaining: expectedReferenceArithmetic,
    calculatedWorkflowRemaining: workflowRemainingAfterHigh7,
    remainingDiscrepancy:
      workflowRemainingAfterHigh7 === expectedReferenceArithmetic ? null : remainingDiscrepancy,
  ...
  fs.writeFileSync(path.join(ROOT, "reports/en-b1-high-owner-review-07.md"), mdParts.join("\n"));
  fs.writeFileSync(
    path.join(ROOT, "reports/temp/en-b1-high-owner-review-07.json"),
    ...
    JSON.stringify(
      {
        ...
        duplicateLinks,
        metadataAnomalies,
        workflowBefore: workflowRemainingBeforeHigh7,
        workflowAfter: workflowRemainingAfterHigh7,
        auditPoolBefore: auditSelectionPool.size,
        auditPoolAfter: auditPoolAfterHigh7,
