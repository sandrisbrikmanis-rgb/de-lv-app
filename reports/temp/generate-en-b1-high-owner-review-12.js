#!/usr/bin/env node
/**
 * Generate EN-DE B1 HIGH owner review block #12 (50 unique cards).
...
function formatFindingBlock(f, cardNum, findingIdx, meta, duplicateNote, metadataAnomaly) {
  const lines = [
...
function main() {
  ...
  const integrity = verifyExclusionIntegrity(
    allPriorBlocks,
    criticalSet,
    [25, 25, 25, 25, 25, 25, 25, 50, 50, 50, 50]
  );
  console.log(`Duplicate logical cards across HIGH #1–#11: ${integrity.duplicateCount}`);
  ...
  const high11Json = JSON.parse(
    fs.readFileSync(path.join(ROOT, "reports/temp/en-b1-high-owner-review-11.json"), "utf8")
  );
  const workflowRemainingBeforeHigh12 =
    high11Json.workflowUnresolvedHighCardsAfterHigh11 ?? high11Json.remainingAfterHigh11 ?? 73;
  const auditPoolAfterHigh11 =
    high11Json.auditSelectionPoolAfterHigh11 ??
    high11Json.remainingAfterSelectionPool ??
    98;
  ...
  const auditPoolAfterHigh12 = auditSelectionPool.size - selectedCardIds.length;
  const workflowRemainingAfterHigh12 = workflowRemainingBeforeHigh12 - BLOCK_SIZE;
  const expectedArithmeticRemaining = workflowRemainingBeforeHigh12 - BLOCK_SIZE;
  const expectedReferenceArithmetic = 73 - BLOCK_SIZE;
  const expectedAuditPoolAfter = auditPoolAfterHigh11 - BLOCK_SIZE;

  if (workflowRemainingAfterHigh12 !== expectedReferenceArithmetic) {
    console.error("WORKFLOW REMAINING DISCREPANCY — STOP", {
      workflowRemainingAfter: workflowRemainingAfterHigh12,
      expectedReference: expectedReferenceArithmetic,
      workflowRemainingBefore: workflowRemainingBeforeHigh12,
    });
    process.exit(1);
  }
  if (auditPoolAfterHigh12 !== expectedAuditPoolAfter) {
    console.error("AUDIT POOL DISCREPANCY — STOP", {
      auditPoolAfter: auditPoolAfterHigh12,
      expectedAuditPoolAfter,
      auditPoolBefore: auditSelectionPool.size,
      auditPoolAfterHigh11Ref: auditPoolAfterHigh11,
    });
    process.exit(1);
  }
  ...
  const mdParts = [
    ...
    `Unique cards selected: ${BLOCK_SIZE}/${BLOCK_SIZE} (assigned after selection)`,
    "",
  ...
  if (headerIdx >= 0) {
    mdParts[headerIdx] = `Unique cards selected: ${selectedCardIds.length}/${BLOCK_SIZE}`;
    mdParts.splice(
      headerIdx + 1,
      0,
      `First Card ID: ${selectedCardIds[0]}`,
      `Last Card ID: ${selectedCardIds[BLOCK_SIZE - 1]}`,
  ...
  mdParts.push("");
  mdParts.push("Previous HIGH selections loaded: 375");
  ...
  mdParts.push(`Workflow unresolved HIGH cards before HIGH #${BLOCK}: ${workflowRemainingBeforeHigh12}`);
  mdParts.push(`HIGH #${BLOCK} selected: ${BLOCK_SIZE}`);
  mdParts.push(`Workflow unresolved HIGH cards after HIGH #${BLOCK}: ${workflowRemainingAfterHigh12}`);
  mdParts.push(`Audit-selection pool before HIGH #${BLOCK}: ${auditSelectionPool.size}`);
  mdParts.push(`Audit-selection pool after HIGH #${BLOCK}: ${auditPoolAfterHigh12}`);
  mdParts.push(`Expected arithmetic: 73 - 50 = ${expectedReferenceArithmetic}`);
  mdParts.push(`Calculated workflow remaining: ${workflowRemainingAfterHigh12}`);
  ...
  const output = {
    ...
    exclusionVerification: {
      ...
      totalPreviousHighSelections: 375,
      high9TagungResolutionExclusion: "PASS",
      tageordnungGhostAuditExclusion: "PASS",
      ...
      high4BiegenNormalizedExclusion: "PASS",
      duplicateLogicalCardsAcrossHigh1To11: 0,
      duplicateLogicalCardsInsideHigh12: 0,
    },
    qualityGate: {
    ...
    workflowUnresolvedHighCardsBeforeHigh12: workflowRemainingBeforeHigh12,
    high12Selected: BLOCK_SIZE,
    workflowUnresolvedHighCardsAfterHigh12: workflowRemainingAfterHigh12,
    auditSelectionPoolBeforeHigh12: auditSelectionPool.size,
    auditSelectionPoolAfterHigh12: auditPoolAfterHigh12,
    auditPoolAfterHigh11Reference: auditPoolAfterHigh11,
    expectedArithmeticRemaining: expectedReferenceArithmetic,
    calculatedWorkflowRemaining: workflowRemainingAfterHigh12,
    ...
    JSON.stringify(
      {
        ...
        duplicateLinks,
        metadataAnomalies,
        workflowBefore: workflowRemainingBeforeHigh12,
        workflowAfter: workflowRemainingAfterHigh12,
        auditPoolBefore: auditSelectionPool.size,
        auditPoolAfter: auditPoolAfterHigh12,
