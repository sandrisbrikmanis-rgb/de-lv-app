# ET–DE A1 — OWNER VIEW (grupa 54, 2651–2700)

**Standard:** `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9
**Auditors:** deterministika + GPT-5.6 Luna (READ-ONLY)
**Audit PR:** [#621](https://github.com/sandrisbrikmanis-rgb/de-lv-app/pull/621)

| Navigācija | Saite |
|------------|-------|
| GitHub indekss | [et-b1-owner-review-GITHUB.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-review-GITHUB.md) |
| OWNER VIEW (visi) | [et-b1-owner-view.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-view.md) |
| Decisions (šī grupa) | [et-b1-owner-decisions-group54.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions-group54.md) |
| Decisions (viss) | [et-b1-owner-decisions.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions.md) |

Avots: [et-b1-full-audit.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-full-audit.md)

## ET-B1-4536
**Audit ID:** ET-B1-4536
**Card ID:** `b1-schicht`
**Field/path:** `study.comparison[1].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Schicht
**LV MASTER reference:** situācija vai novietojums
**CURRENT:** olukord või asend
**PROPOSED_ET (audit ieteikums):** ühiskonnakiht
**Problēma:** Olukord või asend translates Lage, not Schicht. A valid additional sense of Schicht is a social stratum.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4537
**Audit ID:** ET-B1-4537
**Card ID:** `b1-schleifen`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** schleifen
**LV MASTER reference:** kleita velkas pa grīdu.
**CURRENT:** kleit vedeleb mööda põrandat.
**PROPOSED_ET (audit ieteikums):** kleit lohiseb mööda põrandat.
**Problēma:** Vedelema means to lie around; it does not express dragging or trailing along the floor.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4538
**Audit ID:** ET-B1-4538
**Card ID:** `b1-schleifen`
**Field/path:** `study.comparison[0].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** schleifen
**LV MASTER reference:** slīpēt, asināt • vilkties pa zemi
**CURRENT:** lihvima, teritama • mööda maad vedelema
**PROPOSED_ET (audit ieteikums):** lihvima, teritama • mööda maad lohisema
**Problēma:** Vedelema means lying around, whereas schleifen in this sense means to drag or trail along the ground.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4539
**Audit ID:** ET-B1-4539
**Card ID:** `b1-schleifen`
**Field/path:** `study.comparison[2].meaning`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** SEMANTICS
**DE (read-only):** schleifen
**LV MASTER reference:** vilkt
**CURRENT:** vedama
**PROPOSED_ET (audit ieteikums):** lohistama
**Problēma:** Vedama is a passive participle form and is not the infinitive required here; lohistama precisely means to drag.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4540
**Audit ID:** ET-B1-4540
**Card ID:** `b1-schmelzen`
**Field/path:** `study.comparison[2].meaning`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** schmelzen
**LV MASTER reference:** vārīt
**CURRENT:** keetma
**PROPOSED_ET (audit ieteikums):** sulatama
**Problēma:** Keetma means to boil, not to melt. The relevant transitive sense of schmelzen is sulatama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4541
**Audit ID:** ET-B1-4541
**Card ID:** `b1-schuldig`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** schuldig
**LV MASTER reference:** viņa jūtas vainīga.
**CURRENT:** ta tunneb end süüdi olevat.
**PROPOSED_ET (audit ieteikums):** ta tunneb end süüdi.
**Problēma:** The current sentence is grammatical but unnecessarily cumbersome; Estonian naturally says tundma end süüdi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4542
**Audit ID:** ET-B1-4542
**Card ID:** `b1-schwanken`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** schwanken
**LV MASTER reference:** viņš šaubās starp divām iespējām.
**CURRENT:** ta kahtleb kahe võimaluse vahel.
**PROPOSED_ET (audit ieteikums):** ta kõhkleb kahe võimaluse vahel.
**Problēma:** Valikute vahel schwanken tähendab eesti keeles pigem kõhklema, mitte kahtlema.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4543
**Audit ID:** ET-B1-4543
**Card ID:** `b1-sowie`
**Field/path:** `study.translation`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** sowie
**LV MASTER reference:** kā arī
**CURRENT:** samuti kui
**PROPOSED_ET (audit ieteikums):** samuti, nagu ka; ning
**Problēma:** Sowie tähendab tavaliselt „samuti, nagu ka” või „ning”; „samuti kui” tähendab pigem „just nagu”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4544
**Audit ID:** ET-B1-4544
**Card ID:** `b1-sowie`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** sowie
**LV MASTER reference:** tiklīdz man būs laiks, es tev piezvanīšu.
**CURRENT:** niipea kui mul on aega, helistan sulle.
**PROPOSED_ET (audit ieteikums):** Ma helistan sulle ning saadan sulle ka sõnumi.
**Problēma:** Näide tähendab „niipea kui” ehk sobald, mitte sowie („ning, samuti”).
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4545
**Audit ID:** ET-B1-4545
**Card ID:** `b1-spannung`
**Field/path:** `study.comparison[0].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Spannung
**LV MASTER reference:** spriedze • elektrībā spriegums
**CURRENT:** pinge • elektris pinge
**PROPOSED_ET (audit ieteikums):** pinge • elektriline pinge
**Problēma:** „Elektris pinge” on ebaloomulik; elektrilise tähenduse täpne vaste on „elektriline pinge”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4546
**Audit ID:** ET-B1-4546
**Card ID:** `b1-spannung`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Spannung
**LV MASTER reference:** Stāstā vai attiecībās spriedze; elektrībā spriegums.
**CURRENT:** Loos või suhetes pinge; elektris pinge (voltides).
**PROPOSED_ET (audit ieteikums):** Loos või suhetes on pinge; elektriline pinge on voltides.
**Problēma:** Algne teine osa „elektris pinge” on grammatiliselt ja kollokatsiooniliselt vigane.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4547
**Audit ID:** ET-B1-4547
**Card ID:** `b1-spitze`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Spitze
**LV MASTER reference:** viņa ir uzņēmuma vadībā.
**CURRENT:** ta on ettevõtte juhtkonnas.
**PROPOSED_ET (audit ieteikums):** ta on ettevõtte eesotsas.
**Problēma:** „An der Spitze” tähendab ettevõtte eesotsas olemist; „juhtkonnas” võib tähendada mis tahes juhtkonnaliiget.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4548
**Audit ID:** ET-B1-4548
**Card ID:** `b1-spitze`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Spitze
**LV MASTER reference:** an der Spitze nozīmē vadībā, nevis tikai fiziski augšā.
**CURRENT:** an der Spitze tähendab juhtkonnas, mitte ainult füüsiliselt üleval.
**PROPOSED_ET (audit ieteikums):** an der Spitze tähendab eesotsas olemist, mitte ainult füüsiliselt üleval olemist.
**Problēma:** „Juhtkonnas” on „an der Spitze” tähenduse jaoks liiga üldine ja ebatäpne.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4549
**Audit ID:** ET-B1-4549
**Card ID:** `b1-stift`
**Field/path:** `study.comparison[2].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Stift
**LV MASTER reference:** lodīšu pildspalva
**CURRENT:** kuulpeen
**PROPOSED_ET (audit ieteikums):** pastapliiats
**Problēma:** „Kuulpeen” ei ole korrektne ega loomulik eesti vaste kuuliga pastapliiatsile.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4550
**Audit ID:** ET-B1-4550
**Card ID:** `b1-stoßen`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** stoßen
**LV MASTER reference:** mēs uzdūrāmies problēmai.
**CURRENT:** me sattusime probleemile otsa.
**PROPOSED_ET (audit ieteikums):** me puutusime kokku probleemiga.
**Problēma:** Probleemiga kohtumise tähenduses on „probleemiga kokku puutuma” loomulikum ja korrektsem kollokatsioon.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4551
**Audit ID:** ET-B1-4551
**Card ID:** `b1-stoßen`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** stoßen
**LV MASTER reference:** auf etwas stoßen nozīmē uzdurties kaut kam, nevis vienkārši grūst.
**CURRENT:** auf etwas stoßen tähendab millelegi otsa sattumist, mitte lihtsalt tõukamist.
**PROPOSED_ET (audit ieteikums):** auf etwas stoßen tähendab millegagi kokku puutumist, mitte lihtsalt tõukamist.
**Problēma:** „Millegagi kokku puutuma” on tähenduses „auf etwas stoßen” loomulikum eesti väljend.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4552
**Audit ID:** ET-B1-4552
**Card ID:** `b1-strom`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Strom
**LV MASTER reference:** lielā upe ietek jūrā.
**CURRENT:** suur vool suubub merre.
**PROPOSED_ET (audit ieteikums):** suur jõgi suubub merre.
**Problēma:** Siin tähendab saksa Strom suurt jõge; „vool” tähistab eesti keeles vee liikumist, mitte jõge.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4553
**Audit ID:** ET-B1-4553
**Card ID:** `b1-stürzen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** stürzen
**LV MASTER reference:** vētra nogāza vairākus kokus.
**CURRENT:** torm murdis mitu puud.
**PROPOSED_ET (audit ieteikums):** torm kukutas mitu puud.
**Problēma:** „Murdis” tähendab puude katki murdmist; siin on tähendus, et torm paiskas puud ümber.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4554
**Audit ID:** ET-B1-4554
**Card ID:** `b1-szene`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** Szene
**LV MASTER reference:** keine Szene machen nozīmē netaisīt skandālu.
**CURRENT:** keine Szene machen tähendab skandaali mitte tegemist.
**PROPOSED_ET (audit ieteikums):** keine Szene machen tähendab, et ei tohi skandaali teha.
**Problēma:** Praegune nominalisatsioon „skandaali mitte tegemist” on eesti keeles kohmakas ja ebaloomulik.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4555
**Audit ID:** ET-B1-4555
**Card ID:** `b1-tank`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Tank
**LV MASTER reference:** Auto un iekārtās Tank = tvertne; militāri parasti Panzer.
**CURRENT:** Autos ja seadmetes Tank = paak; sõjaväeliselt tavaliselt Panzer.
**PROPOSED_ET (audit ieteikums):** Autodes ja seadmetes Tank = paak; sõjaväes tavaliselt Panzer.
**Problēma:** „Autos” on siin ainsuse seesütlev; paralleelse üldistuse puhul peab olema mitmuse vorm „autodes”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4556
**Audit ID:** ET-B1-4556
**Card ID:** `b1-tank`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Tank
**LV MASTER reference:** Latviskais tanks vāciski parasti ir der Panzer, nevis der Tank.
**CURRENT:** Eestikeelne tank on saksa keeles tavaliselt der Panzer, mitte der Tank.
**PROPOSED_ET (audit ieteikums):** Eestikeelne sõna tank on saksa keeles tavaliselt der Panzer, mitte der Tank.
**Problēma:** „Eestikeelne tank” tähendab sõna otseses mõttes Eesti tanki, mitte eestikeelset sõna.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4557
**Audit ID:** ET-B1-4557
**Card ID:** `b1-taufen`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** taufen
**LV MASTER reference:** taufen nav derēt; tas nozīmē kristīt vai dot vārdu.
**CURRENT:** taufen ei ole derēt; see tähendab ristimist või nime andmist.
**PROPOSED_ET (audit ieteikums):** taufen ei tähenda sobima; see tähendab ristimist või nime andmist.
**Problēma:** Vorm „ei ole sobima” on eesti keeles grammatiliselt väär; tähenduse eitamisel kasutatakse „ei tähenda”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4558
**Audit ID:** ET-B1-4558
**Card ID:** `b1-ton`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** CRITICAL
**Category:** FOREIGN_REMNANT
**DE (read-only):** Ton
**LV MASTER reference:** in einem Ton runājot nozīmē runas manieri, nevis tikai skaņas augstumu.
**CURRENT:** in einem Ton rääkides tähendab kõnetooni, mitte ainult heli kõrgust.
**PROPOSED_ET (audit ieteikums):** in einem Ton rääkides tähendab kõnetooni, mitte ainult heli kõrgust.
**Problēma:** Eestikeelses väljas on alles läti sõna „runājot”; see tuleb asendada eestikeelse sõnaga „rääkides”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4559
**Audit ID:** ET-B1-4559
**Card ID:** `b1-trauen`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** trauen
**LV MASTER reference:** Kādam uzticies: trauen; pats uzdrošinies: sich trauen.
**CURRENT:** Kellelegi usalda: trauen; ise julge: sich trauen.
**PROPOSED_ET (audit ieteikums):** Kedagi usaldades: trauen; ise julgedes: sich trauen.
**Problēma:** „Kellelegi usalda” ja „ise julge” on siin ebakorrektsed ning moodustavad ebaloomuliku juhise.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4560
**Audit ID:** ET-B1-4560
**Card ID:** `b1-übergeben`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** ORTHOGRAPHY
**DE (read-only):** übergeben
**LV MASTER reference:** Bez sich tas ir nodot; ar sich tas ir vemt.
**CURRENT:** Ilma sich-ta on see üleandmine; sich-ga on see oksendamine.
**PROPOSED_ET (audit ieteikums):** Ilma sich-ta on see üleandmine; sich-iga on see oksendamine.
**Problēma:** Võõrkeelse sõna käändelõpp peab olema -iga, mitte -ga.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4561
**Audit ID:** ET-B1-4561
**Card ID:** `b1-überholen`
**Field/path:** `study.translation`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** überholen
**LV MASTER reference:** apdzīt
**CURRENT:** mööduma
**PROPOSED_ET (audit ieteikums):** mööda sõitma
**Problēma:** Liiklusmõistes tähendab überholen eeskätt möödasõitu; mööduma on siin liiga üldine ja intransitiivne.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4562
**Audit ID:** ET-B1-4562
**Card ID:** `b1-überholen`
**Field/path:** `study.examples[0].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** überholen
**LV MASTER reference:** kravas auto apdzen automašīnu.
**CURRENT:** veoauto möödub autost.
**PROPOSED_ET (audit ieteikums):** veoauto sõidab autost mööda.
**Problēma:** Eesti keeles väljendatakse sõiduki möödasõitu loomulikumalt ühendverbiga sõitma mööda.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4563
**Audit ID:** ET-B1-4563
**Card ID:** `b1-überholen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** überholen
**LV MASTER reference:** uz šī ceļa nedrīkst apdzīt.
**CURRENT:** sellel teel ei tohi mööduda.
**PROPOSED_ET (audit ieteikums):** sellel teel ei tohi mööda sõita.
**Problēma:** Liikluse kontekstis on õige vaste mööda sõitma; mööduma ei täpsusta möödasõitu.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4564
**Audit ID:** ET-B1-4564
**Card ID:** `b1-überholen`
**Field/path:** `study.comparison[0].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** überholen
**LV MASTER reference:** apdzīt • kapitāli remontēt
**CURRENT:** mööduma • kapitaalremonti tegema
**PROPOSED_ET (audit ieteikums):** mööda sõitma • kapitaalremonti tegema
**Problēma:** Esimene tähendus peab kajastama liikluses kasutatavat möödasõitu, mitte üldist möödumist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4565
**Audit ID:** ET-B1-4565
**Card ID:** `b1-überholen`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** überholen
**LV MASTER reference:** Ceļā pārspēj citu auto: überholen.
**CURRENT:** Teel möödud teisest autost: überholen.
**PROPOSED_ET (audit ieteikums):** Teel sõidad teisest autost mööda: überholen.
**Problēma:** Teise auto möödasõidu kohta on loomulikum ja täpsem sõnastus sõidad teisest autost mööda.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4566
**Audit ID:** ET-B1-4566
**Card ID:** `b1-überholen`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** überholen
**LV MASTER reference:** Einen Motor überholen nozīmē kapitāli remontēt, nevis apdzīt motoru.
**CURRENT:** Einen Motor überholen tähendab kapitaalremonti, mitte mootorist möödumist.
**PROPOSED_ET (audit ieteikums):** Einen Motor überholen tähendab kapitaalremonti, mitte mootorist mööda sõitmist.
**Problēma:** Mootorist möödumine kõlab sõidukontekstis ebaloomulikult; siin on parem vastandada mööda sõitmist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4567
**Audit ID:** ET-B1-4567
**Card ID:** `b1-übersehen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** übersehen
**LV MASTER reference:** no šejienes var pārredzēt visu pilsētu.
**CURRENT:** siit saab kogu linna üle vaadata.
**PROPOSED_ET (audit ieteikums):** siit on näha kogu linn.
**Problēma:** Üle vaatama tähendab eesti keeles pigem kontrollima või läbi vaatama, mitte kaugelt kogu linna nägema.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4568
**Audit ID:** ET-B1-4568
**Card ID:** `b1-unterhalten`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** ORTHOGRAPHY
**DE (read-only):** unterhalten
**LV MASTER reference:** Bez sich: izklaidēt vai uzturēt; ar sich: sarunāties.
**CURRENT:** Ilma sich-ta: lõbustama või ülal pidama; sich-ga: vestlema.
**PROPOSED_ET (audit ieteikums):** Ilma sich-ta: lõbustama või ülal pidama; sich-iga: vestlema.
**Problēma:** Võõrkeelse sõna käändelõpp peab olema -iga, mitte -ga.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4569
**Audit ID:** ET-B1-4569
**Card ID:** `b1-untersuchung`
**Field/path:** `study.comparison[1].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Untersuchung
**LV MASTER reference:** eksāmens vai formāla pārbaude
**CURRENT:** eksam või ametlik kontroll
**PROPOSED_ET (audit ieteikums):** läbivaatus või ametlik kontroll
**Problēma:** Untersuchung ei tähenda saksa keeles tavaliselt eksamit; selles kontekstis sobib läbivaatus või uurimine.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4570
**Audit ID:** ET-B1-4570
**Card ID:** `b1-verbindung`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Verbindung
**LV MASTER reference:** ir tiešs savienojums uz Berlīni.
**CURRENT:** on otseühendus Berliini.
**PROPOSED_ET (audit ieteikums):** on otseühendus Berliiniga.
**Problēma:** Ühenduse sihtkoha väljendamiseks on siin vaja kaasaütlevat käänet: Berliiniga.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4571
**Audit ID:** ET-B1-4571
**Card ID:** `b1-verfolgen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** verfolgen
**LV MASTER reference:** es katru vakaru sekoju līdzi ziņām.
**CURRENT:** ma jälgin iga õhtu uudiseid.
**PROPOSED_ET (audit ieteikums):** ma jälgin igal õhtul uudiseid.
**Problēma:** Korduva aja väljendamisel on loomulik ja grammatiliselt õige vorm „igal õhtul“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4572
**Audit ID:** ET-B1-4572
**Card ID:** `b1-verfolgen`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** GRAMMAR
**DE (read-only):** verfolgen
**LV MASTER reference:** verfolgen ar cilvēku bieži nozīmē vajāt, nevis mierīgi sekot.
**CURRENT:** verfolgen inimesega tähendab sageli jälitamist, mitte rahulikult jälgimist.
**PROPOSED_ET (audit ieteikums):** inimese puhul tähendab verfolgen sageli jälitamist, mitte rahulikult jälgimist.
**Problēma:** Väljend „verfolgen inimesega“ on eesti keeles ebagrammatiline; sobib „inimese puhul“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4573
**Audit ID:** ET-B1-4573
**Card ID:** `b1-verhältnis`
**Field/path:** `study.comparison[2].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Verhältnis
**LV MASTER reference:** daļa no kopuma
**CURRENT:** osa tervikust
**PROPOSED_ET (audit ieteikums):** suhe tervikuga
**Problēma:** Verhältnis tähendab siin suhet või proportsiooni, mitte tervikust eraldiseisvat osa.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4574
**Audit ID:** ET-B1-4574
**Card ID:** `b1-verletzen`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** verletzen
**LV MASTER reference:** viņš aizstāv skaidru viedokli.
**CURRENT:** ta esindab selget seisukohta.
**PROPOSED_ET (audit ieteikums):** ta kaitseb selget seisukohta.
**Problēma:** Siin tähendab vertreten seisukoha kaitsmist või pooldamist, mitte lihtsalt selle esindamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4575
**Audit ID:** ET-B1-4575
**Card ID:** `b1-verletzen`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** GRAMMAR
**DE (read-only):** verletzen
**LV MASTER reference:** Regeln verletzen nozīmē pārkāpt noteikumus, nevis fiziski tos ievainot.
**CURRENT:** Regeln verletzen tähendab reeglite rikkumist, mitte nende füüsilist vigastamist.
**PROPOSED_ET (audit ieteikums):** Regeln verletzen tähendab reeglite rikkumist, mitte nende füüsilist vigastamist.
**Problēma:** Praegune lause on grammatiliselt korrektne.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4576
**Audit ID:** ET-B1-4576
**Card ID:** `b1-vertreten`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** GRAMMAR
**DE (read-only):** vertreten
**LV MASTER reference:** Kollegen vertreten nozīmē aizvietot kolēģi, nevis tikai viņu pārstāvēt.
**CURRENT:** Kollegen vertreten tähendab kolleegi asendamist, mitte ainult teda esindamist.
**PROPOSED_ET (audit ieteikums):** Kollegen vertreten tähendab kolleegi asendamist, mitte ainult tema esindamist.
**Problēma:** Omastavalise täiendiga nominalisatsiooni järel peab olema „tema esindamist“, mitte „teda esindamist“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4577
**Audit ID:** ET-B1-4577
**Card ID:** `b1-vertreten`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** vertreten
**LV MASTER reference:** viņš aizstāv skaidru viedokli.
**CURRENT:** ta esindab selget seisukohta.
**PROPOSED_ET (audit ieteikums):** ta kaitseb selget seisukohta.
**Problēma:** Seisukoha puhul tähendab vertreten eelkõige selle kaitsmist või pooldamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4578
**Audit ID:** ET-B1-4578
**Card ID:** `b1-vorkommen`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** vorkommen
**LV MASTER reference:** vorkommen ir sadalāms: Das kommt oft vor.
**CURRENT:** vorkommen on lahknev tegusõna: Das kommt oft vor.
**PROPOSED_ET (audit ieteikums):** vorkommen on lahutatav tegusõna: Das kommt oft vor.
**Problēma:** „Lahknev tegusõna” ei ole õige termin; lahutatava eesliitega verbi kohta kasutatakse „lahutatav tegusõna”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4579
**Audit ID:** ET-B1-4579
**Card ID:** `b1-vorziehen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** vorziehen
**LV MASTER reference:** mēs pārceļam termiņu uz trešdienu agrāk.
**CURRENT:** me nihutame tähtaja varasemaks kolmapäevale.
**PROPOSED_ET (audit ieteikums):** me toome tähtaja kolmapäevale.
**Problēma:** „Varasemaks kolmapäevale” on ebaloomulik; tähtaega varasemaks tooma väljendab tähendust selgemalt.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4580
**Audit ID:** ET-B1-4580
**Card ID:** `b1-vorziehen`
**Field/path:** `study.comparison[2].meaning`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** vorziehen
**LV MASTER reference:** pārcelt citā laikā
**CURRENT:** edasi lükkama
**PROPOSED_ET (audit ieteikums):** ettepoole nihutama
**Problēma:** „Vorziehen” tähendab tähtaja või sündmuse varasemaks toomist, mitte edasi lükkamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4581
**Audit ID:** ET-B1-4581
**Card ID:** `b1-wagen`
**Field/path:** `study.comparison[1].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Wagen
**LV MASTER reference:** vagons (vilciens)
**CURRENT:** vagun (rongi)
**PROPOSED_ET (audit ieteikums):** vagun (rong)
**Problēma:** „Rongi” on siin vale käändevorm; sulgudes oleva täpsustusena sobib „rong” või loomulikumalt „rongivagun”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4582
**Audit ID:** ET-B1-4582
**Card ID:** `b1-wenden`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** wenden
**LV MASTER reference:** apgrieziet lapu.
**CURRENT:** pöörake leht.
**PROPOSED_ET (audit ieteikums):** pöörake leht ümber.
**Problēma:** „Wenden” tähendab lehe ümberpööramist; „pöörake leht” jätab selle suuna tähistamata.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4583
**Audit ID:** ET-B1-4583
**Card ID:** `b1-wenden`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** wenden
**LV MASTER reference:** Maini virzienu vai lapas pusi: wenden; meklē palīdzību: sich wenden an.
**CURRENT:** Muuda suunda või lehe külge: wenden; otsi abi: sich wenden an.
**PROPOSED_ET (audit ieteikums):** Muuda suunda või pööra leht ümber: wenden; otsi abi: sich wenden an.
**Problēma:** „Lehe külge” ei ole siin loomulik eestikeelne vaste; mõeldud on lehe ümberpööramist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4584
**Audit ID:** ET-B1-4584
**Card ID:** `b1-werben`
**Field/path:** `study.comparison[2].meaning`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** werben
**LV MASTER reference:** iesniegt pretenziju
**CURRENT:** esitama kaebuse
**PROPOSED_ET (audit ieteikums):** värbama
**Problēma:** „Werben” ei tähenda kaebuse esitamist; sobiv tähendus on näiteks „värbama” või „reklaamima”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4585
**Audit ID:** ET-B1-4585
**Card ID:** `b1-zeugnis`
**Field/path:** `study.comparison[0].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Zeugnis
**LV MASTER reference:** liecība, apliecība, izziņa
**CURRENT:** tunnistus, tõend, väljavõte
**PROPOSED_ET (audit ieteikums):** tunnistus, tõend, õiend
**Problēma:** Väljavõte tähendab extract/avaldist ega ole Zeugnis sobiv vaste selles tähenduste loetelus.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---