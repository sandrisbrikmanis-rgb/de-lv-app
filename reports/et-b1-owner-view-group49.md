# ET–DE A1 — OWNER VIEW (grupa 49, 2401–2450)

**Standard:** `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9
**Auditors:** deterministika + GPT-5.6 Luna (READ-ONLY)
**Audit PR:** [#621](https://github.com/sandrisbrikmanis-rgb/de-lv-app/pull/621)

| Navigācija | Saite |
|------------|-------|
| GitHub indekss | [et-b1-owner-review-GITHUB.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-review-GITHUB.md) |
| OWNER VIEW (visi) | [et-b1-owner-view.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-view.md) |
| Decisions (šī grupa) | [et-b1-owner-decisions-group49.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions-group49.md) |
| Decisions (viss) | [et-b1-owner-decisions.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions.md) |

Avots: [et-b1-full-audit.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-full-audit.md)

## ET-B1-4286
**Audit ID:** ET-B1-4286
**Card ID:** `b1-erfrieren-781`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** erfrieren
**LV MASTER reference:** nosalt
**CURRENT:** külmuma
**PROPOSED_ET (audit ieteikums):** surnuks külmuma
**Problēma:** „Erfrieren” tähendab tavaliselt külmumist surnuks; „külmuma” on liiga üldine ja võib tähendada lihtsalt jäätumist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4287
**Audit ID:** ET-B1-4287
**Card ID:** `b1-Erklärung-795`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Erklärung
**LV MASTER reference:** paziņojums
**CURRENT:** teadaanne
**PROPOSED_ET (audit ieteikums):** selgitus
**Problēma:** „Teadaanne” tähendab teadet või teadaandmist, mitte „Erklärung” põhitähendust „selgitus”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4288
**Audit ID:** ET-B1-4288
**Card ID:** `b1-Ernährung-808`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Ernährung
**LV MASTER reference:** uzturs
**CURRENT:** toit
**PROPOSED_ET (audit ieteikums):** toitumine
**Problēma:** „Ernährung“ tähendab toitumist või toitumisviisi, mitte toitu („toit“).
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4289
**Audit ID:** ET-B1-4289
**Card ID:** `b1-feige-880`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** feige
**LV MASTER reference:** gļēvs
**CURRENT:** argpüks
**PROPOSED_ET (audit ieteikums):** arg
**Problēma:** Saksa omadussõna „feige” tähendab „arg”; „argpüks” on nimisõna „coward”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4290
**Audit ID:** ET-B1-4290
**Card ID:** `b1-fluchen-919`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** fluchen
**LV MASTER reference:** lamāties
**CURRENT:** sõimlema
**PROPOSED_ET (audit ieteikums):** vanduma
**Problēma:** Sõimlema tähendab solvavalt tülitsema; fluchen tähendab vanduma või ropendama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4291
**Audit ID:** ET-B1-4291
**Card ID:** `b1-Fortschritt-939`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Fortschritt
**LV MASTER reference:** progress
**CURRENT:** edu
**PROPOSED_ET (audit ieteikums):** edasiminek
**Problēma:** Edu tähendab pigem edu või kordaminekut; Fortschritt on areng või edasiminek.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4292
**Audit ID:** ET-B1-4292
**Card ID:** `b1-Freie-944`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** das Freie
**LV MASTER reference:** brīva daba
**CURRENT:** vaba loodus
**PROPOSED_ET (audit ieteikums):** vaba õhk
**Problēma:** Das Freie tähendab eeskätt vaba õhku või õues olemist; „vaba loodus” ei ole eesti keeles loomulik vaste.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4293
**Audit ID:** ET-B1-4293
**Card ID:** `b1-Frost-957`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Frost
**LV MASTER reference:** sals
**CURRENT:** külm
**PROPOSED_ET (audit ieteikums):** pakane
**Problēma:** Külm tähendab üldiselt madalat temperatuuri; Frost on täpsemalt pakane või külmumisnähtus.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4294
**Audit ID:** ET-B1-4294
**Card ID:** `b1-frühzeitig-959`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** frühzeitig
**LV MASTER reference:** savlaicīgs
**CURRENT:** õigeaegne
**PROPOSED_ET (audit ieteikums):** varajane
**Problēma:** Õigeaegne tähendab õigel ajal tehtut; frühzeitig tähendab varajast või enneaegset.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4295
**Audit ID:** ET-B1-4295
**Card ID:** `b1-Gasfeuerzeug-983`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Gasfeuerzeug
**LV MASTER reference:** gāzes šķiltavas
**CURRENT:** gaasitulemasin
**PROPOSED_ET (audit ieteikums):** gaasisüütaja
**Problēma:** „Gaasitulemasin“ ei tähenda gaasisüütajat; õige vaste on „gaasisüütaja“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4296
**Audit ID:** ET-B1-4296
**Card ID:** `b1-Gasthaus-990`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Gasthaus
**LV MASTER reference:** viesnīca
**CURRENT:** külalistemaja
**PROPOSED_ET (audit ieteikums):** võõrastemaja
**Problēma:** „Gasthaus“ on võõrastemaja või kõrts, mitte „külalistemaja“, mis tähendab guesthouse’i.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4297
**Audit ID:** ET-B1-4297
**Card ID:** `b1-gedankenlos-1006`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** gedankenlos
**LV MASTER reference:** neapdomīgs
**CURRENT:** hoolimatu
**PROPOSED_ET (audit ieteikums):** mõtlematu
**Problēma:** „Hoolimatu“ tähendab hoolimatut või hooletut; „gedankenlos“ tähendab mõtlematut.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4298
**Audit ID:** ET-B1-4298
**Card ID:** `b1-Gefrierfach-1019`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Gefrierfach
**LV MASTER reference:** saldētava
**CURRENT:** sügavkülmik
**PROPOSED_ET (audit ieteikums):** sügavkülmikuosa
**Problēma:** „Gefrierfach“ on sügavkülmiku lahter või osa; „sügavkülmik“ tähendab tervet seadet.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4299
**Audit ID:** ET-B1-4299
**Card ID:** `b1-glätten-1120`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** glätten
**LV MASTER reference:** nogludināt
**CURRENT:** siledaks silumaks
**PROPOSED_ET (audit ieteikums):** siluma
**Problēma:** Praegune väljend on ebaloomulik ja vormiliselt vigane; „siluma” tähendab saksa verbi „glätten”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4300
**Audit ID:** ET-B1-4300
**Card ID:** `b1-gültig-1162`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** gültig
**LV MASTER reference:** derīgs
**CURRENT:** kõlbulik
**PROPOSED_ET (audit ieteikums):** kehtiv
**Problēma:** „Kehtiv” tähendab gültig; „kõlbulik” tähendab sobiv ja on lisaks ortograafiliselt väär vorm.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4301
**Audit ID:** ET-B1-4301
**Card ID:** `b1-Gummischuh-1163`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Gummischuh
**LV MASTER reference:** galoša
**CURRENT:** kalossid
**PROPOSED_ET (audit ieteikums):** kaloss
**Problēma:** Saksa lähtevõrre on ainsuses; „kalossid” on mitmus.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4302
**Audit ID:** ET-B1-4302
**Card ID:** `b1-gutmachen-1167`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** gutmachen
**LV MASTER reference:** izlabot
**CURRENT:** parandama
**PROPOSED_ET (audit ieteikums):** heastama
**Problēma:** „Gutmachen” tähendab kahju või vea heastamist/kompenseerimist, mitte üldiselt parandamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4303
**Audit ID:** ET-B1-4303
**Card ID:** `b1-halbtags-1182`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** halbtags
**LV MASTER reference:** nepilnu darba dienu
**CURRENT:** osaajaga
**PROPOSED_ET (audit ieteikums):** poole päeva kaupa
**Problēma:** „Osaajaga” tähendab üldiselt osalise tööajaga; „halbtags” täpsustab poole päeva kestust.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4304
**Audit ID:** ET-B1-4304
**Card ID:** `b1-häuslich-1215`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** häuslich
**LV MASTER reference:** mājas
**CURRENT:** kodu-
**PROPOSED_ET (audit ieteikums):** kodune
**Problēma:** „kodu-“ on üksnes liitsõnade eesliide, mitte iseseisev eestikeelne vaste omadussõnale „häuslich“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4305
**Audit ID:** ET-B1-4305
**Card ID:** `b1-hölzern-1304`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** STRUCTURE
**DE (read-only):** hölzern
**LV MASTER reference:** koka
**CURRENT:** puu-
**PROPOSED_ET (audit ieteikums):** puidust
**Problēma:** „Puu-“ on ainult liitsõnaosa, mitte iseseisev vaste saksa omadussõnale „hölzern“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4306
**Audit ID:** ET-B1-4306
**Card ID:** `b1-Hungersnot-1328`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Hungersnot
**LV MASTER reference:** bads
**CURRENT:** nälg
**PROPOSED_ET (audit ieteikums):** näljahäda
**Problēma:** „Hungersnot“ tähendab näljahäda või näljahädast tingitud katastroofi, mitte lihtsalt nälga.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4307
**Audit ID:** ET-B1-4307
**Card ID:** `b1-ihrerseits-1339`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** ihrerseits
**LV MASTER reference:** no viņas puses
**CURRENT:** tema poolt
**PROPOSED_ET (audit ieteikums):** omalt poolt
**Problēma:** „Ihrerseits“ tähendab „omalt poolt“ või „tema poolt vaadatuna“; „tema poolt“ viitab pigem tegija või autorluse tähendusele.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4308
**Audit ID:** ET-B1-4308
**Card ID:** `b1-Inbegriff-1349`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Inbegriff
**LV MASTER reference:** simbols
**CURRENT:** sümbol
**PROPOSED_ET (audit ieteikums):** kehastus
**Problēma:** „Inbegriff“ tähendab millegi kehastust või tüüpilist näidet, mitte üldiselt sümbolit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4309
**Audit ID:** ET-B1-4309
**Card ID:** `b1-inständig-1379`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** inständig
**LV MASTER reference:** neatlaidīgs
**CURRENT:** püsiv
**PROPOSED_ET (audit ieteikums):** tungivalt
**Problēma:** „Inständig“ tähendab tungivalt või paluvalt, mitte püsivalt.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4310
**Audit ID:** ET-B1-4310
**Card ID:** `b1-irdisch-1394`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** STRUCTURE
**DE (read-only):** irdisch
**LV MASTER reference:** zemes-
**CURRENT:** maa-
**PROPOSED_ET (audit ieteikums):** maine
**Problēma:** „Maa-“ on eesti keeles liitega seotud vorm, mitte iseseisev vaste sõnale „irdisch“; „maine“ on iseseisev omadussõna.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4311
**Audit ID:** ET-B1-4311
**Card ID:** `b1-irreführen-1396`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** irreführen
**LV MASTER reference:** maldināt
**CURRENT:** petma
**PROPOSED_ET (audit ieteikums):** eksitama
**Problēma:** „Irreführen“ tähendab eksitamist või eksiteele viimist; „petma“ tähendab pigem petmist või altvedamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4312
**Audit ID:** ET-B1-4312
**Card ID:** `b1-Irrtum-1398`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Irrtum
**LV MASTER reference:** pārpratums
**CURRENT:** arusaamatus
**PROPOSED_ET (audit ieteikums):** eksitus
**Problēma:** „Irrtum“ on viga või eksitus, mitte arusaamatus.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4313
**Audit ID:** ET-B1-4313
**Card ID:** `b1-Kabelkanal-1440`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Kabelkanal
**LV MASTER reference:** kabeļtelevīzijas kanāls
**CURRENT:** kaabeltelevisiooni kanal
**PROPOSED_ET (audit ieteikums):** kaablikanal
**Problēma:** Kabelkanal tähendab kaablikanalit või kaablikõri, mitte kaabeltelevisiooni kanalit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4314
**Audit ID:** ET-B1-4314
**Card ID:** `b1-kämpfen-1460`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** kämpfen
**LV MASTER reference:** cīnīties
**CURRENT:** maadlema
**PROPOSED_ET (audit ieteikums):** võitlema
**Problēma:** „Maadlema” tähendab maadlust pidama ehk wrestle, mitte üldiselt võitlema ehk fight.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4315
**Audit ID:** ET-B1-4315
**Card ID:** `b1-Kanten-1464`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Kanten
**LV MASTER reference:** maizes dona
**CURRENT:** leivakäär
**PROPOSED_ET (audit ieteikums):** leivakonts
**Problēma:** „Leivakäär” on leivaviil; Kanten tähendab siin leiva otsa ehk kontsu.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4316
**Audit ID:** ET-B1-4316
**Card ID:** `b1-Kantine-1465`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Kantine
**LV MASTER reference:** bufete
**CURRENT:** puhvet
**PROPOSED_ET (audit ieteikums):** söökla
**Problēma:** Kantine tähendab tavaliselt sööklat; „puhvet” on pigem buffet või einelaud.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4317
**Audit ID:** ET-B1-4317
**Card ID:** `b1-Kapelle-1467`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Kapelle
**LV MASTER reference:** kapela
**CURRENT:** kapell
**PROPOSED_ET (audit ieteikums):** kabel
**Problēma:** „Kapell” tähendab eesti keeles muusikakollektiivi; saksa Kapelle tähendab siin kabelit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4318
**Audit ID:** ET-B1-4318
**Card ID:** `b1-karg-1471`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** karg
**LV MASTER reference:** trūcīgs
**CURRENT:** vaene
**PROPOSED_ET (audit ieteikums):** kasin
**Problēma:** Karg tähendab vähest, nappi või kasinat; „vaene” tähendab eeskätt majanduslikult vaest.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4319
**Audit ID:** ET-B1-4319
**Card ID:** `b1-Kassenpatient-1481`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** Kassenpatient
**LV MASTER reference:** slimokasē apdrošināts pacients
**CURRENT:** haigekassa patsient
**PROPOSED_ET (audit ieteikums):** haigekassapatsient
**Problēma:** Eesti keeles on see liitsõna; praegune lahkukirjutus on ebaloomulik.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4320
**Audit ID:** ET-B1-4320
**Card ID:** `b1-kegeln-1486`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** kegeln
**LV MASTER reference:** sist ķegļus
**CURRENT:** keegleid mängima
**PROPOSED_ET (audit ieteikums):** keeglit mängima
**Problēma:** Spordiala nimetus on eesti keeles tavaliselt „keegel” ainsuse osastavas: mängima keeglit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4321
**Audit ID:** ET-B1-4321
**Card ID:** `b1-Klang-1513`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Klang
**LV MASTER reference:** skaņa
**CURRENT:** hääl
**PROPOSED_ET (audit ieteikums):** kõla
**Problēma:** „Hääl” tähendab inimese või looma häält; „Klang” tähendab heli või kõla.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4322
**Audit ID:** ET-B1-4322
**Card ID:** `b1-knistern-1550`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** knistern
**LV MASTER reference:** čaukstēt
**CURRENT:** sahisema
**PROPOSED_ET (audit ieteikums):** praksuma
**Problēma:** „Knistern” tähendab praksumist või krõpsumist; „sahisema” tähendab peamiselt sahisemist või kahisemist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4323
**Audit ID:** ET-B1-4323
**Card ID:** `b1-Kollegbuch-1562`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Kollegbuch
**LV MASTER reference:** studenta atzīmju grāmatiņa
**CURRENT:** õpiraamat
**PROPOSED_ET (audit ieteikums):** õppemärkmik
**Problēma:** „Õpiraamat” on õpik või õppimiseks mõeldud raamat; „Kollegbuch” tähendab pigem õppe- või loengumärkmikku.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4324
**Audit ID:** ET-B1-4324
**Card ID:** `b1-Konserve-1584`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Konserve
**LV MASTER reference:** konservi
**CURRENT:** konservid
**PROPOSED_ET (audit ieteikums):** konserv
**Problēma:** German is singular; the Estonian translation is incorrectly plural.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4325
**Audit ID:** ET-B1-4325
**Card ID:** `b1-Muskelkrampf-1614`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Muskelkrampf
**LV MASTER reference:** muskuļu krampji
**CURRENT:** lihaskrambid
**PROPOSED_ET (audit ieteikums):** lihaskramp
**Problēma:** German is singular; the Estonian translation is incorrectly plural.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4326
**Audit ID:** ET-B1-4326
**Card ID:** `b1-Magenkrampf-1615`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Magenkrampf
**LV MASTER reference:** kuņģa spazmas
**CURRENT:** maokrambid
**PROPOSED_ET (audit ieteikums):** maokramp
**Problēma:** German is singular; the Estonian translation is incorrectly plural.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4327
**Audit ID:** ET-B1-4327
**Card ID:** `b1-Kursbuch-1680`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Kursbuch
**LV MASTER reference:** vilcienu saraksts
**CURRENT:** rongide sõiduplaan
**PROPOSED_ET (audit ieteikums):** kursuseõpik
**Problēma:** Saksa Kursbuch tähendab kursuseõpikut, mitte rongide sõiduplaani; LV allikas on siin vastuolus saksa tähendusega.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4328
**Audit ID:** ET-B1-4328
**Card ID:** `b1-kuscheln-1690`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** kuscheln
**LV MASTER reference:** paglausties
**CURRENT:** end hellitada laskma
**PROPOSED_ET (audit ieteikums):** kaisutama
**Problēma:** Kuscheln tähendab kaisutama või hellitavalt embama; praegune vaste tähendab enda hellitada laskmist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4329
**Audit ID:** ET-B1-4329
**Card ID:** `b1-Laib-1713`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** GRAMMAR
**DE (read-only):** Laib
**LV MASTER reference:** klaips
**CURRENT:** pätsi
**PROPOSED_ET (audit ieteikums):** päts
**Problēma:** Pätsi on sõna päts omastav vorm; sõnavarakaardil peab vaste olema algvorm päts.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4330
**Audit ID:** ET-B1-4330
**Card ID:** `b1-Länderspiel-1716`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Länderspiel
**LV MASTER reference:** starptautiskās sacensības
**CURRENT:** rahvusvahelised võistlused
**PROPOSED_ET (audit ieteikums):** maavõistlus
**Problēma:** Länderspiel on riikide või rahvuskoondiste vaheline üksikmäng; praegune vaste on mitmuslik ja liiga üldine.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4331
**Audit ID:** ET-B1-4331
**Card ID:** `b1-ländlich-1718`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** ländlich
**LV MASTER reference:** lauku
**CURRENT:** maa-
**PROPOSED_ET (audit ieteikums):** maalähedane
**Problēma:** Maa- on liitsõnade eesliide, mitte iseseisev vaste; ländlich tähendab maalähedast või maapiirkondlikku.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4332
**Audit ID:** ET-B1-4332
**Card ID:** `b1-Lehm-1746`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Lehm
**LV MASTER reference:** māls
**CURRENT:** savi
**PROPOSED_ET (audit ieteikums):** savi
**Problēma:** Savi tähendab 'māls' ehk savi, kuid saksa Lehm tähendab lehma.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4333
**Audit ID:** ET-B1-4333
**Card ID:** `b1-lehnen-1747`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** lehnen
**LV MASTER reference:** piesliet
**CURRENT:** toetama
**PROPOSED_ET (audit ieteikums):** toetuma
**Problēma:** Lehnen tähendab nõjatuma või toetuma; toetama tähendab kellegi või millegi toetamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4334
**Audit ID:** ET-B1-4334
**Card ID:** `b1-Lichtung-1771`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Lichtung
**LV MASTER reference:** izcirtums
**CURRENT:** raiestik
**PROPOSED_ET (audit ieteikums):** raiesmik
**Problēma:** Metsa või metsalangi tähendus on eesti keeles raiesmik; raiestik ei ole siin õige vaste.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4335
**Audit ID:** ET-B1-4335
**Card ID:** `b1-Maifeier-1819`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** CRITICAL
**Category:** FOREIGN_REMNANT
**DE (read-only):** Maifeier
**LV MASTER reference:** pirmā Maija svētki
**CURRENT:** vappu
**PROPOSED_ET (audit ieteikums):** maipidu
**Problēma:** „Vappu” is Finnish, not Estonian. The German noun means a May celebration or May Day party.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---