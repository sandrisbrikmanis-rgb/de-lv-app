# ET–DE A1 — OWNER VIEW (grupa 51, 2501–2550)

**Standard:** `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9
**Auditors:** deterministika + GPT-5.6 Luna (READ-ONLY)
**Audit PR:** [#610](https://github.com/sandrisbrikmanis-rgb/de-lv-app/pull/610)

| Navigācija | Saite |
|------------|-------|
| GitHub indekss | [et-b1-owner-review-GITHUB.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-review-GITHUB.md) |
| OWNER VIEW (visi) | [et-b1-owner-view.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-view.md) |
| Decisions (šī grupa) | [et-b1-owner-decisions-group51.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions-group51.md) |
| Decisions (viss) | [et-b1-owner-decisions.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions.md) |

Avots: [et-b1-full-audit.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-full-audit.md)

## ET-B1-4386
**Audit ID:** ET-B1-4386
**Card ID:** `b1-Sprungschanze-2709`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Sprungschanze
**LV MASTER reference:** tramplīns
**CURRENT:** hüppelaud
**PROPOSED_ET (audit ieteikums):** suusahüppemägi
**Problēma:** Hüppelaud tähendab hüppelauda; Sprungschanze on suusahüpete mägi või rajatis.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4387
**Audit ID:** ET-B1-4387
**Card ID:** `b1-Stadtbahn-2717`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Stadtbahn
**LV MASTER reference:** elektriskais dzelzceļš pilsētas robežās
**CURRENT:** linnalähirong
**PROPOSED_ET (audit ieteikums):** linnaraudtee
**Problēma:** Linnalähirong on linnalähedane pendelrong; Stadtbahn tähendab linnasisest kergraudteed või linnaraudteed.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4388
**Audit ID:** ET-B1-4388
**Card ID:** `b1-Standlicht-2726`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** GRAMMAR
**DE (read-only):** Standlicht
**LV MASTER reference:** gabarītugunis automašīnām
**CURRENT:** seisutuled
**PROPOSED_ET (audit ieteikums):** seisutuli
**Problēma:** Saksa märksõna on ainsuses; Estonian vaste peaks olema samuti ainsuses.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4389
**Audit ID:** ET-B1-4389
**Card ID:** `b1-Stecknadel-2735`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Stecknadel
**LV MASTER reference:** kniepadata
**CURRENT:** nõel
**PROPOSED_ET (audit ieteikums):** nööpnõel
**Problēma:** Nõel on liiga üldine; Stecknadel tähendab konkreetselt nööpnõela või kinnitustihvti.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4390
**Audit ID:** ET-B1-4390
**Card ID:** `b1-Steingut-2739`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Steingut
**LV MASTER reference:** keramika
**CURRENT:** keraamika
**PROPOSED_ET (audit ieteikums):** fajanss
**Problēma:** Keraamika on üldmõiste; Steingut tähistab täpsemalt fajanssi ehk põletatud savikeraamikat.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4391
**Audit ID:** ET-B1-4391
**Card ID:** `b1-Stiftung-2760`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Stiftung
**LV MASTER reference:** fonds
**CURRENT:** fond
**PROPOSED_ET (audit ieteikums):** sihtasutus
**Problēma:** Fond tähendab eeskätt fondi, Stiftung tähendab tavaliselt sihtasutust või heategevuslikku fondi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4392
**Audit ID:** ET-B1-4392
**Card ID:** `b1-stöhnen-2769`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** ORTHOGRAPHY
**DE (read-only):** stöhnen
**LV MASTER reference:** stenēt
**CURRENT:** oigama
**PROPOSED_ET (audit ieteikums):** õigama
**Problēma:** Estonian õ-täht on puudu; õige kirjakuju on „õigama“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4393
**Audit ID:** ET-B1-4393
**Card ID:** `b1-Stoßzahn-2777`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** Stoßzahn
**LV MASTER reference:** ziloņa ilknis
**CURRENT:** elevandiluu kihv
**PROPOSED_ET (audit ieteikums):** elevandi kihv
**Problēma:** „Elevandiluu kihv“ tähendab sõna-sõnalt elevandiluust kihva; loomulik vaste on „elevandi kihv“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4394
**Audit ID:** ET-B1-4394
**Card ID:** `b1-Strandkorb-2784`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Strandkorb
**LV MASTER reference:** sauļošanās grozs pludmalē
**CURRENT:** rannatool
**PROPOSED_ET (audit ieteikums):** rannakorv
**Problēma:** Strandkorb on spetsiaalne rannakorv, mitte lihtsalt rannatool; praegune vaste kaotab olulise tähenduse.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4395
**Audit ID:** ET-B1-4395
**Card ID:** `b1-Stuck-2808`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Stuck
**LV MASTER reference:** ģipša java
**CURRENT:** kipsimört
**PROPOSED_ET (audit ieteikums):** stukk
**Problēma:** Stuck tähendab dekoratiivset krohvi või stukkornamenti; „kipsimört“ tähistab pigem materjali.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4396
**Audit ID:** ET-B1-4396
**Card ID:** `b1-tauglich-2864`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** ORTHOGRAPHY
**DE (read-only):** tauglich
**LV MASTER reference:** derīgs
**CURRENT:** kõlbulik
**PROPOSED_ET (audit ieteikums):** kõlblik
**Problēma:** Standardne eesti kirjakuju on „kõlblik“; „kõlbulik“ on õigekirjaviga.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4397
**Audit ID:** ET-B1-4397
**Card ID:** `b1-Tiefkühlfach-2888`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Tiefkühlfach
**LV MASTER reference:** saldētājkamera
**CURRENT:** sügavkülmik
**PROPOSED_ET (audit ieteikums):** sügavkülmikuosa
**Problēma:** „Sügavkülmik” tähendab tervet sügavkülmikut, mitte selle sees olevat sügavkülmasektsiooni või -lahtrit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4398
**Audit ID:** ET-B1-4398
**Card ID:** `b1-trampen-2904`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** trampen
**LV MASTER reference:** ceļot ar autostopu
**CURRENT:** autostopiga rändama
**PROPOSED_ET (audit ieteikums):** autostopiga sõitma
**Problēma:** Tähendus on arusaadav, kuid „autostopiga sõitma” on eesti keeles loomulikum vaste autostopiga reisimisele.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4399
**Audit ID:** ET-B1-4399
**Card ID:** `b1-Tramper-2905`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** Tramper
**LV MASTER reference:** ceļotājs ar autostopu
**CURRENT:** autostopiga rändaja
**PROPOSED_ET (audit ieteikums):** autostopiga sõitja
**Problēma:** „Autostopiga rändaja” on mõistetav, kuid inimese kohta on loomulikum „autostopiga sõitja”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4400
**Audit ID:** ET-B1-4400
**Card ID:** `b1-Trittbrett-2922`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Trittbrett
**LV MASTER reference:** kāpslis
**CURRENT:** aste
**PROPOSED_ET (audit ieteikums):** astelaud
**Problēma:** „Aste” tähendab sammu või astet; Trittbrett on sõiduki või vaguni astelaud.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4401
**Audit ID:** ET-B1-4401
**Card ID:** `b1-unentschieden-2997`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** unentschieden
**LV MASTER reference:** neizšķirts
**CURRENT:** viigistatud
**PROPOSED_ET (audit ieteikums):** viigis
**Problēma:** „Viigistatud” tähendab pigem viigiks muudetud; tulemuse kohta on loomulikum „viigis” või „viiki jäänud”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4402
**Audit ID:** ET-B1-4402
**Card ID:** `b1-Unwetter-3033`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Unwetter
**LV MASTER reference:** slikts laiks
**CURRENT:** halb ilm
**PROPOSED_ET (audit ieteikums):** torm
**Problēma:** „Unwetter” tähendab tugevat tormi või ränka ilma, mitte lihtsalt üldist halba ilma.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4403
**Audit ID:** ET-B1-4403
**Card ID:** `b1-unwohl-3034`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** unwohl
**LV MASTER reference:** nevesels
**CURRENT:** halb
**PROPOSED_ET (audit ieteikums):** halb enesetunne
**Problēma:** „Halb” tähendab üldiselt „halb”; „unwohl” viitab konkreetselt halvale enesetundele või haiglasele olekule.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4404
**Audit ID:** ET-B1-4404
**Card ID:** `b1-Vergehen-3064`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Vergehen
**LV MASTER reference:** pārkāpums
**CURRENT:** eksimus
**PROPOSED_ET (audit ieteikums):** rikkumine
**Problēma:** Eksimus tähendab viga või eksitust, mitte õigusrikkumist ega üleastumist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4405
**Audit ID:** ET-B1-4405
**Card ID:** `b1-Vergnügen-3067`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Vergnügen
**LV MASTER reference:** prieks
**CURRENT:** rõõm
**PROPOSED_ET (audit ieteikums):** lõbu
**Problēma:** Vergnügen tähendab eeskätt lõbu või naudingut; rõõm tähendab pigem joy ehk emotsionaalset rõõmu.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4406
**Audit ID:** ET-B1-4406
**Card ID:** `b1-sich verlaufen-3087`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** sich verlaufen
**LV MASTER reference:** norisināties
**CURRENT:** toimuma
**PROPOSED_ET (audit ieteikums):** ära eksima
**Problēma:** Toimuma tähendab toimumist; sich verlaufen tähendab ära eksima või teed kaotama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4407
**Audit ID:** ET-B1-4407
**Card ID:** `b1-sich verrechnen-3098`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** sich verrechnen
**LV MASTER reference:** pārrēķināties
**CURRENT:** arvestust eksima
**PROPOSED_ET (audit ieteikums):** arvutamisel eksima
**Problēma:** Estonian requires a natural construction such as „arvutamisel eksima“; „arvestust eksima“ is not idiomatic.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4408
**Audit ID:** ET-B1-4408
**Card ID:** `b1-versammeln-3100`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** versammeln
**LV MASTER reference:** [sa]pulcēt
**CURRENT:** koguma
**PROPOSED_ET (audit ieteikums):** kokku koguma
**Problēma:** „Koguma“ usually means collect; the German verb means gathering people or things together.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4409
**Audit ID:** ET-B1-4409
**Card ID:** `b1-versäumen-3102`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** versäumen
**LV MASTER reference:** nokavēt
**CURRENT:** hilinema
**PROPOSED_ET (audit ieteikums):** maha magama
**Problēma:** „Hilinema“ means be late, whereas „versäumen“ means miss or fail to do something.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4410
**Audit ID:** ET-B1-4410
**Card ID:** `b1-verschlafen-3105`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** verschlafen
**LV MASTER reference:** nogulēt
**CURRENT:** magama jääma
**PROPOSED_ET (audit ieteikums):** sisse magama
**Problēma:** „Magama jääma“ means fall asleep; „verschlafen“ means oversleep or sleep through something.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4411
**Audit ID:** ET-B1-4411
**Card ID:** `b1-sich verschlafen-3106`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** ORTHOGRAPHY
**DE (read-only):** sich verschlafen
**LV MASTER reference:** aizgulēties
**CURRENT:** end ülemagama
**PROPOSED_ET (audit ieteikums):** sisse magama
**Problēma:** The current form is not standard Estonian; the idiomatic equivalent of oversleep is „sisse magama“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4412
**Audit ID:** ET-B1-4412
**Card ID:** `b1-sich versorgen-3110`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** sich versorgen
**LV MASTER reference:** nodrošināties
**CURRENT:** end kindlustama
**PROPOSED_ET (audit ieteikums):** enda eest hoolitsema
**Problēma:** „End kindlustama“ means insure oneself, not provide for or take care of oneself.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4413
**Audit ID:** ET-B1-4413
**Card ID:** `b1-sich versprechen-3113`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** sich versprechen
**LV MASTER reference:** pārteikties
**CURRENT:** end reservama
**PROPOSED_ET (audit ieteikums):** sõnu sassi ajama
**Problēma:** The German verb means misspeak or make a verbal slip; the current translation has a different meaning.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4414
**Audit ID:** ET-B1-4414
**Card ID:** `b1-verwandt-3131`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** GRAMMAR
**DE (read-only):** verwandt
**LV MASTER reference:** radniecīgs
**CURRENT:** sugulane
**PROPOSED_ET (audit ieteikums):** suguluses
**Problēma:** The German entry is an adjective; „sugulane“ is a noun meaning relative. Use the adjective-like „suguluses“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4415
**Audit ID:** ET-B1-4415
**Card ID:** `b1-verzichten-3138`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** verzichten
**LV MASTER reference:** atteikties
**CURRENT:** keelduma
**PROPOSED_ET (audit ieteikums):** loobuma
**Problēma:** „Verzichten“ means do without or give up something; „keelduma“ primarily means refuse something.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4416
**Audit ID:** ET-B1-4416
**Card ID:** `b1-Wasserski-3208`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Wasserski
**LV MASTER reference:** ūdensslēpe
**CURRENT:** veesuusk
**PROPOSED_ET (audit ieteikums):** veesuusatamine
**Problēma:** „Veesuusk” tähendab veesuuska, kuid saksa Wasserski viitab veesuusatamisele kui spordialale.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4417
**Audit ID:** ET-B1-4417
**Card ID:** `b1-Web-3210`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Web
**LV MASTER reference:** internets
**CURRENT:** internet
**PROPOSED_ET (audit ieteikums):** veeb
**Problēma:** Saksa „Web” tähendab veebi; „internet” on laiem mõiste ega ole täpne vaste.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4418
**Audit ID:** ET-B1-4418
**Card ID:** `b1-weiblich-3220`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** weiblich
**LV MASTER reference:** sieviešu
**CURRENT:** naiste
**PROPOSED_ET (audit ieteikums):** naissoost
**Problēma:** „Naiste” tähendab „naiste oma/naistele kuuluv”; „weiblich” tähendab „naissoost” või „naiselik”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4419
**Audit ID:** ET-B1-4419
**Card ID:** `b1-Weinbrand-3227`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Weinbrand
**LV MASTER reference:** konjaks
**CURRENT:** konjak
**PROPOSED_ET (audit ieteikums):** brändi
**Problēma:** „Weinbrand” tähendab brändit üldiselt; „konjak” on selle konkreetne, kaitstud päritoluga liik.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4420
**Audit ID:** ET-B1-4420
**Card ID:** `b1-Weise-3228`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Weise
**LV MASTER reference:** gudrs
**CURRENT:** tark
**PROPOSED_ET (audit ieteikums):** viis
**Problēma:** Nimisõna „die Weise” tähendab „viis” või „mood”; „tark” vastab omadussõnale „weise”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4421
**Audit ID:** ET-B1-4421
**Card ID:** `b1-Wimper-3264`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Wimper
**LV MASTER reference:** skropsta
**CURRENT:** ripse
**PROPOSED_ET (audit ieteikums):** rips
**Problēma:** “Ripse” is the partitive form; the dictionary headword corresponding to the singular noun is “rips”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4422
**Audit ID:** ET-B1-4422
**Card ID:** `b1-Zutat-3303`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Zutat
**LV MASTER reference:** piedeva
**CURRENT:** lisand
**PROPOSED_ET (audit ieteikums):** koostisosa
**Problēma:** “Lisand” means an addition or accompaniment; “koostisosa” is the precise Estonian term for an ingredient.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4423
**Audit ID:** ET-B1-4423
**Card ID:** `b1-sich umziehen-3355`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** sich umziehen
**LV MASTER reference:** pārģērbties
**CURRENT:** end ümber riietama
**PROPOSED_ET (audit ieteikums):** ümber riietuma
**Problēma:** Praegune vorm tähendab kellegi teise riietamist; saksa väljend tähendab ise riiete vahetamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4424
**Audit ID:** ET-B1-4424
**Card ID:** `b1-anmachen`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** anmachen
**LV MASTER reference:** viņa sagatavo salātus ar eļļu.
**CURRENT:** ta valmistab salatit õliga.
**PROPOSED_ET (audit ieteikums):** ta maitsestab salatit õliga.
**Problēma:** Siin tähendab anmachen salati õliga maitsestamist või segamist, mitte üldiselt valmistamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4425
**Audit ID:** ET-B1-4425
**Card ID:** `b1-aufführen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** aufführen
**LV MASTER reference:** vārdi ir norādīti sarakstā.
**CURRENT:** nimed on nimekirjas märgitud.
**PROPOSED_ET (audit ieteikums):** nimed on nimekirjas loetletud.
**Problēma:** Aufführen tähendab siin nimekirjas loetlemist; „märgitud” ei anna seda tähendust täpselt edasi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4426
**Audit ID:** ET-B1-4426
**Card ID:** `b1-sich-aufhalten`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** sich aufhalten
**LV MASTER reference:** sastrēgums mūs ilgi aizkavēja.
**CURRENT:** ummik takistas meid kaua.
**PROPOSED_ET (audit ieteikums):** me viibisime ummikus kaua.
**Problēma:** Praegune näide väljendab transitiivset aufhalten-tähendust „takistama”, mitte refleksiivset „viibima”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4427
**Audit ID:** ET-B1-4427
**Card ID:** `b1-aussprache`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Aussprache
**LV MASTER reference:** Parastai sarunai lieto Gespräch; Aussprache kā pārrunas skan formālāk.
**CURRENT:** Tavalise vestluse jaoks kasutatakse Gespräch; Aussprache kui läbirääkimine kõlab formaalsemalt.
**PROPOSED_ET (audit ieteikums):** Tavalise vestluse jaoks kasutatakse Gespräch; Aussprache kui arutelu või asjade selgeks rääkimine kõlab formaalsemalt.
**Problēma:** Aussprache tähendab siin arutelu või asjade selgeks rääkimist, mitte tingimata läbirääkimist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4428
**Audit ID:** ET-B1-4428
**Card ID:** `b1-ausüben`
**Field/path:** `study.examples[0].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** ausüben
**LV MASTER reference:** viņa strādā šajā profesijā jau desmit gadus.
**CURRENT:** ta töötab sellel elukutsel juba kümme aastat.
**PROPOSED_ET (audit ieteikums):** ta töötab sellel kutsealal juba kümme aastat.
**Problēma:** Eesti keeles on „sellel kutsealal töötama” loomulikum kui „sellel elukutsel töötama”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4429
**Audit ID:** ET-B1-4429
**Card ID:** `b1-becken`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** Becken
**LV MASTER reference:** Ūdens baseinā → Becken; ķermenis → iegurnis; virtuvē/tehnikā → tvertne vai bļoda.
**CURRENT:** Vesi basseinis → Becken; keha → vaagen; köögis/tehnikas → mahuti või kauss.
**PROPOSED_ET (audit ieteikums):** Vesi basseinis → Becken; keha → vaagen; köögis või tehnika puhul → mahuti või kauss.
**Problēma:** „Köögis/tehnikas” on ebaloomulik ja tähenduselt ebaselge; siin sobib „köögis või tehnika puhul”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4430
**Audit ID:** ET-B1-4430
**Card ID:** `b1-behandeln`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** behandeln
**LV MASTER reference:** viņa pret visiem izturas draudzīgi.
**CURRENT:** ta käitub kõigiga sõbralikult.
**PROPOSED_ET (audit ieteikums):** ta kohtleb kõiki sõbralikult.
**Problēma:** Inimest kohtlema tähendab siin „behandeln”; „käituma kõigiga” ei ole loomulik ega täpne.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4431
**Audit ID:** ET-B1-4431
**Card ID:** `b1-behandeln`
**Field/path:** `study.comparison[0].meaning`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** behandeln
**LV MASTER reference:** ārstēt / izturēties / apspriest tēmu
**CURRENT:** ravima / käituma / teemat käsitlema
**PROPOSED_ET (audit ieteikums):** ravima / kohtlema / teemat käsitlema
**Problēma:** Selles tähenduses on behandeln inimese suhtes „kohtlema”, mitte „käituma”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4432
**Audit ID:** ET-B1-4432
**Card ID:** `b1-behandeln`
**Field/path:** `study.tip.leftBlocks[0].text`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** behandeln
**LV MASTER reference:** Skaties uz objektu: pacientu behandeln = ārstēt; cilvēku behandeln = izturēties; tēmu behandeln = apspriest.
**CURRENT:** Vaata objekti: patsienti behandeln = ravima; inimest behandeln = käituma; teemat behandeln = käsitlema.
**PROPOSED_ET (audit ieteikums):** Vaata objekti: patsienti behandeln = ravima; inimest behandeln = kohtlema; teemat behandeln = käsitlema.
**Problēma:** Inimese kohtlemist kirjeldav vaste on „kohtlema”; „käituma” muudab verbi tähendust.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4433
**Audit ID:** ET-B1-4433
**Card ID:** `b1-behandeln`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** behandeln
**LV MASTER reference:** behandeln nav tas pats, kas heilen: behandeln ir ārstēšanas process, heilen ir izārstēšanas rezultāts.
**CURRENT:** behandeln ei ole sama, mis heilen: behandeln on ravimise protsess, heilen on tervenemise tulemus.
**PROPOSED_ET (audit ieteikums):** behandeln ei ole sama, mis heilen: behandeln on ravimise protsess, heilen tähendab terveks ravima.
**Problēma:** Heilen on verb „terveks ravima/paranema”, mitte „tervenemise tulemus”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4434
**Audit ID:** ET-B1-4434
**Card ID:** `b1-belegen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** belegen
**LV MASTER reference:** es piesakos vācu valodas kursam.
**CURRENT:** ma registreerun saksa keele kursusele.
**PROPOSED_ET (audit ieteikums):** ma osalen saksa keele kursusel.
**Problēma:** Einen Kurs belegen tähendab kursusel osalema või kursust võtma, mitte ainult sellele registreeruma.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4435
**Audit ID:** ET-B1-4435
**Card ID:** `b1-belegen`
**Field/path:** `study.comparison[0].meaning`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** belegen
**LV MASTER reference:** aizņemt / pieteikties / pierādīt
**CURRENT:** hõivama / registreeruma / tõestama
**PROPOSED_ET (audit ieteikums):** hõivama / kursusel osalema / tõestama
**Problēma:** Kursuse tähenduses väljendab „kursusel osalema” saksa verbi belegen täpsemalt kui „registreeruma”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---