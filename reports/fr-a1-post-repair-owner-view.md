# FR–DE A1 — OWNER VIEW

**Standard:** `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.12
**Auditors:** deterministika + GPT-5.6 Luna (READ-ONLY)
**MAIN_BASE_SHA:** `a2a769e8600291411a7a66eab0483dd4659c9151`
**WORK_BRANCH:** `cursor/fr-a1-owner-gala-copy-only-f5bc`
**Audit PR:** [#683](https://github.com/sandrisbrikmanis-rgb/de-lv-app/pull/683)
**SCOPE:** FR–DE A1 (`data/fr/a1.js`)
**Findings:** **10** (OWNER_BACKLOG_FINAL after §7.11–§7.19 discovery-stability)

> OBJECT_COVERAGE = 702/702 (100%). DISCOVERY_COMPLETENESS = NOT_GUARANTEED.
> 702/702 does NOT mean all possible defects were found.
> Visi ieraksti sākotnēji **PENDING**. OWNER aizpilda [fr-a1-post-repair-owner-decisions.md](fr-a1-post-repair-owner-decisions.md).
> **DE = STRICT READ-ONLY.** Production: `data/fr/a1.js` + `www/data/fr/a1.js`.

## GitHub atvēršana

| Fails | GitHub |
|-------|--------|
| GitHub indekss | [fr-a1-post-repair-owner-review-GITHUB.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/fr-a1-owner-gala-copy-only-f5bc/reports/fr-a1-post-repair-owner-review-GITHUB.md) |
| OWNER README | [fr-a1-post-repair-owner-review-README.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/fr-a1-owner-gala-copy-only-f5bc/reports/fr-a1-post-repair-owner-review-README.md) |
| OWNER DECISIONS | [fr-a1-post-repair-owner-decisions.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/fr-a1-owner-gala-copy-only-f5bc/reports/fr-a1-post-repair-owner-decisions.md) |
| Pilns audits | [fr-a1-post-repair-audit.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/fr-a1-owner-gala-copy-only-f5bc/reports/fr-a1-post-repair-audit.md) |
| OWNER authority (filled) | [owner-authority/](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/fr-a1-owner-gala-copy-only-f5bc/reports/owner-authority/) | 423 LABOT · 481 NELABOT |

## Grupas (pa 50 findingiem)

| Grupa | Findings | VIEW | DECISIONS |
|-------|----------|------|-----------|
| 1–10 | 10 | [fr-a1-post-repair-owner-view-group01.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/fr-a1-owner-gala-copy-only-f5bc/reports/fr-a1-post-repair-owner-view-group01.md) | [fr-a1-post-repair-owner-decisions-group01.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/fr-a1-owner-gala-copy-only-f5bc/reports/fr-a1-post-repair-owner-decisions-group01.md) |

## Īsais saraksts (visi findingi)

- **FR-A1-0231** `a1-also` · `study.sectionAccents (examples)` · MEDIUM · Accent term not found in section text
- **FR-A1-0332** `a1-also` · `study.sectionAccents.examples.lv` · MEDIUM · sectionAccents termins "Vous" nav atrodams sadaļā examples
- **FR-A1-0334** `a1-Butterbrot-112` · `frText` · MEDIUM · «Butterbrot» désigne du pain beurré, pas nécessairement un sandwich avec une gar…
- **FR-A1-0361** `a1-eis` · `study.examples[0].lv` · LOW · La phrase française ne comporte pas de ponctuation finale.
- **FR-A1-0362** `a1-eis` · `study.examples[1].lv` · LOW · La question française ne comporte pas de point d'interrogation final.
- **FR-A1-0367** `a1-erst` · `study.comparison[2].meaning` · LOW · Aucune correction linguistique n'est requise; ce champ ne contient pas plusieurs…
- **FR-A1-0370** `a1-etwas` · `study.examples[2].lv` · LOW · La phrase française ne comporte pas de ponctuation finale.
- **FR-A1-0371** `a1-etwas` · `study.examples[3].lv` · LOW · La phrase française ne comporte pas de ponctuation finale.
- **FR-A1-0417** `a1-passen` · `study.examples[1].lv` · MEDIUM · Pour exprimer qu'un vêtement convient à quelqu'un, le français indique normaleme…
- **FR-A1-0433** `a1-verstehen` · `study.tip.text` · MEDIUM · Le point médian sépare deux instructions distinctes dans un champ destiné à l'ap…

## Pilns findingu pārskats (visi findingi)

## FR-A1-0231
**Audit ID:** FR-A1-0231
**Card ID:** `a1-also`
**Field/path:** `study.sectionAccents (examples)`
**Production file:** `data/fr/a1.js`
**Severity:** MEDIUM
**Category:** SECTIONACCENTS_LANGUAGE
**DE (read-only):** —
**CURRENT:** Vous
**PROPOSED_ET (audit ieteikums):** (termins no FR teksta)
**Problēma:** Accent term not found in section text
**Avots:** deterministic
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0332
**Audit ID:** FR-A1-0332
**Card ID:** `a1-also`
**Field/path:** `study.sectionAccents.examples.lv`
**Production file:** `data/fr/a1.js`
**Severity:** MEDIUM
**Category:** SECTIONACCENTS_LANGUAGE
**DE (read-only):** also
**CURRENT:** Vous
**PROPOSED_ET (audit ieteikums):** (termins no attiecīgā FR teksta)
**Problēma:** sectionAccents termins "Vous" nav atrodams sadaļā examples
**Avots:** validate-study-design
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0334
**Audit ID:** FR-A1-0334
**Card ID:** `a1-Butterbrot-112`
**Field/path:** `frText`
**Production file:** `data/fr/a1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Butterbrot
**LV MASTER reference:** sviestmaize
**CURRENT:** Un sandwich
**PROPOSED_ET (audit ieteikums):** Une tartine beurrée
**Problēma:** «Butterbrot» désigne du pain beurré, pas nécessairement un sandwich avec une garniture.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0361
**Audit ID:** FR-A1-0361
**Card ID:** `a1-eis`
**Field/path:** `study.examples[0].lv`
**Production file:** `data/fr/a1.js`
**Severity:** LOW
**Category:** ORTHOGRAPHY
**DE (read-only):** Eis
**LV MASTER reference:** es ēdu saldējumu.
**CURRENT:** Je mange de la glace
**PROPOSED_ET (audit ieteikums):** Je mange de la glace.
**Problēma:** La phrase française ne comporte pas de ponctuation finale.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0362
**Audit ID:** FR-A1-0362
**Card ID:** `a1-eis`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/fr/a1.js`
**Severity:** LOW
**Category:** ORTHOGRAPHY
**DE (read-only):** Eis
**LV MASTER reference:** vai tu gribi saldējumu?
**CURRENT:** Veux-tu de la glace
**PROPOSED_ET (audit ieteikums):** Veux-tu de la glace ?
**Problēma:** La question française ne comporte pas de point d'interrogation final.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0367
**Audit ID:** FR-A1-0367
**Card ID:** `a1-erst`
**Field/path:** `study.comparison[2].meaning`
**Production file:** `data/fr/a1.js`
**Severity:** LOW
**Category:** MULTIPLE_TRANSLATIONS_DETECTED
**DE (read-only):** erst
**LV MASTER reference:** tikai
**CURRENT:** Seulement
**PROPOSED_ET (audit ieteikums):** Seulement
**Problēma:** Aucune correction linguistique n'est requise; ce champ ne contient pas plusieurs traductions.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0370
**Audit ID:** FR-A1-0370
**Card ID:** `a1-etwas`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/fr/a1.js`
**Severity:** LOW
**Category:** ORTHOGRAPHY
**DE (read-only):** etwas
**LV MASTER reference:** es esmu nedaudz noguris.
**CURRENT:** Je suis un peu fatigué
**PROPOSED_ET (audit ieteikums):** Je suis un peu fatigué.
**Problēma:** La phrase française ne comporte pas de ponctuation finale.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0371
**Audit ID:** FR-A1-0371
**Card ID:** `a1-etwas`
**Field/path:** `study.examples[3].lv`
**Production file:** `data/fr/a1.js`
**Severity:** LOW
**Category:** ORTHOGRAPHY
**DE (read-only):** etwas
**LV MASTER reference:** man tev ir kaut kas.
**CURRENT:** J'ai quelque chose pour toi
**PROPOSED_ET (audit ieteikums):** J'ai quelque chose pour toi.
**Problēma:** La phrase française ne comporte pas de ponctuation finale.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0417
**Audit ID:** FR-A1-0417
**Card ID:** `a1-passen`
**Field/path:** `study.examples[1].lv`
**Production file:** `data/fr/a1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** passen
**LV MASTER reference:** kleita labi der.
**CURRENT:** La robe va bien.
**PROPOSED_ET (audit ieteikums):** La robe me va bien.
**Problēma:** Pour exprimer qu'un vêtement convient à quelqu'un, le français indique normalement la personne avec « me va bien ».
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## FR-A1-0433
**Audit ID:** FR-A1-0433
**Card ID:** `a1-verstehen`
**Field/path:** `study.tip.text`
**Production file:** `data/fr/a1.js`
**Severity:** MEDIUM
**Category:** MULTIPLE_TRANSLATIONS_DETECTED
**DE (read-only):** verstehen
**LV MASTER reference:** Atceries: saprast tekstu/cilvēku → verstehen; prast kaut ko darīt → können.
**CURRENT:** N'oubliez pas : comprendre le texte/la personne → verstehen • Savoir comment faire quelque chose → können.
**PROPOSED_ET (audit ieteikums):** N'oubliez pas : comprendre le texte/la personne → verstehen ; savoir faire quelque chose → können.
**Problēma:** Le point médian sépare deux instructions distinctes dans un champ destiné à l'apprenant.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---