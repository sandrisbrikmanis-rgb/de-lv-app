# ET–DE A1 — OWNER VIEW (grupa 48, 2351–2400)

**Standard:** `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9
**Auditors:** deterministika + GPT-5.6 Luna (READ-ONLY)
**Audit PR:** [#621](https://github.com/sandrisbrikmanis-rgb/de-lv-app/pull/621)

| Navigācija | Saite |
|------------|-------|
| GitHub indekss | [et-b1-owner-review-GITHUB.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-review-GITHUB.md) |
| OWNER VIEW (visi) | [et-b1-owner-view.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-view.md) |
| Decisions (šī grupa) | [et-b1-owner-decisions-group48.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions-group48.md) |
| Decisions (viss) | [et-b1-owner-decisions.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions.md) |

Avots: [et-b1-full-audit.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-full-audit.md)

## ET-B1-4236
**Audit ID:** ET-B1-4236
**Card ID:** `b1-Dank`
**Field/path:** `study.sectionAccents.comparison.example`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SECTIONACCENTS_LANGUAGE
**DE (read-only):** Dank
**CURRENT:** aitäh
**PROPOSED_ET (audit ieteikums):** (termins no attiecīgā ET teksta)
**Problēma:** sectionAccents termins "aitäh" nav atrodams sadaļā comparison
**Avots:** validate-study-design
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4237
**Audit ID:** ET-B1-4237
**Card ID:** `b1-Dank`
**Field/path:** `study.sectionAccents.comparison.example`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SECTIONACCENTS_LANGUAGE
**DE (read-only):** Dank
**CURRENT:** sind
**PROPOSED_ET (audit ieteikums):** (termins no attiecīgā ET teksta)
**Problēma:** sectionAccents termins "sind" nav atrodams sadaļā comparison
**Avots:** validate-study-design
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4238
**Audit ID:** ET-B1-4238
**Card ID:** `b1-Dank`
**Field/path:** `study.sectionAccents.comparison.example`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SECTIONACCENTS_LANGUAGE
**DE (read-only):** Dank
**CURRENT:** tänan
**PROPOSED_ET (audit ieteikums):** (termins no attiecīgā ET teksta)
**Problēma:** sectionAccents termins "tänan" nav atrodams sadaļā comparison
**Avots:** validate-study-design
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4239
**Audit ID:** ET-B1-4239
**Card ID:** `b1-Dank`
**Field/path:** `study.sectionAccents.comparison.example`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SECTIONACCENTS_LANGUAGE
**DE (read-only):** Dank
**CURRENT:** suur tänu
**PROPOSED_ET (audit ieteikums):** (termins no attiecīgā ET teksta)
**Problēma:** sectionAccents termins "suur tänu" nav atrodams sadaļā comparison
**Avots:** validate-study-design
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4240
**Audit ID:** ET-B1-4240
**Card ID:** `b1-Angabe-21`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Angabe
**LV MASTER reference:** norāde
**CURRENT:** viide
**PROPOSED_ET (audit ieteikums):** andmed
**Problēma:** „Viide” tähendab viidet või osutust; „Angabe” tähendab siin andmeid, infot või täpsustust.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4241
**Audit ID:** ET-B1-4241
**Card ID:** `b1-Abiturient-57`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** ORTHOGRAPHY
**DE (read-only):** Abiturient
**LV MASTER reference:** abiturients
**CURRENT:** abituriend
**PROPOSED_ET (audit ieteikums):** abitურient
**Problēma:** Eesti keeles on ainsuse nimetav „abituri ent”; „abituriend” on tüvekuju, mitte korrektne märksõnavorm.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4242
**Audit ID:** ET-B1-4242
**Card ID:** `b1-abgehen`
**Field/path:** `study.important.text`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** abgehen
**LV MASTER reference:** Sarunvalodā abgehen var būt ļoti atkarīgs no konteksta; frontē drošāk mācīties pamata nozīmi aiziet.
**CURRENT:** Kõnekeeles võib abgehen olla väga kontekstist sõltuv; algajal on kindlam õppida põhitähendust lahkuma.
**PROPOSED_ET (audit ieteikums):** Kõnekeeles võib abgehen olla väga kontekstist sõltuv; algajal on kindlam õppida põhitähendusena „lahkuma“.
**Problēma:** Konstruktsioon „õppida põhitähendust lahkuma“ on grammatiliselt ja tähenduslikult ebaselge.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4243
**Audit ID:** ET-B1-4243
**Card ID:** `b1-absatz`
**Field/path:** `study.examples[2].lv`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Absatz
**LV MASTER reference:** kurpes papēdis ir salūzis.
**CURRENT:** kingakontsa on katki läinud.
**PROPOSED_ET (audit ieteikums):** kingakonts on katki läinud.
**Problēma:** Lause alus peab olema nimetavas käändes; „kingakontsa“ on siin vale vorm.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4244
**Audit ID:** ET-B1-4244
**Card ID:** `b1-abliefern-63`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** abliefern
**LV MASTER reference:** nodot
**CURRENT:** reetma
**PROPOSED_ET (audit ieteikums):** kohale toimetama
**Problēma:** „Reetma” tähendab saksa „verraten”; „abliefern” tähendab üle andma või kohale toimetama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4245
**Audit ID:** ET-B1-4245
**Card ID:** `b1-abschaffen-71`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** abschaffen
**LV MASTER reference:** atcelt
**CURRENT:** tühistama
**PROPOSED_ET (audit ieteikums):** kaotama
**Problēma:** „Abschaffen” tähendab millegi kaotamist või likvideerimist, mitte tavaliselt konkreetse sündmuse tühistamist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4246
**Audit ID:** ET-B1-4246
**Card ID:** `b1-absperren-84`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** absperren
**LV MASTER reference:** norobežot
**CURRENT:** piirama
**PROPOSED_ET (audit ieteikums):** tõkestama
**Problēma:** „Piirama” tähendab piirangute seadmist; „absperren” tähendab ala või juurdepääsu tõkestamist või sulgemist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4247
**Audit ID:** ET-B1-4247
**Card ID:** `b1-Ackerbau-90`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Ackerbau
**LV MASTER reference:** zemkopība
**CURRENT:** põllumajandus
**PROPOSED_ET (audit ieteikums):** põlluharimine
**Problēma:** „Põllumajandus” on üldmõiste; „Ackerbau” tähistab konkreetselt põldude harimist ja taimekasvatust.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4248
**Audit ID:** ET-B1-4248
**Card ID:** `b1-Ambulanz-106`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Ambulanz
**LV MASTER reference:** neatliekamā palīdzība
**CURRENT:** kiirabi
**PROPOSED_ET (audit ieteikums):** ambulatoorium
**Problēma:** Saksa „Ambulanz” tähendab tavaliselt ambulatoorset haiglaüksust või vastuvõttu, mitte kiirabiteenust.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4249
**Audit ID:** ET-B1-4249
**Card ID:** `b1-anlehnen-120`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** anlehnen
**LV MASTER reference:** piesliet
**CURRENT:** toetama
**PROPOSED_ET (audit ieteikums):** vastu toetama
**Problēma:** Toetama tähendab toetama või supportima; anlehnen tähendab millegi vastu nõjatama või toetama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4250
**Audit ID:** ET-B1-4250
**Card ID:** `b1-anrichten-133`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** anrichten
**LV MASTER reference:** nodarīt
**CURRENT:** tekitama
**PROPOSED_ET (audit ieteikums):** kahju tekitama
**Problēma:** Selles tähenduses tähendab anrichten kahju põhjustamist; tekitama üksi on liiga üldine.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4251
**Audit ID:** ET-B1-4251
**Card ID:** `b1-anschreiben-141`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** anschreiben
**LV MASTER reference:** pierakstīt
**CURRENT:** üles kirjutama
**PROPOSED_ET (audit ieteikums):** kirjutama
**Problēma:** Üles kirjutama tähendab saksa keeles pigem aufschreiben; anschreiben tähendab kirjutama või kellelegi kirjutama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4252
**Audit ID:** ET-B1-4252
**Card ID:** `b1-anweisen-160`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** anweisen
**LV MASTER reference:** norādīt
**CURRENT:** juhatama
**PROPOSED_ET (audit ieteikums):** juhendama
**Problēma:** Anweisen tähendab juhendama või korraldust andma; juhatama tähendab eelkõige juhtima või teed näitama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4253
**Audit ID:** ET-B1-4253
**Card ID:** `b1-Auflauf-179`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Auflauf
**LV MASTER reference:** sacepums
**CURRENT:** vorm
**PROPOSED_ET (audit ieteikums):** vormiroog
**Problēma:** Kulinaarne Auflauf on vormiroog või ahjus küpsetatud roog; „vorm“ üksi tähendab peamiselt vormi või anumat.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4254
**Audit ID:** ET-B1-4254
**Card ID:** `b1-sich aufregen-200`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** sich aufregen
**LV MASTER reference:** uztraukties
**CURRENT:** ärevil olema
**PROPOSED_ET (audit ieteikums):** ärrituma
**Problēma:** Sich aufregen tähendab ärrituma või pahandama; „ärevil olema“ tähendab pigem ärevuses olemist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4255
**Audit ID:** ET-B1-4255
**Card ID:** `b1-Faschingsball-237`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Faschingsball
**LV MASTER reference:** karnevāls
**CURRENT:** karneval
**PROPOSED_ET (audit ieteikums):** karnevaliball
**Problēma:** Karneval tähendab pidustust või karnevali, kuid Faschingsball on konkreetselt karnevaliball.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4256
**Audit ID:** ET-B1-4256
**Card ID:** `b1-befragen-278`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** befragen
**LV MASTER reference:** iztaujāt
**CURRENT:** üle küsitlema
**PROPOSED_ET (audit ieteikums):** küsitlema
**Problēma:** Standardne ja loomulikum vaste saksa verbile befragen on küsitlema; üle küsitlema on ebaloomulik ühend.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4257
**Audit ID:** ET-B1-4257
**Card ID:** `b1-Begleitung-294`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Begleitung
**LV MASTER reference:** pavadījums
**CURRENT:** saatel
**PROPOSED_ET (audit ieteikums):** saatmine
**Problēma:** „Saatel” tähendab „saatel/koos saatjaga” ega ole siin loomulik nimisõnaline vaste sõnale „Begleitung”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4258
**Audit ID:** ET-B1-4258
**Card ID:** `b1-Behälter-302`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Behälter
**LV MASTER reference:** tvertne
**CURRENT:** kanister
**PROPOSED_ET (audit ieteikums):** mahuti
**Problēma:** „Kanister” on kindlat tüüpi anum, „Behälter” on üldisem mahuti või anum.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4259
**Audit ID:** ET-B1-4259
**Card ID:** `b1-beherrschen-306`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** beherrschen
**LV MASTER reference:** pārvaldīt
**CURRENT:** valitsema
**PROPOSED_ET (audit ieteikums):** valdama
**Problēma:** „Valitsema” tähendab peamiselt valitsemist; oskuse või keele valdamiseks on õige „valdama”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4260
**Audit ID:** ET-B1-4260
**Card ID:** `b1-behindern-308`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** behindern
**LV MASTER reference:** traucēt
**CURRENT:** segama
**PROPOSED_ET (audit ieteikums):** takistama
**Problēma:** „Behindern” tähendab eeskätt takistamist või tõkestamist; „segama” tähendab tavaliselt häirimist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4261
**Audit ID:** ET-B1-4261
**Card ID:** `b1-benachbart-340`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** STRUCTURE
**DE (read-only):** benachbart
**LV MASTER reference:** kaimiņu
**CURRENT:** naabri-
**PROPOSED_ET (audit ieteikums):** naabruses asuv
**Problēma:** “naabri-” is only a combining form and cannot function as a standalone Estonian translation.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4262
**Audit ID:** ET-B1-4262
**Card ID:** `b1-Berechnung-351`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Berechnung
**LV MASTER reference:** aprēķins
**CURRENT:** arvestus
**PROPOSED_ET (audit ieteikums):** arvutus
**Problēma:** “Arvestus” usually means accounting or keeping records; “arvutus” directly means calculation.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4263
**Audit ID:** ET-B1-4263
**Card ID:** `b1-Bergbahn-360`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Bergbahn
**LV MASTER reference:** pacēlājs kalnos
**CURRENT:** mäestikutross
**PROPOSED_ET (audit ieteikums):** mägiraudtee
**Problēma:** “Mäestikutross” is not a natural Estonian term; “mägiraudtee” is the standard translation for Bergbahn.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4264
**Audit ID:** ET-B1-4264
**Card ID:** `b1-Bescheid-377`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Bescheid
**LV MASTER reference:** atbilde
**CURRENT:** vastus
**PROPOSED_ET (audit ieteikums):** teatis
**Problēma:** As a noun, “Bescheid” usually means an official notice or notification, not simply an answer.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4265
**Audit ID:** ET-B1-4265
**Card ID:** `b1-besehen-384`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** besehen
**LV MASTER reference:** apskatīt
**CURRENT:** vaatama
**PROPOSED_ET (audit ieteikums):** üle vaatama
**Problēma:** “Besehen” means to inspect or examine; the generic “vaatama” is too broad.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4266
**Audit ID:** ET-B1-4266
**Card ID:** `b1-beseitigen-385`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** beseitigen
**LV MASTER reference:** novērst
**CURRENT:** ära hoidma
**PROPOSED_ET (audit ieteikums):** kõrvaldama
**Problēma:** “Ära hoidma” means prevent, whereas “beseitigen” means remove, eliminate, or rectify.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4267
**Audit ID:** ET-B1-4267
**Card ID:** `b1-besichtigen-387`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** besichtigen
**LV MASTER reference:** apskatīt
**CURRENT:** vaatama
**PROPOSED_ET (audit ieteikums):** külastama
**Problēma:** “Besichtigen” means to visit or inspect a place or sight; “vaatama” is too generic.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4268
**Audit ID:** ET-B1-4268
**Card ID:** `b1-beunruhigen-416`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** beunruhigen
**LV MASTER reference:** satraukt
**CURRENT:** häirima
**PROPOSED_ET (audit ieteikums):** rahutuks tegema
**Problēma:** „Häirima” tähendab peamiselt häirima või segama; „beunruhigen” tähendab kedagi rahutuks või murelikuks tegema.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4269
**Audit ID:** ET-B1-4269
**Card ID:** `b1-sich blamieren-453`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** sich blamieren
**LV MASTER reference:** izblamēties
**CURRENT:** end häbisse tegema
**PROPOSED_ET (audit ieteikums):** ennast häbistama
**Problēma:** Reflexive construction is unnatural; “ennast häbistama” is the standard Estonian equivalent.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4270
**Audit ID:** ET-B1-4270
**Card ID:** `b1-Brathuhn-490`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Brathuhn
**LV MASTER reference:** cepta vista
**CURRENT:** praetud kana
**PROPOSED_ET (audit ieteikums):** ahjukana
**Problēma:** “Praetud kana” means fried chicken; Brathuhn is typically roast or oven-baked chicken.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4271
**Audit ID:** ET-B1-4271
**Card ID:** `b1-Büchse-518`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Büchse
**LV MASTER reference:** kārba
**CURRENT:** karp
**PROPOSED_ET (audit ieteikums):** plekkpurk
**Problēma:** Büchse tähendab siin eeskätt plekist purki või konservipurki; „karp“ on liiga üldine ja ebatäpne.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4272
**Audit ID:** ET-B1-4272
**Card ID:** `b1-Camp-534`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Camp
**LV MASTER reference:** nometne ar teltīm vai koka mājiņām
**CURRENT:** telkla
**PROPOSED_ET (audit ieteikums):** laager
**Problēma:** „Telkla“ ei ole korrektne eestikeelne sõna; Camp vaste on „laager“ või täpsemalt „telklaager“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4273
**Audit ID:** ET-B1-4273
**Card ID:** `b1-checken-540`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** checken
**LV MASTER reference:** pārbaudīt
**CURRENT:** üle kontrollima
**PROPOSED_ET (audit ieteikums):** kontrollima
**Problēma:** „Üle kontrollima“ tähendab pigem uuesti või üle kontrollima; checken vaste on üldisem „kontrollima“.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4274
**Audit ID:** ET-B1-4274
**Card ID:** `b1-Chipkarte-543`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Chipkarte
**LV MASTER reference:** maksājumu karte
**CURRENT:** maksekaart
**PROPOSED_ET (audit ieteikums):** kiipkaart
**Problēma:** Chipkarte on üldmõiste kiibiga kaardi kohta; „maksekaart“ kitsendab tähenduse ainult maksmiseks kasutatavale kaardile.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4275
**Audit ID:** ET-B1-4275
**Card ID:** `b1-demonstrieren-575`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** demonstrieren
**LV MASTER reference:** demonstrēt
**CURRENT:** esitama
**PROPOSED_ET (audit ieteikums):** demonstreerima
**Problēma:** „Esitama“ tähendab esitama või ette kandma, mitte demonstreerima.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4276
**Audit ID:** ET-B1-4276
**Card ID:** `b1-Dose-601`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Dose
**LV MASTER reference:** kārba
**CURRENT:** karp
**PROPOSED_ET (audit ieteikums):** purk
**Problēma:** „Karp“ tähendab karpi või karbikest; saksa „Dose“ on tavaliselt purk või plekkpurk.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4277
**Audit ID:** ET-B1-4277
**Card ID:** `b1-dritt-610`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** dritt
**LV MASTER reference:** treškārt
**CURRENT:** kolmandaks
**PROPOSED_ET (audit ieteikums):** kolmekesi
**Problēma:** „Kolmandaks“ tähendab „drittens“ ehk kolmandaks; „dritt“ tähendab tavaliselt kolmekesi või kolmekaupa.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4278
**Audit ID:** ET-B1-4278
**Card ID:** `b1-Drogerie-613`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Drogerie
**LV MASTER reference:** drogu preču veikals
**CURRENT:** kaubamaja väiketarvete osakond
**PROPOSED_ET (audit ieteikums):** kosmeetika- ja hügieenitarvete pood
**Problēma:** Praegune vaste kirjeldab kaubamaja osakonda, mitte tavapärast kosmeetika- ja hügieenitarvete poodi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4279
**Audit ID:** ET-B1-4279
**Card ID:** `b1-einordnen-696`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** einordnen
**LV MASTER reference:** iekļaut
**CURRENT:** integreerima
**PROPOSED_ET (audit ieteikums):** liigitama
**Problēma:** „Einordnen“ tähendab eeskätt liigitamist või konteksti paigutamist, mitte integreerimist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4280
**Audit ID:** ET-B1-4280
**Card ID:** `b1-Einreise-698`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Einreise
**LV MASTER reference:** ieceļošana
**CURRENT:** sisserändamine
**PROPOSED_ET (audit ieteikums):** riiki sisenemine
**Problēma:** „Sisserändamine“ tähendab immigratsiooni; „Einreise“ on riiki sisenemine või saabumine.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4281
**Audit ID:** ET-B1-4281
**Card ID:** `b1-Einrichtung-700`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Einrichtung
**LV MASTER reference:** iekārta
**CURRENT:** seade
**PROPOSED_ET (audit ieteikums):** sisustus
**Problēma:** „Seade“ tähendab peamiselt üksikut aparaati; „Einrichtung“ tähendab siin pigem sisustust või sisseseadet.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4282
**Audit ID:** ET-B1-4282
**Card ID:** `b1-einschließen-703`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** einschließen
**LV MASTER reference:** ieskaitīt
**CURRENT:** kaasa arvestama
**PROPOSED_ET (audit ieteikums):** hõlmama
**Problēma:** „Kaasa arvestama“ tähendab arvesse võtma; „einschließen“ tähendab siin hõlmama või kaasama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4283
**Audit ID:** ET-B1-4283
**Card ID:** `b1-einstöckig-713`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** einstöckig
**LV MASTER reference:** vienstāva-
**CURRENT:** ühekordne
**PROPOSED_ET (audit ieteikums):** ühekorruseline
**Problēma:** „Ühekordne“ tähendab ühekordset; „einstöckig“ tähendab ühe korrusega või ühekorruselist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4284
**Audit ID:** ET-B1-4284
**Card ID:** `b1-enttäuschen-763`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** enttäuschen
**LV MASTER reference:** pievilt
**CURRENT:** petta jätma
**PROPOSED_ET (audit ieteikums):** pettumust valmistama
**Problēma:** „Petta jätma” ei ole tavapärane vaste tähendusele „pettumust valmistama” või „alt vedama”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4285
**Audit ID:** ET-B1-4285
**Card ID:** `b1-erfolglos-777`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** ORTHOGRAPHY
**DE (read-only):** erfolglos
**LV MASTER reference:** neveiksmīgs
**CURRENT:** ebaedukas
**PROPOSED_ET (audit ieteikums):** ebaedukas
**Problēma:** Estonian õige kirjapilt sisaldab tähte ä: „ebaedukas”, mitte „ebaedukas”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---