# ET–DE A1 — OWNER VIEW (grupa 50, 2451–2500)

**Standard:** `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9
**Auditors:** deterministika + GPT-5.6 Luna (READ-ONLY)
**Audit PR:** [#610](https://github.com/sandrisbrikmanis-rgb/de-lv-app/pull/610)

| Navigācija | Saite |
|------------|-------|
| GitHub indekss | [et-b1-owner-review-GITHUB.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-review-GITHUB.md) |
| OWNER VIEW (visi) | [et-b1-owner-view.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-view.md) |
| Decisions (šī grupa) | [et-b1-owner-decisions-group50.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions-group50.md) |
| Decisions (viss) | [et-b1-owner-decisions.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-owner-decisions.md) |

Avots: [et-b1-full-audit.md](https://github.com/sandrisbrikmanis-rgb/de-lv-app/blob/cursor/et-de-b1-full-audit-4a7c/reports/et-b1-full-audit.md)

## ET-B1-4336
**Audit ID:** ET-B1-4336
**Card ID:** `b1-Makler-1824`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Makler
**LV MASTER reference:** aģents
**CURRENT:** agent
**PROPOSED_ET (audit ieteikums):** maakler
**Problēma:** „Agent” is a broader term; German „Makler” specifically means a broker or intermediary, commonly „maakler” in Estonian.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4337
**Audit ID:** ET-B1-4337
**Card ID:** `b1-Maskenfest-1843`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Maskenfest
**LV MASTER reference:** masku balle
**CURRENT:** maskiball
**PROPOSED_ET (audit ieteikums):** maskipidu
**Problēma:** Maskenfest tähendab maskipidu või maskifestivali, mitte kitsalt maskiballi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4338
**Audit ID:** ET-B1-4338
**Card ID:** `b1-mehrfach-1859`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** mehrfach
**LV MASTER reference:** vairākkārtīgs
**CURRENT:** korduv
**PROPOSED_ET (audit ieteikums):** mitmekordne
**Problēma:** Korduv tähendab 'wiederholt'; mehrfach tähendab 'mehrfach/mehrere Male' või 'mitmekordne'.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4339
**Audit ID:** ET-B1-4339
**Card ID:** `b1-Mohn-1897`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Mohn
**LV MASTER reference:** magone
**CURRENT:** mooniõis
**PROPOSED_ET (audit ieteikums):** moon
**Problēma:** Moon on moonitaim; mooniõis tähendab kitsamalt selle taime õit, mitte taime üldnimetust.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4340
**Audit ID:** ET-B1-4340
**Card ID:** `b1-Molkerei-1898`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Molkerei
**LV MASTER reference:** pienotava
**CURRENT:** piimatööstus
**PROPOSED_ET (audit ieteikums):** meierei
**Problēma:** Molkerei on meierei või piimatöötlemisettevõte; piimatööstus tähistab kogu tööstusharu.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4341
**Audit ID:** ET-B1-4341
**Card ID:** `b1-Monatsgehalt-1900`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Monatsgehalt
**LV MASTER reference:** ierēdņa vai kalpotāja alga
**CURRENT:** ametipalk
**PROPOSED_ET (audit ieteikums):** kuupalk
**Problēma:** Monatsgehalt tähendab kuupalka; ametipalk tähendab ametikohaga seotud palka ega pruugi olla kuupalk.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4342
**Audit ID:** ET-B1-4342
**Card ID:** `b1-nachahmen-1940`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** nachahmen
**LV MASTER reference:** atdarināt kādu
**CURRENT:** kellegi järele tegema
**PROPOSED_ET (audit ieteikums):** jäljendama
**Problēma:** Kellegi järele tegema on selles tähenduses ebaloomulik; jäljendama on täpne ja loomulik vaste.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4343
**Audit ID:** ET-B1-4343
**Card ID:** `b1-Notdienst-1996`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Notdienst
**LV MASTER reference:** avārijas dienests
**CURRENT:** häirekeskus
**PROPOSED_ET (audit ieteikums):** valveteenistus
**Problēma:** „Häirekeskus” tähendab häirekeskust; Notdienst on valve- või erakorraline teenistus.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4344
**Audit ID:** ET-B1-4344
**Card ID:** `b1-Panne-2053`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Panne
**LV MASTER reference:** ķibele
**CURRENT:** pahandus
**PROPOSED_ET (audit ieteikums):** äpardus
**Problēma:** Pahandus tähendab pigem pahategu või tüli; Panne on äpardus või rike.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4345
**Audit ID:** ET-B1-4345
**Card ID:** `b1-Pastor-2070`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Pastor
**LV MASTER reference:** luterāņu mācītājs
**CURRENT:** luterlik pastor
**PROPOSED_ET (audit ieteikums):** pastor
**Problēma:** Luterlik lisab saksa sõnas puuduvat kitsenduse; Pastor tähendab üldiselt pastorit või vaimulikku.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4346
**Audit ID:** ET-B1-4346
**Card ID:** `b1-Pellkartoffel-2074`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** NATURALNESS
**DE (read-only):** Pellkartoffel
**LV MASTER reference:** ar mizu vārīts kartupelis
**CURRENT:** koorimata keedukartul
**PROPOSED_ET (audit ieteikums):** koorega keedetud kartul
**Problēma:** Pellkartoffel on koorega keedetud kartul; „koorimata keedukartul” on ebaloomulik ja võib tähendada lihtsalt koorimata kartulit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4347
**Audit ID:** ET-B1-4347
**Card ID:** `b1-Pendelbus-2077`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Pendelbus
**LV MASTER reference:** piepilsētas autobuss
**CURRENT:** eeslinnabuss
**PROPOSED_ET (audit ieteikums):** pendelbuss
**Problēma:** Pendelbus tähendab edasi-tagasi sõitvat süstik- või pendelbussi, mitte tingimata eeslinna bussi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4348
**Audit ID:** ET-B1-4348
**Card ID:** `b1-Pendelzug-2078`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** Pendelzug
**LV MASTER reference:** piepilsētas vilciens
**CURRENT:** eeslinnarong
**PROPOSED_ET (audit ieteikums):** pendelrong
**Problēma:** Pendelzug tähendab edasi-tagasi sõitvat süstik- või pendelrongi, mitte tingimata eeslinna rongi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4349
**Audit ID:** ET-B1-4349
**Card ID:** `b1-per-2079`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** per
**LV MASTER reference:** pa
**CURRENT:** kaudu
**PROPOSED_ET (audit ieteikums):** kaupa
**Problēma:** Saksa per väljendab määra või ühiku kohta; „kaudu” tähendab vahendust või läbimist.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4350
**Audit ID:** ET-B1-4350
**Card ID:** `b1-Pfeiler-2092`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Pfeiler
**LV MASTER reference:** pīlārs
**CURRENT:** pilaster
**PROPOSED_ET (audit ieteikums):** sammas
**Problēma:** Pfeiler on üldine sammas või tugisammas; „pilaster” on kitsam, seina külge seotud arhitektuurielement.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4351
**Audit ID:** ET-B1-4351
**Card ID:** `b1-pflegeleicht-2098`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** pflegeleicht
**LV MASTER reference:** viegli kopjams
**CURRENT:** hooldusvaba
**PROPOSED_ET (audit ieteikums):** kergesti hooldatav
**Problēma:** „Hooldusvaba” tähendab hooldust mittevajavat; pflegeleicht tähendab kergesti hooldatavat.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4352
**Audit ID:** ET-B1-4352
**Card ID:** `b1-plangemäß-2118`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** plangemäß
**LV MASTER reference:** plānveidīgs
**CURRENT:** planeeritud
**PROPOSED_ET (audit ieteikums):** plaanikohane
**Problēma:** „Planeeritud” tähendab kavandatud; „plangemäß” tähendab plaani kohaselt või plaanile vastavalt.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4353
**Audit ID:** ET-B1-4353
**Card ID:** `b1-planmäßig-2119`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** planmäßig
**LV MASTER reference:** plānveidīgs
**CURRENT:** planeeritud
**PROPOSED_ET (audit ieteikums):** plaanipärane
**Problēma:** „Planeeritud” tähendab kavandatud; „planmäßig” tähendab plaani järgi toimuvat või plaanipärast.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4354
**Audit ID:** ET-B1-4354
**Card ID:** `b1-Polster-2142`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** SEMANTICS
**DE (read-only):** Polster
**LV MASTER reference:** polsteris dīvānam
**CURRENT:** diivanipolster
**PROPOSED_ET (audit ieteikums):** polster
**Problēma:** „Polster” on üldisem padi või pehmendus; „diivanipolster” kitsendab tähenduse diivani polstriks.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4355
**Audit ID:** ET-B1-4355
**Card ID:** `b1-pro-2164`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** pro
**LV MASTER reference:** par
**CURRENT:** eest
**PROPOSED_ET (audit ieteikums):** kohta
**Problēma:** „Eest” tähendab vastutasuks; saksa „pro” tähenduses „per” vastab eesti keeles „kohta”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4356
**Audit ID:** ET-B1-4356
**Card ID:** `b1-Profit-2168`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Profit
**LV MASTER reference:** peļņa
**CURRENT:** teenistus
**PROPOSED_ET (audit ieteikums):** kasum
**Problēma:** „Teenistus” tähendab sissetulekut või töötasu, „Profit” tähendab kasumit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4357
**Audit ID:** ET-B1-4357
**Card ID:** `b1-Prüfzeit-2178`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Prüfzeit
**LV MASTER reference:** pārbaudes laiks
**CURRENT:** katseaeg
**PROPOSED_ET (audit ieteikums):** kontrolliaeg
**Problēma:** „Katseaeg” tähendab eesti keeles peamiselt prooviperioodi või katseaega tööl; „Prüfzeit” on kontrollimise aeg.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4358
**Audit ID:** ET-B1-4358
**Card ID:** `b1-Rasse-2221`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Rasse
**LV MASTER reference:** šķirne
**CURRENT:** sort
**PROPOSED_ET (audit ieteikums):** tõug
**Problēma:** “Sort” tähendab eeskätt liiki või sorti; Rasse vastena on loomade puhul täpsem “tõug”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4359
**Audit ID:** ET-B1-4359
**Card ID:** `b1-Rollschuh-2332`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Rollschuh
**LV MASTER reference:** skrituļkurpes
**CURRENT:** rulluisud
**PROPOSED_ET (audit ieteikums):** rulluisk
**Problēma:** Saksa ainsus Rollschuh tähendab üht rulluisku; praegune tõlge on mitmus.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4360
**Audit ID:** ET-B1-4360
**Card ID:** `b1-rostig-2337`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** rostig
**LV MASTER reference:** sarūsējis
**CURRENT:** roostetanud
**PROPOSED_ET (audit ieteikums):** roostes
**Problēma:** Rostig tähendab omadust „roostes”; roostetanud tähendab pigem juba roostest kahjustatud või kattunud.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4361
**Audit ID:** ET-B1-4361
**Card ID:** `b1-Ruck-2341`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Ruck
**LV MASTER reference:** rāviens
**CURRENT:** tõmme
**PROPOSED_ET (audit ieteikums):** jõnks
**Problēma:** Ruck on järsk jõnks või nõks; tõmme tähendab pigem tõmbamist või vedu.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4362
**Audit ID:** ET-B1-4362
**Card ID:** `b1-Rührei-2357`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** GRAMMAR
**DE (read-only):** Rührei
**LV MASTER reference:** olu kultenis
**CURRENT:** munaputru
**PROPOSED_ET (audit ieteikums):** munapuder
**Problēma:** Sõnavorm peab olema algvormis; munaputru on partitiiv, mitte leksikaalne vaste „scrambled eggs”.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4363
**Audit ID:** ET-B1-4363
**Card ID:** `b1-Rundfahrt-2361`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** NATURALNESS
**DE (read-only):** Rundfahrt
**LV MASTER reference:** ekskursijas brauciens
**CURRENT:** ekskursioonisõit
**PROPOSED_ET (audit ieteikums):** ringsõit
**Problēma:** Ekskursioonisõit on ebaloomulik liitsõna; Rundfahrt vaste on loomulikumalt ringsõit.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4364
**Audit ID:** ET-B1-4364
**Card ID:** `b1-Sanitäter-2385`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Sanitäter
**LV MASTER reference:** sanitārs
**CURRENT:** sanitar
**PROPOSED_ET (audit ieteikums):** parameedik
**Problēma:** Sanitar tähendab eesti keeles peamiselt haiglaabilist; Sanitäter on parameedik või kiirabitöötaja.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4365
**Audit ID:** ET-B1-4365
**Card ID:** `b1-saufen-2391`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** saufen
**LV MASTER reference:** žūpot
**CURRENT:** priiskama
**PROPOSED_ET (audit ieteikums):** purjutama
**Problēma:** Priiskama tähendab raiskama; saufen on alkoholi liigtarvitama või purjutama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4366
**Audit ID:** ET-B1-4366
**Card ID:** `b1-Schall-2406`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** Schall
**LV MASTER reference:** skaņa
**CURRENT:** hääl
**PROPOSED_ET (audit ieteikums):** heli
**Problēma:** Hääl tähendab inimese häält; Schall on üldiselt heli.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4367
**Audit ID:** ET-B1-4367
**Card ID:** `b1-scheren-2424`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** SEMANTICS
**DE (read-only):** scheren
**LV MASTER reference:** cirpt
**CURRENT:** niitma
**PROPOSED_ET (audit ieteikums):** pügama
**Problēma:** Niitma tähendab rohtu või vilja lõikama; scheren tähendab karva või villa pügama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4368
**Audit ID:** ET-B1-4368
**Card ID:** `b1-Scheune-2426`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Scheune
**LV MASTER reference:** šķūnis
**CURRENT:** kuur
**PROPOSED_ET (audit ieteikums):** küün
**Problēma:** Kuur on üldine shed; Scheune tähendab eesti keeles küüni ehk põllumajanduslikku heinahoonet.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4369
**Audit ID:** ET-B1-4369
**Card ID:** `b1-Schiene-2430`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** GRAMMAR
**DE (read-only):** Schiene
**LV MASTER reference:** sliede
**CURRENT:** rööpas
**PROPOSED_ET (audit ieteikums):** rööbas
**Problēma:** Rööpas on seesütleva käände vorm; Schiene vaste on nimetavas käändes rööbas.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4370
**Audit ID:** ET-B1-4370
**Card ID:** `b1-schleppen-2461`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** schleppen
**LV MASTER reference:** stiept
**CURRENT:** venitama
**PROPOSED_ET (audit ieteikums):** lohistama
**Problēma:** Venitama tähendab venitama või pikalt tegema; schleppen tähendab midagi lohistama või vaevaliselt vedama.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4371
**Audit ID:** ET-B1-4371
**Card ID:** `b1-schlingen-2467`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** schlingen
**LV MASTER reference:** vīt
**CURRENT:** punuma
**PROPOSED_ET (audit ieteikums):** põimuma
**Problēma:** Schlingen tähendab selles tähenduses põimuma või ümber keerduma; punuma tähendab eeskätt punuma või kuduma.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4372
**Audit ID:** ET-B1-4372
**Card ID:** `b1-Schnalle-2490`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Schnalle
**LV MASTER reference:** sprādze
**CURRENT:** pandel
**PROPOSED_ET (audit ieteikums):** pannal
**Problēma:** Schnalle tähendab eesti keeles „pannal“; „pandel“ ei ole selle tähenduse korrektne vaste.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4373
**Audit ID:** ET-B1-4373
**Card ID:** `b1-schonen-2507`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** schonen
**LV MASTER reference:** saudzēt
**CURRENT:** hoidma
**PROPOSED_ET (audit ieteikums):** säästma
**Problēma:** „Schonen“ tähendab siin säästmist või säästvalt kohtlemist; „hoidma“ on liiga üldine ja ebatäpne.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4374
**Audit ID:** ET-B1-4374
**Card ID:** `b1-Schrecken-2517`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Schrecken
**LV MASTER reference:** izbailes
**CURRENT:** kohkumus
**PROPOSED_ET (audit ieteikums):** hirm
**Problēma:** „Schrecken“ tähendab tavaliselt hirmu või õudust; „kohkumus“ tähistab pigem ehmumist kui kestvat hirmu.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4375
**Audit ID:** ET-B1-4375
**Card ID:** `b1-Schubkarren-2523`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** LOW
**Category:** SEMANTICS
**DE (read-only):** Schubkarren
**LV MASTER reference:** ķerra
**CURRENT:** käru
**PROPOSED_ET (audit ieteikums):** käsikäru
**Problēma:** „Käru“ on üldnimetus; „Schubkarren“ tähendab täpsemalt käsikäru ehk aiakäru.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4376
**Audit ID:** ET-B1-4376
**Card ID:** `b1-Schuld-2526`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** GRAMMAR
**DE (read-only):** Schuld
**LV MASTER reference:** būt vainīgam
**CURRENT:** süüdi olema
**PROPOSED_ET (audit ieteikums):** süü
**Problēma:** Saksa sõna „Schuld“ on nimisõna, kuid „süüdi olema“ on verbifraas; vorm ja sõnaliik ei vasta.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4377
**Audit ID:** ET-B1-4377
**Card ID:** `b1-Schwiele-2560`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Schwiele
**LV MASTER reference:** tulzna
**CURRENT:** vill
**PROPOSED_ET (audit ieteikums):** nahapaksend
**Problēma:** „Vill” tähendab eesti keeles peamiselt villi; „Schwiele” on naha paksend ehk kallus.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4378
**Audit ID:** ET-B1-4378
**Card ID:** `b1-schwindeln-2564`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** TRANSLATION
**DE (read-only):** schwindeln
**LV MASTER reference:** reibt
**CURRENT:** peapööritust tundma
**PROPOSED_ET (audit ieteikums):** valetama
**Problēma:** „Schwindeln” tähendab valetama või petma, mitte peapööritust tundma.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4379
**Audit ID:** ET-B1-4379
**Card ID:** `b1-Seegang-2573`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Seegang
**LV MASTER reference:** jūras viļņošanās
**CURRENT:** merelaine
**PROPOSED_ET (audit ieteikums):** merelainetus
**Problēma:** „Merelaine” on üks merelaine; „Seegang” tähendab mere lainetust või lainetust tervikuna.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4380
**Audit ID:** ET-B1-4380
**Card ID:** `b1-Seefahrt-2574`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Seefahrt
**LV MASTER reference:** kuģošana
**CURRENT:** laevasõit
**PROPOSED_ET (audit ieteikums):** meresõit
**Problēma:** „Laevasõit” tähendab pigem laevareisi; „Seefahrt” on meresõit või merel sõitmine üldiselt.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4381
**Audit ID:** ET-B1-4381
**Card ID:** `b1-Seifenschale-2587`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** TRANSLATION
**DE (read-only):** Seifenschale
**LV MASTER reference:** ziepju trauciņš
**CURRENT:** seebikarp
**PROPOSED_ET (audit ieteikums):** seebialus
**Problēma:** „Seebikarp” on seebikarp või -karp; „Seifenschale” tähendab seebialust või seebikaussi.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4382
**Audit ID:** ET-B1-4382
**Card ID:** `b1-seitwärts-2590`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** seitwärts
**LV MASTER reference:** uz sāniem
**CURRENT:** kõrvale
**PROPOSED_ET (audit ieteikums):** külgsuunas
**Problēma:** „Kõrvale” tähendab pigem kõrvale või eemale; „seitwärts” tähendab külgsuunas.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4383
**Audit ID:** ET-B1-4383
**Card ID:** `b1-Slalomlauf-2640`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Slalomlauf
**LV MASTER reference:** slaloms
**CURRENT:** slaalom
**PROPOSED_ET (audit ieteikums):** slaalomisõit
**Problēma:** Slaalom tähendab slalomit üldiselt ega väljenda saksa sõna Lauf ehk sõitu/jooksu.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4384
**Audit ID:** ET-B1-4384
**Card ID:** `b1-Sorge-2654`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** MEDIUM
**Category:** SEMANTICS
**DE (read-only):** Sorge
**LV MASTER reference:** rūpes
**CURRENT:** mured
**PROPOSED_ET (audit ieteikums):** mure
**Problēma:** Sorge is singular in German; mured is the Estonian plural of mure.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---
## ET-B1-4385
**Audit ID:** ET-B1-4385
**Card ID:** `b1-Spange-2666`
**Field/path:** `etText`
**Production file:** `data/et/b1.js`
**Severity:** HIGH
**Category:** ORTHOGRAPHY
**DE (read-only):** Spange
**LV MASTER reference:** sprādze
**CURRENT:** pandel
**PROPOSED_ET (audit ieteikums):** pannal
**Problēma:** The Estonian word for a clasp or buckle is pannal; pandel is not the correct lemma here.
**Avots:** gpt-5.6-luna
**OWNER STATUS:** PENDING
**OWNER_DECISION:** [nav aizpildīts]
> PROPOSED_ET nav OWNER apstiprināts. DE lauki nemainīt.
---