...
## ⚖️ STINGRI NOTEIKUMI SALĪDZINĀJUMA TABULĀM (COMPARISON LOGIC)

1. **"abholen" KARTĪTE IR ZELTA STANDARTS:** Jebkurai kartītei, kurai tiek veidota salīdzinājuma sadaļa (`comparisonTable`), satura dziļumam un pedagoģiskajai jēgai ir jābūt identiskai ar "abholen" paraugu.
2. **AIZLIEGTI MĀKSLĪGI / BURTU SARAKSTI (Kā "anfechten"):** Ir kategoriski aizliegts tabulā grupēt vārdus tikai tāpēc, ka tie sākas ar vienu burtu (piemēram, "anfechten" un "anderweitig"). Tā ir kritiska satura kļūda.
3. **TIKAI REĀLI JAUCAMI VĀRDI:** Salīdzinājuma tabulu drīkst ģenerēt TIKAI tad, ja atlasītie vārdi ir:
   - **Īsti sinonīmi** ar smalkām nozīmes atšķirībām (piemēram, *abholen* vs *bekommen* vs *nehmen*).
   - Vārdi, kurus studenti **reāli jauc ikdienas praksē**.
4. **AIZLIEGTA SATURA DUBLĒŠANA:** Tabulā nedrīkst vairākas reizes atkārtot vienu un to pašu vārdu ar dažādiem tulkojumiem (kā tas bija nepareizajā "anfechten" piemērā). Katrai rindai jābūt ar jaunu, unikālu vārdu.
5. **DABISKI UN UNIKĀLI PIEMĒRI TABULĀ:** Katram tabulā iekļautajam salīdzinājuma vārdam OBLIGĀTI jāuzģenerē savs, unikāls piemēra teikums ar tulkojumu (piemēram, *Ich hole Wasser = Es atnesu ūdeni*), kas skaidri ilustrē tieši šī vārda specifisko jēgu un atšķirību.
6. **JA NAV KO SALĪDZINĀT -> "standardStudy":** Ja vārdam nav dabiski jaucamu sinonīmu vai prievārdu pāru, tabula ir OBLIGĀTI JĀNOŅEM. Šādā gadījumā kartītei ir jāizmanto tīrs `layout: "standardStudy"` formāts (tikai ar sadaļām: Galvenā doma, Skaidrojums, Piemēri, Padoms, Svarīgi).  
