...
"use strict";
/**
 * ET B1 multi-translation OWNER review (MASTER v1.12) — analysis only, no repair.
 */
const fs = require("fs");
...
const OUT_VIEW = path.join(ROOT, "reports/et-b1-multitranslation-owner-view.md");
const OUT_DEC = path.join(ROOT, "reports/et-b1-multitranslation-owner-decisions.md");
const OUT_JSON = path.join(ROOT, "reports/temp/et-b1-multitranslation-owner-review.json");
...
function germanSense(de, candidate) {
  ...
  return `Learner-facing Estonian sense for German «${de}» at B1 level`;
}
...
function candidateRelation(de, candidates) {
  ...
  return candidates.length === 2
    ? "Two distinct B1-level senses — evaluate which is primary for the learner-facing main field."
...
function recommendationJustification(de, candidates, recommended, entry, field) {
  ...
  const studyHint = study
    ...
    : " Ordinary card — pick the most frequent practical B1 gloss for flashcard front.";
  return `Recommended **${recommended}** as the primary B1 learner-facing gloss (${gs}). ${rel}${studyHint}`;
}
...
function suitability(de, candidate, candidates, entry, field, recommended) {
  const c = candidate.trim();
  if (c === recommended) return "Strong candidate for single main translation at B1.";
...
function build() {
  ...
  for (let i = 0; i < findings.length; i++) {
    ...
    const auditId = `ET-B1-MT-${String(i + 1).padStart(4, "0")}`;
  ...
  const productionBlob = execSync("git hash-object data/et/b1.js", { cwd: ROOT, encoding: "utf8" }).trim();
  ...
  const view = [
    "# ET–DE B1 — Multi-translation OWNER VIEW",
    "",
    `**Generated:** ${generatedAt}`,
    `**MASTER:** v1.12`,
    `**ORIGIN_MAIN_SHA:** \`${originSha}\``,
    `**B1_PRODUCTION_BLOB:** \`${productionBlob}\``,
    ...
    "## Verdict",
    "",
    "**ET_B1_MULTITRANSLATION_OWNER_REVIEW_READY**",
    "",
  ];
  ...
  const jsonPayload = {
    ...
    productionBlob,
    verdict: "ET_B1_MULTITRANSLATION_OWNER_REVIEW_READY",
