...
"use strict";
/**
 * FR-DE B1 FIRST FULL DISCOVERY audit orchestrator (MASTER v1.9, READ-ONLY).
 * Usage: node scripts/run-fr-de-b1-full-audit.js [--skip-luna] [--test-luna] [--fresh-luna]
 */
...
const {
  ...
  TOTAL_CARDS,
  STUDY_COUNT,
} = require("./lib/fr-b1-discovery-config");
const {
  LUNA_JSON,
  buildCards,
  entryId,
  mapSeverity,
  mapCategory,
} = require("./lib/fr-b1-audit-helpers");
const { classifyFindings, mapValidationStatus, NON_ERROR_CATEGORIES } = require("./lib/openai-fr-b1-audit");
...
const OUT_MD = path.join(ROOT, "reports/fr-de-b1-full-audit.md");
const OUT_JSON = path.join(ROOT, "reports/fr-de-b1-full-audit.json");
const OUT_SUMMARY = path.join(ROOT, "reports/fr-de-b1-full-audit-summary.md");
const COLLECT_JSON = path.join(ROOT, "reports/temp/fr-de-b1-audit-data.json");
...
function consolidateOwnerObjects(findings, words) {
  ...
  for (const [, row] of byTarget) {
    ...
    owners.push({
      id: `FR-DE-B1-OWNER-${String(seq++).padStart(4, "0")}`,
...
function buildSummaryMd(ctx) {
  return [
    ...
    "",
    "Pilns audits: `reports/fr-de-b1-full-audit.md`",
    "OWNER skats: `reports/fr-de-b1-full-audit-owner-view.md`",
    "",
  ].join("\n");
}
...
function buildFullMd(ctx) {
  const lines = [
    "# FR–DE B1 pilns lingvistiskais audits (MASTER v1.9 FIRST_FULL_DISCOVERY)",
    "",
    `**HEAD:** \`${ctx.head}\``,
    ...
    "| Rīks | Komanda |",
    "|------|---------|",
    "| Kolektors | `node scripts/audit-fr-b1-collect.js` |",
    "| Luna | `node scripts/audit-fr-b1-linguistic.js` |",
    "| Orķestrators | `node scripts/run-fr-de-b1-full-audit.js` |",
    "| Validācija | `node scripts/validate-fr-de-b1-full-audit.js` |",
    "",
  ];
  return lines.join("\n");
}
...
async function main() {
  console.log("\n=== FR–DE B1 FIRST FULL DISCOVERY (MASTER v1.9) ===\n");
  ...
  runNode("audit-fr-b1-collect.js");

  execSync("node --check data/fr/b1.js", { cwd: ROOT, stdio: "pipe" });
  execSync("node --check www/data/fr/b1.js", { cwd: ROOT, stdio: "pipe" });
  ...
  if (FRESH_LUNA) {
    const progress = path.join(ROOT, "scripts/.fr-de-b1-luna-progress.json");
  ...
  if (!SKIP_LUNA) {
    ...
    runNode("audit-fr-b1-linguistic.js", lunaArgs);
  }
  ...
  runNode("build-fr-de-b1-full-audit-owner-source.js");
  runNode("build-fr-de-b1-full-audit-owner-view.js");
  if (coverageComplete) runNode("validate-fr-de-b1-full-audit.js");
