#!/usr/bin/env node
/**
 * BS-DE A1 audit report generator (read-only). Writes reports/temp/bs-a2-findings-consolidated.json
 */
const fs = require("fs");
...
const AUDIT_DATA = path.join(ROOT, "reports", "temp", "bs-a2-audit-data.json");
const OUT = path.join(ROOT, "reports", "temp", "bs-a2-findings-consolidated.json");
...
function main() {
  ...
  function add(f) {
    ...
    seen.add(key);
    findings.push({ ...f, file: "data/bs/a2.js" });
  }
