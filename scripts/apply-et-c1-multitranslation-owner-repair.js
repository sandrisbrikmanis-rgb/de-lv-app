...
"use strict";
/**
 * ET C1 multi-translation OWNER COPY-ONLY apply (102 LABOT, MASTER v1.12).
 */
const fs = require("fs");
...
const APPLY_LOG = path.join(ROOT, "reports/temp/et-c1-multitranslation-owner-apply-log.json");
const REPORT_MD = path.join(ROOT, "reports/et-c1-multitranslation-owner-repair-apply.md");
const DATA_REL = "data/et/c1.js";
const FILES = [path.join(ROOT, DATA_REL), path.join(ROOT, "www/data/et/c1.js")];
const C1_KEY = "C1_WORDS";
...
function loadWords(filePath) {
  ...
  return ctx.window[C1_KEY];
}

function writeWords(filePath, words) {
  fs.writeFileSync(
    filePath,
    `const ${C1_KEY} = ${JSON.stringify(words, null, 2)};\n\nwindow.${C1_KEY} = ${C1_KEY};\n`,
    "utf8",
  );
}
...
function main() {
  ...
  if (apply.length !== 102) {
    console.error(`Expected 102 LABOT rows, got ${apply.length}`);
    process.exit(1);
  }
  ...
  if (!DRY_RUN && log.staged.length > 0) {
    ...
    gitDiffPass = gitDiffNonEmpty([DATA_REL, "www/data/et/c1.js"], ROOT);
  ...
  fs.writeFileSync(
    REPORT_MD,
    [
      "# ET C1 — Multi-translation OWNER COPY-ONLY apply",
      "",
      "**MASTER:** v1.12",
      `**Authority:** \`reports/et-c1-multitranslation-owner-decisions-accepted-materialized.md\``,
