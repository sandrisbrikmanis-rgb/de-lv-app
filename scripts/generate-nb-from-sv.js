#!/usr/bin/env node
/**
 * Generate NB-DE data from SV baseline with batched translation (SV → NB).
...
const OUT_DIR = path.join(ROOT, "data", "nb");
const CACHE_PATH = path.join(ROOT, "scripts", ".nb-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "sv";
const TO_LANG = "no";
...
function loadSvTrainingCardsFromData() {
  const cardsPath = path.join(ROOT, "data/sv/courseTrainingCards.js");
  ...
  vm.runInContext(code, ctx);
  const cards = {};
  const arrayPattern = /window\.(lesson\d+(?:Training|Exercise)CardsSv)\s*=\s*(\[[\s\S]*?\n\]);/g;
  let match;
...
async function main() {
  ...
  console.log("Loading SV baseline and LV German source...");
  ...
  const svA1 = loadData("data/sv/a1.js").A1_WORDS;
  const svA2 = loadData("data/sv/a2.js").A2_WORDS;
  const svB1 = loadData("data/sv/b1.js").B1_WORDS;
  const svB2 = loadData("data/sv/b2.js").B2_WORDS;
  const svC1 = loadData("data/sv/c1.js").C1_WORDS;
  const svC2 = loadData("data/sv/c2.js").C2_WORDS;
  const svSentences = loadData("data/sv/sentences.js").SENTENCE_ENTRIES;
  const svVerbs = loadData("data/sv/verbs.js").VERB_ENTRIES;
  const svDialogue = loadData("data/sv/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const svCourse = loadData("data/sv/courseLessons.js");
  const svTraining = loadSvTrainingCardsFromData();

  [svA1, svA2, svB1, svB2, svC1, svC2, svSentences, svVerbs].forEach((arr) => arr.forEach((e) => collectStrings(e, allStrings)));
  Object.values(svDialogue).forEach((e) => collectStrings(e, allStrings));
  Object.values(svCourse.COURSE_LESSON_DATA || {}).forEach((e) => collectStrings(e, allStrings));
  Object.values(svCourse.COURSE_LESSON_HTML || {}).forEach((html) => {
    [...String(html).matchAll(/>([^<]{3,300})</g)].forEach((m) => {
      const t = m[1].trim();
      if (t) allStrings.add(t);
    });
  });
  Object.values(svTraining).forEach((deck) => {
  ...
  console.log(`Found ${unique.length} unique SV strings (${alreadyCached} cached)`);
  ...
  function mergeLevel(lvArr, svArr) {
    return lvArr.map((lvEntry, i) => {
      const svEntry = svArr[i];
      if (!svEntry || lvEntry.de !== svEntry.de) {
        console.warn(`Order mismatch at ${lvEntry.de}`);
        return lvEntry;
      }
      const trEntry = applyTranslation(svEntry, map);
      return preserveGermanFields(lvEntry, trEntry);
    });
  }

  console.log("\nWriting NB files...");
  writeArrayFile(path.join(OUT_DIR, "a1.js"), "A1_WORDS", mergeLevel(lvA1, svA1));
  writeArrayFile(path.join(OUT_DIR, "a2.js"), "A2_WORDS", mergeLevel(lvA2, svA2));
  writeArrayFile(path.join(OUT_DIR, "b1.js"), "B1_WORDS", mergeLevel(lvB1, svB1));
  writeArrayFile(path.join(OUT_DIR, "b2.js"), "B2_WORDS", mergeLevel(lvB2, svB2));
  writeArrayFile(path.join(OUT_DIR, "c1.js"), "C1_WORDS", mergeLevel(lvC1, svC1));
  writeArrayFile(path.join(OUT_DIR, "c2.js"), "C2_WORDS", mergeLevel(lvC2, svC2));
  writeArrayFile(path.join(OUT_DIR, "sentences.js"), "SENTENCE_ENTRIES", mergeLevel(lvSentences, svSentences));

  const nbVerbs = lvVerbs.map((lvEntry, i) => {
    const svEntry = svVerbs[i];
    const te = {};
    for (const form of Object.keys(lvEntry)) {
      te[form] = {
        de: lvEntry[form].de,
        lv: map[svEntry[form].lv] || svEntry[form].lv,
      };
    }
    return te;
  });
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", nbVerbs);

  const nbDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const svEntry = svDialogue[id];
    nbDialogue[id] = { de: lvEntry.de, lv: map[svEntry.lv] || svEntry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(nbDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  const nbHtml = {};
  for (const [key, html] of Object.entries(svCourse.COURSE_LESSON_HTML || {})) {
    nbHtml[key] = await translateHtmlLesson(html, map);
  }
  const nbData = applyTranslation(svCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(nbHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(nbData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Norwegian Bokmål course training cards for NB-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(svTraining)) {
    if (!Array.isArray(deck)) continue;
    const nbKey = key.replace(/Sv$/, "Nb");
    const translated = deck.map((card) => ({
      front: map[card.front] || card.front,
      back: card.back || "",
    }));
    outLines.push(`window.${nbKey} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nNB-DE data generation complete (SV→NB with LV DE preservation).");
}
