...
for (const row of rows) {
  ...
  if (lang === "is" && LV_LEAK.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_LV_IN_IS", msg: allText.slice(0, 100) });
  }
  if (lang === "is" && ET_LEAK.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_ET_IN_IS", msg: allText.slice(0, 100) });
  }
  if (lang === "is" && HU_IN_IS.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_HU_IN_IS", msg: allText.slice(0, 100) });
  }
  if (lang === "is" && NO_IN_IS.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_NO_IN_IS", msg: allText.slice(0, 100) });
  }
