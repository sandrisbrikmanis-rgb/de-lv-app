...
const BG_MK_SL =
  /\b(Разберете|Германия|Баща|Мога|Познайте|Запомнете|Отговаря|Използва|Доаѓа|Дојдовме|Prosimo|utihnite|čakal|Ves dan|Ranjen|Tudi|nekaj|za seboj|v nemščini)\b/i;
...
function nestedForCard(lang, cardKey) {
  ...
  return (
    ...
    Object.values(map).find(
      (entry) => entry?.study?.id === cardKey || entry?.study?.id === `a1-${cardKey}`
    )
  );
}
...
function validateMergedCard(lang, cardKey, merged) {
  ...
  if (lang === "hr") {
    if (LV_LEAK.test(allText)) failures.push({ type: "LV_LEAK_IN_HR" });
    if (FR_IN_HR.test(allText)) failures.push({ type: "FR_LEAK_IN_HR" });
    if (/[\u0370-\u03FF]/.test(allText)) failures.push({ type: "GR_LEAK_IN_HR" });
    if (BG_MK_SL.test(allText)) failures.push({ type: "BG_MK_SL_LEAK_IN_HR" });
  }
  failures.push(
...
const proof = {
  batch_id: BATCH,
  classification: pass
    ? "LRB_022_OWNER_PREP_READY_FOR_LINGUISTIC_REVIEW"
    : "LRB_022_OWNER_PREP_BLOCKED",
  pdf_reaudit: true,
  ...
  gates: {
    ...
    LABOT: `${labot}/50`,
    NELABOT: nelabot,
    HR_ROWS: `${rows.filter((r) => r.languages === "hr").length}/48`,
    HU_ROWS: `${rows.filter((r) => r.languages === "hu").length}/2`,
  ...
  productionSource: "data/hr/a1.js + data/hu/a1.js",
  languages: { hr: 48, hu: 2 },
