...
'use strict';
/**
 * CS-DE A2 Repair Group 08 — COPY-ONLY from cs-a2-repair-group08-spec.json
 */
const fs = require('fs');
...
function generateReport(summary, results, sectionAccentIssues, lvRemnants) {
  const lines = [
    `# CS–DE A2 Repair Group ${GROUP}`,
    '',
    `COPY-ONLY repair from \`scripts/cs-a2-repair-group${GROUP}-spec.json\`.`,
    '',
    '## Summary',
    '',
    '| Metric | Value |',
  ...
  if (sectionAccentIssues.length) {
    lines.push('## sectionAccents (OWNER spec retained)', '');
    const byCard = new Map();
    for (const issue of sectionAccentIssues) {
      if (!byCard.has(issue.cardId)) byCard.set(issue.cardId, []);
      byCard.get(issue.cardId).push(issue.target);
    }
    lines.push('| cardId | stale targets |', '|---|---|');
    for (const [cardId, targets] of byCard) {
      lines.push(`| ${cardId} | ${[...new Set(targets)].join(', ')} |`);
    }
    lines.push('');
  }

  if (lvRemnants.length) {
    lines.push('## LV remnants (documented, not auto-fixed)', '');
    for (const h of lvRemnants) lines.push(`- ${h.cardId} \`${h.field}\`: ${h.types.join(', ')}`);
    lines.push('');
  }
...
function main() {
  ...
  const hardFail = results.filter((r) => ['CARD_NOT_FOUND', 'INDEX_MISMATCH'].includes(r.status));
  if (blocked.length || hardFail.length) {
    console.error(JSON.stringify({ blocked, hardFail }, null, 2));
    process.exit(1);
  }
  ...
  const summary = {
    ...
    regressionFindings: lvRemnants.length + sectionAccentIssues.length,
    branch: BRANCH,
  };

  fs.writeFileSync(
    path.join(ROOT, `reports/cs-a2-repair-group${GROUP}.md`),
    generateReport(summary, results, sectionAccentIssues, lvRemnants),
  );

  const accounted = applied + summary.alreadyCorrect;
  const pass = accounted === 50
