...
"use strict";
/**
 * CS-DE C2 — single consolidated OWNER review file (cards with findings only).
 * Usage: node scripts/build-cs-c2-all-findings-by-card.js
 */
const fs = require("fs");
...
const AUDIT_DIR = path.join(ROOT, "reports/temp/cs-c2-audit");
const OUT_FILE = path.join(ROOT, "reports/cs-c2-all-findings-by-card.md");
const C2_TOTAL = 219;

function loadWords(filePath, key = "C2_WORDS") {
...
function entryId(entry, index) {
  ...
  if (entry.de) return `c2-${entry.de}-${index}`;
  return `c2-${index}`;
}
...
function resolveCardId(cardId, cardIndexById, cards) {
  ...
  const m = String(cardId).match(/^c2-(.+)-(\d+)$/);
  if (!m) return null;
...
function main() {
  if (!fs.existsSync(AUDIT_DIR)) {
    throw new Error(`Missing audit directory: ${AUDIT_DIR}`);
  }

  const words = loadWords(C2_FILE);
  if (words.length !== C2_TOTAL) throw new Error(`Expected ${C2_TOTAL} cards, found ${words.length}`);
  ...
  const lines = [
    "# CS–DE C2 — All Findings by Card",
    "",
    "READ-ONLY export for OWNER linguistic review.",
    "",
    `Generated: ${new Date().toISOString()}`,
    `Source production: \`data/cs/c2.js\``,
    `Source audit: \`reports/cs-c2-full-audit.md\`, \`reports/temp/cs-c2-audit/\``,
    "",
    "---",
    "",
  ];
  ...
  lines.push("");
  lines.push(`C2 production cards: ${C2_TOTAL}`);
