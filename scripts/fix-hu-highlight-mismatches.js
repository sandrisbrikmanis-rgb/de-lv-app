#!/usr/bin/env node
/**
 * Fixes sectionAccents "term does not appear in text" mismatches for HU
 * (data/hu/{level}.js)
...
const ACCHUT_COLORS = ["blue", "green", "yellow", "orange", "purple", "red"];
...
function reshapeTipLeftBlocksMismatch(study) {
  ...
  const hasColors = ACCHUT_COLORS.some((c) => Array.isArray(sa.tip[c]));
  if (!hasColors) return;
  const blocks = study.tip.leftBlocks;
  const perBlock = blocks.map(() => ({}));
  for (const color of ACCHUT_COLORS) {
    ...
    for (const [sectionKey, rules] of Object.entries(sectionAccents)) {
      if (Array.isArray(rules)) {
        rules.forEach((entry, index) => {
          ...
          const hasColors = ACCHUT_COLORS.some((c) => Array.isArray(entry[c]));
      ...
      if (rules && typeof rules === "object") {
        const hasColors = ACCHUT_COLORS.some((c) => Array.isArray(rules[c]));
