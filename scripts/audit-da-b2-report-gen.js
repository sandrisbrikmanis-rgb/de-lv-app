#!/usr/bin/env node
/**
 * DA-DE B2 audit report generator (GPT-5.6 Luna).
 * Reads reports/temp/da-b2-audit-data.json → writes reports/da-b2-full-audit.md
 */
const fs = require("fs");
...
const IN = path.join(ROOT, "reports/temp/da-b2-audit-data.json");
const OUT = path.join(ROOT, "reports/da-b2-full-audit.md");
...
function globOwnerReviewFiles() {
  ...
  return fs
    .readdirSync(dir)
    .filter((f) => f.startsWith("da-b2-owner-review-") && f.endsWith(".md"))
    .sort()
    .map((f) => `- [\`${f}\`](./${f})`);
}
...
function main() {
  ...
  const md = [];
  md.push("# DA–DE B2 pilns lingvistiskais un kvalitātes audits");
  md.push("");
  md.push(`**Datums:** ${meta.date}`);
  md.push("**Auditors:** GPT-5.6 Luna (READ-ONLY)");
  md.push(`**Audita režīms:** ${meta.mode || "READ-ONLY"}`);
  md.push("**Production fails:** `data/da/b2.js` (primārais) + `www/data/da/b2.js` (mirror)");
  ...
  md.push(`| Language parity (B2) | ${parityPass ? "PASS" : "FAIL"} |`);
  md.push("");
  md.push("---");
  md.push("");
  ...
  if (ownerFiles.length) {
    ...
    md.push("_Vēl nav ģenerēti. Palaid: `node scripts/build-da-b2-owner-review-groups.js`_");
  }
  md.push("");
  md.push("Indekss: [`da-b2-owner-review-README.md`](./da-b2-owner-review-README.md)");
  md.push("");
  md.push("GitHub: [`da-b2-owner-review-GITHUB.md`](./da-b2-owner-review-GITHUB.md)");
  md.push("");

  const tracePath = path.join(ROOT, "reports/temp/da-b2-owner-review-traceability.json");
  if (fs.existsSync(tracePath)) {
    const trace = JSON.parse(fs.readFileSync(tracePath, "utf8"));
    md.push("## 4b. Traceability");
    md.push("");
    md.push("| Metrika | Skaitlis |");
    md.push("|---------|--------:|");
    md.push(`| Validated OWNER candidates | **${trace.validatedOwnerCandidates}** |`);
    md.push(`| OWNER review entries | **${trace.reviewEntries}** |`);
    md.push(`| OWNER decision entries | **${trace.decisionEntries}** |`);
    md.push(`| Missing | **${trace.missing}** |`);
    md.push(`| Duplicates | **${trace.duplicates}** |`);
    md.push(`| Traceability | **${trace.pass ? "PASS" : "FAIL"}** |`);
    md.push("");
  }
  ...
  md.push("");
  md.push("**Auditors:** GPT-5.6 Luna (READ-ONLY)");
  md.push("1. `node scripts/audit-da-b2-collect.js` — READ-ONLY kolektors (DE etalons `data/b2.js`)");
  md.push("2. `node scripts/build-da-b2-owner-review-groups.js` — OWNER review batch faili");
  md.push("3. `node scripts/audit-da-b2-report-gen.js` — šis pārskats");
