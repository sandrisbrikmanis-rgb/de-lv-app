...
if (Object.keys(COMPOSITE_TARGETS).length !== 1) {
  console.error(`Expected 1 composite, got ${Object.keys(COMPOSITE_TARGETS).length}`);
  process.exit(1);
}
...
const NELABOT_CARDS = ["Tee", "teuer", "und"];
...
if (labot !== 47) {
  issues.push({ type: "LABOT_COUNT", msg: `expected 47 LABOT, got ${labot}` });
  semanticViolations++;
}
...
const proof = {
  batch_id: BATCH,
  classification: pass
    ? "LRB_016_FULL_50_50_LINGUISTIC_REVIEW_PASS"
    : "LRB_016_LINGUISTIC_REVIEW_BLOCKED",
  pdf_reaudit: true,
