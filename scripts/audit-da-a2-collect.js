#!/usr/bin/env node
/**
 * DA-DE A2 full audit collector (READ-ONLY).
 * Compares data/da/a2.js vs DE etalon data/a2.js; mirrors www/data/da/a2.js.
 * Writes reports/temp/da-a2-audit-data.json
 */
const fs = require("fs");
...
const OUT = path.join(ROOT, "reports/temp/da-a2-audit-data.json");
...
function main() {
  const de = loadArrayDataset("data/a2.js");
  const da = loadArrayDataset("data/da/a2.js");
  const findings = [];
  ...
  function add(raw) {
    ...
    findingId++;
    findings.push({
      id: `DA-A2-${String(findingId).padStart(4, "0")}`,
      cardId: raw.cardId,
      field: raw.field,
  ...
  const meta = {
    ...
    daFile: "data/da/a2.js",
    note: "Dāņu tulkojumi glabājas laukā lv (projekta konvencija).",
    deReference: "data/a2.js (DE parity only, READ-ONLY)",
    wwwFile: "www/data/da/a2.js",
  ...
  if (da.length !== EXPECTED_CARDS) {
    add({
      ...
      problem: "Kartīšu skaits neatbilst sagaidāmajam A2 apjomam",
      rationale: `App UI sagaida ${EXPECTED_CARDS} A2 kartītes`,
      severity: "CRITICAL",
    });
  ...
  if (meta.studyCount !== EXPECTED_STUDIES) {
    add({
      ...
      rationale: `DA A2 Study kartīšu skaitam jābūt ${EXPECTED_STUDIES}`,
      severity: "CRITICAL",
    });
  ...
  if (!meta.mirrorIdentical) {
    add({
      ...
      field: "mirror",
      deContext: "",
      currentDa: "data/da/a2.js ≠ www/data/da/a2.js",
    ...
    if (!entry.lv || !String(entry.lv).trim()) {
      ...
      if (segments.length >= 3) {
        add({
          ...
          rationale: "A2 galvenajā laukā ieteicams 1–2 dabiski dāņu sinonīmi",
          severity: "MEDIUM",
