#!/usr/bin/env node
/**
 * Fix PL sectionAccents terms that don't match section text (technical only).
...
function inflectionCandidates(term) {
  ...
  const t = String(term);
  // Polish noun/adjective patterns
  if (t.length >= 3 && t.endsWith('a')) {
    const stem = t.slice(0, -1);
    candidates.push(`${stem}y`, `${stem}ę`, `${stem}ie`, `${stem}ą`, `${stem}ach`, `${stem}ami`, `${stem}o`);
  }
  if (t.length >= 3 && t.endsWith('y')) {
    const stem = t.slice(0, -1);
    candidates.push(`${stem}ego`, `${stem}emu`, `${stem}ym`, `${stem}i`, `${stem}ą`);
  }
  if (t.length >= 3 && t.endsWith('ć')) {
    const stem = t.slice(0, -2);
    candidates.push(`${stem}czę`, `${stem}czysz`, `${stem}czy`, `${stem}czą`, `${stem}czenie`);
  }
  if (t.length >= 4 && t.endsWith('enie')) {
    const stem = t.slice(0, -4);
    candidates.push(`${stem}ę`, `${stem}esz`, `${stem}e`, `${stem}ą`);
  }
  if (t.length >= 3 && t.endsWith('o')) {
    const stem = t.slice(0, -1);
    candidates.push(`${stem}a`, `${stem}u`, `${stem}em`);
  }
...
function main() {
  ...
  for (const [file, varName] of PL_WORD_FILES) {
