#!/usr/bin/env node
/**
 * Generate PT-DE data from ES baseline with batched translation (ES → PT).
...
const OUT_DIR = path.join(ROOT, "data", "pt");
const CACHE_PATH = path.join(ROOT, "scripts", ".es-pt-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "es";
const TO_LANG = "pt";
...
function loadEsTrainingCardsFromData() {
  const cardsPath = path.join(ROOT, "data/es/courseTrainingCards.js");
...
async function main() {
  ...
  console.log("Loading ES baseline and LV German source...");
  ...
  const esA1 = loadData("data/es/a1.js").A1_WORDS;
  const esA2 = loadData("data/es/a2.js").A2_WORDS;
  const esB1 = loadData("data/es/b1.js").B1_WORDS;
  const esB2 = loadData("data/es/b2.js").B2_WORDS;
  const esC1 = loadData("data/es/c1.js").C1_WORDS;
  const esC2 = loadData("data/es/c2.js").C2_WORDS;
  const esSentences = loadData("data/es/sentences.js").SENTENCE_ENTRIES;
  const esVerbsSource = loadData("data/es/verbs.js").VERB_ENTRIES;
  const esDialogueSource = loadData("data/es/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const esCourse = loadData("data/es/courseLessons.js");
  const esTraining = loadEsTrainingCardsFromData();

  [esA1, esA2, esB1, esB2, esC1, esC2, esSentences, esVerbsSource].forEach((arr) => arr.forEach((e) => collectStrings(e, allStrings)));
  Object.values(esDialogueSource).forEach((e) => collectStrings(e, allStrings));
  Object.values(esCourse.COURSE_LESSON_DATA || {}).forEach((e) => collectStrings(e, allStrings));
  Object.values(esCourse.COURSE_LESSON_HTML || {}).forEach((html) => {
    [...String(html).matchAll(/>([^<]{3,300})</g)].forEach((m) => {
      const t = m[1].trim();
      if (t) allStrings.add(t);
    });
  });
  Object.values(esTraining).forEach((deck) => {
  ...
  console.log(`Found ${unique.length} unique ES strings (${alreadyCached} cached)`);
  ...
  function mergeLevel(lvArr, esArr) {
    return lvArr.map((lvEntry, i) => {
      const esEntry = esArr[i];
      if (!esEntry || lvEntry.de !== esEntry.de) {
        console.warn(`Order mismatch at ${lvEntry.de}`);
        return lvEntry;
      }
      const trEntry = applyTranslation(esEntry, map);
      return preserveGermanFields(lvEntry, trEntry);
    });
  }

  console.log("\nWriting PT files...");
  writeArrayFile(path.join(OUT_DIR, "a1.js"), "A1_WORDS", mergeLevel(lvA1, esA1));
  writeArrayFile(path.join(OUT_DIR, "a2.js"), "A2_WORDS", mergeLevel(lvA2, esA2));
  writeArrayFile(path.join(OUT_DIR, "b1.js"), "B1_WORDS", mergeLevel(lvB1, esB1));
  writeArrayFile(path.join(OUT_DIR, "b2.js"), "B2_WORDS", mergeLevel(lvB2, esB2));
  writeArrayFile(path.join(OUT_DIR, "c1.js"), "C1_WORDS", mergeLevel(lvC1, esC1));
  writeArrayFile(path.join(OUT_DIR, "c2.js"), "C2_WORDS", mergeLevel(lvC2, esC2));
  writeArrayFile(path.join(OUT_DIR, "sentences.js"), "SENTENCE_ENTRIES", mergeLevel(lvSentences, esSentences));

  const ptVerbs = lvVerbs.map((lvEntry, i) => {
    const esEntry = esVerbsSource[i];
    const te = {};
    for (const form of Object.keys(lvEntry)) {
      te[form] = {
        de: lvEntry[form].de,
        lv: map[esEntry[form].lv] || esEntry[form].lv,
      };
    }
    return te;
  });
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", ptVerbs);

  const ptDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const esEntry = esDialogueSource[id];
    ptDialogue[id] = { de: lvEntry.de, lv: map[esEntry.lv] || esEntry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(ptDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  const ptHtml = {};
  for (const [key, html] of Object.entries(esCourse.COURSE_LESSON_HTML || {})) {
    ptHtml[key] = await translateHtmlLesson(html, map);
  }
  const ptData = applyTranslation(esCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(ptHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(ptData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Portuguese course training cards for PT-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(esTraining)) {
    if (!Array.isArray(deck)) continue;
    const ptKey = key.replace(/Es$/, "Pt");
  ...
  console.log("\nPT-DE data generation complete (ES→PT with LV DE preservation).");
}
