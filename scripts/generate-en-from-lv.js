#!/usr/bin/env node
/**
 * Generate EN-DE (British English) data from LV baseline (DE READ-ONLY).
 * Translates only native-language (lv) fields LV → en-GB.
...
const OUT_DIR = path.join(ROOT, "data", "en");
...
const NAME_REPLACEMENTS = [
  ...
  [/\bJānis\b/g, "John"], [/\bJāņa\b/g, "John"],
  [/\bRūdolfs\b/g, "Rudolf"], [/\bRoberts\b/g, "Robert"],
  [/\bMarija\b/g, "Maria"], [/\bMarie\b/g, "Maria"],
  [/\bAlbert\b/g, "Albert"], [/\bAlberta\b/g, "Alberta"],
  [/\bMarta\b/g, "Martha"], [/\bPauls\b/g, "Paul"], [/\bPaul\b/g, "Paul"],
  [/\bHanna\b/g, "Hannah"], [/\bGertrud\b/g, "Gertrude"], [/\bAnna\b/g, "Anna"],
...
const BRITISH_SPELLINGS = [
  ...
  [/\benroll\b/gi, "enrol"],
  [/\benrolls\b/gi, "enrols"],
  [/\benrolled\b/gi, "enrolled"],
  [/\bpractice\b/gi, (m) => (m[0] === m[0].toUpperCase() ? "Practise" : "practise")],
];
...
async function main() {
  ...
  console.log("Collecting unique Latvian strings from LV-DE for British English translation...");
  ...
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", enVerbs);

  const enDialogue = {};
  for (const [id, entry] of Object.entries(dialogue)) {
    enDialogue[id] = { de: entry.de, lv: map[entry.lv] || entry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(enDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  for (const [key, html] of Object.entries(course.COURSE_LESSON_HTML || {})) {
    enHtml[key] = await translateHtmlLesson(html, map);
  }
  const enData = applyTranslation(course.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(enHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(enData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// British English course training cards for EN-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Lt$/, "En")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nEN-DE (British English) data generation complete (LV → en-GB).");
}
