...
"use strict";
/**
 * ES-DE C2 OWNER COPY-ONLY apply (91 LABOT, 1 NELABOT).
 * Authority: reports/es-de-c2-owner-decisions-final.json only.
 * Usage: node scripts/apply-es-de-c2-owner-decisions-final.js [--dry-run]
 */
const fs = require("fs");
...
const cfg = getModuleConfig("c2");
const DECISIONS_JSON = path.join(ROOT, "reports/es-de-c2-owner-decisions-final.json");
const APPLY_LOG = path.join(ROOT, "reports/temp/es-de-c2-owner-repair-apply-final-log.json");
const REPORT_MD = path.join(ROOT, "reports/es-de-c2-owner-repair-apply-final.md");
...
function writeReport(log) {
  const s = log.summary;
  const lines = [
    "# ES–DE C2 — OWNER COPY-ONLY repair apply (final)",
    "",
    "**Standard:** `REPAIR_APPLY_SAFETY_STANDARD.md` + `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9",
    "**Authority:** `reports/es-de-c2-owner-decisions-final.json`",
    `**Branch:** \`${BRANCH}\``,
...
function main() {
  ...
  const finalVerdict = passApply
    ? "PASS — ALL 91 OWNER LABOT APPLIED AND VERIFIED"
