...
"use strict";
/**
 * ES Kurss Lessons 1–21 — V2 MAIN-CURRENT OWNER COPY-ONLY apply (single batch).
 * Authority: reports/es-kurss-lesson-*-owner-decisions-v2-main-current.json
 *
 * Usage: node scripts/apply-es-kurss-lessons-v2-main-current-copy-only.js
 */
const fs = require("fs");
...
const EXPECTED_LABOT = 326;
const EXPECTED_NELABOT = 2625;
const REPORT_MD = path.join(ROOT, "reports/es-kurss-lessons-v2-main-current-apply-report.md");
const LOG_JSON = path.join(ROOT, "reports/temp/es-kurss-lessons-v2-main-current-apply-log.json");
...
function isAcceptedNelabotOnMain(target) {
  return (
    target.status === "NELABOT" &&
    target.reviewVersion === 2 &&
    target.reviewed &&
    normalizeCompare(target.new) === normalizeCompare(target.current) &&
    /jau atbilst/i.test(String(target.decisionReason || ""))
  );
}
...
function buildReport(log) {
  const lines = [
    "# ES Kurss Lessons 1–21 — V2 MAIN-CURRENT COPY-ONLY apply report",
    "",
    `**Generated:** ${log.generatedAt}`,
    `**Authority:** V2 MAIN-CURRENT OWNER decisions (reviewVersion 2)`,
    "",
    "## Gates",
    "",
    "| Gate | Result |",
    ...
    "## Verdict",
    "",
    log.pass ? "**PASS** — V2 MAIN-CURRENT COPY-ONLY apply complete." : "**FAIL** — see log JSON for details.",
    "",
  ];
...
function main() {
  const allTargets = loadAllDecisionsV2MainCurrent();
  ...
  if (labotAll.length !== EXPECTED_LABOT) {
    console.error(`Expected ${EXPECTED_LABOT} LABOT targets, found ${labotAll.length}`);
    process.exit(2);
  }
  if (nelabot.length !== EXPECTED_NELABOT) {
    console.error(`Expected ${EXPECTED_NELABOT} NELABOT targets, found ${nelabot.length}`);
    process.exit(2);
  }
  ...
  for (const target of nelabot) {
    ...
    if (isAcceptedNelabotOnMain(target)) {
      log.nelabotUnchanged++;
      continue;
    }
  ...
  log.deChanges = compareDeSnapshots(deBefore, extractDeSnapshots(data, html)).length;

  const gatesOk =
    log.appliedVerified === EXPECTED_LABOT &&
    log.nelabotUnchanged === EXPECTED_NELABOT &&
  ...
  log.deChanges = compareDeSnapshots(deBefore, extractDeSnapshots(reloaded.data, reloaded.html)).length;
  const htmlCheck = verifyHtmlStructure(reloaded.html);
  log.htmlStructure = htmlCheck.pass ? "PASS" : "FAIL";
  if (!htmlCheck.pass) log.failures.push(...htmlCheck.issues.map((issue) => ({ id: "HTML", note: issue })));
