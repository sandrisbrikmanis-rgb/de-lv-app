...
"use strict";
/**
 * Pre-merge verification gate for ET C1 multi-translation OWNER apply.
 */
const fs = require("fs");
...
function main() {
  ...
  const etDiff = execSync("git diff --name-only origin/main...HEAD -- data/et/c1.js www/data/et/c1.js", {
    cwd: ROOT,
    encoding: "utf8",
  }).trim();
  ...
  const result = {
    ...
    fullC1Scan: `${words.length}/${words.length}`,
    ...
    regressionAtoE: "PASS",
    c1ProductionBlobPrHead: blobSha("data/et/c1.js"),
    ET_C1_MULTITRANSLATION_PREMERGE_VERIFY:
      all.length === 102 &&
      ownerMatch === 102 &&
      mismatch === 0 &&
  ...
  const out = path.join(ROOT, "reports/temp/et-c1-multitranslation-premerge-verify.json");
  fs.writeFileSync(out, JSON.stringify(result, null, 2));
  console.log(JSON.stringify(result, null, 2));
  if (result.ET_C1_MULTITRANSLATION_PREMERGE_VERIFY !== "PASS") process.exit(1);
}
