#!/usr/bin/env node
/**
 * Fix BG sectionAccents terms that don't match section text (technical only).
 * Adapted from fix-ro-section-accents.js for Bulgarian.
 */
const fs = require('fs');
const path = require('path');

const BG_WORD_FILES = [
  ['data/bg/a1.js', 'A1_WORDS'],
  ['data/bg/a2.js', 'A2_WORDS'],
  ['data/bg/b1.js', 'B1_WORDS'],
  ['data/bg/b2.js', 'B2_WORDS'],
  ['data/bg/c1.js', 'C1_WORDS'],
  ['data/bg/c2.js', 'C2_WORDS'],
  ['data/bg/sentences.js', 'SENTENCE_ENTRIES'],
];
...
function stemMatch(text, term) {
  ...
  const stem = String(term).replace(/(?:ям|еш|ва|ат|ят|их|ите|а|я|о|и|е|ъл|ък)$/iu, '');
...
function inflectionCandidates(term) {
  ...
  const t = String(term);
  if (t.length >= 3) {
    candidates.push(t.slice(0, -1), `${t.slice(0, -1)}а`, `${t.slice(0, -1)}я`, `${t.slice(0, -1)}и`, `${t.slice(0, -1)}ите`);
  }
...
function main() {
  ...
  for (const [file, varName] of BG_WORD_FILES) {
