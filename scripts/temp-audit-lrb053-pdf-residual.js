...
const EXPECTED_UNIQUE_CARDS = 29;

const FI_CARD_ALIASES = {
...
function nestedForCard(lang, cardKey) {
  ...
  const resolved = FI_CARD_ALIASES[cardKey] || cardKey;
  return (
    map[resolved] ||
    map[cardKey] ||
    map[`a1-${resolved}`] ||
...
function validateMergedCard(lang, cardKey, merged) {
  ...
  if (lang === "fi") {
    if (LV_LEAK.test(allText)) failures.push({ type: "LV_LEAK_IN_FI" });
    if (ET_LEAK.test(allText)) failures.push({ type: "ET_LEAK_IN_FI" });
    if (HU_IN_IS.test(allText)) failures.push({ type: "HU_LEAK_IN_FI" });
    if (NO_IN_IS.test(allText)) failures.push({ type: "NO_LEAK_IN_FI" });
  }
  failures.push(
...
const proof = {
  batch_id: BATCH,
  classification: pass
    ? "LRB_053_OWNER_PREP_READY_FOR_LINGUISTIC_REVIEW"
    : "LRB_053_OWNER_PREP_BLOCKED",
  pdf_reaudit: true,
  ...
  gates: {
    ...
    NELABOT: nelabot,
    FI_ROWS: `${rows.filter((r) => r.languages === "fi").length}/${EXPECTED_ROWS}`,
