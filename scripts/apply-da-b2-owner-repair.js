...
"use strict";
/**
 * DA–DE B2 OWNER COPY-ONLY repair apply.
 * Usage: node scripts/apply-da-b2-owner-repair.js [--dry-run]
 */
const fs = require("fs");
...
const { getAt, setAt, findEntry, normalizeField } = require("./lib/da-b2-owner-path");

const APPLY_MAP = path.join(ROOT, "reports/temp/da-b2-owner-apply-map.json");
const APPLY_LOG = path.join(ROOT, "reports/temp/da-b2-owner-apply-log.json");
const FILES = [path.join(ROOT, "data/da/b2.js"), path.join(ROOT, "www/data/da/b2.js")];
...
function main() {
  ...
  if (applyMap.apply.length === 0) {
    log.summary = { applied: 0, failed: 0, studyCount: countStudies(words), deChanges: 0 };
  ...
  for (const row of applyMap.apply) {
    ...
    if (result.ok) {
      log.applied.push({ finding: row.finding, cardId: row.cardId, field: row.field, action: row.action });
    } else {
  ...
  if (log.failed.length) {
    ...
    log.failed.slice(0, 15).forEach((f) => console.error(JSON.stringify(f)));
    process.exit(1);
  }
}
