#!/usr/bin/env node
/**
 * Generate data/bs/b2.js from LV baseline (data/b2.js) using GPT-5.6 Luna batch translation.
 * German fields remain READ-ONLY.
 *
 * Usage:
 *   node scripts/generate-bs-b2-from-lv.js [--test-batch] [--skip-section-accents]
 */
const fs = require("fs");
...
const SOURCE_FILE = path.join(ROOT, "data", "b2.js");
const OUT_FILE = path.join(ROOT, "data", "bs", "b2.js");
const WWW_OUT_FILE = path.join(ROOT, "www", "data", "bs", "b2.js");
const CACHE_PATH = path.join(ROOT, "scripts", ".bs-b2-openai-translation-cache.json");
const STATS_PATH = path.join(ROOT, "scripts", ".bs-b2-openai-stats.json");
const REPORT_PATH = path.join(ROOT, "reports", "bs-b2-translation-report.md");
...
const B1_CACHE_PATH = path.join(ROOT, "scripts", ".bs-b1-openai-translation-cache.json");
const PROGRESS_PATH = path.join(ROOT, "reports", "temp", "bs-b2-translation-progress.json");
const SOURCE_ISSUES_PATH = path.join(ROOT, "reports", "temp", "bs-b2-source-issues.json");
...
function loadProgress() {
  if (!fs.existsSync(PROGRESS_PATH)) return { completedBatches: [], sourceIssues: [] };
  try {
    return JSON.parse(fs.readFileSync(PROGRESS_PATH, "utf8"));
  } catch {
    return { completedBatches: [], sourceIssues: [] };
  }
}

function saveProgress(progress, stats) {
  ...
  fs.writeFileSync(PROGRESS_PATH, JSON.stringify({
    ...
    stats: {
      ...
      uniqueStringCount: stats.uniqueStringCount,
...
function saveSourceIssues(issues) {
  fs.mkdirSync(path.dirname(SOURCE_ISSUES_PATH), { recursive: true });
  fs.writeFileSync(SOURCE_ISSUES_PATH, JSON.stringify(issues, null, 2));
}
...
function writeB2File(filePath, data) {
...
async function translatePendingBatches(words, cache, stats) {
  ...
  console.log(`BS B2: ${uniqueTexts.length} unique strings, ${cachedCount} cache hits, ${pendingByText.size} to translate`);
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex += 1) {
    ...
    progress.completedBatches = [...completed].sort((a, b) => a - b);
    saveProgress(progress, stats);
  }
}
...
function writeReport(words, stats, validation, sectionAccentStats) {
  ...
  const lines = [
    "# BS–DE B2 — ģenerēšanas atskaite (GPT-5.6 Luna + batching)",
    "",
    ...
    "## 1. Kopsavilkums",
    "",
    "BS–DE B2 initial translation no LV–DE etalona (`data/b2.js`) ar GPT-5.6 Luna batch arhitektūru.",
    "",
    "**Statuss:** BS–DE B2 INITIAL TRANSLATION = COMPLETE",
    "",
    "## 2. Datu apjoms",
    "",
    "| Metrika | Vērtība |",
    "|---|---:|",
    `| B2 kartītes | ${words.length} |`,
    ...
    "| C1 Luna pilots | 46 |",
    `| B2 (batch + dedup + cache) | ${stats.requestCount} |`,
    ...
    "## 7. Secinājums",
    "",
    "B2 initial translation posms pabeigts. Pilns neatkarīgais lingvistiskais audits tiks veikts atsevišķi.",
    "",
...
async function runValidation(words) {
  ...
  try {
    ...
    const b2 = report.levels?.b2;
    results["Valodu paritāte"] = report.pass && b2?.countMatch ? `PASS (${b2.langCount}/${b2.lvCount})` : "CHECK";
  } catch {
  ...
  try {
    ...
    const report = JSON.parse(out);
    const b2 = report.perFile.find((file) => file.file.includes("b2.js"));
    results["sectionAccents TECHNICAL"] = `${b2?.sectionAccentIssues || 0} issues`;
  } catch {
    results["sectionAccents TECHNICAL"] = "CHECK";
  }
  const LV=/[āēīūģķļņĀĒĪŪĢĶĻŅ]/;
  function walk(v,h=[],skipDe=false){
    if(typeof v==='string'){if(LV.test(v))h.push(v);return h;}
    if(Array.isArray(v))v.forEach(i=>walk(i,h,skipDe));
    else if(v&&typeof v==='object'){
      for(const [k,val] of Object.entries(v)){
        if(k==='de'||k==='de_article'||k==='de_plural') continue;
        walk(val,h,skipDe);
      }
    }
    return h;
...
function runSectionAccentsFix() {
  const out = require("child_process").execSync("node scripts/fix-bs-b2-section-accents.js", { encoding: "utf8" });
  const logPath = path.join(ROOT, "reports", "temp", "bs-b2-section-accents-fix-log.json");
...
async function main() {
  ...
  if (TEST_BATCH) {
    console.log("TEST BATCH complete — API pipeline OK. Exiting without writing B2 files.");
  ...
  writeB2File(OUT_FILE, translatedWords);
  fs.mkdirSync(path.dirname(WWW_OUT_FILE), { recursive: true });
  writeB2File(WWW_OUT_FILE, translatedWords);
