# Apply B2->C1/C2 moves. Removes from b2.js, appends to end of c1.js / c2.js. No ConvertTo-Json on data files.
param(
  [string]$PlanPath = "",
  [switch]$DryRun
)
...
if (-not $PlanPath) { $PlanPath = Join-Path $PSScriptRoot "cefr-b2-c1-c2-plan.json" }
...
if (-not $plan.moves -or $plan.moves.Count -eq 0) {
  Write-Output "No B2->C1/C2 moves in plan."
  exit 0
}
...
$b2Kept = [System.Collections.Generic.List[object]]::new()
$movedC1 = 0; $movedC2 = 0

$parsedB2 = Parse-FileCards "data/b2.js" "B2"
foreach ($card in $parsedB2.Cards) {
  ...
  if ($moveMap.ContainsKey($key)) {
    ...
    [void]$b2Kept.Add([pscustomobject]@{ Lines = $card.Lines; De = $card.De })
  }
}
...
if ($DryRun) {
  Write-Output "DRY RUN: would move $($movedC1 + $movedC2) cards from B2"
  Write-Output "  B2: $($b2Kept.Count) cards (was $($parsedB2.Cards.Count))"
...
$b2Out = [System.Collections.Generic.List[object]]::new()
foreach ($c in $b2Kept) { [void]$b2Out.Add($c) }
Write-LevelFile $parsedB2 $b2Out "data/b2.js"
Write-Output "data/b2.js: $($b2Kept.Count) cards written"
...
Write-Output ""
Write-Output "B2->C1/C2 move complete: C1=$movedC1, C2=$movedC2 relocated"
