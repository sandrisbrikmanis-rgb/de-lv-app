...
"use strict";
/**
 * ET–DE Kurss targeted regression after OWNER 310 LABOT COPY-ONLY apply.
 * Usage: node scripts/audit-et-kurss-owner-repair-targeted-regression.js
 */
const fs = require("fs");
...
const REPORT_MD = path.join(ROOT, "reports/et-kurss-targeted-regression.md");
const REPORT_JSON = path.join(ROOT, "reports/temp/et-kurss-targeted-regression.json");
...
const EXPECTED_NELABOT = 3;
const EXPECTED_FP = 8;
const EXPECTED_NSR = 2;
const BLOCKED_NSR = new Set(["ET-KURSS-0226", "ET-KURSS-0227"]);
...
function main() {
  ...
  for (const row of ownerRows) {
    ...
    if (row.status === "NELABOT" && row.path) {
      ...
      if (normalizeCompare(beforeVal) === normalizeCompare(afterVal)) nelabotRetained++;
      else {
        add("HIGH", row.id, row.path, "NELABOT field changed during apply", {
    ...
    if (row.status === "NEEDS_SOURCE_REVIEW") {
      const beforeVal = readPathValue(row.path, beforeSrc);
      const afterVal = readPathValue(row.path, afterSrc);
      if (normalizeCompare(beforeVal) === normalizeCompare(afterVal)) nsrUntouched++;
      else {
        add("HIGH", row.id, row.path, "NEEDS_SOURCE_REVIEW field changed during apply", {
          before: String(beforeVal).slice(0, 100),
          after: String(afterVal).slice(0, 100),
        });
      }
      if (BLOCKED_NSR.has(row.id)) {
        const actual = readPathValue(row.path, afterSrc);
        if (normalizeCompare(actual) !== normalizeCompare(row.currentEt)) {
          add("HIGH", row.id, row.path, "Blocked NSR row value drift", {
            expected: row.currentEt,
            actual: String(actual).slice(0, 100),
          });
        }
      }
    }
  ...
  const verdict = pass ? "ET_KURSS_OWNER_REPAIR_310_PASS" : "ET_KURSS_OWNER_REPAIR_310_FAIL";
  ...
  const md = [
    "# ET–DE Kurss — OWNER repair targeted regression",
    "",
    ...
    `| NSR untouched | **${nsrUntouched}/${EXPECTED_NSR}** |`,
    `| ET-KURSS-0226 unchanged | **${BLOCKED_NSR.has("ET-KURSS-0226") ? "checked" : "n/a"}** |`,
    `| ET-KURSS-0227 unchanged | **${BLOCKED_NSR.has("ET-KURSS-0227") ? "checked" : "n/a"}** |`,
  ];
