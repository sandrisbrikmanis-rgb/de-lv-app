...
function productionEntryForCard(words, cardId) {
  if (!words) return null;
  const id = String(cardId || "").split("|")[0];
  const entry =
    words.find((e) => e.de === id) ||
    words.find((e) => e.study?.id === id) ||
    words.find((e) => e.study?.id === `a1-${id}`);
...
function getFullGalaCard(lang, cardId) {
  ...
  return productionEntryForCard(words, cardId);
}
...
function cardIdx(cardId) {
  return parseInt((String(cardId).match(/idx:(\d+)/) || [0, 0])[1], 10);
}

function buildLvDeSourceCards(batchId, findings, options) {
  const deWords = loadLangWords("de");
  const cards = {};
  for (const finding of findings) {
    const key = `${finding.lang}|${finding.cardId}`;
    if (cards[key]) continue;
    const lbCard = finding.lang === "lb" ? getFullGalaCard("lb", finding.cardId) : null;
    cards[key] = {
      lang: finding.lang,
      card_object_id: finding.cardId,
      idx: cardIdx(finding.cardId),
      de_reference: finding.deReference,
      lv_source_gloss: finding.lvSource,
      de_source_card: productionEntryForCard(deWords, finding.cardId),
      lb_production_card: lbCard,
      gala_snapshot: finding.galaCard,
    };
  }
  const list = Object.values(cards).sort((a, b) => a.idx - b.idx || a.card_object_id.localeCompare(b.card_object_id));
  return {
    batch_id: batchId,
    generatedAt: options.generatedAt,
    sourceCommit: options.sourceCommit,
    direction: "LV_SOURCE_GLOSS_TO_DE_MASTER",
...
function buildGithubIndex(batchId, files, options) {
  ...
  return [
    ...
    `- [LV→DE source cards JSON](${blob(files.lvDeSourceCards)})`,
    `- [Batch manifest](${blob(files.manifest)})`,
    `- [Batch input CSV](${blob(files.input)})`,
    files.checkpoint ? `- [Parallel checkpoint](${blob(files.checkpoint)})` : null,
    "",
    `**Branch:** \`${options.branch}\``,
    `**Source commit:** \`${options.sourceCommit}\``,
    "",
...
function main() {
  ...
  const files = {
    ...
    lvDeSourceCards: path.relative(ROOT, path.join(outDir, `${batchId}-lv-de-source-cards.json`)),
    ...
    checkpoint: fs.existsSync(path.join(ROOT, "reports/g2-a1-owner/parallel-checkpoint.json"))
      ? "reports/g2-a1-owner/parallel-checkpoint.json"
      : null,
  };
  ...
  const options = {
    ...
    reservedFor: process.env.LRB_RESERVED_FOR || manifest.reserved_for || "LB_OWNER_PREP",
  ...
  writeAtomic(
    path.join(ROOT, files.lvDeSourceCards),
    `${JSON.stringify(buildLvDeSourceCards(batchId, findings, options), null, 2)}\n`,
  );
  ...
  const proof = {
    ...
    lvDeSourceCards: new Set(findings.map((f) => `${f.lang}|${f.cardId}`)).size,
