...
"use strict";
/**
 * ET A2 residual 2 multi-translation OWNER COPY-ONLY apply (MASTER v1.12).
 */
const fs = require("fs");
...
const ACCEPTED = path.join(ROOT, "reports/et-a2-multitranslation-residual-2-owner-accepted.md");
const UPLOAD = path.join(ROOT, "uploads/et-a2-multitranslation-residual-2-owner-accepted_61f1.md");
const APPLY_LOG = path.join(ROOT, "reports/temp/et-a2-multitranslation-residual-2-apply-log.json");
const REPORT_MD = path.join(ROOT, "reports/et-a2-multitranslation-residual-2-repair-apply.md");
...
function parseAccepted() {
  ...
  for (const line of src.split("\n")) {
    ...
    if (!cardId.startsWith("a2-")) continue;
...
function main() {
  ...
  fs.writeFileSync(
    REPORT_MD,
    [
      "# ET A2 — Residual 2 multi-translation OWNER apply",
      "",
      "**MASTER:** v1.12",
      `**Authority:** \`reports/et-a2-multitranslation-residual-2-owner-accepted.md\``,
