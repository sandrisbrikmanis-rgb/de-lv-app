#!/usr/bin/env node
/**
 * Generate DA-DE data from EN baseline with batched translation (EN → DA).
...
const CACHE_PATH = path.join(ROOT, "scripts", ".da-en-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "en";
const TO_LANG = "da";
...
function postProcess(text) {
  ...
  out = out
    .replace(/\bMain idea:/gi, "Hovedidé:")
    .replace(/\bRemember:/gi, "Husk:")
    .replace(/\bIn German\b/gi, "På tysk")
    .replace(/\bin German\b/g, "på tysk");
...
async function main() {
  ...
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    ...
    daDialogue[id] = { de: lvEntry.de, lv: map[enEntry.lv] || enEntry.lv };
  }
  fs.writeFileSync(
  ...
  const daData = applyTranslation(enCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
  ...
  console.log("\nDA-DE data generation complete (EN→DA with LV DE preservation).");
}
