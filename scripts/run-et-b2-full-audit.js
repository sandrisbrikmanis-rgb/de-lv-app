...
"use strict";
/**
 * ET-DE B2 full READ-ONLY audit orchestrator (deterministic + GPT-5.6 Luna).
 * Usage: node scripts/run-et-b2-full-audit.js [--skip-luna] [--test-luna] [--fresh-luna]
...
const {
  ...
  LUNA_JSON,
  TEMP_DIR,
} = require("./lib/et-b2-audit-helpers");
const { loadOwnerHistory, classifyWithOwnerHistory } = require("./lib/et-b2-owner-history");
...
const {
  ...
  AUDIT_RUNS,
  OWNER_SOURCES,
} = require("./lib/et-b2-discovery-config");
...
const OUT_MD = path.join(ROOT, "reports", "et-b2-full-audit.md");
const OUT_JSON = path.join(ROOT, "reports", "temp", "et-b2-full-audit.json");
const MASTER_VERSION = "1.9";
const PRODUCTION_PATH = "data/et/b2.js";
const WWW_PATH = "www/data/et/b2.js";
const TOTAL_CARDS = 2118;
const LV_STUDY_COUNT = 60;
/** ET-DE B2: no prior FINAL_CLOSED integration on origin/main (first FULL_DISCOVERY). */
...
function computeOwnerHistoryCoverage(ownerHistory, etEntries) {
  ...
  etEntries.forEach((entry, index) => {
    const id = entry.study?.id || `b2-${entry.de}-${index}`;
    cardById.set(id, entry);
    cardById.set(`b2-${entry.de}`, entry);
  });
  let checked = 0;
  let matching = 0;
  let drifted = 0;
...
function computeBaselineGate(ownerHistory) {
  ...
  if (datasetProductionSha !== wwwDatasetBlobSha) {
    return {
      ...
      deReadOnly: "PASS",
      blocked: true,
      blockReason: "data/et/b2.js blob !== www/data/et/b2.js blob",
    };
  }
  ...
  const branchPatterns = [
    "origin/cursor/et-de-b2-full-audit-4a7c",
  ];
...
function mergeFindings(detFindings, collectData, lunaData, validateStudy) {
  ...
  function addFromLvRemnant(issue) {
    all.push({
      findingId: `ET-B2-${String(seq++).padStart(4, "0")}`,
  ...
  const b2Validate = validateStudy?.perFile?.find((f) => f.file === "data/et/b2.js");
  for (const issue of b2Validate?.examples?.sectionAccentIssues || []) {
    all.push({
      findingId: `ET-B2-${String(seq++).padStart(4, "0")}`,
...
function buildReport(ctx) {
  ...
  const lines = [];
  lines.push("# ET–DE B2 pilns lingvistiskais audits (MASTER v1.9 FULL_DISCOVERY)");
  lines.push("");
  ...
  lines.push("|---------|---------|");
  lines.push("| Kartītes | **2118** |");
  lines.push(`| Luna coverage | **${ctx.lunaCoverage}** |`);
  if (ctx.lunaCoverage === "skipped") {
    lines.push("");
    lines.push("> **Luna (GPT-5.6) nav palaists** (`--skip-luna` vai nav `OPENAI_API_KEY`). Šis ziņojums satur deterministisko slāni. Lai pabeigtu pilnu FULL_DISCOVERY ar Luna: `node scripts/run-et-b2-full-audit.js --fresh-luna`.");
  }
...
async function main() {
  ...
  console.log("\n=== ET–DE B2 FULL_DISCOVERY — MASTER v1.9 ===\n");
  ...
  if (FRESH_LUNA) {
    const progress = path.join(ROOT, "scripts/.et-b2-luna-progress.json");
    if (fs.existsSync(progress)) fs.unlinkSync(progress);
    if (fs.existsSync(LUNA_JSON)) fs.unlinkSync(LUNA_JSON);
    if (fs.existsSync(TEMP_DIR)) fs.rmSync(TEMP_DIR, { recursive: true, force: true });
  }

  run("node scripts/audit-et-b2-collect.js");
  runCapture(["audit-language-parity.js", "--lang=et"], path.join(ROOT, "reports/temp/et-b2-parity.json"));
  const moj = spawnSync("node", [path.join(ROOT, "scripts", "audit-mojibake.js"), "--lang=et"], {
    cwd: ROOT,
    env: process.env,
    encoding: "utf8",
  });
  fs.writeFileSync(path.join(ROOT, "reports/temp/et-b2-mojibake.json"), moj.stdout || "");
  runCapture(["validate-study-design.js", "--lang=et"], path.join(ROOT, "reports/temp/et-b2-validate-study.json"));
  run("node --check data/et/b2.js");

  const collectData = loadJsonSafe(path.join(ROOT, "reports/temp/et-b2-audit-data.json"), {});
  const validateStudy = loadJsonSafe(path.join(ROOT, "reports/temp/et-b2-validate-study.json"), {});
  ...
  if (!SKIP_LUNA) {
    ...
    runNode("audit-et-b2-linguistic.js", lunaArgs);
  } else {
  ...
  const ctx = {
    ...
    lvRemnantCards,
    sectionAccentCount: b2Validate?.sectionAccentIssues || 0,
  ...
  fs.writeFileSync(path.join(ROOT, "reports/et-b2-full-audit.json"), JSON.stringify(payload, null, 2));
  fs.writeFileSync(OUT_MD, buildReport(ctx));

  if (discoveryStability.gates.ownerBacklogAllowed && ownerBacklogFinal.length > 0) {
    fs.writeFileSync(
      path.join(ROOT, "reports/temp/et-b2-full-audit.json"),
  ...
  if (ownerBacklogFinal.length > 0 && discoveryStability.gates.ownerBacklogAllowed) {
    runPostAuditOwnerReview("et-b2-full", { force: true });
  }
}
