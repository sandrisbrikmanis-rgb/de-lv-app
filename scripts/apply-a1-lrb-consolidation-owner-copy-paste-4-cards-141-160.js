...
const PROOF_PATH = path.join(
  OUT_DIR,
  "A1-LRB-CONSOLIDATION-OWNER-CARDS-141-160-COPY-PASTE-PROOF.json"
);
const EXPECTED_CLASSIFICATION =
  "A1_LRB_CONSOLIDATION_OWNER_CARDS_141_160_FULL_CARD_COPY_PASTE_READY";
const PROOF_CLASSIFICATION =
  "A1_LRB_CONSOLIDATION_OWNER_CARDS_141_160_COPY_PASTE_COMPLETE_AWAITING_GALA_VERDICT";
const EXPECTED_OWNER_SHA = "0b509ae89734093174f7a0610db1bb90c767d5b6f5295b8f875126a9a637a197";
const EXPECTED_SOURCE_HEAD = "e6467b132e2fe357e3225f1877a77b49c87f78d1";

function main() {
  const srcArg = process.argv[2];
  const src = srcArg ? path.resolve(srcArg) : DEFAULT_BUNDLE;
  if (!fs.existsSync(src)) {
    console.error("Owner cards 141–160 bundle not found:", src);
    process.exit(1);
  }
  ...
  try {
    const proof = applyOwnerCopyPasteBatch({
      ...
      ownerRaw,
      ownerBundleRelPath:
        "reports/g2-a1-owner/consolidation/owner-review/A1-LRB-CONSOLIDATION-decisions-COPY-PASTE-4-CARDS-141-160.json",
    ...
    const taskLines = [
      "A1 LRB — kartīšu 141–160 OWNER COPY/PASTE (piemērots)",
      "",
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "A1-LRB-CONSOLIDATION-VEICAMO-LABOJUMU-UZDEVUMS-4-CARDS-141-160-APPLIED.txt"),
      taskLines.join("\n") + "\n"
    );
