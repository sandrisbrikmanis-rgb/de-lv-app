...
const BATCH = "LRB-060";
const PASTE_PATH = process.argv[2] || path.join(ROOT, "scripts/data/g2-a1-owner-pending/LRB-060-decisions-COPY-PASTE.json");
const EXPECTED_PASTE_SHA = "d24ec1d85d49e93c63fbc5eb0d44641b0af4b0d2bbf1dba17f1dc24070e6d592";
...
function normalizePasteDecision(d) {
  ...
  if (!d.owner_new || typeof d.owner_new !== "object") return out;
  ...
  if (card === "fahren|idx:172") {
    ...
    if (imp && typeof imp === "object" && !Array.isArray(imp)) {
      on["study.important"] = JSON.stringify([imp.text, imp.example].filter(Boolean));
    }
    const sa = parseMaybeJson(on["study.sectionAccents"]);
    if (sa?.examples?.[2]?.lv?.purple) {
      sa.examples[2].lv.purple = ["féiere"];
    }
    if (sa?.important) {
      sa.important = [
        { blue: ["fahren"], purple: ["fueren", "féieren"] },
        { purple: ["Transportmëttel", "féieren", "fueren"] },
      ];
    }
  ...
  if (card === "erst|idx:165") {
    const sa = parseMaybeJson(on["study.sectionAccents"]);
    if (sa?.comparison?.[2]?.example?.purple) {
      sa.comparison[2].example.purple = ["n\u00ebmme", "f\u00ebnnef"];
    }
    on["study.sectionAccents"] = JSON.stringify(sa);
  }

  if (card === "etwas|idx:169") {
    const sa = parseMaybeJson(on["study.sectionAccents"]);
    if (sa?.important?.[1]) {
      sa.important[1] = { purple: ["Saz", "Bedeitung"] };
      on["study.sectionAccents"] = JSON.stringify(sa);
    }
  }
...
function main() {
  ...
  const galaOut = {
    ...
    classification: "LRB_060_COPY_PASTE_COMPLETE_AWAITING_GALA_VERDICT",
    cards: galaCards,
  };
