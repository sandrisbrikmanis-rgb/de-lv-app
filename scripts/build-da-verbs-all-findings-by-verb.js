...
const OUT = path.join(ROOT, "reports/da-verbs-all-findings-by-verb.md");
...
function main() {
  ...
  findings.forEach((f, i) => {
    lines.push(
      `| ${i + 1} | ${f.id} | \`${f.cardId}\` | \`${f.field}\` | ${truncate(escapeCell(f.deContext), 60)} | ${truncate(escapeCell(f.currentDa), 60)} | ${truncate(escapeCell(f.proposedDa), 60)} | ${f.severity} | PENDING | |`
    );
  });
