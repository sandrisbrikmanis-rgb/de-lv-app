#!/usr/bin/env node
/**
 * Generate NL-DE data from LV baseline (DE READ-ONLY).
 * Translates only native-language (lv) fields LV → NL.
...
async function main() {
  ...
  console.log("Collecting unique Latvian strings from LV-DE for NL translation...");
  ...
  for (const [id, entry] of Object.entries(dialogue)) {
    nlDialogue[id] = { de: entry.de, lv: map[entry.lv] || entry.lv };
  }
  fs.writeFileSync(
  ...
  const nlData = applyTranslation(course.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
  ...
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Lt$/, "Nl")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nNL-DE data generation complete (LV → NL).");
}
