#!/usr/bin/env node
/**
 * Generate IT-DE data from FR baseline with batched translation (FR → IT).
...
const OUT_DIR = path.join(ROOT, "data", "it");
...
function loadFrTrainingCardsFromData() {
  const cardsPath = path.join(ROOT, "data/fr/courseTrainingCards.js");
...
async function main() {
  ...
  console.log("Loading FR baseline and LV German source...");
  ...
  const nlA1 = loadData("data/fr/a1.js").A1_WORDS;
  const nlA2 = loadData("data/fr/a2.js").A2_WORDS;
  const nlB1 = loadData("data/fr/b1.js").B1_WORDS;
  const nlB2 = loadData("data/fr/b2.js").B2_WORDS;
  const nlC1 = loadData("data/fr/c1.js").C1_WORDS;
  const nlC2 = loadData("data/fr/c2.js").C2_WORDS;
  const nlSentences = loadData("data/fr/sentences.js").SENTENCE_ENTRIES;
  const nlVerbs = loadData("data/fr/verbs.js").VERB_ENTRIES;
  const nlDialogue = loadData("data/fr/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const nlCourse = loadData("data/fr/courseLessons.js");
  const nlTraining = loadFrTrainingCardsFromData();
  ...
  console.log(`Found ${unique.length} unique FR strings (${alreadyCached} cached)`);
  ...
  console.log("\nIT-DE data generation complete (FR→IT with LV DE preservation).");
}
