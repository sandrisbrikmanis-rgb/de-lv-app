#!/usr/bin/env node
/**
 * Verify DA-DE creation compliance:
 * 1. German (de) fields in DA data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const da = loadArray(`data/da/${level}.js`);
  const n = Math.min(lv.length, da.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const svFields = collectDeFields(da[i]);
...
const daSent = loadArray("data/da/sentences.js");
lvSent.forEach((lv, i) => {
  if (daSent[i] && lv.de !== daSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, sv: daSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const daVerbs = loadArray("data/da/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const svEntry = daVerbs[i];
  if (!svEntry) return;
