...
const SYSTEM_BY_LANG = {
  ...
  pt: [
    "You are GPT-5.6 Luna, Portuguese (pt) linguistic reviewer for G2/A1 German vocabulary cards.",
    "LV (lv_source) is meaning reference only — never copy Latvian as Portuguese.",
    "DE (de_reference) is STRICT READ-ONLY — never propose DE changes.",
    "Review current Portuguese (target_current) and produce corrected Portuguese owner_new values.",
    "Return ONLY valid JSON: { \"decisions\": [ ... ] }.",
    "Each decision: { finding_stable_ids, owner_decision (LABOT|NELABOT), owner_new, owner_note }.",
    "LABOT: provide exact replacement Portuguese text in owner_new.",
    "NELABOT: owner_new empty, explain why current is acceptable.",
    "For scalar lv field: owner_new is plain string.",
    "For composite/multi-field paths: owner_new is JSON string matching production structure.",
    "For MULTI_TRANSLATION native fields: owner_new uses 'X • Y' bullet format.",
    "Fix wrong-language remnants (Latvian, English, Romanian, Bulgarian, etc.), mistranslations, POS errors.",
    "Portuguese must be natural, grammatical, semantically aligned with DE and LV meaning.",
    "Each owner_note must be unique and cite the specific DE word and fix rationale.",
    "Do NOT use bulk/template notes. DE untouched.",
  ].join("\n"),
};
...
function normalizeDecision(raw) {
  const d = String(raw || "LABOT").toUpperCase();
  if (d === "DISMISS" || d === "ACCEPT" || d === "KEEP" || d === "NELABOT") return "NELABOT";
  return d === "LABOT" ? "LABOT" : "LABOT";
}
...
async function main() {
  ...
  const ptItems = items.filter((i) => i.language === "pt");

  for (const [lang, langItems] of [["ro", roItems], ["pt", ptItems]]) {
  ...
  for (const d of all) {
    ...
    const decision = normalizeDecision(d.owner_decision);
    byId[d.finding_stable_ids] = {
      owner_status: "DECIDED",
      owner_decision: decision,
      owner_new: decision === "LABOT" ? (d.owner_new || "") : "",
  ...
  for (const row of rows) {
    if (byId[row.finding_stable_ids]) continue;
    const cardKey = row.card_object_id || "";
    const hit = all.find(
      (d) =>
        d.finding_stable_ids &&
        (d.finding_stable_ids === row.finding_stable_ids ||
          d.finding_stable_ids.includes(cardKey)),
    );
    if (hit) {
      const decision = normalizeDecision(hit.owner_decision);
      byId[row.finding_stable_ids] = {
        owner_status: "DECIDED",
        owner_decision: decision,
        owner_new: decision === "LABOT" ? (hit.owner_new || "") : "",
        owner_note: hit.owner_note || "",
      };
      if (decision === "LABOT") labot++;
      else nelabot++;
    }
