#!/usr/bin/env node
"use strict";

/**
 * COPY-ONLY apply 78 OWNER LABOT NEEDS_OWNER_REVIEW decisions → repo + Crowdin.
 *
 * Authority: reports/crowdin-ui-needs-owner-review-decisions.json only
 * Run: node scripts/apply-crowdin-ui-needs-owner-review.js --apply
 */

const fs = require("fs");
const path = require("path");
const vm = require("vm");
const { execSync } = require("child_process");
const {
  ROOT,
  parseCrowdinJson,
  flattenUiStrings,
  loadUiObject,
  UI_JS_REL,
  applySurgicalCrowdinPatch,
  extractPlaceholderMultiset,
  extractHtmlTagStructure,
  crowdinCodeFromRepoLang,
} = require("./lib/ui-crowdin-bridge");

const PROJECT_ID = 923473;
const SOURCE_FILE_PATH = "/main/crowdin/ui/lv.json";
const EXPECTED = 78;

const DECISIONS_JSON = path.join(ROOT, "reports", "crowdin-ui-needs-owner-review-decisions.json");
const PROOF_JSON = path.join(ROOT, "reports", "crowdin-ui-needs-owner-review-apply-proof.json");
const PROOF_CSV = path.join(ROOT, "reports", "crowdin-ui-needs-owner-review-apply-proof.csv");
const PROOF_MD = path.join(ROOT, "reports", "crowdin-ui-needs-owner-review-apply-proof.md");

function requireToken() {
  const token = process.env.CROWDIN_PERSONAL_TOKEN;
  if (!token) throw new Error("CROWDIN_PERSONAL_TOKEN is not set");
  return token;
}

function multisetEqual(a, b) {
  const keys = new Set([...Object.keys(a), ...Object.keys(b)]);
  for (const key of keys) {
    if ((a[key] || 0) !== (b[key] || 0)) return false;
  }
  return true;
}

function csvEscape(value) {
  const text = String(value ?? "");
  if (/[",\n\r]/.test(text)) return `"${text.replace(/"/g, '""')}"`;
  return text;
}

function escapeTableCell(value) {
  return String(value ?? "").replace(/\|/g, "\\|").replace(/\n/g, " ");
}

async function crowdinRequest(token, method, endpoint, body) {
  const response = await fetch(`https://api.crowdin.com/api/v2${endpoint}`, {
    method,
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
    body: body ? JSON.stringify(body) : undefined,
  });
  const text = await response.text();
  let payload = null;
  if (text) {
    try {
      payload = JSON.parse(text);
    } catch {
      payload = { raw: text };
    }
  }
  return { status: response.status, payload, text };
}

async function crowdinGetAll(token, endpoint, key = "data") {
  const items = [];
  let offset = 0;
  const limit = 500;
  while (true) {
    const separator = endpoint.includes("?") ? "&" : "?";
    const { status, payload } = await crowdinRequest(
      token,
      "GET",
      `${endpoint}${separator}limit=${limit}&offset=${offset}`
    );
    if (status !== 200) throw new Error(`GET ${endpoint} failed HTTP ${status}`);
    const page = payload?.[key] || [];
    items.push(...page);
    if (!payload?.pagination || page.length < limit) break;
    offset += limit;
  }
  return items;
}

function normalizeCrowdinIdentifier(identifier) {
  if (typeof identifier !== "string") return "";
  if (identifier.startsWith('"') && identifier.endsWith('"') && identifier.length >= 2) {
    return identifier.slice(1, -1);
  }
  return identifier;
}

function effectiveCrowdinCurrent(translationText, sourceText) {
  if (translationText !== null && translationText !== undefined) return translationText;
  return sourceText;
}

async function getTranslationForString(token, crowdinLang, stringId) {
  const { status, payload } = await crowdinRequest(
    token,
    "GET",
    `/projects/${PROJECT_ID}/languages/${crowdinLang}/translations?stringIds=${stringId}`
  );
  if (status !== 200) return { ok: false, status, translation: null };
  const row = payload?.data?.[0]?.data;
  if (!row) return { ok: true, status, translation: null };
  return {
    ok: true,
    status,
    translation: { translation_id: row.translationId, text: row.text },
  };
}

function verifyUiJsSyntax(filePath) {
  const code = fs.readFileSync(filePath, "utf8");
  const ctx = { window: {} };
  vm.createContext(ctx);
  vm.runInContext(code, ctx);
  if (!ctx.window.LANGUAGE_UI_STRINGS) {
    throw new Error(`Syntax/structure error: missing LANGUAGE_UI_STRINGS in ${filePath}`);
  }
}

function loadDecisions() {
  const decisions = JSON.parse(fs.readFileSync(DECISIONS_JSON, "utf8"));
  if (decisions.schemaVersion !== 2) {
    throw new Error(`Expected schemaVersion 2, got ${decisions.schemaVersion}`);
  }
  if (decisions.ownerApproval?.status !== "OWNER_ACCEPTED") {
    throw new Error(`ownerApproval.status must be OWNER_ACCEPTED`);
  }
  const rows = (decisions.rows || []).filter(
    (r) =>
      r.ownerStatus === "LABOT" &&
      r.ownerApproved === true &&
      r.ownerDecision === "LABOT" &&
      typeof r.newValue === "string" &&
      r.newValue.length > 0
  );
  if (rows.length !== EXPECTED) {
    throw new Error(`Expected ${EXPECTED} LABOT ownerApproved rows, got ${rows.length}`);
  }
  return rows;
}

function groupByLanguage(rows) {
  const map = new Map();
  for (const row of rows) {
    if (!map.has(row.language)) map.set(row.language, []);
    map.get(row.language).push(row);
  }
  return map;
}

function applyRepoForLanguage(lang, langRows) {
  const jsonPath = path.join(ROOT, "crowdin", "ui", `${lang}.json`);
  const uiRel = UI_JS_REL(lang);
  const jsonFlat = parseCrowdinJson(fs.readFileSync(jsonPath, "utf8"));
  const { filePath, code, obj } = loadUiObject(uiRel);
  const uiFlat = flattenUiStrings(obj);

  const beforeJson = { ...jsonFlat };
  const beforeUi = { ...uiFlat };
  const entries = [];
  const toApply = [];

  for (const row of langRows) {
    const entry = {
      sequence: row.sequence,
      language: row.language,
      key: row.key,
      owner_current: row.current,
      owner_newValue: row.newValue,
      repo_before_json: jsonFlat[row.key],
      repo_before_ui: uiFlat[row.key],
      repo_after_json: "",
      repo_after_ui: "",
      crowdin_before: "",
      crowdin_after: "",
      repo_status: "pending",
      crowdin_status: "pending",
      crowdin_verified: "NO",
      apply_status: "pending",
      verification_status: "pending",
      string_id: "",
      translation_id: "",
    };

    if (jsonFlat[row.key] !== row.current || uiFlat[row.key] !== row.current) {
      if (jsonFlat[row.key] === row.newValue && uiFlat[row.key] === row.newValue) {
        entry.repo_status = "already_applied";
        entry.repo_after_json = row.newValue;
        entry.repo_after_ui = row.newValue;
        entry.apply_status = "already_applied";
        entry.verification_status = "already_pass";
        entries.push(entry);
        continue;
      }
      entry.repo_status = "skipped_current_mismatch";
      entry.apply_status = "CURRENT_VALUE_MISMATCH";
      entry.verification_status = "current_value_mismatch";
      entries.push(entry);
      continue;
    }

    if (row.newValue === row.current) {
      entry.repo_status = "already_correct";
      entry.repo_after_json = row.newValue;
      entry.repo_after_ui = row.newValue;
      entry.apply_status = "already_correct";
      entry.verification_status = "already_pass";
      entries.push(entry);
      continue;
    }

    toApply.push({ row, entry });
  }

  for (const { row } of toApply) {
    jsonFlat[row.key] = row.newValue;
  }

  if (toApply.length) {
    const sortedJson = {};
    for (const key of Object.keys(jsonFlat).sort()) sortedJson[key] = jsonFlat[key];
    fs.writeFileSync(jsonPath, `${JSON.stringify(sortedJson, null, 2)}\n`, "utf8");

    const patchFlat = {};
    for (const { row } of toApply) patchFlat[row.key] = row.newValue;
    const patch = applySurgicalCrowdinPatch(code, uiFlat, patchFlat);
    if (patch.changed) {
      fs.writeFileSync(filePath, patch.content, "utf8");
    }
  }

  verifyUiJsSyntax(filePath);

  const afterJsonFlat = parseCrowdinJson(fs.readFileSync(jsonPath, "utf8"));
  const afterUiFlat = flattenUiStrings(loadUiObject(uiRel).obj);

  let unexpectedChanges = 0;
  const labotKeys = new Set(langRows.map((r) => r.key));
  for (const key of Object.keys(beforeJson)) {
    if (labotKeys.has(key)) continue;
    if (afterJsonFlat[key] !== beforeJson[key] || afterUiFlat[key] !== beforeUi[key]) {
      unexpectedChanges += 1;
    }
  }

  let appliedVerified = 0;
  let currentValueMismatch = 0;
  let failed = 0;

  for (const entry of entries) {
    if (entry.verification_status === "already_pass") {
      if (
        !multisetEqual(
          extractPlaceholderMultiset(entry.owner_current),
          extractPlaceholderMultiset(entry.repo_after_json || entry.owner_newValue)
        )
      ) {
        entry.verification_status = "placeholder_mismatch";
        failed += 1;
        continue;
      }
      if (
        extractHtmlTagStructure(entry.owner_current) !==
        extractHtmlTagStructure(entry.repo_after_json || entry.owner_newValue)
      ) {
        entry.verification_status = "html_mismatch";
        failed += 1;
        continue;
      }
      appliedVerified += 1;
    } else if (entry.verification_status === "current_value_mismatch") {
      currentValueMismatch += 1;
    }
  }

  for (const { row, entry } of toApply) {
    entry.repo_after_json = afterJsonFlat[row.key];
    entry.repo_after_ui = afterUiFlat[row.key];

    if (entry.repo_after_json !== row.newValue || entry.repo_after_ui !== row.newValue) {
      entry.repo_status = "verify_fail";
      entry.apply_status = "repo_after_mismatch";
      entry.verification_status = "after_mismatch";
      failed += 1;
      entries.push(entry);
      continue;
    }

    if (
      !multisetEqual(
        extractPlaceholderMultiset(row.lvSource || row.current),
        extractPlaceholderMultiset(entry.repo_after_json)
      )
    ) {
      entry.repo_status = "placeholder_fail";
      entry.apply_status = "placeholder_fail";
      entry.verification_status = "placeholder_mismatch";
      failed += 1;
      entries.push(entry);
      continue;
    }

    if (
      extractHtmlTagStructure(row.lvSource || row.current) !==
      extractHtmlTagStructure(entry.repo_after_json)
    ) {
      entry.repo_status = "html_fail";
      entry.apply_status = "html_fail";
      entry.verification_status = "html_mismatch";
      failed += 1;
      entries.push(entry);
      continue;
    }

    entry.repo_status = "applied";
    entry.apply_status = "repo_applied";
    entry.verification_status = "repo_pass";
    appliedVerified += 1;
    entries.push(entry);
  }

  return {
    entries,
    unexpectedChanges,
    appliedVerified,
    currentValueMismatch,
    failed,
    changedFiles: [jsonPath, path.join(ROOT, uiRel)],
  };
}

async function applyCrowdinForLanguage(token, crowdinLang, entries, stringsByKey) {
  let crowdinVerified = 0;
  let crowdinFailed = 0;

  for (const entry of entries) {
    if (entry.verification_status === "current_value_mismatch") {
      entry.crowdin_status = "skipped_mismatch";
      continue;
    }
    if (entry.verification_status !== "repo_pass" && entry.verification_status !== "already_pass") {
      entry.crowdin_status = "skipped_repo_fail";
      continue;
    }

    const stringData = stringsByKey.get(entry.key);
    if (!stringData) {
      entry.crowdin_status = "missing_string";
      entry.crowdin_verified = "NO";
      crowdinFailed += 1;
      continue;
    }

    entry.string_id = stringData.id;
    const tr = await getTranslationForString(token, crowdinLang, stringData.id);
    const beforeText = effectiveCrowdinCurrent(tr.translation?.text, stringData.text);
    entry.crowdin_before = beforeText;

    if (beforeText === entry.owner_newValue) {
      entry.crowdin_status = "already_applied";
      entry.crowdin_after = entry.owner_newValue;
      entry.translation_id = tr.translation?.translation_id || "";
      entry.crowdin_verified = "YES";
      entry.apply_status =
        entry.apply_status === "already_applied" ? "already_applied" : "crowdin_already_applied";
      entry.verification_status =
        entry.verification_status === "already_pass" ? "already_pass" : "pass";
      crowdinVerified += 1;
      continue;
    }

    if (beforeText !== entry.owner_current) {
      entry.crowdin_status = "crowdin_current_mismatch";
      entry.crowdin_verified = "NO";
      entry.apply_status = "CURRENT_VALUE_MISMATCH";
      entry.verification_status = "current_value_mismatch";
      crowdinFailed += 1;
      continue;
    }

    if (tr.translation?.translation_id) {
      const patchResult = await crowdinRequest(token, "PATCH", `/projects/${PROJECT_ID}/translations`, [
        {
          op: "replace",
          path: `/${tr.translation.translation_id}`,
          value: { text: entry.owner_newValue },
        },
      ]);
      if (patchResult.status !== 200) {
        entry.crowdin_status = `patch_fail_${patchResult.status}`;
        entry.crowdin_verified = "NO";
        entry.apply_status = "crowdin_patch_fail";
        crowdinFailed += 1;
        continue;
      }
      entry.translation_id =
        patchResult.payload?.data?.[0]?.data?.id || tr.translation.translation_id;
      entry.crowdin_status = "patched";
    } else {
      const postResult = await crowdinRequest(
        token,
        "POST",
        `/projects/${PROJECT_ID}/translations`,
        { stringId: stringData.id, languageId: crowdinLang, text: entry.owner_newValue }
      );
      if (postResult.status !== 201) {
        entry.crowdin_status = `post_fail_${postResult.status}`;
        entry.crowdin_verified = "NO";
        entry.apply_status = "crowdin_post_fail";
        crowdinFailed += 1;
        continue;
      }
      entry.translation_id = postResult.payload?.data?.id || "";
      entry.crowdin_status = "posted";
    }

    const verify = await getTranslationForString(token, crowdinLang, stringData.id);
    entry.crowdin_after = verify.translation?.text ?? "";
    if (verify.translation?.text === entry.owner_newValue) {
      entry.crowdin_verified = "YES";
      entry.apply_status = entry.apply_status.includes("already") ? entry.apply_status : "applied_verified";
      entry.verification_status =
        entry.verification_status === "already_pass" ? "already_pass" : "pass";
      crowdinVerified += 1;
    } else {
      entry.crowdin_verified = "NO";
      entry.crowdin_status += "_verify_fail";
      entry.apply_status = "crowdin_verify_fail";
      crowdinFailed += 1;
    }
  }

  return { crowdinVerified, crowdinFailed };
}

function writeProof(summary, entries) {
  fs.mkdirSync(path.dirname(PROOF_JSON), { recursive: true });
  fs.writeFileSync(PROOF_JSON, `${JSON.stringify({ summary, entries }, null, 2)}\n`, "utf8");

  const csvHeader =
    "sequence,language,key,owner_current,owner_newValue,repo_before_json,repo_before_ui,repo_after_json,repo_after_ui,crowdin_before,crowdin_after,repo_status,crowdin_status,crowdin_verified,apply_status,verification_status";
  const csvRows = entries.map((e) =>
    [
      e.sequence,
      e.language,
      csvEscape(e.key),
      csvEscape(e.owner_current),
      csvEscape(e.owner_newValue),
      csvEscape(e.repo_before_json),
      csvEscape(e.repo_before_ui),
      csvEscape(e.repo_after_json),
      csvEscape(e.repo_after_ui),
      csvEscape(e.crowdin_before),
      csvEscape(e.crowdin_after),
      csvEscape(e.repo_status),
      csvEscape(e.crowdin_status),
      csvEscape(e.crowdin_verified),
      csvEscape(e.apply_status),
      csvEscape(e.verification_status),
    ].join(",")
  );
  fs.writeFileSync(PROOF_CSV, `${[csvHeader, ...csvRows].join("\n")}\n`, "utf8");

  const md = [
    "# Crowdin UI — NEEDS_OWNER_REVIEW 78 LABOT COPY-ONLY apply proof",
    "",
    `**Generated:** ${summary.generatedAt}  `,
    `**Commit before:** \`${summary.commitBefore}\`  `,
    `**Commit after:** \`${summary.commitAfter || "pending"}\`  `,
    `**Authority:** \`reports/crowdin-ui-needs-owner-review-decisions.json\`  `,
    `**Project ID:** ${PROJECT_ID}`,
    "",
    "## Metrics",
    "",
    "| Metric | Value |",
    "|---|---:|",
    `| REQUESTED | ${summary.REQUESTED} |`,
    `| APPLIED_VERIFIED | ${summary.APPLIED_VERIFIED} |`,
    `| CURRENT_VALUE_MISMATCH | ${summary.CURRENT_VALUE_MISMATCH} |`,
    `| FAILED | ${summary.FAILED} |`,
    `| UNEXPECTED_CHANGES | ${summary.UNEXPECTED_CHANGES} |`,
    `| CROWDIN_VERIFIED | ${summary.CROWDIN_VERIFIED} |`,
    `| idempotentRerunVerified | ${summary.idempotentRerunVerified ? "YES" : "NO"} |`,
    "",
    `**Result:** ${summary.result}`,
    "",
    "## Changed production files",
    "",
    ...summary.changedFiles.map((f) => `- \`${f.replace(`${ROOT}/`, "")}\``),
    "",
    "## Per-row detail",
    "",
    "| lang | key | owner current → newValue | verification |",
    "| --- | --- | --- | --- |",
    ...entries.map(
      (e) =>
        `| ${e.language} | ${escapeTableCell(e.key)} | ${escapeTableCell(e.owner_current)} → ${escapeTableCell(e.owner_newValue)} | ${e.verification_status} |`
    ),
    "",
  ];
  fs.writeFileSync(PROOF_MD, `${md.join("\n")}\n`, "utf8");
}

async function main() {
  const apply = process.argv.includes("--apply");
  const commitBefore = execSync("git rev-parse HEAD", { encoding: "utf8" }).trim();
  const rows = loadDecisions();
  const byLang = groupByLanguage(rows);

  const summary = {
    generatedAt: new Date().toISOString(),
    commitBefore,
    commitAfter: null,
    projectId: PROJECT_ID,
    authority: "reports/crowdin-ui-needs-owner-review-decisions.json",
    REQUESTED: EXPECTED,
    APPLIED_VERIFIED: 0,
    CURRENT_VALUE_MISMATCH: 0,
    FAILED: 0,
    UNEXPECTED_CHANGES: 0,
    CROWDIN_VERIFIED: 0,
    TOKEN_EXPOSED: "NO",
    idempotentRerunVerified: false,
    changedFiles: [],
    result: "PENDING",
  };

  if (!apply) {
    let mismatch = 0;
    for (const [lang, langRows] of byLang) {
      const jsonFlat = parseCrowdinJson(
        fs.readFileSync(path.join(ROOT, "crowdin", "ui", `${lang}.json`), "utf8")
      );
      const uiFlat = flattenUiStrings(loadUiObject(UI_JS_REL(lang)).obj);
      for (const row of langRows) {
        if (jsonFlat[row.key] !== row.current || uiFlat[row.key] !== row.current) mismatch += 1;
      }
    }
    console.log(
      JSON.stringify(
        {
          ...summary,
          message: "Dry run. Re-run with --apply.",
          languages: [...byLang.keys()],
          preflightCurrentMismatch: mismatch,
          readyToApply: mismatch === 0,
        },
        null,
        2
      )
    );
    return;
  }

  const allEntries = [];
  let totalUnexpected = 0;
  let totalMismatch = 0;
  let totalFailed = 0;
  let totalApplied = 0;

  for (const [lang, langRows] of byLang) {
    const repoResult = applyRepoForLanguage(lang, langRows);
    allEntries.push(...repoResult.entries);
    totalUnexpected += repoResult.unexpectedChanges;
    totalMismatch += repoResult.currentValueMismatch;
    totalFailed += repoResult.failed;
    totalApplied += repoResult.appliedVerified;
    summary.changedFiles.push(...repoResult.changedFiles.map((f) => f.replace(`${ROOT}/`, "")));
  }

  summary.APPLIED_VERIFIED = totalApplied;
  summary.CURRENT_VALUE_MISMATCH = totalMismatch;
  summary.FAILED = totalFailed;
  summary.UNEXPECTED_CHANGES = totalUnexpected;

  if (
    totalFailed > 0 ||
    totalMismatch > 0 ||
    totalUnexpected > 0 ||
    totalApplied !== EXPECTED
  ) {
    summary.result = "FAIL";
    writeProof(summary, allEntries.sort((a, b) => a.sequence - b.sequence));
    console.log(JSON.stringify(summary, null, 2));
    process.exit(1);
  }

  const token = requireToken();
  const files = await crowdinGetAll(token, `/projects/${PROJECT_ID}/files`);
  const sourceFile = files
    .map((item) => item.data || item)
    .find((f) => f.path === SOURCE_FILE_PATH);
  if (!sourceFile) throw new Error(`Missing source file ${SOURCE_FILE_PATH}`);

  const strings = await crowdinGetAll(
    token,
    `/projects/${PROJECT_ID}/strings?fileId=${sourceFile.id}`
  );
  const stringsByKey = new Map();
  for (const item of strings) {
    const s = item.data || item;
    const k = normalizeCrowdinIdentifier(s.identifier);
    if (k) stringsByKey.set(k, s);
  }

  let crowdinVerified = 0;
  let crowdinFailed = 0;

  for (const [lang, langRows] of byLang) {
    const crowdinLang = crowdinCodeFromRepoLang(lang);
    const langEntries = allEntries.filter((e) => e.language === lang);
    const probe = await crowdinRequest(
      token,
      "GET",
      `/projects/${PROJECT_ID}/languages/${crowdinLang}/translations?limit=1`
    );
    if (probe.status !== 200) {
      throw new Error(`Crowdin probe failed for ${crowdinLang} HTTP ${probe.status}`);
    }
    const cr = await applyCrowdinForLanguage(token, crowdinLang, langEntries, stringsByKey);
    crowdinVerified += cr.crowdinVerified;
    crowdinFailed += cr.crowdinFailed;
  }

  summary.CROWDIN_VERIFIED = crowdinVerified;
  summary.FAILED += crowdinFailed;

  const pass =
    summary.REQUESTED === EXPECTED &&
    summary.APPLIED_VERIFIED === EXPECTED &&
    summary.CURRENT_VALUE_MISMATCH === 0 &&
    summary.FAILED === 0 &&
    summary.UNEXPECTED_CHANGES === 0 &&
    summary.CROWDIN_VERIFIED === EXPECTED;

  summary.idempotentRerunVerified = pass;
  summary.result = pass ? "PASS" : "FAIL";

  writeProof(summary, allEntries.sort((a, b) => a.sequence - b.sequence));
  console.log(JSON.stringify(summary, null, 2));
  if (!pass) process.exit(1);
}

main().catch((err) => {
  console.error(err.message);
  process.exit(1);
});
