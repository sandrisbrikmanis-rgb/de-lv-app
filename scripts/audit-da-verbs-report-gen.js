...
const REPORT = path.join(ROOT, "reports/da-verbs-full-audit.md");
...
function main() {
  ...
  const verdict = !meta.completenessPass
    ? "**DA–DE VERBS FULL AUDIT — INCOMPLETE**"
    : realCount > 0
      ? "**DA–DE VERBS FULL AUDIT — NEEDS REPAIR**"
      : "**DA–DE VERBS FULL AUDIT — PASS**";
  ...
  for (const sev of ["CRITICAL", "HIGH", "MEDIUM", "LOW", "NEEDS_SOURCE_REVIEW"]) {
    ...
    for (const f of group.slice(0, 25)) {
      ...
      md.push(`- **DA_CURRENT:** ${truncate(f.currentDa)}`);
      md.push(`- **PROPOSED_DA:** ${truncate(f.proposedDa)}`);
