...
"use strict";
/**
 * ES-DE module collector (READ-ONLY).
 * Usage: node scripts/audit-es-de-collect.js --module=b2|c1|c2|sentences|verbs
 */
const fs = require("fs");
...
function classifyForeign(text) {
  ...
  if (IT_WORDS.test(text)) reasons.push("IT");
...
function collectVocab() {
  ...
  if (lvStudy !== esStudy) {
    data.structural.pass = false;
    data.structural.issues.push({ severity: "critical", message: `Study count mismatch DE=${lvStudy} ES=${esStudy}` });
  }
