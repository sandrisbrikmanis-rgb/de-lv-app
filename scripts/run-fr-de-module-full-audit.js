...
"use strict";
/**
 * FR-DE single-module FIRST FULL DISCOVERY audit orchestrator (MASTER v1.9, READ-ONLY).
 * Usage: node scripts/run-fr-de-module-full-audit.js --module=b2|c1|c2|sentences|verbs [--skip-luna] [--test-luna] [--fresh-luna]
 */
...
const { parseModuleArg, MASTER_VERSION, MASTER_FILE } = require("./lib/fr-de-audit-config");
const {
  buildCards,
  mapSeverity,
  mapCategory,
...
const { classifyFindings, mapValidationStatus } = require("./lib/openai-fr-de-audit");
...
function buildSummaryMd(ctx) {
  return [
    `# FR–DE ${cfg.moduleKey} — audita kopsavilkums`,
    "",
    `**Verdict:** **${ctx.verdict}**`,
    `**HEAD:** \`${ctx.head}\``,
...
function buildFullMd(ctx) {
  return [
    `# FR–DE ${cfg.moduleKey} pilns lingvistiskais audits (MASTER v1.9 FIRST_FULL_DISCOVERY)`,
    "",
    `**HEAD:** \`${ctx.head}\``,
...
async function main() {
  console.log(`\n=== FR–DE ${cfg.moduleKey} FIRST FULL DISCOVERY (MASTER v1.9) ===\n`);
  const head = git("git rev-parse HEAD");

  fs.mkdirSync(path.join(ROOT, "reports/temp"), { recursive: true });
  runNode("audit-fr-de-collect.js", [`--module=${cfg.moduleKey.toLowerCase()}`]);
  ...
  if (!SKIP_LUNA) {
    ...
    runNode("audit-fr-de-linguistic.js", lunaArgs);
  }
  ...
  runNode("build-fr-de-module-owner.js", [`--module=${cfg.moduleKey.toLowerCase()}`]);
  if (coverageComplete) runNode("validate-fr-de-module-full-audit.js", [`--module=${cfg.moduleKey.toLowerCase()}`]);
