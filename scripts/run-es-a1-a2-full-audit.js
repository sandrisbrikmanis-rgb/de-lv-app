#!/usr/bin/env node
/**
 * ES-DE A1+A2 full READ-ONLY audit orchestrator (deterministic + GPT-5.6 Luna).
 * Usage: node scripts/run-es-a1-a2-full-audit.js [--skip-luna] [--test-luna] [--fresh-luna]
 */
"use strict";
...
const { LUNA_JSON, TEMP_DIR } = require("./lib/es-a1-a2-audit-helpers");
const { classifyFindings } = require("./lib/openai-es-a1-a2-audit");
...
function formatLunaFinding(f, index) {
  ...
  return [
    ...
    `**Field:** ${f.field}`,
    `**CURRENT:** ${f.currentEs || "—"}`,
    f.proposedEs ? `**PROPOSED_ES:** ${f.proposedEs}` : "",
    `**Problēma:** ${f.reason}`,
...
function buildReport(ctx) {
  const lines = [];
  lines.push("# ES–DE A1+A2 pilns lingvistiskais un kvalitātes audits");
  lines.push("");
  ...
  lines.push("**Apjoms:** `data/es/a1.js`, `data/es/a2.js` (+ `www/data/es/` mirror)");
  ...
  lines.push(`| Study objekti (LV / ES) | **134 / ${ctx.a1.esStudy}** | **231 / ${ctx.a2.esStudy}** | **365 / ${ctx.a1.esStudy + ctx.a2.esStudy}** |`);
  ...
  if (!ctx.lunaRan) {
    ...
    lines.push(
      "> **Luna TEST BATCH režīms** — pārbaudīts tikai 1 batch. Pilnam auditam: `node scripts/run-es-a1-a2-full-audit.js --fresh-luna` (bez `--test-luna`).",
    );
    lines.push("");
  }
  ...
  if (ctx.lunaFindings.length) {
    ...
    if (ctx.lunaFindings.length > show.length) {
      lines.push(`> … un vēl **${ctx.lunaFindings.length - show.length}** atradumi — pilns saraksts: \`reports/temp/es-a1-a2-linguistic-audit.json\``);
      lines.push("");
    }
  }
  ...
  lines.push("");
  lines.push("Pilns deterministiskais JSON: `reports/temp/es-de-a1-a2-audit-data.json`");
  lines.push("");
  ...
  lines.push("|------|---------|");
  lines.push("| Kolektors | `node scripts/audit-es-a1-a2-collect.js` |");
  lines.push("| Luna | `node scripts/audit-es-a1-a2-linguistic.js [--test-batch] [--resume]` |");
  lines.push("| Orķestrators | `node scripts/run-es-a1-a2-full-audit.js [--skip-luna] [--test-luna] [--fresh-luna]` |");
  lines.push("| Paritāte | `node scripts/audit-language-parity.js --lang=es` |");
  lines.push("| Study dizains | `node scripts/validate-study-design.js --lang=es` |");
  lines.push("");
...
function main() {
  ...
  if (FRESH_LUNA) {
    ...
    const progressFile = path.join(ROOT, "scripts", ".es-a1-a2-luna-progress.json");
  ...
  if (!SKIP_LUNA) {
    if (!process.env.OPENAI_API_KEY?.trim()) {
      ...
      try {
        ...
        runNode("audit-es-a1-a2-linguistic.js", lunaArgs);
        lunaRan = true;
  ...
  const a1 = {
    esStudy: collect.levels?.[0]?.counts?.esStudy || 0,
  ...
  const a2 = {
    esStudy: collect.levels?.[1]?.counts?.esStudy || 0,
