...
"use strict";
/**
 * FR-DE A1 linguistic audit with GPT-5.6 Luna (READ-ONLY).
 * Usage: node scripts/audit-fr-a1-linguistic.js [--test-batch] [--resume]
 */
...
const {
  ...
  auditCardsBatch,
  classifyFindings,
} = require("./lib/openai-fr-a1-audit");
...
async function main() {
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "fr-a1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME) {
    auditData = { meta: { dataset: "fr-a1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }

  const { simple, study } = buildCards();
  const totalCards = simple.length + study.length;
  console.log(`FR A1 Luna audit: ${totalCards} entries (${simple.length} simple, ${study.length} study)`);
