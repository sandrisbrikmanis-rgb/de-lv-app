#!/usr/bin/env node
/**
 * Generate BS-DE data from SL baseline (SL was generated from MK with DE READ-ONLY).
 * Translates only native-language (lv) fields SL → BS.
 * German (de) fields are preserved verbatim from SL/LV source (READ-ONLY).
 */
const fs = require("fs");
...
const CACHE_PATH = path.join(ROOT, "scripts", ".bs-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "sl";
const TO_LANG = "bs";
...
async function main() {
  ...
  console.log("Collecting unique Slovenian strings from SL-DE for BS translation...");
  const a1 = loadData("data/sl/a1.js").A1_WORDS;
  const a2 = loadData("data/sl/a2.js").A2_WORDS;
  const b1 = loadData("data/sl/b1.js").B1_WORDS;
  const b2 = loadData("data/sl/b2.js").B2_WORDS;
  const c1 = loadData("data/sl/c1.js").C1_WORDS;
  const c2 = loadData("data/sl/c2.js").C2_WORDS;
  const sentences = loadData("data/sl/sentences.js").SENTENCE_ENTRIES;
  const verbs = loadData("data/sl/verbs.js").VERB_ENTRIES;
  const dialogue = loadData("data/sl/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const course = loadData("data/sl/courseLessons.js");
  ...
  console.log("\nBS-DE data generation complete (SL → BS).");
}
