#!/usr/bin/env node
/**
 * Verify IS-DE creation compliance:
 * 1. German (de) fields in IS data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const is = loadArray(`data/is/${level}.js`);
  const n = Math.min(lv.length, is.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const isFields = collectDeFields(is[i]);
    for (const lf of lvFields) {
      const tf = isFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, is: tf?.value });
      }
    }
  }
}
...
const isSent = loadArray("data/is/sentences.js");
lvSent.forEach((lv, i) => {
  if (isSent[i] && lv.de !== isSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, sv: isSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const isVerbs = loadArray("data/is/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const isEntry = isVerbs[i];
  if (!isEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== isEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, sv: isEntry[form]?.de });
    }
  }
});
