#!/usr/bin/env node
/**
 * Fix NN data: restore German comparison.word and other DE fields from LV source.
 */
const fs = require("fs");
...
for (const level of LEVELS) {
  ...
  const nb = loadArray(`data/nn/${level}.js`);
  for (let i = 0; i < Math.min(lv.length, nb.length); i++) {
    restoreDeFields(lv[i], nb[i]);
  }
  writeArrayFile(path.join(ROOT, "data/nn", `${level}.js`), VAR_NAMES[level], nb);
  console.log(`Fixed ${level}: ${nb.length} entries`);
}

const lvSent = loadArray("data/sentences.js");
const csSent = loadArray("data/nn/sentences.js");
lvSent.forEach((t, i) => { if (csSent[i]) csSent[i].de = t.de; });
writeArrayFile(path.join(ROOT, "data/nn/sentences.js"), "SENTENCE_ENTRIES", csSent);
console.log(`Fixed sentences: ${csSent.length}`);

const lvVerbs = loadArray("data/verbs.js");
const csVerbs = loadArray("data/nn/verbs.js");
lvVerbs.forEach((t, i) => {
  if (!csVerbs[i]) return;
...
writeArrayFile(path.join(ROOT, "data/nn/verbs.js"), "VERB_ENTRIES", csVerbs);
...
const csDlgCode = fs.readFileSync(path.join(ROOT, "data/nn/dialogueIdMap.js"), "utf8");
...
fs.writeFileSync(
  path.join(ROOT, "data/nn/dialogueIdMap.js"),
