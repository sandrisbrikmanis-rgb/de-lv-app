...
"use strict";
/**
 * ES-DE B2, C1, C2, Teikumi, Darbības vārdi — FIRST FULL DISCOVERY master orchestrator.
 * Usage: node scripts/run-es-de-b2-c1-c2-sentences-verbs-full-audit.js [--skip-luna] [--module=b2] [--fresh-luna]
 */
...
const { ALL_MODULE_KEYS, getModuleConfig, MASTER_VERSION, MASTER_FILE } = require("./lib/es-de-audit-config");
...
function runModule(moduleKey) {
  ...
  const result = spawnSync("node", [path.join(ROOT, "scripts/run-es-de-module-full-audit.js"), ...args], {
    cwd: ROOT,
    env: process.env,
    encoding: "utf8",
...
function main() {
  ...
  console.log(`\n=== ES–DE B2/C1/C2/SENTENCES/VERBS FIRST FULL DISCOVERY ===`);
  ...
  const summaryLines = [
    "# ES–DE B2/C1/C2/Teikumi/Darbības vārdi — FIRST FULL DISCOVERY",
    "",
    `**HEAD:** \`${head}\``,
  ...
  const masterSummary = path.join(ROOT, "reports/es-de-b2-c1-c2-sentences-verbs-full-audit-summary.md");
