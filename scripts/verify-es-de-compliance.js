#!/usr/bin/env node
/**
 * Verify ES-DE creation compliance:
 * 1. German (de) fields in ES data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const es = loadArray(`data/es/${level}.js`);
  const n = Math.min(lv.length, es.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const esFields = collectDeFields(es[i]);
    for (const lf of lvFields) {
      const tf = esFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, es: tf?.value });
      }
    }
  }
}
...
const esSent = loadArray("data/es/sentences.js");
lvSent.forEach((lv, i) => {
  if (esSent[i] && lv.de !== esSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, es: esSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const esVerbs = loadArray("data/es/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const esEntry = esVerbs[i];
  if (!esEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== esEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, es: esEntry[form]?.de });
    }
  }
});
