...
for (const level of LEVELS) {
  ...
  const da = loadArray(`data/es/${level}.js`);
...
const csSent = loadArray("data/es/sentences.js");
lvSent.forEach((t, i) => { if (csSent[i]) csSent[i].de = t.de; });
writeArrayFile(path.join(ROOT, "data/es/sentences.js"), "SENTENCE_ENTRIES", csSent);
console.log(`Fixed sentences: ${csSent.length}`);

const lvVerbs = loadArray("data/verbs.js");
const csVerbs = loadArray("data/es/verbs.js");
lvVerbs.forEach((t, i) => {
  if (!csVerbs[i]) return;
...
writeArrayFile(path.join(ROOT, "data/es/verbs.js"), "VERB_ENTRIES", csVerbs);
...
const csDlgCode = fs.readFileSync(path.join(ROOT, "data/es/dialogueIdMap.js"), "utf8");
...
fs.writeFileSync(
  path.join(ROOT, "data/es/dialogueIdMap.js"),
