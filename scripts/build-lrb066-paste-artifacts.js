...
const PASTE_PATH =
  process.argv[2] ||
  path.join(ROOT, "scripts/data/g2-a1-owner-pending/LRB-066-decisions-COPY-PASTE.json");
const EXPECTED_PASTE_SHA =
  "1b3661b611e31cd44f77b3486ae96457221edece966ad82e4068ad5622367798";
...
function resolveCardDecision(mapping, cardObjectId) {
  if (mapping.nelabot && cardObjectId in mapping.nelabot) {
    const nelabotNote =
      ...
      `LRB-066 OWNER mapping NELABOT: preserve production for ${cardObjectId}.`;
    return {
  ...
  if (mapping.labot_full_composite && cardObjectId in mapping.labot_full_composite) {
    ...
    return {
      ...
      owner_note: `LRB-066 OWNER mapping full_card_target_fields: ${cardObjectId}.`,
    };
  }
  if (mapping.labot_scalar && cardObjectId in mapping.labot_scalar) {
    return {
      ...
      replacement_scope: "finding_field_only",
      owner_note: `LRB-066 OWNER mapping scalar lv: ${cardObjectId}.`,
    };
  }
  return null;
}
...
function main() {
  ...
  const expectedInputSha =
    "0d5895c65316b6d28402ef2ba6960af64219f742d98a023b7f99ac3e5c845dcb";
  ...
  if (labot !== 48 || nelabot !== 2 || pending !== 0) {
  ...
  if (fullCardIds.size !== 5) {
    throw new Error(`Expected 5 full composites, got ${fullCardIds.size}`);
  }

  const scopeLabel = mapping.scope
    ? `${mapping.scope.from}..${mapping.scope.to} (50 LB rows, 50 unique cards)`
    : "LRB-066";
  ...
  const galaOut = {
    ...
    classification: "LRB_066_COPY_PASTE_COMPLETE_AWAITING_GALA_VERDICT",
    cards: galaCards,
  };
