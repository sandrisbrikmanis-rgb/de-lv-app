#!/usr/bin/env node
/**
 * Verify NB-DE creation compliance:
 * 1. German (de) fields in NB data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const nb = loadArray(`data/nb/${level}.js`);
  const n = Math.min(lv.length, nb.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const svFields = collectDeFields(nb[i]);
...
const nbSent = loadArray("data/nb/sentences.js");
lvSent.forEach((lv, i) => {
  if (nbSent[i] && lv.de !== nbSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, sv: nbSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const nbVerbs = loadArray("data/nb/verbs.js");
