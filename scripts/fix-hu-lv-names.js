...
function replaceNames(text) {
  ...
  let out = text;
  for (const [from, to] of NAME_REPLACEMHUTS) out = out.replace(from, to);
  return out;
}
...
["a1", "a2", "b1", "b2", "c1", "c2"].forEach((level) => {
  processJsFile(`data/hu/${level}.js`);
});
processJsFile("data/hu/sentences.js", "SHUTHUCE_HUTRIES");
processJsFile("data/hu/verbs.js", "VERB_HUTRIES");
processJsFile("data/hu/dialogueIdMap.js", "DIALOGUE_ID_MAP");
processJsFile("data/hu/courseLessons.js");
// courseTrainingCards.js uses multiple window.* assignments — regenerate separately

console.log("HU name replacement complete.");
