#!/usr/bin/env node
/**
 * BS-DE A1 full audit collector (read-only). Writes reports/temp/bs-a1-audit-data.json
 */
const fs = require("fs");
...
const OUT = path.join(ROOT, "reports", "temp", "bs-a1-audit-data.json");
...
function main() {
  ...
  const data = {
    meta: {
      ...
      lvFile: "data/a1.js",
      bsFile: "data/bs/a1.js",
      wwwFile: "www/data/bs/a1.js",
      lvCount: lv.length,
      bsCount: bs.length,
    },
  ...
  if (!data.layerIdentity.identical) {
    ...
    data.structural.issues.push({ severity: "critical", message: "data/bs/a1.js and www/data/bs/a1.js are not identical" });
  }
