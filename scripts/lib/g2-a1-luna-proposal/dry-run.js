...
function renderQueueSummary(result) {
  return [
    ...
    `- Owner conflict review required: ${result.reconciliation.ownerConflictReviewRequired || 0}`,
    `- Batch-eligible units: ${result.counts.batchEligibleUnits || result.counts.individualExecutionUnits}`,
...
function runDryRun(options = {}) {
  ...
  const result = {
    ...
    reconciliation: {
      ...reconciliation,
      ownerConflictReviewRequired: reconciliation.ownerConflictReviewRequired || 0,
    },
    batchPlan: {
  ...
  if (options.writeArtifacts !== false) {
    ...
    writeJson(path.join(outDir, "queue-proof.json"), {
      ...
      reconciliation: {
        ...
        ownerConflictReviewRequired: result.reconciliation.ownerConflictReviewRequired || 0,
      },
