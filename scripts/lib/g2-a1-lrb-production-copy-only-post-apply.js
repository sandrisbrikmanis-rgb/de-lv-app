...
function runPostApplyVerification(options = {}) {
  ...
  for (const card of atomic.cards) {
    ...
    if (seenTargetKeys.has(card.card_key)) duplicateTargets += 1;
    seenTargetKeys.add(card.card_key);
  ...
  let changedTargetCards = 0;
  for (const card of atomic.cards) {
    if (card.atomic_status !== "ATOMIC_READY_APPLY") continue;
    const resolved = resolveProductionTargetStrict(card.target_language, card.canonical_card_object_id);
    if (!resolved.ok) continue;
    const liveSha = productionEntrySha256(resolved.entry);
    if (liveSha !== card.production_current_entry_sha256) changedTargetCards += 1;
  }
  for (const lang of langs) {
    const dataRel = productionFileRel(lang);
  ...
  const changedFiles = git(`git diff --name-only ${baseHead}`).split("\n").filter(Boolean);
  ...
  if (changedTargetCards !== CARD_COUNT) {
    blockers.push(`changed_target_cards:${changedTargetCards}`);
  }
  ...
  if (duplicateTargets !== 0) blockers.push(`duplicate_targets:${duplicateTargets}`);

  const pass =
    blockers.length === 0 &&
    appliedCards === CARD_COUNT &&
    ownerLeafMatches === LEAF_COUNT &&
    changedProductionFiles === FILE_COUNT &&
    changedTargetCards === CARD_COUNT &&
  ...
  return {
    ...
    changed_target_cards: changedTargetCards,
    ...
    classification: pass
      ? "A1_LRB_001_103_PRODUCTION_COPY_ONLY_APPLY_COMPLETE_AWAITING_OWNER_VERIFICATION"
      : "A1_LRB_001_103_PRODUCTION_COPY_ONLY_APPLY_BLOCKED",
    next_action: pass ? "OWNER_VERIFY_PRODUCTION_APPLY_AND_AUTHORIZE_MERGE" : "BLOCKED_DO_NOT_REAPPLY",
  };
}
...
function writePostApplyArtifacts(verification, liveApplySummary = null) {
  ...
  const result = {
    ...
    applied_cards: verification.applied_cards,
    owner_leaf_matches: verification.owner_leaf_matches,
  ...
  const proof = {
    ...
    cards_applied: `${verification.applied_cards}/${verification.total_cards}`,
...
| Applied cards | ${verification.applied_cards}/${verification.total_cards} |
...
Live apply executed **once** on branch \`cursor/a1-lrb-001-103-production-copy-only-apply\`. PR not merged until OWNER verification.
`;
