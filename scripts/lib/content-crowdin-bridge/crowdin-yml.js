#!/usr/bin/env node
"use strict";

const fs = require("fs");
const path = require("path");
const YAML = require("yaml");
const { ROOT } = require("../audit-common");
const {
  CROWDIN_TARGET_LOCALE_IDS,
  crowdinLocaleToRepo,
  buildCrowdinLanguagesMapping,
} = require("./locale-map");

const CROWDIN_YML_PATH = path.join(ROOT, "crowdin.yml");

const EXPECTED_UI_FILE = {
  source: "/crowdin/ui/lv.json",
  translation: "/crowdin/ui/%two_letters_code%.json",
};

const EXPECTED_G2_A1_FILE = {
  source: "/crowdin/content/g2/lv-a1.json",
  translation: "/crowdin/content/g2/%locale%-a1.json",
};

function loadCrowdinYmlConfig(ymlPath = CROWDIN_YML_PATH) {
  const text = fs.readFileSync(ymlPath, "utf8");
  if (/^\s*languages_mapping:/m.test(text) && !/translation:[^\n]+\n\s+languages_mapping:/m.test(text)) {
    throw new Error("CROWDIN_YML_INVALID: top-level languages_mapping is not allowed");
  }
  let parsed;
  try {
    parsed = YAML.parse(text);
  } catch (err) {
    const wrap = new Error(`CROWDIN_YML_PARSE_ERROR:${err.message}`);
    wrap.code = "CROWDIN_YML_PARSE_ERROR";
    throw wrap;
  }
  if (!parsed || typeof parsed !== "object" || !Array.isArray(parsed.files)) {
    throw new Error("CROWDIN_YML_INVALID: root must contain files array");
  }
  return parsed;
}

function findG2A1FileEntry(config) {
  const entry = config.files.find((f) => f && f.source === EXPECTED_G2_A1_FILE.source);
  if (!entry) throw new Error("CROWDIN_YML_INVALID: missing G2/A1 file entry");
  return entry;
}

function findUiFileEntry(config) {
  const entry = config.files.find((f) => f && f.source === EXPECTED_UI_FILE.source);
  if (!entry) throw new Error("CROWDIN_YML_INVALID: missing UI file entry");
  return entry;
}

function repoFilenameForCrowdinLocale(crowdinLocale, level = "a1", localeMap) {
  if (!localeMap || typeof localeMap !== "object") {
    throw new Error(`UNKNOWN_CROWDIN_LOCALE:${crowdinLocale}`);
  }
  if (!Object.prototype.hasOwnProperty.call(localeMap, crowdinLocale)) {
    throw new Error(`UNKNOWN_CROWDIN_LOCALE:${crowdinLocale}`);
  }
  const repoLang = localeMap[crowdinLocale];
  return `${repoLang}-${level}.json`;
}

function validateCrowdinYmlG2A1(ymlPath = CROWDIN_YML_PATH) {
  const errors = [];
  const config = loadCrowdinYmlConfig(ymlPath);
  const expectedFullMap = buildCrowdinLanguagesMapping();

  if (config.files.length !== 2) {
    errors.push(`FILES_COUNT:${config.files.length}`);
  }

  const ui = findUiFileEntry(config);
  if (ui.source !== EXPECTED_UI_FILE.source || ui.translation !== EXPECTED_UI_FILE.translation) {
    errors.push("UI_MAPPING_CHANGED");
  }
  if (ui.languages_mapping) {
    errors.push("UI_ENTRY_HAS_LANGUAGES_MAPPING");
  }

  const g2 = findG2A1FileEntry(config);
  if (g2.translation !== EXPECTED_G2_A1_FILE.translation) {
    errors.push("G2_A1_TRANSLATION_PATTERN_CHANGED");
  }
  const localeMap = g2.languages_mapping?.locale;
  if (!localeMap || typeof localeMap !== "object") {
    errors.push("G2_A1_LANGUAGES_MAPPING_MISSING");
    return { pass: false, errors, ui, g2, repoFilenames: [] };
  }

  const yamlCrowdinIds = Object.keys(localeMap);
  if (yamlCrowdinIds.length !== 31) {
    errors.push(`LOCALE_MAP_COUNT:${yamlCrowdinIds.length}`);
  }

  for (const crowdinId of yamlCrowdinIds) {
    if (!CROWDIN_TARGET_LOCALE_IDS.includes(crowdinId)) {
      errors.push(`UNKNOWN_CROWDIN_LOCALE_IN_YAML:${crowdinId}`);
    }
  }

  for (const crowdinId of CROWDIN_TARGET_LOCALE_IDS) {
    if (!Object.prototype.hasOwnProperty.call(localeMap, crowdinId)) {
      errors.push(`LOCALE_MAP_MISSING:${crowdinId}`);
    } else if (localeMap[crowdinId] !== expectedFullMap[crowdinId]) {
      errors.push(`LOCALE_MAP_MISMATCH:${crowdinId}`);
    }
  }

  const filenames = new Set();
  for (const crowdinId of CROWDIN_TARGET_LOCALE_IDS) {
    try {
      const repoFile = repoFilenameForCrowdinLocale(crowdinId, "a1", localeMap);
      if (filenames.has(repoFile)) {
        errors.push(`DUPLICATE_REPO_FILENAME:${repoFile}`);
      }
      filenames.add(repoFile);
      if (repoFile !== `${expectedFullMap[crowdinId]}-a1.json`) {
        errors.push(`REPO_FILENAME_MISMATCH:${crowdinId}`);
      }
    } catch (err) {
      errors.push(err.message);
    }
  }

  if (repoFilenameForCrowdinLocale("el", "a1", localeMap) !== "gr-a1.json") {
    errors.push("EL_NOT_GR_A1");
  }
  const regionalChecks = {
    "en-GB": "en-a1.json",
    "es-ES": "es-a1.json",
    "pt-PT": "pt-a1.json",
    "nn-NO": "nn-a1.json",
    "sv-SE": "sv-a1.json",
  };
  for (const [crowdinId, expectedFile] of Object.entries(regionalChecks)) {
    if (repoFilenameForCrowdinLocale(crowdinId, "a1", localeMap) !== expectedFile) {
      errors.push(`REGIONAL_FILENAME_MISMATCH:${crowdinId}`);
    }
  }

  try {
    repoFilenameForCrowdinLocale("zz-XX", "a1", localeMap);
    errors.push("UNKNOWN_LOCALE_SHOULD_FAIL");
  } catch (err) {
    if (!String(err.message).includes("UNKNOWN_CROWDIN_LOCALE")) {
      errors.push(`UNKNOWN_LOCALE_UNEXPECTED_ERROR:${err.message}`);
    }
  }

  return {
    pass: errors.length === 0,
    errors,
    ui,
    g2,
    repoFilenames: [...filenames].sort(),
    localeMap,
  };
}

module.exports = {
  CROWDIN_YML_PATH,
  EXPECTED_UI_FILE,
  EXPECTED_G2_A1_FILE,
  loadCrowdinYmlConfig,
  validateCrowdinYmlG2A1,
  repoFilenameForCrowdinLocale,
};
