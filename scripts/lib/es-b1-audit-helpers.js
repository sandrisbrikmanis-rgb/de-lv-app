...
const { PRODUCTION_PATH, WWW_PATH, LV_REFERENCE_PATH } = require("./es-b1-discovery-config");
...
const TEMP_DIR = path.join(ROOT, "reports/temp/es-de-b1-full-audit-luna");
const LUNA_JSON = path.join(ROOT, "reports/temp/es-de-b1-luna-raw.json");
const PROGRESS_FILE = path.join(ROOT, "scripts/.es-de-b1-luna-progress.json");
...
function buildSimpleCard(lvEntry, esEntry, index) {
  return {
    cardId: entryId(esEntry, index),
    level: "B1",
    index,
    de: esEntry.de,
...
function buildStudyCard(lvEntry, esEntry, index) {
  const fields = [];
