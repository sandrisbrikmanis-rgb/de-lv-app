...
} = require("./lrb021-luna-repair-data");
...
function applySectionAccentOverrides(sectionAccents, overrides) {
  if (!overrides || !sectionAccents) return sectionAccents;
  const out = JSON.parse(JSON.stringify(sectionAccents));
...
function scrubFrInheritedLvLeaks(study) {
  const scrub = (s) =>
    typeof s === "string" ? s.replace(/\bAtceries\b/g, "Retenez") : s;
  if (!study?.tip) return;
  if (Array.isArray(study.tip)) {
    study.tip = study.tip.map(scrub);
  } else if (typeof study.tip === "object" && study.tip !== null) {
    if (study.tip.text) study.tip.text = scrub(study.tip.text);
    if (study.tip.example) study.tip.example = scrub(study.tip.example);
  } else if (typeof study.tip === "string") {
    study.tip = scrub(study.tip);
  }
}
...
function repairCard(lang, cardKey, nested) {
  ...
  const patches =
    LUNA_PATCHES[lang]?.[resolved] ||
    LUNA_PATCHES[lang]?.[cardKey] ||
    LUNA_PATCHES[lang]?.[cardKey.charAt(0).toUpperCase() + cardKey.slice(1)];
  if (patches) out = applyFlatPatches(out, patches);
  if (lang === "fr" || lang === "hr") scrubFrInheritedLvLeaks(out.study);
  ...
  if (accentOverrides) {
    out.study.sectionAccents = applySectionAccentOverrides(
...
function buildFindingMaps() {
  const { rows } = loadCsv(
    path.join(__dirname, "../../reports/g2-a1-owner/batches-pending/LRB-021-input.csv")
  );
  const findingToCard = {};
  const findingToLang = {};
