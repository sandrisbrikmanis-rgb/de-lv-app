...
const SYSTEM_PROMPT = [
  "You are a full linguistic quality auditor for FR-DE A1 (French translations of German A1 vocabulary and study cards).",
  ...
  "Category: TRANSLATION | GRAMMAR | SEMANTICS | ORTHOGRAPHY | NATURALNESS | STUDY | COMPARISON | SECTIONACCENTS_LANGUAGE | FOREIGN_REMNANT | STRUCTURE | MULTIPLE_TRANSLATIONS_DETECTED.",
  ...
  "CRITICAL: Latvian/Dutch/English/Estonian text in French fields (e.g. lūdzu, Man ir, Es esmu, het/de/een, I am).",
  "Check French: gender/number agreement, articles (le/la/les/un/une/des), verb conjugation, accents (é è ê à ù ç œ), CEFR A1 register.",
  "Flag MULTIPLE_TRANSLATIONS_DETECTED when learner-facing field has • / / ; separating distinct meanings — OWNER_DECISION_REQUIRED.",
  "Do NOT flag fr !== study.translation unless semantically wrong or contradictory.",
...
function normalizeItem(item) {
  ...
  return {
    ...
    currentFr: item.currentFr || item.currentText || item.frText || item.existingFrText || item.currentEt || "",
    proposedFr: item.proposedFr || item.recommendedFix || item.proposedFix || item.proposedEt || "",
...
async function auditCardsBatch(options) {
  ...
  const payload = { auditType, dataset: "fr-a1", cards };

  const input = [
    "Full FR-DE A1 linguistic audit. Return JSON items array. PASS for correct cards; findings only for real issues.",
