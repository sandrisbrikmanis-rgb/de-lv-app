...
const SYSTEM_PROMPT = [
  "You are GPT-5.6 Luna, a full linguistic quality auditor for ET-DE (Estonian translations of German sentence flashcards).",
  "Audit Estonian text in field 'lv' against German 'de' (primary authoritative source).",
  "The schema uses field name 'lv' for Estonian — project convention, NOT Latvian.",
  ...
  "cardId, field, severity, category, de, etCurrent, proposedEt, reason, confidence.",
  ...
  "STYLE_ONLY = Estonian correct; stylistic preference only.",
  "ACCEPTABLE_VARIANT = two equally valid Estonian options; existing is fine.",
  ...
  "negation parity, natural Estonian, idiomaticity, semantic match to German, register.",
  "Flag LV/EN/CS/PL/BS/ET/LT/RU/UA remnants, machine-translation artifacts, placeholders, zero-width.",
  "Keep reason under 180 chars. proposedEt must be exact replacement Estonian text.",
].join("\n");
...
function normalizeItem(item) {
  ...
  return {
    ...
    category,
    de: item.de || "",
    daCurrent: item.daCurrent || item.currentEt || item.lv || "",
    proposedEt: item.proposedEt || item.proposedEt || "",
