...
const SYSTEM_PROMPT = [
  "You are a full linguistic quality auditor for ES-DE A1+A2 (Spanish translations of German A1 and A2 vocabulary and study cards).",
  "Audit Spanish text against German meaning (primary authoritative source) and Latvian source (secondary context only).",
  "The data schema uses field name 'lv' for Spanish native text — this is a project convention, NOT Latvian language.",
  "Return ONLY valid JSON: { \"items\": [ ... ] }.",
  "For each card/field: if correct, return { cardId, status: \"PASS\" }.",
  "For real issues return finding objects with:",
  "cardId, field, severity, category, de, lvSource, currentEs, proposedEs, reason, confidence.",
  ...
  "SOURCE_LV_ISSUE = LV conflicts with DE but Spanish is correct per DE.",
  "DE_SOURCE_ISSUE = possible DE source problem; do not suggest DE changes.",
  "STYLE_ONLY = Spanish correct; stylistic preference only — NOT a quality finding.",
  ...
  "CRITICAL: Latvian/Italian/English text in Spanish fields (e.g. lūdzu, Man ir, Es esmu, mājās, Vilciens atiet, latviešu).",
  "Check Spanish: gender/number agreement, verb conjugation, articles, natural collocations, accents (á é í ó ú ñ ü), CEFR A1/A2 register.",
...
function normalizeItem(item) {
  ...
  return {
    ...
    currentEs:
      ...
      item.existingEsText ||
      "",
    proposedEs: item.proposedEs || item.proposedEt || item.recommendedFix || item.proposedFix || "",
...
async function auditCardsBatch(options) {
  ...
  const payload = { auditType, dataset: "es-a1-a2", cards };

  const input = [
    "Full ES-DE A1+A2 linguistic audit. Return JSON items array. PASS for correct cards; findings only for real issues.",
