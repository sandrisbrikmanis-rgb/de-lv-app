"use strict";

module.exports = {
  SECTION_ACCENT_OVERRIDES: {
  "gr": {
    "also": {
      "examples[0].lv": {
        "purple": [
          "γι' αυτό"
        ]
      },
      "examples[1].lv": {
        "purple": [
          "γι' αυτό"
        ]
      },
      "comparison[2].example": {
        "green": [
          "deshalb"
        ],
        "purple": [
          "γι' αυτό"
        ]
      },
      "comparison[2].meaning": {
        "purple": [
          "Γι' αυτό",
          "Επομένως"
        ]
      },
      "comparison[1].example": {
        "green": [
          "auch"
        ],
        "purple": [
          "κι εγώ"
        ]
      }
    },
    "Bitte": {
      "comparison[1].example": {
        "green": [
          "bitte"
        ],
        "purple": [
          "παρακαλώ"
        ]
      }
    },
    "Eis": {
      "comparison[1].example": {
        "green": [
          "Schnee"
        ],
        "purple": [
          "χιόνι"
        ]
      },
      "comparison[2].example": {
        "green": [
          "kalt"
        ],
        "purple": [
          "κρύο"
        ]
      },
      "comparison[3].example": {
        "green": [
          "Dessert"
        ],
        "purple": [
          "επιδόρπιο"
        ]
      }
    },
    "Geschwister": {
      "comparison[1].example": {
        "green": [
          "Bruder"
        ],
        "purple": [
          "αδελφός"
        ]
      },
      "comparison[2].example": {
        "green": [
          "Schwester"
        ],
        "purple": [
          "αδελφή"
        ]
      }
    },
    "Großeltern": {
      "comparison[1].example": {
        "green": [
          "Großmutter"
        ],
        "purple": [
          "γιαγιά"
        ]
      },
      "comparison[2].example": {
        "green": [
          "Großvater"
        ],
        "purple": [
          "παππούς"
        ]
      }
    },
    "Hand": {
      "comparison[1].example": {
        "green": [
          "Arm"
        ],
        "purple": [
          "βραχίονάς"
        ]
      }
    },
    "Land": {
      "comparison[1].example": {
        "green": [
          "Stadt"
        ],
        "purple": [
          "πόλη"
        ]
      },
      "comparison[2].example": {
        "green": [
          "Dorf"
        ],
        "purple": [
          "χωριό"
        ]
      },
      "comparison[3].example": {
        "green": [
          "Erde"
        ],
        "purple": [
          "Γη"
        ]
      }
    },
    "aber": {
      "comparison[1].example": {
        "green": [
          "sondern"
        ],
        "purple": [
          "αλλά"
        ]
      },
      "comparison[2].example": {
        "green": [
          "jedoch"
        ],
        "purple": [
          "αλλά"
        ]
      }
    },
    "bitte": {
      "comparison[1].example": {
        "green": [
          "Bitte"
        ],
        "purple": [
          "παράκληση"
        ]
      }
    },
    "da": {
      "comparison[0].example": {
        "green": [
          "Da"
        ],
        "purple": [
          "Εκεί"
        ]
      }
    },
    "das": {
      "comparison[2].example": {
        "green": [
          "welches"
        ],
        "purple": [
          "οποίο"
        ]
      }
    },
    "dass": {
      "comparison[1].example": {
        "green": [
          "weil"
        ],
        "purple": [
          "επειδή"
        ]
      },
      "comparison[2].example": {
        "green": [
          "damit"
        ],
        "purple": [
          "για να"
        ]
      },
      "comparison[3].example": {
        "green": [
          "ob"
        ],
        "purple": [
          "αν"
        ]
      }
    },
    "erst": {
      "comparison[2].example": {
        "green": [
          "nur"
        ],
        "purple": [
          "μόνο"
        ]
      }
    },
    "es": {
      "comparison[0].example": {
        "green": [
          "Es"
        ]
      }
    },
    "etwas": {
      "comparison[1].example": {
        "green": [
          "was"
        ],
        "purple": [
          "κάτι"
        ]
      },
      "comparison[2].example": {
        "green": [
          "ein bisschen"
        ],
        "purple": [
          "λίγο"
        ]
      }
    },
    "euch": {
      "comparison[2].example": {
        "green": [
          "euer"
        ],
        "purple": [
          "σας"
        ]
      }
    },
    "kosten": {
      "comparison[2].example": {
        "green": [
          "zahlen"
        ],
        "purple": [
          "πληρώσω"
        ]
      }
    },
    "Laden": {
      "examples[3]": {
        "de": {
          "blue": [
            "laden"
          ]
        },
        "lv": {
          "purple": [
            "φορτίσω"
          ]
        }
      }
    }
  },
  "fr": {
    "Urlaub": {
      "comparison[1].example": {
        "green": [
          "Ferien"
        ],
        "purple": [
          "vacances"
        ]
      }
    },
    "Wetter": {
      "comparison[1].example": {
        "green": [
          "Zeit"
        ],
        "purple": [
          "temps"
        ]
      },
      "comparison[2].example": {
        "green": [
          "Regen"
        ],
        "purple": [
          "pleut"
        ]
      },
      "comparison[3].example": {
        "green": [
          "Sonne"
        ],
        "purple": [
          "soleil"
        ]
      }
    },
    "Zug": {
      "comparison[1].example": {
        "green": [
          "Bahn"
        ],
        "purple": [
          "train"
        ]
      },
      "comparison[2].example": {
        "green": [
          "Bus"
        ],
        "purple": [
          "bus"
        ]
      },
      "comparison[3].example": {
        "green": [
          "Straßenbahn"
        ],
        "purple": [
          "tramway"
        ]
      }
    },
    "unter": {
      "comparison[1].example": {
        "green": [
          "über"
        ],
        "purple": [
          "au-dessus"
        ]
      }
    },
    "vom": {
      "comparison[3].example": {
        "green": [
          "ab"
        ],
        "purple": [
          "à partir"
        ]
      }
    },
    "wenn": {
      "comparison[1].example": {
        "green": [
          "ob"
        ],
        "purple": [
          "si"
        ]
      },
      "comparison[3].example": {
        "green": [
          "weil"
        ],
        "purple": [
          "parce que"
        ]
      }
    },
    "zu": {
      "comparison[1].example": {
        "green": [
          "nach"
        ],
        "purple": [
          "à Berlin"
        ]
      },
      "comparison[2].example": {
        "green": [
          "in"
        ],
        "purple": [
          "à l’école"
        ]
      },
      "comparison[3].example": {
        "green": [
          "bei"
        ],
        "purple": [
          "chez"
        ]
      }
    },
    "zum": {
      "comparison[1].example": {
        "green": [
          "zur"
        ],
        "purple": [
          "à l’école"
        ]
      },
      "comparison[2].example": {
        "green": [
          "zu"
        ],
        "purple": [
          "chez"
        ]
      }
    }
  }
},
  LUNA_PATCHES: {
  "fr": {
    "unter": {
      "lv": "Sous • Parmi",
      "study.translation": "Sous • Parmi",
      "study.examples[1].lv": "Le chat est couché sous la chaise.",
      "study.comparison": [
        {
          "word": "unter",
          "meaning": "Sous • Parmi",
          "example": "Die Tasche ist unter dem Tisch. – Le sac est sous la table."
        },
        {
          "word": "über",
          "meaning": "Au-dessus de",
          "example": "Die Lampe hängt über dem Tisch. – La lampe est suspendue au-dessus de la table."
        },
        {
          "word": "zwischen",
          "meaning": "Entre deux éléments",
          "example": "Zwischen den Häusern. – Entre les maisons."
        },
        {
          "word": "auf",
          "meaning": "Sur une surface",
          "example": "Auf dem Tisch. – Sur la table."
        }
      ],
      "study.tip": {
        "text": "Retenez : sous la table → unter dem Tisch."
      }
    },
    "Urlaub": {
      "lv": "Vacances • Congé",
      "study.translation": "Vacances • Congé",
      "study.examples": [
        {
          "de": "Mein Vater ist im Urlaub.",
          "lv": "Mon père est en vacances."
        },
        {
          "de": "Nächste Woche habe ich Urlaub.",
          "lv": "Je suis en congé la semaine prochaine."
        },
        {
          "de": "Wir machen Urlaub in Spanien.",
          "lv": "Nous passons nos vacances en Espagne."
        },
        {
          "de": "im Urlaub",
          "lv": "en vacances"
        }
      ],
      "study.explanation": [
        "Idée principale : der Urlaub désigne les vacances ou le congé d’une personne, notamment par rapport au travail.",
        "On dit im Urlaub sein pour être en vacances et Urlaub machen pour prendre ou passer des vacances.",
        "Die Ferien désigne surtout les vacances scolaires ou universitaires et s’emploie au pluriel."
      ],
      "study.comparison": [
        {
          "word": "der Urlaub",
          "meaning": "Vacances ou congé d’une personne",
          "example": "Mein Vater ist im Urlaub. – Mon père est en vacances."
        },
        {
          "word": "die Ferien",
          "meaning": "Vacances scolaires ou universitaires",
          "example": "Die Kinder haben Ferien. – Les enfants sont en vacances."
        }
      ],
      "study.important": [
        "Urlaub : im Urlaub sein / Urlaub machen.",
        "Ferien s’emploie surtout pour les vacances scolaires ou universitaires."
      ],
      "study.tip": [
        "Urlaub s'emploie généralement au singulier pour les vacances ou le congé d'une personne.",
        "Pour les vacances scolaires ou universitaires, utilisez die Ferien."
      ]
    },
    "verstehen": {
      "study.comparison": [
        {
          "word": "verstehen",
          "meaning": "Comprendre",
          "example": "Ich verstehe dich. – Je te comprends."
        },
        {
          "word": "können",
          "meaning": "Pouvoir • Savoir faire",
          "example": "Ich kann schwimmen. – Je sais nager."
        },
        {
          "word": "wissen",
          "meaning": "Savoir un fait",
          "example": "Ich weiß das. – Je le sais."
        },
        {
          "word": "kennen",
          "meaning": "Connaître",
          "example": "Ich kenne ihn. – Je le connais."
        }
      ],
      "study.important": [
        "verstehen = comprendre ; wissen = savoir ; kennen = connaître ; können = pouvoir ou savoir faire."
      ],
      "lv": "Comprendre",
      "study.translation": "Comprendre",
      "study.explanation": [
        "Idée principale : verstehen signifie comprendre.",
        "Il s'emploie pour une langue, une personne, un texte ou une situation.",
        "Ne le confondez pas avec können, wissen ou kennen."
      ],
      "study.examples": [
        {
          "de": "Ich verstehe dich.",
          "lv": "Je te comprends."
        },
        {
          "de": "Verstehst du Deutsch?",
          "lv": "Comprends-tu l'allemand ?"
        },
        {
          "de": "Ich verstehe das nicht.",
          "lv": "Je ne comprends pas cela."
        },
        {
          "de": "Ich kann Deutsch sprechen.",
          "lv": "Je sais parler allemand."
        }
      ],
      "study.tip": {
        "text": "Comprendre un texte ou une personne → verstehen ; savoir faire quelque chose → können."
      }
    },
    "vom": {
      "lv": "Du • De la • De chez",
      "study.translation": "Du • De la • De chez",
      "study.explanation": [
        "vom est la contraction de von + dem.",
        "Il s’emploie avec un nom masculin ou neutre au datif.",
        "Selon le contexte, vom se traduit par du, de la ou de chez."
      ],
      "study.tip": {
        "text": "Retenez : von + dem = vom."
      },
      "study.important": [
        "vom est toujours la contraction de von + dem ; ne l’employez pas avec un nom féminin ou pluriel."
      ],
      "study.examples": [
        {
          "de": "Ich komme vom Bahnhof.",
          "lv": "Je viens de la gare."
        },
        {
          "de": "Das Geschenk ist vom Vater.",
          "lv": "Le cadeau vient du père."
        },
        {
          "de": "Er kommt vom Arzt.",
          "lv": "Il revient de chez le médecin."
        },
        {
          "de": "Sie fährt vom Flughafen.",
          "lv": "Elle part de l'aéroport en voiture."
        },
        {
          "de": "Das ist vom Markt.",
          "lv": "Cela vient du marché."
        },
        {
          "de": "Wir kommen vom Fest.",
          "lv": "Nous revenons de la fête."
        },
        {
          "de": "Er holt Milch vom Bauern.",
          "lv": "Il va chercher du lait chez le fermier."
        },
        {
          "de": "Die Nachricht ist vom Chef.",
          "lv": "Le message vient du patron."
        }
      ],
      "study.comparison": [
        {
          "word": "vom",
          "meaning": "von + dem",
          "example": "vom Bahnhof – de la gare"
        },
        {
          "word": "von",
          "meaning": "De • Provenance",
          "example": "von mir – de moi"
        },
        {
          "word": "aus",
          "meaning": "De l'intérieur • Origine",
          "example": "aus Deutschland – d'Allemagne"
        },
        {
          "word": "ab",
          "meaning": "À partir de",
          "example": "ab Montag – à partir de lundi"
        },
        {
          "word": "zu",
          "meaning": "Vers • Chez",
          "example": "zum Arzt – chez le médecin"
        }
      ]
    },
    "vor": {
      "study.explanation": [
        "vor signifie avant dans le temps et devant dans l’espace.",
        "Pour l’heure, vor indique les minutes avant l’heure suivante : fünf vor acht.",
        "Selon le sens, vor régit l’accusatif ou le datif."
      ],
      "study.comparison": [
        {
          "word": "vor",
          "meaning": "Avant • Devant",
          "example": "Vor dem Essen… – Avant le repas…"
        },
        {
          "word": "nach",
          "meaning": "Après",
          "example": "Nach dem Essen… – Après le repas…"
        },
        {
          "word": "neben",
          "meaning": "À côté de",
          "example": "Neben dem Haus. – À côté de la maison."
        },
        {
          "word": "hinter",
          "meaning": "Derrière",
          "example": "Hinter dem Haus. – Derrière la maison."
        }
      ],
      "study.tip": {
        "text": "Temps : avant → vor ; espace : devant → vor."
      }
    },
    "was": {
      "lv": "Quoi • Que",
      "study.translation": "Quoi • Que",
      "study.explanation": [
        "Idée principale : was interroge sur une chose, un fait ou un événement.",
        "En français, il se traduit selon la construction par quoi, que, qu’est-ce que ou quel.",
        "La forme allemande was ne change pas selon sa fonction dans la phrase."
      ],
      "study.examples": [
        {
          "de": "Was ist das?",
          "lv": "Qu'est-ce que c'est ?"
        },
        {
          "de": "Was ist passiert?",
          "lv": "Que s'est-il passé ?"
        },
        {
          "de": "Was machst du gerade?",
          "lv": "Que fais-tu en ce moment ?"
        },
        {
          "de": "Was möchtest du trinken?",
          "lv": "Que veux-tu boire ?"
        },
        {
          "de": "Was bedeutet dieses Wort?",
          "lv": "Que signifie ce mot ?"
        },
        {
          "de": "Was ist dein Lieblingsessen?",
          "lv": "Quel est ton plat préféré ?"
        },
        {
          "de": "Was hast du gesagt?",
          "lv": "Qu'as-tu dit ?"
        }
      ],
      "study.tip": {
        "text": "Choisissez en français quoi, que, qu’est-ce que ou quel selon la construction ; l’allemand garde was."
      },
      "study.important": [
        "was concerne les choses ou les faits ; wer concerne les personnes."
      ]
    },
    "wenn": {
      "study.examples[0].lv": "Si tu as le temps, passe.",
      "study.examples[3].lv": "Je ne sais pas s’il vient.",
      "study.explanation": [
        "wenn signifie si pour une condition et quand pour une situation répétée ou future.",
        "Dans la proposition introduite par wenn, le verbe conjugué se place à la fin.",
        "Pour une question indirecte oui/non, utilisez ob ; pour demander à quel moment, utilisez wann."
      ],
      "study.comparison": [
        {
          "word": "wenn",
          "meaning": "Si • Quand",
          "example": "Wenn du Zeit hast… – Si tu as le temps…"
        },
        {
          "word": "ob",
          "meaning": "Si, dans une question indirecte",
          "example": "Ich weiß nicht, ob… – Je ne sais pas si…"
        },
        {
          "word": "wann",
          "meaning": "Quand ? • À quel moment ?",
          "example": "Wann kommst du? – Quand viens-tu ?"
        },
        {
          "word": "weil",
          "meaning": "Parce que",
          "example": "Ich bleibe, weil ich krank bin. – Je reste parce que je suis malade."
        }
      ],
      "study.important": [
        "wenn = condition ou moment ; ob = question indirecte ; wann = question sur le moment."
      ],
      "study.tip": {
        "text": "Condition ou situation répétée → wenn ; question « quand ? » → wann."
      }
    },
    "wer": {
      "study.explanation": [
        "Idée principale : wer demande l’identité d’une personne et correspond à qui.",
        "wer est le nominatif et joue le rôle de sujet.",
        "Pour les autres cas, on rencontre notamment wen à l’accusatif et wem au datif."
      ],
      "study.tip": {
        "text": "Personne comme sujet → wer = qui."
      },
      "study.important": [
        "wer concerne une personne ; was concerne une chose, un fait ou un événement."
      ],
      "lv": "Qui",
      "study.translation": "Qui",
      "study.examples": [
        {
          "de": "Wer ist das?",
          "lv": "Qui est-ce ?"
        },
        {
          "de": "Wer bist du?",
          "lv": "Qui es-tu ?"
        },
        {
          "de": "Wer kommt heute?",
          "lv": "Qui vient aujourd'hui ?"
        },
        {
          "de": "Wer ist deine Lehrerin?",
          "lv": "Qui est ton enseignante ?"
        },
        {
          "de": "Wer von euch spricht Deutsch?",
          "lv": "Lequel d'entre vous parle allemand ?"
        },
        {
          "de": "Wer hat das gesagt?",
          "lv": "Qui a dit cela ?"
        },
        {
          "de": "Wer möchte Kaffee?",
          "lv": "Qui veut du café ?"
        }
      ]
    },
    "werden": {
      "study.explanation": [
        "Idée principale : werden signifie devenir ou indique un changement d’état.",
        "Au niveau A1, retenez surtout les formes ich werde, du wirst, er/sie/es wird.",
        "werden sert aussi d’auxiliaire pour le futur et le passif, mais ce ne sont pas les sens principaux de cette carte."
      ],
      "study.comparison": [
        {
          "word": "werden",
          "meaning": "Devenir • Changement",
          "example": "Ich werde müde. – Je commence à être fatigué."
        },
        {
          "word": "sein",
          "meaning": "Être • État",
          "example": "Ich bin müde. – Je suis fatigué."
        },
        {
          "word": "bleiben",
          "meaning": "Rester",
          "example": "Ich bleibe hier. – Je reste ici."
        },
        {
          "word": "machen",
          "meaning": "Faire",
          "example": "Ich mache das. – Je fais cela."
        }
      ],
      "study.important": [
        "Ich werde müde décrit un changement ; Ich bin müde décrit un état."
      ],
      "study.examples": [
        {
          "de": "Ich werde müde.",
          "lv": "Je commence à être fatigué."
        },
        {
          "de": "Es wird kalt.",
          "lv": "Il commence à faire froid."
        },
        {
          "de": "Sie wird Ärztin.",
          "lv": "Elle devient médecin."
        },
        {
          "de": "Ich bin müde.",
          "lv": "Je suis fatigué."
        }
      ],
      "study.tip": {
        "text": "Retenez : changement d'état → werden."
      }
    },
    "Wetter": {
      "study.explanation[1]": "En allemand, das Wetter désigne la météo ; die Zeit désigne le temps disponible ou un moment.",
      "study.comparison": [
        {
          "word": "Wetter",
          "meaning": "Météo",
          "example": "Das Wetter ist schön. – Il fait beau."
        },
        {
          "word": "Zeit",
          "meaning": "Temps disponible • Moment",
          "example": "Ich habe keine Zeit. – Je n’ai pas le temps."
        },
        {
          "word": "Regen",
          "meaning": "Pluie",
          "example": "Es gibt viel Regen. – Il pleut beaucoup."
        },
        {
          "word": "Sonne",
          "meaning": "Soleil",
          "example": "Die Sonne scheint. – Le soleil brille."
        }
      ],
      "lv": "Météo",
      "study.translation": "Météo",
      "study.examples": [
        {
          "de": "Wie ist das Wetter heute?",
          "lv": "Quel temps fait-il aujourd'hui ?"
        },
        {
          "de": "Das Wetter ist schön.",
          "lv": "Il fait beau."
        },
        {
          "de": "Das Wetter ist schlecht.",
          "lv": "Il fait mauvais."
        },
        {
          "de": "Im Winter ist das Wetter oft kalt.",
          "lv": "En hiver, il fait souvent froid."
        },
        {
          "de": "Wir sprechen über das Wetter.",
          "lv": "Nous parlons de la météo."
        },
        {
          "de": "Morgen wird das Wetter besser.",
          "lv": "Demain, le temps s'améliorera."
        }
      ],
      "study.tip": [
        "Pour le soleil, la pluie ou la température extérieure, utilisez das Wetter.",
        "Wie ist das Wetter? = Quel temps fait-il ?"
      ],
      "study.explanation[4]": "À ne pas confondre avec die Zeit : le temps disponible ou un moment, comme dans Ich habe keine Zeit."
    },
    "wie": {
      "lv": "Comment • Combien",
      "study.translation": "Comment • Combien",
      "study.explanation": [
        "wie demande la manière, l’état ou la façon : Wie geht es dir?",
        "wie viel demande une quantité, wie alt un âge et wie lange une durée.",
        "Dans so … wie, wie sert à former une comparaison d’égalité."
      ],
      "study.examples": [
        {
          "de": "Wie geht es dir?",
          "lv": "Comment vas-tu ?"
        },
        {
          "de": "Wie heißt du?",
          "lv": "Comment t'appelles-tu ?"
        },
        {
          "de": "Wie viel kostet das?",
          "lv": "Combien cela coûte-t-il ?"
        },
        {
          "de": "Wie alt bist du?",
          "lv": "Quel âge as-tu ?"
        },
        {
          "de": "Wie lange dauert der Film?",
          "lv": "Combien de temps dure le film ?"
        },
        {
          "de": "Er ist so groß wie sein Vater.",
          "lv": "Il est aussi grand que son père."
        }
      ],
      "study.tip": {
        "text": "Manière ou état → wie ; quantité → wie viel ; durée → wie lange."
      },
      "study.important": [
        "Ne traduisez pas wie toujours par un seul mot français : la formulation dépend de la question."
      ]
    },
    "wissen": {
      "study.comparison": [
        {
          "word": "wissen",
          "meaning": "Savoir un fait",
          "example": "Ich weiß das. – Je le sais."
        },
        {
          "word": "kennen",
          "meaning": "Connaître une personne, un lieu ou une chose",
          "example": "Ich kenne ihn. – Je le connais."
        }
      ],
      "study.important": [
        "wissen = savoir une information ; kennen = connaître quelqu’un ou quelque chose."
      ],
      "study.explanation": [
        "Idée principale : wissen signifie savoir un fait, une réponse ou une information.",
        "kennen signifie connaître une personne, un lieu ou une chose."
      ],
      "study.examples": [
        {
          "de": "Ich weiß, wo er wohnt.",
          "lv": "Je sais où il habite."
        },
        {
          "de": "Woher wissen Sie das?",
          "lv": "Comment le savez-vous ?"
        },
        {
          "de": "Ich weiß die Antwort.",
          "lv": "Je sais la réponse."
        }
      ]
    },
    "zu": {
      "lv": "À • Chez • Trop",
      "study.translation": "À • Chez • Trop",
      "study.explanation": [
        "zu indique souvent une destination vers une personne ou un lieu fonctionnel : zum Arzt, zur Schule.",
        "Devant un adjectif ou un adverbe, zu signifie trop : zu teuer.",
        "zu introduit aussi l’infinitif dans certaines constructions."
      ],
      "study.comparison": [
        {
          "word": "zu",
          "meaning": "À • Chez",
          "example": "Ich gehe zum Arzt. – Je vais chez le médecin."
        },
        {
          "word": "nach",
          "meaning": "Vers une ville ou un pays sans article",
          "example": "Ich fahre nach Berlin. – Je vais à Berlin."
        },
        {
          "word": "in",
          "meaning": "Dans • À l’intérieur de",
          "example": "Ich gehe in die Schule. – Je vais à l’école."
        },
        {
          "word": "bei",
          "meaning": "Chez • Auprès de, sans mouvement",
          "example": "Ich bin bei Anna. – Je suis chez Anna."
        }
      ],
      "study.important": [
        "zu teuer = trop cher ; zum = zu + dem ; zur = zu + der."
      ]
    },
    "Zug": {
      "study.comparison": [
        {
          "word": "der Zug",
          "meaning": "Train",
          "example": "Der Zug kommt. – Le train arrive."
        },
        {
          "word": "die Bahn",
          "meaning": "Chemin de fer • Transport ferroviaire",
          "example": "Ich fahre mit der Bahn. – Je voyage en train."
        },
        {
          "word": "der Bus",
          "meaning": "Bus",
          "example": "Der Bus kommt. – Le bus arrive."
        },
        {
          "word": "die Straßenbahn",
          "meaning": "Tramway",
          "example": "Die Straßenbahn ist hier. – Le tramway est ici."
        }
      ],
      "lv": "Train",
      "study.translation": "Train",
      "study.explanation": [
        "Idée principale : der Zug signifie le plus souvent train au niveau A1.",
        "Il s'emploie pour parler des trajets, des arrivées et des départs.",
        "Dans d'autres contextes, Zug peut aussi désigner un cortège, une traction, un courant d'air ou un trait du visage ; ce ne sont pas les sens principaux au niveau A1.",
        "Expressions fréquentes : mit dem Zug fahren et Der Zug kommt."
      ],
      "study.examples": [
        {
          "de": "Der Zug kommt um acht Uhr.",
          "lv": "Le train arrive à huit heures."
        },
        {
          "de": "Ich fahre mit dem Zug.",
          "lv": "Je voyage en train."
        },
        {
          "de": "Der Zug ist voll.",
          "lv": "Le train est plein."
        },
        {
          "de": "Der Bus kommt später.",
          "lv": "Le bus arrive plus tard."
        }
      ],
      "study.tip": {
        "text": "Retenez : un train déterminé → der Zug."
      }
    },
    "zum": {
      "lv": "Au • À la • Chez le",
      "study.translation": "Au • À la • Chez le",
      "study.explanation": [
        "zum est la contraction de zu + dem.",
        "Il s’emploie devant un nom masculin ou neutre au datif.",
        "Selon le contexte, il se traduit notamment par au, à la ou chez le."
      ],
      "study.comparison": [
        {
          "word": "zum",
          "meaning": "zu + dem, masculin ou neutre",
          "example": "Ich gehe zum Arzt. – Je vais chez le médecin."
        },
        {
          "word": "zur",
          "meaning": "zu + der, féminin",
          "example": "Ich gehe zur Schule. – Je vais à l’école."
        },
        {
          "word": "zu",
          "meaning": "À • Chez",
          "example": "Ich gehe zu Anna. – Je vais chez Anna."
        }
      ],
      "study.important": [
        "zum = zu + dem ; zur = zu + der."
      ],
      "study.examples": [
        {
          "de": "Ich gehe zum Arzt.",
          "lv": "Je vais chez le médecin."
        },
        {
          "de": "Wir fahren zum Bahnhof.",
          "lv": "Nous allons à la gare."
        },
        {
          "de": "Sie geht zum Supermarkt.",
          "lv": "Elle va au supermarché."
        },
        {
          "de": "Komm zum Essen!",
          "lv": "Viens manger !"
        },
        {
          "de": "Er fährt zum Flughafen.",
          "lv": "Il va à l'aéroport."
        },
        {
          "de": "Wir gehen zum Konzert.",
          "lv": "Nous allons au concert."
        },
        {
          "de": "Das Geschenk ist zum Geburtstag.",
          "lv": "Le cadeau est pour l'anniversaire."
        },
        {
          "de": "Ich gehe zum Friseur.",
          "lv": "Je vais chez le coiffeur."
        }
      ],
      "study.tip": [
        "Retenez : zu + dem → zum.",
        "Avec un nom féminin : zu + der → zur."
      ]
    }
  },
  "gr": {
    "ab": {
      "study.tip.text": "Θυμήσου: αφετηρία στον χρόνο ή στον χώρο → ab.",
      "lv": "Από",
      "study.translation": "Από",
      "study.comparison": [
        {
          "word": "ab",
          "meaning": "Από ένα χρονικό ή τοπικό σημείο",
          "example": "ab Montag – από τη Δευτέρα"
        },
        {
          "word": "von",
          "meaning": "Από κάποιον ή κάτι • Προέλευση",
          "example": "von mir – από εμένα"
        },
        {
          "word": "aus",
          "meaning": "Από μέσα προς τα έξω • Προέλευση",
          "example": "aus dem Haus – από το σπίτι"
        }
      ]
    },
    "aber": {
      "lv": "Αλλά • Ωστόσο",
      "study.translation": "Αλλά • Ωστόσο",
      "study.comparison": [
        {
          "word": "aber",
          "meaning": "Αλλά • Ωστόσο",
          "example": "Ich komme, aber später. – Θα έρθω, αλλά αργότερα."
        },
        {
          "word": "sondern",
          "meaning": "Αλλά, μετά από άρνηση",
          "example": "Ich wollte keinen Tee, sondern Kaffee. – Δεν ήθελα τσάι αλλά καφέ."
        },
        {
          "word": "jedoch",
          "meaning": "Ωστόσο",
          "example": "Es ist kalt, jedoch sonnig. – Κάνει κρύο, αλλά έχει ήλιο."
        }
      ]
    },
    "an": {
      "lv": "Σε • Πάνω σε • Δίπλα σε",
      "study.translation": "Σε • Πάνω σε • Δίπλα σε",
      "study.tip.text": "Θυμήσου: επαφή με κάθετη επιφάνεια → an der Wand.",
      "study.important": [
        "Το an δηλώνει συχνά επαφή με όριο ή κάθετη επιφάνεια· η ακριβής μετάφραση εξαρτάται από τα συμφραζόμενα."
      ],
      "study.examples": [
        {
          "de": "an der Wand",
          "lv": "Στον τοίχο"
        },
        {
          "de": "am Fenster",
          "lv": "Δίπλα στο παράθυρο"
        },
        {
          "de": "am Meer",
          "lv": "Δίπλα στη θάλασσα"
        }
      ],
      "study.comparison": [
        {
          "word": "an",
          "meaning": "Σε όριο ή κάθετη επιφάνεια",
          "example": "an der Wand – στον τοίχο"
        },
        {
          "word": "auf",
          "meaning": "Πάνω σε οριζόντια επιφάνεια",
          "example": "auf dem Tisch – πάνω στο τραπέζι"
        },
        {
          "word": "bei",
          "meaning": "Σε πρόσωπο ή μέρος",
          "example": "beim Arzt – στον γιατρό"
        }
      ]
    },
    "Bitte": {
      "lv": "Παράκληση • Αίτημα",
      "study.translation": "Παράκληση • Αίτημα",
      "study.examples": [
        {
          "de": "Ich habe eine Bitte.",
          "lv": "Έχω μια παράκληση."
        },
        {
          "de": "Er erfüllt meine Bitte.",
          "lv": "Ικανοποιεί το αίτημά μου."
        },
        {
          "de": "Sie hat zwei Bitten.",
          "lv": "Έχει δύο αιτήματα."
        }
      ],
      "study.explanation": [
        "Κύρια ιδέα: die Bitte είναι ουσιαστικό και σημαίνει παράκληση ή αίτημα.",
        "Γράφεται με κεφαλαίο αρχικό και έχει άρθρο die.",
        "Ο πληθυντικός είναι die Bitten."
      ],
      "study.important": [
        "Bitte με κεφαλαίο = παράκληση ή αίτημα· bitte με πεζό = παρακαλώ."
      ],
      "study.tip": [
        "Ουσιαστικό: die Bitte. Ευγενική λέξη: bitte."
      ],
      "study.comparison": [
        {
          "word": "die Bitte",
          "meaning": "Παράκληση • Αίτημα",
          "example": "Ich habe eine Bitte. – Έχω μια παράκληση."
        },
        {
          "word": "bitte",
          "meaning": "Παρακαλώ",
          "example": "Komm bitte herein. – Πέρασε μέσα, παρακαλώ."
        }
      ]
    },
    "bitte": {
      "lv": "Παρακαλώ",
      "study.translation": "Παρακαλώ",
      "study.examples": [
        {
          "de": "Eine Tasse Kaffee, bitte.",
          "lv": "Ένα φλιτζάνι καφέ, παρακαλώ."
        },
        {
          "de": "Komm bitte herein.",
          "lv": "Πέρασε μέσα, παρακαλώ."
        },
        {
          "de": "Bitte schön!",
          "lv": "Παρακαλώ!"
        }
      ],
      "study.explanation": [
        "Κύρια ιδέα: bitte είναι ευγενική λέξη και σημαίνει παρακαλώ.",
        "Με κεφαλαίο και άρθρο, die Bitte είναι ουσιαστικό: παράκληση ή αίτημα."
      ],
      "study.comparison": [
        {
          "word": "bitte",
          "meaning": "Παρακαλώ",
          "example": "Komm bitte herein. = Πέρασε μέσα, παρακαλώ."
        },
        {
          "word": "die Bitte",
          "meaning": "Παράκληση • Αίτημα",
          "example": "Ich habe eine Bitte. = Έχω μια παράκληση."
        }
      ],
      "study.important": [
        "bitte με πεζό = παρακαλώ· die Bitte με κεφαλαίο = παράκληση ή αίτημα."
      ],
      "study.tip": [
        "bitte με πεζό = παρακαλώ.",
        "die Bitte με κεφαλαίο = παράκληση ή αίτημα."
      ]
    },
    "bleiben": {
      "study.examples[0].lv": "Μένω στο σπίτι.",
      "study.examples[3].lv": "Πηγαίνω στο σπίτι.",
      "study.tip.text": "Θυμήσου: παραμένω → bleiben· φεύγω ή πηγαίνω → gehen/fahren.",
      "lv": "Μένω • Παραμένω",
      "study.translation": "Μένω • Παραμένω",
      "study.explanation": [
        "Κύρια ιδέα: bleiben σημαίνει μένω ή παραμένω.",
        "Χρησιμοποιείται όταν κάποιος ή κάτι δεν φεύγει και παραμένει στο ίδιο μέρος ή στην ίδια κατάσταση.",
        "Δεν σημαίνει περιμένω: warten = περιμένω."
      ],
      "study.comparison": [
        {
          "word": "bleiben",
          "meaning": "Μένω • Παραμένω",
          "example": "Ich bleibe hier. – Μένω εδώ."
        },
        {
          "word": "gehen",
          "meaning": "Πηγαίνω με τα πόδια",
          "example": "Ich gehe nach Hause. – Πηγαίνω στο σπίτι."
        },
        {
          "word": "fahren",
          "meaning": "Πηγαίνω με όχημα",
          "example": "Ich fahre nach Hause. – Πηγαίνω στο σπίτι με όχημα."
        },
        {
          "word": "warten",
          "meaning": "Περιμένω",
          "example": "Ich warte hier. – Περιμένω εδώ."
        }
      ]
    },
    "bringen": {
      "study.examples": [
        {
          "de": "Ich bringe dir ein Buch.",
          "lv": "Σου φέρνω ένα βιβλίο."
        },
        {
          "de": "Ich bringe das Paket zur Post.",
          "lv": "Μεταφέρω το δέμα στο ταχυδρομείο."
        },
        {
          "de": "Ich bringe die Kinder zur Schule.",
          "lv": "Πηγαίνω τα παιδιά στο σχολείο."
        }
      ],
      "study.comparison": [
        {
          "word": "bringen",
          "meaning": "Φέρνω • Μεταφέρω προς έναν προορισμό",
          "example": "Ich bringe das Buch zu dir. = Σου φέρνω το βιβλίο."
        },
        {
          "word": "holen",
          "meaning": "Πηγαίνω να πάρω και φέρνω",
          "example": "Ich hole das Buch. = Πηγαίνω να πάρω το βιβλίο."
        },
        {
          "word": "nehmen",
          "meaning": "Παίρνω",
          "example": "Ich nehme das Buch. = Παίρνω το βιβλίο."
        }
      ],
      "study.important": [
        "bringen = φέρνω προς έναν προορισμό· holen = πηγαίνω να πάρω και επιστρέφω με κάτι."
      ],
      "lv": "Φέρνω • Μεταφέρω",
      "study.translation": "Φέρνω • Μεταφέρω",
      "study.explanation": [
        "Κύρια ιδέα: bringen σημαίνει φέρνω ή μεταφέρω κάτι προς έναν προορισμό.",
        "nehmen σημαίνει παίρνω, ενώ holen σημαίνει πηγαίνω να πάρω κάτι και το φέρνω πίσω."
      ],
      "study.tip": {
        "text": "Προς έναν προορισμό → bringen· πηγαίνω να πάρω και επιστρέφω → holen."
      }
    },
    "da": {
      "study.explanation": [
        "Κύρια ιδέα: da δηλώνει συχνά εκεί ή παρουσιάζει κάτι που εμφανίζεται.",
        "Ανάλογα με τα συμφραζόμενα μπορεί επίσης να εισάγει αιτία με τη σημασία επειδή."
      ],
      "study.tip.text": "Θυμήσου: da = εκεί· σε ορισμένες προτάσεις = επειδή.",
      "study.examples": [
        {
          "de": "Da ist mein Auto.",
          "lv": "Εκεί είναι το αυτοκίνητό μου."
        },
        {
          "de": "Ich war da.",
          "lv": "Ήμουν εκεί."
        },
        {
          "de": "Da kommt er.",
          "lv": "Να τος, έρχεται."
        },
        {
          "de": "Komm mal da her!",
          "lv": "Έλα εδώ!"
        }
      ],
      "study.comparison": [
        {
          "word": "da",
          "meaning": "Εκεί • Εδώ, ανάλογα με τα συμφραζόμενα",
          "example": "Da ist mein Auto. – Εκεί είναι το αυτοκίνητό μου."
        },
        {
          "word": "hier",
          "meaning": "Εδώ",
          "example": "Hier ist mein Auto. – Εδώ είναι το αυτοκίνητό μου."
        },
        {
          "word": "dort",
          "meaning": "Εκεί, πιο μακριά",
          "example": "Dort ist mein Auto. – Εκεί είναι το αυτοκίνητό μου."
        },
        {
          "word": "dann",
          "meaning": "Τότε",
          "example": "Dann gehen wir nach Hause. – Τότε πηγαίνουμε σπίτι."
        }
      ]
    },
    "das": {
      "lv": "Οριστικό άρθρο ουδέτερου • Αυτό",
      "study.translation": "Οριστικό άρθρο ουδέτερου • Αυτό",
      "study.explanation": [
        "Κύρια ιδέα: das είναι το οριστικό άρθρο ουδέτερων ουσιαστικών.",
        "Μπορεί επίσης να λειτουργεί ως δεικτική αντωνυμία με τη σημασία αυτό ή ως αναφορική αντωνυμία. "
      ],
      "study.comparison[2]": {
        "word": "welches",
        "meaning": "Το οποίο",
        "example": "Das Buch, welches ich lese. = Το βιβλίο το οποίο διαβάζω."
      },
      "study.tip.text": "Θυμήσου: das + ουδέτερο ουσιαστικό· μόνο του μπορεί να σημαίνει αυτό.",
      "study.important": [
        "Στο επίπεδο Α1, μάθε πρώτα το das ως οριστικό άρθρο ουδέτερου γένους.",
        "das μπορεί να είναι άρθρο ή αντωνυμία· dass είναι σύνδεσμος και σημαίνει ότι."
      ]
    },
    "dass": {
      "study.comparison": [
        {
          "word": "dass",
          "meaning": "Ότι",
          "example": "Ich weiß, dass er kommt. – Ξέρω ότι θα έρθει."
        },
        {
          "word": "weil",
          "meaning": "Επειδή",
          "example": "Ich bleibe zu Hause, weil es regnet. – Μένω σπίτι επειδή βρέχει."
        },
        {
          "word": "damit",
          "meaning": "Για να",
          "example": "Ich lerne Deutsch, damit ich in Deutschland arbeiten kann. – Μαθαίνω γερμανικά για να μπορώ να εργαστώ στη Γερμανία."
        },
        {
          "word": "ob",
          "meaning": "Αν, σε πλάγια ερώτηση",
          "example": "Ich weiß nicht, ob er kommt. – Δεν ξέρω αν θα έρθει."
        }
      ],
      "study.important": [
        "dass = ότι και εισάγει δευτερεύουσα πρόταση.",
        "das είναι άρθρο ή αντωνυμία· dass είναι σύνδεσμος."
      ]
    },
    "ein": {
      "lv": "Αόριστο άρθρο • Ένας • Ένα",
      "study.translation": "Αόριστο άρθρο • Ένας • Ένα",
      "study.examples": [
        {
          "de": "Ein Mann wartet draußen.",
          "lv": "Ένας άντρας περιμένει έξω."
        },
        {
          "de": "Ich habe ein Buch.",
          "lv": "Έχω ένα βιβλίο."
        },
        {
          "de": "Er sucht einen Stift.",
          "lv": "Ψάχνει ένα στυλό."
        },
        {
          "de": "Ein Kind spielt.",
          "lv": "Ένα παιδί παίζει."
        }
      ],
      "study.tip": {
        "text": "Θυμήσου: αόριστο άρθρο → ein."
      }
    },
    "Eis": {
      "study.important": [
        "Στα γερμανικά das Eis μπορεί να σημαίνει πάγος ή παγωτό· στα ελληνικά οι δύο έννοιες αποδίδονται με διαφορετικές λέξεις."
      ],
      "lv": "Πάγος • Παγωτό",
      "study.translation": "Πάγος • Παγωτό",
      "study.explanation": [
        "Κύρια ιδέα: das Eis μπορεί να σημαίνει πάγος ή παγωτό.",
        "Για παγωμένο νερό χρησιμοποιούμε τη λέξη πάγος.",
        "Για το επιδόρπιο χρησιμοποιούμε τη λέξη παγωτό.",
        "Τα συμφραζόμενα δείχνουν ποια σημασία εννοείται."
      ],
      "study.examples": [
        {
          "de": "Ich esse ein Eis.",
          "lv": "Τρώω ένα παγωτό."
        },
        {
          "de": "Möchtest du ein Eis?",
          "lv": "Θέλεις ένα παγωτό;"
        },
        {
          "de": "Im Winter liegt Eis auf dem See.",
          "lv": "Τον χειμώνα υπάρχει πάγος στη λίμνη."
        },
        {
          "de": "Das Eis ist kalt.",
          "lv": "Ο πάγος είναι κρύος."
        },
        {
          "de": "Ich nehme ein Eis mit Schokolade.",
          "lv": "Θα πάρω ένα παγωτό σοκολάτα."
        }
      ],
      "study.comparison": [
        {
          "word": "das Eis",
          "meaning": "Πάγος • Παγωτό",
          "example": "Ich esse ein Eis. – Τρώω ένα παγωτό."
        },
        {
          "word": "der Schnee",
          "meaning": "Χιόνι",
          "example": "Der Schnee ist weiß. – Το χιόνι είναι λευκό."
        },
        {
          "word": "kalt",
          "meaning": "Κρύος",
          "example": "Das Wasser ist kalt. – Το νερό είναι κρύο."
        },
        {
          "word": "das Dessert",
          "meaning": "Επιδόρπιο",
          "example": "Eis ist ein Dessert. – Το παγωτό είναι επιδόρπιο."
        }
      ],
      "study.examples[4].lv": "Θα πάρω ένα παγωτό με σοκολάτα.",
      "study.tip": {
        "text": "Θυμήσου: φαγητό → παγωτό· χειμώνας ή παγωμένο νερό → πάγος."
      }
    },
    "erst": {
      "study.tip.text": "Θυμήσου: χρόνος ή σειρά → erst· περιορισμένη ποσότητα → nur.",
      "lv": "Μόλις • Πρώτα",
      "study.translation": "Μόλις • Πρώτα",
      "study.examples": [
        {
          "de": "Erst lernen, dann spielen.",
          "lv": "Πρώτα διάβασμα, μετά παιχνίδι."
        },
        {
          "de": "Ich komme erst morgen.",
          "lv": "Δεν θα έρθω παρά μόνο αύριο."
        },
        {
          "de": "Er ist erst 18 Jahre alt.",
          "lv": "Είναι μόλις 18 ετών."
        },
        {
          "de": "Wir essen erst um acht Uhr.",
          "lv": "Θα φάμε μόλις στις οκτώ."
        }
      ],
      "study.comparison": [
        {
          "word": "erst",
          "meaning": "Μόλις • Πρώτα",
          "example": "Erst arbeiten, dann Pause. – Πρώτα δουλειά, μετά διάλειμμα."
        },
        {
          "word": "zuerst",
          "meaning": "Πρώτα • Στην αρχή",
          "example": "Zuerst frühstücken wir. – Πρώτα τρώμε πρωινό."
        },
        {
          "word": "nur",
          "meaning": "Μόνο, ως περιορισμός",
          "example": "Ich habe nur 5 Euro. – Έχω μόνο 5 ευρώ."
        },
        {
          "word": "dann",
          "meaning": "Μετά • Τότε",
          "example": "Dann gehen wir nach Hause. – Μετά πηγαίνουμε σπίτι."
        }
      ],
      "study.important": [
        "Τα erst και nur μπορούν και τα δύο να αποδοθούν ως «μόνο», αλλά δεν χρησιμοποιούνται με τον ίδιο τρόπο.",
        "Το erst αναφέρεται συχνά σε χρόνο, σειρά ή σημείο που μόλις επιτεύχθηκε· το nur περιορίζει μια ποσότητα.",
        "Το zuerst σημαίνει συνήθως «πρώτα»."
      ]
    },
    "es": {
      "study.examples": [
        {
          "de": "Es regnet.",
          "lv": "Βρέχει."
        },
        {
          "de": "Es ist kalt.",
          "lv": "Κάνει κρύο."
        },
        {
          "de": "Das Kind schläft.",
          "lv": "Το παιδί κοιμάται."
        },
        {
          "de": "Es ist müde.",
          "lv": "Είναι κουρασμένο."
        }
      ],
      "study.explanation": [
        "Κύρια ιδέα: es είναι η ουδέτερη αντωνυμία αυτό, αλλά χρησιμοποιείται και σε απρόσωπες εκφράσεις.",
        "Σε προτάσεις όπως Es regnet, στα ελληνικά συνήθως δεν μεταφράζεται ως ξεχωριστή αντωνυμία."
      ],
      "study.important": [
        "es ≠ ich: es = αυτό ή απρόσωπο υποκείμενο· ich = εγώ."
      ],
      "study.tip.text": "Θυμήσου: es = αυτό ή απρόσωπο υποκείμενο, όχι εγώ.",
      "lv": "Αυτό • Απρόσωπο υποκείμενο",
      "study.translation": "Αυτό • Απρόσωπο υποκείμενο",
      "study.comparison": [
        {
          "word": "es",
          "meaning": "Αυτό • Ουδέτερη αντωνυμία ή απρόσωπο υποκείμενο",
          "example": "Es regnet. – Βρέχει."
        },
        {
          "word": "ich",
          "meaning": "Εγώ",
          "example": "Ich lerne Deutsch. – Μαθαίνω γερμανικά."
        }
      ]
    },
    "etwas": {
      "study.explanation[1]": "Όταν το etwas αντικαθιστά ένα άγνωστο ή απροσδιόριστο πράγμα, αποδίδεται συνήθως ως κάτι.",
      "study.tip.text": "Θυμήσου: απροσδιόριστο πράγμα → etwas· μικρή ποσότητα → ein bisschen.",
      "study.examples": [
        {
          "de": "Ich möchte etwas trinken.",
          "lv": "Θα ήθελα να πιω κάτι."
        },
        {
          "de": "Hast du etwas Zeit?",
          "lv": "Έχεις λίγο χρόνο;"
        },
        {
          "de": "Ich bin etwas müde.",
          "lv": "Είμαι λίγο κουρασμένος."
        },
        {
          "de": "Ich habe etwas für dich.",
          "lv": "Έχω κάτι για σένα."
        },
        {
          "de": "Das ist etwas teuer.",
          "lv": "Αυτό είναι κάπως ακριβό."
        }
      ],
      "study.important": [
        "etwas = κάτι ή λίγο· nichts = τίποτα.",
        "etwas trinken = πίνω κάτι· nichts trinken = δεν πίνω τίποτα."
      ],
      "study.comparison[0].meaning": "Κάτι • Λίγο"
    },
    "euch": {
      "lv": "Σας • Σε εσάς",
      "study.translation": "Σας • Σε εσάς",
      "study.examples": [
        {
          "de": "Ich sehe euch.",
          "lv": "Σας βλέπω."
        },
        {
          "de": "Ich helfe euch.",
          "lv": "Σας βοηθώ."
        },
        {
          "de": "Ich gebe euch das Buch.",
          "lv": "Σας δίνω το βιβλίο."
        },
        {
          "de": "Ich danke euch.",
          "lv": "Σας ευχαριστώ."
        },
        {
          "de": "Ihr erinnert euch.",
          "lv": "Θυμάστε."
        }
      ],
      "study.explanation": [
        "Κύρια ιδέα: euch είναι αντωνυμία β΄ προσώπου πληθυντικού.",
        "Χρησιμοποιείται ως άμεσο ή έμμεσο αντικείμενο και αποδίδεται συνήθως ως σας ή σε εσάς."
      ],
      "study.comparison": [
        {
          "word": "ihr",
          "meaning": "Εσείς",
          "example": "Ihr seid freundlich. – Είστε φιλικοί."
        },
        {
          "word": "euch",
          "meaning": "Σας • Σε εσάς",
          "example": "Ich helfe euch. – Σας βοηθώ."
        },
        {
          "word": "euer",
          "meaning": "Σας, κτητικό",
          "example": "Das ist euer Haus. – Αυτό είναι το σπίτι σας."
        }
      ],
      "study.tip": {
        "text": "Θυμήσου: ihr = εσείς· euch = σας / σε εσάς· euer = σας, κτητικό."
      },
      "study.important": [
        "euch είναι αιτιατική ή δοτική του ihr και αποδίδεται ως σας ή σε εσάς.",
        "ihr = εσείς· euch = σας / σε εσάς· euer = σας, κτητικό."
      ]
    },
    "Frau": {
      "lv": "Γυναίκα • Σύζυγος",
      "study.translation": "Γυναίκα • Σύζυγος",
      "study.explanation": [
        "Κύρια ιδέα: die Frau σημαίνει γυναίκα και, με κτητικό όπως meine Frau, μπορεί να σημαίνει σύζυγος.",
        "Ο πληθυντικός είναι die Frauen."
      ],
      "study.important": [
        "Frau = γυναίκα· meine Frau = η σύζυγός μου."
      ],
      "study.examples": [
        {
          "de": "Sie ist eine nette Frau.",
          "lv": "Είναι μια ευγενική γυναίκα."
        },
        {
          "de": "Das ist meine Frau.",
          "lv": "Αυτή είναι η σύζυγός μου."
        },
        {
          "de": "Wie viele Frauen sind hier?",
          "lv": "Πόσες γυναίκες είναι εδώ;"
        },
        {
          "de": "Meine Frau arbeitet in Berlin.",
          "lv": "Η σύζυγός μου εργάζεται στο Βερολίνο."
        },
        {
          "de": "Die Frau trägt ein Kleid.",
          "lv": "Η γυναίκα φοράει φόρεμα."
        },
        {
          "de": "Seine Frau ist Ärztin.",
          "lv": "Η σύζυγός του είναι γιατρός."
        }
      ],
      "study.tip": [
        "Με κτητικό, όπως meine Frau, η λέξη συνήθως σημαίνει σύζυγος.",
        "Χωρίς κτητικό, die Frau ή eine Frau συνήθως σημαίνει γυναίκα."
      ],
      "study.examples[0].lv": "Είναι μια συμπαθητική γυναίκα."
    },
    "Geschwister": {
      "study.comparison[0].example": "Meine Geschwister – τα αδέλφια μου",
      "lv": "Αδέλφια",
      "study.translation": "Αδέλφια",
      "study.examples": [
        {
          "de": "Ich habe zwei Geschwister.",
          "lv": "Έχω δύο αδέλφια."
        },
        {
          "de": "Meine Geschwister wohnen in Berlin.",
          "lv": "Τα αδέλφια μου μένουν στο Βερολίνο."
        }
      ]
    },
    "gleich": {
      "study.important": [
        "gleich μπορεί να δηλώνει χρόνο ή ισότητα· η σωστή απόδοση εξαρτάται από τα συμφραζόμενα."
      ],
      "study.explanation": [
        "Κύρια ιδέα: gleich σημαίνει αμέσως ή σε λίγο όταν αναφέρεται στον χρόνο και ίδιος όταν γίνεται σύγκριση.",
        "Η σωστή σημασία εξαρτάται από τα συμφραζόμενα."
      ],
      "study.examples": [
        {
          "de": "Ich komme gleich.",
          "lv": "Έρχομαι αμέσως."
        },
        {
          "de": "Wir haben die gleiche Farbe.",
          "lv": "Έχουμε το ίδιο χρώμα."
        },
        {
          "de": "Das Essen ist gleich fertig.",
          "lv": "Το φαγητό θα είναι έτοιμο σε λίγο."
        },
        {
          "de": "Beide Wege sind gleich lang.",
          "lv": "Και οι δύο διαδρομές έχουν το ίδιο μήκος."
        },
        {
          "de": "Bis gleich!",
          "lv": "Τα λέμε σε λίγο!"
        },
        {
          "de": "Sie sind gleich groß.",
          "lv": "Έχουν το ίδιο ύψος."
        }
      ],
      "study.tip": [
        "Χρόνος: gleich = αμέσως ή σε λίγο.",
        "Σύγκριση: gleich = ίδιος / ίση / ίδιο."
      ]
    },
    "groß": {
      "study.explanation": [
        "Κύρια ιδέα: groß σημαίνει μεγάλος ως προς το μέγεθος ή ψηλός για άνθρωπο.",
        "Η ακριβής ελληνική λέξη εξαρτάται από το ουσιαστικό: μεγάλο σπίτι, ψηλός άνθρωπος."
      ],
      "study.important": [
        "Για ανθρώπους groß αποδίδεται συχνά ως ψηλός· για αντικείμενα ως μεγάλος."
      ],
      "study.examples": [
        {
          "de": "Das Haus ist groß.",
          "lv": "Το σπίτι είναι μεγάλο."
        },
        {
          "de": "Berlin ist eine große Stadt.",
          "lv": "Το Βερολίνο είναι μεγάλη πόλη."
        },
        {
          "de": "Er ist groß.",
          "lv": "Είναι ψηλός."
        },
        {
          "de": "Das Zimmer ist groß.",
          "lv": "Το δωμάτιο είναι μεγάλο."
        }
      ],
      "study.tip": [
        "Για αντικείμενα ή μέρη: groß = μεγάλος.",
        "Για ανθρώπους: groß = ψηλός."
      ],
      "lv": "Μεγάλος • Ψηλός",
      "study.translation": "Μεγάλος • Ψηλός"
    },
    "Großeltern": {
      "study.comparison[0].example": "meine Großeltern – οι παππούδες και οι γιαγιάδες μου",
      "study.comparison[1].example": "meine Großmutter – η γιαγιά μου",
      "study.comparison[2].example": "mein Großvater – ο παππούς μου",
      "study.explanation[0]": "Κύρια ιδέα: Großeltern σημαίνει παππούδες και γιαγιάδες ως σύνολο.",
      "lv": "Παππούδες και γιαγιάδες",
      "study.translation": "Παππούδες και γιαγιάδες",
      "study.examples": [
        {
          "de": "Meine Großeltern wohnen auf dem Land.",
          "lv": "Οι παππούδες και οι γιαγιάδες μου μένουν στην ύπαιθρο."
        },
        {
          "de": "Ich besuche meine Großeltern.",
          "lv": "Επισκέπτομαι τους παππούδες και τις γιαγιάδες μου."
        }
      ]
    },
    "gut": {
      "lv": "Καλός • Καλά",
      "study.translation": "Καλός • Καλά",
      "study.explanation": [
        "Κύρια ιδέα: gut σημαίνει καλός ως επίθετο και καλά ως επίρρημα.",
        "Δεν πρέπει να συγχέεται με το ουσιαστικό das Gut, που έχει διαφορετική σημασία."
      ],
      "study.important": [
        "gut = καλός • καλά· das Gut είναι διαφορετικό ουσιαστικό."
      ],
      "study.tip": [
        "gut χωρίς άρθρο = καλός ή καλά.",
        "das Gut με κεφαλαίο και άρθρο είναι διαφορετικό ουσιαστικό."
      ],
      "study.examples[1].lv": "Πώς είσαι; – Καλά, ευχαριστώ!"
    },
    "haben": {
      "study.explanation": [
        "Κύρια ιδέα: haben σημαίνει έχω και δηλώνει κατοχή ή σχέση.",
        "Το αντικείμενο του haben βρίσκεται συνήθως σε αιτιατική.",
        "Μην το συγχέεις με sein = είμαι ή bekommen = λαμβάνω."
      ],
      "study.important": [
        "Ich habe + Akkusativ = έχω κάτι· όχι *mir habe."
      ],
      "study.examples": [
        {
          "de": "Ich habe ein Auto.",
          "lv": "Έχω ένα αυτοκίνητο."
        },
        {
          "de": "Hast du Zeit?",
          "lv": "Έχεις χρόνο;"
        },
        {
          "de": "Wir haben Hunger.",
          "lv": "Πεινάμε."
        },
        {
          "de": "Ich habe das gemacht.",
          "lv": "Το έκανα."
        }
      ],
      "study.comparison": [
        {
          "word": "haben",
          "meaning": "Έχω",
          "example": "Ich habe Zeit. – Έχω χρόνο."
        },
        {
          "word": "sein",
          "meaning": "Είμαι",
          "example": "Ich bin hier. – Είμαι εδώ."
        },
        {
          "word": "bekommen",
          "meaning": "Λαμβάνω • Παίρνω",
          "example": "Ich bekomme ein Geschenk. – Παίρνω ένα δώρο."
        },
        {
          "word": "machen",
          "meaning": "Κάνω • Φτιάχνω",
          "example": "Ich mache das. – Το κάνω."
        }
      ]
    },
    "halten": {
      "study.tip.text": "Θυμήσου: κρατώ → halten· σταματώ → anhalten· θεωρώ → für … halten.",
      "lv": "Κρατώ • Σταματώ",
      "study.translation": "Κρατώ • Σταματώ",
      "study.explanation": [
        "Κύρια ιδέα: halten σημαίνει κρατώ· για όχημα μπορεί να σημαίνει σταματώ.",
        "Η έκφραση etwas für richtig halten σημαίνει θεωρώ κάτι σωστό.",
        "Το anhalten είναι χωριζόμενο ρήμα και σημαίνει σταματώ."
      ],
      "study.comparison": [
        {
          "word": "halten",
          "meaning": "Κρατώ • Σταματώ",
          "example": "Der Bus hält. – Το λεωφορείο σταματά."
        },
        {
          "word": "nehmen",
          "meaning": "Παίρνω",
          "example": "Ich nehme die Tasche. – Παίρνω την τσάντα."
        },
        {
          "word": "anhalten",
          "meaning": "Σταματώ",
          "example": "Bitte halten Sie an. – Παρακαλώ, σταματήστε."
        },
        {
          "word": "denken",
          "meaning": "Σκέφτομαι",
          "example": "Ich denke, das ist richtig. – Νομίζω ότι είναι σωστό."
        }
      ]
    },
    "Hand": {
      "study.tip": [
        "Hand = χέρι· Arm = βραχίονας."
      ],
      "study.important": [
        "die Hand δεν σημαίνει μόνο παλάμη· καλύπτει το χέρι από τον καρπό ως τα δάχτυλα."
      ],
      "lv": "Χέρι",
      "study.translation": "Χέρι",
      "study.explanation": [
        "Κύρια ιδέα: die Hand σημαίνει το χέρι από τον καρπό ως τα δάχτυλα.",
        "der Arm σημαίνει βραχίονας ή ολόκληρο το άνω άκρο, ανάλογα με τα συμφραζόμενα."
      ],
      "study.examples": [
        {
          "de": "Ich wasche meine Hände.",
          "lv": "Πλένω τα χέρια μου."
        },
        {
          "de": "Sie hält das Glas in der Hand.",
          "lv": "Κρατά το ποτήρι στο χέρι της."
        },
        {
          "de": "Mein Arm tut weh.",
          "lv": "Πονάει το χέρι μου."
        }
      ],
      "study.comparison": [
        {
          "word": "die Hand",
          "meaning": "Χέρι από τον καρπό ως τα δάχτυλα",
          "example": "in der Hand – στο χέρι"
        },
        {
          "word": "der Arm",
          "meaning": "Βραχίονας • Άνω άκρο",
          "example": "Mein Arm tut weh. – Πονάει το χέρι μου."
        }
      ],
      "study.examples[2].lv": "Πονάει ο βραχίονάς μου.",
      "study.comparison[1]": {
        "word": "der Arm",
        "meaning": "Βραχίονας • Άνω άκρο",
        "example": "Mein Arm tut weh. – Πονάει ο βραχίονάς μου."
      }
    },
    "heißen": {
      "lv": "Ονομάζομαι • Σημαίνω",
      "study.translation": "Ονομάζομαι • Σημαίνω",
      "study.tip.text": "Θυμήσου: Wie heißt du? = Πώς σε λένε; Was heißt das? = Τι σημαίνει αυτό;",
      "study.important": [
        "heißen χρησιμοποιείται για όνομα ή σημασία· nennen σημαίνει ονομάζω/αποκαλώ."
      ],
      "study.examples": [
        {
          "de": "Ich heiße Anna.",
          "lv": "Με λένε Άννα."
        },
        {
          "de": "Wie heißt du?",
          "lv": "Πώς σε λένε;"
        },
        {
          "de": "Wie heißt das auf Deutsch?",
          "lv": "Πώς λέγεται αυτό στα γερμανικά;"
        },
        {
          "de": "Was heißt das?",
          "lv": "Τι σημαίνει αυτό;"
        }
      ],
      "study.comparison": [
        {
          "word": "heißen",
          "meaning": "Ονομάζομαι • Σημαίνω",
          "example": "Ich heiße Anna. – Με λένε Άννα."
        },
        {
          "word": "nennen",
          "meaning": "Ονομάζω • Αποκαλώ",
          "example": "Er nennt mich Tom. – Με αποκαλεί Τομ."
        },
        {
          "word": "bedeuten",
          "meaning": "Σημαίνω",
          "example": "Was bedeutet das? – Τι σημαίνει αυτό;"
        },
        {
          "word": "rufen",
          "meaning": "Φωνάζω • Καλώ",
          "example": "Ich rufe dich. – Σε φωνάζω."
        },
        {
          "word": "anrufen",
          "meaning": "Τηλεφωνώ",
          "example": "Ich rufe dich an. – Σου τηλεφωνώ."
        }
      ],
      "study.explanation": [
        "Κύρια ιδέα: heißen χρησιμοποιείται κυρίως για να πούμε πώς ονομάζεται κάποιος.",
        "Η φράση Ich heiße... σημαίνει «με λένε...».",
        "Για λέξεις ή εκφράσεις, heißen μπορεί επίσης να σημαίνει «σημαίνω».",
        "Στο επίπεδο Α1, η σημαντικότερη φράση είναι Wie heißt du?"
      ]
    },
    "hoch": {
      "study.important": [
        "hoch περιγράφει ύψος ή επίπεδο· groß περιγράφει γενικό μέγεθος και συχνά το ύψος ανθρώπου."
      ],
      "lv": "Ψηλός • Υψηλός",
      "study.translation": "Ψηλός • Υψηλός",
      "study.explanation": [
        "Κύρια ιδέα: hoch σημαίνει ψηλός ή υψηλός.",
        "Περιγράφει κατακόρυφο ύψος ή υψηλό επίπεδο, τιμή ή ποσότητα."
      ],
      "study.examples": [
        {
          "de": "Der Berg ist hoch.",
          "lv": "Το βουνό είναι ψηλό."
        },
        {
          "de": "Das Regal ist zwei Meter hoch.",
          "lv": "Το ράφι έχει ύψος δύο μέτρα."
        },
        {
          "de": "Die Miete ist hoch.",
          "lv": "Το ενοίκιο είναι υψηλό."
        },
        {
          "de": "Die Mauer ist hoch.",
          "lv": "Ο τοίχος είναι ψηλός."
        },
        {
          "de": "Die Preise sind hoch.",
          "lv": "Οι τιμές είναι υψηλές."
        }
      ],
      "study.tip": [
        "Ύψος ή υψηλό επίπεδο → hoch.",
        "Γενικό μέγεθος → groß."
      ]
    },
    "können": {
      "lv": "Μπορώ • Ξέρω να",
      "study.translation": "Μπορώ • Ξέρω να",
      "study.comparison[0].meaning": "Μπορώ • Ξέρω να",
      "study.comparison[1].meaning": "Επιτρέπεται",
      "study.comparison[2].meaning": "Πρέπει",
      "study.comparison[3].meaning": "Ξέρω ένα γεγονός",
      "study.important": [
        "können = ικανότητα ή δυνατότητα· dürfen = άδεια· müssen = υποχρέωση."
      ],
      "study.explanation": [
        "Κύρια ιδέα: können σημαίνει μπορώ ή ξέρω να κάνω κάτι.",
        "Δηλώνει ικανότητα ή δυνατότητα.",
        "Είναι τροπικό ρήμα, γι' αυτό το δεύτερο ρήμα βρίσκεται συνήθως στο τέλος."
      ],
      "study.examples": [
        {
          "de": "Ich kann Deutsch sprechen.",
          "lv": "Ξέρω να μιλάω γερμανικά."
        },
        {
          "de": "Kannst du mir helfen?",
          "lv": "Μπορείς να με βοηθήσεις;"
        },
        {
          "de": "Wir können heute kommen.",
          "lv": "Μπορούμε να έρθουμε σήμερα."
        },
        {
          "de": "Er kann gut schwimmen.",
          "lv": "Ξέρει να κολυμπάει καλά."
        }
      ]
    },
    "kosten": {
      "study.important": [
        "kosten = κοστίζω· bezahlen = πληρώνω. Δεν είναι συνώνυμα."
      ],
      "lv": "Κοστίζω",
      "study.translation": "Κοστίζω",
      "study.explanation": [
        "Κύρια ιδέα: kosten σημαίνει κοστίζω και χρησιμοποιείται για την τιμή.",
        "Για την ενέργεια της πληρωμής χρησιμοποιούνται bezahlen ή zahlen.",
        "Συχνή ερώτηση: Was kostet das? = Πόσο κοστίζει αυτό;"
      ],
      "study.examples": [
        {
          "de": "Das kostet 5 Euro.",
          "lv": "Αυτό κοστίζει 5 ευρώ."
        },
        {
          "de": "Was kostet das?",
          "lv": "Πόσο κοστίζει αυτό;"
        },
        {
          "de": "Wie viel kostet der Pullover?",
          "lv": "Πόσο κοστίζει το πουλόβερ;"
        },
        {
          "de": "Das Essen kostet nicht viel.",
          "lv": "Το φαγητό δεν κοστίζει πολύ."
        },
        {
          "de": "Ich bezahle die Rechnung.",
          "lv": "Πληρώνω τον λογαριασμό."
        },
        {
          "de": "Kann ich bar bezahlen?",
          "lv": "Μπορώ να πληρώσω με μετρητά;"
        },
        {
          "de": "Er zahlt mit Karte.",
          "lv": "Πληρώνει με κάρτα."
        },
        {
          "de": "Ich zahle gleich.",
          "lv": "Θα πληρώσω αμέσως."
        }
      ],
      "study.comparison": [
        {
          "word": "kosten",
          "meaning": "Κοστίζω • Έχω τιμή",
          "example": "Das kostet 5 Euro. – Αυτό κοστίζει 5 ευρώ."
        },
        {
          "word": "bezahlen",
          "meaning": "Πληρώνω • Εξοφλώ",
          "example": "Ich bezahle die Rechnung. – Πληρώνω τον λογαριασμό."
        },
        {
          "word": "zahlen",
          "meaning": "Πληρώνω",
          "example": "Kann ich bar zahlen? – Μπορώ να πληρώσω με μετρητά;"
        },
        {
          "word": "Was kostet...?",
          "meaning": "Πόσο κοστίζει...;",
          "example": "Was kostet das Buch? – Πόσο κοστίζει το βιβλίο;"
        }
      ],
      "study.tip": [
        "Ερώτηση για τιμή → kosten.",
        "Ενέργεια πληρωμής → bezahlen / zahlen."
      ]
    },
    "Laden": {
      "study.tip": [
        "Ουσιαστικό: der Laden = κατάστημα.",
        "Ρήμα: laden = φορτώνω· einladen = προσκαλώ."
      ],
      "study.important": [
        "Laden με κεφαλαίο = κατάστημα· laden με πεζό = ρήμα."
      ],
      "study.explanation": [
        "Κύρια ιδέα: der Laden είναι ουσιαστικό και σημαίνει κατάστημα.",
        "Το ρήμα laden με πεζό αρχικό σημαίνει φορτώνω· σε σύνθετα ρήματα μπορεί επίσης να σχετίζεται με πρόσκληση, όπως einladen.",
        "Ο πληθυντικός του der Laden είναι die Läden."
      ]
    },
    "Land": {
      "lv": "Χώρα • Ύπαιθρος",
      "study.translation": "Χώρα • Ύπαιθρος",
      "study.examples[2].lv": "Πηγαίνουμε στην ύπαιθρο.",
      "study.examples[3].lv": "Η ζωή στην ύπαιθρο είναι ήσυχη.",
      "study.explanation": [
        "Κύρια ιδέα: das Land σημαίνει χώρα ή ύπαιθρος, ανάλογα με τα συμφραζόμενα.",
        "Deutschland ist ein Land = η Γερμανία είναι χώρα.",
        "auf dem Land = στην ύπαιθρο· aufs Land = προς την ύπαιθρο."
      ],
      "study.important": [
        "Land = χώρα· auf dem Land / aufs Land = ύπαιθρος."
      ],
      "study.comparison": [
        {
          "word": "das Land",
          "meaning": "Χώρα • Ύπαιθρος",
          "example": "Deutschland ist ein Land. – Η Γερμανία είναι χώρα."
        },
        {
          "word": "die Stadt",
          "meaning": "Πόλη",
          "example": "Ich wohne in der Stadt. – Μένω στην πόλη."
        },
        {
          "word": "das Dorf",
          "meaning": "Χωριό",
          "example": "Er lebt in einem Dorf. – Μένει σε ένα χωριό."
        },
        {
          "word": "die Erde",
          "meaning": "Γη • Πλανήτης",
          "example": "Die Erde ist rund. – Η Γη είναι στρογγυλή."
        }
      ]
    },
    "lang": {
      "lv": "Μακρύς • Για πολύ",
      "study.translation": "Μακρύς • Για πολύ",
      "study.explanation": [
        "Κύρια ιδέα: lang περιγράφει μήκος ή διάρκεια.",
        "ein langer Tisch = ένα μακρύ τραπέζι· ein langer Tag = μια μεγάλη σε διάρκεια ημέρα.",
        "den ganzen Tag lang = όλη την ημέρα."
      ],
      "study.important": [
        "lang αφορά μήκος ή διάρκεια· weit αφορά απόσταση ή έκταση."
      ],
      "study.examples": [
        {
          "de": "Der Tisch ist sehr lang.",
          "lv": "Το τραπέζι είναι πολύ μακρύ."
        },
        {
          "de": "Der Film war sehr lang.",
          "lv": "Η ταινία είχε μεγάλη διάρκεια."
        },
        {
          "de": "Wie lange dauert es?",
          "lv": "Πόσο διαρκεί;"
        },
        {
          "de": "Sie hat lange Haare.",
          "lv": "Έχει μακριά μαλλιά."
        },
        {
          "de": "Ich warte schon lange.",
          "lv": "Περιμένω εδώ και πολύ καιρό."
        },
        {
          "de": "Den ganzen Tag lang.",
          "lv": "Όλη την ημέρα."
        }
      ],
      "study.tip": [
        "Μήκος αντικειμένου → lang = μακρύς.",
        "Χρονική διάρκεια → lang = για πολύ / μεγάλης διάρκειας."
      ]
    },
    "lassen": {
      "study.examples[0].lv": "Αφήνω την τσάντα εδώ.",
      "study.examples[1].lv": "Άφησέ το στο τραπέζι, παρακαλώ.",
      "study.examples[2].lv": "Οι γονείς μου με αφήνουν να φύγω.",
      "study.examples[3].lv": "Άφησέ με ήσυχο!",
      "study.important": [
        "lassen = αφήνω ή επιτρέπω· Lass mich in Ruhe! = Άφησέ με ήσυχο!"
      ],
      "lv": "Αφήνω • Επιτρέπω",
      "study.translation": "Αφήνω • Επιτρέπω",
      "study.explanation": [
        "Κύρια ιδέα: lassen σημαίνει αφήνω κάτι ή επιτρέπω να συμβεί κάτι.",
        "Μπορεί να δηλώνει ότι κάτι παραμένει σε ένα μέρος ή ότι κάποιος παίρνει άδεια.",
        "Lass mich in Ruhe! σημαίνει Άφησέ με ήσυχο!"
      ],
      "study.comparison": [
        {
          "word": "lassen",
          "meaning": "Αφήνω • Επιτρέπω",
          "example": "Ich lasse das hier. – Το αφήνω εδώ."
        },
        {
          "word": "bleiben",
          "meaning": "Μένω • Παραμένω",
          "example": "Ich bleibe hier. – Μένω εδώ."
        },
        {
          "word": "erlauben",
          "meaning": "Επιτρέπω",
          "example": "Sie erlaubt mir das. – Μου το επιτρέπει."
        },
        {
          "word": "geben",
          "meaning": "Δίνω",
          "example": "Gib mir das Buch. – Δώσε μου το βιβλίο."
        }
      ],
      "study.tip": {
        "text": "Αφήνω κάτι ή επιτρέπω σε κάποιον → lassen."
      }
    },
    "laufen": {
      "lv": "Τρέχω • Λειτουργώ",
      "study.translation": "Τρέχω • Λειτουργώ",
      "study.explanation": [
        "Κύρια ιδέα: laufen σημαίνει τρέχω για άνθρωπο ή ζώο.",
        "Για μηχανή, συσκευή ή πρόγραμμα μπορεί να σημαίνει λειτουργώ.",
        "Για ταινία ή εκδήλωση μπορεί να σημαίνει προβάλλεται ή βρίσκεται σε εξέλιξη."
      ],
      "study.important": [
        "Άνθρωπος: laufen = τρέχω· συσκευή: laufen = λειτουργώ."
      ],
      "study.comparison": [
        {
          "word": "laufen",
          "meaning": "Τρέχω • Λειτουργώ",
          "example": "Er läuft schnell. – Τρέχει γρήγορα."
        },
        {
          "word": "gehen",
          "meaning": "Πηγαίνω με τα πόδια",
          "example": "Ich gehe nach Hause. – Πηγαίνω στο σπίτι."
        },
        {
          "word": "fahren",
          "meaning": "Μετακινούμαι με όχημα",
          "example": "Ich fahre mit dem Bus. – Πηγαίνω με το λεωφορείο."
        },
        {
          "word": "funktionieren",
          "meaning": "Λειτουργώ",
          "example": "Das funktioniert gut. – Αυτό λειτουργεί καλά."
        }
      ],
      "study.tip": {
        "text": "Γρήγορη κίνηση με τα πόδια → laufen· μετακίνηση με όχημα → fahren."
      }
    },
    "also": {
      "study.explanation": "Χρησιμοποιείται για να εκφράσει συμπέρασμα ή αποτέλεσμα. Σημαίνει «άρα», «λοιπόν» ή «επομένως».",
      "study.comparison": [
        {
          "word": "also",
          "meaning": "Άρα • Επομένως",
          "example": "Es regnet, also bleibe ich zu Hause. – Βρέχει, οπότε μένω σπίτι."
        },
        {
          "word": "auch",
          "meaning": "Επίσης",
          "example": "Ich komme auch. – Θα έρθω κι εγώ."
        },
        {
          "word": "deshalb",
          "meaning": "Γι' αυτό • Επομένως",
          "example": "Es regnet, deshalb bleibe ich zu Hause. – Βρέχει, γι' αυτό μένω σπίτι."
        }
      ],
      "study.tip": {
        "text": "Θυμήσου: συμπέρασμα → also = άρα / λοιπόν."
      },
      "study.important": [
        "Το also δηλώνει συμπέρασμα: η επόμενη ιδέα προκύπτει από την προηγούμενη.",
        "Το deshalb μπορεί επίσης να αποδοθεί ως γι' αυτό ή επομένως."
      ]
    }
  }
},
};
