...
const SYSTEM_PROMPT = [
  "You audit ES-DE Kurss Pronunciación (German pronunciation for Spanish learners). READ-ONLY audit.",
  "Each item: Spanish user-visible text (currentEs), German examples in deContext, Latvian MASTER reference (lvReference) for structure only — do NOT copy LV wording.",
  "Audit Spanish language: grammar, orthography, terminology, naturalness, foreign leftovers (LV, EN, IT, CS, etc.).",
  "Audit German phonetics: vowel length, umlauts, ch, sch, sp/st, z/s, ß, w/v, eu/äu/ie/ei — only topics present in the item.",
  "German headwords MUST remain German (warm, Hut, Stall, Zahl, etc.) — flag if replaced by Spanish words.",
  "Pronunciation cues in parentheses must NOT be mistranslated as Spanish word meanings.",
  "Categories: PHONETIC_ERROR, ES_GRAMMAR, ES_ORTHOGRAPHY, ES_TERMINOLOGY, ES_NATURALNESS, PEDAGOGICAL_ISSUE, FOREIGN_LEFTOVER, SEMANTIC_MISMATCH, TRANSCRIPTION_ISSUE, STRUCTURE.",
...
async function auditCardsBatch(options) {
  const {
    ...
    stats = null,
    batchLabel = "",
    auditType = "es_kurss_pronunciation",
    dataset = "kurss_pronunciacion",
  ...
  const input = [
    `Full ES-DE Kurss pronunciation audit. Return JSON items array. PASS for correct units; findings only for real issues.`,
  ...
  if (stats) {
    ...
    if (batchLabel) {
      process.stdout.write(
        `  luna ${batchLabel}: ${cards.length} units, findings=${findings.length}, pass=${passCount}, tokens=${response.usage?.total_tokens || 0}\n`,
      );
    }
  }
