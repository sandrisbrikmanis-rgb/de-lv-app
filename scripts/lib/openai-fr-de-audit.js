...
const VOCAB_PROMPT = [
  "You are a full linguistic quality auditor for FR-DE vocabulary and study cards (French translations of German).",
  "Audit French text against German meaning (primary authoritative source). Latvian in DE reference file is context only.",
...
const SENTENCES_PROMPT = [
  "You are a full linguistic quality auditor for FR-DE Sätze/Teikumi (French translations of German sentence flashcards).",
  "Audit French text in field 'lv' against German 'de' (primary authoritative source).",
  "The schema uses field name 'lv' for French — project convention convention, NOT Latvian.",
  ...
  "For real issues return: cardId, field (always 'lv'), severity, category, de, pairedGermanText, currentFr, proposedFr, reason, confidence.",
  "Severity: CRITICAL | HIGH | MEDIUM | LOW.",
  ...
  "Check: idiomatic French, register, negation mapping, question/exclamation punctuation, accents.",
  "CRITICAL: Latvian/Spanish/English/other foreign remnants in French.",
  "Keep reason under 160 chars. proposedFr must be exact replacement text.",
].join("\n");

const VERBS_PROMPT = [
  "You are a full linguistic quality auditor for FR-DE VERBS (French translations of German verb forms).",
  "Audit each French learner-facing verb form against German meaning (primary authoritative source).",
  "The data schema uses field name 'lv' for French native text — project convention, NOT Latvian language.",
  "Return ONLY valid JSON: { \"items\": [ ... ] }.",
  "For each form slot: if correct, return { cardId, field, status: \"PASS\" }.",
  "For real issues return: cardId, field, severity, category, de, pairedGermanText, currentFr, proposedFr, reason, confidence.",
  ...
  "Non-error: SOURCE_LV_ISSUE | SOURCE_DE_ISSUE | NEEDS_OWNER_REVIEW | STYLE_ONLY | PROJECT_CONVENTION | FALSE_POSITIVE.",
  "Check paradigm consistency across all 5 forms. German Konjunktiv II → French conditional/subjunctive.",
  "Partizip must reflect French past participle sense where German uses Partizip II.",
  "Use bullet • for multiple variants. Do NOT suggest DE changes.",
  "Keep reason under 140 chars. proposedFr must be exact replacement text.",
].join("\n");
...
function normalizeVocabItem(item) {
  ...
  return {
    ...
    currentFr: item.currentFr || item.currentText || "",
    proposedFr: item.proposedFr || item.proposedNew || "",
    reason: item.reason || "",
...
async function auditCardsBatch({ cards, stats, batchLabel, auditType, dataset, moduleType = "vocab", model = DEFAULT_MODEL }) {
  ...
  const response = await getClient().responses.create({
    ...
    instructions,
    input: [
      `Full FR-DE ${moduleType} linguistic audit. Return JSON items array. PASS for correct; findings only for real issues.`,
