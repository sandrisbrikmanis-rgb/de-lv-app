...
const SYSTEM_PROMPT = [
  "You are a full linguistic quality auditor for ET-DE A2 (Estonian translations of German A1 vocabulary and study cards).",
  "Audit Estonian text against German meaning (primary authoritative source) and Latvian source (secondary context only).",
  "The data schema uses field name 'lv' for Estonian native text — this is a project convention, NOT Latvian language.",
  "Return ONLY valid JSON: { \"items\": [ ... ] }.",
  "For each card/field: if correct, return { cardId, status: \"PASS\" }.",
  "For real issues return finding objects with:",
  "cardId, field, severity, category, de, lvSource, currentEt, proposedEt, reason, confidence.",
  ...
  "SOURCE_LV_ISSUE = LV conflicts with DE but Estonian is correct per DE.",
  "DE_SOURCE_ISSUE = possible DE source problem; do not suggest DE changes.",
  "STYLE_ONLY = Estonian correct; stylistic preference only — NOT a quality finding.",
  ...
  "CRITICAL: Latvian/Polish/Russian text in Estonian fields (e.g. lūdzu, Man ir, Es esmu, mājās, vācu valodu).",
  "Check Estonian: cases (käänded), verb forms, word order, natural collocations, õ/ä/ö/ü/š/ž orthography, CEFR C1/C2 register.",
  "Do NOT flag et !== study.translation unless semantically wrong or contradictory.",
  "Do NOT suggest changes to German (DE) fields. comparison.word stays German.",
  "Keep reason under 160 chars. proposedEt must be exact replacement text.",
].join("\n");
...
function normalizeItem(item) {
  ...
  return {
    ...
    currentEt: item.currentEt || item.currentText || item.etText || item.existingEtText || "",
    proposedEt: item.proposedEt || item.recommendedFix || item.proposedFix || "",
...
async function auditCardsBatch(options) {
  ...
  const payload = { auditType, dataset: "et-c1c2", cards };

  const input = [
    "Full ET-DE A2 linguistic audit. Return JSON items array. PASS for correct cards; findings only for real issues.",
