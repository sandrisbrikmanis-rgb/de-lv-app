...
function isFinalClosureAuditOnMainA1(dataset) {
  return dataset === "a1"
    && (process.argv.includes("--final-closure-audit-on-main")
      || process.env.CS_A1_FINAL_CLOSURE_AUDIT_ON_MAIN === "1");
}

function isFinal702AuditOnMainA1(dataset) {
  return dataset === "a1"
    && !isFinalClosureAuditOnMainA1(dataset)
    && (process.argv.includes("--final-702-audit-on-main")
      || process.env.CS_A1_FINAL_702_AUDIT_ON_MAIN === "1");
}

function isFinalAuditOnMainA1(dataset) {
  return dataset === "a1"
    && !isFinalClosureAuditOnMainA1(dataset)
    && !isFinal702AuditOnMainA1(dataset)
    && (process.argv.includes("--final-audit-on-main")
      || process.env.CS_A1_FINAL_AUDIT_ON_MAIN === "1");
}

function isFinalPostRepairA1(dataset) {
  return dataset === "a1"
    && !isFinalAuditOnMainA1(dataset)
...
function tempDir(dataset) {
  ...
  if (isFinal702AuditOnMainA1(dataset)) {
    return path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main");
  }
  if (isFinalAuditOnMainA1(dataset)) {
    return path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main");
  }
...
function finalPostRepairPaths(dataset) {
  ...
  return {
    ...
    findingIdPrefix: "FINAL-A1",
    auditType: "FINAL POST-REPAIR FULL AUDIT",
  };
}

function finalClosureAuditOnMainPaths(dataset) {
  if (dataset !== "a1") return null;
  return {
    tempDir: path.join(ROOT, "reports", "temp", "cs-a1-final-closure-audit-on-main"),
    progressFile: path.join(ROOT, "scripts", ".cs-a1-final-closure-audit-on-main-luna-progress.json"),
    validationProgressFile: path.join(ROOT, "scripts", ".cs-a1-final-closure-audit-on-main-validate-progress.json"),
    consolidatedJson: path.join(ROOT, "reports", "temp", "cs-a1-final-closure-audit-on-main.json"),
    validatedJson: path.join(ROOT, "reports", "temp", "cs-a1-final-closure-audit-on-main-validated.json"),
    coverageManifest: path.join(ROOT, "reports", "temp", "cs-a1-final-closure-audit-on-main", "coverage-manifest.json"),
    repairRetentionJson: path.join(ROOT, "reports", "temp", "cs-a1-final-closure-audit-on-main", "repair-retention.json"),
    sanityJson: path.join(ROOT, "reports", "temp", "cs-a1-final-closure-audit-on-main", "sanity-snapshots.json"),
    mainShaManifest: path.join(ROOT, "reports", "temp", "cs-a1-final-closure-audit-on-main", "main-sha-manifest.json"),
    reportMd: path.join(ROOT, "reports", "cs-a1-final-closure-audit-on-main.md"),
    findingIdPrefix: "CLOSURE-A1",
    auditType: "FINAL CLOSURE AUDIT ON MAIN",
    expectedMainSha: "3bfbb4bba56ee9bf1be2df8c539ee58d31736fcb",
  };
}

function final702AuditOnMainPaths(dataset) {
  if (dataset !== "a1") return null;
  return {
    tempDir: path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main"),
    progressFile: path.join(ROOT, "scripts", ".cs-a1-final-702-audit-on-main-luna-progress.json"),
    validationProgressFile: path.join(ROOT, "scripts", ".cs-a1-final-702-audit-on-main-validate-progress.json"),
    consolidatedJson: path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main.json"),
    validatedJson: path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main-validated.json"),
    coverageManifest: path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main", "coverage-manifest.json"),
    repairRetentionJson: path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main", "repair-retention.json"),
    sanityJson: path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main", "sanity-snapshots.json"),
    mainShaManifest: path.join(ROOT, "reports", "temp", "cs-a1-final-702-audit-on-main", "main-sha-manifest.json"),
    reportMd: path.join(ROOT, "reports", "cs-a1-final-702-audit-on-main.md"),
    findingIdPrefix: "FINAL702-A1",
    auditType: "FINAL 702/702 AUDIT ON MAIN",
    expectedMainSha: "d658e2b591837e9656bbb322fa039faee2293c8d",
  };
}

function finalAuditOnMainPaths(dataset) {
  if (dataset !== "a1") return null;
  return {
    tempDir: path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main"),
    progressFile: path.join(ROOT, "scripts", ".cs-a1-final-audit-on-main-luna-progress.json"),
    validationProgressFile: path.join(ROOT, "scripts", ".cs-a1-final-audit-on-main-validate-progress.json"),
    consolidatedJson: path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main.json"),
    validatedJson: path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main-validated.json"),
    coverageManifest: path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main", "coverage-manifest.json"),
    repairRetentionJson: path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main", "repair-retention.json"),
    sanityJson: path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main", "sanity-snapshots.json"),
    mainShaManifest: path.join(ROOT, "reports", "temp", "cs-a1-final-audit-on-main", "main-sha-manifest.json"),
    reportMd: path.join(ROOT, "reports", "cs-a1-final-audit-on-main.md"),
    findingIdPrefix: "MAIN-A1",
    auditType: "FINAL AUDIT ON MAIN",
  };
}

function auditPathsForA1(dataset) {
  if (dataset !== "a1") return null;
  if (isFinalClosureAuditOnMainA1(dataset)) return finalClosureAuditOnMainPaths(dataset);
  if (isFinal702AuditOnMainA1(dataset)) return final702AuditOnMainPaths(dataset);
  if (isFinalAuditOnMainA1(dataset)) return finalAuditOnMainPaths(dataset);
  if (isFinalPostRepairA1(dataset)) return finalPostRepairPaths(dataset);
  return postRepairPaths(dataset);
}
...
module.exports = {
  ...
  isFinalAuditOnMainA1,
  tempDir,
  postRepairPaths,
  finalPostRepairPaths,
  finalClosureAuditOnMainPaths,
  final702AuditOnMainPaths,
  finalAuditOnMainPaths,
  auditPathsForA1,
  ensureDir,
  batchLabel,
  buildSimpleCard,
