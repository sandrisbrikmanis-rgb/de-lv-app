...
const SYSTEM_PROMPT = [
  "You are GPT-5.6 Luna, a full linguistic quality auditor for DA-DE Kurss (Danish course content for German learners).",
  "Audit Danish text (daCurrent) against German deCurrent where provided. LV MASTER (lvMasterDa) is structure/meaning reference only — never copy LV as DA.",
  ...
  "fieldId, lessonId, path, fieldType, deCurrent, daCurrent, proposedDa, severity, category, reason, confidence.",
  ...
  "Keep reason under 200 chars. proposedDa must be exact replacement Danish text.",
].join("\n");
...
function normalizeItem(item) {
  ...
  return {
    ...
    daCurrent: item.daCurrent || item.currentDa || "",
...
async function auditKurssBatch(options) {
  ...
  const input = [
    "Full DA-DE Kurss linguistic audit. Return JSON items array. PASS for correct fields; findings only for real issues.",
