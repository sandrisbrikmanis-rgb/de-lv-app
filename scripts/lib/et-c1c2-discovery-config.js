"use strict";

const path = require("path");
const { ROOT } = require("./audit-common");

const DATASET = "et-c1c2";
const PRODUCTION_PATH = "data/et/c1.js";
const WWW_PATH = "www/data/et/c1.js";
const PRODUCTION_PATHS = ["data/et/c1.js", "data/et/c2.js"];
const REGISTRY_DIR = path.join(ROOT, "reports/discovery-registry/et-c1c2");

/** First FULL_DISCOVERY for ET-DE C1/C2 — no prior audit runs in registry. */
const AUDIT_RUNS = [];

const OWNER_SOURCES = [];

const REGRESSION_EXPECTED = {
  CURRENT_FINDINGS: 0,
  PREVIOUSLY_SEEN_RAW_LLM_CANDIDATE: 0,
  PRE_EXISTING_BUT_PREVIOUSLY_MISSED: 0,
  OWNER_DECISION_CONFIRMED: 0,
  OWNER_DECISION_REOPEN_REQUIRED: 0,
  REPAIR_REGRESSION: 0,
  FALSE_POSITIVE_OR_STYLE_ONLY: 0,
  GENUINELY_NEW_VALIDATED_REAL_FINDING: 0,
  AUDIT_DISCOVERY_NON_REPRODUCIBILITY: "N/A",
};

const SEMANTIC_DEDUP_CARDS = [];

module.exports = {
  DATASET,
  PRODUCTION_PATH,
  WWW_PATH,
  PRODUCTION_PATHS,
  REGISTRY_DIR,
  AUDIT_RUNS,
  OWNER_SOURCES,
  REGRESSION_EXPECTED,
  SEMANTIC_DEDUP_CARDS,
};
