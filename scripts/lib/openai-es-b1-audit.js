...
const SYSTEM_PROMPT = [
  "You are a full linguistic quality auditor for ES-DE B1 (Spanish translations of German B1 vocabulary and study cards).",
  "Audit Spanish text against German meaning (primary authoritative source). Latvian in data/b1.js is reference context only.",
...
function normalizeItem(item) {
  ...
  return {
    ...
    currentEs: item.currentEs || item.currentEt || item.currentText || "",
    proposedEs: item.proposedEs || item.proposedEt || item.proposedNew || "",
    reason: item.reason || "",
...
async function auditCardsBatch({ cards, stats, batchLabel, auditType = "full_linguistic", model = DEFAULT_MODEL }) {
  ...
  const payload = { auditType, dataset: "es-de-b1", cards };
  const response = await getClient().responses.create({
    model,
    instructions: SYSTEM_PROMPT,
    input: [
      "Full ES-DE B1 linguistic audit. Return JSON items array. PASS for correct cards; findings only for real issues.",
