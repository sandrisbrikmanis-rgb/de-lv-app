...
function normalizeField(field) {
  ...
  if (!f.startsWith("study.") && f !== "lv" && f.includes(".")) {
    f = `study.${f}`;
  }
...
function findEntry(words, cardId) {
  ...
  const prefixMatch = cardId.match(/^c[12]-(.+?)(?:-\d+)?$/);
  if (prefixMatch) {
  ...
  const base = cardId.replace(/^c[12]-/, "").replace(/-study.*$/, "");
  entry = words.find((e) => e.de === base || e.de.toLowerCase() === base.toLowerCase());
  if (entry) return entry;
  ...
  if (stripped !== base) {
    ...
    if (entry) return entry;
    const studyId = cardId.replace(/-\d+$/, "");
    entry = words.find(
      (e) => e.study?.id === studyId || e.study?.id?.toLowerCase() === studyId.toLowerCase()
    );
    if (entry) return entry;
  }
