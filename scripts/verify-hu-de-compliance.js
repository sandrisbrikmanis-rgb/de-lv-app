#!/usr/bin/env node
/**
 * Verify HU-DE (British Magyar) creation compliance:
 * 1. German (de) fields in EN data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const en = loadArray(`data/hu/${level}.js`);
  const n = Math.min(lv.length, en.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const enFields = collectDeFields(en[i]);
    for (const lf of lvFields) {
      const tf = enFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, en: tf?.value });
      }
    }
  }
}
...
const enSent = loadArray("data/hu/sentences.js");
lvSent.forEach((lv, i) => {
  if (enSent[i] && lv.de !== enSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, en: enSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const enVerbs = loadArray("data/hu/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const enEntry = enVerbs[i];
  if (!enEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== enEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, en: enEntry[form]?.de });
    }
  }
});
