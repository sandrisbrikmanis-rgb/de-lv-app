...
"use strict";
/**
 * Build DA–DE B1 OWNER review group files from audit JSON (READ-ONLY).
...
function loadWords() {
  ...
  vm.runInContext(fs.readFileSync(path.join(ROOT, "data/b1.js"), "utf8"), lvCtx);
  return { da: ctx.window.B1_WORDS, lv: lvCtx.window.B1_WORDS };
}

function findingNum(f) {
  return Number(String(f.id).replace("DA-B1-", ""));
}

function categorize(f) {
  if (f.field === "study" && /Trūkst Study|missing/i.test(f.problem)) return "missingStudy";
...
function deForFinding(f, daWords) {
  ...
  const entry = daWords.find((w) => w.study?.id === f.cardId || w.de === f.cardId.replace(/^b1-/, ""));
...
function renderFinding(f, daWords) {
  ...
  lines.push(`**LV reference:** (skat. LV etalonu \`data/b1.js\`)`);
  ...
  lines.push(`**Avots:** GPT-5.6 Luna audit (\`reports/da-b1-full-audit.md\`)`);
  lines.push("");
...
function renderGroupFile(title, slug, batchIndex, slice, daWords, findingRange) {
  const lines = [];
  lines.push(`# DA–DE B1 — OWNER review ${title}`);
  lines.push("");
  lines.push(`Avots: \`reports/da-b1-full-audit.md\` / \`reports/temp/da-b1-audit-data.json\``);
  lines.push(`Findings: **${findingRange}** (${slice.length} ieraksti)`);
  lines.push(`Fails: \`reports/da-b1-owner-review-${slug}.md\``);
  lines.push("");
  lines.push(
...
function renderDecisionsTemplate(slug, slice, findingRange) {
  const lines = [];
  lines.push(`# DA–DE B1 — OWNER decisions — ${slug}`);
  lines.push("");
  lines.push(`Avots: \`reports/da-b1-owner-review-${slug}.md\``);
  lines.push(`Findings: **${findingRange}** (${slice.length} ieraksti)`);
  lines.push("");
...
function renderReadme(groups, total) {
  const byCat = { comparison: 0, sectionaccents: 0, misc: 0, missingStudy: 0 };
  groups.forEach((g) => {
  ...
  const tableRows = groups
    .map((g) => {
      const review = `da-b1-owner-review-${g.slug}.md`;
      const decisions = `da-b1-owner-decisions-${g.slug}.md`;
      return `| ${mdLink(review)} | ${g.range} | ${g.count} | ${g.category} | ${mdLink(decisions)} |`;
    })
    .join("\n");

  return `# DA–DE B1 — OWNER review (Copy-Only workflow)
...
| Missing Study | **${byCat.missingStudy || 0}** |
...
function main() {
  ...
  const missingStudy = findings.filter((f) => categorize(f) === "missingStudy");
  ...
  if (missingStudy.length) {
    const msBatches = chunk(missingStudy, 10);
    msBatches.forEach((batch, i) => {
      const nums = batch.map(findingNum);
      const slug = i === 0 && msBatches.length === 1 ? "missing-study" : `missing-study-${String(i + 1).padStart(2, "0")}`;
      groups.push({
        slug,
        batch,
        range: `${Math.min(...nums)}–${Math.max(...nums)}`,
        count: batch.length,
        category: "missingStudy",
      });
    });
  }
  ...
  fs.writeFileSync(path.join(ROOT, "reports/da-b1-owner-review-README.md"), renderReadme(groups, findings.length));
  written.push({ file: "reports/da-b1-owner-review-README.md", count: 1 });
    ...
    const title =
      g.category === "comparison"
        ...
        : g.category === "sectionAccents"
          ...
          : g.category === "missingStudy"
            ? `Missing Study ${g.slug.replace("missing-study", "part ").replace(/^part $/, "")}`
            : `Misc ${g.slug.replace("misc-", "part ")}`;

    const reviewPath = path.join(ROOT, `reports/da-b1-owner-review-${g.slug}.md`);
    fs.writeFileSync(
      reviewPath,
      renderGroupFile(title, g.slug, g.slug, g.batch, da, g.range)
    );
    written.push({ file: `reports/da-b1-owner-review-${g.slug}.md`, count: g.count, range: g.range });

    const decisionsPath = path.join(ROOT, `reports/da-b1-owner-decisions-${g.slug}.md`);
    fs.writeFileSync(decisionsPath, renderDecisionsTemplate(g.slug, g.batch, g.range));
    written.push({ file: `reports/da-b1-owner-decisions-${g.slug}.md`, count: g.count, template: true });
  }
