#!/usr/bin/env node
/**
 * Generate SV-DE data from FI baseline with batched translation (FI → SV).
...
const OUT_DIR = path.join(ROOT, "data", "sv");
const CACHE_PATH = path.join(ROOT, "scripts", ".sv-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "fi";
const TO_LANG = "sv";
...
function loadFiTrainingCardsFromData() {
  const cardsPath = path.join(ROOT, "data/fi/courseTrainingCards.js");
  ...
  vm.runInContext(code, ctx);
  const cards = {};
  const arrayPattern = /window\.(lesson\d+(?:Training|Exercise)CardsFi)\s*=\s*(\[[\s\S]*?\n\]);/g;
  let match;
...
async function main() {
  ...
  console.log("Loading FI baseline and LV German source...");
  ...
  const fiA1 = loadData("data/fi/a1.js").A1_WORDS;
  const fiA2 = loadData("data/fi/a2.js").A2_WORDS;
  const fiB1 = loadData("data/fi/b1.js").B1_WORDS;
  const fiB2 = loadData("data/fi/b2.js").B2_WORDS;
  const fiC1 = loadData("data/fi/c1.js").C1_WORDS;
  const fiC2 = loadData("data/fi/c2.js").C2_WORDS;
  const fiSentences = loadData("data/fi/sentences.js").SENTENCE_ENTRIES;
  const fiVerbs = loadData("data/fi/verbs.js").VERB_ENTRIES;
  const fiDialogue = loadData("data/fi/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const fiCourse = loadData("data/fi/courseLessons.js");
  const fiTraining = loadFiTrainingCardsFromData();

  [fiA1, fiA2, fiB1, fiB2, fiC1, fiC2, fiSentences, fiVerbs].forEach((arr) => arr.forEach((e) => collectStrings(e, allStrings)));
  Object.values(fiDialogue).forEach((e) => collectStrings(e, allStrings));
  Object.values(fiCourse.COURSE_LESSON_DATA || {}).forEach((e) => collectStrings(e, allStrings));
  Object.values(fiCourse.COURSE_LESSON_HTML || {}).forEach((html) => {
    [...String(html).matchAll(/>([^<]{3,300})</g)].forEach((m) => {
      const t = m[1].trim();
      if (t) allStrings.add(t);
    });
  });
  Object.values(fiTraining).forEach((deck) => {
  ...
  console.log(`Found ${unique.length} unique FI strings (${alreadyCached} cached)`);
  ...
  function mergeLevel(lvArr, fiArr) {
    return lvArr.map((lvEntry, i) => {
      const fiEntry = fiArr[i];
      if (!fiEntry || lvEntry.de !== fiEntry.de) {
        console.warn(`Order mismatch at ${lvEntry.de}`);
        return lvEntry;
      }
      const trEntry = applyTranslation(fiEntry, map);
      return preserveGermanFields(lvEntry, trEntry);
    });
  }

  console.log("\nWriting SV files...");
  writeArrayFile(path.join(OUT_DIR, "a1.js"), "A1_WORDS", mergeLevel(lvA1, fiA1));
  writeArrayFile(path.join(OUT_DIR, "a2.js"), "A2_WORDS", mergeLevel(lvA2, fiA2));
  writeArrayFile(path.join(OUT_DIR, "b1.js"), "B1_WORDS", mergeLevel(lvB1, fiB1));
  writeArrayFile(path.join(OUT_DIR, "b2.js"), "B2_WORDS", mergeLevel(lvB2, fiB2));
  writeArrayFile(path.join(OUT_DIR, "c1.js"), "C1_WORDS", mergeLevel(lvC1, fiC1));
  writeArrayFile(path.join(OUT_DIR, "c2.js"), "C2_WORDS", mergeLevel(lvC2, fiC2));
  writeArrayFile(path.join(OUT_DIR, "sentences.js"), "SENTENCE_ENTRIES", mergeLevel(lvSentences, fiSentences));

  const svVerbs = lvVerbs.map((lvEntry, i) => {
    const fiEntry = fiVerbs[i];
    const te = {};
    for (const form of Object.keys(lvEntry)) {
      te[form] = {
        de: lvEntry[form].de,
        lv: map[fiEntry[form].lv] || fiEntry[form].lv,
      };
    }
    return te;
  });
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", svVerbs);

  const svDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const fiEntry = fiDialogue[id];
    svDialogue[id] = { de: lvEntry.de, lv: map[fiEntry.lv] || fiEntry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(svDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  const svHtml = {};
  for (const [key, html] of Object.entries(fiCourse.COURSE_LESSON_HTML || {})) {
    svHtml[key] = await translateHtmlLesson(html, map);
  }
  const svData = applyTranslation(fiCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(svHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(svData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Swedish course training cards for SV-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(fiTraining)) {
    if (!Array.isArray(deck)) continue;
    const svKey = key.replace(/Fi$/, "Sv");
    const translated = deck.map((card) => ({
      front: map[card.front] || card.front,
      back: card.back || "",
    }));
    outLines.push(`window.${svKey} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nSV-DE data generation complete (FI→SV with LV DE preservation).");
}
