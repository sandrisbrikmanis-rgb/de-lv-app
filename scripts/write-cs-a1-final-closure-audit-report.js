#!/usr/bin/env node
/**
 * CS-DE A1 final closure audit on main report (read-only).
 * Usage: node scripts/write-cs-a1-final-closure-audit-report.js --final-closure-audit-on-main
 */
const fs = require('fs');
...
function main() {
  ...
  const repairOk = (cr.missing || 0) === 0 && (cr.conflicting || 0) === 0
    ...
    && (repair.sectionAccents?.realRemaining ?? accents.REAL ?? 0) === 0;
  ...
  const report = `# CS–DE A1 FINAL CLOSURE AUDIT ON MAIN
...
| accounted | ${cr.accounted || 0}/125 |
| APPLIED retained | ${cr.appliedRetained || 0} |
| NO_OP retained | ${cr.noOp || 0} |
| missing | ${cr.missing || 0} |
| conflicting | ${cr.conflicting || 0} |
| OWNER source | 5 |
| LABOT retained | ${owner.labot || 0} |
| OWNER_KEEP retained | ${owner.ownerKeep || 0} |
| OWNER unresolved | ${owner.remaining || 0} |
| a1-ferien content repair | ${repair.ferien?.importantRetained ? 'PASS' : 'FAIL'} |
...
| DE changes | ${technical.deChanges || 0} |
