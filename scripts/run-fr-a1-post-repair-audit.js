...
"use strict";
/**
 * FR-DE A1 POST-REPAIR full READ-ONLY audit (deterministic + GPT-5.6 Luna).
 * Usage: node scripts/run-fr-a1-post-repair-audit.js [--skip-luna] [--test-luna] [--resume-luna]
 * Runs on PR #683 HEAD after OWNER copy-only apply. MASTER v1.12 closure gate.
 */
process.env.FR_A1_POST_REPAIR = "1";
...
const FRESH_LUNA = !RESUME_LUNA;
const OUT_MD = path.join(ROOT, "reports", "fr-a1-post-repair-audit.md");
const OUT_JSON = path.join(ROOT, "reports", "temp", "fr-a1-post-repair-audit.json");
const OUT_JSON_PUBLISHED = path.join(ROOT, "reports", "fr-a1-post-repair-audit.json");
...
function computeBaselineGate(ownerHistory) {
  ...
  const headProductionSha = git(`git rev-parse HEAD:${PRODUCTION_PATH}`);
  const headWwwSha = git(`git rev-parse HEAD:${WWW_PATH}`);
  const applyBaseSha = git("git merge-base HEAD origin/main") || originMainSha;

  if (headProductionSha !== headWwwSha) {
    return {
      auditMode: "POST_REPAIR_FULL_DISCOVERY",
      masterVersion: MASTER_VERSION,
      originMainSha,
      applyBaseSha,
      datasetProductionSha: headProductionSha,
      datasetProductionBlobSha: headProductionSha,
      wwwDatasetBlobSha: headWwwSha,
      ...
      deReadOnly: "PASS",
      blocked: true,
      blockReason: "data/fr/a1.js blob !== www/data/fr/a1.js blob on HEAD",
    };
  }
  ...
  return {
    ...
    lastFinalClosureDatasetBlobSha: null,
    unmergedRepairBranchesFound: [],
    baselineStatus: "POST_REPAIR_PR683",
...
function buildReport(ctx) {
  ...
  const lines = [];
  lines.push("# FR–DE A1 POST-REPAIR pilns lingvistiskais audits (MASTER v1.12 · PR #683 HEAD)");
  lines.push("");
...
async function main() {
  ...
  console.log("\n=== FR–DE A1 POST-REPAIR FULL_DISCOVERY — MASTER v1.12 · PR #683 ===\n");
  ...
  if (FRESH_LUNA) {
    const progress = path.join(ROOT, "scripts/.fr-a1-post-repair-luna-progress.json");
  ...
  if (baseline.baselineStatus === "POST_REPAIR_PR683") {
    // Discovery history gates from registry + OWNER filled authority (not N/A).
  } else if (baseline.baselineStatus === "FIRST_AUDIT_NO_CLOSURE_BASELINE") {
  ...
  runPostAuditOwnerReview("fr-a1-post-repair", {
    backlogCount: ownerBacklogFinal.length,
    skipPublish: true,
  });
}
