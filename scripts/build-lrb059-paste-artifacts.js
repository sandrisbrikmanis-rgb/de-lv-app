...
const BATCH = "LRB-059";
const PASTE_PATH = process.argv[2] || path.join(ROOT, "scripts/data/g2-a1-owner-pending/LRB-059-decisions-COPY-PASTE.json");
const EXPECTED_PASTE_SHA = "b8e774d26eab6460a9f1ce7206ccb7cae18e6a61f60af2f71991cb29c19237bb";
...
const FULL_CARD_BOILERPLATE_NOTE =
  "Pilnā gala kartīte individuāli salīdzināta ar LV→DE avotu: galvenā doma, visi piemēri, salīdzinājumi, padoms, svarīgais un akcenti aizvietoti ar korektu luksemburgiešu saturu; DE teksti saglabāti.";

function fixSectionAccentsImportant(ownerNew, cardObjectId) {
  if (!ownerNew || typeof ownerNew !== "object") return ownerNew;
  const key = "study.sectionAccents";
  if (!ownerNew[key]) return ownerNew;
  const accents = parseMaybeJson(ownerNew[key]);
  if (!accents?.important || !Array.isArray(accents.important)) return ownerNew;

  if (cardObjectId === "Bitte|idx:94") {
    accents.important = [
      { green: ["die Bitte"] },
      { green: ["die Bitten"] },
      { purple: ["Substantiv", "groussem Ufanksbuschtaf"] },
    ];
  } else if (cardObjectId === "bringen|idx:111") {
    accents.important = [
      { red: ["nehmen"], purple: ["huelen"] },
      { blue: ["bringen"], purple: ["bréngen", "liwweren"] },
    ];
  } else {
    return ownerNew;
  }

  return { ...ownerNew, [key]: JSON.stringify(accents) };
}

function normalizePasteDecision(d) {
  let ownerNote = d.owner_note;
  if (
    d.replacement_scope === "full_card_target_fields" &&
    String(ownerNote || "").trim() === FULL_CARD_BOILERPLATE_NOTE
  ) {
    ownerNote = `${FULL_CARD_BOILERPLATE_NOTE} [${d.card_object_id}]`;
  }
  const ownerNew = fixSectionAccentsImportant(d.owner_new, d.card_object_id);
  return { ...d, owner_note: ownerNote, owner_new: ownerNew };
}
...
function main() {
  ...
  for (const row of rows) {
    ...
    if (ownerNew != null && typeof ownerNew === "object") ownerNew = JSON.stringify(ownerNew);
  ...
  const galaOut = {
    ...
    classification: "LRB_059_COPY_PASTE_COMPLETE_AWAITING_GALA_VERDICT",
    cards: galaCards,
  };
