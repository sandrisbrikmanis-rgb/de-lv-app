#!/usr/bin/env node
/**
 * ET-DE C2 full audit collector (read-only). Writes reports/temp/et-c2-audit-data.json
 */
const fs = require("fs");
...
const LV_FILE = path.join(ROOT, "data", "c2.js");
const ET_FILE = path.join(ROOT, "data", "et", "c2.js");
const WWW_FILE = path.join(ROOT, "www", "data", "et", "c2.js");
const OUT = path.join(ROOT, "reports", "temp", "et-c2-audit-data.json");

const LV_ONLY = /[āēīūģķļņĀĒĪŪĢĶĻŅ]/;
// Note: omit bare `\bvest\b` — collides with valid Estonian *vest* gloss.
...
function main() {
  ...
  const data = {
    meta: {
      ...
      lvFile: "data/c2.js",
      etFile: "data/et/c2.js",
      wwwFile: "www/data/et/c2.js",
      lvCount: lv.length,
      etCount: et.length,
    },
  ...
  // Layer identity
  data.layerIdentity.identical = fs.readFileSync(ET_FILE).equals(fs.readFileSync(WWW_FILE));
  if (!data.layerIdentity.identical) {
    data.layerIdentity.pass = false;
    data.structural.issues.push({ severity: "critical", message: "data/et/c2.js and www/data/et/c2.js are not identical" });
  }

  // Structural
  if (lv.length !== et.length) {
    data.structural.pass = false;
    data.structural.issues.push({ severity: "critical", message: `Record count mismatch LV=${lv.length} ET=${et.length}` });
  }

  const lvStudy = lv.filter((e) => e.study).length;
  const etStudy = et.filter((e) => e.study).length;
  data.meta.lvStudyCount = lvStudy;
  data.meta.etStudyCount = etStudy;
  if (lvStudy !== etStudy) {
    data.structural.pass = false;
    data.structural.issues.push({ severity: "critical", message: `Study count mismatch LV=${lvStudy} ET=${etStudy}` });
  }
  ...
  for (let i = 0; i < Math.min(lv.length, et.length); i++) {
    ...
    if (lvE.de !== etE.de) {
      data.structural.pass = false;
      data.structural.issues.push({ severity: "critical", id, message: `Order mismatch at index ${i}: LV.de=${lvE.de} ET.de=${etE.de}` });
    }

    if (etE.study?.id) {
      if (ids.has(etE.study.id)) {
        data.structural.issues.push({ severity: "high", id, message: `Duplicate study id: ${etE.study.id}` });
        data.structural.pass = false;
      }
      ids.add(etE.study.id);
    }

    const lvKeys = new Set(schemaKeys(lvE).filter((k) => k !== "lv" && !k.includes(".lv")));
    const etKeys = new Set(schemaKeys(etE).filter((k) => k !== "lv" && !k.includes(".lv")));
    const missing = [...lvKeys].filter((k) => !etKeys.has(k));
    const extra = [...etKeys].filter((k) => !lvKeys.has(k));
    if (missing.length) {
      data.structural.issues.push({ severity: "high", id, de: etE.de, message: `Missing fields vs LV: ${missing.join(", ")}` });
      data.structural.pass = false;
    }
    if (extra.length) {
      data.structural.issues.push({ severity: "medium", id, de: etE.de, message: `Extra fields vs LV: ${extra.join(", ")}` });
    }

    const lvLayout = lvE.study?.layout || (lvE.study ? "standardStudy" : null);
    const etLayout = etE.study?.layout || (etE.study ? "standardStudy" : null);
    if (lvLayout !== etLayout) {
      data.structural.issues.push({ severity: "high", id, de: etE.de, message: `Layout mismatch LV=${lvLayout} ET=${etLayout}` });
      data.structural.pass = false;
    }

    if (etE.study?.layout === "comparisonStudy") data.comparisonStudy.count++;
    else if (etE.study) data.standardStudy.count++;

    // German integrity
    for (const field of ["de", "de_article", "de_plural", "level"]) {
      if (lvE[field] !== etE[field]) {
  ...
  if (lvE.study && etE.study) {
      ...
      walkStrings(etE.study, (text, ctx) => {
        if (ctx.parentKey === "de" || ctx.path.includes(".de") && !ctx.path.includes(".lv")) etDeStrings.push(text);
      });
      // Better: walk paired
    ...
    const main = {
      id,
      de: etE.de,
      de_article: etE.de_article,
      lvGloss: lvE.lv,
      etMain: etE.lv,
      studyTranslation: etE.study?.translation || null,
      status: "OK",
      flags: [],
    };
    if (!etE.lv || !etE.lv.trim()) {
      main.status = "ERROR";
      main.flags.push("empty_main_translation");
    }
    if (/^[A-ZÕÄÖÜŠŽ]/.test(etE.lv) && !etE.lv.includes("•")) {
      main.flags.push("unexpected_capitalization");
      main.status = main.status === "ERROR" ? "ERROR" : "WARNING";
    }
    if (LV_ONLY.test(etE.lv) || LV_WORDS.test(etE.lv)) {
      main.status = "ERROR";
      main.flags.push("latvian_remnant_in_main");
    }
    if (hasTechnicalArtifact(etE.lv)) {
      main.status = "ERROR";
        ...
        function checkAccentsBlock(block, sectionName, index) {
          ...
          for (const color of ACCENT_COLORS) {
            ...
            for (const term of termsToCheck) {
              ...
              if (!accentTermMatches(text, term) && !accentTermMatches(etE.lv, term)) {
