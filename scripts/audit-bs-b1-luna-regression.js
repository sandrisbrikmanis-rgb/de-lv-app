#!/usr/bin/env node
/**
 * BS-DE B1 Luna targeted regression audit (read-only).
 * Audits only cards from bs-b1-luna-targeted-regression.json.
 *
 * Usage:
 *   node scripts/audit-bs-b1-luna-regression.js [--test-batch]
 */
const fs = require("fs");
...
const PROGRESS_PATH = path.join(ROOT, "scripts", ".bs-b1-luna-regression-audit-progress.json");
...
async function main() {
  ...
  const targetIds = new Set(regression.allCardIds || []);
  if (targetIds.size !== 328) {
    console.warn(`Warning: expected 328 cards, got ${targetIds.size}`);
  }
  ...
  console.log(`Luna regression audit: ${targetIds.size} target cards (${simpleCards.length} simple, ${studyCards.length} study)`);
  ...
  if (simpleCards.length > 0) {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const findings = await auditBatchWithRetry(batches[i], stats, batchKey, "regression_simple");
      allFindings.push(...findings);
      completed.add(batchKey);
      saveFindings(allFindings);
      saveProgress({ completedBatches: [...completed], auditedCards: countAudited(completed, simpleCards, studyCards) });
    }
  }
  ...
  if (studyCards.length > 0) {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const findings = await auditBatchWithRetry(batches[i], stats, batchKey, "regression_study");
      allFindings.push(...findings);
      completed.add(batchKey);
      saveFindings(allFindings);
      saveProgress({ completedBatches: [...completed], auditedCards: countAudited(completed, simpleCards, studyCards) });
  ...
  console.log("\n=== Luna regression audit complete ===");
...
function countAudited(completed, simpleCards, studyCards) {
  let n = 0;
  const simpleBatches = chunk(simpleCards, SIMPLE_BATCH);
  const studyBatches = chunk(studyCards, STUDY_BATCH);
  for (let i = 0; i < simpleBatches.length; i++) {
    if (completed.has(`simple-${i}`)) n += simpleBatches[i].length;
  }
  for (let i = 0; i < studyBatches.length; i++) {
    if (completed.has(`study-${i}`)) n += studyBatches[i].length;
  }
  return n;
}

main().catch((error) => {
  console.error("Luna regression audit failed:", error.message);
  process.exit(1);
});
