#!/usr/bin/env node
/**
 * FR-DE A1+A2 full audit collector (read-only).
 * Writes reports/temp/fr-de-a1-a2-audit-data.json
 */
const fs = require("fs");
...
const LEVELS = ["a1", "a2"];
const OUT = path.join(ROOT, "reports", "temp", "fr-de-a1-a2-audit-data.json");
...
function auditLevel(level) {
  ...
  const es = load(`data/fr/${level}.js`);
  const result = {
    level,
    counts: {
      cards: lv.length,
      lvStudy: lv.filter((e) => e.study).length,
      frStudy: es.filter((e) => e.study).length,
    },
  ...
  if (result.counts.lvStudy !== result.counts.frStudy) {
    result.structural.pass = false;
    result.structural.issues.push({
      severity: "CRITICAL",
      message: `Study count mismatch LV=${result.counts.lvStudy} ES=${result.counts.frStudy}`,
    });
  }
  ...
  if (!result.mirror.identical) {
    ...
    result.structural.issues.push({
      severity: "CRITICAL",
      message: `Mirror mismatch data/fr/${level}.js vs www/data/fr/${level}.js`,
    });
  }
  ...
  for (let i = 0; i < Math.min(lv.length, es.length); i++) {
    ...
    walkStrings(esE, (text, ctx) => {
      ...
      if (ES_WORDS.test(text) && !/\bespañol\b/i.test(text)) {
        result.foreignRemnants.pass = false;
        result.foreignRemnants.issues.push({
          severity: "MEDIUM",
          id,
...
function main() {
  ...
  const payload = {
    ...
    summary: {
      ...
      a1StudyGap: levels[0].counts.lvStudy - levels[0].counts.frStudy,
      a2StudyGap: levels[1].counts.lvStudy - levels[1].counts.frStudy,
