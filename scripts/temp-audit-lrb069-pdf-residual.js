...
const EXPECTED_LABOT = 44;
const EXPECTED_NELABOT = 0;
const EXPECTED_UNIQUE_CARDS = 44;
...
function countDuplicateExamplePairs(study, deStudy) {
  const examples = study?.examples;
  const deExamples = deStudy?.examples;
  if (!Array.isArray(examples)) return 0;
  let dups = 0;
  for (let i = 0; i < examples.length; i += 1) {
    const pairKey = `${examples[i]?.de ?? ""}\0${examples[i]?.lv ?? ""}`;
    for (let j = 0; j < i; j += 1) {
      const priorKey = `${examples[j]?.de ?? ""}\0${examples[j]?.lv ?? ""}`;
      if (pairKey !== priorKey) continue;
      const deTemplateRepeat =
        Array.isArray(deExamples) &&
        deExamples[j]?.de === examples[j]?.de &&
        deExamples[i]?.de === examples[i]?.de &&
        deExamples[j]?.de === deExamples[i]?.de;
      if (!deTemplateRepeat) dups += 1;
    }
  }
  return dups;
}
...
for (const [key, { lang, card }] of UNIQUE_CARDS) {
  ...
  if (isCompositeRepair) {
    const deStudy = nestedDeForCard(card)?.study;
    const dupes = countDuplicateExamplePairs(merged.study, deStudy);
    duplicateExamplePairs += dupes;
    if (dupes > 0) {
      issues.push({ type: "DUPLICATE_EXAMPLE_PAIR", card, lang, count: dupes });
      semanticViolations += dupes;
    }
  }
...
const proof = {
  batch_id: BATCH,
  classification: pass
    ? "LRB_069_OWNER_PREP_READY_FOR_LINGUISTIC_REVIEW"
    : "LRB_069_OWNER_PREP_BLOCKED",
  pdf_reaudit: true,
  ...
  gates: {
    ...
    de_source_mismatches_across_22_full_composites: deSourceMismatchesAcrossFullComposites,
