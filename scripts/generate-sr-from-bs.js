#!/usr/bin/env node
/**
 * Generate SR-DE data from BS baseline (BS was generated from SL with DE READ-ONLY).
 * Translates only native-language (lv) fields BS → SR.
 * German (de) fields are preserved verbatim from BS/LV source (READ-ONLY).
 */
const fs = require("fs");
...
const OUT_DIR = path.join(ROOT, "data", "sr");
const CACHE_PATH = path.join(ROOT, "scripts", ".sr-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "bs";
const TO_LANG = "sr";
...
async function main() {
  ...
  console.log("Collecting unique Bosnian strings from BS-DE for SR translation...");
  const a1 = loadData("data/bs/a1.js").A1_WORDS;
  const a2 = loadData("data/bs/a2.js").A2_WORDS;
  const b1 = loadData("data/bs/b1.js").B1_WORDS;
  const b2 = loadData("data/bs/b2.js").B2_WORDS;
  const c1 = loadData("data/bs/c1.js").C1_WORDS;
  const c2 = loadData("data/bs/c2.js").C2_WORDS;
  const sentences = loadData("data/bs/sentences.js").SENTENCE_ENTRIES;
  const verbs = loadData("data/bs/verbs.js").VERB_ENTRIES;
  const dialogue = loadData("data/bs/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const course = loadData("data/bs/courseLessons.js");
  const training = fs.existsSync(path.join(ROOT, "data/bs/courseTrainingCards.js"))
    ? loadData("data/bs/courseTrainingCards.js")
    : {};
  ...
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", srVerbs);

  const srDialogue = {};
  for (const [id, entry] of Object.entries(dialogue)) {
    srDialogue[id] = { de: entry.de, lv: map[entry.lv] || entry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(srDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  for (const [key, html] of Object.entries(course.COURSE_LESSON_HTML || {})) {
    srHtml[key] = await translateHtmlLesson(html, map);
  }
  const srData = applyTranslation(course.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(srHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(srData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Serbian course training cards for SR-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Bs$/, "Sr")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nSR-DE data generation complete (BS → SR).");
}
