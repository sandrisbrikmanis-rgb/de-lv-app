...
"use strict";
/**
 * DA–DE Kurss final closure (FCA) OWNER COPY-ONLY repair apply.
 * Usage: node scripts/apply-da-kurss-final-closure-owner-repair.js [--dry-run]
 */
const fs = require("fs");
...
const REPORT = path.join(ROOT, "reports/da-kurss-final-closure-owner-repair-apply.md");
...
function isDataLegacyHtmlFragmentPath(entryPath) {
  return /^COURSE_LESSON_DATA\.\w+\.legacyHtml$/.test(String(entryPath || ""));
}
...
function isHtmlMapFragmentPath(entryPath) {
  return /^COURSE_LESSON_HTML\.\w+$/.test(String(entryPath || ""));
}

function htmlMapKey(entryPath) {
  return String(entryPath || "").replace(/^COURSE_LESSON_HTML\./, "");
}
    ...
    lesson.sections.forEach((section, si) => {
      section.cards?.forEach((card, ci) => {
        for (const [k, v] of Object.entries(card || {})) {
          if (k === "front" || k === "lv" || k === "title" || k === "subtitle" || k === "intro") continue;
          if (typeof v === "string" && v.trim() && ["back", "de", "infinitive", "du", "ihr", "sie", "prompt", "answer", "base"].includes(k)) {
...
const DE_PROTECTED = /\.(prompt|answer|answer2|de|back|infinitive|du|ihr|sie|base)$/;

function isDeProtectedField(entry, actual) {
  if (!entry.deCurrent || !String(entry.deCurrent).trim()) return false;
  if (!DE_PROTECTED.test(String(entry.path || ""))) return false;
  return currentMatches(actual, entry.deCurrent);
}
...
function renderReport(log, map) {
  return [
    "# DA–DE Kurss — final closure OWNER repair apply",
    "",
    `**Generated:** ${log.generatedAt}`,
    `**Dry run:** ${log.dryRun}`,
    `**Signed file:** ${map.signedFile || "—"}`,
    "",
    "| Metric | Value |",
    "|--------|-------|",
    ...
    `| DE protected (SKIP) | **${log.deProtected.length}** |`,
...
function main() {
  execSync("node scripts/build-da-kurss-final-closure-owner-apply-map.js", {
    cwd: ROOT,
    stdio: "pipe",
  });
  ...
  for (const entry of map.apply) {
    ...
    if (isDeProtectedField(entry, actual)) {
      log.deProtected.push({
        ...record,
        status: "DE_PROTECTED_SKIP",
        reason: "DE field matches deCurrent; DA change blocked",
      });
      continue;
    }
