#!/usr/bin/env node
/**
 * Generate NN-DE data from NB baseline with batched translation (NB → NN).
...
const OUT_DIR = path.join(ROOT, "data", "nn");
const CACHE_PATH = path.join(ROOT, "scripts", ".nn-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "no";
const TO_LANG = "nn";
...
function loadNbTrainingCardsFromData() {
  const cardsPath = path.join(ROOT, "data/nb/courseTrainingCards.js");
  ...
  vm.runInContext(code, ctx);
  const cards = {};
  const arrayPattern = /window\.(lesson\d+(?:Training|Exercise)CardsNb)\s*=\s*(\[[\s\S]*?\n\]);/g;
  let match;
...
async function main() {
  ...
  console.log("Loading NB baseline and LV German source...");
  ...
  const nbA1 = loadData("data/nb/a1.js").A1_WORDS;
  const nbA2 = loadData("data/nb/a2.js").A2_WORDS;
  const nbB1 = loadData("data/nb/b1.js").B1_WORDS;
  const nbB2 = loadData("data/nb/b2.js").B2_WORDS;
  const nbC1 = loadData("data/nb/c1.js").C1_WORDS;
  const nbC2 = loadData("data/nb/c2.js").C2_WORDS;
  const nbSentences = loadData("data/nb/sentences.js").SENTENCE_ENTRIES;
  const nbVerbs = loadData("data/nb/verbs.js").VERB_ENTRIES;
  const nbDialogue = loadData("data/nb/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const nbCourse = loadData("data/nb/courseLessons.js");
  const nbTraining = loadNbTrainingCardsFromData();

  [nbA1, nbA2, nbB1, nbB2, nbC1, nbC2, nbSentences, nbVerbs].forEach((arr) => arr.forEach((e) => collectStrings(e, allStrings)));
  Object.values(nbDialogue).forEach((e) => collectStrings(e, allStrings));
  Object.values(nbCourse.COURSE_LESSON_DATA || {}).forEach((e) => collectStrings(e, allStrings));
  Object.values(nbCourse.COURSE_LESSON_HTML || {}).forEach((html) => {
    [...String(html).matchAll(/>([^<]{3,300})</g)].forEach((m) => {
      const t = m[1].trim();
      if (t) allStrings.add(t);
    });
  });
  Object.values(nbTraining).forEach((deck) => {
  ...
  console.log(`Found ${unique.length} unique NB strings (${alreadyCached} cached)`);
  ...
  function mergeLevel(lvArr, nbArr) {
    return lvArr.map((lvEntry, i) => {
      const nbEntry = nbArr[i];
      if (!nbEntry || lvEntry.de !== nbEntry.de) {
        console.warn(`Order mismatch at ${lvEntry.de}`);
        return lvEntry;
      }
      const trEntry = applyTranslation(nbEntry, map);
      return preserveGermanFields(lvEntry, trEntry);
    });
  }

  console.log("\nWriting NN files...");
  writeArrayFile(path.join(OUT_DIR, "a1.js"), "A1_WORDS", mergeLevel(lvA1, nbA1));
  writeArrayFile(path.join(OUT_DIR, "a2.js"), "A2_WORDS", mergeLevel(lvA2, nbA2));
  writeArrayFile(path.join(OUT_DIR, "b1.js"), "B1_WORDS", mergeLevel(lvB1, nbB1));
  writeArrayFile(path.join(OUT_DIR, "b2.js"), "B2_WORDS", mergeLevel(lvB2, nbB2));
  writeArrayFile(path.join(OUT_DIR, "c1.js"), "C1_WORDS", mergeLevel(lvC1, nbC1));
  writeArrayFile(path.join(OUT_DIR, "c2.js"), "C2_WORDS", mergeLevel(lvC2, nbC2));
  writeArrayFile(path.join(OUT_DIR, "sentences.js"), "SENTENCE_ENTRIES", mergeLevel(lvSentences, nbSentences));

  const nnVerbs = lvVerbs.map((lvEntry, i) => {
    const nbEntry = nbVerbs[i];
    const te = {};
    for (const form of Object.keys(lvEntry)) {
      te[form] = {
        de: lvEntry[form].de,
        lv: map[nbEntry[form].lv] || nbEntry[form].lv,
      };
    }
    return te;
  });
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", nnVerbs);

  const nnDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const nbEntry = nbDialogue[id];
    nnDialogue[id] = { de: lvEntry.de, lv: map[nbEntry.lv] || nbEntry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(nnDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  const nnHtml = {};
  for (const [key, html] of Object.entries(nbCourse.COURSE_LESSON_HTML || {})) {
    nnHtml[key] = await translateHtmlLesson(html, map);
  }
  const nnData = applyTranslation(nbCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(nnHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(nnData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Norwegian Nynorsk course training cards for NN-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(nbTraining)) {
    if (!Array.isArray(deck)) continue;
    const nnKey = key.replace(/Nb$/, "Nn");
    const translated = deck.map((card) => ({
      front: map[card.front] || card.front,
      back: card.back || "",
    }));
    outLines.push(`window.${nnKey} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nNN-DE data generation complete (NB→NN with LV DE preservation).");
}
