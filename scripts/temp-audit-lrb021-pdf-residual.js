...
function validateMergedCard(lang, cardKey, merged) {
  ...
  if (lang === "gr" || lang === "hr") {
    if (LV_LEAK.test(allText)) failures.push({ type: `LV_LEAK_IN_${lang.toUpperCase()}` });
    if (FR_IN_GR.test(allText)) failures.push({ type: `FR_LEAK_IN_${lang.toUpperCase()}` });
  }
  if (lang === "hr" && /[\u0370-\u03FF]/.test(allText)) {
    failures.push({ type: "GR_LEAK_IN_HR" });
  }
  failures.push(
...
const proof = {
  ...
  gates: {
    ...
    LABOT: `${labot}/50`,
    NELABOT: nelabot,
    GR_ROWS: `${rows.filter((r) => r.languages === "gr").length}/26`,
    HR_ROWS: `${rows.filter((r) => r.languages === "hr").length}/24`,
  ...
  productionSource: "data/gr/a1.js + data/hr/a1.js",
