#!/usr/bin/env node
/**
 * Generate TR-DE data from LV baseline with batched unique-string translation.
...
async function main() {
  ...
  console.log("Collecting unique Latvian strings for TR-DE...");
  ...
  for (const [id, entry] of Object.entries(dialogue)) {
    trDialogue[id] = { de: entry.de, lv: map[entry.lv] || entry.lv };
  }
  fs.writeFileSync(
  ...
  const trData = applyTranslation(course.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
  ...
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Lt$/, "Tr").replace(/Lv$/, "Tr")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nTR-DE data generation complete.");
}
