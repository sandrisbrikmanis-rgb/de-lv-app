#!/usr/bin/env node
/**
 * Verify SQ-DE creation compliance:
 * 1. German (de) fields in SQ data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const n = Math.min(lv.length, sq.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const sqFields = collectDeFields(sq[i]);
    for (const lf of lvFields) {
      const tf = sqFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, sq: tf?.value });
      }
    }
  }
}
...
lvSent.forEach((lv, i) => {
  if (sqSent[i] && lv.de !== sqSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, sq: sqSent[i].de });
  }
});
...
lvVerbs.forEach((lvEntry, i) => {
  const sqEntry = sqVerbs[i];
  if (!sqEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== sqEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, sq: sqEntry[form]?.de });
    }
  }
});
