#!/usr/bin/env node
/**
 * Fix RO sectionAccents terms that don't match section text (technical only).
...
function stemMatch(text, term) {
  ...
  const stem = String(term).replace(/(?:are|eze|esc|ește|ești)$/i, '');
...
function inflectionCandidates(term) {
  ...
  const t = String(term);
  // Romanian feminine -ă/-a patterns
  if (t.length >= 3 && (t.endsWith('ă') || t.endsWith('a'))) {
    const stem = t.endsWith('ă') ? t.slice(0, -1) : t.slice(0, -1);
    candidates.push(`${stem}a`, `${stem}ă`, `${stem}ei`, `${stem}ele`, `${stem}ii`, `${stem}ilor`);
  }
  // Romanian masculine -u patterns
  if (t.length >= 3 && t.endsWith('u')) {
    const stem = t.slice(0, -1);
    candidates.push(`${stem}ul`, `${stem}ului`, `${stem}i`, `${stem}ii`);
  }
  // Romanian -e neuter/feminine
  if (t.length >= 3 && t.endsWith('e')) {
    const stem = t.slice(0, -1);
    candidates.push(`${stem}ea`, `${stem}ele`, `${stem}ii`);
  }
  // Romanian verbs -a
  if (t.length >= 3 && t.endsWith('a')) {
    const stem = t.slice(0, -1);
    candidates.push(`${stem}ez`, `${stem}ezi`, `${stem}ează`, `${stem}ăm`, `${stem}ați`, `${stem}are`);
  }
  // Romanian verbs -ea
  if (t.length >= 4 && t.endsWith('ea')) {
    const stem = t.slice(0, -2);
    candidates.push(`${stem}esc`, `${stem}ești`, `${stem}ește`, `${stem}em`, `${stem}eți`);
  }
...
function main() {
  ...
  for (const [file, varName] of RO_WORD_FILES) {
