#!/usr/bin/env node
/**
 * Fix CS data: restore German comparison.word and other DE fields from LV source.
 */
const fs = require("fs");
...
for (const level of LEVELS) {
  ...
  for (let i = 0; i < Math.min(lv.length, cs.length); i++) {
    restoreDeFields(lv[i], cs[i]);
  }
  writeArrayFile(path.join(ROOT, "data/cs", `${level}.js`), VAR_NAMES[level], cs);
  console.log(`Fixed ${level}: ${cs.length} entries`);
}
...
writeArrayFile(path.join(ROOT, "data/cs/sentences.js"), "SENTENCE_ENTRIES", csSent);
...
writeArrayFile(path.join(ROOT, "data/cs/verbs.js"), "VERB_ENTRIES", csVerbs);
...
const csDlgCode = fs.readFileSync(path.join(ROOT, "data/cs/dialogueIdMap.js"), "utf8");
...
fs.writeFileSync(
  path.join(ROOT, "data/cs/dialogueIdMap.js"),
