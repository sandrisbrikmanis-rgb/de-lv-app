...
"use strict";
/**
 * ET A1 residual 2 multi-translation OWNER COPY-ONLY apply (MASTER v1.12).
 */
const fs = require("fs");
...
const ACCEPTED = path.join(ROOT, "reports/et-a1-multitranslation-residual-2-owner-accepted.md");
const UPLOAD = path.join(ROOT, "uploads/et-a1-multitranslation-residual-2-owner-accepted_38c3.md");
const APPLY_LOG = path.join(ROOT, "reports/temp/et-a1-multitranslation-residual-2-apply-log.json");
const REPORT_MD = path.join(ROOT, "reports/et-a1-multitranslation-residual-2-repair-apply.md");
...
function parseAccepted() {
  ...
  for (const line of src.split("\n")) {
    ...
    const cardId = parts[1].replace(/^`|`$/g, "");
    if (!cardId.startsWith("a1-")) continue;
    const status = parts[6];
    if (status !== "LABOT") continue;
    rows.push({
      cardId,
      field: parts[2].replace(/^`|`$/g, ""),
      de: parts[3].replace(/^`|`$/g, ""),
      current: parts[4].replace(/^`|`$/g, ""),
...
function runResidualScan(words) {
...
function main() {
  ...
  if (apply.length !== 2) {
    console.error(`Expected 2 LABOT rows, got ${apply.length}`);
    process.exit(1);
  }
  ...
  let residual = DRY_RUN ? null : runResidualScan(words);
  ...
  if (!DRY_RUN && log.staged.length > 0) {
    ...
    residual = runResidualScan(loadWords(FILES[0]));
  ...
  const residualPass = residual && residual.violations === 0;
  const finalVerdict =
    log.hardFail ||
    (log.failed.length > 0
      ? "FAIL"
  ...
  log.summary = {
    ...
    multiTranslationResidual: residual,
    multipleMainTranslationsValidatedReal: residual?.violations ?? "N/A",
    mainTranslationCountViolations: residual?.violations ?? "N/A",
    finalVerdict,
  };
  ...
  fs.writeFileSync(
    REPORT_MD,
    [
      "# ET A1 — Residual 2 multi-translation OWNER apply",
      "",
      "**MASTER:** v1.12",
      `**Authority:** \`reports/et-a1-multitranslation-residual-2-owner-accepted.md\``,
      ...
      `| MAIN_TRANSLATION_COUNT_VIOLATIONS | **${residual?.violations ?? "N/A"}** |`,
