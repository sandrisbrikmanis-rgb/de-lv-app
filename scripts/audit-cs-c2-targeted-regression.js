...
"use strict";
/**
 * CS-DE C2 Targeted Regression Audit (READ-ONLY)
 * GPT-5.6 Luna — OWNER repair scope only (75 mappings).
 *
 * Usage:
 *   node scripts/audit-cs-c2-targeted-regression.js [--skip-luna] [--resume-luna]
 */
...
const { loadOwnerLabotMappings, parseFieldPath, getAt, parseJsonish, valuesEqual } = require("./lib/cs-c2-owner-mappings");
...
const BRANCH = "cursor/cs-c2-targeted-regression-audit-6ea4";
...
const OUT_JSON = path.join(ROOT, "reports/temp/cs-c2-targeted-regression-audit.json");
const OUT_MD = path.join(ROOT, "reports/cs-c2-targeted-regression.md");
const TEMP_DIR = path.join(ROOT, "reports/temp/cs-c2-targeted-regression-audit");
...
const PROGRESS_FILE = path.join(ROOT, "scripts", ".cs-c2-targeted-luna-progress.json");
const DET_COLLECT_DIR = path.join(ROOT, "reports/temp/cs-c2-audit");

const TARGETED_PROMPT = [
  "You are a targeted CS-DE C2 repair regression auditor (GPT-5.6 Luna).",
  "These cards were changed by OWNER-approved COPY-ONLY C2 repair (75 mappings).",
  "Audit the FULL current production card — lv/csText and study fields if present.",
  "Independently verify whether OWNER-approved NEW Czech is semantically correct vs German source.",
  "Do NOT assume production is correct just because OWNER approved it — report objective Czech errors.",
  "This is NOT a full C2 discovery audit of unchanged cards.",
  ...
  "Pay extra attention to OWNER-refined cases: Baugenossenschaft, Fallschirmspringen, Schlittschuhkufe, Geschwindigkeitskontrolle, Gewerkschaftsbeitrag, Staatsangehörigkeit, Steuererleichterung, instand, in Stand, Sachverständige, gesellschaftlich.",
  "For c2-gesellschaftlich-150 verify NEW quality: Společenský • Sociální vs German gesellschaftlich.",
...
function enrichFindingFromProduction(f, words) {
  ...
  const idx = words.findIndex((e, i) => entryId(e, i, "c2") === f.cardId);
  if (idx < 0) return f;
...
function runDeterministicCollect() {
  ...
  if (!fs.existsSync(outFile)) {
    const result = spawnSync("node", ["scripts/audit-cs-collect.js", "--dataset=c2"], {
      cwd: ROOT,
      encoding: "utf8",
    });
    if (result.stdout) process.stdout.write(result.stdout);
    if (result.stderr) process.stderr.write(result.stderr);
    if (result.status !== 0) throw new Error("audit-cs-collect.js --dataset=c2 failed");
  }
...
function buildLunaCards(targetSet, words) {
  const lv = loadArray("data/c2.js", "C2_WORDS");
  const simple = [];
  const study = [];
  for (let i = 0; i < words.length; i++) {
    const id = entryId(words[i], i, "c2");
    if (!targetSet.has(id)) continue;
    if (words[i].study) study.push(buildStudyCard(lv[i], words[i], i, "c2"));
    else simple.push(buildSimpleCard(lv[i], words[i], i, "c2"));
  }
  return { simple, study };
}
...
async function runTargetedLuna(words, targetSet) {
  ...
  const hashBefore = fileHash(C2_FILE);
  ...
  await auditBatch(simple, BATCH_SIZE, "simple", "c2_targeted_simple");
  await auditBatch(study, STUDY_BATCH_SIZE, "study", "c2_targeted_study");

  if (fileHash(C2_FILE) !== hashBefore) throw new Error("Production changed during Luna audit");
...
function validateFindings(rawFindings, words) {
  const lvWords = loadArray("data/c2.js", "C2_WORDS");
...
function buildMarkdown(data) {
  ...
  const lines = [
    ...
    `| Syntax | ${s.syntax} |`,
    `| ID/order | ${s.idOrder} |`,
    `| Card count | ${s.cardCount}/${C2_TOTAL} |`,
...
async function main() {
  ...
  const productionChanges = fileHash(C2_FILE) !== baselineHash ? 1 : 0;
  const integrity = checkIntegrity(loadArray("data/cs/c2.js", "C2_WORDS"), baselineDeHash);
  ...
  if (integrity.cardCount !== C2_TOTAL) throw new Error("Card count mismatch");
}
