...
"use strict";
/**
 * ET-DE C1/C2 full READ-ONLY audit orchestrator (deterministic + GPT-5.6 Luna).
 * Usage: node scripts/run-et-c1c2-full-audit.js [--skip-luna] [--test-luna] [--fresh-luna]
 * MASTER v1.9: production audit must NOT use --force-baseline (§7.9.4).
 */
...
const {
  ...
  LUNA_JSON,
  TEMP_DIR,
} = require("./lib/et-c1c2-audit-helpers");
const { loadOwnerHistory, classifyWithOwnerHistory } = require("./lib/et-c1c2-owner-history");
...
const {
  ...
  AUDIT_RUNS,
  OWNER_SOURCES,
} = require("./lib/et-c1c2-discovery-config");
...
const OUT_MD = path.join(ROOT, "reports", "et-c1c2-full-audit.md");
const OUT_JSON = path.join(ROOT, "reports", "temp", "et-c1c2-full-audit.json");
const MASTER_VERSION = "1.9";
const PRODUCTION_PATH = "data/et/c1.js";
const WWW_PATH = "www/data/et/c1.js";
const TOTAL_CARDS = 791;
const TOTAL_C1 = 572;
const TOTAL_C2 = 219;
const LV_STUDY_COUNT = 19;
const C2_PRODUCTION_PATH = "data/et/c2.js";
const C2_WWW_PATH = "www/data/et/c2.js";
/** ET-DE C1/C2: no prior FINAL_CLOSED integration on origin/main (first FULL_DISCOVERY). */
...
function computeOwnerHistoryCoverage(ownerHistory, etEntries) {
  ...
  etEntries.forEach((entry, index) => {
    const level = entry.level === "C2" ? "c2" : "c1";
    const id = entry.study?.id || `${level}-${entry.de}-${index}`;
    cardById.set(id, entry);
    cardById.set(`${level}-${entry.de}`, entry);
  });
  let checked = 0;
  let matching = 0;
  let drifted = 0;
...
function computeBaselineGate(ownerHistory) {
  ...
  const c1ProductionSha = git(`git rev-parse origin/main:${PRODUCTION_PATH}`);
  const c1WwwSha = git(`git rev-parse origin/main:${WWW_PATH}`);
  const c2ProductionSha = git(`git rev-parse origin/main:${C2_PRODUCTION_PATH}`);
  const c2WwwSha = git(`git rev-parse origin/main:${C2_WWW_PATH}`);
  const datasetProductionSha = `${c1ProductionSha}+${c2ProductionSha}`;
  const wwwDatasetBlobSha = `${c1WwwSha}+${c2WwwSha}`;
  ...
  if (c1ProductionSha !== c1WwwSha) {
    return {
      ...
      deReadOnly: "PASS",
      blocked: true,
      blockReason: "data/et/c1.js blob !== www/data/et/c1.js blob",
    };
  }

  if (c2ProductionSha !== c2WwwSha) {
    return {
      auditMode: "FULL_DISCOVERY",
      masterVersion: MASTER_VERSION,
      originMainSha,
      ...
      deReadOnly: "PASS",
      blocked: true,
      blockReason: "data/et/c2.js blob !== www/data/et/c2.js blob",
    };
  }
...
function loadProductionWords() {
  const loadLevel = (filePath, globalKey) => {
  ...
  return [
    ...loadLevel(PRODUCTION_PATH, "C1_WORDS"),
    ...loadLevel(C2_PRODUCTION_PATH, "C2_WORDS"),
  ];
}
...
function mergeFindings(detFindings, collectData, lunaData, validateStudy) {
  ...
  for (const filePath of ["data/et/c1.js", "data/et/c2.js"]) {
    const level = filePath.includes("/c2.js") ? "c2" : "c1";
    const levelValidate = validateStudy?.perFile?.find((f) => f.file === filePath);
    for (const issue of levelValidate?.examples?.sectionAccentIssues || []) {
    all.push({
      findingId: `ET-C1C2-${String(seq++).padStart(4, "0")}`,
      source: "validate-study-design",
      cardId: `${level}-${issue.de}`,
...
function buildReport(ctx) {
  ...
  const lines = [];
  lines.push("# ET–DE C1/C2 pilns lingvistiskais audits (MASTER v1.9 FULL_DISCOVERY)");
  lines.push("");
  ...
  lines.push("");
  lines.push("**AUTHORITATIVE STANDARD:** `PROJECT_LANGUAGE_MASTER_STANDARD.md` **v1.9**");
  ...
  lines.push("|---------|---------|");
  lines.push("| Kartītes | **791** |");
  lines.push(`| Luna coverage | **${ctx.lunaCoverage}** |`);
  if (ctx.lunaCoverage === "skipped") {
    lines.push("");
    lines.push("> **Luna (GPT-5.6) nav palaists** (`--skip-luna` vai nav `OPENAI_API_KEY`). Šis ziņojums satur deterministisko slāni. Lai pabeigtu pilnu FULL_DISCOVERY ar Luna: `node scripts/run-et-c1c2-full-audit.js --fresh-luna`.");
  }
...
async function main() {
  if (process.argv.includes("--force-baseline")) {
    console.error("MASTER v1.9 §7.9.4: --force-baseline is prohibited for production FULL_DISCOVERY.");
    console.error("AUDIT_VALIDITY = INVALID · VALID_FOR_REPAIR = NO · DIAGNOSTIC_ONLY");
    process.exit(3);
  }

  console.log("\n=== ET–DE C1/C2 FULL_DISCOVERY — MASTER v1.9 ===\n");
  ...
  if (FRESH_LUNA) {
    const progress = path.join(ROOT, "scripts/.et-c1c2-luna-progress.json");
    if (fs.existsSync(progress)) fs.unlinkSync(progress);
    if (fs.existsSync(LUNA_JSON)) fs.unlinkSync(LUNA_JSON);
    if (fs.existsSync(TEMP_DIR)) fs.rmSync(TEMP_DIR, { recursive: true, force: true });
  }

  run("node scripts/audit-et-c1c2-collect.js");
  runCapture(["audit-language-parity.js", "--lang=et"], path.join(ROOT, "reports/temp/et-c1c2-parity.json"));
  const moj = spawnSync("node", [path.join(ROOT, "scripts", "audit-mojibake.js"), "--lang=et"], {
    cwd: ROOT,
    env: process.env,
    encoding: "utf8",
  });
  fs.writeFileSync(path.join(ROOT, "reports/temp/et-c1c2-mojibake.json"), moj.stdout || "");
  runCapture(["validate-study-design.js", "--lang=et"], path.join(ROOT, "reports/temp/et-c1c2-validate-study.json"));
  run("node --check data/et/c1.js");
  run("node --check data/et/c2.js");

  const collectData = loadJsonSafe(path.join(ROOT, "reports/temp/et-c1c2-audit-data.json"), {});
  const validateStudy = loadJsonSafe(path.join(ROOT, "reports/temp/et-c1c2-validate-study.json"), {});
  ...
  if (!SKIP_LUNA) {
    ...
    runNode("audit-et-c1c2-linguistic.js", lunaArgs);
  } else {
    console.log("\n=== Luna SKIPPED (--skip-luna) ===\n");
  }

  const lunaData = loadJsonSafe(LUNA_JSON, { findings: [], qualityFindings: [] });
  const detFindings = deterministicStructuralFindings();
  ...
  const c1Validate = validateStudy?.perFile?.find((f) => f.file === "data/et/c1.js");
  const c2Validate = validateStudy?.perFile?.find((f) => f.file === "data/et/c2.js");
  const sectionAccentCount =
    (c1Validate?.sectionAccentIssues || 0) + (c2Validate?.sectionAccentIssues || 0);
  ...
  fs.writeFileSync(path.join(ROOT, "reports/et-c1c2-full-audit.json"), JSON.stringify(payload, null, 2));
  fs.writeFileSync(OUT_MD, buildReport(ctx));

  if (discoveryStability.gates.ownerBacklogAllowed && ownerBacklogFinal.length > 0) {
    fs.writeFileSync(
      path.join(ROOT, "reports/temp/et-c1c2-full-audit.json"),
  ...
  if (ownerBacklogFinal.length > 0 && discoveryStability.gates.ownerBacklogAllowed) {
    runPostAuditOwnerReview("et-c1c2-full", { force: true });
  }
}
