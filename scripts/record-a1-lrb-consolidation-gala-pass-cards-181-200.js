...
const PASS_CLASSIFICATION = "A1_LRB_CONSOLIDATION_CARDS_181_200_GALA_PASS";
const PASS_PROOF_PATH = path.join(
  OUT_DIR,
  "A1-LRB-CONSOLIDATION-CARDS-181-200-GALA-PASS-PROOF.json"
);
...
function main() {
  execSync("node scripts/verify-a1-lrb-consolidation-gala-correction-1-cards-181-200.js", {
    cwd: ROOT,
    stdio: "pipe",
  });
  ...
  const scopeCards = copyPaste.cards.slice(180, 200);
  ...
  const gates = runGalaCorrection181200Gates({
    scopeCards,
    appliedByKey,
  ...
  const cards181200Sha256 = sha256(JSON.stringify(cardsPayload));
  ...
  const proof = {
    ...
    cards_181_200_payload_sha256: cards181200Sha256,
  ...
  copyPaste.cards_181_200_gala_pass = {
    classification: PASS_CLASSIFICATION,
  ...
  const summaryLines = [
    "A1 LRB konsolidācija — kartīšu 181–200 GALA PASS",
    "",
    ...
    "| Precīzi ievietoti labojumi | 2/2 |",
    "| Nemainītas NELABOT kartītes | 18/18 |",
    ...
    "| Dublikāti vai pretrunas | 0 |",
    "| Shēmas/sectionAccents kļūdas | 0 |",
    "",
    `Git HEAD: ${head}`,
    `Cards 181–200 payload SHA-256: ${cards181200Sha256}`,
    `Pass proof SHA-256: ${sha256(fs.readFileSync(PASS_PROOF_PATH))}`,
  ];
  fs.writeFileSync(
    path.join(OUT_DIR, "A1-LRB-CONSOLIDATION-CARDS-181-200-GALA-PASS.txt"),
    summaryLines.join("\n") + "\n"
  );
