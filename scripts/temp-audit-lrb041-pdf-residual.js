...
const IS_CARD_ALIASES = {
  "a1-heissen": "a1-heißen",
  "a1-koennen": "a1-können",
  "a1-natuerlich": "a1-natürlich",
  "a1-ueber": "a1-über",
};
...
function nestedForCard(lang, cardKey) {
  ...
  const resolved = IS_CARD_ALIASES[cardKey] || cardKey;
  return (
    map[resolved] ||
    map[cardKey] ||
    map[`a1-${resolved}`] ||
...
function validateMergedCard(lang, cardKey, merged) {
  ...
  if (lang === "is") {
    if (LV_LEAK.test(allText)) failures.push({ type: "LV_LEAK_IN_IS" });
    if (ET_LEAK.test(allText)) failures.push({ type: "ET_LEAK_IN_IS" });
    if (HU_IN_IS.test(allText)) failures.push({ type: "HU_LEAK_IN_IS" });
    if (NO_IN_IS.test(allText)) failures.push({ type: "NO_LEAK_IN_IS" });
  }
  failures.push(
...
if (labot !== EXPECTED_ROWS) semanticViolations++;
...
const proof = {
  batch_id: BATCH,
  classification: pass
    ? "LRB_041_OWNER_PREP_READY_FOR_LINGUISTIC_REVIEW"
    : "LRB_041_OWNER_PREP_BLOCKED",
  pdf_reaudit: true,
  ...
  is_rows: rows.filter((r) => r.languages === "is").length,
  gates: {
    ROWS: `${rows.length}/${EXPECTED_ROWS}`,
    PENDING: pending,
    LABOT: `${labot}/${EXPECTED_ROWS}`,
    NELABOT: nelabot,
    IS_ROWS: `${rows.filter((r) => r.languages === "is").length}/${EXPECTED_ROWS}`,
  ...
  productionSource: "data/is/a1.js",
  languages: { is: EXPECTED_ROWS },
