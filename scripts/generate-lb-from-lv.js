#!/usr/bin/env node
/**
 * Generate LB-DE data from LV baseline (DE READ-ONLY).
 * Translates only native-language (lv) fields LV → LB.
...
const OUT_DIR = path.join(ROOT, "data", "lb");
const CACHE_PATH = path.join(ROOT, "scripts", ".lb-lv-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "lv";
const TO_LANG = "lb";
...
async function main() {
  ...
  console.log("Collecting unique Latvian strings from LV-DE for LB translation...");
  ...
  for (const [id, entry] of Object.entries(dialogue)) {
    lbDialogue[id] = { de: entry.de, lv: map[entry.lv] || entry.lv };
  }
  fs.writeFileSync(
  ...
  const lbData = applyTranslation(course.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
  ...
  const outLines = ["// Luxembourgish course training cards for LB-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Lt$/, "Lb")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nLB-DE data generation complete (LV → LB).");
}
