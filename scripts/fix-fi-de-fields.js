#!/usr/bin/env node
/**
 * Fix FI data: restore German comparison.word and other DE fields from LV source.
 */
const fs = require("fs");
...
for (const level of LEVELS) {
  ...
  for (let i = 0; i < Math.min(lv.length, fi.length); i++) {
    restoreDeFields(lv[i], fi[i]);
  }
  writeArrayFile(path.join(ROOT, "data/fi", `${level}.js`), VAR_NAMES[level], fi);
  console.log(`Fixed ${level}: ${fi.length} entries`);
}
...
writeArrayFile(path.join(ROOT, "data/fi/sentences.js"), "SENTENCE_ENTRIES", csSent);
...
writeArrayFile(path.join(ROOT, "data/fi/verbs.js"), "VERB_ENTRIES", csVerbs);
...
const csDlgCode = fs.readFileSync(path.join(ROOT, "data/fi/dialogueIdMap.js"), "utf8");
...
fs.writeFileSync(
  path.join(ROOT, "data/fi/dialogueIdMap.js"),
