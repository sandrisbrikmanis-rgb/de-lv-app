...
"use strict";
/**
 * DA–DE Verbs final post-repair OWNER COPY-ONLY repair apply.
 * Usage: node scripts/apply-da-verbs-final-post-repair-owner-repair.js [--dry-run]
 */
const fs = require("fs");
...
const APPLY_MAP = path.join(ROOT, "reports/temp/da-verbs-final-post-repair-owner-apply-map.json");
const APPLY_LOG = path.join(ROOT, "reports/temp/da-verbs-final-post-repair-owner-apply-log.json");
const REPORT = path.join(ROOT, "reports/da-verbs-final-post-repair-owner-repair-apply.md");
...
function main() {
  execSync("node scripts/build-da-verbs-final-post-repair-owner-apply-map.js", {
    cwd: ROOT,
    stdio: "pipe",
  });
  ...
  const log = { dryRun: DRY_RUN, applied: [], skipped: [], failed: [], unchanged: [] };
  ...
  for (const row of applyMap.apply || []) {
    ...
    if (normalizeCompare(actualCurrent) === normalizeCompare(row.ownerNew)) {
      log.unchanged.push({
        auditId: row.auditId,
        cardId: row.cardId,
        field: formKey,
  ...
  const md = [
    "# DA–DE Verbs final post-repair OWNER repair apply",
    "",
    ...
    "| Metric | Value |",
    "|--------|-------|",
    `| Signed rows | **${applyMap.totalRows || 0}** |`,
    `| Applied | **${log.summary.applied}** |`,
    `| Unchanged (already correct) | **${log.summary.unchanged}** |`,
    `| Skipped (CURRENT mismatch) | **${log.summary.skipped}** |`,
  ...
  if (log.skipped.length) {
    ...
    for (const s of log.skipped) {
      md.push(`- \`${s.auditId}\` \`${s.cardId}.${s.field}\`: expected \`${s.expected}\`, actual \`${s.actual}\``);
    }
    md.push("");
  }
  ...
  if (log.failed.length) {
    log.failed.forEach((f) => console.error(JSON.stringify(f)));
    process.exit(1);
  }
  if (log.applied.length === 0 && log.unchanged.length === 0) process.exit(1);
}
