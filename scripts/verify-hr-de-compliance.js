#!/usr/bin/env node
/**
 * Verify HR-DE creation compliance:
 * 1. German (de) fields in HR data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const n = Math.min(lv.length, hr.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const hrFields = collectDeFields(hr[i]);
    for (const lf of lvFields) {
      const tf = hrFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, hr: tf?.value });
      }
    }
  }
}
...
lvSent.forEach((lv, i) => {
  if (hrSent[i] && lv.de !== hrSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, hr: hrSent[i].de });
  }
});
...
lvVerbs.forEach((lvEntry, i) => {
  const hrEntry = hrVerbs[i];
  if (!hrEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== hrEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, hr: hrEntry[form]?.de });
    }
  }
});
...
try {
  const diff = execSync(
    "git diff --name-only HEAD -- languages/lv languages/lt languages/ru languages/pl languages/uk languages/et languages/ro languages/bg languages/gr languages/tr languages/sq languages/mk languages/sl languages/bs languages/sr data/lv data/lt data/ru data/pl data/uk data/et data/ro data/bg data/gr data/tr data/sq data/mk data/sl data/bs data/sr data/a1.js data/a2.js data/b1.js data/b2.js data/c1.js data/c2.js data/sentences.js data/verbs.js data/courseLessons.js",
