#!/usr/bin/env node
/**
 * Fix RU-DE sectionAccents highlight terms that don't match section text.
 ...
 *
 * Run: node scripts/fix-ru-section-accents.js [--dry-run]
 */
const fs = require("fs");
...
const RU_FILES = [
  ["data/ru/a1.js", "A1_WORDS"],
  ["data/ru/a2.js", "A2_WORDS"],
  ["data/ru/b1.js", "B1_WORDS"],
  ["data/ru/b2.js", "B2_WORDS"],
  ["data/ru/c1.js", "C1_WORDS"],
  ["data/ru/c2.js", "C2_WORDS"],
  ["data/ru/sentences.js", "SENTENCE_ENTRIES"],
];
...
function ruInflectionCandidates(term) {
  const candidates = [String(term)];
  const t = String(term);
  const stripSuffixes = [
    'ться', 'тся', 'ение', 'ения', 'ениями', 'ениях', 'остью', 'ость', 'ости',
    'ами', 'ями', 'ах', 'ях', 'ов', 'ей', 'ий', 'ый', 'ого', 'ому', 'ыми', 'ими',
    'ою', 'ею', 'ть', 'ла', 'ло', 'ли', 'л', 'а', 'я', 'е', 'ё', 'и', 'ы', 'у', 'ю', 'о',
  ];
  const addSuffixes = [
    'ть', 'ться', 'л', 'ла', 'ло', 'ли', 'ет', 'ют', 'ешь', 'ете', 'ем',
    'а', 'ы', 'и', 'у', 'ю', 'ов', 'ам', 'ах', 'ом', 'е', 'я', 'ей', 'ий', 'ого', 'ому',
  ];
  for (const suf of stripSuffixes) {
    if (t.length - suf.length >= 3 && t.endsWith(suf)) {
      const stem = t.slice(0, -suf.length);
      for (const add of addSuffixes) candidates.push(stem + add);
    }
  }
...
function findReplacementInTexts(texts, term) {
  ...
  for (const candidate of [...ruInflectionCandidates(term), ...germanStemCandidates(term)]) {
    for (const text of texts) {
...
const stats = { total: 0, autoFixed: 0, dropped: 0, reshaped: 0, cardsTouched: 0 };
const touchedCards = new Set();
const unresolvedSample = [];

for (const [relPath, varName] of RU_FILES) {
  ...
  for (const card of file.arr) {
    ...
    const saBefore = JSON.stringify(study.sectionAccents);
    ...
    if (JSON.stringify(study.sectionAccents) !== saBefore) {
      touchedCards.add(`${relPath}:${card.id || card.de}`);
      stats.cardsTouched++;
    }
  }
...
fs.writeFileSync(path.join(ROOT, "audits_and_reports/uk-section-accents-fix-unresolved.json"), JSON.stringify(unresolvedSample, null, 2));
