#!/usr/bin/env node
/**
 * Fix IS data: restore German comparison.word and other DE fields from LV source.
 */
const fs = require("fs");
...
for (const level of LEVELS) {
  ...
  for (let i = 0; i < Math.min(lv.length, is.length); i++) {
    restoreDeFields(lv[i], is[i]);
  }
  writeArrayFile(path.join(ROOT, "data/is", `${level}.js`), VAR_NAMES[level], is);
  console.log(`Fixed ${level}: ${is.length} entries`);
}

const lvSent = loadArray("data/sentences.js");
const csSent = loadArray("data/is/sentences.js");
lvSent.forEach((t, i) => { if (csSent[i]) csSent[i].de = t.de; });
writeArrayFile(path.join(ROOT, "data/is/sentences.js"), "SENTENCE_ENTRIES", csSent);
console.log(`Fixed sentences: ${csSent.length}`);

const lvVerbs = loadArray("data/verbs.js");
const csVerbs = loadArray("data/is/verbs.js");
lvVerbs.forEach((t, i) => {
  if (!csVerbs[i]) return;
...
writeArrayFile(path.join(ROOT, "data/is/verbs.js"), "VERB_ENTRIES", csVerbs);
...
const csDlgCode = fs.readFileSync(path.join(ROOT, "data/is/dialogueIdMap.js"), "utf8");
...
fs.writeFileSync(
  path.join(ROOT, "data/is/dialogueIdMap.js"),
