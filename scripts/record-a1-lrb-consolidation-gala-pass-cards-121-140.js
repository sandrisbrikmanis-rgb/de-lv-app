...
const PASS_CLASSIFICATION =
  "A1_LRB_CONSOLIDATION_CARDS_121_140_FULL_LINGUISTIC_REVIEW_PASS";
const PASS_PROOF_PATH = path.join(
  OUT_DIR,
  "A1-LRB-CONSOLIDATION-CARDS-121-140-GALA-PASS-PROOF.json"
);
...
function main() {
  execSync("node scripts/verify-a1-lrb-consolidation-gala-correction-1-cards-121-140.js", {
    cwd: ROOT,
    stdio: "pipe",
  });
  ...
  const cards121140Sha256 = sha256(JSON.stringify(cardsPayload));
  ...
  const proof = {
    ...
    cards_121_140_payload_sha256: cards121140Sha256,
  ...
  copyPaste.cards_121_140_gala_pass = {
    classification: PASS_CLASSIFICATION,
  ...
  const summaryLines = [
    "A1 LRB konsolidācija — kartīšu 121–140 GALA PASS",
    "",
    ...
    "| Precīzi ievietotie labojumi | 3/3 |",
    "| Nemainītās NELABOT kartītes | 17/17 |",
    "| Pilno kartīšu pilnīgums | PASS |",
    "",
    `Git HEAD: ${head}`,
    `Cards 121–140 payload SHA-256: ${cards121140Sha256}`,
    `Pass proof SHA-256: ${sha256(fs.readFileSync(PASS_PROOF_PATH))}`,
  ];
  fs.writeFileSync(
    path.join(OUT_DIR, "A1-LRB-CONSOLIDATION-CARDS-121-140-GALA-PASS.txt"),
    summaryLines.join("\n") + "\n"
  );
