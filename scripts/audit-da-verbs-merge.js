...
"use strict";
/**
 * Merge deterministic + Luna findings for DA-DE Verbs audit.
 */
const fs = require("fs");
const path = require("path");
const { ROOT } = require("./lib/audit-common");

const COLLECT = path.join(ROOT, "reports/temp/da-verbs-audit-data.json");
const LUNA_DIR = path.join(ROOT, "reports/temp/da-verbs-luna");
const OUT = path.join(ROOT, "reports/temp/da-verbs-merged-audit.json");
...
function normalizeFinding(f, source) {
  return {
    ...
    currentDa: f.daCurrent || f.currentDa || "",
    proposedDa: f.proposedDa || (f.severity === "NEEDS_SOURCE_REVIEW" ? "—" : ""),
...
function main() {
  ...
  merged = merged.map((f, i) => ({ ...f, id: `DA-VERB-${String(i + 1).padStart(4, "0")}` }));
