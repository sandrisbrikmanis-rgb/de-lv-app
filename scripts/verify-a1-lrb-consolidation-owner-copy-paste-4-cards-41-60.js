...
const EXPECTED_OWNER_SHA = "d394d3295719c1ba802756e00f9de0a6284557fb71965ca2d3cf2e62774eea39";
...
function main() {
  ...
  const ownerRaw = fs.existsSync(OWNER_BUNDLE) ? fs.readFileSync(OWNER_BUNDLE) : Buffer.alloc(0);
  if (ownerRaw.length && sha256(ownerRaw) !== EXPECTED_OWNER_SHA) {
  ...
  const out = {
    ...
    blockers,
    git_head: head,
    owner_bundle_sha256: ownerRaw.length ? sha256(ownerRaw) : null,
    proof_sha256: proof ? sha256(fs.readFileSync(PROOF_PATH)) : null,
    gates,
    classification: PROOF_CLASSIFICATION,
    checks_run: [
      "verify-a1-lrb-consolidation-owner-copy-paste-4-cards-41-60.js",
