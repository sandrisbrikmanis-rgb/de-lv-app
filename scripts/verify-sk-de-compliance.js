#!/usr/bin/env node
/**
 * Verify SK-DE creation compliance:
...
for (const level of LEVELS) {
  ...
  const n = Math.min(lv.length, sk.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const skFields = collectDeFields(sk[i]);
    for (const lf of lvFields) {
      const tf = skFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, sk: tf?.value });
      }
    }
  }
}
...
lvSent.forEach((lv, i) => {
  if (skSent[i] && lv.de !== skSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, sk: skSent[i].de });
  }
});
...
lvVerbs.forEach((lvEntry, i) => {
  ...
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== skEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, sk: skEntry[form]?.de });
    }
  }
});
