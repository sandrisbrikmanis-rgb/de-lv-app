...
"use strict";
/**
 * CS-DE A2 Final Closure Repair V3 Groups 01–03 — COPY-ONLY apply.
 */
const fs = require("fs");
...
const EXPECTED_COUNTS = { "01": 50, "02": 50, "03": 15 };
const TOTAL = 115;
const BRANCH = "cursor/cs-a2-final-closure-repair-v3-groups01-03-6ea4";
const REPORT_MD = path.join(ROOT, "reports/cs-a2-final-closure-repair-v3-applied.md");
const REPORT_JSON = path.join(ROOT, "reports/temp/cs-a2-final-closure-repair-v3-applied.json");
...
function loadAllSpecs() {
  ...
  for (const g of GROUPS) {
    const specPath = path.join(__dirname, `cs-a2-final-closure-repair-v3-group${g}-spec.json`);
    ...
    specs.push({ group: g, specPath: `scripts/cs-a2-final-closure-repair-v3-group${g}-spec.json`, cards: spec.cards });
  }
...
function generateReportMd(data) {
  ...
  const lines = [
    "# CS–DE A2 FINAL CLOSURE REPAIR V3 — APPLIED",
    "",
    `Branch: \`${BRANCH}\``,
  ...
  lines.push(s.overall === "PASS"
    ? "**FINAL CLOSURE REPAIR V3 APPLIED — READY FOR GPT-5.6 LUNA 1640/1640 READ-ONLY CLOSURE AUDIT**"
