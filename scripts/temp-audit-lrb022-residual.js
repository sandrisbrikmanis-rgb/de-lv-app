...
for (const row of rows) {
  ...
  if (lang === "hr" && LV_LEAK.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_LV_IN_HR", msg: allText.slice(0, 100) });
  }
  if (lang === "hr" && FR_IN_HR.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_FR_IN_HR", msg: allText.slice(0, 100) });
  }
  if (lang === "hr" && /[\u0370-\u03FF]/.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_GR_IN_HR", msg: allText.slice(0, 100) });
  }
  if (lang === "hr" && BG_MK_SL.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_BG_MK_SL_IN_HR", msg: allText.slice(0, 100) });
  }
