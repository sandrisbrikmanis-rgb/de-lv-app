...
function resolveCardDecision(mapping, cardObjectId) {
  if (mapping.nelabot && cardObjectId in mapping.nelabot) {
    const nelabotNote =
      ...
      `LRB-067 OWNER mapping NELABOT: preserve production for ${cardObjectId}.`;
    return {
  ...
  if (mapping.labot_full_composite && cardObjectId in mapping.labot_full_composite) {
    ...
    return {
      ...
      owner_note: `LRB-067 OWNER mapping full_card_target_fields: ${cardObjectId}.`,
    };
  }
  if (mapping.labot_scalar && cardObjectId in mapping.labot_scalar) {
    return {
      ...
      replacement_scope: "finding_field_only",
      owner_note: `LRB-067 OWNER mapping scalar lv: ${cardObjectId}.`,
    };
  }
  return null;
}
...
function main() {
  ...
  const expectedInputSha =
    "77a957543111ade0a69284997245a73a9e57681b09cc50cfaa9194f31dab4113";
  ...
  if (fullCardIds.size !== 10) {
    throw new Error(`Expected 10 full composites, got ${fullCardIds.size}`);
  }
