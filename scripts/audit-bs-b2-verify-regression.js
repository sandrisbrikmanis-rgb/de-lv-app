#!/usr/bin/env node
/**
 * BS-DE B2 verify regression audit (42 cards, read-only).
 * Model: GPT-5.6 Luna. No data changes.
 *
 * Usage: node scripts/audit-bs-b2-verify-regression.js [--test-batch]
 */
const fs = require("fs");
...
const SCOPE_PATH = path.join(ROOT, "reports", "temp", "bs-b2-regression-fix-verify-scope.json");
const APPLY_LOG_PATH = path.join(ROOT, "reports", "temp", "bs-b2-regression-fixes-apply-log.json");
const OUT_JSON = path.join(ROOT, "reports", "temp", "bs-b2-verify-regression.json");
const STATS_PATH = path.join(ROOT, "scripts", ".bs-b2-verify-regression-audit-stats.json");
const PROGRESS_PATH = path.join(ROOT, "scripts", ".bs-b2-verify-regression-audit-progress.json");
...
const SIMPLE_BATCH = 12;
const STUDY_BATCH = 6;
const MAX_RETRIES = 3;
const EXPECTED_SCOPE = 42;

const SPECIAL_CARDS = new Set(["b2-sich-einpraegen", "b2-nachdruck"]);
const ACCENT_CARDS = new Set([
...
function verifyPriority(cardId) {
  if (SPECIAL_CARDS.has(cardId)) return "SPECIAL_CHECK";
...
function validateScope(scopeIds, bs) {
  ...
  return {
    ...
    missingIds: missing,
    duplicateIds,
    valid: scopeIds.size === EXPECTED_SCOPE && missing.length === 0 && duplicateIds === 0,
  };
}
...
function runPreCheck(bs) {
  ...
  try {
    ...
    checks.sectionAccentsTechnical = JSON.parse(line || "{}").sectionAccentsTechnical ?? -1;
  } catch { /* */ }
...
async function auditBatchWithRetry(cards, stats, batchKey) {
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      return await auditCardsBatch({ cards, stats, batchLabel: batchKey, auditType: "verify_regression" });
    } catch (error) {
...
async function main() {
  ...
  if (!scopeCheck.valid) {
    console.error("INVALID VERIFY SCOPE:", scopeCheck);
    process.exit(1);
  }
  ...
  console.log(`B2 verify regression: ${scopeIds.size} cards (${simpleCards.length} simple, ${studyCards.length} study)`);

  const runBatches = async (cards, batchSize, prefix) => {
    const batches = chunk(cards, batchSize);
    const limit = TEST_BATCH ? 1 : batches.length;
    for (let i = 0; i < limit; i++) {
      const batchKey = `${prefix}-${i}`;
      if (completed.has(batchKey)) { console.log(`  skip ${batchKey}`); continue; }
      ...
      auditData.batches.push({
        key: batchKey, cardCount: batches[i].length,
        cardIds: batches[i].map((c) => c.cardId),
        findingsCount: result.findings.length, passCount: result.passCount,
  ...
  await runBatches(simpleCards, SIMPLE_BATCH, "simple");
  if (!TEST_BATCH || studyCards.length > 0) {
    await runBatches(studyCards, STUDY_BATCH, "study");
  }
  ...
  for (const cardId of [...SPECIAL_CARDS, ...ACCENT_CARDS]) {
    ...
    specialResults[cardId] = {
      verdict: pass ? "PASS" : (cardFindings[0]?.verdict || cardFindings[0]?.status || "FIX"),
      findings: cardFindings,
      rektion: entry?.study?.rektion || null,
      forms: entry?.study?.forms || null,
      exampleLv: entry?.study?.examples?.[0]?.lv || null,
      inScope: scopeIds.has(cardId),
    };
  }

  const accentInScope = [...ACCENT_CARDS].filter((id) => scopeIds.has(id));
  ...
  auditData.specialResults = specialResults;
  auditData.accentCardsInScope = accentInScope.length;
  auditData.auditedCardIds = [...auditedCardIds];
  saveAuditJson(auditData);
