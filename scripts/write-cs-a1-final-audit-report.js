...
const { ROOT, finalPostRepairPaths, auditPathsForA1 } = require("./lib/cs-audit-helpers");
...
function main() {
  ...
  const auditTitle = paths.auditType === "FINAL AUDIT ON MAIN"
  ...
  const repairRetentionFile = loadJson(paths.repairRetentionJson) || {};
  const mainSha = loadJson(paths.mainShaManifest) || {};
  const repair = validated.repairRetention || repairRetentionFile || {};
...
| owner_keep | ${repair.ownerKeep || 0} |
| conflicting | ${repair.conflicting || 0} |
| card_not_found | ${repair.cardNotFound || 0} |
...
- Main SHA: ${mainSha.ORIGIN_MAIN_SHA || mainSha.AUDIT_MAIN_SHA || "—"}
