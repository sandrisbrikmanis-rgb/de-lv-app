...
"use strict";
/**
 * ET C1 multi-translation OWNER review (MASTER v1.12) — analysis only, no repair.
 */
const fs = require("fs");
...
const OUT_VIEW = path.join(ROOT, "reports/et-c1-multitranslation-owner-view.md");
const OUT_DEC = path.join(ROOT, "reports/et-c1-multitranslation-owner-decisions.md");
const OUT_JSON = path.join(ROOT, "reports/temp/et-c1-multitranslation-owner-review.json");

const EXPECTED_CARDS = 572;
...
function germanSense(de, candidate) {
  return `Learner-facing Estonian sense for German «${de}» at C1 level`;
}

function candidateRelation(de, candidates) {
  if (candidates.length === 2) {
    return "Two distinct C1-level senses — evaluate which is primary for the learner-facing main field.";
  }
...
function recommendationJustification(de, candidates, recommended, entry, field) {
  ...
  const studyHint = study
    ...
    : " Ordinary card — pick the most frequent practical C1 gloss for flashcard front.";
  return `Recommended **${recommended}** as the primary C1 learner-facing gloss (${gs}). ${rel}${studyHint}`;
}
...
function suitability(de, candidate, candidates, entry, field, recommended) {
  const c = candidate.trim();
  if (c === recommended) return "Strong candidate for single main translation at C1.";
...
function build() {
  ...
  for (let i = 0; i < findings.length; i++) {
    ...
    const auditId = `ET-C1-MT-${String(i + 1).padStart(4, "0")}`;
  ...
  const productionBlob = execSync("git hash-object data/et/c1.js", { cwd: ROOT, encoding: "utf8" }).trim();
  ...
  const view = [
    "# ET–DE C1 — Multi-translation OWNER VIEW",
    "",
    `**Generated:** ${generatedAt}`,
    `**MASTER:** v1.12`,
    `**ORIGIN_MAIN_SHA:** \`${originSha}\``,
    `**C1_PRODUCTION_BLOB:** \`${productionBlob}\``,
    ...
    "## Verdict",
    "",
    "**ET_C1_MULTITRANSLATION_OWNER_REVIEW_READY**",
    "",
  ];
  ...
  const jsonPayload = {
    ...
    productionBlob,
    verdict: "ET_C1_MULTITRANSLATION_OWNER_REVIEW_READY",
