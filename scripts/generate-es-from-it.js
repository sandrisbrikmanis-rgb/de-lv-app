#!/usr/bin/env node
/**
 * Generate ES-DE data from IT baseline with batched translation (IT → ES).
 * Uses Google gtx endpoint (more reliable than google-translate-api-x in cloud).
...
const OUT_DIR = path.join(ROOT, "data", "es");
const CACHE_PATH = path.join(ROOT, "scripts", ".es-it-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "it";
const TO_LANG = "es";
...
const NAME_REPLACEMENTS = [
  ...
  [/\bMaria\b/g, "María"], [/\bMarie\b/g, "María"],
...
async function translateAllGtx(strings, from, to, options = {}) {
  const { cachePath, delayMs = 100, concurrency = 8, onProgress } = options;
...
function loadItTrainingCardsFromData() {
  const cardsPath = path.join(ROOT, "data/it/courseTrainingCards.js");
  if (!fs.existsSync(cardsPath)) return {};
  const code = fs.readFileSync(cardsPath, "utf8");
  const cards = {};
  const arrayPattern = /window\.(lesson\d+(?:Training|Exercise)CardsIt)\s*=\s*(\[[\s\S]*?\n\]);/g;
  let match;
...
async function main() {
  ...
  console.log("Loading IT baseline and LV German source...");
  ...
  const itA1 = loadData("data/it/a1.js").A1_WORDS;
  const itA2 = loadData("data/it/a2.js").A2_WORDS;
  const itB1 = loadData("data/it/b1.js").B1_WORDS;
  const itB2 = loadData("data/it/b2.js").B2_WORDS;
  const itC1 = loadData("data/it/c1.js").C1_WORDS;
  const itC2 = loadData("data/it/c2.js").C2_WORDS;
  const itSentences = loadData("data/it/sentences.js").SENTENCE_ENTRIES;
  const itVerbs = loadData("data/it/verbs.js").VERB_ENTRIES;
  const itDialogue = loadData("data/it/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const itCourse = loadData("data/it/courseLessons.js");
  const itTraining = loadItTrainingCardsFromData();

  [itA1, itA2, itB1, itB2, itC1, itC2, itSentences, itVerbs].forEach((arr) => arr.forEach((e) => collectStrings(e, allStrings)));
  Object.values(itDialogue).forEach((e) => collectStrings(e, allStrings));
  Object.values(itCourse.COURSE_LESSON_DATA || {}).forEach((e) => collectStrings(e, allStrings));
  Object.values(itCourse.COURSE_LESSON_HTML || {}).forEach((html) => {
    [...String(html).matchAll(/>([^<]{3,300})</g)].forEach((m) => {
      const t = m[1].trim();
      if (t) allStrings.add(t);
    });
  });
  Object.values(itTraining).forEach((deck) => {
  ...
  console.log(`Found ${unique.length} unique IT strings (${alreadyCached} cached)`);
  ...
  function mergeLevel(lvArr, itArr) {
    return lvArr.map((lvEntry, i) => {
      const itEntry = itArr[i];
      if (!itEntry || lvEntry.de !== itEntry.de) {
        console.warn(`Order mismatch at ${lvEntry.de}`);
        return lvEntry;
      }
      const trEntry = applyTranslation(itEntry, map);
      return preserveGermanFields(lvEntry, trEntry);
    });
  }

  console.log("\nWriting ES files...");
  writeArrayFile(path.join(OUT_DIR, "a1.js"), "A1_WORDS", mergeLevel(lvA1, itA1));
  writeArrayFile(path.join(OUT_DIR, "a2.js"), "A2_WORDS", mergeLevel(lvA2, itA2));
  writeArrayFile(path.join(OUT_DIR, "b1.js"), "B1_WORDS", mergeLevel(lvB1, itB1));
  writeArrayFile(path.join(OUT_DIR, "b2.js"), "B2_WORDS", mergeLevel(lvB2, itB2));
  writeArrayFile(path.join(OUT_DIR, "c1.js"), "C1_WORDS", mergeLevel(lvC1, itC1));
  writeArrayFile(path.join(OUT_DIR, "c2.js"), "C2_WORDS", mergeLevel(lvC2, itC2));
  writeArrayFile(path.join(OUT_DIR, "sentences.js"), "SENTENCE_ENTRIES", mergeLevel(lvSentences, itSentences));

  const esVerbs = lvVerbs.map((lvEntry, i) => {
    const itEntry = itVerbs[i];
    const te = {};
    for (const form of Object.keys(lvEntry)) {
      te[form] = {
        de: lvEntry[form].de,
        lv: map[itEntry[form].lv] || itEntry[form].lv,
      };
    }
    return te;
  });
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", esVerbs);

  const esDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const itEntry = itDialogue[id];
    esDialogue[id] = { de: lvEntry.de, lv: map[itEntry.lv] || itEntry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(esDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  const esHtml = {};
  for (const [key, html] of Object.entries(itCourse.COURSE_LESSON_HTML || {})) {
    esHtml[key] = await translateHtmlLesson(html, map);
  }
  const esData = applyTranslation(itCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(esHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(esData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Spanish course training cards for ES-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(itTraining)) {
    if (!Array.isArray(deck)) continue;
    const esKey = key.replace(/It$/, "Es");
  ...
  console.log("\nES-DE data generation complete (IT→ES with LV DE preservation).");
}
