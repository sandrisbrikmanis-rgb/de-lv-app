#!/usr/bin/env node
/**
 * Verify CS-DE creation compliance:
 * 1. German (de) fields in CS data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const cs = loadArray(`data/cs/${level}.js`);
  const n = Math.min(lv.length, cs.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const csFields = collectDeFields(cs[i]);
    for (const lf of lvFields) {
      const tf = csFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, cs: tf?.value });
      }
    }
  }
}
...
const csSent = loadArray("data/cs/sentences.js");
lvSent.forEach((lv, i) => {
  if (csSent[i] && lv.de !== csSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, cs: csSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const csVerbs = loadArray("data/cs/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const csEntry = csVerbs[i];
  if (!csEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== csEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, cs: csEntry[form]?.de });
    }
  }
});
...
try {
  const diff = execSync(
    "git diff --name-only HEAD -- languages/lv languages/lt languages/ru languages/pl languages/uk languages/et languages/ro languages/bg languages/gr languages/tr languages/sq languages/mk languages/sl languages/bs languages/sr languages/hr languages/sk data/lv data/lt data/ru data/pl data/uk data/et data/ro data/bg data/gr data/tr data/sq data/mk data/sl data/bs data/sr data/hr data/sk data/a1.js data/a2.js data/b1.js data/b2.js data/c1.js data/c2.js data/sentences.js data/verbs.js data/courseLessons.js",
