...
const OUT = path.join(ROOT, "reports/temp/da-a2-low29-owner-apply-map.json");

const STATUS = "(LABOT|FALSE_POSITIVE|NELABOT|NEEDS_SOURCE_REVIEW)";
const ROW = new RegExp(
  `^\\|\\s*(\\d+)\\s*\\|\\s*[^|]+\\|\\s*\`([^\`]+)\`\\s*\\|\\s*\`([^\`]+)\`\\s*\\|\\s*\`([^\`]+)\`\\s*\\|\\s*(?:\\*\\*)?${STATUS}(?:\\*\\*)?\\s*\\|\\s*(.*?)\\s*\\|\\s*$`
);
...
function main() {
  ...
  console.log(JSON.stringify({ rows: apply.length, out: OUT }, null, 2));
}
