...
async function main() {
  ...
  if (FRESH) {
    ...
    if (fs.existsSync(path.join(__dirname, ".es-a1-a2-luna-progress.json"))) {
      fs.unlinkSync(path.join(__dirname, ".es-a1-a2-luna-progress.json"));
    }
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "es-a1-a2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME || FRESH) {
    auditData = { meta: { dataset: "es-a1-a2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }

  const { simple, study } = buildCards();
  const totalCards = simple.length + study.length;
  console.log(`ES A1+A2 Luna audit: ${totalCards} entries (${simple.length} simple, ${study.length} study)`);
