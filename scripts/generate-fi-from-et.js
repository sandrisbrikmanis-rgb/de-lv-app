#!/usr/bin/env node
/**
 * Generate FI-DE data from ET baseline with batched translation (ET → FI).
...
const OUT_DIR = path.join(ROOT, "data", "fi");
const ET_DIR = path.join(ROOT, "data", "et");
const CACHE_PATH = path.join(ROOT, "scripts", ".fi-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "et";
const TO_LANG = "fi";
...
function loadEtTrainingCardsFromUi() {
  const uiCode = fs.readFileSync(path.join(ROOT, "ui.js"), "utf8");
  const cards = {};
  const arrayPattern = /const (lesson\d+(?:Training|Exercise)CardsEt) = (\[[\s\S]*?\n\]);/g;
  let match;
  while ((match = arrayPattern.exec(uiCode)) !== null) {
    const key = match[1];
    const arrCode = match[2];
    const ctx = {};
    vm.createContext(ctx);
    vm.runInContext(`result = ${arrCode}`, ctx);
    if (Array.isArray(ctx.result)) cards[key] = ctx.result;
  }
  return cards;
}
...
async function main() {
  ...
  console.log("Loading ET baseline and LV German source...");
  ...
  const etA1 = loadData("data/et/a1.js").A1_WORDS;
  const etA2 = loadData("data/et/a2.js").A2_WORDS;
  const etB1 = loadData("data/et/b1.js").B1_WORDS;
  const etB2 = loadData("data/et/b2.js").B2_WORDS;
  const etC1 = loadData("data/et/c1.js").C1_WORDS;
  const etC2 = loadData("data/et/c2.js").C2_WORDS;
  const etSentences = loadData("data/et/sentences.js").SENTENCE_ENTRIES;
  const etVerbs = loadData("data/et/verbs.js").VERB_ENTRIES;
  const etDialogue = loadData("data/et/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const etCourse = loadData("data/et/courseLessons.js");
  const etTraining = loadEtTrainingCardsFromUi();

  [etA1, etA2, etB1, etB2, etC1, etC2, etSentences, etVerbs].forEach((arr) => arr.forEach((e) => collectStrings(e, allStrings)));
  Object.values(etDialogue).forEach((e) => collectStrings(e, allStrings));
  Object.values(etCourse.COURSE_LESSON_DATA || {}).forEach((e) => collectStrings(e, allStrings));
  Object.values(etCourse.COURSE_LESSON_HTML || {}).forEach((html) => {
    [...String(html).matchAll(/>([^<]{3,300})</g)].forEach((m) => {
      const t = m[1].trim();
      if (t) allStrings.add(t);
    });
  });
  Object.values(etTraining).forEach((deck) => {
  ...
  console.log(`Found ${unique.length} unique ET strings (${alreadyCached} cached)`);
  ...
  function mergeLevel(lvArr, etArr) {
    return lvArr.map((lvEntry, i) => {
      const etEntry = etArr[i];
      if (!etEntry || lvEntry.de !== etEntry.de) {
        console.warn(`Order mismatch at ${lvEntry.de}`);
        return lvEntry;
      }
      const trEntry = applyTranslation(etEntry, map);
      return preserveGermanFields(lvEntry, trEntry);
    });
  }

  console.log("\nWriting FI files...");
  writeArrayFile(path.join(OUT_DIR, "a1.js"), "A1_WORDS", mergeLevel(lvA1, etA1));
  writeArrayFile(path.join(OUT_DIR, "a2.js"), "A2_WORDS", mergeLevel(lvA2, etA2));
  writeArrayFile(path.join(OUT_DIR, "b1.js"), "B1_WORDS", mergeLevel(lvB1, etB1));
  writeArrayFile(path.join(OUT_DIR, "b2.js"), "B2_WORDS", mergeLevel(lvB2, etB2));
  writeArrayFile(path.join(OUT_DIR, "c1.js"), "C1_WORDS", mergeLevel(lvC1, etC1));
  writeArrayFile(path.join(OUT_DIR, "c2.js"), "C2_WORDS", mergeLevel(lvC2, etC2));
  writeArrayFile(path.join(OUT_DIR, "sentences.js"), "SENTENCE_ENTRIES", mergeLevel(lvSentences, etSentences));

  const fiVerbs = lvVerbs.map((lvEntry, i) => {
    const etEntry = etVerbs[i];
    const te = {};
    for (const form of Object.keys(lvEntry)) {
      te[form] = {
        de: lvEntry[form].de,
        lv: map[etEntry[form].lv] || etEntry[form].lv,
      };
    }
    return te;
  });
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", fiVerbs);

  const fiDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const etEntry = etDialogue[id];
    fiDialogue[id] = { de: lvEntry.de, lv: map[etEntry.lv] || etEntry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(fiDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  const fiHtml = {};
  for (const [key, html] of Object.entries(etCourse.COURSE_LESSON_HTML || {})) {
    fiHtml[key] = await translateHtmlLesson(html, map);
  }
  const fiData = applyTranslation(etCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(fiHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(fiData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Finnish course training cards for FI-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(etTraining)) {
    if (!Array.isArray(deck)) continue;
    const fiKey = key.replace(/Et$/, "Fi");
    const translated = deck.map((card) => ({
      front: map[card.front] || card.front,
      back: card.back || "",
    }));
    outLines.push(`window.${fiKey} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nFI-DE data generation complete (ET→FI with LV DE preservation).");
}
