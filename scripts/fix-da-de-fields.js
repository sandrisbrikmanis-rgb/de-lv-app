#!/usr/bin/env node
/**
 * Fix NB data: restore German comparison.word and other DE fields from LV source.
 */
const fs = require("fs");
...
for (const level of LEVELS) {
  ...
  for (let i = 0; i < Math.min(lv.length, da.length); i++) {
    restoreDeFields(lv[i], da[i]);
  }
  writeArrayFile(path.join(ROOT, "data/da", `${level}.js`), VAR_NAMES[level], da);
  console.log(`Fixed ${level}: ${da.length} entries`);
}

const lvSent = loadArray("data/sentences.js");
const csSent = loadArray("data/da/sentences.js");
lvSent.forEach((t, i) => { if (csSent[i]) csSent[i].de = t.de; });
writeArrayFile(path.join(ROOT, "data/da/sentences.js"), "SENTENCE_ENTRIES", csSent);
console.log(`Fixed sentences: ${csSent.length}`);

const lvVerbs = loadArray("data/verbs.js");
const csVerbs = loadArray("data/da/verbs.js");
lvVerbs.forEach((t, i) => {
  if (!csVerbs[i]) return;
...
writeArrayFile(path.join(ROOT, "data/da/verbs.js"), "VERB_ENTRIES", csVerbs);
...
const csDlgCode = fs.readFileSync(path.join(ROOT, "data/da/dialogueIdMap.js"), "utf8");
...
fs.writeFileSync(
  path.join(ROOT, "data/da/dialogueIdMap.js"),
