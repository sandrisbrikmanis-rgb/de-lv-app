...
const PASS_CLASSIFICATION =
  "A1_LRB_CONSOLIDATION_CARDS_081_100_FULL_LINGUISTIC_REVIEW_PASS";
const PASS_PROOF_PATH = path.join(
  OUT_DIR,
  "A1-LRB-CONSOLIDATION-CARDS-081-100-GALA-PASS-PROOF.json"
);
...
function main() {
  execSync("node scripts/verify-a1-lrb-consolidation-gala-correction-1-cards-081-100.js", {
    cwd: ROOT,
    stdio: "pipe",
  });
  ...
  const scopeCards = copyPaste.cards.slice(80, 100);
  ...
  const gates = runGalaCorrection081100Gates({
    scopeCards,
    appliedByKey,
  ...
  for (const c of scopeCards) {
    ...
    if (assessFullCompositeCompleteness(card)) fullCompositesPass += 1;
  ...
  const cards081100Sha256 = sha256(JSON.stringify(cardsPayload));
  ...
  const proof = {
    ...
    cards_081_100_payload_sha256: cards081100Sha256,
  ...
  copyPaste.cards_081_100_gala_pass = {
    classification: PASS_CLASSIFICATION,
  ...
  const summaryLines = [
    "A1 LRB konsolidācija — kartīšu 081–100 GALA PASS",
    "",
    ...
    "| LABOT | 1/1 piemērots precīzi |",
    "| NELABOT | 19/19 nemainītas |",
    "| PENDING | 0 |",
    "| Pilnās gala kartītes | 20/20 PASS |",
    "",
    `Git HEAD: ${head}`,
    `Cards 081–100 payload SHA-256: ${cards081100Sha256}`,
    `Pass proof SHA-256: ${sha256(fs.readFileSync(PASS_PROOF_PATH))}`,
  ];
  fs.writeFileSync(
    path.join(OUT_DIR, "A1-LRB-CONSOLIDATION-CARDS-081-100-GALA-PASS.txt"),
    summaryLines.join("\n") + "\n"
  );
