...
"use strict";
/**
 * ET B2 multi-translation OWNER COPY-ONLY apply (829 LABOT, MASTER v1.12).
 */
const fs = require("fs");
...
const APPLY_LOG = path.join(ROOT, "reports/temp/et-b2-multitranslation-owner-apply-log.json");
const REPORT_MD = path.join(ROOT, "reports/et-b2-multitranslation-owner-repair-apply.md");
const DATA_REL = "data/et/b2.js";
const FILES = [path.join(ROOT, DATA_REL), path.join(ROOT, "www/data/et/b2.js")];
...
function main() {
  ...
  if (apply.length !== 829) {
    console.error(`Expected 829 LABOT rows, got ${apply.length}`);
    process.exit(1);
  }
  ...
  if (!DRY_RUN && log.staged.length > 0) {
    ...
    gitDiffPass = gitDiffNonEmpty([DATA_REL, "www/data/et/b2.js"], ROOT);
  ...
  fs.writeFileSync(
    REPORT_MD,
    [
      "# ET B2 — Multi-translation OWNER COPY-ONLY apply",
      "",
      "**MASTER:** v1.12",
      `**Authority:** \`reports/et-b2-multitranslation-owner-decisions-accepted-materialized.md\``,
