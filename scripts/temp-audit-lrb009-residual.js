...
for (const row of rows) {
  ...
  if (d.owner_decision === "LABOT" && normalizeVal(effective) === prod) {
    issues.push({ id, type: "LABOT_NO_CHANGE", msg: "owner_new equals production" });
    semanticViolations++;
  }
...
if (nelabot !== 4) {
  issues.push({ type: "NELABOT_COUNT", msg: `expected 4 NELABOT, got ${nelabot}` });
  semanticViolations++;
}
