#!/usr/bin/env node
/**
 * Fix BS-DE B1 study.sectionAccents to match existing Bosnian Study text.
 * Only modifies sectionAccents in data/bs/b1.js (then syncs www/).
 *
 * Run: node scripts/fix-bs-b1-section-accents.js [--dry-run]
 */
const fs = require("fs");
...
const REPORT_PATH = path.join(ROOT, "reports", "temp", "bs-b1-section-accents-fix-log.json");
...
function main() {
  ...
  for (let i = 0; i < bs.length; i++) {
    ...
    const cardId = bsEntry.study.id || `b1-${bsEntry.de}-${i}`;
