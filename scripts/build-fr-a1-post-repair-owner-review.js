...
function buildView(findings) {
  ...
  const main = [
    ...
    `| OWNER authority (filled) | [owner-authority/](${gh("reports/owner-authority/")}) | 423 LABOT · 481 NELABOT |`,
    "",
...
function buildReadme(findings, groupFiles) {
  ...
  const content = [
    ...
    `**Audit PR:** [#${PR_NUMBER}](https://github.com/${REPO}/pull/${PR_NUMBER})`,
    "",
    `Avots: [${AUDIT_MD}](${gh(`reports/${AUDIT_MD}`)}) · [GitHub indekss](${gh("reports/fr-a1-post-repair-owner-review-GITHUB.md")})`,
    "",
    "## Kopsavilkums",
    "",
    "| Metrika | Skaitlis |",
...
function buildGithub(findings, groupFiles, coverage, auditData) {
  ...
  const content = [
    ...
    `| [OWNER apply report](${gh("reports/fr-a1-owner-gala-copy-only-apply-report.md")}) | PR #683 COPY-ONLY · 423/423 LABOT |`,
    "",
