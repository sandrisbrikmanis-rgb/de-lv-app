#!/usr/bin/env node
/**
 * Verify BG-DE creation compliance:
 * 1. German (de) fields in BG data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const bg = loadArray(`data/bg/${level}.js`);
  const n = Math.min(lv.length, bg.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const bgFields = collectDeFields(bg[i]);
    for (const lf of lvFields) {
      const bf = bgFields.find((f) => f.path === lf.path);
      if (!bf || bf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, bg: bf?.value });
      }
    }
  }
}
...
const bgSent = loadArray("data/bg/sentences.js");
lvSent.forEach((lv, i) => {
  if (bgSent[i] && lv.de !== bgSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, bg: bgSent[i].de });
  }
});

// Verbs DE check
const lvVerbs = loadArray("data/verbs.js");
const bgVerbs = loadArray("data/bg/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const bgEntry = bgVerbs[i];
  if (!bgEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== bgEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, bg: bgEntry[form]?.de });
    }
  }
});

// Check other language files unchanged via git
let otherLangChanges = [];
try {
  const diff = execSync("git diff --name-only HEAD -- languages/lv languages/lt languages/ru languages/pl languages/uk languages/et languages/ro data/lv data/lt data/ru data/pl data/uk data/et data/ro data/a1.js data/a2.js data/b1.js data/b2.js data/c1.js data/c2.js data/sentences.js data/verbs.js data/courseLessons.js", { cwd: ROOT, encoding: "utf8" }).trim();
