...
const PROOF_PATH = path.join(
  OUT_DIR,
  "A1-LRB-CONSOLIDATION-OWNER-CARDS-201-220-COPY-PASTE-PROOF.json"
);
const EXPECTED_CLASSIFICATION =
  "A1_LRB_CONSOLIDATION_OWNER_CARDS_201_220_FULL_CARD_COPY_PASTE_READY";
const PROOF_CLASSIFICATION =
  "A1_LRB_CONSOLIDATION_OWNER_CARDS_201_220_COPY_PASTE_COMPLETE_AWAITING_GALA_VERDICT";
const EXPECTED_OWNER_SHA = "8c971b5d83ef40e4827dcd4ee18418f2204227d2090e07e74a966c0517672d8f";
const EXPECTED_SOURCE_HEAD = "bb54caa4b44e149717e8fe9b6549a4280753d45a";

function main() {
  const srcArg = process.argv[2];
  const src = srcArg ? path.resolve(srcArg) : DEFAULT_BUNDLE;
  if (!fs.existsSync(src)) {
    console.error("Owner cards 201–220 bundle not found:", src);
    process.exit(1);
  }
  ...
  try {
    const proof = applyOwnerCopyPasteBatch({
      ...
      ownerRaw,
      ownerBundleRelPath:
        "reports/g2-a1-owner/consolidation/owner-review/A1-LRB-CONSOLIDATION-decisions-COPY-PASTE-4-CARDS-201-220.json",
    ...
    const taskLines = [
      "A1 LRB — kartīšu 201–220 OWNER COPY/PASTE (piemērots)",
      "",
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "A1-LRB-CONSOLIDATION-VEICAMO-LABOJUMU-UZDEVUMS-4-CARDS-201-220-APPLIED.txt"),
      taskLines.join("\n") + "\n"
    );
