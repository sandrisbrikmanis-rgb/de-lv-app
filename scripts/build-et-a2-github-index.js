...
"use strict";
/**
 * Generate reports/et-a2-owner-review-GITHUB.md — clickable GitHub blob links (MASTER §7.6 / §7.10.1).
 */
const fs = require("fs");
...
const OUT = path.join(ROOT, "reports/et-a2-owner-review-GITHUB.md");
...
function groupRows(total) {
  ...
  for (let i = 1; i <= groupCount; i++) {
    ...
    rows.push(
      `| ${start}–${end} | ${link(`et-a2-owner-view-group${id}.md`, "VIEW")} | ${link(`et-a2-owner-decisions-group${id}.md`, "DECISIONS")} | **PENDING** |`,
    );
  }
  return rows.join("\n");
}
...
function verifyGithubArtifacts(findings) {
  const required = [
    ...
    "reports/et-a2-full-audit.json",
  ];
  const groupCount = Math.ceil(findings.length / GROUP_SIZE);
  for (let i = 1; i <= groupCount; i++) {
    const id = String(i).padStart(2, "0");
    required.push(`reports/et-a2-owner-view-group${id}.md`, `reports/et-a2-owner-decisions-group${id}.md`);
  }
...
function main() {
  ...
  const md = `# ET–DE A2 — GitHub atvēršanas indekss
...
| Fails | Apraksts |
|-------|----------|
| ${link("et-a2-owner-review-README.md", "OWNER README")} | Workflow un kopsavilkums |
| ${link("et-a2-owner-review-GITHUB.md", "Šis indekss")} | Visas GitHub saites |
| ${link("et-a2-full-audit.json", "Audit JSON")} | ${TOTAL_CARDS}/${TOTAL_CARDS} · OWNER backlog **${n}** |
| ${link("et-a2-full-audit.md", "Pilns audits (MD)")} | Liels fails — JSON ieteicams |
...
| Tips | Fails |
|------|-------|
| OWNER VIEW | ${link("et-a2-owner-view.md", "et-a2-owner-view.md")} |
| OWNER DECISIONS | ${link("et-a2-owner-decisions.md", "et-a2-owner-decisions.md")} |
| Audit JSON | ${link("et-a2-full-audit.json", "et-a2-full-audit.json")} |
...
1. Atver ${link("et-a2-owner-view-group01.md", "VIEW 1–50")} + ${link("et-a2-owner-decisions-group01.md", "DECISIONS 1–50")} (tad pārējās grupas).
