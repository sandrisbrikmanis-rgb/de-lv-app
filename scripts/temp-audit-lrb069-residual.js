...
for (const row of rows) {
  ...
  if (lang === "lb" && LV_LEAK.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_LV_IN_LB", msg: allText.slice(0, 100) });
  }
  if (lang === "lb" && ET_LEAK.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_ET_IN_LB", msg: allText.slice(0, 100) });
  }
  if (lang === "lb" && HU_IN_IS.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_HU_IN_LB", msg: allText.slice(0, 100) });
  }
  if (lang === "lb" && NO_IN_IS.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_NO_IN_LB", msg: allText.slice(0, 100) });
  }
