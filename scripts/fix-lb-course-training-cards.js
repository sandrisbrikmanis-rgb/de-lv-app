...
const CACHE_PATH = path.join(ROOT, "scripts", ".lb-nl-translation-cache.json");
const cache = loadCache(CACHE_PATH);

function translate(text) {
  if (!text) return text;
  return cache[cacheKey("nl", "lb", text)] || text;
}
...
while ((match = arrayPattern.exec(code)) !== null) {
  const lbKey = match[1].replace(/Nl$/, "Lb");
  const innerCtx = {};
...
fs.writeFileSync(path.join(ROOT, "data/lb/courseTrainingCards.js"), outLines.join("\n"), "utf8");
console.log(`Written ${outLines.length - 1} decks to data/lb/courseTrainingCards.js`);
