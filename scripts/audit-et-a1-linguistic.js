...
"use strict";
/**
 * ET-DE A1 linguistic audit with GPT-5.6 Luna (READ-ONLY).
 * Usage: node scripts/audit-et-a1-linguistic.js [--test-batch] [--resume]
 */
...
const {
  ...
  auditCardsBatch,
  classifyFindings,
} = require("./lib/openai-et-a1-audit");
...
async function main() {
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "et-a1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME) {
    auditData = { meta: { dataset: "et-a1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }
