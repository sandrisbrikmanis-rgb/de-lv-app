...
"use strict";
/**
 * ES-DE C1 OWNER COPY-ONLY apply (253 LABOT, 2 NELABOT).
 * Authority: reports/es-de-c1-owner-decisions-final.json only.
 * Usage: node scripts/apply-es-de-c1-owner-decisions-final.js [--dry-run]
 */
const fs = require("fs");
...
const cfg = getModuleConfig("c1");
const DECISIONS_JSON = path.join(ROOT, "reports/es-de-c1-owner-decisions-final.json");
const APPLY_LOG = path.join(ROOT, "reports/temp/es-de-c1-owner-repair-apply-final-log.json");
const REPORT_MD = path.join(ROOT, "reports/es-de-c1-owner-repair-apply-final.md");
...
function writeWords(words) {
  const content = `const ${cfg.globalKey} = ${JSON.stringify(words, null, 2)};\n\nwindow.${cfg.globalKey} = ${cfg.globalKey};\n`;
...
function verifyApplied(loadFn, staged, readFn) {
  ...
  for (const row of staged) {
    ...
    const actualAfter = readFn(entry, row.field);
...
function verifyNelabotUnchanged(words, nelabotItems) {
  ...
  for (const item of nelabotItems) {
    const { entry } = resolveCard(words, item.cardId, cfg);
    if (!entry) {
...
function writeReport(log) {
  const s = log.summary;
  const lines = [
    "# ES–DE C1 — OWNER COPY-ONLY repair apply (final)",
    "",
    "**Standard:** `REPAIR_APPLY_SAFETY_STANDARD.md` + `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9",
    "**Authority:** `reports/es-de-c1-owner-decisions-final.json`",
    `**Branch:** \`${BRANCH}\``,
...
function main() {
  ...
  for (const row of prereq.labot) {
    const { entry } = resolveCard(words, row.cardId, cfg);
    if (!entry) {
      log.failed.push({
        auditId: row.id,
        cardId: row.cardId,
    ...
    const result = applySet(entry, row.field, parseOwnerNew(row.new));
    if (!result.ok) {
      log.failed.push({
        auditId: row.id,
        cardId: row.cardId,
    ...
    log.staged.push({
      ...
      ownerNew: parseOwnerNew(row.new),
  ...
  const { verified, failures } = DRY_RUN
    ...
    : verifyApplied(loadFn, log.staged, readFn);
  ...
  const passApply =
    ...
    syntaxPass &&
    mirrorPass &&
    afterWords.length === cfg.totalCards;

  const finalVerdict = passApply
    ? "PASS — ALL 253 OWNER LABOT APPLIED AND VERIFIED"
  ...
  log.summary = {
    ...
    idOrderPass: afterWords.length === cfg.totalCards ? "PASS" : "FAIL",
    reconciliation,
    finalVerdict,
  };
