#!/usr/bin/env node
/**
 * Verify SV-DE creation compliance:
 * 1. German (de) fields in SV data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const n = Math.min(lv.length, sv.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const svFields = collectDeFields(sv[i]);
...
const svSent = loadArray("data/sv/sentences.js");
lvSent.forEach((lv, i) => {
  if (svSent[i] && lv.de !== svSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, sv: svSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const svVerbs = loadArray("data/sv/verbs.js");
