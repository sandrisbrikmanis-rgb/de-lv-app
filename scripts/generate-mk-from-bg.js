#!/usr/bin/env node
/**
 * Generate MK-DE data from BG baseline (BG was generated from RU with DE READ-ONLY).
 * Translates only native-language (lv) fields BG → MK.
 * German (de) fields are preserved verbatim from BG/LV source (READ-ONLY).
 */
const fs = require("fs");
...
const OUT_DIR = path.join(ROOT, "data", "mk");
const CACHE_PATH = path.join(ROOT, "scripts", ".mk-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "bg";
const TO_LANG = "mk";
...
async function main() {
  ...
  console.log("Collecting unique Bulgarian strings from BG-DE for MK translation...");
  const a1 = loadData("data/bg/a1.js").A1_WORDS;
  const a2 = loadData("data/bg/a2.js").A2_WORDS;
  const b1 = loadData("data/bg/b1.js").B1_WORDS;
  const b2 = loadData("data/bg/b2.js").B2_WORDS;
  const c1 = loadData("data/bg/c1.js").C1_WORDS;
  const c2 = loadData("data/bg/c2.js").C2_WORDS;
  const sentences = loadData("data/bg/sentences.js").SENTENCE_ENTRIES;
  const verbs = loadData("data/bg/verbs.js").VERB_ENTRIES;
  const dialogue = loadData("data/bg/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const course = loadData("data/bg/courseLessons.js");
  const training = fs.existsSync(path.join(ROOT, "data/bg/courseTrainingCards.js"))
    ? loadData("data/bg/courseTrainingCards.js")
    : {};
  ...
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", mkVerbs);

  const mkDialogue = {};
  for (const [id, entry] of Object.entries(dialogue)) {
    mkDialogue[id] = { de: entry.de, lv: map[entry.lv] || entry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(mkDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  for (const [key, html] of Object.entries(course.COURSE_LESSON_HTML || {})) {
    mkHtml[key] = await translateHtmlLesson(html, map);
  }
  const mkData = applyTranslation(course.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(mkHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(mkData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Macedonian course training cards for MK-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Bg$/, "Mk")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nMK-DE data generation complete (BG → MK).");
}
