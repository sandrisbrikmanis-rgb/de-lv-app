#!/usr/bin/env node
/**
 * DA-DE B2 full audit collector (READ-ONLY).
 * Compares data/da/b2.js vs DE etalon data/b2.js; mirrors www/data/da/b2.js.
 * Writes reports/temp/da-b2-audit-data.json
 */
const fs = require("fs");
...
const OUT = path.join(ROOT, "reports/temp/da-b2-audit-data.json");
...
function main() {
  const de = loadArrayDataset("data/b2.js");
  const da = loadArrayDataset("data/da/b2.js");
  ...
  function add(raw) {
    ...
    findingId++;
    findings.push({
      id: `DA-B2-${String(findingId).padStart(4, "0")}`,
      cardId: raw.cardId,
      field: raw.field,
  ...
  const meta = {
    ...
    mode: "READ-ONLY",
    daFile: "data/da/b2.js",
    note: "Dāņu tulkojumi glabājas laukā lv (projekta konvencija).",
    deReference: "data/b2.js (DE parity only, READ-ONLY)",
    wwwFile: "www/data/da/b2.js",
  ...
  if (da.length !== EXPECTED_CARDS) {
    add({
      ...
      problem: "Kartīšu skaits neatbilst sagaidāmajam B2 apjomam",
      rationale: `App UI sagaida ${EXPECTED_CARDS} B2 kartītes`,
      severity: "CRITICAL",
    });
  ...
  if (meta.studyCount !== EXPECTED_STUDIES) {
    add({
      ...
      rationale: `DA Study kartīšu skaitam jābūt ${EXPECTED_STUDIES}`,
      severity: "CRITICAL",
    });
  ...
  if (!meta.mirrorIdentical) {
    add({
      ...
      field: "mirror",
      deContext: "",
      currentDa: "data/da/b2.js ≠ www/data/da/b2.js",
    ...
    if (!entry.lv || !String(entry.lv).trim()) {
      ...
      if (segments.length >= 3) {
        add({
          ...
          rationale: "B2 galvenajā laukā ieteicams 1–2 dabiski dāņu sinonīmi",
          severity: "MEDIUM",
