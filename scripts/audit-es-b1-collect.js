#!/usr/bin/env node
/**
 * ES-DE B1 full audit collector (read-only). Writes reports/temp/es-de-b1-audit-data.json
 */
const fs = require("fs");
...
const OUT = path.join(ROOT, "reports", "temp", "es-de-b1-audit-data.json");
...
function main() {
  ...
  const data = {
    meta: {
      ...
      lvFile: "data/b1.js",
      esFile: "data/es/b1.js",
      wwwFile: "www/data/es/b1.js",
      lvCount: lv.length,
      esCount: es.length,
    },
  ...
  if (!data.layerIdentity.identical) {
    ...
    data.structural.issues.push({ severity: "critical", message: "data/es/b1.js and www/data/es/b1.js are not identical" });
  }
  ...
  const esStudy = es.filter((e) => e.study).length;
  data.meta.lvStudyCount = lvStudy;
  data.meta.esStudyCount = esStudy;
  if (lvStudy !== esStudy) {
    data.structural.pass = false;
    data.structural.issues.push({ severity: "critical", message: `Study count mismatch LV=${lvStudy} ES=${esStudy}` });
  }
