 ...
 * Usage: node scripts/audit-da-b1-targeted-regression.js
 */
const fs = require("fs");
...
const REPORT = path.join(ROOT, "reports/da-b1-targeted-regression-audit.md");
...
function main() {
  ...
  if (!fs.existsSync(BEFORE_REF)) {
    execSync(`git show origin/main:data/da/b1.js > ${BEFORE_REF}`, { cwd: ROOT, stdio: "pipe" });
  }
  ...
  function add(severity, cardId, field, problem, detail = {}) {
    ...
    findings.push({ id: `DA-B1-REG-${String(fid).padStart(4, "0")}`, severity, cardId, field, problem, ...rest });
  }
  ...
  for (const row of applyMap.apply) {
    ...
    if (row.action === "FJERN_ACCENT") {
      ...
      if (termStillPresent(actualVal, term)) {
        add("MEDIUM", row.cardId, fieldPath, `FJERN term still present: ${term}`, {
          actual: String(actualVal).slice(0, 80),
        });
      }
    }
  }
    ...
    if (entry.study) {
      ...
      if (entry.study.sectionAccents) {
        for (const m of validateSectionAccents(entry.study, entry.study.sectionAccents, entry.de)) {
          const sev = classifyForeign(m.target).length > 0 ? "MEDIUM" : "LOW";
  ...
  const verdict =
    bySev.CRITICAL === 0 && bySev.HIGH === 0 && bySev.MEDIUM === 0
      ? `**DA–DE B1: REPAIR SCOPE CLOSED** (targeted regression PASS${bySev.LOW ? `; ${bySev.LOW} sectionAccent stale LOW residual` : ""})`
      : "**DA–DE B1: REGRESSION ISSUES REMAIN**";
  ...
  const md = [
    ...
    changedCards.size > 100 ? `\n_… and ${changedCards.size - 100} more changed cards._\n` : "",
  ];
  ...
  if (findings.length) {
    ...
    if (findings.length > 100) md.push(`_… ${findings.length - 100} more findings omitted._`, "");
  } else {
    ...
    JSON.stringify(
      {
        ...
        report: REPORT,
        pass: bySev.CRITICAL + bySev.HIGH + bySev.MEDIUM === 0,
      },
      null,
      2
    )
