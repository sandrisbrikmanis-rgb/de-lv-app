#!/usr/bin/env node
/**
 * Verify IT-DE creation compliance:
 * 1. German (de) fields in LB data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const nl = loadArray(`data/it/${level}.js`);
  const n = Math.min(lv.length, nl.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const svFields = collectDeFields(nl[i]);
    for (const lf of lvFields) {
      const tf = svFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, sv: tf?.value });
      }
    }
  }
}
...
const nlSent = loadArray("data/it/sentences.js");
lvSent.forEach((lv, i) => {
  if (nlSent[i] && lv.de !== nlSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, sv: nlSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const nlVerbs = loadArray("data/it/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const svEntry = nlVerbs[i];
  if (!svEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== svEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, sv: svEntry[form]?.de });
    }
  }
});
