...
"use strict";
/**
 * ES-DE module linguistic audit with GPT-5.6 Luna (READ-ONLY).
 * Usage: node scripts/audit-es-de-linguistic.js --module=b2|c1|c2|sentences|verbs [--test-batch] [--resume] [--fresh]
 */
...
const { createStats } = require("./lib/openai-es-de-audit");
const { auditCardsBatch, classifyFindings } = require("./lib/openai-es-de-audit");
...
async function runVocabLuna() {
  ...
  console.log(`ES ${cfg.moduleKey} Luna audit: ${totalCards} entries (${simple.length} simple, ${study.length} study)`);
...
async function runBatchLunaFromCollect() {
  ...
  console.log(`ES ${cfg.moduleKey} Luna audit: ${batches.length} batches`);
...
function finalizeLunaData(auditData, stats, progress, auditedCardIds, totalCards) {
  ...
  for (const f of qualityFindings) {
    const key = `${f.cardId}|${f.field || "lv"}|${String(f.currentEs || "").slice(0, 60)}`;
