...
const PASTE_PATH =
  process.argv[2] ||
  path.join(ROOT, "scripts/data/g2-a1-owner-pending/LRB-068-decisions-COPY-PASTE.json");
const EXPECTED_PASTE_SHA =
  "0c2fa92ff747fb4a1769f2e90c2d1bbb54de42c737f08c99a13d029206545cde";
...
function resolveCardDecision(mapping, cardObjectId) {
  if (mapping.nelabot && cardObjectId in mapping.nelabot) {
    const nelabotNote =
      ...
      `LRB-068 OWNER mapping NELABOT: preserve production for ${cardObjectId}.`;
    return {
  ...
  if (mapping.labot_full_composite && cardObjectId in mapping.labot_full_composite) {
    ...
    return {
      ...
      owner_note: `LRB-068 OWNER mapping full_card_target_fields: ${cardObjectId}.`,
    };
  }
  if (mapping.labot_scalar && cardObjectId in mapping.labot_scalar) {
    return {
      ...
      replacement_scope: "finding_field_only",
      owner_note: `LRB-068 OWNER mapping scalar lv: ${cardObjectId}.`,
    };
  }
  return null;
}
...
function main() {
  ...
  const expectedInputSha =
    "1b1e2fff72478e9f91a7951664143ba955fe3753182247c00ec0b4c7c8a978c1";
  ...
  if (labot !== 50 || nelabot !== 0 || pending !== 0) {
  ...
  if (fullCardIds.size !== 6) {
    throw new Error(`Expected 6 full composites, got ${fullCardIds.size}`);
  }
  ...
  const galaOut = {
    ...
    classification: "LRB_068_COPY_PASTE_COMPLETE_AWAITING_GALA_VERDICT",
    cards: galaCards,
  };
