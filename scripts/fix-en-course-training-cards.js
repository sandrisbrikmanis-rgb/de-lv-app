...
function translate(text) {
  if (!text) return text;
  return cache[cacheKey("lv", "en", text)] || text;
}
...
const outLines = ["// British English course training cards for NL-DE Kurss lessons 1-7.\n"];
const arrayPattern = /window\.(lesson\d+(?:Training|Exercise)CardsLt)\s*=\s*(\[[\s\S]*?\n\]);/g;
let match;
while ((match = arrayPattern.exec(code)) !== null) {
  const nlKey = match[1].replace(/Lt$/, "En");
  const innerCtx = {};
...
fs.writeFileSync(path.join(ROOT, "data/en/courseTrainingCards.js"), outLines.join("\n"), "utf8");
console.log(`Written ${outLines.length - 1} decks to data/en/courseTrainingCards.js`);
