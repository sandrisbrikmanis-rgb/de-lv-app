...
"use strict";
/**
 * FR-DE A1+A2 linguistic audit with GPT-5.6 Luna (READ-ONLY).
 * Usage: node scripts/audit-fr-a1-a2-linguistic.js [--test-batch] [--resume] [--fresh] [--final-regression]
 */
...
const {
  ...
  loadProgress,
  saveProgress,
} = require("./lib/fr-a1-a2-audit-helpers");
const {
  createStats,
  auditCardsBatch,
  classifyFindings,
} = require("./lib/openai-fr-a1-a2-audit");
...
async function main() {
  ...
  if (FRESH) {
    ...
    if (fs.existsSync(path.join(__dirname, ".fr-a1-a2-luna-progress.json"))) {
      fs.unlinkSync(path.join(__dirname, ".fr-a1-a2-luna-progress.json"));
    }
  ...
  const progress = RESUME && !FRESH ? loadProgress() : { completedBatches: [], auditedCardIds: [] };
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "fr-a1-a2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME || FRESH) {
    auditData = { meta: { dataset: "fr-a1-a2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }

  const { simple, study } = buildCards();
  const totalCards = simple.length + study.length;
  console.log(`FR A1+A2 Luna audit: ${totalCards} entries (${simple.length} simple, ${study.length} study)`);
  ...
  for (const f of qualityFindings) {
    const key = `${f.cardId}|${f.field}|${String(f.currentFr || "").slice(0, 60)}`;
  ...
  auditData.meta.testBatch = TEST_BATCH;
