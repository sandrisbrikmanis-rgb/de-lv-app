#!/usr/bin/env node
/**
 * Generate RU-DE data files from LV baseline.
 * Translates only native-language (lv) text fields to Russian.
...
async function translateText(text) {
  ...
  try {
    const res = await translate(trimmed, { from: "lv", to: "ru", forceBatch: false });
...
async function translateExamplePair(example) {
  ...
  const ruPart = await translateText(m[3]);
  return `${m[1]}${m[2]}${ruPart}`;
}
...
async function processWordFile(srcRel, destName, varName) {
  console.log(`\nProcessing ${srcRel} -> data/ru/${destName}`);
...
async function translateHtmlLesson(html) {
  ...
  for (const match of matches) {
    ...
    if (text.includes("—") || text.includes("–")) {
      ...
      const ru = translated.join(" — ");
      result = result.split(`>${text}<`).join(`>${ru}<`);
    } else {
      const ru = await translateText(text);
      result = result.split(`>${text}<`).join(`>${ru}<`);
    }
  }
  return result;
...
async function main() {
  ...
  console.log(`Generating RU-DE data in ${OUT_DIR}`);
