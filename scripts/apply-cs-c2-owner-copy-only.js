...
"use strict";
/**
 * CS-DE C2 — OWNER approved COPY-ONLY apply from cs-c2-owner-approved-master-repair.md
 * Usage: node scripts/apply-cs-c2-owner-copy-only.js
 */
const fs = require("fs");
...
const FILES = [path.join(ROOT, "data/cs/c2.js"), path.join(ROOT, "www/data/cs/c2.js")];
const REPORT_MD = path.join(ROOT, "reports/cs-c2-owner-copy-only-apply.md");
const REPORT_JSON = path.join(ROOT, "reports/temp/cs-c2-owner-copy-only-apply.json");
const EXPECTED_CARD_COUNT = 219;
...
function writeWords(filePath, words) {
  fs.writeFileSync(
    filePath,
    `const C2_WORDS = ${JSON.stringify(words, null, 2)};\n\nwindow.C2_WORDS = C2_WORDS;\n`,
    "utf8",
  );
}
...
function renderReport(stats, details) {
  const lines = [
    "# CS–DE C2 — OWNER COPY-ONLY Apply",
    "",
    `Generated: ${new Date().toISOString()}`,
    `Source: \`cs-c2-owner-approved-master-repair.md\``,
    "",
    "## Summary",
    "",
    "| Metric | Count |",
...
function main() {
  ...
  const byIdPre = buildCardIndex(baseline);
  const refinedRows = refineMappings(parsedRows, byIdPre);
  const { raw, skipped, labotRows, unique, conflicts } = dedupeMappings(refinedRows);
