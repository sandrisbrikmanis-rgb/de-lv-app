...
"use strict";
/**
 * GPT-5.6 Luna API pass for ET Kurss full audit batches.
 * Usage: node scripts/audit-et-kurss-full-luna-api.js [--force]
 */
const fs = require("fs");
const path = require("path");
require("dotenv").config({ path: path.join(__dirname, "..", ".env") });
const { ROOT } = require("./lib/audit-common");
const { auditKurssBatch, createStats } = require("./lib/openai-et-kurss-audit");
...
async function main() {
  ...
  if (!batches.length) {
    console.error("No Luna batches found. Run: node scripts/audit-et-kurss-full.js --export-only");
    process.exit(1);
  }
