...
function main() {
  ...
  for (const row of decided) {
    ...
    if (!noteGroups.has(n)) noteGroups.set(n, []);
    noteGroups.get(n).push(String(row.owner_new || ""));
  }
  let duplicateNotes = 0;
  for (const ownerNews of noteGroups.values()) {
    if (ownerNews.length <= 3) continue;
    const uniqueOwnerNew = new Set(ownerNews.map((o) => sha256Hex(o)));
    if (uniqueOwnerNew.size === 1) duplicateNotes++;
  }
