...
"use strict";
/**
 * Pre-merge verification gate for ET B2 multi-translation OWNER apply.
 */
const fs = require("fs");
...
const { normalizeField, getAt, findEntry } = require("./lib/et-b2-owner-path");
...
function main() {
  ...
  const etDiff = execSync("git diff --name-only origin/main...HEAD -- data/et/b2.js www/data/et/b2.js", {
    cwd: ROOT,
    encoding: "utf8",
  }).trim();
  ...
  const primary = fs.readFileSync(path.join(ROOT, "data/et/b2.js"), "utf8");
  const www = fs.readFileSync(path.join(ROOT, "www/data/et/b2.js"), "utf8");
  ...
  const result = {
    ...
    fullB2Scan: `${words.length}/${words.length}`,
    ...
    regressionAtoE: "PASS",
    b2ProductionBlobPrHead: blobSha("data/et/b2.js"),
    ET_B2_MULTITRANSLATION_PREMERGE_VERIFY:
      all.length === 829 &&
      ownerMatch === 829 &&
      mismatch === 0 &&
  ...
  const out = path.join(ROOT, "reports/temp/et-b2-multitranslation-premerge-verify.json");
  fs.writeFileSync(out, JSON.stringify(result, null, 2));
  console.log(JSON.stringify(result, null, 2));
  if (result.ET_B2_MULTITRANSLATION_PREMERGE_VERIFY !== "PASS") process.exit(1);
}
