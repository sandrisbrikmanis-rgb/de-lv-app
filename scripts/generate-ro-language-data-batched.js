#!/usr/bin/env node
/**
 * Generate RO-DE data from LV baseline with batched unique-string translation.
...
const OUT_DIR = path.join(ROOT, "data", "ro");
const CACHE_PATH = path.join(ROOT, "scripts", ".ro-translation-cache.json");
const BULLET = "•";
const BATCH_SIZE = 100;

const NAME_REPLACEMENTS = [
  [/\bPēteris\b/g, "Petru"], [/\bPētera\b/g, "Petru"],
  [/\bJānis\b/g, "Ion"], [/\bJāņa\b/g, "Ion"],
  ...
  [/\bHanna\b/g, "Hanna"], [/\bGertrud\b/g, "Gertrud"], [/\bAnna\b/g, "Ana"],
...
async function translateBatch(strings) {
  ...
  for (let i = 0; i < pending.length; i += BATCH_SIZE) {
    ...
    try {
      const res = await translate(chunk, { from: "lv", to: "ro", forceBatch: true });
      ...
      for (const src of chunk) {
        try {
          const res = await translate(src.trim(), { from: "lv", to: "ro" });
...
async function main() {
  ...
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", roVerbs);

  const roDialogue = {};
  for (const [id, entry] of Object.entries(dialogue)) {
    roDialogue[id] = { de: entry.de, lv: map[entry.lv] || entry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(roDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  for (const [key, html] of Object.entries(course.COURSE_LESSON_HTML || {})) {
    roHtml[key] = await translateHtmlLesson(html, map);
  }
  const roData = applyTranslation(course.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(roHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(roData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Romanian course training cards for RO-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Lt$/, "Ro")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  ...
  saveCache();
  console.log("\nRO-DE data generation complete.");
}
