#!/usr/bin/env node
/**
 * Generate IS-DE data from NB baseline with batched translation (NB → IS).
...
const OUT_DIR = path.join(ROOT, "data", "is");
const CACHE_PATH = path.join(ROOT, "scripts", ".is-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "nb";
const TO_LANG = "is";
...
async function main() {
  ...
  const isVerbs = lvVerbs.map((lvEntry, i) => {
    const svEntry = nbVerbs[i];
    const te = {};
  ...
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", isVerbs);

  const isDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const nbEntry = nbDialogue[id];
    isDialogue[id] = { de: lvEntry.de, lv: map[nbEntry.lv] || nbEntry.lv };
  }
  fs.writeFileSync(
    path.join(OUT_DIR, "dialogueIdMap.js"),
    `const DIALOGUE_ID_MAP = ${JSON.stringify(isDialogue, null, 2)};\n\nwindow.DIALOGUE_ID_MAP = DIALOGUE_ID_MAP;\n`,
    "utf8"
  );
  ...
  for (const [key, html] of Object.entries(nbCourse.COURSE_LESSON_HTML || {})) {
    isHtml[key] = await translateHtmlLesson(html, map);
  }
  const isData = applyTranslation(nbCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
    path.join(OUT_DIR, "courseLessons.js"),
    `const COURSE_LESSON_HTML = ${JSON.stringify(isHtml, null, 2)};\n\nconst COURSE_LESSON_DATA = ${JSON.stringify(isData, null, 2)};\n\nwindow.COURSE_LESSON_HTML = COURSE_LESSON_HTML;\nwindow.COURSE_LESSON_DATA = COURSE_LESSON_DATA;\n`,
    "utf8"
  );

  const outLines = ["// Icelandic course training cards for IS-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(nbTraining)) {
    if (!Array.isArray(deck)) continue;
    const isKey = key.replace(/Nb$/, "Is");
    const translated = deck.map((card) => ({
      front: map[card.front] || card.front,
      back: card.back || "",
    }));
    outLines.push(`window.${isKey} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nIS-DE data generation complete (NB→IS with LV DE preservation).");
}
