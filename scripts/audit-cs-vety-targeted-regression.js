...
"use strict";
/**
 * CS-DE Věty Targeted Regression Audit (READ-ONLY)
 * GPT-5.6 Luna — OWNER repair scope only (328 mappings).
 *
 * Usage:
 *   node scripts/audit-cs-vety-targeted-regression.js [--skip-luna] [--resume-luna]
 */
...
const OWNER_TOTAL = 328;
const BRANCH = "cursor/cs-vety-targeted-regression-audit-6ea4";
...
const OUT_JSON = path.join(ROOT, "reports/temp/cs-vety-targeted-regression.json");
const OUT_MD = path.join(ROOT, "reports/cs-vety-targeted-regression.md");
const TEMP_DIR = path.join(ROOT, "reports/temp/cs-vety-targeted-regression-audit");
...
const PROGRESS_FILE = path.join(ROOT, "scripts", ".cs-vety-targeted-luna-progress.json");
const DET_COLLECT_DIR = path.join(ROOT, "reports/temp/cs-vety-audit");

const TARGETED_PROMPT = [
  "You are a targeted CS-DE Věty (sentences) repair regression auditor (GPT-5.6 Luna).",
  "These sentence cards were changed by OWNER-approved COPY-ONLY repair (328 lv mappings).",
  "Audit the FULL current production Czech sentence (csText / lv field) against the German source.",
  ...
  "This is NOT a full Věty discovery audit of unchanged sentences.",
  "Return ONLY valid JSON: { \"items\": [ ... ] }.",
  "PASS for correct content. Findings only for objective Czech linguistic errors.",
  "Check: semantic accuracy DE→CS, Czech naturalness, grammar, spelling, register/formality vs DE,",
  "lost or added meaning, unnatural duplicates or variants.",
  ...
  "Multiple senses separated by • are valid project convention when semantically justified.",
...
function enrichFindingFromProduction(f, entries) {
  ...
  const index = resolveIndex(f.cardId);
  if (index === null || index < 0) return f;
  const value = entries[index]?.lv;
...
function runDeterministicCollect() {
  ...
  if (!fs.existsSync(outFile)) {
    const result = spawnSync("node", ["scripts/audit-cs-collect.js", "--dataset=vety"], {
      cwd: ROOT,
      encoding: "utf8",
    });
    if (result.stdout) process.stdout.write(result.stdout);
    if (result.stderr) process.stderr.write(result.stderr);
    if (result.status !== 0) throw new Error("audit-cs-collect.js --dataset=vety failed");
  }
...
function buildLunaCards(targetSet, entries) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    cards.push(buildSentenceCard(lv[i], entries[i], i));
  }
...
function buildMarkdown(data) {
  ...
  const lines = [
    ...
    `| Target cards | ${s.uniqueChangedCards} |`,
    ...
    `| CS changes during regression | ${s.productionChanges} |`,
    `| Unexpected production changes | ${s.unexpectedProductionChanges} |`,
    `| Syntax | ${s.syntax} |`,
    `| ID/order | ${s.idOrder} |`,
    `| Card count | ${s.cardCount}/${VETY_TOTAL} |`,
...
async function main() {
  ...
  console.log("\n=== Věty OWNER prerequisite check (328 mappings) ===");
  ...
  console.log("\n=== Deterministic collect (Věty) — filtered to scope ===");
  ...
  if (!SKIP_LUNA) {
    ...
    lunaResult = await runTargetedLuna(entries, scope.cardIdSet);
  ...
  const integrity = checkIntegrity(loadArray("data/cs/sentences.js", "SENTENCE_ENTRIES"), baselineDeHash);
  ...
  if (integrity.cardCount !== VETY_TOTAL) throw new Error("Card count mismatch");
}
