 ...
 * Source: reports/temp/da-kurss-final-post-repair-audit.json (330 Luna findings)
 */
const fs = require("fs");
...
const DEFAULT_BRANCH = "cursor/da-kurss-final-post-repair-regression-audit-fffe";
const DEFAULT_PR = "570";
const INCLUDE_NSR = process.env.KURSS_INCLUDE_NSR === "1";

const FILES = {
  index: "da-kurss-owner-review-final-post-repair-index.md",
  ...
  reviewGroup: (n, range) =>
    `da-kurss-owner-review-final-post-repair-group${String(n).padStart(2, "0")}-${range.replace(/–/g, "-")}.md`,
  decisionsGroup: (n, range) =>
    `da-kurss-owner-decisions-final-post-repair-group${String(n).padStart(2, "0")}-${range.replace(/–/g, "-")}.md`,
...
function loadAuditFindings() {
  if (!fs.existsSync(AUDIT_JSON)) {
    console.error(`Missing ${AUDIT_JSON}. Run audit-da-kurss-final-post-repair.js first.`);
    process.exit(1);
  }
  const data = JSON.parse(fs.readFileSync(AUDIT_JSON, "utf8"));
  return (data.findings || [])
    .filter((f) => f.category !== "FALSE_POSITIVE")
...
function renderReviewHeader(groupNum, count, range) {
  return [
    `# DA–DE Kurss — OWNER review — final post-repair Group ${String(groupNum).padStart(2, "0")}`,
    "",
...
function renderDecisionsHeader(groupNum, range, count) {
  return [
    `# DA–DE Kurss — OWNER decisions — final post-repair Group ${String(groupNum).padStart(2, "0")}`,
    "",
...
function renderIndex(groups, rows) {
  const fileList = groups
    ...
    .join("\n");
  return `# DA–DE Kurss — OWNER review — final post-repair INDEX
...
| ${ghMd(AUDIT_MD, "Final post-repair audit")} | ${rows.length} findings |
...
function main() {
  ...
  fs.writeFileSync(path.join(reports, FILES.decisions), renderPendingTable(rows, "OWNER decisions final post-repair (PENDING)"));
