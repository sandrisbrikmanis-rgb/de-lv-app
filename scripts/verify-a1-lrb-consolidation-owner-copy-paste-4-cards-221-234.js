...
function main() {
  ...
  if (copyPaste.cards.length !== 234) blockers.push(`copy_paste_row_count:${copyPaste.cards.length}`);
  const scopeCards = copyPaste.cards.slice(SLICE_START, SLICE_END);
  if (scopeCards.length !== 14) blockers.push(`scope_length:${scopeCards.length}`);
  ...
  const out = {
    ...
    checks_run: [
      "verify-a1-lrb-consolidation-owner-copy-paste-4-cards-221-234.js",
