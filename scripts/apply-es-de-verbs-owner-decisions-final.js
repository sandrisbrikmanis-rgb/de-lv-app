...
"use strict";
/**
 * ES-DE Verbs OWNER COPY-ONLY apply (545 LABOT, 5 NELABOT).
 * Authority: reports/es-de-verbs-owner-decisions-final.json only.
 * Usage: node scripts/apply-es-de-verbs-owner-decisions-final.js [--dry-run]
 */
const fs = require("fs");
...
const { resolveCard } = require("./lib/es-de-audit-helpers");
const { buildReconciliation } = require("./lib/repair-apply-safety");

const cfg = getModuleConfig("verbs");
const FORM_KEYS = new Set(cfg.formKeys);
const DECISIONS_JSON = path.join(ROOT, "reports/es-de-verbs-owner-decisions-final.json");
const APPLY_LOG = path.join(ROOT, "reports/temp/es-de-verbs-owner-repair-apply-final-log.json");
const REPORT_MD = path.join(ROOT, "reports/es-de-verbs-owner-repair-apply-final.md");
...
function verifyDeUnchanged(before, after) {
  ...
  for (let i = 0; i < after.length; i++) {
    for (const formKey of FORM_KEYS) {
      if (JSON.stringify(before[i]?.[formKey]?.de) !== JSON.stringify(after[i]?.[formKey]?.de)) n++;
    }
  }
  return n;
}
...
function verifyApplied(loadFn, staged) {
  ...
  for (const row of staged) {
    ...
    const actualAfter = readCurrent(entry, row.field);
...
function validatePrerequisites(decisions) {
  ...
  for (const item of items) {
    const key = `${item.cardId}|${normalizeField(item.field)}`;
...
function verifyNelabotUnchanged(verbs, nelabotItems) {
  let unchanged = 0;
  const changed = [];
  for (const item of nelabotItems) {
    const { entry } = resolveCard(verbs, item.cardId, cfg);
    if (!entry) {
...
function writeReport(log) {
  const s = log.summary;
  const lines = [
    "# ES–DE Darbības vārdi/Verbs — OWNER COPY-ONLY repair apply (final)",
    "",
    "**Standard:** `REPAIR_APPLY_SAFETY_STANDARD.md` + `PROJECT_LANGUAGE_MASTER_STANDARD.md` v1.9",
    "**Authority:** `reports/es-de-verbs-owner-decisions-final.json`",
    `**Branch:** \`${BRANCH}\``,
...
function main() {
  ...
  const beforeAll = deepClone(verbs);
  ...
  for (const row of prereq.labot) {
    const { entry } = resolveCard(verbs, row.cardId, cfg);
    if (!entry) {
      log.failed.push({
        auditId: row.id,
        cardId: row.cardId,
  ...
  const afterVerbs = DRY_RUN ? verbs : loadVerbs();
  const nelabotCheck = verifyNelabotUnchanged(afterVerbs, prereq.nelabot);
  const sourceDeCheck = verifyNelabotUnchanged(afterVerbs, prereq.sourceDeIssue);

  const deChanges = verifyDeUnchanged(beforeAll, afterVerbs);
  ...
  const passApply =
    ...
    syntaxPass &&
    mirrorPass &&
    afterVerbs.length === cfg.totalCards;

  const finalVerdict = passApply
    ? "PASS — ALL 545 OWNER LABOT APPLIED AND VERIFIED"
