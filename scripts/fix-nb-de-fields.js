...
for (const level of LEVELS) {
  ...
  writeArrayFile(path.join(ROOT, "data/nb", `${level}.js`), VAR_NAMES[level], nb);
  console.log(`Fixed ${level}: ${nb.length} entries`);
}

const lvSent = loadArray("data/sentences.js");
const csSent = loadArray("data/nb/sentences.js");
lvSent.forEach((t, i) => { if (csSent[i]) csSent[i].de = t.de; });
writeArrayFile(path.join(ROOT, "data/nb/sentences.js"), "SENTENCE_ENTRIES", csSent);
console.log(`Fixed sentences: ${csSent.length}`);

const lvVerbs = loadArray("data/verbs.js");
const csVerbs = loadArray("data/nb/verbs.js");
lvVerbs.forEach((t, i) => {
  if (!csVerbs[i]) return;
...
writeArrayFile(path.join(ROOT, "data/nb/verbs.js"), "VERB_ENTRIES", csVerbs);
...
const csDlgCode = fs.readFileSync(path.join(ROOT, "data/nb/dialogueIdMap.js"), "utf8");
...
fs.writeFileSync(
  path.join(ROOT, "data/nb/dialogueIdMap.js"),
