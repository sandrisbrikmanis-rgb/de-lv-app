#!/usr/bin/env node
/**
 * Generate SL-DE data from MK baseline (MK was generated from BG with DE READ-ONLY).
 * Translates only native-language (lv) fields MK → SL.
 * German (de) fields are preserved verbatim from MK/LV source (READ-ONLY).
 */
const fs = require("fs");
...
async function main() {
  ...
  console.log("Collecting unique Macedonian strings from MK-DE for SL translation...");
  const a1 = loadData("data/mk/a1.js").A1_WORDS;
  const a2 = loadData("data/mk/a2.js").A2_WORDS;
  const b1 = loadData("data/mk/b1.js").B1_WORDS;
  const b2 = loadData("data/mk/b2.js").B2_WORDS;
  const c1 = loadData("data/mk/c1.js").C1_WORDS;
  const c2 = loadData("data/mk/c2.js").C2_WORDS;
  const sentences = loadData("data/mk/sentences.js").SENTENCE_ENTRIES;
  const verbs = loadData("data/mk/verbs.js").VERB_ENTRIES;
  const dialogue = loadData("data/mk/dialogueIdMap.js").DIALOGUE_ID_MAP;
  const course = loadData("data/mk/courseLessons.js");
  const training = fs.existsSync(path.join(ROOT, "data/mk/courseTrainingCards.js"))
    ? loadData("data/mk/courseTrainingCards.js")
    : {};
  ...
  for (const [key, deck] of Object.entries(training)) {
    ...
    outLines.push(`window.${key.replace(/Mk$/, "Sl")} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nSL-DE data generation complete (MK → SL).");
}
