#!/usr/bin/env node
/**
 * Verify NN-DE creation compliance:
 * 1. German (de) fields in NN data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const nn = loadArray(`data/nn/${level}.js`);
  const n = Math.min(lv.length, nn.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const nnFields = collectDeFields(nn[i]);
    for (const lf of lvFields) {
      const tf = nnFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, nn: tf?.value });
      }
    }
  }
}
...
const nnSent = loadArray("data/nn/sentences.js");
lvSent.forEach((lv, i) => {
  if (nnSent[i] && lv.de !== nnSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, nn: nnSent[i].de });
  }
});

const lvVerbs = loadArray("data/verbs.js");
const nnVerbs = loadArray("data/nn/verbs.js");
lvVerbs.forEach((lvEntry, i) => {
  const nnEntry = nnVerbs[i];
  if (!nnEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== nnEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, nn: nnEntry[form]?.de });
    }
  }
});
