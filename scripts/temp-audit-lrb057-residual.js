...
for (const row of rows) {
  ...
  if (lang === "es" && LV_LEAK.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_LV_IN_ES", msg: allText.slice(0, 100) });
  }
  if (lang === "es" && ET_LEAK.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_ET_IN_ES", msg: allText.slice(0, 100) });
  }
  if (lang === "es" && HU_IN_IS.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_HU_IN_ES", msg: allText.slice(0, 100) });
  }
  if (lang === "es" && NO_IN_IS.test(allText)) {
    wrongLanguage++;
    issues.push({ id, type: "WRONG_LANG_NO_IN_ES", msg: allText.slice(0, 100) });
  }
