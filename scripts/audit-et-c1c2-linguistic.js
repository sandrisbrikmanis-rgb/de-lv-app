 ...
 * Usage: node scripts/audit-et-c1c2-linguistic.js [--test-batch] [--resume]
 */
...
const {
  ...
  auditCardsBatch,
  classifyFindings,
} = require("./lib/openai-et-c1c2-audit");
...
async function main() {
  ...
  const hashBefore = `${md5(LEVELS.c1.etFile)}+${md5(LEVELS.c2.etFile)}`;
  ensureDir(TEMP_DIR);
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "et-c1c2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME) {
    auditData = { meta: { dataset: "et-c1c2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }

  const { simple, study } = buildCards();
  const totalCards = simple.length + study.length;
  console.log(`ET C1C2 Luna audit: ${totalCards} entries (${simple.length} simple, ${study.length} study)`);
  ...
  const hashAfter = `${md5(LEVELS.c1.etFile)}+${md5(LEVELS.c2.etFile)}`;
