#!/usr/bin/env node
/**
 * Verify TR-DE creation compliance:
 * 1. German (de) fields in TR data match LV source exactly (READ-ONLY)
...
for (const level of LEVELS) {
  ...
  const n = Math.min(lv.length, tr.length);
  for (let i = 0; i < n; i++) {
    const lvFields = collectDeFields(lv[i]);
    const trFields = collectDeFields(tr[i]);
    for (const lf of lvFields) {
      const tf = trFields.find((f) => f.path === lf.path);
      if (!tf || tf.value !== lf.value) {
        deMismatches.push({ level, de: lv[i].de, path: lf.path, lv: lf.value, tr: tf?.value });
      }
    }
  }
}
...
lvSent.forEach((lv, i) => {
  if (trSent[i] && lv.de !== trSent[i].de) {
    deMismatches.push({ level: "sentences", index: i, lv: lv.de, tr: trSent[i].de });
  }
});
...
lvVerbs.forEach((lvEntry, i) => {
  const trEntry = trVerbs[i];
  if (!trEntry) return;
  for (const form of Object.keys(lvEntry)) {
    if (lvEntry[form].de !== trEntry[form]?.de) {
      deMismatches.push({ level: "verbs", form, lv: lvEntry[form].de, tr: trEntry[form]?.de });
    }
  }
});
