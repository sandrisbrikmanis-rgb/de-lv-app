#!/usr/bin/env node
/**
 * Fix BS-DE A2 study.sectionAccents to match existing Bosnian Study text.
 * Only modifies sectionAccents in data/bs/a2.js (then syncs www/).
 *
 * Run: node scripts/fix-bs-a2-section-accents.js [--dry-run]
 */
const fs = require("fs");
...
const REPORT_PATH = path.join(ROOT, "reports", "temp", "bs-a2-section-accents-fix-log.json");
...
function main() {
  ...
  for (let i = 0; i < bs.length; i++) {
    ...
    const cardId = bsEntry.study.id || `a2-${bsEntry.de}-${i}`;
