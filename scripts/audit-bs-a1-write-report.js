#!/usr/bin/env node
/**
 * Writes reports/bs-a1-full-audit.md from collected audit data (read-only).
 */
const fs = require("fs");
const path = require("path");
const { ROOT } = require("./lib/audit-common");

const OUT = path.join(ROOT, "reports", "bs-a1-full-audit.md");
const audit = JSON.parse(fs.readFileSync(path.join(ROOT, "reports/temp/bs-a1-audit-data.json"), "utf8"));
const consolidated = JSON.parse(fs.readFileSync(path.join(ROOT, "reports/temp/bs-a1-findings-consolidated.json"), "utf8"));
const validate = JSON.parse(fs.readFileSync(path.join(ROOT, "reports/temp/validate-bs-study.json"), "utf8"));
const fragment = fs.readFileSync(path.join(ROOT, "reports/temp/bs-a1-findings-md-fragment.md"), "utf8");

const a1Validate = validate.perFile.find((f) => f.file === "data/bs/a1.js");
...
const report = `# BS–DE A1 pilns audits
...
| Vidēja | ${consolidated.totals.medium} | sectionAccents: termins nav BS tekstā |
...
Kopā **${medium.length}** accent term neatbilstības **${new Set(medium.map((x) => x.id)).size}** Study kartītēs. Pilns saraksts: \`reports/temp/bs-a1-findings-consolidated.json\` (meklēt \`severity: "medium"\`).
