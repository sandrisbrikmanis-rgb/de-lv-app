...
const { ROOT, DATASET_CONFIG, tempDir, postRepairPaths } = require("./lib/cs-audit-helpers");
...
function parseArgs() {
  let dataset = null;
  let postRepair = false;
  for (const arg of process.argv.slice(2)) {
    if (arg.startsWith("--dataset=")) dataset = arg.slice("--dataset=".length).trim().toLowerCase();
    if (arg === "--post-repair") postRepair = true;
  }
  if (!dataset) throw new Error("Usage: node scripts/audit-cs-write-report.js --dataset=a1 [--post-repair]");
  return { dataset, postRepair: postRepair || process.env.CS_A1_POST_REPAIR === "1" };
}
...
function main() {
  const { dataset, postRepair } = parseArgs();
  const label = LABEL_MAP[dataset];
  const prPaths = postRepair && dataset === "a1" ? postRepairPaths("a1") : null;
  const outFile = prPaths ? prPaths.reportMd : path.join(ROOT, REPORT_MAP[dataset]);
  ...
  const auditTitle = postRepair
    ? "FULL POST-REPAIR LINGUISTIC + DETERMINISTIC AUDIT"
    : "FULL AUDIT";
  const tempArtifactDir = postRepair
    ? "reports/temp/cs-a1-post-repair-audit"
    : `reports/temp/cs-${dataset}-audit`;
  const consolidatedJson = postRepair
    ? "reports/temp/cs-a1-post-repair-audit.json"
    : null;

  const lunaBlocks = (ling.batches || []).map((b) => `- ${b.batch}: ${b.cardCount} cards (${b.auditType})`).join("\n");
  const submittedCount = new Set((ling.batches || []).flatMap((b) => b.cardIds || [])).size;
  const dupCount = (ling.batches || []).flatMap((b) => b.cardIds || []).length - submittedCount;

  const report = `# CS–DE ${label} ${postRepair ? "POST-REPAIR " : ""}FULL AUDIT

## KOPSAVILKUMS

- Dataset: ${label}
- Audit type: ${auditTitle}
- Model: GPT-5.6 Luna
- Audit mode: READ-ONLY
- Total objects: ${total}
- Audited objects: ${audited}
- Cards submitted to Luna: ${submittedCount || audited}
- Cards audited by Luna: ${audited}
- Missing cards: ${Math.max(0, total - audited)}
- Duplicate audited cards: ${dupCount}
- Coverage: ${coverage}
...
- MEDIUM: ${sev.MEDIUM}
- LOW: ${sev.LOW}
- DE_SOURCE_ISSUE: ${status.SOURCE_DE_ISSUE}
- STRUCTURAL_OWNER_REVIEW: ${status.NEEDS_OWNER_REVIEW}
- FALSE_POSITIVE: ${status.FALSE_POSITIVE}
- Production changes: 0
- DE changes: 0
- Other-language changes: 0
${consolidatedJson ? `- Consolidated JSON: ${consolidatedJson}` : ""}

## LUNA AUDIT BLOCKS

${lunaBlocks || "_Nav Luna bloku datu._"}
...
| ID/order | ${det.structural?.pass === false ? "FAIL" : "PASS"} |
| Mirror/parity (data/www) | ${det.layerIdentity?.identical !== false ? "PASS" : "FAIL"} |
| Syntax | ${det.meta?.jsSyntax || "PASS"} |
...
- Statuss: ${det.sectionAccents.pass === false ? "FAIL" : "PASS"}
- REAL linguistic findings: ${(det.sectionAccents.issues || []).filter((i) => i.severity && i.severity !== "INFO").length}` : "(Nav attiecināms)"}

## FOREIGN REMNANTS

${det.foreignRemnants ? `- Atradumi: ${det.foreignRemnants.issues?.length || 0}
- Statuss: ${det.foreignRemnants.pass === false ? "FAIL" : "PASS"}` : "(Nav datu)"}
...
_Pagaidu artefakti: ${tempArtifactDir}_
`;
