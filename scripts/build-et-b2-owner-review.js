...
"use strict";
/**
 * Build ET–DE B2 OWNER-PREP package per PROJECT_LANGUAGE_MASTER_STANDARD.md §7.6 / §7.10 / §7.20.
 */
const fs = require("fs");
...
const VALIDATION_JSON = path.join(ROOT, "reports/temp/et-b2-pr603-owner-history-validation.json");
const HISTORY_FILTER = process.argv.includes("--history-filtered") || process.env.OWNER_HISTORY_FILTER === "1";
const AUDIT_MD = "et-b2-full-audit.md";
...
const OUT = {
  view: path.join(ROOT, "reports/et-b2-owner-view.md"),
  decisions: path.join(ROOT, "reports/et-b2-owner-decisions.md"),
  github: path.join(ROOT, "reports/et-b2-owner-review-GITHUB.md"),
  readme: path.join(ROOT, "reports/et-b2-owner-review-README.md"),
};
...
function countDecisionRowsInFile(filePath) {
  ...
  return fs.readFileSync(filePath, "utf8").split("\n").filter((l) => l.startsWith("| ET-B2-")).length;
}
...
function countAllDecisionRows() {
  ...
  for (const name of fs.readdirSync(path.join(ROOT, "reports"))) {
    if (/^et-b2-owner-decisions-group\d+\.md$/.test(name)) {
...
function loadFindings() {
  if (!fs.existsSync(AUDIT_JSON)) {
    console.error(`Missing ${AUDIT_JSON}. Run: node scripts/run-et-b2-full-audit.js`);
    process.exit(1);
  }
  ...
  if (!fs.existsSync(VALIDATION_JSON)) {
    console.error(`Missing ${VALIDATION_JSON}. Run: node scripts/build-et-b2-pr603-owner-history-validation.js`);
    process.exit(1);
  }
  ...
  return filtered.map((f, i) => {
    ...
    const newId = `ET-B2-${String(i + 1).padStart(4, "0")}`;
    return {
      ...f,
      findingId: newId,
...
function verifyOwnerArtifactCoverage(findings) {
  ...
  if (fs.existsSync(OUT.view)) {
    for (const m of fs.readFileSync(OUT.view, "utf8").matchAll(/^## (ET-B2-\d+)/gm)) {
      viewIds.add(m[1]);
    }
  }
...
function buildView(findings) {
  cleanStaleGroupFiles("et-b2-owner-view");
  cleanStaleGroupFiles("et-b2-owner-decisions");
  const groups = [];
  ...
  groups.forEach((slice, gi) => {
    ...
    const viewName = `et-b2-owner-view-group${id}.md`;
    const decName = `et-b2-owner-decisions-group${id}.md`;
    const viewRel = `reports/${viewName}`;
    const decRel = `reports/${decName}`;
    const content = [
      `# ET–DE B2 — OWNER VIEW (grupa ${gi + 1}, ${start}–${end})`,
      "",
      ...
      "| Navigācija | Saite |",
      "|------------|-------|",
      `| GitHub indekss | [et-b2-owner-review-GITHUB.md](${ghFile("et-b2-owner-review-GITHUB.md")}) |`,
      `| OWNER VIEW (indekss) | [et-b2-owner-view.md](${ghFile("et-b2-owner-view.md")}) |`,
      `| Decisions (šī grupa) | [${decName}](${ghFile(decName)}) |`,
      `| Decisions (indekss) | [et-b2-owner-decisions.md](${ghFile("et-b2-owner-decisions.md")}) |`,
      "",
  ...
  const main = [
    ...
    "|-------|--------|",
    `| GitHub indekss | [et-b2-owner-review-GITHUB.md](${ghFile("et-b2-owner-review-GITHUB.md")}) |`,
    `| OWNER README | [et-b2-owner-review-README.md](${ghFile("et-b2-owner-review-README.md")}) |`,
    `| OWNER DECISIONS (indekss) | [et-b2-owner-decisions.md](${ghFile("et-b2-owner-decisions.md")}) |`,
    `| Audit JSON | [et-b2-full-audit.json](${ghFile("et-b2-full-audit.json")}) |`,
    "",
...
function buildDecisions(findings, groupFiles) {
  const header = [
    ...
    "|-------|--------|",
    `| GitHub indekss | [et-b2-owner-review-GITHUB.md](${ghFile("et-b2-owner-review-GITHUB.md")}) |`,
    `| OWNER VIEW | [et-b2-owner-view.md](${ghFile("et-b2-owner-view.md")}) |`,
    ...groupFiles.map(
  ...
  groupFiles.forEach((g) => {
    const groupContent = [
      `# ET–DE B2 — OWNER DECISIONS (grupa ${g.id}, ${g.start}–${g.end})`,
      "",
      `**Standard:** \`PROJECT_LANGUAGE_MASTER_STANDARD.md\` v1.9`,
      `**Audit PR:** [#${PR_NUMBER}](https://github.com/${REPO}/pull/${PR_NUMBER})`,
      "",
...
function buildReadme(findings, groupFiles) {
  ...
  const content = [
    ...
    `**Audit PR:** [#${PR_NUMBER}](https://github.com/${REPO}/pull/${PR_NUMBER})`,
    "",
    `Avots: [${AUDIT_MD}](${ghFile(AUDIT_MD)}) · [GitHub indekss](${ghFile("et-b2-owner-review-GITHUB.md")})`,
    "",
    "## Kopsavilkums",
    "",
    "| Metrika | Skaitlis |",
    ...
    "| Tips | Fails | Apraksts |",
    "|------|-------|----------|",
    `| README | [et-b2-owner-review-README.md](${ghFile("et-b2-owner-review-README.md")}) | Šis fails |`,
    `| Indekss | [et-b2-owner-review-GITHUB.md](${ghFile("et-b2-owner-review-GITHUB.md")}) | Visas saites |`,
    `| VIEW | [et-b2-owner-view.md](${ghFile("et-b2-owner-view.md")}) | Authoritative monolithic VIEW |`,
    `| DECISIONS | [et-b2-owner-decisions.md](${ghFile("et-b2-owner-decisions.md")}) | Authoritative monolithic DECISIONS |`,
    "",
    "## Grupas",
    "",
