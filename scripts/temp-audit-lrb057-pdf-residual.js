...
const NESTED_BY_LANG = {
  es: buildNestedMap(loadA1("es")),
};

const BATCH = "LRB-057";
const EXPECTED_ROWS = 5;
const EXPECTED_LABOT = 5;
const EXPECTED_NELABOT = 0;
const EXPECTED_UNIQUE_CARDS = 5;

const ES_CARD_ALIASES = {};
...
function nestedForCard(lang, cardKey) {
  ...
  const resolved = ES_CARD_ALIASES[cardKey] || cardKey;
  return (
    map[resolved] ||
    map[cardKey] ||
    map[`a1-${resolved}`] ||
...
function validateMergedCard(lang, cardKey, merged) {
  ...
  if (lang === "es") {
    if (LV_LEAK.test(allText)) failures.push({ type: "LV_LEAK_IN_ES" });
    if (ET_LEAK.test(allText)) failures.push({ type: "ET_LEAK_IN_ES" });
    if (HU_IN_IS.test(allText)) failures.push({ type: "HU_LEAK_IN_ES" });
    if (NO_IN_IS.test(allText)) failures.push({ type: "NO_LEAK_IN_ES" });
  }
  failures.push(
...
const proof = {
  batch_id: BATCH,
  classification: pass
    ? "LRB_057_OWNER_PREP_READY_FOR_LINGUISTIC_REVIEW"
    : "LRB_057_OWNER_PREP_BLOCKED",
  pdf_reaudit: true,
  ...
  es_rows: rows.filter((r) => r.languages === "es").length,
  gates: {
    ROWS: `${rows.length}/${EXPECTED_ROWS}`,
    PENDING: pending,
    LABOT: `${labot}/${EXPECTED_LABOT}`,
    NELABOT: nelabot,
    ES_ROWS: `${rows.filter((r) => r.languages === "es").length}/${EXPECTED_ROWS}`,
  ...
  productionSource: "data/es/a1.js",
  languages: { es: 50 },
