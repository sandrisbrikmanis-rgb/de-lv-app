 ...
 * pre-generated batch findings in reports/temp/da-sentences-luna/.
 * Usage: node scripts/audit-da-sentences-linguistic.js [--test-batch]
 */
const fs = require("fs");
const path = require("path");
const { ROOT } = require("./lib/audit-common");
const { auditSentencesBatch, createStats, classifyFindings } = require("./lib/openai-da-sentences-audit");

const COLLECT = path.join(ROOT, "reports/temp/da-sentences-audit-data.json");
const BATCH_DIR = path.join(ROOT, "reports/temp/da-sentences-audit-batches");
const LUNA_DIR = path.join(ROOT, "reports/temp/da-sentences-luna");
const OUT = path.join(ROOT, "reports/temp/da-sentences-linguistic-audit.json");
...
function main() {
  if (!fs.existsSync(COLLECT)) {
    throw new Error("Run audit-da-sentences-collect.js first");
  }
