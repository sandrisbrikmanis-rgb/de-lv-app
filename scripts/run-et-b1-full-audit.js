...
"use strict";
/**
 * ET-DE B1 full READ-ONLY audit orchestrator (deterministic + GPT-5.6 Luna).
 * Usage: node scripts/run-et-b1-full-audit.js [--skip-luna] [--test-luna] [--fresh-luna]
...
const {
  ...
  LUNA_JSON,
  TEMP_DIR,
} = require("./lib/et-b1-audit-helpers");
const { loadOwnerHistory, classifyWithOwnerHistory } = require("./lib/et-b1-owner-history");
...
const {
  ...
  AUDIT_RUNS,
  OWNER_SOURCES,
} = require("./lib/et-b1-discovery-config");
...
const OUT_MD = path.join(ROOT, "reports", "et-b1-full-audit.md");
const OUT_JSON = path.join(ROOT, "reports", "temp", "et-b1-full-audit.json");
const MASTER_VERSION = "1.9";
const PRODUCTION_PATH = "data/et/b1.js";
const WWW_PATH = "www/data/et/b1.js";
const TOTAL_CARDS = 3367;
const LV_STUDY_COUNT = 324;
/** ET-DE B1: no prior FINAL_CLOSED integration on origin/main (first FULL_DISCOVERY). */
...
function computeOwnerHistoryCoverage(ownerHistory, etEntries) {
  ...
  etEntries.forEach((entry, index) => {
    const id = entry.study?.id || `b1-${entry.de}-${index}`;
    cardById.set(id, entry);
    cardById.set(`b1-${entry.de}`, entry);
  });
  let checked = 0;
  let matching = 0;
  let drifted = 0;
...
function computeBaselineGate(ownerHistory) {
  ...
  if (datasetProductionSha !== wwwDatasetBlobSha) {
    return {
      ...
      deReadOnly: "PASS",
      blocked: true,
      blockReason: "data/et/b1.js blob !== www/data/et/b1.js blob",
    };
  }
  ...
  const branchPatterns = [
    "origin/cursor/et-de-b1-full-audit-4a7c",
  ];
...
function mergeFindings(detFindings, collectData, lunaData, validateStudy) {
  ...
  function addFromLvRemnant(issue) {
    all.push({
      findingId: `ET-B1-${String(seq++).padStart(4, "0")}`,
  ...
  const b1Validate = validateStudy?.perFile?.find((f) => f.file === "data/et/b1.js");
...
function buildReport(ctx) {
  ...
  const lines = [];
  lines.push("# ET–DE B1 pilns lingvistiskais audits (MASTER v1.9 FULL_DISCOVERY)");
  lines.push("");
  ...
  lines.push("|---------|---------|");
  lines.push("| Kartītes | **3367** |");
  lines.push(`| Luna coverage | **${ctx.lunaCoverage}** |`);
  if (ctx.lunaCoverage === "skipped") {
    lines.push("");
    lines.push("> **Luna (GPT-5.6) nav palaists** (`--skip-luna` vai nav `OPENAI_API_KEY`). Šis ziņojums satur deterministisko slāni. Lai pabeigtu pilnu FULL_DISCOVERY ar Luna: `node scripts/run-et-b1-full-audit.js --fresh-luna`.");
  }
...
async function main() {
  ...
  console.log("\n=== ET–DE B1 FULL_DISCOVERY — MASTER v1.9 ===\n");
  ...
  if (FRESH_LUNA) {
    const progress = path.join(ROOT, "scripts/.et-b1-luna-progress.json");
    if (fs.existsSync(progress)) fs.unlinkSync(progress);
    if (fs.existsSync(LUNA_JSON)) fs.unlinkSync(LUNA_JSON);
    if (fs.existsSync(TEMP_DIR)) fs.rmSync(TEMP_DIR, { recursive: true, force: true });
  }

  run("node scripts/audit-et-b1-collect.js");
  runCapture(["audit-language-parity.js", "--lang=et"], path.join(ROOT, "reports/temp/et-b1-parity.json"));
  const moj = spawnSync("node", [path.join(ROOT, "scripts", "audit-mojibake.js"), "--lang=et"], {
    cwd: ROOT,
    env: process.env,
    encoding: "utf8",
  });
  fs.writeFileSync(path.join(ROOT, "reports/temp/et-b1-mojibake.json"), moj.stdout || "");
  runCapture(["validate-study-design.js", "--lang=et"], path.join(ROOT, "reports/temp/et-b1-validate-study.json"));
  run("node --check data/et/b1.js");

  const collectData = loadJsonSafe(path.join(ROOT, "reports/temp/et-b1-audit-data.json"), {});
  const validateStudy = loadJsonSafe(path.join(ROOT, "reports/temp/et-b1-validate-study.json"), {});
  ...
  if (!SKIP_LUNA) {
    ...
    runNode("audit-et-b1-linguistic.js", lunaArgs);
  } else {
  ...
  const ctx = {
    ...
    lvRemnantCards,
    sectionAccentCount: b1Validate?.sectionAccentIssues || 0,
  ...
  fs.writeFileSync(path.join(ROOT, "reports/et-b1-full-audit.json"), JSON.stringify(payload, null, 2));
  fs.writeFileSync(OUT_MD, buildReport(ctx));

  if (discoveryStability.gates.ownerBacklogAllowed && ownerBacklogFinal.length > 0) {
    fs.writeFileSync(
      path.join(ROOT, "reports/temp/et-b1-full-audit.json"),
    ...
    try {
      execSync("node scripts/build-et-b1-owner-review.js", { cwd: ROOT, stdio: "inherit" });
    } catch (e) {
  ...
  if (ownerBacklogFinal.length > 0 && discoveryStability.gates.ownerBacklogAllowed) {
    runPostAuditOwnerReview("et-b1-full", { force: true });
  }
}
