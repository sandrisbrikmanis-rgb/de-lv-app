...
"use strict";
/**
 * FR-DE B1 linguistic audit with GPT-5.6 Luna (READ-ONLY).
 * Usage: node scripts/audit-fr-b1-linguistic.js [--test-batch] [--resume] [--fresh]
 */
...
const { createStats } = require("./lib/openai-fr-a1-a2-audit");
const { auditCardsBatch, classifyFindings } = require("./lib/openai-fr-b1-audit");
...
async function main() {
  ...
  if (FRESH) {
    ...
    if (fs.existsSync(path.join(__dirname, ".fr-de-b1-luna-progress.json"))) {
      fs.unlinkSync(path.join(__dirname, ".fr-de-b1-luna-progress.json"));
    }
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "fr-de-b1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME || FRESH) {
    auditData = { meta: { dataset: "fr-de-b1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }
