#!/usr/bin/env node
/**
 * Fix BS-DE C1 study.sectionAccents to match existing Bosnian Study text.
 * Only modifies sectionAccents in data/bs/c1.js (then syncs www/).
 *
 * Run: node scripts/fix-bs-c1-section-accents.js [--dry-run]
 */
const fs = require("fs");
...
const WWW_FILE = path.join(ROOT, "www", "data", "bs", "c1.js");
const REPORT_PATH = path.join(ROOT, "reports", "temp", "bs-c1-section-accents-fix-log.json");
...
function getLvTermAt(lvAccents, sectionKey, index, field, color, termIndex) {
  ...
  try {
    ...
    const arr = node[color];
    if (!Array.isArray(arr)) return null;
    return arr[termIndex] ?? null;
  } catch {
    return null;
  }
}
...
function fixColorArray({
  ...
  for (const color of ACCENT_COLORS) {
    ...
    const next = [];
    accentMap[color].forEach((rawTerm, termIndex) => {
...
function main() {
  ...
  for (let i = 0; i < bs.length; i++) {
    ...
    const cardId = bsEntry.study.id || `c1-${bsEntry.de}-${i}`;
