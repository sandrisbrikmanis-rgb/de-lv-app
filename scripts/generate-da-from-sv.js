#!/usr/bin/env node
/**
 * Generate DA-DE data from SV baseline with batched translation (SV → DA).
...
const OUT_DIR = path.join(ROOT, "data", "da");
const CACHE_PATH = path.join(ROOT, "scripts", ".da-translation-cache.json");
const BULLET = "•";
const FROM_LANG = "sv";
const TO_LANG = "da";
...
async function main() {
  ...
  const daVerbs = lvVerbs.map((lvEntry, i) => {
  ...
  writeArrayFile(path.join(OUT_DIR, "verbs.js"), "VERB_ENTRIES", daVerbs);

  const daDialogue = {};
  for (const [id, lvEntry] of Object.entries(lvDialogue)) {
    const svEntry = svDialogue[id];
    daDialogue[id] = { de: lvEntry.de, lv: map[svEntry.lv] || svEntry.lv };
  }
  fs.writeFileSync(
  ...
  for (const [key, html] of Object.entries(svCourse.COURSE_LESSON_HTML || {})) {
    daHtml[key] = await translateHtmlLesson(html, map);
  }
  const daData = applyTranslation(svCourse.COURSE_LESSON_DATA, map);
  fs.writeFileSync(
  ...
  const outLines = ["// Danish course training cards for DA-DE Kurss lessons 1-7.\n"];
  for (const [key, deck] of Object.entries(svTraining)) {
    if (!Array.isArray(deck)) continue;
    const daKey = key.replace(/Sv$/, "Da");
    const translated = deck.map((card) => ({
      front: map[card.front] || card.front,
      back: card.back || "",
    }));
    outLines.push(`window.${daKey} = ${JSON.stringify(translated, null, 2)};\n`);
  }
  if (outLines.length > 1) {
    fs.writeFileSync(path.join(OUT_DIR, "courseTrainingCards.js"), outLines.join("\n"), "utf8");
  }

  console.log("\nDA-DE data generation complete (SV→DA with LV DE preservation).");
}
