...
"use strict";
/**
 * ET-DE B2 linguistic audit with GPT-5.6 Luna (READ-ONLY).
 * Usage: node scripts/audit-et-b2-linguistic.js [--test-batch] [--resume]
 */
...
const {
  ...
  auditCardsBatch,
  classifyFindings,
} = require("./lib/openai-et-b2-audit");
...
async function main() {
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "et-b2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME) {
    auditData = { meta: { dataset: "et-b2", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }

  const { simple, study } = buildCards();
  const totalCards = simple.length + study.length;
  console.log(`ET B2 Luna audit: ${totalCards} entries (${simple.length} simple, ${study.length} study)`);
