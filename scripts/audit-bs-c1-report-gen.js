#!/usr/bin/env node
/**
 * BS-DE A1 audit report generator (read-only). Writes reports/temp/bs-c1-findings-consolidated.json
 */
const fs = require("fs");
...
const BS_FILE = path.join(ROOT, "data", "bs", "c1.js");
const AUDIT_DATA = path.join(ROOT, "reports", "temp", "bs-c1-audit-data.json");
const OUT = path.join(ROOT, "reports", "temp", "bs-c1-findings-consolidated.json");
...
function main() {
  ...
  function add(f) {
    ...
    seen.add(key);
    findings.push({ ...f, file: "data/bs/c1.js" });
  }
    ...
    walkAccents(entry.study.sectionAccents, (term, ctx) => {
      ...
      if (LV_DIACRITICS.test(term) || LV_WORDS.test(term)) {
        add({
          ...
          severity: "high",
          existingBsText: term,
          lvEtalon: "(LV term in sectionAccents — should be Bosnian)",
          deContext: entry.de,
          justification: `Latvian remnant "${term}" in sectionAccents; BS section text uses Bosnian, accents must match BS text.`,
          recommendedFix: `Replace with the exact Bosnian term from the corresponding BS study section (e.g. mali, reći, trčati, vrijeme, jednom).`,
      ...
      if (ENGLISH_ACCENT.test(term)) {
        add({
          ...
          deContext: entry.de,
          justification: `English remnant "${term}" in sectionAccents instead of Bosnian translation term.`,
          recommendedFix: `Replace with Bosnian terms from examples (e.g. na, uz, kod) matching the BS lv field text.`,
          standardPoint: "§11 sectionAccents — BS translations in accents",
        });
      }
    });
  }
  ...
  for (const iss of auditData.sectionAccents.issues) {
    ...
    const entry = bs.find((e) => e.study?.id === iss.id);
    if (!entry) continue;
    ...
    if (caseInsensitive) {
      add({
        ...
        lvEtalon: "—",
        deContext: entry.de,
        justification: `Accent term "${iss.term}" not found with case-sensitive boundary match; likely capitalization mismatch (term exists with different casing in BS text).`,
        recommendedFix: `Align accent term casing with BS section text, or adjust BS text to match accent.`,
      ...
      add({
        ...
        lvEtalon: "—",
        deContext: entry.de,
        justification: `Accent term "${iss.term}" not found in BS ${iss.section} text.`,
        recommendedFix: `Update accent term to match exact word in BS section text, or fix BS text.`,
        standardPoint: "§11 sectionAccents — term must exist in section text",
      });
    }
  }
  ...
  if (warn) {
    add({
      ...
      status: "WARNING",
      severity: "low",
      existingBsText: warn.bsMain,
      lvEtalon: warn.lvGloss,
      deContext: warn.de,
      justification: "Capitalized main translation flagged by heuristic; Božić is correct as proper noun/holiday name in Bosnian.",
      recommendedFix: "No change needed.",
      standardPoint: "§7 main translation — capitalization",
    });
  }

  // False positive: c1-liter
  ...
  const out = {
    ...
    totals: {
      entries: bs.length,
      studyCards: bs.filter((e) => e.study).length,
