 ...
 * Usage: node scripts/audit-et-b1-linguistic.js [--test-batch] [--resume]
 */
...
const {
  ...
  auditCardsBatch,
  classifyFindings,
} = require("./lib/openai-et-b1-audit");
...
async function main() {
  ...
  let auditData = fs.existsSync(LUNA_JSON)
    ...
    : { meta: { dataset: "et-b1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };

  if (!RESUME) {
    auditData = { meta: { dataset: "et-b1", startedAt: new Date().toISOString() }, batches: [], allResults: [], findings: [] };
  }

  const { simple, study } = buildCards();
  const totalCards = simple.length + study.length;
  console.log(`ET B1 Luna audit: ${totalCards} entries (${simple.length} simple, ${study.length} study)`);
