#!/usr/bin/env node
/**
 * Generate data/bs/a1.js from LV baseline (data/a1.js) using openai-translate.
...
const SOURCE_FILE = path.join(ROOT, "data", "a1.js");
const OUT_FILE = path.join(ROOT, "data", "bs", "a1.js");
const WWW_OUT_FILE = path.join(ROOT, "www", "data", "bs", "a1.js");
...
function writeA1File(filePath, data) {
...
async function main() {
  ...
  const concurrency = 8;
  console.log(`BS A1: ${unique.length} unique strings, ${pending.length} to translate via OpenAI`);
  ...
  writeA1File(OUT_FILE, translatedWords);
  fs.mkdirSync(path.dirname(WWW_OUT_FILE), { recursive: true });
  writeA1File(WWW_OUT_FILE, translatedWords);
