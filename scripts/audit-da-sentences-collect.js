...
"use strict";
/**
 * DA-DE Sätze/Teikumi full audit collector (READ-ONLY).
 * Structural + deterministic sweep + batch export for Luna linguistic audit.
 * Usage: node scripts/audit-da-sentences-collect.js
 */
const fs = require("fs");
...
const OUT = path.join(ROOT, "reports/temp/da-sentences-audit-data.json");
const BATCH_DIR = path.join(ROOT, "reports/temp/da-sentences-audit-batches");
const EXPECTED = 796;
const BATCH_SIZE = 50;
...
function main() {
  ...
  const da = loadSentences(DA_FILE);
  ...
  const data = {
    meta: {
      ...
      daFile: "data/da/sentences.js",
      wwwFile: "www/data/da/sentences.js",
  ...
  if (!mirrorPass) {
    data.structural.issues.push({ severity: "CRITICAL", problem: "data/da/sentences.js ≠ www/data/da/sentences.js" });
  }

  const countPass = deRef.length === da.length && da.length === EXPECTED;
  data.structural.countPass = countPass;
  if (!countPass) {
    data.structural.issues.push({
      severity: "CRITICAL",
      problem: `Count mismatch: DE=${deRef.length}, DA=${da.length}, expected=${EXPECTED}`,
    });
  }

  let deIntegrityPass = true;
  let idOrderPass = true;
  const daByDe = new Map();
  ...
  for (let i = 0; i < da.length; i++) {
    ...
    if (de !== entry.de) {
      ...
      idOrderPass = false;
      addFinding(data, {
        id: `DA-SENT-${String(data.findings.length + 1).padStart(4, "0")}`,
        cardId,
        index: i,
        batch,
        field: "de",
    ...
    const foreign = classifyForeign(daText);
    if (foreign.length) {
      ...
      addFinding(data, {
        ...
        deContext: entry.de,
        currentDa: daText,
        proposedDa: "(Natural Danish sentence)",
        problem: `Foreign remnant or artifact: ${foreign.join(", ")}`,
        rationale: "DA sentence must be natural Danish without foreign fragments",
        severity: sev,
        status: "LABOT",
      });
    }
    ...
    const daNeg = negationTokens(daText, "da");
    if (deNeg > 0 && daNeg === 0 && entry.de.length > 12) {
      addFinding(data, {
        ...
        deContext: entry.de,
        currentDa: daText,
        proposedDa: "(Danish with matching negation)",
    ...
    const normDa = normalizeText(daText.split("•")[0]);
    if (daByDe.has(normDa)) {
      const other = daByDe.get(normDa);
      if (other.de !== entry.de) {
        addFinding(data, {
          ...
          deContext: entry.de,
          currentDa: daText,
          proposedDa: "(Context-specific Danish for this DE sentence)",
      ...
      daByDe.set(normDa, { index: i, de: entry.de });
    }
  ...
  for (let i = 0; i < da.length; i++) {
    if (JSON.stringify(da[i]) !== JSON.stringify(www[i])) {
  ...
  data.findings.forEach((f, idx) => {
    if (!f.id) f.id = `DA-SENT-${String(idx + 1).padStart(4, "0")}`;
  });
