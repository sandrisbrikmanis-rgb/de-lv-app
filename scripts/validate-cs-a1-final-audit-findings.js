...
const PARTIAL_STUDY_GAP = new Set(["a1-bitte", "a1-bitte-study", "a1-ein", "a1-es"]);
...
function loadRawFindings(tempDir) {
  ...
  const add = (f, source) => {
    n += 1;
    raw.push({
      findingId: `FINAL-A1-${String(n).padStart(5, "0")}`,
      cardId: f.cardId,
      index: f.index,
...
async function main() {
  ...
  console.log("CS-DE A1 FINAL AUDIT — FINDING VALIDATION");
  const { raw, det, ling } = loadRawFindings(paths.tempDir);
  ...
  const payload = {
    meta: {
      model: DEFAULT_MODEL,
      auditType: "FINAL POST-REPAIR VALIDATION",
